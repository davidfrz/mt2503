...
static kal_uint32 pcmsink_WriteSilenceToDsp(kal_bool all_silence, kal_uint32 word_cnt)
{
    volatile uint16 *pIdmaPtr;
...
#if defined(__BT_SERIAL_PLAYBACK_AUDIO_SYNC__)
static kal_uint32 pcmsink_bt_serial_PutData(AudComHdlInt *pIntCompHandle, kal_bool fMono2Stereo)
{
    kal_uint32 uSrcByte, uSrcWord, uUsedWord, uRealDataCount=0;
    kal_uint32 uTotalPutWord=0;
    kal_uint8 *pSrcBuf;
    kal_bool fFillData = PcmSinkStatic.uPcmFunction & PCM_FUNC_AUDIO;
    kal_uint32 duration;
    
    if (fFillData)
    {
        kal_uint32 uSilenceWord, uDropWord;
        
        uDropWord = BT_Serial_Sync_Get_Silence_Count();
        ...
        while(1)
        {
            ...
            if (uSrcWord == 0)
            {
                kal_uint32 uDspCurrentSampleCount = pcmsink_DspGetDataCount();
                
                if (uDspCurrentSampleCount < PcmSinkStatic.uSoundFrameSamples)
                {
                    uSilenceWord = pcmsink_WriteSilenceToDsp(KAL_FALSE, PcmSinkStatic.uSoundFrameSamples - uDspCurrentSampleCount);
                }
                else
                {
                    uSilenceWord = 0;
                ...
                uDropWord += uSilenceWord;
                
                uUsedWord = 0;
                
                if (uSilenceWord > 0)
                {
                    kal_brief_trace(TRACE_GROUP_BT_AUD_SYNC, BT_AUD_SYNC_PCMSINK_PUT_SILENCE, uSilenceWord);
                }
            }
            else
            {
                ...
                if (uDropWord > 0)
                {
                    if (uDropWord >= uSrcWord)
                    {
                        kal_brief_trace(TRACE_GROUP_BT_AUD_SYNC, BT_AUD_SYNC_PCMSINK_DROP_REMAINED, uSrcWord, uDropWord - uSrcWord);
                    ...
                    {
                        kal_brief_trace(TRACE_GROUP_BT_AUD_SYNC, BT_AUD_SYNC_PCMSINK_DROP_DONE, uDropWord, uSrcWord - uDropWord);
                        
                        uUsedWord = uDropWord + pcmsink_WriteDataToDsp((kal_uint16 *)(pSrcBuf + (uDropWord << 1)), uSrcWord - uDropWord, fMono2Stereo, BT_Serial_Sync_Get_Channel_Type());
                        uDropWord = 0;
                    }
                }
                else
                {
                    uUsedWord = pcmsink_WriteDataToDsp((kal_uint16 *)pSrcBuf, uSrcWord, fMono2Stereo, BT_Serial_Sync_Get_Channel_Type());
                }
            }
            
            uTotalPutWord += uUsedWord + uSilenceWord;
        ...
        BT_Serial_Sync_Set_Silence_Count(uDropWord);
        
        uRealDataCount = uTotalPutWord;
    }
    else
    {   // fill silence
        pcmsink_WriteSilenceToDsp(KAL_TRUE, 0);
    }
...
static kal_uint32 pcmsink_PutData(AudComHdlInt *pIntCompHandle, kal_bool fMono2Stereo)
{
   ...
   {
        uTotalPutWord = pcmsink_WriteSilenceToDsp(KAL_TRUE, 0);
   }
...
static void pcmsink_Hisr_AudioData()
{
   ...
   if(PcmSinkStatic.uState == PCMSINK_STATE_RUNNING)
   {
        ...
        {
            uMovedToDspWord = pcmsink_bt_serial_PutData(pIntCompHandle, (PcmSink.uChannelNumber==1));
        }
        else
#endif  // defined(__BT_SERIAL_PLAYBACK_AUDIO_SYNC__)
        {
            uMovedToDspWord = pcmsink_PutData(pIntCompHandle, (PcmSink.uChannelNumber==1));
        }
