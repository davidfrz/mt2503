...
#if defined(__TINY_SYS__)
	MMI_BOOL is_hiber_mode;  //for tiny system, if the mode is hibernation mode, then not treat as exception mode
#endif	
...
#if defined(__TINY_SYS__)
			g_srv_bootup_cntx.is_hiber_mode = ((AP_check_boot_up_source() == HIBERNATION_EXCEPTION) || (AP_check_boot_up_source() == WARM_BOOT)) ? MMI_TRUE : MMI_FALSE;
#endif
            break;
...
#if defined(__TINY_SYS__)
MMI_BOOL srv_bootup_is_hibernation_mode(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    return (g_srv_bootup_cntx.is_hiber_mode);
