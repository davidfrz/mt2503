...
//#define GT9XX_FW_STRING L"C:\\GT9XX\\fwupd.bin"
#define GT9XX_FW_STRING  "ctp_firmware_rusklmeoxkwjadfjnklruo3"

#define GT9XX_SET_INT_PIN(output) DclGPIO_Control(int_handle, (output?GPIO_CMD_WRITE_HIGH:GPIO_CMD_WRITE_LOW), NULL);

kal_bool gt9xx_i2c_read(kal_uint8 id, kal_uint16 reg, kal_uint8 *data, kal_uint16 len)
{
    kal_bool ret;
...
kal_bool gt9xx_i2c_write(kal_uint8 id, kal_uint16 reg, kal_uint8 *data, kal_uint16 len)
{
    kal_bool ret;
...
static kal_uint16 get_chip_version( kal_uint16 sw_ver )
{
    if ( sw_ver < TPD_CHIP_VERSION_C_FIRMWARE_BASE )
        return TPD_GT9XX_VERSION_B;
    else if ( sw_ver < TPD_CHIP_VERSION_D_FIRMWARE_BASE )
        return TPD_GT9XX_VERSION_C;
    else
        return TPD_GT9XX_VERSION_D;
}
...
static kal_uint8 gt9xx_nvram_store( void )
{
    kal_uint8 ret;
    int i = 0;
        
do_store:        
    ret = gt9xx_i2c_read( guitar_i2c_address, REG_NVRCS, inbuf, 1 );
    ...
    ret = gt9xx_i2c_write( guitar_i2c_address, REG_NVRCS, outbuf, 1 );
...
static kal_uint8 gt9xx_nvram_recall( void )
{
    kal_uint8 ret;
...
static kal_uint8 gt9xx_reset( void )
{
    kal_uint8 ret = 1;
...
search_i2c:
    //gt9xx_i2c_write( guitar_i2c_address, 0x00FF, outbuf, 1 );

    //kal_sleep_task( 12 );

    gt9xx_i2c_read( guitar_i2c_address, 0x00FF, inbuf, 1 );
...
static kal_uint8 gt9xx_set_address_2( kal_uint8 addr )
{
    kal_uint8 in;
    kal_int32 i;
    ...
    for ( i = 0 ; i < 12 ; i++ )
    {
        if ( gt9xx_i2c_read( addr, REG_ANY, &in, 1 ) )
        {
...
static kal_uint8 gt9xx_update_firmware( kal_uint8 *nvram, kal_uint16 length )
{
    ...
    do
    {
        ...
        //ret = gt9xx_i2c_write( guitar_i2c_address, NVRAM_UPDATE_START_ADDR, nvram, length );
        ret = gt9xx_i2c_write( guitar_i2c_address, NVRAM_UPDATE_START_ADDR, nvram, length );	
...
/* under construction !*/
    ...
    for ( cur_frame_num = 0 ; cur_frame_num < total_frame_num ; cur_frame_num++ )        // read out all the code
    {
        ...
        {
            err = 0;
            ret = gt9xx_i2c_read( guitar_i2c_address, cur_code_addr, inbuf, 1);
...
static kal_uint8 gt9xx_update_proc( kal_uint8 *nvram, kal_uint16 length )
{
    kal_uint8 ret;
    kal_uint8 error = 0;
    ctp_info_t ctp_info;
    ...
    ret = gt9xx_set_address_2( guitar_i2c_address );
    ...
    ret = gt9xx_update_firmware( nvram, length );
    ...
    while ( gt9xx_i2c_read( guitar_i2c_address, TPD_VERSION_INFO_REG, (kal_uint8 *)&ctp_info, sizeof( ctp_info_t ) ) == 0 ) ;

    //while ( gt9xx_i2c_write( guitar_i2c_address, TPD_CONFIG_REG_BASE, cfg_data, sizeof(cfg_data)) == 0 );
    
    return 1;
}

static kal_int32 gt9xx_downloader_probe( kal_uint16 cur_ver, kal_uint8 *firmware_ptr )
{
	  //TODO: process file
    //struct file *fp;
    kal_uint32 rlen;	
    ...
    kal_wsprintf( CTP_FIRMWARE_PATH, "%c:\\%s", drv_letter, GT9XX_FW_STRING );
    ...
    while ( gt9xx_update_proc( data_ptr, length ) == 0 && retry < 2 ) retry++;
...
kal_int32 init_gt9xx_download_module( kal_uint16 cur_ver, kal_uint8 *firmware_ptr )
{
    kal_int32 ret = 0;
    ...
    ret = gt9xx_downloader_probe( cur_ver, firmware_ptr );
