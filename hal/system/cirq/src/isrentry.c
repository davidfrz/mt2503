...
#include "csci.h"
CSCI_DEFINE_VAR(kal_uint64,unknown_irq_from_tiny)
CSCI_DEFINE_VAR(kal_uint64,unknown_eint_from_tiny)
CSCI_DEFINE_VAR(kal_uint64,unknown_irq_sens_from_tiny)	 /* record the	sensitve setting before trigger sw interrupt */
CSCI_DEFINE_VAR(kal_uint64,unknown_eint_sens_from_tiny)  /* record the	sensitve setting before trigger sw interrupt */

extern void IRQSensitivity_internal(kal_uint8 code, kal_bool edge);
...
/* under construction !*/
   ...
   {
   		IRQSensitivity_internal(tp_irq_code,(~(unknown_irq_sens_from_tiny >> tp_irq_code)) & 0x01); /* use "~" ,because API is  opposite with spec*/
