...
kal_uint8 isSimCardRdy(kal_uint32 SIM_index)
{
	usim_dcb_struct *usim_dcb;
	Sim_Card *SimCard;

	SimCard = GET_SIM_CB(SIM_index);	
	usim_dcb = GET_USIM_CB(SIM_index);

	sprintf(sim_dbg_str,"isSimCardRdy :%d phy_proto:%d  usim_dcb->main_state:%d SimCard->State:%d",SIM_index,usim_dcb->phy_proto,usim_dcb->main_state
		,SimCard->State);
	...
	else
	{
		...
		if (usim_dcb->main_state==MAIN_CMD_READY_STATE ||usim_dcb->main_state==CMD_TX_STATE
		||usim_dcb->main_state==CMD_RX_BLOCK_REC_STATE ||usim_dcb->main_state==CMD_RX_STATE
		||usim_dcb->main_state==CLK_STOPPING_STATE ||usim_dcb->main_state==CLK_STOPPED_STATE)
		{
			return 1;
		}
		else
...
static void SIM_EINT_HISR_RM1(void)
{
   ...
   sim_HW_cb *simHwCb = (sim_HW_cb *)(hwCbArray[sim_get_logicalNum_from_app(regedHotSwapApp)]);
	...
	sprintf(sim_dbg_str,"plug out handler:0x%x  plug in handler:0x%x  ",simHwCb->simHotPlugOut,simHwCb->simHotPlugIn);
	kal_print(sim_dbg_str);	
		

   if (simHwCb->polarity == simHwCb->Defaultpolarity)
   {
	sprintf(sim_dbg_str,"(1)if:%d:%d default polarity:%d polarity:%d eint:%d deb:%d ,remove SIM",simHwCb->simInterface,regedHotSwapApp,simHwCb->Defaultpolarity
		,simHwCb->polarity,simHwCb->eint,simHwCb->debounceTime);
	kal_print(sim_dbg_str);
	...
	if (simHwCb->simHotPlugOut != NULL)
	{
		...
		kal_print("hot swap plug out call back exist");	
	}
	else
	{
		kal_print("hot swap plug out call back does not exist");	
	}
   ...
   {
	sprintf(sim_dbg_str,"(1)if:%d:%d default polarity:%d polarity:%d eint:%d deb:%d  ,insert SIM",simHwCb->simInterface,regedHotSwapApp,simHwCb->Defaultpolarity
		,simHwCb->polarity,simHwCb->eint,simHwCb->debounceTime);
	kal_print(sim_dbg_str);
	...
	if (simHwCb->simHotPlugIn != NULL)
	{
		...
		kal_print("hot swap plug in call back exist");	
	}
	else
	{
		kal_print("hot swap plug out call back does not exist");	
	...
	simHwCb->polarity = !simHwCb->polarity;
	EINT_SW_Debounce_Modify(simHwCb->eint,simHwCb->debounceTime);// 32768Hz, set debounce time	   
	EINT_Set_Polarity(simHwCb->eint, simHwCb->polarity);
   	EINT_UnMask((kal_uint8)simHwCb->eint);
                      ...
                      DCL_SIM_PLUG_OUT_CALLBACK hotPlugOutCb)
{
   ...
   simHwCb->simHotPlugOut = hotPlugOutCb;
	sprintf(sim_dbg_str,"%x, %x ,SIM_RegHotPlugCb",hotPlugInCb,hotPlugOutCb);
	kal_print(sim_dbg_str);   
		...
		switch(value & 0xf0)
		{
			case 0x10:
			   /* Get polarity setting for REMOVE */
				...
				kal_sprintf(sim_dbg_str,"[SIM_DRV-0x10]EINT INFO:%x, EINTNo:%d, Pol:%d, GPIOSP:%x, simIf;%x, value:%x",SIM_PRINT_INFO_BASE, simHwCb->eint, simHwCb->polarity, SIM_Reg(0x80020220), simInterface, value);
				kal_print(sim_dbg_str);
			...
			case 0x20:
			   /* Get polarity setting for REMOVE */
				...
				kal_sprintf(sim_dbg_str,"[SIM_DRV-0x20]EINT INFO:%x, EINTNo:%d, Pol:%d, GPIOSP:%x, simIf;%x, value:%x",SIM_PRINT_INFO_BASE, simHwCb->eint, simHwCb->polarity, SIM_Reg(0x80020220), simInterface, value);
				kal_print(sim_dbg_str);
