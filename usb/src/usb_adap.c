...
void USB_Boot_Trace_Init(void)
{
	...
	//TINY system use DTR to sync with catcher tool. It prevents data lost!
	while((g_UsbACM[0].line_state & CONTROL_LINE_SIGNAL_DTR) != CONTROL_LINE_SIGNAL_DTR)
	{
		USB_HISR();
	}
#endif
...
extern kal_uint8 usb_pdn_handle;
kal_uint8 USB_Query_L1SM_Handle(void)
{
	return usb_pdn_handle;
}
