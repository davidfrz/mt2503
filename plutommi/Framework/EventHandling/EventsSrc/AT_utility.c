...
extern void srv_bt_noti_data_ut_handle(U8  index, kal_uint8 *string);
#endif
...
#define TOUCH_PEN_EVENT_TRACK_DISPLAY_AND_PREDICT
	...
	extern void touch_track_config(U8 index, kal_uint8 *string);
#endif
...
#ifdef __MMI_BLE_ANCS_SUPPORT__ 
    const char *btancs = "ANCS_";
#endif

#ifdef __MMI_HF_SUPPORT__
    const char *btcall = "BTD_";
#endif

#ifdef __MMI_ONE_KEY_SOS_SUPPORT__
    const char *oksos = "OKSOS_";
#endif /* __MMI_ONE_KEY_SOS_SUPPORT__ */
...
#ifdef __MMI_BT_NOTI_SRV__
    const char *btnotify = "BTNOTIFY";
#endif

#ifdef TOUCH_PEN_EVENT_TRACK_DISPLAY_AND_PREDICT
	const char *touch_track = "TOUCH_TRACK_";
#endif
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
...
#ifdef __MMI_BLE_DOGP_SUPPORT__
    if (strncmp((const kal_char*)inject_str->string, "SOGP", strlen("SOGP")) == 0)
    {
        sogps_ut_handle(inject_str->index, (kal_uint8*)inject_str->string);
    }
#endif

#ifdef __MMI_BLE_BAS_SUPPORT__
    if (strncmp((const kal_char*)inject_str->string, "BAS", strlen("BAS")) == 0)
    {
        bas_ut_handle(inject_str->index, (kal_uint8*)inject_str->string);
    }
#endif

#ifdef __MMI_BT_NOTI_SRV__
    if (strncmp((const kal_char*)inject_str->string, btnotify, strlen(btnotify)) == 0)
    {
        srv_bt_noti_data_ut_handle(inject_str->index, (kal_uint8 *)inject_str->string);
    }
#endif


#ifdef __MMI_BLE_ANCS_SUPPORT__ 
    if (strncmp((const kal_char*)inject_str->string, btancs, strlen(btancs)) == 0)
    {
        srv_ancs_ut_handle(inject_str->index, (kal_uint8 *)inject_str->string);
    }
#endif

#ifdef __MMI_HF_SUPPORT__
    if (strncmp((const kal_char*)inject_str->string, btcall, strlen(btcall)) == 0)
    {
        srv_ucm_bt_proc_inject_string(inject_str->index, (kal_uint8 *)inject_str->string);
    }
#endif /* __MMI_HF_SUPPORT__ */

#ifdef __MMI_ONE_KEY_SOS_SUPPORT__
    if (strncmp((const kal_char*)inject_str->string, oksos, strlen(oksos)) == 0)
    {
        srv_oksos_ut_handle(inject_str->index, (kal_uint8 *)inject_str->string);
    }
#endif
...
#ifdef WEARABLE_UT
//wearable
        kal_prompt_trace(MOD_MMI, "[WEARABLE_UT] Starting....");
        if (strncmp((const kal_char*)inject_str->string, wearable_move_tune, strlen(wearable_move_tune)) == 0)
        {
            mmi_frm_touch_screen_wearable_ut(inject_str->index, (kal_uint8 *)inject_str->string);
        }
        kal_prompt_trace(MOD_MMI, "[WEARABLE_UT] Finished.");  

		kal_prompt_trace(MOD_MMI, "[LIST_SSP_UT] Starting....");
		if (strncmp((const kal_char*)inject_str->string, list_ssp_tune, strlen(list_ssp_tune)) == 0)
		{
			gui_list_ssp_ut(inject_str->index, (kal_uint8 *)inject_str->string);
		}
		kal_prompt_trace(MOD_MMI, "[LIST_SSP_UT] Finished.");  
#endif
...
#ifdef TOUCH_PEN_EVENT_TRACK_DISPLAY_AND_PREDICT 
    if (strncmp((const kal_char*)inject_str->string, touch_track, strlen(touch_track)) == 0)
    {
        touch_track_config(inject_str->index, (kal_uint8 *)inject_str->string);
    }
#endif
