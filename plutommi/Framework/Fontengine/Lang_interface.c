...
U16 mmi_get_language_index_by_lang_code(S8 *lang_code)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if (lang_code == NULL)
    {
        return 0xFFFF;
				...
				MMI_PRINT(MOD_MMI_COMMON_APP,TRACE_GROUP_4,"mmi_get_language_index_by_lang_code");

    for (idx = 0; idx < gMaxDeployedLangs; idx++)
    {
        memcpy(LangCode, gLanguageArray[idx].aLangCountryCode, 5);
				LangCode[2] = '_' ;
        LangCode[5] = '\0';

				MMI_PRINT(MOD_MMI_COMMON_APP,TRACE_GROUP_4,"[language]auto sync,Lang_code = %s", LangCode);
				MMI_PRINT(MOD_MMI_COMMON_APP,TRACE_GROUP_4,"[language]auto sync,lang_code = %s", lang_code);


        if (strncmp(lang_code, (S8*)LangCode, 5) == 0 && gLanguageMappingArray[idx])
