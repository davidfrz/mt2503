...
//huking
	...
	gui_fill_rectangle(x1, y1, x2, y2, f->c); //draw full screen pop_up background
#else	
...
//				history_buffer);
		...
		#ifdef __MMI_WEARABLE_DEVICE__
		dm_data.s32flags |= DM_SHOW_SWATCH_SOFTKEY;
		wgui_softkey_set_is_in_popup(MMI_TRUE); //set the softkey is in popup
		#endif /*__MMI_WEARABLE_DEVICE__*/
	}
...
#ifdef __MMI_WEARABLE_DEVICE__
static UI_string_type g_str_cat6333_appname;
static UI_string_type g_str_cat6333_datetime;

void ExitCategory6333Screen(void)
{
	g_str_cat6333_datetime = NULL;
	g_str_cat6333_appname = NULL;
	wgui_softkey_set_is_in_popup(MMI_FALSE);
	//reset_small_screen();
}
static void DrawCat6333ControlArea(dm_coordinates *coordinate)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
   // U16 str_appname[] = L"App name...";
	...
	S32 offset_x,offset_y;
	//S32 item_gap;
    S32 message_width, message_height;
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
   offset_x = coordinate->s16X;
   ...
   gdi_layer_push_and_set_clip(
    ...
    /*gdi_draw_solid_rect(
        ...
        coordinate->s16X + coordinate->s16Width - 1,
        coordinate->s16Y + coordinate->s16Height - 1, 
        gdi_act_color_from_rgb(255, 52, 149, 165));*/
   ...
   //show app_name
   gui_measure_string((UI_string_type) g_str_cat6333_appname, &str_width, &str_height);
	if (r2lMMIFlag)
	{
		gui_move_text_cursor(offset_x + str_width, offset_y);
		gui_print_truncated_text(offset_x + str_width, offset_y, coordinate->s16Width, (UI_string_type) g_str_cat6333_appname);
	}
	else
	{
		gui_move_text_cursor(offset_x, offset_y);
		gui_print_truncated_text(offset_x, offset_y, coordinate->s16Width, (UI_string_type) g_str_cat6333_appname);
	}

	
   	//show pop up contents
   	offset_y = offset_y + CAT6333_APPNAME_AREA_HEIGHT;
	MMI_multiline_inputbox.text = (UI_buffer_type)MMI_message_string;
    message_width = coordinate->s16Width;
	message_height = CAT6333_INPUTBOX_HEIGHT;

	wgui_inputs_ml_resize_fit(message_width, CAT6333_INPUTBOX_HEIGHT, &message_width, &message_height);
   // item_gap = (coordinate->s16Height - message_height - progress_height - text_height) >> 2;

	if (message_height > CAT6333_INPUTBOX_HEIGHT)
	{
		message_height = CAT6333_INPUTBOX_HEIGHT;
	}
	offset_y = offset_y + (CAT6333_INPUTBOX_HEIGHT - message_height) / 2;
    wgui_inputs_ml_move(((UI_device_width - coordinate->s16Width)>>1), offset_y);
    ...
    gui_measure_string((UI_string_type) g_str_cat6333_datetime, &str_width, &str_height);//str_datetime
    
	offset_y = coordinate->s16Y + CAT6333_APPNAME_AREA_HEIGHT + CAT6333_INPUTBOX_HEIGHT + CAT6333_INPUTBOX_TIME_GAP;

	if (r2lMMIFlag)
	{
		gui_move_text_cursor(offset_x + str_width, offset_y);
		gui_print_truncated_text(offset_x + str_width, offset_y, coordinate->s16Width, (UI_string_type) g_str_cat6333_datetime);
	}
	else
	{
		gui_move_text_cursor(offset_x, offset_y);
		gui_print_truncated_text(offset_x, offset_y, coordinate->s16Width, (UI_string_type) g_str_cat6333_datetime);
	}
	gdi_layer_pop_clip();
}
void wgui_showcategory6333_screen(
				UI_string_type str_appname,
				UI_string_type left_softkey,
                UI_string_type right_softkey,
				UI_string_type str_message,
				UI_string_type str_datetime)
{
	...
	g_str_cat6333_appname = str_appname;

    g_str_cat6333_datetime = str_datetime;
	...
	dm_add_line(line_color);
	dm_add_line(line_color);*/

	if (NULL == str_appname && NULL == str_datetime)
	{
		MMI_multiline_inputbox.flags |= UI_MULTI_LINE_INPUT_BOX_CENTER_JUSTIFY;
	}
	else
    ...
    dm_register_category_controlled_callback(DrawCat6333ControlArea);
	ExitCategoryFunction = ExitCategory6333Screen;
    dm_setup_category_functions(dm_redraw_category_screen, dummy_get_history, dummy_get_history_size);
    dm_data.s32ScrId = (S32) GetActiveScreenId();
    dm_data.s32CatId = MMI_CAT6333_ID;

    dm_data.s32flags = DM_CLEAR_SCREEN_BACKGROUND | DM_SHOW_SWATCH_SOFTKEY | DM_SET_NEW_LAYER_ACTIVE;
	wgui_softkey_set_is_in_popup(MMI_TRUE);
...
UI_string_type wgui_cat6335_msg;
U8 wgui_cat6335_percentage;
/*****************************************************************************
 * FUNCTION
 *  wgui_cat6335_exit
 * DESCRIPTION
 *  Exits the category6335 screen
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
static void wgui_cat6335_exit(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
/*****************************************************************************
 * FUNCTION
 *  wgui_cat6335_show_string
 * DESCRIPTION
 *  Displays the category6335 screen string
 * PARAMETERS
 *  index           [IN]        Index of string in current screen
 *  buf             [IN]        String buffer
 *  x               [IN]        x position
 ...
 *  void
 *****************************************************************************/
static void wgui_cat6335_show_string(
        U32 index, 
        UI_string_type buf, 
        S32 x, S32 y, S32 width, S32 height)
{
...
/*****************************************************************************
 * FUNCTION
 *  wgui_cat6335_show_progress
 * DESCRIPTION
 *  draw 6335 progress bar
 * PARAMETERS
 *  
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_cat6335_show_progress(
	S32 x, 
	S32 y, 
	S32 width, 
	S32 height, 
	U8 percentage)
{
    ...
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    S32 percentage_width = (width - CAT6335_PROGRESS_BG_OFFSET * 2 ) * percentage/100;
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/   
    ...
    gdi_draw_solid_rect(
        x + CAT6335_PROGRESS_BG_OFFSET, 
        y + CAT6335_PROGRESS_BG_OFFSET, 
        x + CAT6335_PROGRESS_BG_OFFSET + percentage_width - 1, 
        y + CAT6335_PROGRESS_BG_OFFSET +(height - CAT6335_PROGRESS_BG_OFFSET * 2) - 1, 
        gdi_act_color_from_rgb(255, 52, 149, 165));

}
/*****************************************************************************
 * FUNCTION
 *  wgui_cat6335_draw_ctrl_area
 * DESCRIPTION
 *  Redraw category control area of category6335 screen
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
static void wgui_cat6335_draw_ctrl_area(dm_coordinates *coordinate)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
   // U16 buffer[16];
    S32 text_area_height;
    //S32 y_center = coordinate->s16Y;
   
    S32 progress_height = CAT6335_PROGRESS_HEIGHT + CAT6335_PROGRESS_BG_OFFSET * 2;
    S32 progress_width = coordinate->s16Width;
    S32 progress_x = coordinate->s16X;
...
/* under construction !*/
   ...
   // kal_wsprintf((kal_wchar*)buffer, "%d", wgui_cat6335_percentage);
   // kal_wstrcat((kal_wchar*)buffer, L"%");

    gui_set_font(&MMI_medium_font);
	text_area_height = coordinate->s16Height - CAT6335_PROGRESS_HEIGHT - CAT6335_PROGRESS_BG_OFFSET * 2 - CAT6335_TEXT_PROGRESS_AREA_GAP; 
    ...
    wgui_cat6335_show_string(
                0, 
                wgui_cat6335_msg, 
                coordinate->s16X, coordinate->s16Y, coordinate->s16Width, text_area_height);  

     /* Draw progress bar */
     wgui_cat6335_show_progress(
                progress_x, 
                coordinate->s16Y + text_area_height + CAT6335_TEXT_PROGRESS_AREA_GAP, 
                progress_width, 
                progress_height, 
                wgui_cat6335_percentage);
...
/*****************************************************************************
 * FUNCTION
 *  wgui_cat6335_update_all
 * DESCRIPTION
 *  Displays the category6335 screen
 * PARAMETERS
 *  message                 [IN]        Message string
 *  sub_message             [IN]        Message sub string
 *  percentage              [IN]        Percentage of progress Ex:80 => 80%
        ...
        U8 percentage)
{
    ...
    wgui_cat6335_msg = message;
    wgui_cat6335_percentage = percentage;

	//bg_c = wgui_pop_up_dialog_background.c;
	...
	/*gdi_draw_solid_rect(
            ...
            coordinate.s16X + coordinate.s16Width - 1,
            coordinate.s16Y + coordinate.s16Height - 1,
            gdi_act_color_from_rgb(UI_translate_gui_alpha_to_gdi_alpha(bg_c.alpha), bg_c.r, bg_c.g, bg_c.b));*/
    gui_fill_rectangle(coordinate.s16X, coordinate.s16Y, coordinate.s16X + coordinate.s16Width - 1,\
    				coordinate.s16Y + coordinate.s16Height - 1, wgui_pop_up_dialog_background.c);

    wgui_cat6335_draw_ctrl_area(&coordinate);
 ...
 *  wgui_cat6335_show
 * DESCRIPTION
 *  Displays the category6335 screen, do not support Hisotry, small progress screeen
 * PARAMETERS
 *  left_softkey            [IN]        Left softkey label
 *  right_softkey           [IN]        Right softkey label
 *  message                 [IN]        Message string
 ...
 *  void
 *****************************************************************************/
void wgui_showcategory6335_screen(
        U16 left_softkey,
        U16 right_softkey,
        WCHAR* message,
        U8 percentage)
{
	...
	//add line
	if ((NULL != left_softkey) || (NULL != right_softkey))
	{
		line_color.r = 255;
		line_color.g = 255;
		line_color.b = 255;
    ...
    catId = MMI_CAT6335_ID;
    
    wgui_cat6335_msg = message;
    wgui_cat6335_percentage = percentage;

    dm_register_category_controlled_callback(wgui_cat6335_draw_ctrl_area);
    
    gui_unlock_double_buffer();
	flags |= DM_NO_TITLE | DM_NO_STATUS_BAR | DM_SHOW_SWATCH_SOFTKEY | DM_SET_NEW_LAYER_ACTIVE;
	wgui_softkey_set_is_in_popup(MMI_TRUE);
    wgui_cat_setup_category(
        catId,
        flags,
