...
srv_apmode_config_struct g_srv_apmode_config = {0};
srv_apmode_config_struct *g_srv_apmode_config_ptr = NULL;
...
#ifdef __IOT__
void srv_apmode_get_mac_address(void *mac_addr);
static void srv_apmode_get_common_settings(srv_tethering_wifi_hs_settings_struct *settings);
static void srv_apmode_abm_send_activate_req();
static void srv_apmode_abm_send_deactivate_req();
...
void srv_tethering_init(void)
{
    ...
    memset(&srv_tethering_cntx, 0, sizeof(srv_tethering_cntx_struct));
        ...
        void *user_data)
{
    ...
    #ifndef __WIFI_AP_STA_CONCURRENCY__
...
#ifdef __IOT__
    if (tethering_type == SRV_TETHERING_TYPE_WIFI_AP)
    {
        U32 i;
        S32 result = SRV_APMODE_FAILED;
        ...
        {
            result = SRV_APMODE_OK;
        }
        
        if (srv_tethering_cntx.apmode_switch_on_callback.funcptr != NULL)
        {
            (*srv_tethering_cntx.apmode_switch_on_callback.funcptr)(
                result,
                srv_tethering_cntx.apmode_switch_on_callback.user_data);
            srv_tethering_cntx.apmode_switch_on_callback.funcptr = NULL;
        }
        return;
    }
#endif
...
void srv_tethering_abm_deactivate_cnf_hdlr(void *msg)
{
    ...
    if (tethering_type == SRV_TETHERING_TYPE_WIFI_AP)
    {
        ...
        if (srv_tethering_cntx.apmode_switch_off_callback.funcptr != NULL)
        {
            (*srv_tethering_cntx.apmode_switch_off_callback.funcptr)(
                result,
                srv_tethering_cntx.apmode_switch_off_callback.user_data);
            srv_tethering_cntx.apmode_switch_off_callback.funcptr = NULL;
        }
        return;
    }
#endif
...
cbm_bearer_enum srv_tethering_type_srv_to_ps(srv_tethering_type_enum tethering_type)
{
    ...
    switch(tethering_type)
    {
        ...
        case SRV_TETHERING_TYPE_WIFI_AP:
            return CBM_AP_MODE;
...
srv_tethering_type_enum srv_tethering_type_ps_to_srv(cbm_bearer_enum bearer_type)
{
    ...
    switch(bearer_type)
    {
        ...
        case CBM_AP_MODE:
            return SRV_TETHERING_TYPE_WIFI_AP;
...
static void srv_apmode_get_common_settings(srv_tethering_wifi_hs_settings_struct *settings)
{
    ...
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    srv_apmode_config_struct nvram_record = {0};
    U8 mac_addr[6];
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    ...
    MMI_ASSERT(NVRAM_EF_SRV_TETHERING_WIFI_HS_MAX_PSK_LEN == SRV_TETHERING_HOSTAP_PSK_MAX_BUF_LEN);
    //write to nvram
    srv_apmode_config_get_para(&nvram_record);
    
    if (strlen((CHAR *)&(nvram_record.ssid)) == 0)
    {
        srv_apmode_config_struct new_config;
        
        srv_apmode_get_mac_address(&mac_addr);
        sprintf((CHAR *)&(settings->ssid), "MTK_%02x%02x%02x", mac_addr[3], mac_addr[4], mac_addr[5]);

        memcpy((CHAR *)&(new_config.ssid), (CHAR *)&(settings->ssid), SRV_TETHERING_HOSTAP_SSID_MAX_BUF_LEN);
        srv_apmode_config_set_para(&new_config, SRV_CONFIG_APMODE_PARA_SSID);
    }
    else
    {
        memcpy((CHAR *)&(settings->ssid), (CHAR *)&(nvram_record.ssid), SRV_TETHERING_HOSTAP_SSID_MAX_BUF_LEN);
    }
...
static void srv_apmode_abm_send_activate_req()
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_CONN_TRC_G10_TETHERING, TRC_SRV_TETHERING_SEND_ACTIVATE_REQ, SRV_TETHERING_TYPE_WIFI_AP);

    srv_tethering_set_status(SRV_TETHERING_TYPE_WIFI_AP, SRV_TETHERING_STATUS_ON_SWITCHING_ON);

    activate_req = OslAllocDataPtr(mmi_abm_share_bearer_activate_req_struct);
    
    activate_req->share_bearer = CBM_AP_MODE;
 
    srv_apmode_get_common_settings(&wifi_hs_settings);
    // ssid
    activate_req->hostap_config_profile.ssid_len = strlen((CHAR*)wifi_hs_settings.ssid);
    MMI_ASSERT(activate_req->hostap_config_profile.ssid_len <= HOSTAP_SSID_MAX_LEN);
    memcpy(
...
static void srv_apmode_abm_send_deactivate_req()
{
    ...
    MMI_TRACE(
        MMI_CONN_TRC_G10_TETHERING,
        TRC_SRV_TETHERING_DEACTIVATE_REQ,
        SRV_TETHERING_TYPE_WIFI_AP);

    srv_tethering_set_status(SRV_TETHERING_TYPE_WIFI_AP, SRV_TETHERING_STATUS_ON_SWITCHING_OFF);
    
    deactivate_req = OslAllocDataPtr(mmi_abm_share_bearer_deactivate_req_struct);
    deactivate_req->share_bearer = srv_tethering_type_srv_to_ps(SRV_TETHERING_TYPE_WIFI_AP);
...
MMI_BOOL srv_apmode_is_on(void)
{
    srv_tethering_status_enum status = srv_tethering_get_status(SRV_TETHERING_TYPE_WIFI_AP);
                ...
                void *user_data)
{
    ...
    if(status != SRV_TETHERING_STATUS_SRV_OFF)
    {
        S32 result = SRV_APMODE_OK;
        if (status != SRV_TETHERING_STATUS_SRV_ON)
        {
            result = SRV_APMODE_BUSY;
        }
    ...
    {
        srv_tethering_cntx.apmode_switch_on_callback.funcptr = funcptr;
        srv_tethering_cntx.apmode_switch_on_callback.user_data = user_data; 
        // send msg
        srv_apmode_abm_send_activate_req();
    }
}


void srv_apmode_switch_off(
                srv_apmode_switch_cb_funcptr_type funcptr,
                void *user_data)
{
    /*----------------------------------------------------------------*/
    ...
    {
        srv_tethering_cntx.apmode_switch_off_callback.funcptr = funcptr;
        srv_tethering_cntx.apmode_switch_off_callback.user_data = user_data;
        srv_apmode_abm_send_deactivate_req();
    }
}


MMI_BOOL srv_apmode_config_set_para(
                    ...
                    srv_apmode_config_para_enum flag)
{
    ...
    /*----------------------------------------------------------------*/
    S16 error = 1;
    srv_apmode_config_struct temp = {0};
    ...
    if(NULL == g_srv_apmode_config_ptr)
    {
        ReadRecord(
        ...
        if(error >= 0)
        {        
            g_srv_apmode_config_ptr = &g_srv_apmode_config;
        }
        else
        {
            return KAL_FALSE;
    ...
    memcpy(&temp, g_srv_apmode_config_ptr, sizeof(srv_apmode_config_struct));
    

    if(flag & SRV_CONFIG_APMODE_PARA_AUTH_MODE)
    {        
        temp.auth_mode= para->auth_mode;
    }
    
    if(flag & SRV_CONFIG_APMODE_PARA_SSID)
    {
        memcpy(temp.ssid, para->ssid, SRV_CONFIG_AP_MODE_MAX_SSID_LEN); 
    }

    if(flag & SRV_CONFIG_APMODE_PARA_PSK)
    {        
        memcpy(temp.psk, para->psk, SRV_CONFIG_AP_MODE_MAX_PSK_LEN); 
    }  
    ...
    if(error >= 0)
    {   
        memcpy(g_srv_apmode_config_ptr, &temp, sizeof(srv_apmode_config_struct));
    }
    else
    {
        return KAL_FALSE;
...
MMI_BOOL srv_apmode_config_get_para(
                    srv_apmode_config_struct *para)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    ...
    memcpy(para, g_srv_apmode_config_ptr, sizeof(srv_apmode_config_struct));
    return KAL_TRUE;
}
