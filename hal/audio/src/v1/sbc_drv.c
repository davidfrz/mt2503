...
const bt_a2dp_aac_codec_cap_struct aac_cap[1] = {
   {
...
void SBC_Resample_Set(kal_bool on)
{
   if(on)
   {
   	  //kal_prompt_trace(MOD_L1SP,"ori SR=%d, sbc SR=%d",sbc.frame_state.sampling_frequency,sbc.src_sample_rate);
      if(sbc.frame_state.sampling_frequency != sbc.src_sample_rate)
      {
      	 kal_uint32 workBufSize = 0;
      	 BLI_GetMemSize(sbc.src_sample_rate, sbc.frame_state.channels, sbc.frame_state.sampling_frequency, sbc.frame_state.channels, &workBufSize);
      	 sbc.bli_workBufPtr = (char *)audio_alloc_mem_cacheable( workBufSize * sizeof(kal_int8));
      	 ASSERT(sbc.bli_workBufPtr != NULL);
      	 sbc.bli_handle = BLI_Open(sbc.src_sample_rate, sbc.frame_state.channels, sbc.frame_state.sampling_frequency, sbc.frame_state.channels, sbc.bli_workBufPtr, audio_alloc_ext_mem_cacheable_simplified);
         sbc.upsample_rate = ( sbc.frame_state.sampling_frequency + (sbc.src_sample_rate/2)) / sbc.src_sample_rate; 
      }
   }
   else
   { 
   	  if (sbc.bli_handle != NULL)
   	  {
   	  	 BLI_Close(sbc.bli_handle, audio_free_ext_mem_simplified);  
   	  	 sbc.bli_handle = NULL;
   	  	 audio_free_mem( (void **) &sbc.bli_workBufPtr);      
   	  	 sbc.bli_workBufPtr = NULL;
   	  	 sbc.upsample_rate = 1;
   	  }
   }
}
...
{    
   kal_uint32 savedMask;
   kal_uint32 uLoopCnt=0, NUM_FRAME_DATA=0;
   ...
   while (uLoopCnt++<2)
   {
      ...
      data_count = (uInDataByte >> 1);
      pInBuf = (kal_int16 *)pPcmBuffer;
      
      while (data_count)
      {
         kal_uint32 uSubbandBufFreeSize = MCU_MIN_SB_BUFFER_SIZE - sbc.subband_buffer_write;
         ...
         if(sbc.bli_handle) {
            kal_uint32 uBliInByte, uBliOutByte, uConsumedInputByte;
            kal_int16 *pBuf16 = sbc.subband_buffer + sbc.subband_buffer_write;
            
            uBliInByte = data_count * 2;
            uBliOutByte = uSubbandBufFreeSize * 2;
            
            // Bli convert
            uConsumedInputByte = BLI_Convert(sbc.bli_handle, pInBuf, &uBliInByte, pBuf16, &uBliOutByte);            
            //kal_prompt_trace(MOD_L1SP,"[1]uBliInByte =%d  uBliOutByte=%d, uConsumed=%d", uBliInByte, uBliOutByte, uConsumedInputByte);
            
            // Update pointer
            sbc.subband_buffer_write += (uBliOutByte >> 1);
            pInBuf += (uConsumedInputByte >> 1);
            data_count -= (uConsumedInputByte >> 1);
            usedLen += uConsumedInputByte;
            
            if((data_count<2) && (!uConsumedInputByte))
            {
            	 //I/P data not enough
            	 break;
            }        
            ...
            if (uSubbandBufFreeSize > data_count) {
               uSubbandBufFreeSize = data_count;
            }
            // Fill Data
            memcpy(sbc.subband_buffer+sbc.subband_buffer_write, pInBuf, uSubbandBufFreeSize*sizeof(kal_int16));
            //kal_prompt_trace(MOD_L1SP,"uSubbandBufFreeSize=%d", uSubbandBufFreeSize);
            // Update pointer
            sbc.subband_buffer_write += uSubbandBufFreeSize;
            pInBuf += uSubbandBufFreeSize;
            data_count -= uSubbandBufFreeSize;
            usedLen += (uSubbandBufFreeSize << 1);
         }
         
         // When subband buffer is full, execute SBC encode
         if (sbc.subband_buffer_write == MCU_MIN_SB_BUFFER_SIZE)
         {
      ...
      rbReadDataDone(&sbc.PCM, usedLen);
   }
