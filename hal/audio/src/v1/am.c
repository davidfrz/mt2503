...
static void AM_WriteSidetoneFIR( void )
{
#if defined(MT6260) || defined(MT6261) || defined(MT2501) || defined(MT2502)
    {
        ...
        volatile uint16 *dsp_ptr = SAL_Sidetone_GetFltCoefAddr();
        for (I = SAL_FLTCOEFLEN_SIDETONE; I > 0; I--)
        {
            *dsp_ptr++ = *ptr++;
        }
    }
...
static void AM_WriteSpeechCompensationFilter( const int16 in_coeff[45], const int16 out_coeff[45] )
{
    ...
    {
        int i;
        volatile uint16 *dsp_ptr = SAL_BKF_GetFltCoefAddr_NB_UL();
    ...
    {
        int i;
        volatile uint16 *dsp_ptr = SAL_BKF_GetFltCoefAddr_NB_DL();
        for (i = 0; i < SAL_FLTCOEFLEN_BKF_NB; i++)
        {
#ifndef  __HQA_AUDIO__
            *dsp_ptr ++ = *out_coeff++;
...
#if defined(MT6260) || defined(MT6261) || defined(MT2501) || defined(MT2502)
const signed short Audio_Flt_Coeff[82] =DEFAULT_AUDIO_FLT_COEFF;
...
static void AM_WriteAntiAliasFilter( void )
{
    ...
    {   //SRC FIR coeff
        ...
        volatile uint16 *dsp_ptr = SAL_SRC_GetFltCoefAddr();
        for (I = SAL_FLTCOEFLEN_SRC; I > 0; I--)
        {
            *dsp_ptr++ = *ptr++;
        }
    }
    ...
    {   //AGC HP IIR coeff
        ...
        volatile uint16 *dsp_ptr = SAL_AGC_GetFltCoefAddr();
        for (I = SAL_FLTCOEFLEN_AGC; I > 0; I--)
        {
            *dsp_ptr++ = *ptr++;
        }
    }
...
const kal_uint16 AGC_DSP_Tables[47] = DEFAULT_AGC_DSP_TABLES;

#else
const int16 AGC_DSP_Parameters[20]=DEFAULT_AGC_DSP_PARAMETERS;
...
/* under construction !*/
            ...
            )
    {
        ...
        AM_DSP_SetSRCCtrl(sp_getSpeFlag());
        am_set_cod_band(IsWBLink(am.speech_mode));
        SAL_3G_Call_Open(am.speech_mode, am.speech_mode, SP3G_IsDTXOn(), DSP_PCM_IDLE_DELAY_TABLE[6][0], DSP_PCM_IDLE_DELAY_TABLE[6][1], DSP_PCM_IDLE_DELAY_TABLE[6][2]);
    }
#endif
...
{   
   ...
   AM_Force_VBI_END_IN_TASK();
   AM_Enqueue(AM_DSP_SpeechOn, rat_mode, (rat_mode == RAT_2G_MODE));
...
static void AM_DSP_SpeechOff( uint32 data )
{
   ...
   if( (am.speech_state & SP_STATE_VMR) && (data&1)==0 )            /* call drop while conversation recording */
      AM_DSP_VMRecordOff( 0 );

#if defined(MT6260) || defined(MT6261) || defined(MT2501) || defined(MT2502)
    am.state &= ~AM_STATE_SPEECH;
    if ((am.state == 0) || ((data&1)!=0))
    {
        VBI_End();
    }
...
/**
	@info: bit[0]: need to foce dsp8k off or not; 1 for force 0 for note
*/
void AM_SpeechOff( uint32 info )
{
   ...
   AM_Enqueue( AM_DSP_SpeechOff, info, false );
...
void AM_DSP_InterRAT_G2W( uint32 data )
{
   if( !(am.state & AM_STATE_SPEECH) )
    ...
    SAL_2G_Call_Close();
    am.speech_mode = data;
    SAL_3G_Upd_Enc_Cod(data);//for 3g codec internal, MT6280
    SAL_3G_Upd_Dec_Cod(data);//for 3g codec internal, MT6280
    SAL_3G_Call_Open(data, data, SP3G_IsDTXOn(), 0, 0, DSP_PCM_IDLE_DELAY_TABLE[6][2]);
    am.sp3g_flag = true;
#else   // chip compile option
   //*DP_SC_FLAGS &= 0xF800;
   *DP_SC_MUTE &= ~0x0002;
...
/**
	@data: [bit 0] enhance is on/off: 1 for on, 0 for off. 
*/
...
void AM_DSP_SetSRCCtrl(uint32 data)
{
    ...
    switch (sph_mode)
    {
		...
		if(isFweOn) {
// #if defined(_FWE_SUPPORT_) // nees to consider background sound
			v = SIDETONE_BIT + UL_SRC_DS + LIMITER_INIT;   // This setting is only for MT6250; FixedWBEnhancement only is applied to DL
#if defined(__DUAL_MIC_SUPPORT__)
	            v = isWBLink ? SIDETONE_BIT : SIDETONE_BIT | DL_SRC_US | UL_SRC_DS | UL2_SRC_DS;
#endif 
		} else  {
//#else // signle mic without FWE 
			v = isWBLink ? SIDETONE_BIT : SIDETONE_BIT | DL_SRC_US | UL_SRC_DS;
// #endif      
		}

#ifdef DSDA_VIA_PCM_INTERFACE
		*DSP_SPH_8K_CTRL |= 0x400; // fix wb 
#endif  //DSDA_VIA_PCM_INTERFACE
...
// #if defined(_FWE_SUPPORT_)
#ifdef DSDA_VIA_PCM_INTERFACE
			if(AFE61M_getVoiceSamplingRate()==1) { // 16k
				v = isWBLink ? 0 : UL_SRC_DS;
			} else 
#endif
			{ // 8k 
				v = DL_SRC_DS;
...
//#endif // _FWE_SUPPORT_
      ...
      //      break;
...
#ifdef DSDA_VIA_PCM_INTERFACE
		if(AFE61M_getVoiceSamplingRate()==0) { // 8k 
		...
		if(isFweOn) {
// #if defined(_FWE_SUPPORT_)
			v = SIDETONE_BIT + UL_SRC_DS + LIMITER_INIT;
		} else {
//#else
			v = isWBLink ? SIDETONE_BIT : SIDETONE_BIT | DL_SRC_US | UL_SRC_DS;
//#endif
...
{
    AM_Enqueue(AM_DSP_SetSRCCtrl, sp_getSpeFlag(), true);
    AM_FlushQFunction();
}
#endif  // chip compile option
...
void AM_AudioPlaybackOn( uint16 asp_type, uint16 asp_fs )
{
   ...
   if(SBC_IsOn())
   {
#if __SBC_ARBITARY_SRC__
      SBC_Resample_Set(KAL_TRUE);
#endif   	
      mhdlHookPcmRut();
   }
#endif
...
#if defined(MT6255) || defined(MT6250) || defined(MT6260) || defined(MT6261) || defined(MT2501) || defined(MT2502)
#ifdef DSDA_VIA_PCM_INTERFACE
	// leave the setting  AM_DSP_SetSRCCtrl()
#else
#if defined(_FWE_SUPPORT_)  
            *DP_SCR_CTRL = 0x100;
#else
   	      *DP_SCR_CTRL = 0x4;
#endif // defined(_FWE_SUPPORT_)
#endif // DSDA_VIA_PCM_INTERFACE
#endif
         }
#endif
...
#if defined(MT6260) || defined(MT6261) || defined(MT2501) || defined(MT2502)
static void am_set_pcm_ex_config (Sal_PCMEx_t *p_cfg, PCM_Ex_Config *p_config, kal_bool idle, kal_bool swi_on, Sal_PcmEx_Type_t type_t, Sal_PcmBand_Config_t band_t)
{
    p_cfg->idle   = idle;
    p_cfg->swi_on = swi_on;
...
void AM_DSP_PCM_EX_On(uint32 type)
{
    ...
    Sal_PcmEx_Type_t     type_t = SAL_PCMEX_TYPE_DACA;
    Sal_PcmBand_Config_t band_t = p_config->bandInfo;
    kal_bool idle = false;
...
{   
    ...
    PCM_Ex_Config *p_config = (PCM_Ex_Config *)type;
    kal_uint32 am_type = p_config->am_type;
    Sal_PcmEx_Type_t type_t = SAL_PCMEX_TYPE_PNW;
    kal_bool idle = false;
    ...
    {
        Sal_PCMEx_t cfg;
        am_set_pcm_ex_config (&cfg, p_config, idle, false, type_t, SAL_PCM_NARROWBAND);
        SAL_PcmEx_Config(&cfg);
    }    
...
static void AM_DSP_BabyMonitor_RecordOn( kal_uint32 codec_mode )
{
   ...
   if (!(am.state & AM_STATE_SPEECH)) {
      SAL_3G_Call_Open(am.bm_enc_mode, am.bm_dec_mode, KAL_FALSE, DSP_PCM_IDLE_DELAY_TABLE[6][0], DSP_PCM_IDLE_DELAY_TABLE[6][1], DSP_PCM_IDLE_DELAY_TABLE[6][2]);
   }
