...
# Constants
#****************************************************************************
my $LISINFO_VERNO     = " v0.23 -> p0.25 -> p0.27 -> p0.28 -> p0.29";
                        # p0.29,  2014/08/25,  Ubin is between ROM and VIVA new layout for IOTFOTA 
                        # p0.28 , 2014/08/11,  support update rom layout for IOTFOTA
                        # p0.27 , 2014/07/02,  Adjust viva check base in CheckROMSize() and GetVIVAMaxAllowSize() if differential fota enabled 
                        # p0.25 , 2014/05/19,  Support Tiny System AAPMC
...
sub ParseLIS
{
    ...
    if(defined $g_LISPath and -e $g_LISPath)
    {
        ...
        while (<FILE_HANDLE>) 
        {
            ...
            elsif (/^\s+(Load|Image)\$\$(\w+)(\$\$)?(RO|RW|ZI)?\$\$(Base|Limit|Length)\s+(0x\w+)\s+Number\s+0\s+anon\$\$obj\.o\s*(\S+)/)
            {#    Image$$EXTSRAM_DSP_RX$$Length            0x00000000   Number         0  anon$$obj.o ABSOLUTE
             #    Image$$EXTSRAM_DSP_TX$$ZI$$Base          0x003d4400   Number         0  anon$$obj.o(linker$$defined$$symbols)
                #$1=(Load|Image), $2= region name, $3=($$),  $4=(RO|RW|ZI), $5=(Base|Limit|Length), $6=(0x\w+)
                my $symbol;
                $symbol= ($3 and $4)? ($1."\$\$".$2."\$\$".$4."\$\$".$5): ($1."\$\$".$2."\$\$".$5);
                $g_linkerSymbols{$symbol} = $6;
                #print "\t $symbol=$g_linkerSymbols{$symbol}\n";
            }
...
#****************************************************************************
# subroutine:  GetLinkerSymbolInfo - Get linker symbol info
# input:       $strSymbolName: Linker symbol name (Case sensitive)
# output:     value of linker symbol
#****************************************************************************
sub GetLinkerSymbolInfo
{
    my ($strSymbolName) = (@_);
    return $g_linkerSymbols{$strSymbolName};
}
...
#****************************************************************************
# should be used with scatInfo.pm
# subroutine:  ts_ClassifyRegionType - For TSReservedSize_AutoConfig.pl to know Region Type
# input:       Makefile Hash Reference
# output:      x
#****************************************************************************
sub ts_ClassifyRegionType
{
    ...
    $g_scatInfo_ref = LIS_new scatInfo(\%g_LoadRegion, $g_LoadRegionCount, \%g_LoadRegionLookUpTable,
                                        \%g_ExeRegion,  $g_ExeRegionCount,  \%g_ExeRegionLookUpTable,
                                        \%g_RegionGroup,\%g_RegionMap);
    $g_scatInfo_ref->ts_ClassifyRegionType();
}
...
#****************************************************************************
# should be used with scatInfo.pm
# subroutine:  ts_GetDumpExeRegions - Get dump regions for TSReservedSize_AutoConfig.pl
# input:       Makefile Hash Reference
# output:      Dump regions' reference
#****************************************************************************
...
{    
    ...
    if($g_scatInfo_ref)
    {
        $hash_ref = $g_scatInfo_ref->ts_GetDumpExeRegions();
    }
    return $hash_ref;
}
...
sub CheckROMSize
{
    ...
    elsif(defined $g_LoadRegionLookUpTable{'DIFFENFOTA_STOR'})
    {
        my $nIndex_DIFFENFOTA = $g_LoadRegionLookUpTable{'DIFFENFOTA_STOR'};
        $check_base = &sysUtil::query_cs1_addr($bb, $g_LoadRegion{$nIndex_DIFFENFOTA}->[1], 0);
    }
    return ($compRegion_end, $check_base);
}
...
sub GetVIVAMaxAllowSize
{
    ...
    my $is_Ubin = $Makefile_href->{'UPDATE_BINARY_FILE'};
    ...
    {
        my $nIndex_DIFFENFOTA = $g_LoadRegionLookUpTable{'DIFFENFOTA_STOR'};
        $nBottomToTop_base = &sysUtil::query_cs1_addr($bb, $g_LoadRegion{$nIndex_DIFFENFOTA}->[1], 0);
    }
    else
    {
    ...
    if(!defined $mba or $mba ne 'TRUE') #if MBA is disabled, the last region before VIVA is ROM region
    {
        ...
        #if ubin is enabled, the free ROM is divided into two parts: ROM <->UPDATE_ROM & VIVA <-> available ROM end(nBottomToTop_base)
        if(defined $is_Ubin and $is_Ubin eq 'TRUE')
        {
            my $nIndex_UBROM = $g_LoadRegionLookUpTable{'UPDATE_ROM'};
            my $nIndex_VIVA = $g_LoadRegionLookUpTable{'VIVA'};

            my $nVIVA_base = &sysUtil::query_cs1_addr($bb, $g_LoadRegion{$nIndex_VIVA}->[1], 0);
            my $nUB_base = &sysUtil::query_cs1_addr($bb, $g_LoadRegion{$nIndex_UBROM}->[1], 0);

            $nMaxAllowSize = ($nBottomToTop_base - $nVIVA_base) + ($nUB_base - ($nROM_base + $nROM_size));
        }
        else
        {
    ...
    else #if MBA is enabled, the free ROM is divided into two parts: CUST/LANG <-> JUMP_TABLE & VIVA <-> available ROM end(nBottomToTop_base)
    {
        my ($nMBA_base, $nMBA_size) = (0, 0);
        my $nJUMP_TABLE_base = 0;        
        my $nIndex_MBA;
        ...
        my $nIndex_VIVA = $g_LoadRegionLookUpTable{'VIVA'};
        my $nVIVA_base = &sysUtil::query_cs1_addr($bb, $g_LoadRegion{$nIndex_VIVA}->[1], 0);
        $nMaxAllowSize = ($nBottomToTop_base - $nVIVA_base) + ($nJUMP_TABLE_base - ($nMBA_base + $nMBA_size));    
    }
