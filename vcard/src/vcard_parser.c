...
maplib_time_struct MapMyTime = {0};
#endif


//add function for FOTA feature;
kal_wchar *map_ucs2_wcscat(kal_wchar *str_dst, const kal_wchar *str_src)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/    
    ASSERT(!((kal_uint32)str_src & 0x01));
    ASSERT(!((kal_uint32)str_dst & 0x01));
	while(*str_tmp)
    {
...
kal_wchar *map_ucs2_wcscpy(kal_wchar *str_dst, const kal_wchar *str_src)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
kal_int32 map_ucs2_wcslen(const kal_wchar *string)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
kal_uint8 maplib_dt_dow(kal_uint16 y, kal_uint8 m, kal_uint8 d)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
#ifndef __MTK_TARGET__
void maplib_dt_get_date_time(maplib_time_struct *t)//for modis;
{
...
/* under construction !*/
    ...
    t->DayIndex = MapMyTime.DayIndex;
    t->nDay = MapMyTime.nDay;
    t->nHour = MapMyTime.nHour;
    t->nMin = MapMyTime.nMin;
    t->nSec = MapMyTime.nSec;
    t->nMonth = MapMyTime.nMonth;
    t->nYear = MapMyTime.nYear;

    t->DayIndex = maplib_dt_dow(t->nYear, t->nMonth, t->nDay);
#else /* EXTERNAL_MMI */
    memset(t, 0, sizeof(maplib_time_struct));
#endif
}
#endif


void maplib_dt_get_rtc_time(maplib_time_struct *t)
{
#ifdef __MTK_TARGET__
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
...
void vcard_init_object(vcard_parser_struct *parser, vcard_property_enum property)
{
    ...
    if (property & VCARD_PROPERTY_PHOTO)
    {
        ...
        while (type_mask)
        {
            ...
            if (parser->mode & VCARD_PARSE_MODE_TEL_ALWAYS)
            {
                if (parser->object->tel[parser->tel_count].tel)
                {
                    ...
                    map_ucs2_wcscpy(parser->object->tel[parser->tel_count].tel, tel.tel);
            ...
            else
            {
                ...
                if (tel.type == VCARD_TEL_TYPE_CELL && parser->object->tel[0].tel && !parser->object->tel[0].tel[0])
                {
                    ...
                    map_ucs2_wcscpy(parser->object->tel[0].tel, tel.tel);
                ...
                else if (tel.type != VCARD_TEL_TYPE_VOICE)
                {
                    for (i = 1; i < VCARD_MAX_TEL_COUNT; i++)
                    {
                        if (parser->object->tel[i].tel && !parser->object->tel[i].tel[0])
                        {
                            ...
                            map_ucs2_wcscpy(parser->object->tel[i].tel, tel.tel);
...
void vcard_decode_photo(vcard_parser_struct *parser, kal_bool end_of_photo)
{
    ...
    if (parser->photo == -1 && parser->object)
    {
        ...
        maplib_time_struct local_time;

	 #ifndef __MTK_TARGET__
        applib_dt_get_rtc_time(&local_time);//update bin API,only for modis;
     #else
        maplib_dt_get_rtc_time(&local_time);
	 #endif
        ...
        if (parser->param_offset > parser->name_offset)
        {
            ...
            if (strstr(parser->line_buffer + parser->name_offset, "JPG"))
            {
                ...
                map_ucs2_wcscat(file_name, L".JPG");
            ...
            else if (strstr(parser->line_buffer + parser->name_offset, "JPEG"))
            {
                ...
                map_ucs2_wcscat(file_name, L".JPEG");
            ...
            if (strstr(parser->line_buffer + parser->name_offset, "GIF"))
            {
                ...
                map_ucs2_wcscat(file_name, L".GIF");
                ...
                !strstr(parser->line_buffer + parser->name_offset, "WBMP"))
            {
                ...
                map_ucs2_wcscat(file_name, L".BMP");
            ...
            else if (strstr(parser->line_buffer + parser->name_offset, "WBMP"))
            {
                ...
                map_ucs2_wcscat(file_name, L".WBMP");
                ...
                !strstr(parser->line_buffer + parser->name_offset, "WBMP"))
            {
                ...
                map_ucs2_wcscat(file_name, L".WBM");
            ...
            else if (strstr(parser->line_buffer + parser->name_offset, "PNG"))
            {
                ...
                map_ucs2_wcscat(file_name, L".PNG");
            ...
            else if (strstr(parser->line_buffer + parser->name_offset, "M3D"))
            {
                ...
                map_ucs2_wcscat(file_name, L".M3D");
            ...
            else if (strstr(parser->line_buffer + parser->name_offset, "SVG"))
            {
                ...
                map_ucs2_wcscat(file_name, L".SVG");
            ...
            offset = map_ucs2_wcslen(parser->object->photo.file_name);
			//app_ucs2_wcscat(parser->object->photo.file_name, file_name);		
			map_ucs2_wcscat(parser->object->photo.file_name, file_name);
                ...
                if (parser->indent == 0)
                {
                    if (parser->property & VCARD_PROPERTY_N)
                    {
                            ...
                            parser->object->n.name && parser->object->n.name[0])
                        {
                            ...
                            map_ucs2_wcscpy(parser->object->n.given, parser->object->n.name);
                        }
                    }
...
vcard_parser_struct *vcard_malloc_parser(FS_HANDLE source, vcard_property_enum property)
{
    ...
    if (property & VCARD_PROPERTY_PHOTO)
    {
        parser->object->photo.file_name = (kal_wchar*) get_ctrl_buffer((VCARD_MAX_PATH_LENGTH + 1) * sizeof(kal_wchar));
    }
...
void vcard_free_parser(vcard_parser_struct *parser)
{
	...
	if (parser->property & VCARD_PROPERTY_PHOTO)
    {
        free_ctrl_buffer(parser->object->photo.file_name);
    }
