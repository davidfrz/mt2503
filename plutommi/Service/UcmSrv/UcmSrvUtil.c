 ...
 *****************************************************************************/
MMI_BOOL srv_ucm_is_alert_outgoing_call()
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    S32 call_count;
    U32 i;
    srv_ucm_index_info_struct index[SRV_UCM_MAX_CALL];
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    call_count = srv_ucm_query_call_count(SRV_UCM_OUTGOING_STATE, SRV_UCM_CALL_TYPE_ALL, index);

    for (i = 0; i < call_count; i++)
    {
        if (g_ucm_srv_p->call_list.group_info[index[i].group_index].call_info[index[i].call_index].is_alert)
        {
            return MMI_TRUE;
        }
    }
...
extern MMI_BOOL srv_ucm_query_call_data(srv_ucm_index_info_struct index, srv_ucm_call_info_struct *call_info)
{
    ...
    #ifdef __MMI_HFP_SUPPORT__
        call_info->call_origin = g_ucm_srv_p->call_list.group_info[group_index].call_info[call_index].call_origin;
    #endif /* __MMI_HFP_SUPPORT__ */
        return MMI_TRUE;
    }
