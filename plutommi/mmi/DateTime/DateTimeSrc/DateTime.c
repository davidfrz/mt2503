...
void mmi_dt_set_dt(const MYTIME *time, mmi_dt_set_time_res_funcptr callback, void* usr_data)
{
	...
	if(MMI_UPDATE_DATETIME_NONE == update_datetime_mode)
		{
			level = SRV_UTC_DT_SYNC_LEVEL1;
		}
	else
		{ 
			level = SRV_UTC_DT_SYNC_LEVEL2;
		}
	//mmi_dt_set_rtc_dt((MYTIME *)time);
		 srv_dt_set_dt(level,
                (applib_time_struct*)time,
                g_mmi_dt_cntx.res_callback,
                g_mmi_dt_cntx.usr_data);
}
...
void mmi_dt_set_rtc_dt(MYTIME *t)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
		if (!applib_dt_is_valid((applib_time_struct*)t))
		{
			return;
		}
