...
VMINT vm_mms_send_mms(vm_send_mms_req_t * req_ptr,  vm_mms_callback_type callback)
{
    ...
    if(!srv_mre_appmgr_check_current_defined_authorization(SRV_MRE_APPMGR_APP_AUTH_MMS_SEND))
    {
        MMI_TRACE(MMI_MRE_TRC_MOD_VMMMS, TRC_MRE_MMS_SEND_FAIL);
		    return FALSE;
    }
    else
    {
        MMI_TRACE(MMI_MRE_TRC_MOD_VMMMS, TRC_MRE_MMS_SEND_SUCCESS);
    }
#endif
...
#ifdef __MMI_MRE_AUTH_SUPPORT__
    if(!srv_mre_appmgr_check_current_defined_authorization(SRV_MRE_APPMGR_APP_AUTH_MMS_READ))
    {
        MMI_TRACE(MMI_MRE_TRC_MOD_VMMMS, TRC_MRE_MMS_READ_FAIL);
		    return -1;
    }
    else
    {
        MMI_TRACE(MMI_MRE_TRC_MOD_VMMMS, TRC_MRE_MMS_READ_SUCCESS);
    }
#endif
    req.msg_id = msg_id;
    req.sim_id = (U8)sim;
