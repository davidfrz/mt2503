...
void gpio_strapping_pin_lowpower_set(void)
{
	...
	for(i = 0; i < GPIO_STRAPPING_PIN_NUMBER; i++)
	{
		GPIO_ModeSetup(strapping_pin_gpio_array[i],STRAPPING_PIN_MODE_SET_VALUE);
		GPIO_InitIO(STRAPPING_PIN_DIR_SET_VALUE,strapping_pin_gpio_array[i]);
		if(GPIO_SPECIAL_OUTPUT_STRAPPING_PIN != strapping_pin_gpio_array[i]){
			GPIO_WriteIO(STRAPPING_PIN_DOUT_SET_VALUE,strapping_pin_gpio_array[i]);}
		else
		{
