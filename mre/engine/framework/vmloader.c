...
extern VMUINT init_address ;
extern VMUINT init_count ;
                                    ...
                                    vm_elf_section_head_t *sh_tab, VMINT sh_num)
{
    ...
    if (sh != NULL) {
		...
		init_address = (VMUINT)context->sysv_node.sysv_init_array;
		init_count = context->sysv_node.sysv_init_count;
    }
...
VMINT vm_read_preload_app_info(vm_preload_app_info_t *preload_info, const VMWSTR fullFileName)
{
    ...
    #if !defined(__IOT__) || defined(__DISPLAY_LEVEL_GDI_ONLY__)//ndef __IOT__// !defined(__IOT__) || defined(__DISPLAY_LEVEL_GDI_ONLY__)//snowwang
    bufSize = 4;
    if (vm_get_vm_tag(fullFileName, VM_CE_INFO_NO_SCREEN, &preload_info->isNoScreen, &bufSize) != GET_TAG_TRUE) {
        preload_info->isNoScreen = 0;
    }
    #else
    preload_info->isNoScreen = 1;
    #endif
    bufSize = 4;
	...
	bufSize = 24;
	if (vm_get_vm_tag(fullFileName, VM_CE_INFO_ISSUE, (void *)tmp_buf, &bufSize) == GET_TAG_TRUE)
	{
	    preload_info->build_id = (tmp_buf[4] << 24) + (tmp_buf[8] << 16) + (tmp_buf[12] << 8) + tmp_buf[16];
	}
	else
	{
	    preload_info->build_id = 0;
	}
    ...
    if (!preload_info->isMREmem)
    {
        ...
        if(p_context->rwSize % 4 == 0)
			preload_info->backupSize = p_context->rwSize;
		else if(p_context->rwSize % 4 == 1)
			preload_info->backupSize = p_context->rwSize+3;
		else if(p_context->rwSize % 4 == 2)
			preload_info->backupSize = p_context->rwSize+2;
		else
			preload_info->backupSize = p_context->rwSize+1;
    }        
#ifndef MRE_ON_MODIS
    if (is_ads) {
