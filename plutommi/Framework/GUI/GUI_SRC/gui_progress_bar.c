...
#ifdef __MMI_WEARABLE_DEVICE_UI_ENHANCE__
void gui_watch_progress_bar_create(
        gui_watch_progress_bar_struct *instance,
        U16 x,
        U16 y,
        U16 max_value,
        U16 level_value)
{
	...
	instance->level_value = level_value;	
}

void gui_watch_progress_bar_draw(gui_watch_progress_bar_struct instance)
{
	/*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
	...
	//base_layer= dm_get_scr_bg_layer();
	...
	gui_measure_image((PU8)get_image(WGUI_PROGRESS_LEVEL_IMAGE_ID), &level_image_width, &level_image_height);
	//draw level image 
	step_size = level_image_width / instance.max_value;
	if (instance.max_value != instance.level_value)
	{
		draw_width = instance.level_value * step_size;
	}
	else
	{
		draw_width = level_image_width;
	}
	gdi_image_draw_resized_id(instance.x,
            instance.y, draw_width, level_image_height, WGUI_PROGRESS_LEVEL_IMAGE_ID);
 ...
 *  gui_volume_bar_draw
 * DESCRIPTION
 *  Creates the volume bar control
 * PARAMETERS
 *  instance        [IN]        Instance of volume bar 
 * RETURNS
 *  void
 *****************************************************************************/
void gui_volume_bar_draw(gui_volume_bar_struct instance)
{
	/*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
	gdi_handle active_layer;
	gdi_handle alpha;
	S32 volume_bar_end_x;
	S32 volume_bar_end_y;
	int i, j;
	BOOL enable;
	U8 value;
	...
	volume_bar_end_x = instance.x + (instance.slot_width * instance.slot_num + (instance.slot_num - 1)*instance.slot_gap) * instance.max_value\
							+ (instance.max_value- 1) * instance.block_gap - 1;
	volume_bar_end_y = instance.y + instance.slot_height - 1;

	kal_prompt_trace(MOD_MMI, "gui_volume_bar_draw in\n");
	kal_prompt_trace(MOD_MMI, "draw begin\n");

	
	if (MMI_TRUE == instance.show_bg)
	{
		volume_bar_end_x = volume_bar_end_x + MMI_VOLUME_BAR_BG_LEFT_OFFSET;		
		volume_bar_end_y = volume_bar_end_y + MMI_VOLUME_BAR_BG_TOP_OFFSET;
		//gdi_layer_push_and_set_clip(instance.x - MMI_VOLUME_BAR_BG_LEFT_OFFSET, instance.y - MMI_VOLUME_BAR_BG_TOP_OFFSET,\
		//				volume_bar_end_x, volume_bar_end_y);
		
		gdi_draw_solid_rect(instance.x - MMI_VOLUME_BAR_BG_LEFT_OFFSET, instance.y - MMI_VOLUME_BAR_BG_TOP_OFFSET, \
			volume_bar_end_x, volume_bar_end_y, \
			instance.bg_color);
		gdi_layer_get_opacity(&enable, &value);
		gdi_layer_set_opacity(MMI_TRUE, instance.bg_opacity);

		gdi_layer_push_and_set_clip(
			instance.x - MMI_VOLUME_BAR_BG_LEFT_OFFSET, 
			instance.y - MMI_VOLUME_BAR_BG_TOP_OFFSET, 
			volume_bar_end_x,
			volume_bar_end_y);

		gdi_layer_flatten_with_clipping(alpha, active_layer, GDI_NULL_HANDLE, GDI_NULL_HANDLE);

		gdi_layer_pop_clip();

		gdi_layer_set_opacity(enable, value);
	}
	else if (MMI_FALSE == instance.show_bg)
	{
			gdi_draw_solid_rect(instance.x, instance.y, \
			volume_bar_end_x, volume_bar_end_y, GDI_COLOR_BLACK);
	}

	//draw value block
	for (i=0; i<instance.level_value; i++)
	{
		for (j=0; j<instance.slot_num; j++)
		{
			gdi_draw_solid_rect(instance.x, instance.y, \
				instance.x+instance.slot_width-1, instance.y+instance.slot_height-1, instance.highlight_color);
			instance.x = instance.x + instance.slot_width + instance.slot_gap;
		}
		instance.x = instance.x - instance.slot_gap + instance.block_gap;
	}

	//draw left volumn bar block
	for (i; i<instance.max_value; i++)
	{
		for (j=0; j<instance.slot_num; j++)
		{
			gdi_draw_solid_rect(instance.x, instance.y, \
				instance.x+instance.slot_width-1, instance.y+instance.slot_height-1, instance.normal_color);
			instance.x = instance.x + instance.slot_width + instance.slot_gap;
		}
		instance.x = instance.x - instance.slot_gap + instance.block_gap;
	}

	kal_prompt_trace(MOD_MMI, "draw end\n");
