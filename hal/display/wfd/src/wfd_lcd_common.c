 ...
 * $Log$
 *
 * 02 24 2015 edwardyc.lin
 * removed!
 * 	.
 *
 * 02 12 2015 edwardyc.lin
 * removed!
 * 	.
 *
 * 10 07 2014 edwardyc.lin
 * removed!
 * 	.
 *
 * 09 03 2014 robin.chen
 * removed!
 * 	1.Modify GPIO control API in ts_lcd.c
 * 	2.Do LCM init if system exit Hibernation mode for KEEP_LCM_ON=false case.
 *
 * 07 03 2014 edwardyc.lin
 * removed!
 * 	.
 *
 ...
 * 	.
 *
 * 06 03 2014 robin.chen
 * removed!
 * 	1.Modify LCD reset initial setting as output high in codegen to avoid lcd reset pin become low while exit hibernation mode.
 * 	2.Skip lcd initiallize whiel exit hibernation mode.
 *
 * 05 20 2014 robin.chen
 * removed!
 * 	1.Support sharp memory lcd support.
 * 	2.Add fake "tansfer frame done" interrupt for BW display.
 *
 * 05 19 2014 robin.chen
 * removed!
 * 	.Merge LCM driver patch from WEARv1_DEV branch
 *
...
extern kal_bool isSleepin;

#ifdef __TINY_SYS__
CSCI_EXTERN_VAR(kal_uint32, g_DBG_TS_actual_switch);
#endif
...
kal_uint32 lcd_debug_dump_register[3][12];	// Add for debug
kal_bool needDump_Registers = KAL_TRUE;
...
__wfd_lcd_TimerCb(void *parameter)
{
  ...
  {
	  // Add a debug register dump
	  lcd_debug_dump_register[1][0] =  (*(volatile kal_uint16 *)0xA0450000);
	  lcd_debug_dump_register[1][1] =  (*(volatile kal_uint16 *)0xA0450004);
	  lcd_debug_dump_register[1][2] =  (*(volatile kal_uint16 *)0xA0450008);
	  lcd_debug_dump_register[1][3] =  (*(volatile kal_uint16 *)0xA045000C);
	  lcd_debug_dump_register[1][4] =  (*(volatile kal_uint16 *)0xA0450010);
	  lcd_debug_dump_register[1][5] =  (*(volatile kal_uint16 *)0xA0450080);
	  lcd_debug_dump_register[1][6] =  (*(volatile kal_uint16 *)0xA0450084);
	  lcd_debug_dump_register[1][7] =  (*(volatile kal_uint16 *)0xA0450090);
	  lcd_debug_dump_register[1][8] =  (*(volatile kal_uint16 *)0xA04501F0);
	  lcd_debug_dump_register[1][9] =  (*(volatile kal_uint16 *)0xA04501F4);
	  lcd_debug_dump_register[1][10] =	(*(volatile kal_uint16 *)0xA04501FC);
	  lcd_debug_dump_register[1][11] =	(*(volatile kal_uint16 *)0xA0450250);
	  // register dump end
...
__wfd_lcd_Hisr(void)
{
    ...
    {
       /* Robin: Use SW CS(workaround) for 2502 TS, see MAUI_03499573. */
       #if defined(__TINY_SYS__) && defined(MT2502) && defined(LCD_CS_GPIO)
       DCL_HANDLE lcd_cs_hnd;
       lcd_cs_hnd = DclGPIO_Open(DCL_GPIO, LCD_CS_GPIO);
       DclGPIO_Control(lcd_cs_hnd, GPIO_CMD_WRITE_HIGH, NULL);
       DclGPIO_Close(lcd_cs_hnd);
       #endif
     ...
     #if defined(DDV2_PERFORMANCE_PROFILING_SUPPORT)
       {
         ...
         kal_uint32 curr = ust_get_current_time();
         kal_uint32 dur = ust_get_duration(g_DBG_TS_actual_switch , curr);
#endif
     ...
     #endif
	 // Add a debug register dump
	 if(needDump_Registers)
	 {
		 lcd_debug_dump_register[0][0] =  (*(volatile kal_uint32 *)0xA0450000);
		 lcd_debug_dump_register[0][1] =  (*(volatile kal_uint32 *)0xA0450004);
		 lcd_debug_dump_register[0][2] =  (*(volatile kal_uint32 *)0xA0450008);
		 lcd_debug_dump_register[0][3] =  (*(volatile kal_uint32 *)0xA045000C);
		 lcd_debug_dump_register[0][4] =  (*(volatile kal_uint32 *)0xA0450010);
		 lcd_debug_dump_register[0][5] =  (*(volatile kal_uint32 *)0xA0450080);
		 lcd_debug_dump_register[0][6] =  (*(volatile kal_uint32 *)0xA0450084);
		 lcd_debug_dump_register[0][7] =  (*(volatile kal_uint32 *)0xA0450090);
		 lcd_debug_dump_register[0][8] =  (*(volatile kal_uint32 *)0xA04501F0);
		 lcd_debug_dump_register[0][9] =  (*(volatile kal_uint32 *)0xA04501F4);
		 lcd_debug_dump_register[0][10] =  (*(volatile kal_uint32 *)0xA04501FC);
		 lcd_debug_dump_register[0][11] =  (*(volatile kal_uint32 *)0xA0450250);
	 }
	 // register dump end
...
__wfd_lcd_PortPowerCtrl(WFD_DEVICE* device, WFD_PORT* port)
{
	...
	#if defined(__KEEP_LCD_ENGINE_ON__)
		port->portFuncTable->EnterIdleMode();
	#else
	...
	#if defined(__KEEP_LCD_ENGINE_ON__)
		port->portFuncTable->ExitIdleMode();
	#else
...
#if defined(__BW_FLATTEN_ONE_LAYER__)
               {
                   kal_uint32 save_irq_mask;
                   WFD_DEVICE* pDevice;
                   pDevice = &_wfd_lcd_device;

                   device->utilityTable._wfd_PowerCtrl(pDevice, wfd_lcd_power_ctrl_handle, WFD_FALSE);
                   //Clear BusyFlag before exec callback.
