...
# Constants
#****************************************************************************
my $SYSGEN2_VERNO     = " v1.73 -> p1.74 -> p1.75 -> p1.76 -> p1.77 -> p1.78 -> p1.79 -> p1.82 -> p1.83.";
                       #  p1.83 ,by Dongming, 20150407, mt2502C slim patch back from MT2502C_DEV, tiny system layout reorg.
                       #  p1.82 ,by Dongming, 20140728, New feature for IOT FOTA
                       #  p1.81 , by Yinli, 20140731, Support MPU protect UPDATE_ROM as RO/cacheable attribute
		               #  p1.80 , by Dongming, 20140728, New feature for IOT FOTA
                       #  p1.79 , 2014/07/02 by Yinli, Support differential fota
                       #  p1.78 , 2014/06/20 by Yinli, Support dump vector table under tiny mode
                       #  p1.77 , 2014/05/19 by Yinli, Support dump CSCI_DATA on AP memory dump
                       #  p1.76 , 2014/05/19 by Yinli, TS layout improvement
...
my $CUSTOM_UBCONFIG_C     = $ARGV[0] . '\\' . "custom_ubconfig.c";
my $CUSTOM_UBCONFIG_H     = $ARGV[0] . '\\' . "custom_ubconfig.h";
...
my $ub_DumpRegions_ref;
if(exists $MAKEFILE_OPTIONS{"UPDATE_BINARY_FILE"} 
   and $MAKEFILE_OPTIONS{'UPDATE_BINARY_FILE'} eq 'TRUE')
{
    $g_UBScat = new scatInfo;
    $g_UBScat->ParseScatterFile($UB_SCATTERFILE);
    $g_UBScat->ub_ClassifyRegionType(\%MAKEFILE_OPTIONS);
    $ub_DumpRegions_ref = $g_UBScat->ub_GetDumpExeRegions($g_bb);
}
#****************************************************************************
...
if(exists $MAKEFILE_OPTIONS{"UPDATE_BINARY_FILE"} 
   and $MAKEFILE_OPTIONS{'UPDATE_BINARY_FILE'} eq 'TRUE')
{
    &GenerateFile($CUSTOM_UBCONFIG_C, undef);
    &GenerateFile($CUSTOM_UBCONFIG_H, undef);
}
#****************************************************************************
# oo >>>  Finished
#****************************************************************************
...
sub BLCONFIG_C_Gen_TEMPLATE_DIFFENFOTA_STOR_ASSIGN
{
    my $template;
    if(!$g_MAUIScat->IsRegionExistent("DIFFENFOTA_STOR"))
    {
        $template = "    *addr = (kal_uint32)0x0;\n";
        $template .= "    *len = (kal_uint32)0x0;";
    }
    else
    {
       my $ref_regionInfo = $g_MAUIScat->GetLoadRegionInfo("DIFFENFOTA_STOR"); 
       my $base_addr = $ref_regionInfo->[Region::Base];
       my $max_size = $ref_regionInfo->[Region::MaxSize];
       $template = "    *addr = (kal_uint32)$base_addr;\n";
       $template .= "    *len = (kal_uint32)$max_size;";
    }
    return $template;
}
...
sub Get_Dump_Regions
{
    ...
    foreach my $strGroupName (%$g_DumpRegions)
    {
        ...
        foreach my $RegionItem (@$array_ref)
        {
            ...
            if(($RegionName eq "DUMP_VECTOR_TABLE")or ($RegionName eq "CSCI_DATA"))
            {
...
sub get_all_code_addr_info
{
    ...
    #for UPDATE_ROM
    {
        my $Info = $g_MAUIScat->GetLoadRegionInfo("UPDATE_ROM");
        if(exists $MAKEFILE_OPTIONS{"UPDATE_BINARY_FILE"} 
            and $MAKEFILE_OPTIONS{'UPDATE_BINARY_FILE'} eq 'TRUE')        
        {
        my $strBase = $Info->[0];
        my $strLen = $Info->[1];
        $template .= "    TOTAL_CODE_REGION[$nIndex].addr = (kal_uint32)$strBase;\n";
        $template .= "    TOTAL_CODE_REGION[$nIndex].len = (kal_uint32)$strLen;\n" ;
        $nIndex++;
    }
    }
    ...
    foreach my $strExeRegionName (@$ExeRegion_ref)
    {
        ...
        #bypass obvious none-code regions 
        next if($strExeRegionName =~ /VIVA|GFH$|^INTSRAM_DATA|SIGNATURE_SECTION$|LANG_PACK|CUSTPACK|DUMMY_END|EXTSRAM_TMP_PAGE_TABLE|EMPTYSECTION$|^jblend_ram$|CSCI_DATA/); 
        #bypass reserved memory on ram
        next if($strExeRegionName =~ /UB_RAM|TS_RESERVE_MEM|TS_EMI_CODE_EV/);
...
sub SCAT_C_Gen_TS_REGION_SYMBOL
{
    ...
    foreach (@$ExeRegion_ref)
    {
        next if($_ eq "TS_DUMP_VECTOR_TABLE");
        $template .= "extern kal_uint32 Image\$\$$_\$\$Base;\n";
        $template .= "extern kal_uint32 Image\$\$$_\$\$RW\$\$Base;\n";
        $template .= "extern kal_uint32 Image\$\$$_\$\$Length;\n";
        $template .= "extern kal_uint32 Image\$\$$_\$\$ZI\$\$Limit;\n";
    }
...
sub SCAT_C_Gen_TEMPLATE_TINYSYS_ROM3_ON_FLASH
{
    ###this address and length tell tiny loader where to find tiny image on flash
    my ($template,$strBase) = (undef,undef); 
    if(exists $MAKEFILE_OPTIONS{'TINY_SYS'} 
        and $MAKEFILE_OPTIONS{'TINY_SYS'} eq 'TRUE')
    {
        my $arr_ref = $g_MAUIScat->GetLoadRegionInfo("TS_ROM3");
        $strBase = $arr_ref->[Region::Base];
        $template = &get_tiny_region_info_template($strBase);
...
sub SCAT_H_Gen_TS_DUMP_REGION_COUNT
{
    ...
    and $MAKEFILE_OPTIONS{'TINY_SYS'} eq 'TRUE')
    {
        ...
        if ($g_TinyScat->IsRegionExistent("TS_EMI_CODE_REGION"))
        {
            $nCount ++;        
        }
...
sub SCAT_C_Gen_TS_TEMPLATE_DUMP_REGION_ASSIGN
{
    ...
    and $MAKEFILE_OPTIONS{'TINY_SYS'} eq 'TRUE')
    {
        ...
        if ($g_TinyScat->IsRegionExistent("TS_EMI_CODE_REGION"))
        {
            $template .= "    region[$i].addr = TS_EMI_CODE_BASE".";\n";
            $template .= "    region[$i].len = MAX_TS_EMI_CODE_SIZE".";\n";
        }
    }
    ...
    foreach my $strGroupName (%$ts_DumpRegions_ref)
    {
        ...
        foreach my $RegionItem (@$array_ref)
        {
            ...
            else
            {
                ...
                my $strBase = ($strGroupName eq 'EV')? "(kal_uint32)&Image\$\$$RegionName\$\$RW\$\$Base" : "(kal_uint32)&Image\$\$$RegionName\$\$Base" ;
                print "[$strGroupName]$RegionName:$strBase,$strLength\n" if($DebugPrint);
                push(@DumpRegions, [$strGroupName, $strBase, $strLength]); 
...
sub SCAT_C_Gen_Get_UB_ROM_Entry
{
    my $template = "    return 0;";
    my $strRegionName = $g_MAUIScat->SearchLoadRegionName("UPDATE_ROM");
    my $region_info_ref = $g_MAUIScat->GetLoadRegionInfo("UPDATE_ROM");
    my $strBase = $region_info_ref->[Region::Base];
    if(defined $strRegionName)
    {
        $template = <<"__TEMPLATE";
	kal_uint32 jmp_addr = (kal_uint32)$strBase;
	jmp_addr += ((GFH_FILE_INFO_v1*)(jmp_addr))->m_content_offset;
	return jmp_addr;
__TEMPLATE
    }
    return $template;
...
sub SCAT_C_Gen_UB_REGION_SYMBOL
{
    my $template;
    if(exists $MAKEFILE_OPTIONS{'UPDATE_BINARY_FILE'} 
    and $MAKEFILE_OPTIONS{'UPDATE_BINARY_FILE'} eq 'TRUE')
    {
        my $ExeRegion_ref = $g_UBScat->GetAllExeRegion();
        foreach (@$ExeRegion_ref)
        {
            $template .= "extern kal_uint32 Image\$\$$_\$\$Base;\n";
            $template .= "extern kal_uint32 Image\$\$$_\$\$Length;\n";
        ...
        foreach my $i (1 .. $g_UBScat->{LoadRegionCount})
        {
            my $LoadRegionName = $g_UBScat->{LoadRegion}{$i}->[0];
            $template .= "extern kal_uint32 Load\$\$$LoadRegionName\$\$Base;\n";
        }
    }
...
sub SCAT_H_Gen_UB_DUMP_REGION_COUNT
{
    my $nCount = 0;
    if(exists $MAKEFILE_OPTIONS{'UPDATE_BINARY_FILE'} 
    and $MAKEFILE_OPTIONS{'UPDATE_BINARY_FILE'} eq 'TRUE')
    {
        if(!defined $ub_DumpRegions_ref)
        {
            $ub_DumpRegions_ref = $g_UBScat->ub_GetDumpExeRegions($g_bb);
        }
        foreach my $strGroupName (keys %$ub_DumpRegions_ref)
        {
            my $array_ref = $ub_DumpRegions_ref->{$strGroupName};
            $nCount += ($#$array_ref +1);
        }
    }
...
sub SCAT_C_Gen_UB_TEMPLATE_DUMP_REGION_Internal_ASSIGN
{
    my $template;
    if(exists $MAKEFILE_OPTIONS{'UPDATE_BINARY_FILE'} 
    and $MAKEFILE_OPTIONS{'UPDATE_BINARY_FILE'} eq 'TRUE')
    {
        my $DumpRegions_ref = &ub_Get_Dump_Regions();
        for(my $i = 0; $i < $#$DumpRegions_ref+1; $i++)
        {
            my $GroupName = $DumpRegions_ref->[$i]->[0];
...
sub ub_Get_Dump_Regions
{
    ...
    my $nIndex = 0;
    foreach my $strGroupName (%$ub_DumpRegions_ref)
    {
        my $array_ref = $ub_DumpRegions_ref->{$strGroupName};
        foreach my $RegionItem (@$array_ref)
        {
            ...
            my $LastRegionName =  $RegionItem->[ scalar(@$RegionItem)-1 ];
            my $strLength = "(kal_uint32)&Image\$\$$LastRegionName\$\$ZI\$\$Limit - region\[$nIndex\].addr";
            my $strBase = "(kal_uint32)&Image\$\$$RegionName\$\$Base" ;
            print "[$strGroupName]$RegionName:$strBase,$strLength\n" if($DebugPrint);
            push(@DumpRegions, [$strGroupName, $strBase, $strLength]); 
...
sub SCAT_C_Gen_Get_dynamic_DC_ZI_MMIPOOL
{
    my $template;
    if(exists $MAKEFILE_OPTIONS{'UPDATE_BINARY_FILE'} 
    and $MAKEFILE_OPTIONS{'UPDATE_BINARY_FILE'} eq 'TRUE')
    {
        $template = "    region->addr = " . "(kal_uint32)&Image\$\$UB_DYNAMIC_CACHEABLE_EXTSRAM_DEFAULT_CACHEABLE_ZI_MMIPOOL\$\$Base" . ";\n";
        $template .= "    region->len = " . "(kal_uint32)&Image\$\$UB_DYNAMIC_CACHEABLE_EXTSRAM_DEFAULT_CACHEABLE_ZI_MMIPOOL\$\$ZI\$\$Limit - region->addr" . ";\n";
    }
    $template .= "	return 0" . ";\n";
    return $template;
}

sub SCAT_C_Gen_Get_UB_ROM_REGION
{
    my $template;
    if(exists $MAKEFILE_OPTIONS{'UPDATE_BINARY_FILE'} 
    and $MAKEFILE_OPTIONS{'UPDATE_BINARY_FILE'} eq 'TRUE')
    {
        my $region_info_ref = $g_MAUIScat->GetLoadRegionInfo("UPDATE_ROM");
        my $strBase = $region_info_ref->[Region::Base];
        my $strLen = $region_info_ref->[Region::MaxSize];
    
        $template = "    region->addr = " . "(kal_uint32)$strBase" . ";\n";
        $template .= "    region->len = " . "(kal_uint32)$strLen" . ";\n";
