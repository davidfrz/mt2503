...
#endif
//this will be called by AT command task.
//That task didn't release to customer by source, so they can only call the fixed function.
kal_bool SD_Check_Card_Present(void)
{
...
//this will be called by AT command task.
//That task didn't release to customer by source, so they can only call the fixed function.
kal_bool SD_Check_Card_Mount(void)
{
#if defined(__MSDC_SD_MMC__)
...
#else //#if defined(__MSDC_SD_MMC__)||defined(__MSDC_SD_SDIO__)
...
#endif //#if defined(__MSDC_SD_MMC__)||defined(__MSDC_SD_SDIO__)
