...
U16 mmi_get_update_status_string_from_nitz(void)
	{
		...
		#endif
		#if defined(__GPS_ADAPTOR_SUPPORT__)
		GPS_status = srv_dt_set_get_gps_time_sync_mode();
		#endif
		#ifdef __GPS_SUPPORT__
		#endif
...
void mmi_set_auto_update_time_on(void)
{
	...
	StartTimer(NTP_SYNC_TIMER, 0, srv_ntp_auto_sync_timer_proc);
	#endif
	#if defined(__GPS_ADAPTOR_SUPPORT__)
...
/* under construction !*/
			#endif
				cui_menu_set_currlist_highlighted_id(cui_id, MENU_ID_NITZ_APP_OFF + mmi_get_update_status_string_from_nitz() - STR_GLOBAL_OFF);            
                }                
            }
        }
        break;
    ...
    if (mmi_dt_is_valid(nitz_timeP))
{
       ...
       // mmi_dt_set_rtc_dt((MYTIME*)nitz_timeP);
        mmi_dt_set_dt((MYTIME*)nitz_timeP, NULL, NULL);
