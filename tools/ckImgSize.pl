...
#restrict zimage size to improve bring-up time when fast boot on wearable_device projects
if(defined $makefile{'__TINY_IOT_FAST_BOOT__'} and $makefile{'__TINY_IOT_FAST_BOOT__'} eq 'TRUE')
{
    &parse_customFeatureConfig($custom_featureconfig_h, \%FEATURE_CONFIG_Value);
    if(defined $FEATURE_CONFIG_Value{'FAST_BOOT_LIMIT_ZIMAGE_MAX_DECOMPRESSED_SIZE'} and (hex($FEATURE_CONFIG_Value{'FAST_BOOT_LIMIT_ZIMAGE_MAX_DECOMPRESSED_SIZE'}) != 0))
    {
        my $FAST_BOOT_LIMIT_SIZE = hex($FEATURE_CONFIG_Value{'FAST_BOOT_LIMIT_ZIMAGE_MAX_DECOMPRESSED_SIZE'});

        my $ZIMAGE_FILE = "$bin_folder\\ZIMAGE_ER";
        &error_handler("$ZIMAGE_FILE: NOT exist!", __FILE__, __LINE__) if (!-e $ZIMAGE_FILE);
        my $ZIMAGE_BIN_SIZE = stat("$ZIMAGE_FILE")->size;
        print "The ZIMAGE_ER bin size = $ZIMAGE_BIN_SIZE\n";

        &checkSize("ZIMAGE_ER","The Max Compressed Size of ZIMAGE_ER","Actual ZIMAGE_ER bin Size",$FAST_BOOT_LIMIT_SIZE,$ZIMAGE_BIN_SIZE);
    }
}
