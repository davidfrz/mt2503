...
#if defined(__GDI_BLT_WITH_DOUBLE_BUF_SUPPORT__)
#define GDI_BLTDB_BUF_CF  GDI_COLOR_FORMAT_16
#define GDI_BLTDB_BUF_W   GDI_LCD_WIDTH
#define GDI_BLTDB_BUF_H   GDI_LCD_HEIGHT

/* double buffer size */
#if GDI_BLTDB_BUF_CF == GDI_COLOR_FORMAT_16
#if !defined(__MMI_GADGET_SUPPORT__)
    #define GDI_BLTDB_BUF_SIZE (GDI_BLTDB_BUF_W*GDI_BLTDB_BUF_H*6)  /* 6 = 2(bpp)*2(double buffer)*1.5(OSD layer) */
#else
    #define GDI_BLTDB_BUF_SIZE (GDI_BLTDB_BUF_W*GDI_BLTDB_BUF_H*8)  /* 8 = 2(bpp)*2(double buffer)*2(OSD layer) */
#endif
#else
    #error "please define double buffer size for other color format."
#endif
