                                ...
                                //gui_draw_filled_area(b->x, bs->y1 - l_height, bs->x2, bs->y2, &f);
                                   ...
                                   if( (b->ext_flags & GUI_MULTI_LINE_INPUT_BOX_SET_BG_COLOR)
                                    ...
                                    if(b->ext_flags & GUI_MULTI_LINE_INPUT_BOX_SINGLINE_STYLE)
                                    {
                                        ...
                                        if (g_inputs_background_filler)//support g_inputs_background_filler
                                        {
                                            g_inputs_background_filler(b->x, bs->y1 - l_height, bs->x2, bs->y2);
                                        }
...
#ifdef  __MMI_MULTILINE_INPUT_BOX_SINGLINE_STYLE__
// it is same with gui_adjust_multi_line_text_offset_y()
U32 gui_get_multi_line_edit_width_in_singline(multi_line_input_box *b, multi_line_input_box_show_struct *bs)
{
   U32 edit_width = 0;

    edit_width = b->width - b->right_width - b->margin.right_margin - b->left_width - b->margin.left_margin;
    #ifdef __MMI_BIDI_ALG__
    if(bs->default_direction == BIDI_L)
    #endif /* __MMI_BIDI_ALG__ */    
    {   
        edit_width = edit_width - GUI_INPUTS_SL_LOCATE_CURSOR_L2R_EXTRA_WIDTH;    
    }
    #ifdef __MMI_BIDI_ALG__
    else
    {
        edit_width = edit_width - GUI_INPUTS_SL_LOCATE_CURSOR_R2L_EXTRA_WIDTH;
    }
    #endif/*__MMI_BIDI_ALG__*/
...
void gui_set_multi_line_text_area_range(multi_line_input_box *b, multi_line_input_box_show_struct *bs)
{
    ...
    bs->edit_width = (bs->x2 - bs->x1 - b->text_x);
        ...
        if(b->ext_flags & GUI_MULTI_LINE_INPUT_BOX_SINGLINE_STYLE)
        {   
            bs->edit_width = gui_get_multi_line_edit_width_in_singline(b,bs);
        }
    #endif/*__MMI_MULTILINE_INPUT_BOX_SINGLINE_STYLE__*/
    #endif/*__MMI_MULTILINE_INPUT_BOX_SINGLINE_STYLE_WITH_FTE__*/
    if (b->ext_flags & GUI_MULTI_LINE_INPUT_BOX_SINGLE_BORDER)
...
void gui_draw_multi_line_one_line(multi_line_input_box *b, multi_line_input_box_show_struct *bs)
{
    ...
    if (!(b->ext_flags & GUI_MULTI_LINE_INPUT_BOX_HIDE_TEXT_CONTENT))
    {
        ...
        {   
            ...
            x1 = b->x+b->text_x;
            x2 = x1 + bs->edit_width + 2;
            if (bs->flags & UI_MULTI_LINE_INPUT_BOX_USE_PRESET_CLIPPING)
            {
                gui_set_clip_preset(x1, bs->y1 ,x2, bs->y2 );
            }
            else
            {
                gui_set_clip(x1, bs->y1 , x2, bs->y2 );
            }        
        }        
#endif/*__MMI_MULTILINE_INPUT_BOX_SINGLINE_STYLE__*/
#endif/*__MMI_MULTILINE_INPUT_BOX_SINGLINE_STYLE_WITH_FTE__*/
...
#ifdef __MMI_MULTILINE_INPUT_BOX_SINGLINE_STYLE__      
           #ifdef __MMI_EDITOR_SSP_SUPPORT__      
            if((b->ext_flags&GUI_MULTI_LINE_INPUT_BOX_FIXED_Y_OFFSET)
                ||(b->ext_flags &GUI_MULTI_LINE_INPUT_BOX_DISABLE_FORCE_CURSOR_APPEAR)                       
                ||(gui_input_box_ssp_get_state() != GUI_INPUT_BOX_SSP_STATE_UNFOCUS)
                 )
            {//for ssp           
