...
//Not support recording the MRE APP handle.
//#if !defined(EMPTY_MMI)
...
/* under construction !*/
#endif
//API for MRE to query message ID with event ID
DCL_STATUS DCL_GetMSGID(DCL_HANDLE		device_handle,
	...
	if(STATUS_OK!=CheckFunctionPointer((void*)(msg_id))){
		return STATUS_FAIL;
	}
	if(STATUS_OK!=CheckFunctionPointer((void*)(Devices[dev_index].event_msg_map))){
		return STATUS_FAIL;
	}
	//*msg_id=(msg_type)(-1);
	//search message id
...
DCL_STATUS DCL_GetEventID_DevHandle(ilm_struct* ilm, 
	...
	if(STATUS_OK!=CheckFunctionPointer((void*)(event_id))){
		return STATUS_FAIL;
	}
	if(STATUS_OK!=CheckFunctionPointer((void*)(dev_handle))){
		return STATUS_FAIL;
	}
	if(STATUS_OK!=CheckFunctionPointer((void*)(ilm))){
		return STATUS_FAIL;
	}
...
DCL_STATUS DCL_Control (
	...
	//As DCL_Control was called frequently, remove this trace for performance concern.
...
DCL_STATUS DCL_RegisterCallback(
	...
	RecordUserHandle(&(Devices[dev_index].user_handle));
	internal_handle = device_handle & DCL_HANDLE_CLEAR_MASK;
//Do not need this any more, MRE do this in their own files.
//#if !defined(EMPTY_MMI)
