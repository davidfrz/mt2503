...
extern void SetDialingKeypadLongPressHandler_SW(void (*handler) (S32));

void mmi_speed_dial_long_press_handler(S32 key)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    else if ((mmi_phb_get_speed_dial_status() == MMI_TRUE) && key >=KEY_2 && key <= KEY_9)
    {
    	cui_dialer_obj_struct *obj;
    	CHAR *phb_dial_string;
    	mmi_ucm_make_call_para_struct param;
	    ...
	    /* Skip if handset cannot find corresponding contact. */
	    phb_dial_string = mmi_phb_speed_dial_get_number(key);
	    if (!phb_dial_string)
	    {
	        cui_dialer_display_popup(STR_ID_CUI_DIALER_NO_SPEED_DIAL_NUM);
	        cui_dialer_emit_close_req(obj); /* close dialer */
...
#endif /*defined(__MMI_WEARABLE_DEVICE__) && defined(__MMI_TELEPHONY_SUPPORT__)*/
/*****************************************************************************
 * FUNCTION
...
/* under construction !*/
...
#endif
    {
		...
		SetDialingKeypadLongPressHandler_SW(mmi_speed_dial_long_press_handler);
