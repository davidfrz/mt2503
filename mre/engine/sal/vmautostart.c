    ...
    if(!srv_safe_mode_is_active())
	{
		...
		{     
			...
			{   
				...
				auto_start_drv_num = srv_mre_appmgr_get_app_num(SRV_APPMGR_INSTALLED_DRIVER_APP_FLAG_TYPE);
                g_autostart_app_num = auto_start_app_num + auto_start_vsp_num + auto_start_drv_num;
				if(g_autostart_app_num > 0)
				{
                    ...
                    if(auto_start_drv_num>0)
                    {
					    srv_mre_appmgr_get_app_list(SRV_APPMGR_INSTALLED_DRIVER_APP_FLAG_TYPE, g_autostart_app_list_p+(auto_start_app_num), auto_start_drv_num);
                    }					
                    if(auto_start_vsp_num>0)
                    {
                        srv_mre_appmgr_get_app_list(SRV_APPMGR_INSTALLED_VSP_FLAG_TYPE, g_autostart_app_list_p+(auto_start_app_num+auto_start_drv_num), auto_start_vsp_num);
                    }
					vm_launch_autostart_app();
				}
				break;
