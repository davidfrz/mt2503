...
extern GB_INT GB_WG_Get_CurrentSysLanguageStr(GB_CHAR* pLanguageStr, GB_INT n);
extern GB_INT GB_WG_Get_IMEI(GB_CHAR *pIMEI, GB_UINT nBuffLen);
extern GB_INT GB_WG_Get_IMSI(GB_CHAR *pIMSI, GB_UINT nBuffLen);
extern GB_CHAR* GB_WG_Get_ScreenSize(GB_VOID);
extern S32 GB_WG_Get_CacheSec(CHAR *pCacheSec, U32 nBuffLen);
extern GB_INT GB_WG_GetPlatformArgument(GB_CHAR* pUrlBuff, GB_UINT nBuffLen);
...
static void GB_WG_Search_Web_Entry(void)
{
	...
	wgui_cat2001_setup_search_button(
		(PU8)get_image(IMG_GB_WINGUO_SKINWHITE_SEARCH_G),
		(PU8)get_image(IMG_GB_WINGUO_SKINWHITE_SEARCH_G),
