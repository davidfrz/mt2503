...
void srv_sms_btmapc_set_curr_bt_index(srv_sms_btmapc_bt_enum curr_tab_index)
{
    sms_cntx->curr_tab_index = curr_tab_index;
}

srv_sms_btmapc_bt_enum srv_sms_btmapc_get_curr_bt_index()
{
    return sms_cntx->curr_tab_index;
}
...
MMI_BOOL srv_sms_btmapc_is_msg_download(srv_sms_btmapc_bt_enum bt_index, U16 msg_id)
{
    WCHAR file_name[SRV_SMS_BTMAPC_MAX_PATH_LEN];
    ...
    if(srv_sms_btmapc_is_msg_exist(msg_id) == MMI_TRUE)
    {
        ...
        {
            srv_sms_btmapc_get_msg_name(bt_index, msg_id, MMI_FALSE, file_name);
 ...
 *  
 *****************************************************************************/
void srv_sms_btmapc_read_me_sms(srv_sms_btmapc_bt_enum bt_index, U16 msg_id, SrvSmsCallbackFunc callback_func, void *user_data)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    FS_GetFileSize(hd, &real_size);
    if(hd > 0)
    {
        //if(read_size > SRV_SMS_BTMAPC_MAX_CONTENT_SIZE)
            //read_size = SRV_SMS_BTMAPC_DISPLAY_CHAR_SIZE;
        utf_buff = (U8 *)OslMalloc(SRV_SMS_BTMAPC_MAX_DISPLAY_BUFF_SIZE);
        memset(utf_buff, 0, SRV_SMS_BTMAPC_MAX_DISPLAY_BUFF_SIZE);
        result = FS_Read(hd, utf_buff, SRV_SMS_BTMAPC_MAX_DISPLAY_BUFF_SIZE, &real_size);
        FS_Close(hd);
        if(result < 0)
        {
        ...
        else
        {
            ...
            mmi_chset_utf8_to_ucs2_string((U8 *)msg_data->content_buff, SRV_SMS_BTMAPC_DISPLAY_BUFFER_SIZE, utf_buff);
            msg_data->content_size = mmi_wcslen((WCHAR *)msg_data->content_buff);
            ...
            void *user_data)
{
    ...
    if(hd > 0)
    {
        //result = FS_Write(hd, sms_content, sms_content_size, &real_size);
        result = srv_sms_btmapc_create_xml_data_usc2_to_utf8(hd, content);
        FS_Close(hd);
