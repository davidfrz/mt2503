...
my $proj_mak    = $ARGV[0]; # ex: make/project.mak
my $output_file = $ARGV[1]; # ex: build/$project/_project.mak
if (!-e $proj_mak) {
...
sub recursive_concatenate
{
	...
	while (<$FIN>) {
		$line = $_;
		if (defined($feature{'PROJECT_MAKEFILE_EXT'}) and ($feature{'PROJECT_MAKEFILE_EXT'} ne "NONE") and
				$line =~ /\s*include\s+(make[\/\\]custom_config[\/\\])/ and 
