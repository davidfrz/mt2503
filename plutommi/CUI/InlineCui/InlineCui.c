...
#ifdef __INLINE_DATETIME_FTE_SLIM__
...
mmi_ret cui_inline_proc_func(mmi_event_struct *evt)
{
    ...
    switch (evt->evt_id)
    {
		...
		{	
			if((mmi_frm_scrn_get_active_id() == CUI_INLINE_MAIN_SCREEN_SCRN_ID)
				&&(complete_inline_item_edit != handle_inline_full_screen_edit_close))
			{
				cui_inline_rsk_function();
				return MMI_RET_ERR;
			}
			else if((mmi_frm_scrn_get_active_id() == CUI_INLINE_MAIN_SCREEN_SCRN_ID)
				&&(complete_inline_item_edit == handle_inline_full_screen_edit_close))
			{
...
/* under construction !*/
            ...
            case INLINE_ITEM_TYPE_TIME:
            {
                ...
                ReConfigureInlineItemTime(
                    ...
                    (PU8) & (p_data->time_data->am_pm_flag),
                    cui_inline_format_time);
#endif /* __INLINE_DATETIME_FTE_SLIM__ */
#endif /* __MMI_INLINE_ITEM_TIME__ */
                break;
            }
#if 0            
...
#ifdef __MMI_INLINECUI_REMOVE_SOFTKEY__	
void cui_inline_editor_lsk_handler_for_editor(void)
{
	if((mmi_frm_scrn_get_active_id() == CUI_INLINE_MAIN_SCREEN_SCRN_ID))
    {
		cui_inline_entry_options_screen();
