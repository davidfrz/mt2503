...
mmi_pen_point_struct g_down_in_title_point = {0};
MMI_BOOL g_swatch_send_down_in_move = MMI_FALSE;
S32 g_pen_down_dismiss = 0;
...
*/
MMI_BOOL wgui_is_list_in_current_scrn(void)
{
	S32 u8CtrlCt = 0;
	S32 u8NoOfUICtrls = 0;
...
//                #endif
				...
				and promote the user experience
				*/
				ret = gui_title_translate_pen_event(
						g_current_title,
						MMI_PEN_EVENT_DOWN,
						point.x,
						point.y,
				...
				if(ret)
				{
					g_pen_to_control = DM_TITLE1;
					g_down_in_title_point.x = point.x;
					g_down_in_title_point.y = point.y;
					return;
				}
				#endif/*#if defined(__MMI_WEARABLE_DEVICE__)*/
	...
	*/
	...
	if(g_pen_to_control == DM_TITLE1)
	{
		gui_title_translate_pen_event(
						...
						point.y,
						&title_event);	
		if(title_event == GUI_TITLE_PEN_MOVE_OUTSIDE)
		{
			if(wgui_is_list_in_current_scrn())
			{
					mmi_pen_point_struct temp_point;
					if ( g_down_in_title_point.y < MMI_TITLE_HEIGHT)
					{
						temp_point.x = g_down_in_title_point.x;
						temp_point.y = MMI_TITLE_HEIGHT;
					}
					g_swatch_send_down_in_move = MMI_TRUE;
					g_pen_down_dismiss = MMI_TITLE_HEIGHT - g_down_in_title_point.y;
					gui_ssp_pen_up_dummy();
					wgui_general_pen_down_hdlr(temp_point);	
					return;
			}
		}
	}
