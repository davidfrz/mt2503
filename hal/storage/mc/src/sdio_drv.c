...
SDIO_setclk_return:
    if(cs2.cardClk)
    {
    	...
    	//MSDC_SetBits32(MSDC_CLKACB_CFG, MSDC_CLKACB_CFG_CLKPAD_RED);//zhilin wang
...
#if defined(MT6250)
#define SDIO_DATA_BUS_NOT_BUSY 0x18000000
#define SDIO_DATA_BUS_BUSY_MASK 0x1F000000
#elif defined(MT6260)
#define SDIO_DATA_BUS_NOT_BUSY 0x01800000	
#define SDIO_DATA_BUS_BUSY_MASK 0x01F00000
#elif defined(MT6261) || defined(MT2502) || defined(MT2501)
#define SDIO_DATA_BUS_NOT_BUSY 0x01800000	
#define SDIO_DATA_BUS_BUSY_MASK 0x01F00000
#endif

kal_bool SDIO_CheckTimeoutWithSleep(kal_int32 start_time,
...
SDC_CMD_STATUS SDIO_WaitLastBlockFinish(void)
{
    ...
    do{
        ...
        if (SDIO_DATA_BUS_NOT_BUSY == (val & SDIO_DATA_BUS_BUSY_MASK))
        {
            status = NO_ERROR;
			break;
        }

		//kal_prompt_trace(MOD_MSDC_HISR,"SDIO_WaitLastBlockFinish,msdc_sta = %x, sdc_sta = %x",MSDC_Reg32(MSDC_STA),MSDC_Reg32(SDC_STA));

		if(SDIO_CheckTimeoutWithSleep(t1,10,1))
		{
   ...
   else/*R1*/
   {
	...
	if(p->rw == SDIO_READ)
	{
      ...
      DisableMSDC_DMA();
		...
		if(p->block)
		{
			...
			for(i=0;i< total;)
			{			      
				//if(!MSDC_IS_FIFO_EMPTY) zhilin change
	...
	{	// SDIO_WRITE
       ...
       if ((forcemcu==0)&&(p->block ==1 || (p->block ==0 && p->count >= mcudma_flag) )) // guilin
       {
		...
		if (SDIO_WaitLastBlockFinish() != NO_ERROR)
	   		goto ERROR_EXIT;	
      ...
      DisableMSDC_DMA();
	   MSDC_CLR_FIFO();	   	         
		...
		if(p->block)
		{
			...
			for(i=0;i< total;)
			{			      			      
				//(!MSDC_IS_FIFO_FULL) zhilin change
				if(MSDC_Reg32(MSDC_STA)&MSDC_STA_DRQ)
				{
