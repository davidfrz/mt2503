...
# Constants
#****************************************************************************
my $CMMGEN_VERNO     = " V1.55 -> p0.56";
                      #  p1.56 , Support IOT_FOTA
...
my $UBIN_ELFFILE  = $ARGV[10];
my $TINY_ELFFILE  = $ARGV[11];
...
ELFFILE: $ELFFILE,\nMAKEFILE: $MAKEFILE
MAUI_BIN: $MAUI_BIN, \nLISFILE: $LISFILE
CC_CMD: $CC_CMD, \nVIA_CMD: $VIA_CMD, \nOPTION_TMP: $OPTION_TMP,\nINSIDE_MTK:$INSIDE_MTK,\nTINY_ELFFILE:$TINY_ELFFILE,
UBIN_ELFFILE=$UBIN_ELFFILE,\nBIN_PATH: $BIN_PATH\n";
...
{
    my $str_load_ubin_elf; #load ubin elf when generate MAUI nocode.cmm
    if(($ACTION == 0) and 
       (defined $MAKEFILE_OPTIONS{'UPDATE_BINARY_FILE'} and $MAKEFILE_OPTIONS{'UPDATE_BINARY_FILE'} eq 'TRUE'))
    {
        $str_load_ubin_elf = << "__TEMPLATE";
D.LOAD.ELF $UBIN_ELFFILE \/RELPATH \/PATH . \/nocode \/noclear        
__TEMPLATE
    }
