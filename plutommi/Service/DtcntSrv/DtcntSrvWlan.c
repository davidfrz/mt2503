...
static void srv_dtcnt_wlan_set_scan_desired_ssid_list_cnf_hdlr(void * info);
static void srv_dtcnt_wlan_set_roaming_para_cnf_hdlr(void *info);
static void srv_dtcnt_wlan_set_dwell_time_cnf_hdlr(void *info);
#endif
...
#if defined (__MMI_WLAN_FEATURES__) && defined(__MTK_TARGET__) && defined(__TCPIP__)
srv_dtcnt_wlan_scan_setting_struct g_srv_dtcnt_wlan_scan_setting_ctx;
#endif
...
srv_dtcnt_prof_wlan_struct g_srv_dtcnt_wlan_conn_prof;  // For srv_wlan_connect() only.
 ...
 * DESCRIPTION
 *  Free certain action in list whose header is *handle. 
 *  The action to be freed must exist in the list.
 * PARAMETERS
 *  handle                  [IN]        
 *  action_to_remove        [?]         
 * RETURNS
 ...
 * DESCRIPTION
 *  Find certain action job(the first one in the list. in fact, there's only one job
 *  for one action). If not found, return NULL.
 * PARAMETERS
 *  handle      [?]         
 *  action      [IN]        
 * RETURNS
 ...
 *  Find the action, invoke the callback with the reason of delete, and also send 
 *  event to tell the reason of delete. Finally, free the action in action queue.
 * PARAMETERS
 *  action      [IN]        
 *  opt         [IN]        
 * RETURNS
...
void srv_dtcnt_wlan_delete_action(srv_dtcnt_wlan_action_enum action, srv_dtcnt_wlan_action_del_opt_enum opt)
{
    ...
    switch (action)
    {
        ...
        case SRV_DTCNT_WLAN_ACTION_DISCONNECT:
            // no callback is set in disconnect api
 ...
 * DESCRIPTION
 *  Process the action jobs in action queue. 
 *  If the state is propable for the action, send the action req and add the action
 *  callback into g_srv_dtcnt_wlan_ctx.proc_job_list, and return true.
 *  Otherwise, invoke action callback with result of failure and try the next action
 *  in the action queue. 
 * PARAMETERS
 *  void
 * RETURNS
 ...
 * DESCRIPTION
 *  Convert the dtcnt profile into the one wndrv can recognize.
 * PARAMETERS
 *  prof        [?]     
 *  profile     [?]     
 * RETURNS
...
*  Handler for event EVT_ID_SRV_SHUTDOWN_DEINIT, which is emited by shutdown srv.
* PARAMETERS
 ...
 *  srv_dtcnt_wlan_shutdown_req
 * DESCRIPTION
 *  shutdown wifi the module. If wifi is open, send ilm to notify wndrv to shutdown.
 * PARAMETERS
 *  void
 * RETURNS
 *  void
...
/*****************************************************************************
 * FUNCTION
 srv_dtcnt_wlan_set_scan_desired_ssid_list_req
 * DESCRIPTION
 *  
 * PARAMETERS
 *  void
 ...
 *  void
 *****************************************************************************/
void srv_dtcnt_wlan_set_scan_desired_ssid_list_req(U8 ssid_list_num, const srv_dtcnt_ssid_struct * const ssid_list)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    mmi_abm_wifi_set_scan_desired_ssid_list_req_struct *p;    

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    SetProtocolEventHandler(srv_dtcnt_wlan_set_scan_desired_ssid_list_cnf_hdlr, MSG_ID_MMI_ABM_WIFI_SET_SCAN_DESIRED_SSID_LIST_CNF);

    p = OslAllocDataPtr(mmi_abm_wifi_set_scan_desired_ssid_list_req_struct);
    p->ssid_list_num = ssid_list_num;
    if (ssid_list)
    {
        memcpy(p->ssid_list, ssid_list, sizeof(srv_dtcnt_ssid_struct) * ssid_list_num);        
    }
    else
    {
        memset(p->ssid_list, 0, sizeof(srv_dtcnt_ssid_struct) * DTCNT_WLAN_MAX_SPEC_SSID_LIST_NUM);        
    }
    
    mmi_frm_send_ilm(MOD_ABM, MSG_ID_MMI_ABM_WIFI_SET_SCAN_DESIRED_SSID_LIST_REQ, (oslParaType*)p, NULL);
}


/*****************************************************************************
 * FUNCTION
 srv_dtcnt_wlan_set_roaming_para_req
 * DESCRIPTION
 *  
 * PARAMETERS
 *  void
 ...
 *  void
 *****************************************************************************/
void srv_dtcnt_wlan_set_roaming_para_req(MMI_BOOL enable_roaming)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    mmi_abm_wifi_set_roaming_para_req_struct *p;    

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    SetProtocolEventHandler(srv_dtcnt_wlan_set_roaming_para_cnf_hdlr, MSG_ID_MMI_ABM_WIFI_SET_ROAMING_PARA_CNF);

    p = OslAllocDataPtr(mmi_abm_wifi_set_roaming_para_req_struct);
    p->fgEnableRoaming = enable_roaming;    
    
    mmi_frm_send_ilm(MOD_ABM, MSG_ID_MMI_ABM_WIFI_SET_ROAMING_PARA_REQ, (oslParaType*)p, NULL);
}


/*****************************************************************************
 * FUNCTION
 srv_dtcnt_wlan_set_dwell_time_req
 * DESCRIPTION
 *  
 * PARAMETERS
 *  void
 ...
 *  void
 *****************************************************************************/
void srv_dtcnt_wlan_set_dwell_time_req(U16 dwell_time)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    mmi_abm_wifi_set_scan_dwell_time_req_struct *p;    

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    SetProtocolEventHandler(srv_dtcnt_wlan_set_dwell_time_cnf_hdlr, MSG_ID_MMI_ABM_WIFI_SET_SCAN_DWELL_TIME_CNF);

    p = OslAllocDataPtr(mmi_abm_wifi_set_scan_dwell_time_req_struct);
    p->u2ChannelDwellTime = dwell_time;
    
    mmi_frm_send_ilm(MOD_ABM, MSG_ID_MMI_ABM_WIFI_SET_SCAN_DWELL_TIME_REQ, (oslParaType*)p, NULL);
}
 ...
 * DESCRIPTION
 *  These information will be initialized to the default value after WLAN INIT.
 * PARAMETERS
 *  void         
 * RETURNS
 *  void
 *****************************************************************************/
void srv_dtcnt_wlan_init_wlan(void)
{
#if defined (__MMI_WLAN_FEATURES__) && defined(__MTK_TARGET__) && defined(__TCPIP__)
    #if 0
/* under construction !*/
/* under construction !*/
    #else    
    g_srv_dtcnt_wlan_scan_setting_ctx.dwell_time = 60;
    g_srv_dtcnt_wlan_scan_setting_ctx.is_roaming = MMI_TRUE;
    g_srv_dtcnt_wlan_scan_setting_ctx.ssid_list_num = 0;
    memset(g_srv_dtcnt_wlan_scan_setting_ctx.ssid_list, 0, sizeof(srv_dtcnt_ssid_struct) * DTCNT_WLAN_MAX_SPEC_SSID_LIST_NUM);
    #endif
#endif
}
...
void srv_dtcnt_wlan_context_init(void)
{
    ...
    for (i = 0; i < SRV_DTCNT_MAX_WLAN_PROXY_EXCLUSIVE_IP_ADDR_NUM; i++)
    {
        ...
        else
        {
            ...
            MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, SRV_DTCNT_WLAN_DEL_URL, g_srv_dtcnt_wlan_proxy_exclusive_ip_addr_list.ips[i].ip_addr);            
...
#if (defined(__MMI_TETHERING_WIFI__) && !defined(__WIFI_AP_STA_CONCURRENCY__))
...
void srv_dtcnt_wlan_wifi_init_cnf_hdlr(void *info)
{
    ...
    SetProtocolEventHandler(srv_dtcnt_wlan_send_sniffer_mode_req, MSG_ID_MMI_ABM_WIFI_SET_SNIFFER_MODE_REQ);
#endif
#ifdef __WPS_SUPPORT__
...
void srv_dtcnt_wlan_connect_cnf_hdlr(void *info)
{
    ...
    if (cnf->result == KAL_TRUE)
    {
        ...
        if (!g_srv_dtcnt_wlan_ctx.connected_wlan_profile && g_srv_dtcnt_wlan_conn_prof.profile_id == 0xFFFFFFFF)
        {
            g_srv_dtcnt_wlan_ctx.connected_wlan_profile = &g_srv_dtcnt_wlan_conn_prof;
        }
        ...
        g_srv_dtcnt_wlan_ctx.cur_ap_info.rssi = cnf->rssi;
...
void srv_dtcnt_wlan_disconnect_ind_hdlr(void *info)
{
    ...
    memset(&g_srv_dtcnt_wlan_conn_prof, 0, sizeof(srv_dtcnt_prof_wlan_struct));
...
/*****************************************************************************
 * FUNCTION
 *  srv_dtcnt_wlan_set_scan_desired_ssid_list_cnf_hdlr
 * DESCRIPTION
 *  
 * PARAMETERS
 *  info        [IN]        Local parameters from protocol
 ...
 *  void
 *****************************************************************************/
void srv_dtcnt_wlan_set_scan_desired_ssid_list_cnf_hdlr(void *info)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    mmi_abm_wifi_set_scan_desired_ssid_list_cnf_struct *cnf;
    srv_dtcnt_wlan_cb_func_ptr callback;
    
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/  
    cnf = (mmi_abm_wifi_set_scan_desired_ssid_list_cnf_struct*) info;

    //MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, SRV_DTCNT_WLAN_SET_PS_MODE_CNF_HDLR,
    //   g_srv_dtcnt_wlan_ctx.prev_state, g_srv_dtcnt_wlan_ctx.state, cnf->ps_mode, cnf->status);

    ClearProtocolEventHandler(MSG_ID_MMI_ABM_WIFI_SET_SCAN_DESIRED_SSID_LIST_CNF);

    /* check set spec ssid list job exist */
    if (g_srv_dtcnt_wlan_ctx.set_spec_ssid_list_job)
    {
        callback = (srv_dtcnt_wlan_cb_func_ptr)g_srv_dtcnt_wlan_ctx.set_spec_ssid_list_job->cb_func_ptr;
        if (cnf->status == KAL_TRUE)
        {
            /* Update the spec ssid list information in dtcnt srv. */
            g_srv_dtcnt_wlan_scan_setting_ctx.ssid_list_num = g_srv_dtcnt_wlan_ctx.set_spec_ssid_list_job->ssid_list_num;            
            memcpy(g_srv_dtcnt_wlan_scan_setting_ctx.ssid_list,
                   g_srv_dtcnt_wlan_ctx.set_spec_ssid_list_job->ssid_list, 
                   sizeof(srv_dtcnt_ssid_struct) * g_srv_dtcnt_wlan_scan_setting_ctx.ssid_list_num);
...
/* under construction !*/
            ...
            if (callback)
            {
                callback(g_srv_dtcnt_wlan_ctx.set_spec_ssid_list_job->user_data, SRV_DTCNT_WLAN_REQ_RES_DONE);
            }
        }
        else
        {
            if (callback)
            {
                callback(g_srv_dtcnt_wlan_ctx.set_spec_ssid_list_job->user_data, SRV_DTCNT_WLAN_REQ_RES_FAILED);
            }
        }
        OslMfree(g_srv_dtcnt_wlan_ctx.set_spec_ssid_list_job);
        g_srv_dtcnt_wlan_ctx.set_spec_ssid_list_job = NULL;
    }    
}
...
/*****************************************************************************
 * FUNCTION
 *  srv_dtcnt_wlan_set_roaming_para_cnf_hdlr
 * DESCRIPTION
 *  
 * PARAMETERS
 *  info        [IN]        Local parameters from protocol
 ...
 *  void
 *****************************************************************************/
void srv_dtcnt_wlan_set_roaming_para_cnf_hdlr(void *info)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    mmi_abm_wifi_set_roaming_para_cnf_struct *cnf;
    srv_dtcnt_wlan_cb_func_ptr callback;
    
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/  
    cnf = (mmi_abm_wifi_set_roaming_para_cnf_struct*) info;

    //MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, SRV_DTCNT_WLAN_SET_PS_MODE_CNF_HDLR,
    //   g_srv_dtcnt_wlan_ctx.prev_state, g_srv_dtcnt_wlan_ctx.state, cnf->ps_mode, cnf->status);

    ClearProtocolEventHandler(MSG_ID_MMI_ABM_WIFI_SET_ROAMING_PARA_CNF);

    /* check set roaming job exist */
    if (g_srv_dtcnt_wlan_ctx.set_roaming_para_job)
    {
        callback = (srv_dtcnt_wlan_cb_func_ptr)g_srv_dtcnt_wlan_ctx.set_roaming_para_job->cb_func_ptr;
        if (cnf->status == KAL_TRUE)
        {
            /* Update roaming parameter in dtcnt srv. */
            g_srv_dtcnt_wlan_scan_setting_ctx.is_roaming = g_srv_dtcnt_wlan_ctx.set_roaming_para_job->enable_roaming;
...
/* under construction !*/
            ...
            if (callback)
            {
                callback(g_srv_dtcnt_wlan_ctx.set_roaming_para_job->user_data, SRV_DTCNT_WLAN_REQ_RES_DONE);
            }   
        }
        else
        {
            if (callback)
            {
                callback(g_srv_dtcnt_wlan_ctx.set_roaming_para_job->user_data, SRV_DTCNT_WLAN_REQ_RES_FAILED);
            }
        }
        OslMfree(g_srv_dtcnt_wlan_ctx.set_roaming_para_job);
        g_srv_dtcnt_wlan_ctx.set_roaming_para_job = NULL;
    }
}
...
/*****************************************************************************
 * FUNCTION
 *  srv_dtcnt_wlan_set_dwell_time_cnf_hdlr
 * DESCRIPTION
 *  
 * PARAMETERS
 *  info        [IN]        Local parameters from protocol
 ...
 *  void
 *****************************************************************************/
void srv_dtcnt_wlan_set_dwell_time_cnf_hdlr(void *info)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    mmi_abm_wifi_set_scan_dwell_time_cnf_struct *cnf;
    srv_dtcnt_wlan_cb_func_ptr callback;
    
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/  
    cnf = (mmi_abm_wifi_set_scan_dwell_time_cnf_struct*) info;

    //MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, SRV_DTCNT_WLAN_SET_PS_MODE_CNF_HDLR,
    //   g_srv_dtcnt_wlan_ctx.prev_state, g_srv_dtcnt_wlan_ctx.state, cnf->ps_mode, cnf->status);

    ClearProtocolEventHandler(MSG_ID_MMI_ABM_WIFI_SET_SCAN_DWELL_TIME_CNF);

    /* check set spec ssid list job exist */
    if (g_srv_dtcnt_wlan_ctx.set_dwell_time_job)
    {
        callback = (srv_dtcnt_wlan_cb_func_ptr)g_srv_dtcnt_wlan_ctx.set_dwell_time_job->cb_func_ptr;
        if (cnf->status == KAL_TRUE)
        {
            /* Update roaming parameter in dtcnt srv. */
            g_srv_dtcnt_wlan_scan_setting_ctx.dwell_time = g_srv_dtcnt_wlan_ctx.set_dwell_time_job->dwell_time;
...
/* under construction !*/
            ...
            if (callback)
            {
                callback(g_srv_dtcnt_wlan_ctx.set_dwell_time_job->user_data, SRV_DTCNT_WLAN_REQ_RES_DONE);
            }  
        }
        else
        {
            if (callback)
            {
                callback(g_srv_dtcnt_wlan_ctx.set_dwell_time_job->user_data, SRV_DTCNT_WLAN_REQ_RES_FAILED);
            }
        }
        OslMfree(g_srv_dtcnt_wlan_ctx.set_dwell_time_job);
        g_srv_dtcnt_wlan_ctx.set_dwell_time_job = NULL;
    }
}
...
U32 srv_dtcnt_wlan_get_total_prof_num(void)
{
    return (g_srv_dtcnt_wlan_setting_ctx.adhoc_prof_num + g_srv_dtcnt_wlan_setting_ctx.infra_prof_num);
}


#ifdef __WIFI_SNIFFER_SUPPORT__
void srv_dtcnt_wlan_add_sniffer_proc_job(srv_dtcnt_wlan_snif_job_type_enum action, srv_dtcnt_wlan_job_struct *job)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    srv_dtcnt_wlan_sniffer_job_list *proc_list = OslMalloc(sizeof(srv_dtcnt_wlan_sniffer_job_list));
    ...
    proc_list->next = NULL;
    proc_list->action = action;
    kal_mem_cpy(&(proc_list->job), job, sizeof(srv_dtcnt_wlan_job_struct));
    
    if (!g_srv_dtcnt_wlan_ctx.sniffer_job_list)
    {
        g_srv_dtcnt_wlan_ctx.sniffer_job_list = proc_list;
    }
    else
    {
        srv_dtcnt_wlan_sniffer_job_list *cur, *next;
        cur = g_srv_dtcnt_wlan_ctx.sniffer_job_list;
        next = g_srv_dtcnt_wlan_ctx.sniffer_job_list->next;
        while (next)
        {
            cur = next;
            next = cur->next;
    ...
    //kal_prompt_trace(MOD_MMI_CONN_APP, "[DTCNT Srv API] add sniffer job: %x, action: %d", proc_list->job.cb_func_ptr, proc_list->action);
    MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, SRV_DTCNT_WLAN_ADD_SNIF_JOB, proc_list->job.cb_func_ptr, proc_list->action);
}

void srv_dtcnt_wlan_get_sniffer_proc_job(srv_dtcnt_wlan_snif_job_type_enum action, srv_dtcnt_wlan_job_struct *job)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    srv_dtcnt_wlan_sniffer_job_list *cur_job, *pre_job = NULL;
    ...
    cur_job = g_srv_dtcnt_wlan_ctx.sniffer_job_list;

    while (cur_job)
    {
        if (cur_job->action == action)
        {
            kal_mem_cpy(job, &(cur_job->job), sizeof(srv_dtcnt_wlan_job_struct));
            if (pre_job)
            {
                pre_job->next = cur_job->next;
            }
            else
            {
                g_srv_dtcnt_wlan_ctx.sniffer_job_list = NULL;
            }

            OslMfree(cur_job);
            //kal_prompt_trace(MOD_MMI_CONN_APP, "[DTCNT Srv API] Get sniffer job: %x, action: \n", job->cb_func_ptr, action);
            MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, SRV_DTCNT_WLAN_GET_SNIF_JOB, job->cb_func_ptr, action);            
            break;
        }
...
/* Map SP auth mode into Wifi Srv auth mode. */
static srv_dtcnt_wlan_auth_mode_enum srv_dtcnt_wlan_authmode_sp2fp_map(srv_dtcnt_wlan_sp_auth_mode_enum sp_am)
{	
	switch (sp_am)
	{
		case SRV_DTCNT_WLAN_SP_AUTH_MODE_OPEN:
		{
			return SRV_DTCNT_WLAN_AUTH_MODE_OPEN;
		}

		/* SP will not send this auth mode to FP. */
		case SRV_DTCNT_WLAN_SP_AUTH_MODE_SHARED:
		{
			return SRV_DTCNT_WLAN_AUTH_MODE_WEP;
		}

		case SRV_DTCNT_WLAN_SP_AUTH_MODE_WPA:
		{
			return SRV_DTCNT_WLAN_AUTH_MODE_WPA_ONLY;
		}

		case SRV_DTCNT_WLAN_SP_AUTH_MODE_WPA2:
		{
			return SRV_DTCNT_WLAN_AUTH_MODE_WPA2_ONLY;
		}

		case SRV_DTCNT_WLAN_SP_AUTH_MODE_WAP1WPA2_PSK:
		case SRV_DTCNT_WLAN_SP_AUTH_MODE_WPA_PSK:
		{
			return SRV_DTCNT_WLAN_AUTH_MODE_WPA_ONLY_PSK;
		}

		case SRV_DTCNT_WLAN_SP_AUTH_MODE_WPA2_PSK:
		{
			return SRV_DTCNT_WLAN_AUTH_MODE_WPA2_ONLY_PSK;
		}
		
		case SRV_DTCNT_WLAN_SP_AUTH_MODE_WAP1WPA2:
		{
			return SRV_DTCNT_WLAN_AUTH_MODE_WPA_WPA2_MIX;
		}	

		/*case SRV_DTCNT_WLAN_SP_AUTH_MODE_WAP1WPA2_PSK:
		{
			return SRV_DTCNT_WLAN_AUTH_MODE_WPA_WPA2_MIX_PSK;
		}*/
		
		default:
		{
			return SRV_DTCNT_WLAN_AUTH_MODE_TOTAL;
		}
	}
}

static MMI_BOOL srv_dtcnt_wlan_sniffer_ap_info_map(
                    srv_dtcnt_prof_wlan_struct * dtcnt_prof,
                    mmi_abm_iot_listened_ap_info_ind_struct* ap_info)
{   
    S32 i;
    if (!dtcnt_prof || !ap_info)
    {
        kal_prompt_trace(MOD_MMI_CONN_APP, "NULL");
        return MMI_FALSE;
    }

    memset(dtcnt_prof, 0, sizeof(srv_dtcnt_prof_wlan_struct));    

    kal_prompt_trace(MOD_MMI_CONN_APP, "auth_mode: %d", ap_info->auth_mode);

	/* WNDRV will not check the value of auth mode. In other words it will pass the value SP sent directly. */
	dtcnt_prof->auth_mode = srv_dtcnt_wlan_authmode_sp2fp_map((srv_dtcnt_wlan_sp_auth_mode_enum)ap_info->auth_mode);
    ...
    switch (dtcnt_prof->auth_mode)
    {
        ...
        case SRV_DTCNT_WLAN_AUTH_MODE_WPA_ONLY_PSK:
        {
            if (SRV_DTCNT_PROF_MAX_PASSPHRASE_LEN < ap_info->password_len)
            {
                //return MMI_FALSE;
                ap_info->password_len = SRV_DTCNT_PROF_MAX_PASSPHRASE_LEN;
            }
            
            memcpy(dtcnt_prof->passphrase, ap_info->password, ap_info->password_len);
            break;
        }
        
        case SRV_DTCNT_WLAN_AUTH_MODE_WEP:
        {
            if (SRV_DTCNT_PROF_MAX_WEP_KEY_LEN < ap_info->password_len)
            {
                //return MMI_FALSE;
                ap_info->password_len = SRV_DTCNT_PROF_MAX_WEP_KEY_LEN;
            }
            
            memcpy(dtcnt_prof->wep_key, ap_info->password, ap_info->password_len);
            break;            
        }
		...
		/* SP to FP auth mode mapping failed. */
		case SRV_DTCNT_WLAN_AUTH_MODE_TOTAL:
		{
			kal_prompt_trace(MOD_MMI_CONN_APP, "SP2FP authmode map failed:%d", dtcnt_prof->auth_mode);
			return MMI_FALSE;
		}
    ...
    //dtcnt_prof->auth_mode = ap_info->auth_mode;
    dtcnt_prof->ssid_len = ap_info->ssid_len;
    memcpy(dtcnt_prof->ssid, ap_info->ssid, ap_info->ssid_len);
    for(i = 0; i < 10; i++)
    {
        kal_prompt_trace(MOD_MMI_CONN_APP, "SSID[%d]:%d", i, dtcnt_prof->ssid[i]);
        kal_prompt_trace(MOD_MMI_CONN_APP, "password[%d]:%d", i, dtcnt_prof->password[i]);
    }
    return MMI_TRUE;
}


void srv_dtcnt_wlan_sniffer_ap_info_ind_hdlr(void *info)
{
    mmi_abm_iot_listened_ap_info_ind_struct * ap_info = (mmi_abm_iot_listened_ap_info_ind_struct *)info;
    srv_dtcnt_wlan_sniffer_on_res_struct res;
    srv_dtcnt_prof_wlan_struct *dtcnt_prof = NULL;
    srv_dtcnt_wlan_job_struct job;
    srv_dtcnt_wlan_sniffer_on_cb_func_ptr callback;
    
    //kal_prompt_trace(MOD_MMI_CONN_APP, "[DTCNT Srv API] srv_dtcnt_wlan_sniffer_ap_info_ind_hdlr()\n");
    MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, SRV_DTCNT_WLAN_SNIF_AP_INFO_HDL);
    ClearProtocolEventHandler(MSG_ID_MMI_ABM_IOT_LISTENED_AP_INFO_IND);    
    
    srv_dtcnt_wlan_get_sniffer_proc_job(SRV_DTCNT_WLAN_SNIF_NOTI_AP, &job);
    if (job.cb_func_ptr)
    {
        res.cb_type = SRV_DTCNT_WLAN_SNIFFER_ON_AP_INFO;
        res.result = SRV_DTCNT_WLAN_SNIFFER_SUCCESS;        
        dtcnt_prof = OslMalloc(sizeof(srv_dtcnt_prof_wlan_struct));
        res.profile = dtcnt_prof;
        kal_prompt_trace(MOD_MMI_CONN_APP, "dtcnt_prof:%d", dtcnt_prof);
        
        if (!srv_dtcnt_wlan_sniffer_ap_info_map(dtcnt_prof, ap_info))
        {
            res.result = SRV_DTCNT_WLAN_SNIFFER_FAIL_UNKNOWN;
			res.profile = NULL;
        }
        
        callback = (srv_dtcnt_wlan_sniffer_on_cb_func_ptr)job.cb_func_ptr;
        callback(job.user_data, &res);
...
/*****************************************************************************
 * FUNCTION
 *  srv_dtcnt_wlan_set_sniffer_mode_cnf_hdlr
 * DESCRIPTION
 *  MSG_ID_MMI_ABM_WIFI_SET_SNIFFER_MODE_CNF handler
 * PARAMETERS
 *  info        [IN]        Local parameters from protocol
 * RETURNS
 *  void
 *****************************************************************************/
void srv_dtcnt_wlan_set_sniffer_mode_cnf_hdlr(void *info)
{
    mmi_abm_wifi_set_sniffer_mode_cnf_struct* cnf = (mmi_abm_wifi_set_sniffer_mode_cnf_struct*)info;
    srv_dtcnt_wlan_job_struct job;
    srv_dtcnt_wlan_sniffer_on_cb_func_ptr on_callback;
    srv_dtcnt_wlan_cb_func_ptr off_callback;
    srv_dtcnt_wlan_sniffer_on_res_struct on_res;
    srv_dtcnt_wlan_sniffer_off_cb_func_ptr off_res;

    //kal_prompt_trace(MOD_MMI_CONN_APP, "[DTCNT Srv API] srv_dtcnt_wlan_set_sniffer_mode_cnf_hdlr()");
    MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, SRV_DTCNT_WLAN_SET_SNIF_MODE_CNF);
    if (!(--g_srv_dtcnt_wlan_ctx.sniffer_mode_req_count))
    {
        ClearProtocolEventHandler(MSG_ID_MMI_ABM_WIFI_SET_SNIFFER_MODE_CNF);
    }
    ...
    switch (cnf->OnOffStatus)
    {
        case 0: /* CNF of sniffer mode off */
        {            
            //kal_prompt_trace(MOD_MMI_CONN_APP, "[DTCNT Srv API] Sniffer off cnf. result: %d\n", cnf->status);
            MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, SRV_DTCNT_WLAN_SET_SNIF_OFF_CNF, cnf->status);
            if (SNIFFER_MODE_SUCCESS == cnf->status)
            {
                g_srv_dtcnt_wlan_ctx.state = SRV_DTCNT_WLAN_STATE_INIT;                
            }
            else
            {
                /* sniffer_off is banned when network is connected. So SNIFFER_MODE_FAIL_NETWORK_IS_CONNECTED 
                 * will not be received. 
                 */
                g_srv_dtcnt_wlan_ctx.state = SRV_DTCNT_WLAN_STATE_SNIFFER_ON;                
            }
            
            srv_dtcnt_wlan_get_sniffer_proc_job(SRV_DTCNT_WLAN_SNIF_OFF, &job);
            if (job.cb_func_ptr)
            {
                off_callback = job.cb_func_ptr;
                off_callback(job.user_data, (srv_dtcnt_wlan_req_res_enum)cnf->status);
            }
            
            break;    
        }

        case 1: /* CNF of sniffer mode on without para */
        case 2: /* CNF of sniffer mode on with para */
        {
            //kal_prompt_trace(MOD_MMI_CONN_APP, "[DTCNT Srv API] sniffer on cnf. result: %d\n", cnf->status);
            MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, SRV_DTCNT_WLAN_SET_SNIF_ON_CNF, cnf->status);
            if (SNIFFER_MODE_SUCCESS == cnf->status)
            {
                g_srv_dtcnt_wlan_ctx.state = SRV_DTCNT_WLAN_STATE_SNIFFER_ON;               
            }
            ...
            srv_dtcnt_wlan_get_sniffer_proc_job(SRV_DTCNT_WLAN_SNIF_ON, &job);

            if (job.cb_func_ptr)
            {
                if (SNIFFER_MODE_SUCCESS == cnf->status)
                {                    
                    srv_dtcnt_wlan_add_sniffer_proc_job(SRV_DTCNT_WLAN_SNIF_NOTI_AP, &job);
                    SetProtocolEventHandler(srv_dtcnt_wlan_sniffer_ap_info_ind_hdlr, MSG_ID_MMI_ABM_IOT_LISTENED_AP_INFO_IND);    
                    //kal_prompt_trace(MOD_MMI_CONN_APP, "[DTCNT Srv API] Wait for AP information gathered by sniffer\n");
                    MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, SRV_DTCNT_WLAN_WAIT_AP_INFO);
                }
                
                on_callback = (srv_dtcnt_wlan_sniffer_on_cb_func_ptr)job.cb_func_ptr;
                on_res.cb_type = SRV_DTCNT_WLAN_SNIFFER_ON_RES;
                on_res.profile = NULL;
                on_res.result = (srv_dtcnt_wlan_sniffer_res_enum)cnf->status;
                on_callback(job.user_data, &on_res);
            }            
            break;
        }
    }
 ...
 *  void
 *****************************************************************************/
void srv_dtcnt_wlan_send_sniffer_mode_req(sniffer_mode_onoff_enum mode, srv_dtcnt_wlan_sniffer_on_cfg_struct *cfg)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    mmi_abm_wifi_set_sniffer_mode_req_struct *p;   

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    p = OslAllocDataPtr(mmi_abm_wifi_set_sniffer_mode_req_struct);
    switch (mode)
    {
        case SNIFFER_MODE_OFF:
        {
            if(SRV_DTCNT_WLAN_STATE_SNIFFER_OFF_WAIT == g_srv_dtcnt_wlan_ctx.state)
            {
                return;
            }
            
            g_srv_dtcnt_wlan_ctx.state = SRV_DTCNT_WLAN_STATE_SNIFFER_OFF_WAIT;
            p->ucOnOffSwitch = SNIFFER_MODE_OFF;
            //kal_prompt_trace(MOD_MMI_CONN_APP, "[DTCNT Srv API] Sniffer off req\n");
            MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, SRV_DTCNT_WLAN_SET_SNIF_OFF_REQ);
            break;    
        }

        case SNIFFER_MODE_ON_WITHOUT_PARA:
        {
            if(SRV_DTCNT_WLAN_STATE_SNIFFER_ON_WAIT == g_srv_dtcnt_wlan_ctx.state)
            {
                return;
            }            
            g_srv_dtcnt_wlan_ctx.state = SRV_DTCNT_WLAN_STATE_SNIFFER_ON_WAIT;
            p->ucOnOffSwitch = SNIFFER_MODE_ON_WITHOUT_PARA;
            //kal_prompt_trace(MOD_MMI_CONN_APP, "[DTCNT Srv API] Sniffer on req without param\n");
            MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, SRV_DTCNT_WLAN_SET_SNIF_ON_NP_REQ);
            break;    
        }

        case SNIFFER_MODE_ON_WITH_PARA:
        {
            if(SRV_DTCNT_WLAN_STATE_SNIFFER_ON_WAIT == g_srv_dtcnt_wlan_ctx.state || !cfg)
            {
                return;
            }            
            g_srv_dtcnt_wlan_ctx.state = SRV_DTCNT_WLAN_STATE_SNIFFER_ON_WAIT;
            p->ucOnOffSwitch = SNIFFER_MODE_ON_WITHOUT_PARA;
            p->ucSnifferFilterOptions = cfg->sniffer_filter_op;
            p->ucChannelNum = cfg->channel_num;
            p->cMinRssi = cfg->min_rssi;
            p->cMaxRssi = cfg->max_rssi;
            p->u2MinPacketLength = cfg->min_pack_len;
            p->u2MaxPacketLength = cfg->max_pack_len; 
            //kal_prompt_trace(MOD_MMI_CONN_APP, "[DTCNT Srv API] Sniffer on req with param\n");
            MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, SRV_DTCNT_WLAN_SET_SNIF_ON_WP_REQ);
            break;    
        }
    ...
    g_srv_dtcnt_wlan_ctx.sniffer_mode_req_count++;
    SetProtocolEventHandler(srv_dtcnt_wlan_set_sniffer_mode_cnf_hdlr, MSG_ID_MMI_ABM_WIFI_SET_SNIFFER_MODE_CNF);    
    mmi_frm_send_ilm(MOD_ABM, MSG_ID_MMI_ABM_WIFI_SET_SNIFFER_MODE_REQ, (oslParaType*)p, NULL);
}


srv_dtcnt_result_enum srv_dtcnt_wlan_sniffer_on(
                        ...
                        void *user_data)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/            
    //kal_prompt_trace(MOD_MMI_CONN_APP, "[WLAN Srv API] srv_dtcnt_wlan_sniffer_on()\n");
    // Check the wifi chip
    if(srv_dtcnt_wlan_check_chip_status() == MMI_FALSE)
    {
        if (callback != NULL)
        {
            res.cb_type = SRV_DTCNT_WLAN_SNIFFER_ON_RES;
            res.profile = NULL;
            res.result = SRV_DTCNT_WLAN_SNIFFER_FAIL_HW_ERROR;
    ...
    switch (srv_dtcnt_wlan_state())
    {
        case SRV_DTCNT_WLAN_STATE_INIT:
        {
            if (callback)
            {
                new_wlan_job.cb_func_ptr = (void *)callback;
                new_wlan_job.user_data = user_data;
                srv_dtcnt_wlan_add_sniffer_proc_job(SRV_DTCNT_WLAN_SNIF_ON, &new_wlan_job);
                // Send req
                if (sniffer_on_config)
                {
                    srv_dtcnt_wlan_send_sniffer_mode_req(SNIFFER_MODE_ON_WITH_PARA, sniffer_on_config);
                }
                else
                {
                    srv_dtcnt_wlan_send_sniffer_mode_req(SNIFFER_MODE_ON_WITHOUT_PARA, NULL);
                }                
            }
            return SRV_DTCNT_RESULT_PROCESSING;
        }
        ...
        default:
        {
            if (callback != NULL)
            {
                res.cb_type = SRV_DTCNT_WLAN_SNIFFER_ON_RES;
                res.profile = NULL;
                res.result = SRV_DTCNT_WLAN_SNIFFER_FAIL_STATE_ERROR;
                callback(user_data, &res);
            }
            return SRV_DTCNT_RESULT_FAILED;
        }               
...
srv_dtcnt_result_enum srv_dtcnt_wlan_sniffer_off(
                        srv_dtcnt_wlan_sniffer_off_cb_func_ptr callback,
                        void *user_data)
{
    srv_dtcnt_wlan_job_struct new_wlan_job;
    ...
    if (SRV_DTCNT_WLAN_STATE_SNIFFER_ON != srv_dtcnt_wlan_state())
    {
        if (callback != NULL)
        {
            callback(user_data, SRV_DTCNT_WLAN_REQ_RES_FAILED);
    ...
    if (callback)
    {
        new_wlan_job.cb_func_ptr = (void *)callback;
        new_wlan_job.user_data = user_data;
        srv_dtcnt_wlan_add_sniffer_proc_job(SRV_DTCNT_WLAN_SNIF_OFF, &new_wlan_job);
        // Send req
        srv_dtcnt_wlan_send_sniffer_mode_req(SNIFFER_MODE_OFF, NULL);              
    }
    return SRV_DTCNT_RESULT_PROCESSING;
}
#endif
...
srv_dtcnt_prof_wlan_struct *srv_dtcnt_get_wlan_conn_prof(void)
{
    return &g_srv_dtcnt_wlan_conn_prof;
}
