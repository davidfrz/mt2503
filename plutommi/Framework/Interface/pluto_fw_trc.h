...
#define MMI_FW_TRC_INPUT_EVT	  TRACE_GROUP_6	
...
BEGIN_TRACE_MAP(MOD_PLUTO_FW)
    ...
    TRC_MSG(TRC_MMI_FRM_NVRAM_READ_VALUE_BEGIN, "[MMI_FRM][NVRAM CACHE] ReadValue <type = %d, value id = %d, dataReadFlag = %d>")
    TRC_MSG(TRC_MMI_FRM_NVRAM_WRITE_VALUE_BEGIN, "[MMI_FRM][NVRAM CACHE] WriteValue <type = %d, value id = %d, dataWriteFlashFlag = %d>")
...
//    TRC_MSG(TRC_MMI_FRM_HIST_FREE_HIST_BUFF_ERR, "[MMI_FRM][Scenario][History] mmi_frm_free_history_buffer <Fail> <scrn_id = %Mmmi_screen_id_cgen_enum>")
	...
	//__MMI_FRM_INPUT_EVT__
	TRC_MSG(TRC_MMI_FRM_INPUT_EVT_INFO_INIT,"[INPUT_EVT][INIT]<%s><read record%d from NVRAM LID failed, error_code=%d>") 
 
    TRC_MSG(TRC_MMI_FRM_INPUT_EVT_SEND,"[INPUT_EVT][EVT_HANDLE]<send><evt_id=%Mmmi_scenario_evt_enum, key_code =%Mmmi_keypads_enum, key_type=%Mmmi_key_types_enum>") 
    TRC_MSG(TRC_MMI_FRM_INPUT_EVT_SEND_END,"[INPUT_EVT][EVT_HANDLE]<send_end><evt_id=%Mmmi_scenario_evt_enum>") 
	
	TRC_MSG(TRC_MMI_FRM_INPUT_EVT_PROC,"[INPUT_EVT][EVT_HANDLE]<group proc><evt_id=%d, key_code =%Mmmi_keypads_enum, key_type=%Mmmi_key_types_enum>") 

	TRC_MSG(TRC_MMI_FRM_INPUT_EVT_FIRST_APP_LAUNCHING,"[INPUT_EVT][FIRST_APP]<launching failed><evt_id=%Mmmi_scenario_evt_enum, current_local_app_index=%Mmmi_btbox_app_indx_enum, launch next app..>") 
	TRC_MSG(TRC_MMI_FRM_INPUT_EVT_FIRST_APP_LAUNCHED,"[INPUT_EVT][FIRST_APP]<launched><evt_id=%Mmmi_scenario_evt_enum, current_local_app_index=%Mmmi_btbox_app_indx_enum>") 

	
	TRC_MSG(TRC_MMI_FRM_INPUT_EVT_NOTIFY,"[INPUT_EVT][NOTIFY]<notify><current hdlr notify_id=%d, led=%Mmmi_input_event_led_enum, audio=%d, vib=%Mmmi_input_event_vib_enum>") 
	TRC_MSG(TRC_MMI_FRM_INPUT_EVT_SWITCH_APP,"[INPUT_EVT][SWITCH_APP]<target app><app_group_id=%d, current_local_app_index=%Mmmi_btbox_app_indx_enum>") 
	TRC_MSG(TRC_MMI_FRM_INPUT_EVT_SWITCH_NEXT_APP,"[INPUT_EVT][SWITCH_NEXT_APP]<next app><current app not launch, try to launch the next>") 
	TRC_MSG(TRC_MMI_FRM_INPUT_EVT_LAUNCH_APP_FAILED,"[INPUT_EVT][LAUNCH_APP]<failed><local app not launch>") 
	TRC_MSG(TRC_MMI_FRM_INPUT_EVT_LAUNCH_EVT_MISSED,"[INPUT_EVT][LAUNCH_APP]<missed><local app %d launch event missed>")
	

	TRC_MSG(TRC_MMI_FRM_INPUT_EVT_NOTIFY_ADD,"[INPUT_EVT][NOTIFY]<add><notify_id=%d, led=%Mmmi_btbox_led_enum, audio=%d, vib=%Mmmi_btbox_vib_enum>") 
	TRC_MSG(TRC_MMI_FRM_INPUT_EVT_NOTIFY_REMOVE,"[INPUT_EVT][NOTIFY]<remove><notify_id=%d, from queue>")
	TRC_MSG(TRC_MMI_FRM_INPUT_EVT_NOTIFY_REMOVE_TOP,"[INPUT_EVT][NOTIFY]<remove><notify_id=%d, from top>")
	TRC_MSG(TRC_MMI_FRM_INPUT_EVT_NOTIFY_AUD,"[INPUT_EVT][NOTIFY]<aud><aud_id=%d, aud_cb=%x, ret=%d>") 
	TRC_MSG(TRC_MMI_FRM_INPUT_EVT_NOTIFY_AUD_COMPLETED,"[INPUT_EVT][NOTIFY]<aud_completed><execute aud_cb=%x>")
	TRC_MSG(TRC_MMI_FRM_INPUT_EVT_NOTIFY_LED_FLASH,"[INPUT_EVT][NOTIFY]<led><led in notify_id=%d, is flash>") 
	TRC_MSG(TRC_MMI_FRM_INPUT_EVT_NOTIFY_LED_ADD_TO_Q,"[INPUT_EVT][NOTIFY]<led><led in notify_id=%d, low priority,add to queue>")
	TRC_MSG(TRC_MMI_FRM_INPUT_EVT_NOTIFY_LED_TOP_TO_Q,"[INPUT_EVT][NOTIFY]<led><led in notify_id=%d, high priority, add to top>")
	TRC_MSG(TRC_MMI_FRM_INPUT_EVT_NOTIFY_LED_EXISTED,"[INPUT_EVT][NOTIFY]<led><notify_id=%d, already exist in queue>")	
	TRC_MSG(TRC_MMI_FRM_INPUT_EVT_NOTIFY_LED_LONG,"[INPUT_EVT][NOTIFY]<led_show><long: led in notify_id=%d>") 
	
	TRC_MSG(TRC_MMI_FRM_INPUT_EVT_NOTIFY_STOP,"[INPUT_EVT][NOTIFY]<stop><stop current notify>")
	TRC_MSG(TRC_MMI_FRM_INPUT_EVT_NOTIFY_ENABLE,"[INPUT_EVT][NOTIFY]<enable><enable current notify>")
	TRC_MSG(TRC_MMI_FRM_INPUT_EVT_NOTIFY_DISABLE,"[INPUT_EVT][NOTIFY]<disable><disable current notify>")
	TRC_MSG(TRC_MMI_FRM_INPUT_EVT_NOTIFY_DISABLED,"[INPUT_EVT][NOTIFY]<disabled><disable all notify>")

	//ASSERT
	TRC_MSG(TRC_MMI_FRM_INPUT_EVT_NOTIFY_ID_UNDEFINED,"[INPUT_EVT][NOTIFY]<query><notify id=%d undefined>")
	TRC_MSG(TRC_MMI_FRM_INPUT_EVT_NOTIFY_ID_NOT_EXISTED,"[INPUT_EVT][NOTIFY]<query><notify id=%d not existed>")
	TRC_MSG(TRC_MMI_FRM_INPUT_EVT_NOTIFY_LED_QUEUE_NOT_ENOUGH,"[INPUT_EVT][NOTIFY]<add><notify led queue not enough>")
	TRC_MSG(TRC_MMI_FRM_INPUT_EVT_NOTIFY_LED_QUEUE_INIT_FAIL,"[INPUT_EVT][NOTIFY]<led init><led queue buffer alloc fail>")
	TRC_MSG(TRC_MMI_FRM_INPUT_EVT_NOTIFY_LED_COLOR_INVALID,"[INPUT_EVT][NOTIFY]<led color><led id= %d color invalid>")
	TRC_MSG(TRC_MMI_FRM_INPUT_EVT_NOTIFY_LED_INDEX_INVALID,"[INPUT_EVT][NOTIFY]<led color><led index= %d invalid>")
	TRC_MSG(TRC_MMI_FRM_INPUT_EVT_NOTIFY_LED_SIZE_INVALID,"[INPUT_EVT][NOTIFY]<led size><sizeof(mmi_input_event_led_notify) MUST BE INPUT_EVENT_LED_ID_END:%d>")
	TRC_MSG(TRC_MMI_FRM_INPUT_EVT_NOTIFY_LED_TBL_OUT_OF_ORDER,"[INPUT_EVT][NOTIFY]<ooo>< led:%d SHOULD be in order: %d>")
	TRC_MSG(TRC_MMI_FRM_INPUT_EVT_NOTIFY_VIB_SIZE_INVALID,"[INPUT_EVT][NOTIFY]<vib size><sizeof(mmi_input_event_vib_notify) MUST BE INPUT_EVENT_VIB_ID_END:%d>")
	TRC_MSG(TRC_MMI_FRM_INPUT_EVT_NOTIFY_VIB_TBL_OUT_OF_ORDER,"[INPUT_EVT][NOTIFY]<ooo>< vib:%d SHOULD be in order: %d>")

	TRC_MSG(TRC_MMI_FRM_INPUT_EVT_APP_PROC_MISSED,"[INPUT_EVT][APP]<proc>< app id=%d, proc function missed>")


	TRC_MSG(TRC_MMI_FRM_INPUT_EVT_CONFLICT_WITH_COM_EVENT,"[INPUT_EVT][EVT_TBL]<evt><conflict with common evt:key_code =%Mmmi_keypads_enum, key_type=%Mmmi_key_types_enum>")
	TRC_MSG(TRC_MMI_FRM_INPUT_EVT_CONFLICT_IN_ONE_APP,"[INPUT_EVT][EVT_TBL]<evt><conflict in app id= %d:key_code =%Mmmi_keypads_enum, key_type=%Mmmi_key_types_enum>")
	TRC_MSG(TRC_MMI_FRM_INPUT_EVT_LAUNCH_EVT_CONFIG_INVALID,"[INPUT_EVT][APP_TBL]<local app><app id= %d, launch evt should be in(GLOBAL_BASE, EVT_ID_SCENARIO_MAX)>")
	TRC_MSG(TRC_MMI_FRM_INPUT_EVT_APP_INFO_MISSED,"[INPUT_EVT][APP_TBL]<local app>< app index = %d, app info missed, please check...>")
