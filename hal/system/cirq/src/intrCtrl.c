...
/*************************************************************************
 * Define CSCI data for hibernation resume
 *************************************************************************/
#if defined(__TINY_SYS__)/* dongming add for tiny sys */
#include "wdt_sw.h"
#include "intrCtrl_Btss_User.h"
#include "csci.h"

#define BTSS_EINT_NUM(n)  | ((kal_uint64)0x1 << n)
#define BTSS_CIRQ_CODE(n) | ((kal_uint64)0x1 << n)
#define BTSS_ALL_EINT_BIT_MASK (BTSS_ALL_SPECIAL_EINT_USER)
#define BTSS_ALL_CIRQ_BIT_MASK (BTSS_ALL_SPECIAL_CIRQ_USER)


extern kal_uint64 unknown_irq_from_tiny;
extern kal_uint64 unknown_eint_from_tiny;
extern kal_uint64 unknown_irq_sens_from_tiny;  /* record the  sensitve setting before trigger sw interrupt */
extern kal_uint64 unknown_eint_sens_from_tiny;  /* record the  sensitve setting before trigger sw interrupt */
CSCI_DEFINE_VAR(kal_uint64,ts_unknown_irq)  /* record the	sensitve setting before trigger sw interrupt */
CSCI_DEFINE_VAR(kal_uint64,ts_unknown_eint)  /* record the	sensitve setting before trigger sw interrupt */

CSCI_DEFINE_VAR(kal_uint64,irq_registered_special_cirq) /* record the special cirq which should light the backlight back for Hibernation */
CSCI_DEFINE_VAR(kal_uint64,irq_registered_special_eint) /* record the special eint which should light the backlight back for Hibernation */


//extern BOOT_UP_SOURCE caused_source;
kal_uint8 ADIE_IRQ_Get_Sensitivity(kal_uint8 code);
extern kal_uint8 EINT_Get_Sensitivity(kal_uint8 eintno);
void IRQSensitivity_internal(kal_uint8 code, kal_bool edge);
kal_uint8 IRQ_Query_If_Special_Resume(void);
kal_uint8 IRQ_Query_If_TS_Unknown_Resume(void);
...
/*------------------------------------------------------*/
/* for tiny system, if related sw register is set, we should not clear this */
/* interrupt,until the sw register bit is set 0. */

#if defined(__TINY_SYS__)
{
	kal_uint64 bitmask = 1;
	if(0 != (unknown_irq_from_tiny & (bitmask << code)))
	{
		return; /* just return */
	}
}
...
void IRQSensitivity_internal(kal_uint8 code, kal_bool edge)
{
   kal_uint8 line;
	  ...
	  if(IS_ADIE_IRQ_CODE(code))
	  {
		  ADIE_IRQSensitivity_internal(IRQ_CODE_GLB_TO_ADIE(code), edge);
...
kal_uint8 IRQ_Get_Sensitivity(kal_uint8 code)
{
   kal_uint8 line;

   if(IS_ADIE_IRQ_CODE(code))
   {
       return ADIE_IRQ_Get_Sensitivity(IRQ_CODE_GLB_TO_ADIE(code));
   }

   line = IRQCode2Line[code];
   if(line > 32)
   {
	   return ((*IRQ_SENS1) >> (line - 32)) & (0x01);
   }
   else
   {
	   return ((*IRQ_SENS0) >> (line)) & (0x01);
   }
...
/*-----------------------------------------------*/
// add for tiny system unknown interrupt resume from hibernation
#if defined(__TINY_SYS__) /* dongming add for tiny sys */
{
...
//step2: init the csci variable if first boot
	if( KAL_TRUE == Is_first_bootup_or_wdt_reset() )
	{
		unknown_irq_from_tiny = 0;
		unknown_eint_from_tiny = 0;
		unknown_irq_sens_from_tiny = 0;
		unknown_eint_sens_from_tiny = 0;
		ts_unknown_eint = 0;
		ts_unknown_irq = 0;

	}

//step3: trigger sw trigger.
#if 0
/* under construction !*/
/* under construction !*/
/* under construction !*/
...
/* under construction !*/
	...
	Unknown_EINT_CIRQ_From_Tiny_Suspend_Mode(KAL_TRUE);
	#endif
}
#endif
/*-----------------------------------------------*/
...
/*************************************************************************
* FUNCTION
*  Unknown_EINT_CIRQ_From_Tiny_Suspend_Mode
*
* DESCRIPTION
*  This function handle unknown cirq or eint form tiny system.
*
* PARAMETERS
*  is_hibernation_mode :  tiny system mode
*
* RETURNS
*  
*  
*************************************************************************/
void Unknown_EINT_CIRQ_From_Tiny_Suspend_Mode(kal_bool is_hibernation_mode)
{
	kal_uint8 eint_idx = 0;
	kal_uint8 irq_code = 0;
	kal_uint64 bitMask = 1;
	
	if(0 != unknown_irq_from_tiny)
	{
		for(irq_code = 0; irq_code < (NUM_IRQ_SOURCES+ADIE_NUM_IRQ_SOURCES);irq_code++)
		{
			if(0 != (unknown_irq_from_tiny & (bitMask << irq_code)) )
			{
				unknown_irq_sens_from_tiny &= (~(bitMask << irq_code)); //clear bit first
				unknown_irq_sens_from_tiny |= (((kal_uint64)IRQ_Get_Sensitivity(irq_code)) << irq_code);
				/* unknown_irq_sens_from_tiny is record in tiny_system , for cold boot will trigger wdt reset. we must record it before wdt reset */
				if(KAL_FALSE == is_hibernation_mode)/*if switch from tiny suspend mode,unmask irq*/
				{
					IRQUnmask(irq_code);
				}
				if(0 == (IRQ_Get_Sensitivity(irq_code)))//only edge we trigger
				{
					IRQSensitivity_internal(irq_code,KAL_FALSE);// set level sensitive
					IRQ_ResetSWRegister(irq_code);
					IRQ_SetSWRegister(irq_code);
				}
				else
				{
					unknown_irq_from_tiny &= (~(bitMask << irq_code));
				}
			}
		}
	}
	if(0 != unknown_eint_from_tiny)
	{
		for(eint_idx = 0; eint_idx < EINT_ALL_NUMBER;eint_idx++)
		{
			if(0 != (unknown_eint_from_tiny & (bitMask << eint_idx)) )
			{
				unknown_eint_sens_from_tiny &= (~(bitMask << eint_idx)); //clear bit first
				unknown_eint_sens_from_tiny |= (((kal_uint64)EINT_Get_Sensitivity(eint_idx)) <<	eint_idx);
				/* unknown_eint_sens_from_tiny is record in tiny_system, for cold boot will trigger wdt reset. we must record it before wdt reset */
				if(KAL_FALSE == is_hibernation_mode)/*if switch from tiny suspend mode,unmask eint*/
				{
					EINT_UnMask(eint_idx);
				}
				if(0 == (EINT_Get_Sensitivity(eint_idx)))//only edge we trigger
				{
					EINT_Set_Sensitivity_internal(eint_idx,KAL_FALSE);// set level sensitive
					EINT_ResetSWRegister(eint_idx);
					EINT_SetSWRegister(eint_idx);
				}
				else
				{
					unknown_eint_from_tiny &= (~(bitMask << eint_idx));
				}
			}
		}
	}	
...
kal_uint8 IRQ_Query_If_Special_Resume(void)
{
	...
	/*every time we need set this two,because csci region init is behind initINTR()
	 * no suitable place to init this two.*/
	irq_registered_special_cirq |= (BTSS_ALL_CIRQ_BIT_MASK);
	irq_registered_special_eint |= (BTSS_ALL_EINT_BIT_MASK);
	if(0 != ((ts_unknown_eint & (irq_registered_special_eint)) | (ts_unknown_irq & (irq_registered_special_cirq)))){
		return 1;}
	else{
		return 0;}
}

/* Note : below function must be called after csci region init. csci region init is at :
*   INC_Initialize->Application_Initialize->HWDInitialization->CSCI_ZI_Region_Init
*/
void IRQ_Register_Special_Eint(kal_uint8 eint_no)
{
	kal_uint64 bitMask = 1;
	irq_registered_special_eint |= (bitMask << eint_no);
}
void IRQ_Register_Special_IRQ(kal_uint8 code)
{
	kal_uint64 bitMask = 1;
	irq_registered_special_cirq |= (bitMask << code);
}


kal_uint8 IRQ_Query_If_TS_Unknown_Resume(void)
{
	if (((0 != ts_unknown_eint) || (0 != ts_unknown_irq)) && 
		(0 == IRQ_Query_If_Special_Resume()) && 
		(WARM_BOOT == AP_check_boot_up_source()))
	{
		return 1;
	}
	else
