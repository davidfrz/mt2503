...
#include "mmi_rp_framework_shutdown_def.h"
...
#if defined(__IOT__)
#define MMI_SHUTDOWN_VIBRATE_TIMEOUT	500 /* ms */
#endif
...
/*****************************************************************************
 * FUNCTION
 *  mmi_shutdown_normal_continue
 * DESCRIPTION
 *  Continue the flow of shutdown
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
static void mmi_shutdown_normal_continue()
{
	/*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
mmi_ret mmi_shutdown_normal_start(mmi_event_struct *evt)
{
    ...
    /*----------------------------------------------------------------*/
#if defined(__IOT__)
		MMI_TRACE(MMI_SHUTDOWN_TRC_GROUP, TRC_FRM_SHUTDOWN_NORMAL_START, (U32)cntx);
		#ifdef __MMI_POWER_ON_OFF_MUTE__
    		srv_vibrator_play_once();
		#endif
		StartTimer(SHUTDOWN_VIBRATE_TIMER, MMI_SHUTDOWN_VIBRATE_TIMEOUT, mmi_shutdown_normal_continue);
#else
    if (g_mmi_shutdown_cntx != NULL)
    {
        return MMI_RET_OK;
...
srv_prof_ret mmi_shutdown_flow_after_tone(srv_prof_ret result)
{
	srv_shutdown_exit_system(FRAMEWORK_SHUTDOWN);
}

#ifdef __DISPLAY_LEVEL_GDI_ONLY__
#if defined(__MMI_MAINLCD_240X240__) || defined(__MMI_MAINLCD_128X128__)
extern const U8 power_off_img[];
static GDI_HANDLE poweroff_anim_handle = GDI_NULL_HANDLE;
#endif
void ShowPowerOffAnimScr()
{
#if defined(__MMI_MAINLCD_240X240__) || defined(__MMI_MAINLCD_128X128__)
    GDI_HANDLE base_layer;
    S32 w, h;
    S32 img_size;
    img_size = sizeof(power_off_img) - 8;
    gdi_layer_get_base_handle(&base_layer);
    gdi_layer_push_and_set_active(base_layer);
    gdi_layer_clear(GDI_COLOR_BLACK);
    gdi_image_get_dimension_mem(GDI_IMAGE_TYPE_GIF, (U8*)power_off_img + 8, img_size, &w, &h);
    gdi_anim_draw_mem_once(0, 0, (U8*)power_off_img + 8, GDI_IMAGE_TYPE_GIF, img_size, &poweroff_anim_handle);
...
static mmi_ret mmi_shutdown_flow_completed(mmi_event_struct *evt)
{
	...
	U16 power_off_tone = AUD_ID_PROF_TONE2;
	U8 volme = MDI_AUD_VOL_6;
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_SHUTDOWN_TRC_GROUP, TRC_FRM_SHUTDOWN_FLOW_COMPLETED);
    ...
    srv_prof_set_current_profile_value(SRV_PROF_SETTINGS_POWER_OFF_TONE, &power_off_tone);
	srv_prof_set_current_profile_value(SRV_PROF_SETTINGS_RING_VOL_LEVEL, &volme);
    srv_prof_play_tone(SRV_PROF_TONE_POWER_OFF, mmi_shutdown_flow_after_tone);
    //srv_shutdown_exit_system(FRAMEWORK_SHUTDOWN);
#ifdef __DISPLAY_LEVEL_GDI_ONLY__
	ShowPowerOffAnimScr();
#endif
...
static void mmi_shutdown_process_timeout_handler(void *arg)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_SHUTDOWN_TRC_GROUP, TRC_FRM_SHUTDOWN_TIMEOUT);
