...
kal_uint8 ADIE_EINT_Get_Sensitivity(kal_uint8 eintno);
kal_uint32 EINT_Set_Sensitivity_internal(kal_uint8 eintno, kal_bool sens);
...
/*===============================================================*/
	...
	EINT_Mask(eintno);// fist mask it. add by dongming. for mre may registration when MMI task run.and may trigger interrupt when domain is set in this function
...
/* under construction !*/
				...
				if(0 != (unknown_eint_from_tiny & (bitmask << tp_eint_indx)))
				{
					EINT_Set_Sensitivity_internal(index,(~(unknown_eint_sens_from_tiny >> index)) & 0x01);
					EINT_ResetSWRegister(index);
					unknown_eint_from_tiny &= (~(bitmask << tp_eint_indx));
				}
...
/*-------------------------------------------------------*/
/* if the unknown_eint_from_tiny bit is set to 1, we should not ack this interrupt. */
/* or,the related eint will not trigger interrupt any more */
#if defined(__TINY_SYS__) /* dongming add for tiny sys */
{
	kal_uint64 bitMask = 1;
	if(0 != (unknown_eint_from_tiny & (bitMask << (eintno))))
	{
		return;  /* just return */
	}
}
...
kal_uint32 EINT_Set_Sensitivity_internal(kal_uint8 eintno, kal_bool sens)
{
   kal_uint32 savedMask;
	  ...
	  if(IS_ADIE_EINT(eintno))
	  {
		 return ADIE_EINT_Set_Sensitivity_internal(EINT_GLB_TO_ADIE(eintno), sens);
	  }
#endif
...
kal_uint8 EINT_Get_Sensitivity(kal_uint8 eintno)
{

	if(IS_ADIE_EINT(eintno))
	{
		return ADIE_EINT_Get_Sensitivity(EINT_GLB_TO_ADIE(eintno));
	}

#if (EINT_TOTAL_CHANNEL > 32)
	if(eintno > 32)
	{
		return ((*EINT_SENS1) >> (eintno - 32)) & (0x01);
	}
	else
	{
		return ((*EINT_SENS0) >> (eintno)) & (0x01);
	}
#else //(EINT_TOTAL_CHANNEL > 32)

	return ((*EINT_SENS) >> (eintno)) & (0x01);
#endif    

}

kal_uint8 EINT_Get_Polarity(kal_uint8 eintno)
{

	if(IS_ADIE_EINT(eintno))
	{
		return ADIE_EINT_Get_Polarity(EINT_GLB_TO_ADIE(eintno));
	}

	return (EINTaddr(eintno) >> 11) & 0x1;
