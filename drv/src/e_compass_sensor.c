...
void e_compass_init(void)
{
    ...
    e_compass_sensor_data.sample_handle = DclSGPT_Open(DCL_GPT_CB,0);
...
void e_compass_nvram_init(void)
{
    ...
    if (KAL_FALSE == is_e_compass_nvram_init)
    {
        ...
        nvram_external_read_data(NVRAM_EF_ECOMPASS_DATA_LID, 1, (kal_uint8 *)&
ec_nvram, sizeof(nvram_ef_ecompass_calibration));
        /* E_COMPASS_DEBUG_OUTPUT("Read from NvRAM usr_moffset_x = %d, 
usr_moffset_y = %d, usr_moffset_z = %d, misc = %d", ec_nvram.usr_moffset_x, 
ec_nvram.usr_moffset_y, ec_nvram.usr_moffset_z, 0); */
        drv_trace4(TRACE_GROUP_10, EC_NVRAM_READBACK, ec_nvram.usr_moffset_x, 
ec_nvram.usr_moffset_y, ec_nvram.usr_moffset_z, ec_nvram.cali_result);
        ...
        e_compass_calibrated_data.x_axis_sensitivity = ec_nvram.
x_axis_sensitivity;
        e_compass_calibrated_data.y_axis_sensitivity = ec_nvram.
y_axis_sensitivity;
...
void e_compass_sensor_power_on(void)
{
    ...
    if (KAL_FALSE == is_middleware_init)
    {
        ...
        e_compass_sensor_custom_fp->ec_init_calibrated_data(&
e_compass_calibrated_data);
        is_middleware_init = KAL_TRUE;
    }

    e_compass_sensor_custom_fp->ec_turn_on(1);
    /* we start timer, after timer expired, it will invoke 
e_compass_sensor_power_on_callback
       because sensor need some time to be ready after power on */
    kal_set_timer(e_compass_power_on_timer_id,(kal_timer_func_ptr)
e_compass_sensor_power_on_callback,NULL,
...
static void e_compass_enable_driver(kal_bool enable)
{
    ...
    if (KAL_TRUE == enable)
    {
        ...
        if (E_COMPASS_SENSOR_START_CALI != e_compass_sensor_data.cali_state)
          {  /* GPTI_StartItem(e_compass_sensor_data.sample_handle,
                                               e_compass_sensor_custom_dp->
normal_sample_dura,
                                               e_compass_sample_cb,
                                               NULL);*/
		    start.u2Tick = e_compass_sensor_custom_dp->normal_sample_dura;
            start.pfCallback = e_compass_sample_cb;
            start.vPara =  0;
            DclSGPT_Control(e_compass_sensor_data.sample_handle,SGPT_CMD_START
,(DCL_CTRL_DATA_T*)&start);
        	}
        else
           {/* GPTI_StartItem(e_compass_sensor_data.sample_handle,
                           e_compass_sensor_custom_dp->calibration_sample_dura,
                           e_compass_sample_cb,
                           NULL);*/
            start.u2Tick = e_compass_sensor_custom_dp->calibration_sample_dura;
            start.pfCallback = e_compass_sample_cb;
            start.vPara =  0;
            DclSGPT_Control(e_compass_sensor_data.sample_handle,SGPT_CMD_START
,(DCL_CTRL_DATA_T*)&start);               
    ...
    else
    {
		...
		DclSGPT_Control( e_compass_sensor_data.sample_handle, SGPT_CMD_STOP, 0);
    }
}
...
*
* DESCRIPTION
*   This function is to configure sensor middleware manually, end user must 
keep handset horizontal.
*
* PARAMETERS
*   None
*
...
calibrated_data)
{
    ...
    /* E_COMPASS_DEBUG_OUTPUT("Calibrated result usr_moffset_x = %d, 
usr_moffset_y = %d, usr_moffset_z = %d, misc = %d", ec_nvram.usr_moffset_x, 
ec_nvram.usr_moffset_y, ec_nvram.usr_moffset_z, 0); */
    drv_trace4(TRACE_GROUP_10, EC_NVRAM_SAVE, ec_nvram.usr_moffset_x, ec_nvram
.usr_moffset_y, ec_nvram.usr_moffset_z, ec_nvram.cali_result);
    ...
    data_stream = construct_peer_buff(sizeof(nvram_ef_ecompass_calibration), 0
, 0, TD_CTRL);

    ((nvram_write_req_struct*) parm_stream)->file_idx = 
NVRAM_EF_ECOMPASS_DATA_LID;
    ((nvram_write_req_struct*) parm_stream)->para = 1;

    pdu_len = sizeof(nvram_ef_ecompass_calibration);
    kal_mem_cpy(get_pdu_ptr(data_stream, &pdu_len), (void*)&ec_nvram, sizeof(
...
void e_compass_main_hdr(void)
{
    ...
    /* disable example, maybe it is not necessary, GPT is one-shot, anyway it 
is harmless */
    e_compass_enable_driver(KAL_FALSE);

    /* read raw data */
    e_compass_sensor_custom_fp->ec_read_adc(&x,&y,&z);
    /* E_COMPASS_DEBUG_OUTPUT("Raw data X = %d, Y = %d, Z = %d, misc = %d", x
, y, z, 0); */
    ...
    case E_COMPASS_SENSOR_CALI_NONE:
...
#if defined (__E_COMPASS_MIDDLEWARE_DEBUG__)
        {
            ...
            e_compass_sensor_custom_fp->ec_middleware_dump(&mag_x_dump,&
mag_y_dump,&mag_z_dump);
            drv_trace4(TRACE_GROUP_10, EC_MIDDLEWARE_DUMP, mag_x_dump, 
mag_y_dump, mag_z_dump, 0);
        }
        ...
        {
            e_compass_sensor_custom_fp->ec_get_calibrated_data(&
e_compass_calibrated_data);
            ...
            e_compass_sensor_data.mag_cb_func(e_compass_sensor_data.mag_para,
mag_state);
            ...
            {
                /* there is no room, if SW goes to here, that means something 
abnormal, MMI has not handled nofication for a long time
                   we do nothing here */
            }
        }
        break;
...
e_compass_calibrated_data);
            if (KAL_TRUE == e_compass_calibrated_data.cali_result)
            {
                ...
                e_compass_sensor_data.cali_cb_func(e_compass_sensor_data.
cali_para,E_COMPASS_SENSOR_CALI_SUCCESS);
            }
            else
            {
                /* calibration finished, we need to inform end user */
                e_compass_sensor_data.cali_cb_func(e_compass_sensor_data.
cali_para,E_COMPASS_SENSOR_CALI_FAILED);
            }
            drv_trace1(TRACE_GROUP_10, EC_CALIBRATION_RESULT, 
e_compass_calibrated_data.cali_result);
    ...
    case E_COMPASS_SENSOR_CALI_ABORTED:
        /* E_COMPASS_DEBUG_OUTPUT("Calibration aborted: %d %d %d %d", 0, 0, 0
, 0); */
    ...
    /* enable sample again, for the safe reason, if MMI has disabled it, 
however driver task still runs to here, we should double check */
