...
static mmi_imm_input_mode_item g_mmi_imm_curr_allowed_input_mode[IMM_MAX_INPUT_MODE_NUM];
...
static mmi_imm_input_mode_hint_item_struct g_mmi_imm_zawgyi[] =
{
    {MMI_IME_HINT_FLAG_INFO_BAR | MMI_IME_HINT_FLAG_CSK, MMI_IME_HINT_TYPE_STRING, WGUI_CATEGORY_ZAWGYI_STRING_ID},
    {MMI_IME_HINT_FLAG_NONE, MMI_IME_HINT_TYPE_MAX, 0},
};
#endif /* __MMI_LANG_ZAWGYI_MYANMAR__ */
...
{
#if defined(__MMI_MAINLCD_96X64__)
    {MMI_IME_HINT_FLAG_INFO_BAR | MMI_IME_HINT_FLAG_CSK, MMI_IME_HINT_TYPE_IMAGE, WGUI_CATEGORY_HINGLISH_UPPER_CASE_IMAGE_ID},
#else
...
{
#if defined(__MMI_MAINLCD_96X64__)
    {MMI_IME_HINT_FLAG_INFO_BAR | MMI_IME_HINT_FLAG_CSK, MMI_IME_HINT_TYPE_IMAGE, WGUI_CATEGORY_HINGLISH_LOWER_CASE_IMAGE_ID},
#else
    {MMI_IME_HINT_FLAG_INFO_BAR | MMI_IME_HINT_FLAG_CSK, MMI_IME_HINT_TYPE_STRING, WGUI_CATEGORY_hinglish_STRING_ID},
#endif
    {MMI_IME_HINT_FLAG_NONE, MMI_IME_HINT_TYPE_MAX, 0},
...
#if defined(__MMI_LANG_ZAWGYI_MYANMAR__)
    {IMM_INPUT_MODE_MULTITAP_MYANMAR, g_mmi_imm_zawgyi},
#endif /* #if defined(__MMI_LANG_MYANMAR__) */
    ...
    mmi_imm_input_mode_enum default_input_mode)
{
    ...
    /* reset data. */
    memset(&g_mmi_imm_curr_allowed_input_mode, 0 , sizeof(mmi_imm_input_mode_item) * IMM_MAX_INPUT_MODE_NUM);
...
#endif
    #if defined(__MMI_LANG_ZAWGYI_MYANMAR__) //we don't support zawgyi right now.
    if (g_mmi_imm_current_input_mode == IMM_INPUT_MODE_SMART_MYANMAR && ire_is_zawgyi_language_activate_in_phone_setting())
    {
        g_mmi_imm_current_input_mode = IMM_INPUT_MODE_MULTITAP_MYANMAR;
    }
    #endif
...
MMI_BOOL mmi_imm_init(void)
{
    ...
    /*----------------------------------------------------------------*/
#if defined(__MMI_SMART_ALPHABETIC_INPUT_METHODS__) || defined(__MMI_SMART_PHONETIC_INPUT_METHODS__)
    ReadValueSlim(NVRAM_IME_SMART_MODE_CONFIG, &data, DS_BYTE);
    ...
    ReadValueSlim(NVRAM_IME_AUTO_CAPITALIZATION_FLAG, &data, DS_BYTE);
    if (data == 0xFF)
    {
    ...
    ReadValueSlim(NVRAM_IME_AUTO_COMPLETION_FLAG, &data, DS_BYTE);
    if (data == 0xFF)
    {
...
#if defined(__MMI_IME_ALPHABETIC_PREDICTION_SETTING__)
    ReadValueSlim(NVRAM_IME_ALPHABETIC_WORD_PREDICTION_FLAG, &data, DS_BYTE);
    if (data == 0xFF)
    {
...
#if  defined(__MMI_IME_PHONETIC_PREDICTION_SETTING__)
    ReadValueSlim(NVRAM_IME_CHINESE_WORD_PREDICTION_FLAG, &data, DS_BYTE);
    if (data == 0xFF)
    {
	...
	//Auto Space
    ReadValueSlim(NVRAM_IME_AUTO_SPACE_FLAG, &data, DS_BYTE);
    if (data == 0xFF)
    {
	...
	//Vibration
	ReadValueSlim(NVRAM_IME_VIBRATION_FLAG, &data, DS_BYTE);
    if (data == 0xFF)
    {
	...
	//Touch feedback
	ReadValueSlim(NVRAM_IME_SOUND_FEEDBACK_FLAG, &data, DS_BYTE);
    if (data == 0xFF)
    {
	...
	// language follow vk
	ReadValueSlim(NVRAM_SETTING_LANGUAGE_FOLLOW_VK, &data, DS_BYTE);
	g_current_handwriting_lang_id = (mmi_imm_support_writing_lang_enum)data;

	
	// handwriting on/off
	ReadValueSlim(NVRAM_SETTING_HANDWRITING_ON, &data, DS_BYTE);
	g_is_handwriting_setting_on = (MMI_BOOL)data;
	
	// stroke color index
	ReadValueSlim(NVRAM_SETTING_STROKE_COLOR_INDEX, &data, DS_BYTE);
#if 0
/* under construction !*/
/* under construction !*/
#endif	
	...
	ReadValueSlim(NVRAM_SETTING_PEN_WIDTH, &data, DS_BYTE);
    ...
    ReadValueSlim(NVRAM_IME_INPUT_PATTERN, &data, DS_BYTE);
    if (data == 0xFF)
    {
        /* Set default 
...
#if defined(__MMI_LANG_ZAWGYI_MYANMAR__) 
    if ((hint_mode == IMM_INPUT_MODE_MULTITAP_MYANMAR) && ire_is_zawgyi_language_activate_in_phone_setting())
    {
...
mmi_imm_input_mode_enum mmi_imm_get_corresponding_input_mode(mmi_imm_input_mode_enum input_mode)
{
    ...
    int i,j;
    #if defined(__MMI_LANG_ZAWGYI_MYANMAR__) //we don't support zawgyi right now.
    if (input_mode == IMM_INPUT_MODE_SMART_MYANMAR && ire_is_zawgyi_language_activate_in_phone_setting())
    {
        return IMM_INPUT_MODE_MULTITAP_MYANMAR;
    }
    #endif
...
S32 mmi_imm_get_curr_allow_input_mode_list_of_imev3(mmi_imm_input_mode_enum *mode_array, S32 max)
{
    ...
    if(g_mmi_imm_current_input_type == IMM_INPUT_TYPE_QUICK_SEARCH)
    {
        ...
        {
			#if defined(__MMI_BT_PBAP_CLIENT__)
			if(gIMEModeArray[g_mmi_imm_curr_allowed_input_mode[i].index].Mode_Special_Attrib & IME_MODE_ATTR_UPPERCASE)
			{
				continue;
			}
			#endif
            if (g_mmi_imm_curr_allowed_input_mode[i].mode_id == IMM_INPUT_MODE_ARABIC_NUMERIC)
            {
                continue;
            }
...
void mmi_imm_config_smart_alphabetic_input_modes(MMI_BOOL is_enabled)
{
    ...
    WriteValueSlim(NVRAM_IME_SMART_MODE_CONFIG, &data, DS_BYTE);
...
void mmi_imm_read_prefered_writing_lang_from_nvram()
{
    ...
    /*----------------------------------------------------------------*/ 
#if defined(__MMI_PREFER_WRITING_LANG__) || defined(__MMI_MIXED_LANG_SUPPORT__)
    ReadValueSlim(NVRAM_SETTING_WRITING_LANG, &g_mmi_imm_main_writing_lang, DS_SHORT);
#endif
}
...
void mmi_imm_write_prefered_writing_lang_to_nvram()
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/ 
    WriteValueSlim(NVRAM_SETTING_WRITING_LANG, &g_mmi_imm_main_writing_lang, DS_SHORT);
#endif
}
...
void mmi_imm_update_prefer_writing_language(mmi_imm_support_writing_lang_enum writing_lang)
{
    ...
    WriteValueSlim(NVRAM_SETTING_WRITING_LANG, &value, DS_SHORT);
    ...
    #if defined(__MMI_LANG_ZAWGYI_MYANMAR__) //we don't support zawgyi right now.
    if (value == IMM_INPUT_MODE_SMART_MYANMAR && ire_is_zawgyi_language_activate_in_phone_setting())
    {
        value = IMM_INPUT_MODE_MULTITAP_MYANMAR;
    }
    #endif
    mmi_imm_set_prefer_input_mode((mmi_imm_input_mode_enum)value);
    WriteValueSlim(NVRAM_SETTING_PREFER_INPUT_METHOD, &value, DS_SHORT);
...
void mmi_imm_phone_language_change_handler(void)
{
   ...
   S16 j = 0;
    ...
    if (!is_bootup)
    {
        ...
        {
            mmi_imm_update_prefer_writing_language(IME_WRITING_LANG_MY);
        }
#endif
    }
    else
...
void mmi_imm_read_prefered_writing_sub_lang_from_nvram()
{
    ...
    /*----------------------------------------------------------------*/ 
#if defined(__MMI_MIXED_LANG_SUPPORT__) 
    ReadValueSlim(NVRAM_SETTING_WRITING_LANG_SUB, &value, DS_SHORT);
...
void mmi_imm_write_prefered_sub_writing_lang_to_nvram()
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/ 
    WriteValueSlim(NVRAM_SETTING_WRITING_LANG_SUB, &g_mmi_imm_sub_writing_lang, DS_SHORT);
#endif
}
...
void mmi_imm_init_writing_lang()
{
    ...
    ReadValueSlim(NVRAM_SETTING_ENABLED_WRITING_LANGUAGE, &g_mmi_imm_enabled_writing_language, DS_SHORT);
...
#if defined(__MMI_PREFER_WRITING_LANG__) || defined(__MMI_MIXED_LANG_SUPPORT__)
    ReadValueSlim(NVRAM_SETTING_WRITING_LANG, &value, DS_SHORT);
    mmi_imm_set_main_lang((mmi_imm_support_writing_lang_enum)value);
#endif
...
U32 mmi_imm_set_writing_language_and_mode_setting_for_list(MMI_BOOL* Index, U16 num)
{
    ...
    WriteValueSlim(NVRAM_SETTING_ENABLED_WRITING_LANGUAGE, &data, DS_SHORT);
}
...
void mmi_imm_init_fuzzy_pinyin_setting()
{
    ...
    // read from NVRAM
    ReadValueSlim((U16)NVRAM_SETTING_ENABLED_FUZZY_PINYIN, (void*)&data, (U8)DS_SHORT);
...
#if defined(__MMI_PREFER_INPUT_METHOD__) || defined(__MMI_PREFER_WRITING_LANG__) || defined(__MMI_MIXED_LANG_SUPPORT__)
    ReadValueSlim(NVRAM_SETTING_PREFER_INPUT_METHOD, &g_mmi_imm_prefer_input_mode, DS_SHORT);
#endif
}
...
#if defined(__MMI_PREFER_INPUT_METHOD__) || defined(__MMI_PREFER_WRITING_LANG__) || defined(__MMI_MIXED_LANG_SUPPORT__)
    WriteValueSlim(NVRAM_SETTING_PREFER_INPUT_METHOD, &input_mode, DS_SHORT);
#endif
}
...
#if defined(__MMI_PREFER_INPUT_METHOD__) || defined(__MMI_PREFER_WRITING_LANG__) || defined(__MMI_MIXED_LANG_SUPPORT__)
    ReadValueSlim(NVRAM_SETTING_PREFER_INPUT_METHOD, &data, DS_SHORT);
#endif
    if (data == 0xFF )
    {
...
void mmi_imm_config_auto_capitalization(MMI_BOOL is_enabled)
{
    ...
    WriteValueSlim(NVRAM_IME_AUTO_CAPITALIZATION_FLAG,&data, DS_BYTE);
#endif
  #ifdef __MMI_PREFER_INPUT_METHOD__
    mmi_imm_get_prefer_input_mode_list(MMI_all_prefered_input_mode_set, IMM_INPUT_MODE_MAX_NUM);
#endif
...
void mmi_imm_config_auto_completion(MMI_BOOL is_enabled)
{
    ...
    WriteValueSlim(NVRAM_IME_AUTO_COMPLETION_FLAG,&data, DS_BYTE);
#endif
    return ;
...
void mmi_imm_config_alphabetic_word_prediction(MMI_BOOL is_enabled)
{
    ...
    WriteValueSlim(NVRAM_IME_ALPHABETIC_WORD_PREDICTION_FLAG,&data, DS_BYTE);
#endif
    return ;
}
...
void mmi_imm_config_auto_space(MMI_BOOL is_enabled)
{
    ...
    WriteValueSlim(NVRAM_IME_AUTO_SPACE_FLAG,&data, DS_BYTE);
...
void mmi_imm_config_vibration(MMI_BOOL is_enabled)
{
    ...
    WriteValueSlim(NVRAM_IME_VIBRATION_FLAG, &data, DS_BYTE);
...
void mmi_imm_config_sound_feedback(MMI_BOOL is_enabled)
{
    ...
    WriteValueSlim(NVRAM_IME_SOUND_FEEDBACK_FLAG, &data, DS_BYTE);
...
void mmi_imm_config_chinese_word_prediction(MMI_BOOL is_enabled)
{
    ...
    WriteValueSlim(NVRAM_IME_CHINESE_WORD_PREDICTION_FLAG,&data, DS_BYTE);
#endif
    return;
  #ifdef __MMI_PREFER_INPUT_METHOD__
    mmi_imm_get_prefer_input_mode_list(MMI_all_prefered_input_mode_set, IMM_INPUT_MODE_MAX_NUM);
...
void mmi_imm_config_keyboard_type(mmi_imm_keypadtype_enum keyboard_type)
{
    ...
    WriteValueSlim(NVRAM_IME_INPUT_PATTERN,&data, DS_BYTE);
#if defined(__MMI_IME_FTE_ENHANCE__) && defined(__MMI_VIRTUAL_KEYBOARD__)
    mmi_imc_pen_open_symbol_lock();
#endif /* defined(__MMI_IME_FTE_ENHANCE__) && defined(__MMI_VIRTUAL_KEYBOARD__) */
#endif
...
void mmi_imm_set_pen_speed(mmi_imm_pen_speed_enum speed)
{
    ...
    if (speed < MMI_IMM_PEN_SPEED_MAX)
    {
        ...
        WriteValueSlim(NVRAM_SETTING_PEN_SPEED, &g_mmi_imm_pen_speed, DS_BYTE);
    }
}
...
void mmi_imm_init_pen_speed(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    ReadValueSlim(NVRAM_SETTING_PEN_SPEED, &g_mmi_imm_pen_speed, DS_BYTE);
    if (g_mmi_imm_pen_speed >= MMI_IMM_PEN_SPEED_MAX)
    {
...
void mmi_imm_config_handwriting_language(UI_string_type string)
    ...
    {    
        ...
        if (mmi_ucs2cmp((char*)string, (char*)g_ime_writing_lang_array[j].lang_str_ptr) == 0)
        {
			...
			WriteValueSlim(NVRAM_SETTING_LANGUAGE_FOLLOW_VK, &temp, DS_BYTE);
            return;
        }
    }
...
void mmi_imm_handwriting_enable(MMI_BOOL bOn)
{
	...
	WriteValueSlim(NVRAM_SETTING_HANDWRITING_ON, &g_is_handwriting_setting_on, DS_BYTE);
}
...
MMI_BOOL mmi_imm_handwriting_set_current_stroke_color( U8 uIndex )
{
	...
	ret_value = WriteValueSlim(NVRAM_SETTING_STROKE_COLOR_INDEX, &g_current_stroke_color_index, DS_BYTE);
...
#endif
    {
        return g_current_stroke_width - 1;
    }
}
...
#endif
    {
        g_current_stroke_width = width + 1;
    }
	WriteValueSlim(NVRAM_SETTING_PEN_WIDTH, &g_current_stroke_width, DS_BYTE);
}
...
void mmi_imm_handwriting_set_follow_vk_lang(MMI_BOOL bFollow)
{
	...
	WriteValueSlim(NVRAM_SETTING_LANGUAGE_FOLLOW_VK, &g_is_handwriting_follow_vk_lang, DS_BYTE);
}
...
MMI_BOOL mmi_imm_get_is_smart_input_mode_allowed_of_curr_writing_language(void)
{
	...
	wlang = mode_details_ptr->lang_id;
#if defined(__MMI_LANG_ZAWGYI_MYANMAR__) //we don't support zawgyi right now
    if (wlang == IME_WRITING_LANG_MY && ire_is_zawgyi_language_activate_in_phone_setting())
    {
        return MMI_FALSE;
    }
#endif
