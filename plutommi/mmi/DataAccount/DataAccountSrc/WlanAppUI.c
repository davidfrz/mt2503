    ...
    for (i = 0; i < ap_list_num; i++)
    {
        ...
        switch (g_wizard_prfile_list[i].type)
        {
            ...
            case WLAN_MATCH_SA:
            {
                ...
                //iconStates[i] = 0;
                if (g_wizard_prfile_list[i].p_sa->rssi > -60)
                {
                    iconStates[i] = IMG_ID_DTCNT_WLAN_SIGNAL3;
                }
                else if (g_wizard_prfile_list[i].p_sa->rssi > -70)
                {
                    iconStates[i] = IMG_ID_DTCNT_WLAN_SIGNAL2;
                }
                else if (g_wizard_prfile_list[i].p_sa->rssi > -80)
                {
                    iconStates[i] = IMG_ID_DTCNT_WLAN_SIGNAL1;
                }
                else
...
/* under construction !*/
#else
void mmi_wlan_change_priority(mmi_pen_point_struct pos)
#endif
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
...
/* under construction !*/
#else
	wgui_register_pen_long_tap_handler(mmi_wlan_change_priority);
#endif
}
