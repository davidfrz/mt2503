...
void uemdrv_power_on_ind_hdlr(local_para_struct *local_para_ptr, peer_buff_struct *peer_buff_ptr)
{
    ...
    switch (param_poweron->power_on)
    {
        ...
        #if !defined(__IOT__)
#if defined(__UPDATE_BINARY_FILE__) && !defined(__UPDATE_PKG_BT_ONLY__) && !defined(__UPDATE_PKG_SENSOR_ONLY__)
            kal_prompt_trace(MOD_UEM, "Call MMIGetGDIShowFullScreenTime()");
            p_mmi_symblo_addr = (unsigned int*)(custom_get_UPDATE_ROM_entry() 
                                    + UBIN_SYMBOL_GET_GDI_SHOW_FULL_SCREEN_TIME * sizeof(void *));
            estimate_time = ((kal_uint32 (*)(void))(*p_mmi_symblo_addr))();
        #else
