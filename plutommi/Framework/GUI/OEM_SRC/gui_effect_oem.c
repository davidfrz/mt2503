...
#define LIST_SSP_MOVE_THRESHOLD	8
typedef enum
{
	MMI_SWATCH_MOVE_DIRECTION_NONE,
	MMI_SWATCH_MOVE_DIRECTION_H,
	MMI_SWATCH_MOVE_DIRECTION_V,
}mmi_swatch_ssp_move_direction_enum;
MMI_BOOL g_gui_ssp_launch = MMI_TRUE;
static S32 g_gui_ssp_pen_down_x;
static mmi_swatch_ssp_move_direction_enum g_gui_ssp_move_direction = MMI_SWATCH_MOVE_DIRECTION_NONE;
#if defined(__MMI_PHB_PBAPC_SYNC_CONTACT__) && defined(__MMI_BT_PBAP_CLIENT__)
MMI_BOOL need_gui_buff = MMI_TRUE;
extern char show_first_item_flag;
void (*update_data_function)() = NULL;
#endif /*defined(__MMI_PHB_PBAPC_SYNC_CONTACT__) && defined(__MMI_BT_PBAP_CLIENT__)*/
MMI_BOOL pen_down_in_list = MMI_FALSE;
MMI_BOOL pen_down_show_highlight = MMI_FALSE;
static MMI_BOOL g_gui_ssp_pen_in_long_tap = MMI_FALSE;
extern MMI_BOOL g_swatch_send_down_in_move;
extern S32 g_pen_down_dismiss;
#endif
...
static float *g_gui_ssp_menu_trigger_free_run_coefficient = &g_gui_list_ssp_parameter.g_gui_ssp_menu_trigger_free_run_coefficient;
...
#if defined(__MMI_WEARABLE_DEVICE__) && defined(__MMI_TOUCH_SCREEN__)
mmi_ret mmi_ssp_pen_move_right_hdlr(mmi_event_struct *evt)
{
	if(evt->evt_id == EVT_ID_WEARABLE_DEVICE_MOVE_RIGHT && g_gui_ssp_move_direction == MMI_SWATCH_MOVE_DIRECTION_V)
	{
...
void gui_swatch_list_disable_scrollbar(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
	...
	//gui_show_list_menu_scrollbar(&MMI_fixed_list_menu, MMI_FALSE);
	x1 = MMI_fixed_list_menu.vbar.x;
    x2 = x1 + MMI_fixed_list_menu.vbar.width - 1;
    y1 = MMI_fixed_list_menu.vbar.y;
    y2 = y1 + MMI_fixed_list_menu.vbar.height - 1;
	gdi_layer_push_and_set_clip(x1, y1, x2, y2);
    //gui_draw_filled_area(x1, y1, x2, y2, &MMI_fixed_list_menu.normal_filler);
    gui_fill_rectangle(x1, 
    	y1, 
    	x2, 
    	y2, 
    	UI_COLOR_BLACK);
...
#endif
#if defined(__MMI_WEARABLE_DEVICE__) && defined(__MMI_TOUCH_SCREEN__)
void swatch_list_menu_set_down_flag(void)
{
    #ifdef __MMI_FTE_SUPPORT__ 
    ...
    #endif
	pen_down_in_list = MMI_TRUE;
	pen_down_show_highlight = MMI_TRUE;
	gui_list_menu_smooth_scrolling_redraw_list();
}
#if defined(__MMI_PHB_PBAPC_SYNC_CONTACT__) && defined(__MMI_BT_PBAP_CLIENT__)
void gui_ssp_update_context(void)
{
	if (0 != MMI_fixed_list_menu.first_displayed_item)
	{
		MMI_fixed_list_menu.first_displayed_item -= 1;
		MMI_fixed_list_menu.last_displayed_item -= 1;
		MMI_fixed_list_menu.n_items -= 1;
		MMI_fixed_list_menu.highlighted_item -= 1;
	}
}
void gui_ssp_update_first_item_data(void)
{
	kal_prompt_trace(MOD_MMI,"gui_ssp_update_first_item_data in\n");
	
	if (NULL != update_data_function)
	{
		...
		if (0 == MMI_fixed_list_menu.first_displayed_item)
		{
				...
				need_gui_buff = MMI_FALSE;
				update_data_function();
				need_gui_buff = MMI_TRUE;
		}
		else
		{
			/*MMI_fixed_list_menu.first_displayed_item -= 1;
	...
	kal_prompt_trace(MOD_MMI,"gui_ssp_update_first_item_data out\n");
...
static MMI_BOOL gui_ssp_pen_down_hdlr(mmi_pen_point_struct point)
{
    ...
    if ((gui_list_menu_smooth_scrolling_get_menu_item_height() * m->n_items) <  m->height)
	{
		*g_gui_ssp_menu_over_limit_coefficient = 0;
	}
	else
	{
		*g_gui_ssp_menu_over_limit_coefficient = GUI_SSP_MENU_OVER_LIMIT_COEFFICIENT;
	}
	pen_down_show_highlight = MMI_FALSE;
	g_gui_ssp_pen_in_long_tap = MMI_FALSE;
	g_gui_ssp_launch = MMI_TRUE;
	#ifdef __MMI_WEARABLE_DEVICE_UI_ENHANCE__
	MMI_fixed_list_menu.flags |= UI_LIST_MENU_DISABLE_SCROLLBAR;
	gui_cancel_timer(gui_swatch_list_disable_scrollbar);
	#endif /*__MMI_WEARABLE_DEVICE_UI_ENHANCE__*/
	#endif /*__MMI_WEARABLE_DEVICE__*/
...
#ifdef __MMI_WEARABLE_DEVICE__
	g_gui_ssp_pen_down_x = (S32)point.x;
	g_gui_ssp_move_direction = MMI_SWATCH_MOVE_DIRECTION_NONE;
#endif /*__MMI_WEARABLE_DEVICE__*/
    ...
    if (gui_ssp_menu_translate_pen_position(m, (S32)point.x, g_gui_ssp_pen_down_y, &new_highlight_item_index))
    {
		...
		if (gui_ssp_menu_translate_pen_position(&MMI_fixed_list_menu, (S32)point.x, (S32)point.y, &item_index))
		{
			if (g_gui_ssp_state != GUI_LIST_SMOOTH_SCROLLING_BY_PEN_STATE_FREE_RUN && g_gui_ssp_have_goto_disabled_item !=MMI_TRUE)
				gui_start_timer(200, swatch_list_menu_set_down_flag);
		}
#endif /*__MMI_WEARABLE_DEVICE__*/
...
//    else
	...
	#ifdef __MMI_WEARABLE_DEVICE__
	if (g_swatch_send_down_in_move == MMI_TRUE)
	{
		g_gui_ssp_pen_down_y -= g_pen_down_dismiss;
		kal_prompt_trace(MOD_MMI, "xl gui_ssp_pen_down_hdlr g_pen_down_dismiss is %d", g_pen_down_dismiss);
		if (g_pen_down_dismiss > (*g_gui_ssp_focus_to_hold_state_y_threshold))
		{
			g_gui_ssp_state = GUI_LIST_SMOOTH_SCROLLING_BY_PEN_STATE_HOLD;
			kal_prompt_trace(MOD_MMI, "xl gui_ssp_pen_down_hdlr set state to hold");
		}
		kal_prompt_trace(MOD_MMI, "xl gui_ssp_pen_down_hdlr change down y to %d", g_gui_ssp_pen_down_y);
		g_swatch_send_down_in_move = MMI_FALSE;
	}
	#endif /*__MMI_WEARABLE_DEVICE__*/
    return MMI_TRUE;
}
...
static void gui_menuitem_handle_ssp_pen_up()
{
    ...
    if (g_gui_list_menu_smooth_scrolling_menu_type != GUI_MENU_TYPE_INLINE || !is_inline_handle)
    {
        ...
        {
			#ifdef __MMI_WEARABLE_DEVICE__
        	if (MMI_TRUE == g_gui_ssp_pen_in_long_tap)
        	{
				//gui_ssp_execute_lsk(KEY_LONG_PRESS);
				g_gui_ssp_pen_in_long_tap = MMI_FALSE;
        	}
			else
			#endif /*__MMI_WEARABLE_DEVICE__*/
		...
		if (NULL != update_data_function)
		{
			gui_cancel_timer(gui_ssp_update_first_item_data);
		}
	#endif /*defined(__MMI_PHB_PBAPC_SYNC_CONTACT__) && defined(__MMI_BT_PBAP_CLIENT__)*/
    #endif /*__MMI_WEARABLE_DEVICE__*/
            if (g_gui_ssp_old_menu_highlight_item_index == MMI_fixed_list_menu.highlighted_item)
...
static MMI_BOOL gui_ssp_pen_up_hdlr(mmi_pen_point_struct point)
{
    ...
    /*----------------------------------------------------------------*/
	#ifdef __MMI_WEARABLE_DEVICE__
	gui_cancel_timer(swatch_list_menu_set_down_flag);
    ...
    if (g_gui_ssp_state == GUI_LIST_SMOOTH_SCROLLING_BY_PEN_STATE_HOLD)
    {
            ...
            (GUI_EFFECT_ABS(pen_down_to_up_y_delta) > (*g_gui_ssp_free_run_movement_y_thresold)))
        {
			...
			#ifdef __MMI_WEARABLE_DEVICE__
			else if (GUI_EFFECT_ABS(g_gui_ssp_free_run_momentum) > (S32)((float)item_height*(*g_gui_ssp_menu_trigger_free_run_coefficient)))
			#else
			...
			#endif 
            {
				...
				#ifdef __MMI_WEARABLE_DEVICE__
				if (GUI_EFFECT_ABS(g_gui_ssp_free_run_momentum) < SWATCH_MIN_FREE_RUN_MOMENTUM)
				{
					kal_prompt_trace(MOD_MMI, "xl gui_ssp_pen_up_hdlr g_gui_ssp_free_run_momentum %d < SWATCH_MIN_FREE_RUN_MOMENTUM %d",\
						g_gui_ssp_free_run_momentum, SWATCH_MIN_FREE_RUN_MOMENTUM);
...
#ifdef __MMI_WEARABLE_DEVICE__
	#if defined(__MMI_PHB_PBAPC_SYNC_CONTACT__) && defined(__MMI_BT_PBAP_CLIENT__)
	if (1 == show_first_item_flag && NULL != update_data_function && 0 == MMI_fixed_list_menu.first_displayed_item\
		&& MMI_fixed_list_menu.n_items != 1)
	{
	...
	if (!(MMI_fixed_list_menu.flags & UI_LIST_MENU_DISABLE_SCROLLBAR))
		gui_start_timer(500, gui_swatch_list_disable_scrollbar);
	#endif /*__MMI_WEARABLE_DEVICE_UI_ENHANCE__*/
...
#ifdef __MMI_WEARABLE_DEVICE__
		if (pen_down_show_highlight == MMI_FALSE)
		{
            #ifdef __MMI_FTE_SUPPORT__ 
         	if(g_gui_list_menu_smooth_scrolling_menu_type != GUI_MENU_TYPE_INLINE)
            #endif/*__MMI_FTE_SUPPORT__*/            
...
void gui_ssp_update_frame(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
	kal_prompt_trace(MOD_MMI,"gui_ssp_update_frame in\n");
    ...
    switch (g_gui_ssp_state)
    {
        case GUI_LIST_SMOOTH_SCROLLING_BY_PEN_STATE_HOLD:
			...
			if (menu_offset_y < 0)
			{
				if (0 == show_first_item_flag && NULL != update_data_function)
				{
            ...
            {
				kal_prompt_trace(MOD_MMI,"gui_ssp_update_frame menu_offset_y < low_bound\n");
...
//                        default_fist_item = m->n_items - gui_list_menu_smooth_scrolling_get_items_per_page();
        ...
        case GUI_LIST_SMOOTH_SCROLLING_BY_PEN_STATE_FREE_RUN:
            ...
            if ((*g_gui_ssp_menu_over_limit_coefficient) != 0)
            {
				#ifdef __MMI_WEARABLE_DEVICE__
				#if defined(__MMI_PHB_PBAPC_SYNC_CONTACT__) && defined(__MMI_BT_PBAP_CLIENT__)
				if (menu_offset_y < 0 && g_gui_ssp_free_run_mode == GUI_SSP_FREE_RUN_MODE_RUN)
				{
					if (0 == show_first_item_flag && NULL != update_data_function)
					{
						show_first_item_flag = 1;
						...
						menu_offset_y = 0;
						g_gui_ssp_state = GUI_LIST_SMOOTH_SCROLLING_BY_PEN_STATE_FREE_RUN;
						kal_prompt_trace(MOD_MMI, "xl gui_ssp_update_frame free run show first data");
					}
				}
				#endif /*defined(__MMI_PHB_PBAPC_SYNC_CONTACT__) && defined(__MMI_BT_PBAP_CLIENT__)*/
				#endif /*__MMI_WEARABLE_DEVICE__*/
...
//            if (g_gui_ssp_free_run_momentum)
	...
	kal_prompt_trace(MOD_MMI,"ssp_update_frame first_displayed_item is %d\n", m->first_displayed_item);
...
#endif
    gui_swla_log_stop("UFM");
	kal_prompt_trace(MOD_MMI,"gui_ssp_update_frame out\n");
}
...
static MMI_BOOL gui_ssp_pen_move_hdlr(mmi_pen_point_struct point)
{
    ...
    /*----------------------------------------------------------------*/
	#ifdef __MMI_WEARABLE_DEVICE__
	S32 delta_x = point.x - g_gui_ssp_pen_down_x;
	S32 delta_y = point.y - g_gui_ssp_pen_down_y;
    ...
    /*----------------------------------------------------------------*/
	#ifdef __MMI_WEARABLE_DEVICE__
	if (g_gui_ssp_move_direction == MMI_SWATCH_MOVE_DIRECTION_NONE)
	{
		if (abs_delta_x > abs_delta_y)
		{
			g_gui_ssp_move_direction = MMI_SWATCH_MOVE_DIRECTION_H;
			#ifdef __MMI_WEARABLE_DEVICE_UI_ENHANCE__
			MMI_fixed_list_menu.flags |= UI_LIST_MENU_DISABLE_SCROLLBAR;
			#endif /*__MMI_WEARABLE_DEVICE_UI_ENHANCE__*/
		}
		...
		{
			g_gui_ssp_move_direction = MMI_SWATCH_MOVE_DIRECTION_V;
			#ifdef __MMI_WEARABLE_DEVICE_UI_ENHANCE__
			MMI_fixed_list_menu.flags &= ~UI_LIST_MENU_DISABLE_SCROLLBAR;
			#endif /*__MMI_WEARABLE_DEVICE_UI_ENHANCE__*/
		}
	}
	if (abs_delta_x > LIST_SSP_MOVE_THRESHOLD || abs_delta_y > GUI_SSP_FOCUS_TO_HOLD_STATE_Y_THRESHOLD)
	{
		gui_cancel_timer(swatch_list_menu_set_down_flag);
		pen_down_in_list = MMI_FALSE;
		g_gui_ssp_launch = MMI_FALSE;
	}
	if (g_gui_ssp_move_direction == MMI_SWATCH_MOVE_DIRECTION_H)
	{
		gui_list_menu_smooth_scrolling_redraw_list();
		return MMI_FALSE;
	}
...
#ifdef __MMI_WEARABLE_DEVICE__
static MMI_BOOL gui_ssp_pen_long_tap_hdlr(mmi_pen_point_struct point)
{
	/*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
	...
	if (gui_is_fixed_list_menu_empty())//(MMI_fixed_list_menu.n_items == 0)
        return MMI_FALSE;
	if (UI_dummy_function != get_softkey_function(KEY_EVENT_LONG_PRESS, MMI_LEFT_SOFTKEY) && \
		gui_ssp_menu_translate_pen_position(&MMI_fixed_list_menu, (S32)point.x, (S32)point.y, &item_index) && \
		g_gui_ssp_state == GUI_LIST_SMOOTH_SCROLLING_BY_PEN_STATE_FOCUS)
	{
		gui_ssp_execute_lsk(KEY_LONG_PRESS);
		g_gui_ssp_pen_in_long_tap = MMI_TRUE;
	}
	return MMI_TRUE;
}
#endif /*__MMI_WEARABLE_DEVICE__*/
	...
	mmi_pen_event_type_enum pen_event)
{
	...
	{
		if (MMI_FALSE == gui_ssp_is_point_in_menu_rect(point.x, point.y))
		{
			return MMI_FALSE;
		}
	}
        ...
        case DM_INLINE_FIXED_LIST1:
            ...
            switch (pen_event)
            {
				...
				case MMI_PEN_EVENT_LONG_TAP:
					return gui_ssp_pen_long_tap_hdlr(point);
#endif /*__MMI_WEARABLE_DEVICE__*/
...
void gui_list_ssp_init_parameter(void)
{
    ...
    *g_gui_ssp_menu_over_limit_coefficient = GUI_SSP_MENU_OVER_LIMIT_COEFFICIENT;
	#ifdef __MMI_WEARABLE_DEVICE__
	*g_gui_ssp_menu_trigger_free_run_coefficient = GUI_SSP_MENU_TRIGGER_FREE_RUN_COEFFICIENT;
	#endif /*__MMI_WEARABLE_DEVICE__*/
}
...
//#define GUI_LIST_SSP_UT
#ifdef GUI_LIST_SSP_UT
void gui_list_ssp_ut(U8 index, kal_uint8* string)
{
	U32 step = 1;

	if (strcmp((char*)string, "SSP_1") == 0)
	{
		step = 1;
	}
	else if (strcmp((char*)string, "SSP_2") == 0)
	{
		step = 2;
	}
	else if (strcmp((char*)string, "SSP_5") == 0)
	{
		step = 5;
	}
	else if (strcmp((char*)string, "SSP_10") == 0)
	{
		step = 10;
	}
	...
	if(index==1)
	{
		kal_prompt_trace(MOD_MMI, "[SSP]*g_gui_ssp_focus_to_hold_state_y_threshold inc: %d", *g_gui_ssp_focus_to_hold_state_y_threshold+step);
		*g_gui_ssp_focus_to_hold_state_y_threshold+=step;
	}
	else if(index == 2)
	{
		kal_prompt_trace(MOD_MMI, "[SSP]*g_gui_ssp_focus_to_hold_state_y_threshold dec: %d", *g_gui_ssp_focus_to_hold_state_y_threshold-step);
		*g_gui_ssp_focus_to_hold_state_y_threshold-=step;
	}
	else if(index == 3)
	{
		if(*g_gui_ssp_menu_over_limit_coefficient <=0.0f)
		{
			kal_prompt_trace(MOD_MMI, "[SSP]*g_gui_ssp_menu_over_limit_coefficient = 0.5f");
			*g_gui_ssp_menu_over_limit_coefficient = 0.5f;
		}	
		else
		{
			kal_prompt_trace(MOD_MMI, "[SSP]*g_gui_ssp_menu_over_limit_coefficient = 0.0f");
			*g_gui_ssp_menu_over_limit_coefficient = 0.0f;
		}	
	}
	else if(index == 4)
	{
		*g_gui_menu_ssp_pos_delta += 1;
		kal_prompt_trace(MOD_MMI, "[SSP]*g_gui_menu_ssp_pos_delta inc: %d", *g_gui_menu_ssp_pos_delta);
	}
	else if(index == 5)
	{
		*g_gui_menu_ssp_pos_delta -= 1;
		kal_prompt_trace(MOD_MMI, "[SSP]*g_gui_menu_ssp_pos_delta dec: %d", *g_gui_menu_ssp_pos_delta);
	}
	else if(index == 6)
	{
		*g_gui_ssp_free_run_scroll_coefficient = *g_gui_ssp_free_run_scroll_coefficient + 0.01f;
		if(*g_gui_ssp_free_run_scroll_coefficient >= 0.99f)
			*g_gui_ssp_free_run_scroll_coefficient = 0.99f;
		kal_prompt_trace(MOD_MMI, "[SSP]*g_gui_ssp_free_run_scroll_coefficient inc: 0.01, now is %.2f", *g_gui_ssp_free_run_scroll_coefficient);
	}
	else if(index == 7)
	{
		*g_gui_ssp_free_run_scroll_coefficient = *g_gui_ssp_free_run_scroll_coefficient - 0.01f;
		if(*g_gui_ssp_free_run_scroll_coefficient <= 0.01f)
			*g_gui_ssp_free_run_scroll_coefficient = 0.01f;
		kal_prompt_trace(MOD_MMI, "[SSP]*g_gui_ssp_free_run_scroll_coefficient dec: 0.01, now is %.2f", *g_gui_ssp_free_run_scroll_coefficient);
	}
	else if(index == 8)
	{
		*g_gui_ssp_free_run_sensitivity = *g_gui_ssp_free_run_sensitivity + 0.01f;
		if(*g_gui_ssp_free_run_sensitivity >= 0.10f)
			*g_gui_ssp_free_run_sensitivity = 0.10f;
		kal_prompt_trace(MOD_MMI, "[SSP]*g_gui_ssp_free_run_sensitivity inc: 0.01, now is %.2f", *g_gui_ssp_free_run_sensitivity);
	}
	else if(index == 9)
	{
		*g_gui_ssp_free_run_sensitivity = *g_gui_ssp_free_run_sensitivity - 0.01f;
		if(*g_gui_ssp_free_run_sensitivity <= 0.01f)
			*g_gui_ssp_free_run_sensitivity = 0.01f;
		kal_prompt_trace(MOD_MMI, "[SSP]*g_gui_ssp_free_run_sensitivity dec: 0.01, now is %.2f", *g_gui_ssp_free_run_sensitivity);
	}
	#ifdef __MMI_WEARABLE_DEVICE__
	else if(index == 10)
	{
		*g_gui_ssp_menu_trigger_free_run_coefficient = *g_gui_ssp_menu_trigger_free_run_coefficient + 0.1f;
		if(*g_gui_ssp_free_run_sensitivity >= 1.0f)
			*g_gui_ssp_free_run_sensitivity = 1.0f;
		kal_prompt_trace(MOD_MMI, "[SSP]*g_gui_ssp_menu_trigger_free_run_coefficient inc: 0.1, now is %.2f", *g_gui_ssp_menu_trigger_free_run_coefficient);
	}
	else if(index == 11)
	{
		*g_gui_ssp_free_run_sensitivity = *g_gui_ssp_free_run_sensitivity - 0.1f;
		if(*g_gui_ssp_free_run_sensitivity <= 0.2f)
			*g_gui_ssp_free_run_sensitivity = 0.2f;
		kal_prompt_trace(MOD_MMI, "[SSP]*g_gui_ssp_menu_trigger_free_run_coefficient dec: 0.1, now is %.2f", *g_gui_ssp_menu_trigger_free_run_coefficient);
	}
	#endif /*__MMI_WEARABLE_DEVICE__*/
}
