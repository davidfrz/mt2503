...
extern void mmi_oppc_connect_ind_handle_for_smart_install(void *para);

extern void mmi_oppc_send_ind_handle_for_smart_install(void* para);
#endif
/*****************************************************************************
 * FUNCTION
 *  cui_bt_send_create
...
void cui_bt_sending_launch(MMI_ID parent_id, const cui_bt_sending_para_struct *para)
{
    ...
    data->total = 1;
#if defined(__MMI_SCREEN_LOCK_ANY_TIME__) || defined(__MMI_SCREEN_LOCK_ANY_TIME_WDEVICE__)
		mmi_frm_cb_reg_event(EVT_ID_SCREEN_LOCK_KEY_PRE_LOCK_EVT_ROUTING, cui_bt_sending_launch_proc, NULL);
#endif
#ifdef __MMI_WEARABLE_DEVICE__
		mmi_frm_cb_reg_event(EVT_ID_WEARABLE_DEVICE_MOVE_RIGHT, cui_bt_sending_launch_proc, NULL);
#endif
...
static void mmi_send_update_prog_tohdlr(void)
{
	...
	#else
		wgui_cat6335_update_all((WCHAR*)GetString(STR_GLOBAL_SENDING), mmi_oppc_calculate_persent());
	#endif
...
static void mmi_send_entry_sending_scr(void)
{
        ...
        mmi_send_entry_sending_scr, MMI_FRM_FULL_SCRN))
    {
        ...
        ShowCategory66Screen(
            ...
            NULL);
	#else
		wgui_showcategory6335_screen(0, STR_GLOBAL_CANCEL, (WCHAR*)GetString(STR_GLOBAL_SENDING), 0);
	    wgui_cat6335_update_all((WCHAR*)GetString(STR_GLOBAL_SENDING), 0);
	#endif
...
static void mmi_send_entry_cancelling_scr(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
	kal_prompt_trace(MOD_MMI_CONN_APP, "[OPPMMI] mmi_send_entry_cancelling_scr, 0");

    if(mmi_frm_scrn_enter(g_btsend_cntx.inst.gid, SCR_OPP_ABORTING, NULL,
        mmi_send_entry_cancelling_scr, MMI_FRM_FULL_SCRN))
    {
    
	kal_prompt_trace(MOD_MMI_CONN_APP, "[OPPMMI] mmi_send_entry_cancelling_scr 1");
    #ifndef __MMI_WEARABLE_DEVICE__
        ShowCategory66Screen(
            ...
            NULL);
	#else
		kal_prompt_trace(MOD_MMI_CONN_APP, "[OPPMMI] mmi_send_entry_cancelling_scr 2");
		wgui_showcategory6335_screen(0, 0, (WCHAR*)GetString(STR_GLOBAL_CANCELLING), 0);
	    wgui_cat6335_update_all((WCHAR*)GetString(STR_GLOBAL_CANCELLING), 0);
	#endif
    }
...
static mmi_ret mmi_send_sending_screen_proc(mmi_event_struct *evt)
{
    ...
    {
        case EVT_ID_SCRN_DEINIT:
		kal_prompt_trace(MOD_MMI_CONN_APP, "[OPPMMI] mmi_send_sending_screen_proc, send_deinit()");

		#if defined(__MMI_SCREEN_LOCK_ANY_TIME__) || defined(__MMI_SCREEN_LOCK_ANY_TIME_WDEVICE__)
			mmi_frm_cb_dereg_event(EVT_ID_SCREEN_LOCK_KEY_PRE_LOCK_EVT_ROUTING, cui_bt_sending_launch_proc,NULL);
		#endif
		#ifdef __MMI_WEARABLE_DEVICE__
			mmi_frm_cb_dereg_event(EVT_ID_WEARABLE_DEVICE_MOVE_RIGHT, cui_bt_sending_launch_proc, NULL);
		#endif
            mmi_opp_sending_scr_del();
            break;
    }
 ...
 *  void
 *****************************************************************************/
#if defined( __MMI_BT_APK_SMART_INSTALL_SUPPORT__) || (defined(__MMI_BT_APPLICATION_BQB__) && defined(__MMI_WEARABLE_DEVICE__))
extern void mmi_bt_send_apk_floder_init(void);
#endif
