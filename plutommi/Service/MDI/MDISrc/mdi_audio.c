...
void mdi_audio_bt_close_stream_ind(void *msg)
{
    ...
    if (state == MDI_AUDIO_BT_STATE_OPEN || state == MDI_AUDIO_BT_STATE_WAIT_CLOSE || state == MDI_AUDIO_BT_STATE_WAIT_OPEN)
    {
        ...
        if( mdi_audio_bt_cntx.pending_connect )
        {
            ...
            aud_send_bt_audio_open_req( MOD_MMI, BT_HFP, mode, 
                mdi_audio_bt_cntx.connect_id,mdi_audio_bt_cntx.pContext,mdi_audio_bt_cntx.req_context);
        }
        if (handler != NULL)
        {
...
void mdi_audio_bt_open_stream_req(void *msg)
{
    ...
    if (state == MDI_AUDIO_BT_STATE_IDLE)
    {
        ...
        aud_send_bt_audio_open_req(MOD_MMI, msg_p->profile, mode, msg_p->connect_id,msg_p->pContext,msg_p->req_context);
        mdi_audio_bt_cntx.state = MDI_AUDIO_BT_STATE_WAIT_OPEN;
    }
 ...
 * MMI_BOOL
 *****************************************************************************/
MMI_BOOL mdi_audio_bt_open_stream(U8 profile, U16 connect_id,void* pContext, void* req_context)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if (state == MDI_AUDIO_BT_STATE_IDLE)
    {
        ...
        else
        {
            ...
            aud_send_bt_audio_open_req(MOD_MMI, profile, mode, connect_id,pContext,req_context);
            mdi_audio_bt_cntx.state = MDI_AUDIO_BT_STATE_WAIT_OPEN;
        }
    ...
    else if( state == MDI_AUDIO_BT_STATE_WAIT_CLOSE )
    {
		...
		mdi_audio_bt_cntx.pContext = pContext;
		mdi_audio_bt_cntx.req_context = req_context;
...
void mdi_bt_speaker_power_off(void)
{
	...
	/* Code Body													  */
	/*----------------------------------------------------------------*/
	if(mdi_audio_mode==MDI_AUDIO_PLAY_BT_MUSIC)
	{
		mdi_audio_state = MDI_AUDIO_IDLE;
		mdi_audio_start_background_timer();
	}
