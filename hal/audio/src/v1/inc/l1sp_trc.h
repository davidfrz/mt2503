...
#define TRACE_GROUP_BT_AUD_SYNC  TRACE_GROUP_4
...
#define TRACE_GROUP_BT_VOC       TRACE_GROUP_1
...
BEGIN_TRACE_MAP(MOD_L1SP)
   ...
   TRC_MSG(DSDA_AUDL_M2A_ENTER, "[SP_Strm_Audl_Handler M2A] Enter")
   TRC_MSG(DSDA_AUDL_M2A_LEAVE, "[SP_Strm_Audl_Handler M2A] Leave %d")
   TRC_MSG(DSDA_AUDL_M2A_CCCI_BUF, "[SP_Strm_Audl_Handler M2A] ccci_buf %x %x %x %x , ptr %x len: %x")
   TRC_MSG(DSDA_AUDL_M2A_SEND, "[SP_Strm_Audl_Handler M2A] SEND j=%d src_len=%d")
   TRC_MSG(DSDA_AUDL_M2A_ATTEMPT, "[SP_Strm_Audl_Handler M2A] the %d-th attemp to send ")
   TRC_MSG(DSDA_AUDL_M2A_UL_SAMPLE_COUNT, "[SP_Strm_Audl_Handler M2A] accumulated ul_sample_count=%d ")
   TRC_MSG(DSDA_AUDL_M2A_WRITE_STATUS, "[SP_Strm_Audl_Handler M2A] WriteRet_size=%d mode=%d data_size=%d")
   TRC_MSG(DSDA_AUDL_M2A_FREEBUFFER, "[SP_Strm_Audl_Handler M2A] free_ctrl_buffer")
   
   TRC_MSG(DSDA_AUDL_A2M_ENTER, "[SP_Strm_Audl_Handler A2M] Enter")
   TRC_MSG(DSDA_AUDL_A2M_LEAVE, "[SP_Strm_Audl_Handler A2M] Leave %d")
   TRC_MSG(DSDA_AUDL_A2M_SEND, "[SP_Strm_Audl_Handler A2M] READ j=%d src_len=%d")
   TRC_MSG(DSDA_AUDL_A2M_ATTEMPT, "[SP_Strm_Audl_Handler A2M] the %d-th attemp to send ")
   TRC_MSG(DSDA_AUDL_A2M_READ_STATUS, "[SP_Strm_Audl_Handler A2M] ReadRet_size=%d mode=%d data_size=%d dl_sample_count=%d")
   TRC_MSG(DSDA_AUDL_A2M_CCCI_BUF, "[SP_Strm_Audl_Handler A2M] SpcIO_A2M_hisr ccci_buf[0-3]= %x %x %x %x")
   // AFE
   ...
   TRC_MSG(AFE_SET_SPH_REF_DIGI_GAIN, "AFE Set speech enhancement reference digital gain %d")
   ...
   // SND
   TRC_MSG(L1SPCSND_DSP_BUFFER_INFO, "BGSND buf count:%d; dsp_buf_addr(toPtr): %x, dsp_buf_len: %d word")
   TRC_MSG(L1SPCSND_FILLBUFFER, "BGSND[%d] Filled %d word to DSP, bgSnd.bufRead=%d, bgSnd.bufWrite=%d")
   TRC_MSG(L1SPCSND_PUT_SILENCE, "BGSND Filled %d word to DSP with silence pattern %x")
   TRC_MSG(L1SPCSND_WRITE_DATA, "BGSND[%d] Write data %d word to buffer %d, bgSnd.bufRead=%d, bgSnd.bufWrite=%d")
   TRC_MSG(L1SPCSND_STOP_FLUSH_COUNT, "BGSND Flush count: %d word")
   TRC_MSG(L1SPCSND_STOP_FLUSH_LEAVE, "BGSND Flush Still Leave: %d word")
   
   TRC_MSG(L1SPCSND_ENTER_BGSND_START, "Enter BGSND_Start")
   TRC_MSG(L1SPCSND_ENTER_BGSND_STOP, "Enter BGSND_Stop")
   TRC_MSG(L1SPCSND_ENTER_EVENT_HANDLER, "Enter bgsndEventHandler with state=%d")
   TRC_MSG(L1SPCSND_CONFIG_ULMIXER, "BGSND ConfigULMixer, flag:%d, gain:%d")
   TRC_MSG(L1SPCSND_CONFIG_DLMIXER, "BGSND ConfigDLMixer, flag:%d, gain:%d")

   TRC_MSG(L1SPCSND_REENTER_HISR, "SND re-enter HISR with isDlHisrCome=%d, isUlHisrCome=%d")
   ...
   //SPC 6252
   TRC_MSG(SPC_6252_SPEECH_ON,              "Spc_Speech_On %d")
   TRC_MSG(SPC_6252_SPEECH_OFF,             "Spc_Speech_Off")
   TRC_MSG(SPC_6252_SET_MIC_VOL,            "Spc_SetMicrophoneVolume %d")
   TRC_MSG(SPC_6252_SET_SIDETONE_VOL,       "Spc_SetSidetoneVolume %d")
   TRC_MSG(SPC_6252_TONE_PLAY,              "Spc_Default_Tone_Play %d %d")
   TRC_MSG(SPC_6252_TONE_STOP,              "Spc_Default_Tone_Stop")
   TRC_MSG(SPC_6252_SET_OUTPUT_VOL1,        "Spc_SetOutputVolume %d digital_gain_index = %d")
   TRC_MSG(SPC_6252_SET_OUTPUT_VOL2,        "Spc_SetOutputVolume %d gain_idx %d")
   TRC_MSG(SPC_6252_SET_INPUT_SRC1,         "Spc_SetInputSource param = %d")
   TRC_MSG(SPC_6252_SET_INPUT_SRC2,         "Spc_SetInputSource device=%d")
   TRC_MSG(SPC_6252_SET_OUTPUT_DEVICE1,     "Spc_SetOutputDevice param=%d")
   TRC_MSG(SPC_6252_SET_OUTPUT_DEVICE2,     "Spc_SetOutputDevice device=%d")
   TRC_MSG(SPC_6252_SET_SPEECHMODE_ADAPT,   "Spc_SetSpeechMode_Adaptation mode = %d")
   TRC_MSG(SPC_6252_RECORD_ON,              "Spc_Record_On")
   TRC_MSG(SPC_6252_RECORD_OFF,             "Spc_Record_Off")
   TRC_MSG(SPC_6252_RECORD_RESULT,          "Spc_Record Result = %d")
   TRC_MSG(SPC_6252_RECORD_OVERFLOW,        "Spc_Record Overflow, r=%d, w=%d")
   
   TRC_MSG(SIDETONE_START, "[sidetone] Enter SIDETONE_UpdateStart")
   TRC_MSG(SIDETONE_STOP, "[sidetone] Enter SIDETONE_UpdateStop") 
   TRC_MSG(SIDETONE_WRITE_TO_HW, "[sidetone] write gain with 0x%04X")
   ...
   TRC_MSG(SPCIO_SDIO_DEBUG2, "SPCIO[SDIO] r_srcLen1=%d r_srcLen2=%d curIdx=%d")   
   ...
   // BT Serial Playback Audio Synchronization
   TRC_MSG(BT_AUD_SYNC_INIT                  , "[BTAS][o]Init" )
   TRC_MSG(BT_AUD_SYNC_SET_START_TIME        , "[BTAS][o]Set start time, ch type %d, start time %X, offset %X" )
   TRC_MSG(BT_AUD_SYNC_SET_CH_TYPE           , "[BTAS][o]Set ch type %d" )
   TRC_MSG(BT_AUD_SYNC_GET_RESERVED_BYTE     , "[BTAS][x]Get reserved bytes %d" )
   TRC_MSG(BT_AUD_SYNC_GET_DELAY_TIMING      , "[BTAS][o]Get delay timing, device %d, delay %d" )
   TRC_MSG(BT_AUD_SYNC_ADD_START_TIME        , "[BTAS][x]Add start time, start time %x, payload len %d" )
   TRC_MSG(BT_AUD_SYNC_CHECK_STREAM          , "[BTAS][x]Check stream, payload len %d" )
   TRC_MSG(BT_AUD_SYNC_GET_SMPL_CNT          , "[BTAS][o]Get sample count, codec type %d, result %d, payload len %d, consumed %d, smpl cnt %d" )
   TRC_MSG(BT_AUD_SYNC_GET_SILENCE_LEN       , "[BTAS][o]Get silence length, codec type %d, silence cnt %d, silence len %d" )
   TRC_MSG(BT_AUD_SYNC_PRODUCE_SILENCE       , "[BTAS][o]Produce silence, codec type %d, result %d, silence cnt %d, silence len %d, produced %d" )
   TRC_MSG(BT_AUD_SYNC_PCMSINK_PUT_SILENCE   , "[BTAS][o]PCM sink put silence, silence word %d" )
   TRC_MSG(BT_AUD_SYNC_PCMSINK_DROP_REMAINED , "[BTAS][o]PCM sink drop remained, this drop word %d, remained drop word %d" )
   TRC_MSG(BT_AUD_SYNC_PCMSINK_DROP_DONE     , "[BTAS][o]PCM sink drop done, drop word %d, real data word %d" )
   TRC_MSG(BT_AUD_SYNC_PCMSINK_START         , "[BTAS][o]PCM sink start, state %d, value %d" )
