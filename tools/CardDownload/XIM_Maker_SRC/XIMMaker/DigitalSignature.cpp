...
unsigned int __stdcall ComputeCheckSum(unsigned char *content, unsigned int contentLen)
{
	unsigned int checksum = 0;

    const unsigned int numDWords = contentLen / 4;
    const unsigned int *p = reinterpret_cast<const unsigned int *>(content);
    
    for (unsigned int DwordIndex=0; DwordIndex<numDWords; ++DwordIndex)
    {
        checksum ^= *(p + DwordIndex);
    }

	// handling the remaing bytes with padding 0x00 when the contentLen is not 4K bytes
	if(contentLen % 4 != 0)
    {
	    printf("[Info]: ComputeCheckSum with padding bytes(len:%d) because contentLen(%d) is not 4-times number\n", 
			4 - (contentLen % 4), contentLen); 
		unsigned char temp_content[4];
		memset(temp_content, 0x0, sizeof(temp_content));
        CopyMemory(temp_content, content+(contentLen-(contentLen % 4)), (contentLen % 4));
		const unsigned int *padded_p = reinterpret_cast<const unsigned int *>(temp_content);
        checksum ^= *padded_p;
	}
...
{	
	// default: just prepare checksum
	unsigned int checksum = ComputeCheckSum(content, contentLen);
	CopyMemory(signature, &checksum, sizeof(checksum));
	return true; 
    //return false;
}
