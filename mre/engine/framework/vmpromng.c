...
#ifdef __IOT__
extern VMINT g_httpd_start_app_flag;
#endif
#ifdef __MMI_WEARABLE_DEVICE__
extern vm_frm_pen_move_exit pen_move_exit;
#endif
...
extern void vm_release_thread(VM_P_HANDLE process_handle);
#endif
...
VMINT vm_get_sym_entry(char* symbol) 
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if(strcmp(symbol,"vm_create_kal_timer") == 0)
    {
        return (VMINT)vm_create_kal_timer;
    }
    
    if(strcmp(symbol,"vm_set_kal_timer") == 0)
    {
        return (VMINT)vm_set_kal_timer;
    }
    
    if(strcmp(symbol,"vm_cancel_kal_timer") == 0)
    {
        return (VMINT)vm_cancel_kal_timer;
    }
	...
	if(strcmp(symbol,"mre") == 0)
		return 0;
	if(strcmp(symbol,"vm_app_log") == 0)
		 return (VMINT)vm_app_log;
	if(strcmp(symbol,"vm_get_tick_count") == 0)
		 return (VMINT)vm_get_tick_count;
	if(strcmp(symbol,"vm_ust_get_current_time") == 0)
		return (VMINT)vm_ust_get_current_time;
		if(strcmp(symbol,"vm_ust_get_duration") == 0)
			return (VMINT)vm_ust_get_duration;
    if(strcmp(symbol,"vm_set_time") == 0)
        return (VMINT)vm_set_time;
    if(strcmp(symbol,"vm_get_time") == 0)
        return (VMINT)vm_get_time;
    if(strcmp(symbol,"vm_get_rtc") == 0)
        return (VMINT)vm_get_rtc;	

#ifdef __MRE_SAL_BTGATT__
    returnvalue = vm_get_btgatt_sym(symbol); 
    if( returnvalue )
    {
        return returnvalue;
    }
	...
	returnvalue = vm_get_dcc_sym(symbol); 
    if( returnvalue )
    {
        return returnvalue;
    }
        ...
        returnvalue = vm_get_nwsetting_sym(symbol);
        if( returnvalue )
        {
            return returnvalue;
        }
	...
	MMI_PRINT(MOD_MRE,MMI_MRE_TRC_G10,"[launch][PID%d]vm_get_sym_entry : got symbol=%s, returnval=%d", vm_pmng_get_current_handle(),symbol, returnvalue);
	if( returnvalue )
	{
		return returnvalue;
	}
    ...
    returnvalue = vm_get_nat_sym(symbol);
    if( returnvalue )
    {
        return returnvalue;
    }
	...
	returnvalue = vm_get_tell_sym(symbol);
	if( returnvalue )
	{
		return returnvalue;
	}
    ...
    if(strcmp(symbol,"vm_disable_auto_screen_lock") == 0)
        return (VMINT)vm_disable_auto_screen_lock;
    if(strcmp(symbol,"vm_enable_auto_screen_lock") == 0)
        return (VMINT)vm_enable_auto_screen_lock;
    if(strcmp(symbol,"vm_switch_backlight") == 0)
        return (VMINT)vm_switch_backlight;  
#endif
    ...
    returnvalue = vm_get_btnotify_sym(symbol);
    if( returnvalue )
    {
        return returnvalue;
    }
...
#ifdef __IOT__
    if(strcmp(symbol,"vm_inet_httpd_start") == 0)
        return (VMINT)vm_inet_httpd_start;
    if(strcmp(symbol,"vm_inet_httpd_stop") == 0)
        return (VMINT)vm_inet_httpd_stop;
    if(strcmp(symbol,"vm_inet_httpd_restart") == 0)
        return (VMINT)vm_inet_httpd_restart;
    if(strcmp(symbol,"vm_inet_httpd_cgi_job_finish") == 0)
        return (VMINT)vm_inet_httpd_cgi_job_finish;
    if(strcmp(symbol,"vm_inet_httpd_read") == 0)
        return (VMINT)vm_inet_httpd_read;
    if(strcmp(symbol,"vm_inet_httpd_write") == 0)
        return (VMINT)vm_inet_httpd_write;
    if(strcmp(symbol,"vm_inet_httpd_getenv") == 0)
        return (VMINT)vm_inet_httpd_getenv;
    if(strcmp(symbol,"vm_inet_httpd_enable_ssl") == 0)
        return (VMINT)vm_inet_httpd_enable_ssl;
    if(strcmp(symbol,"vm_inet_httpd_disable_ssl") == 0)
        return (VMINT)vm_inet_httpd_disable_ssl;        
    if(strcmp(symbol,"vm_srv_funet_trigger") == 0)
        return (VMINT)vm_srv_funet_trigger;

    if(strcmp(symbol,"vm_create_bridge") == 0)
    {
        return (VMINT)vm_bridge_create;
    }  
    if(strcmp(symbol,"vm_close_bridge") == 0)
    {
        return (VMINT)vm_bridge_close;
    }
    if(strcmp(symbol,"vm_bridge_create") == 0)
    {
        return (VMINT)vm_bridge_create;
    }  
    if(strcmp(symbol,"vm_bridge_close") == 0)
    {
        return (VMINT)vm_bridge_close;
    }    

    if(strcmp(symbol,"vm_inet_rtspd_start") == 0)
    {
        return (VMINT)vm_inet_rtspd_start;
    } 
        if(strcmp(symbol,"vm_inet_rtspd_stop") == 0)
    {
        return (VMINT)vm_inet_rtspd_stop;
    } 
        if(strcmp(symbol,"vm_inet_rtspd_restart") == 0)
    {
        return (VMINT)vm_inet_rtspd_restart;
    }  

    if(strcmp(symbol,"vm_inet_ftpd_start") == 0)
    {
        return (VMINT)vm_inet_ftpd_start;
    } 
    if(strcmp(symbol,"vm_inet_ftpd_stop") == 0)
    {
        return (VMINT)vm_inet_ftpd_stop;
    } 
    if(strcmp(symbol,"vm_inet_ftpd_restart") == 0)
    {
        return (VMINT)vm_inet_ftpd_restart;
    }  
#endif
 
    if(strcmp(symbol,"vm_shutdown_normal_start") == 0)
    {
        return (VMINT)vm_shutdown_normal_start;
    } 
    if(strcmp(symbol,"vm_fly_mode_is_switching") == 0)
    {
        return (VMINT)vm_fly_mode_is_switching;
    }  

    if(strcmp(symbol,"vm_fly_mode_is_network_service_available") == 0)
    {
        return (VMINT)vm_fly_mode_is_network_service_available;
    }  
    
    if(strcmp(symbol,"vm_fly_mode_switch") == 0)
    {
        return (VMINT)vm_fly_mode_switch;
    } 

    if(strcmp(symbol,"vm_setting_srv_command") == 0)
    {
        return (VMINT)vm_setting_srv_command;
    }    

    if(strcmp(symbol,"vm_srv_sensor_acc_data_rigister_cb") == 0)
    {
        return (VMINT)vm_srv_sensor_acc_data_rigister_cb;  
    }
        
    if(strcmp(symbol,"vm_srv_sensor_hr_data_rigister_cb") == 0)
    {
        return (VMINT)vm_srv_sensor_hr_data_rigister_cb;  
    }
        
    if(strcmp(symbol,"vm_idle_swatch_pedometer_register_callback") == 0)
    {
        return (VMINT)vm_idle_swatch_pedometer_register_callback;     
    }

    if(strcmp(symbol,"vm_get_nvram_driver") == 0)
    {
        return (VMINT)vm_get_nvram_driver;  
    }
  if(strcmp(symbol,"vm_usb_get_cable_status") == 0)
        return (VMINT)vm_usb_get_cable_status;        

#ifdef __MRE_CORE_SM__
    if(strcmp(symbol,"vm_sm_reg_callback") == 0)
        return (VMINT)vm_sm_reg_callback;        
#endif	

    if(strcmp(symbol,"vm_magic_wrapper_for_customer") == 0)
    {
        return (VMINT)vm_magic_wrapper_for_customer;  
    }

		 
		if(strcmp(symbol,"vm_get_modules") == 0)
			return (VMINT)vm_get_modules;		 
		if(strcmp(symbol,"vm_get_version") == 0)
			return (VMINT)vm_get_version;	
		if(strcmp(symbol,"vm_get_total_mem_size") == 0)
			return (VMINT)vm_get_total_mem_size;		
		if(strcmp(symbol,"vm_pmng_set_pen_move_not_exit") == 0)
			return (VMINT)vm_pmng_set_pen_move_not_exit;		
		if(strcmp(symbol,"vm_pmng_set_pen_move_exit") == 0)
			return (VMINT)vm_pmng_set_pen_move_exit;		
        if(strcmp(symbol, "vm_switch_bt_call") == 0)
        {
            return (VMINT)vm_switch_bt_call;
        }
  
    if(strcmp(symbol,"vm_fudiffnet_trigger") == 0)
			return (VMINT)vm_fudiffnet_trigger;		
		if(strcmp(symbol,"vm_fudiffnet_get_fota_result") == 0)
			return (VMINT)vm_fudiffnet_get_fota_result;		
		if(strcmp(symbol,"vm_fudiffnet_get_firmware_path") == 0)
			return (VMINT)vm_fudiffnet_get_firmware_path;		

    if(strcmp(symbol,"vm_fudiffnet_get_fota_pre_info") == 0)
			return (VMINT)vm_fudiffnet_get_fota_pre_info;	
        
    if(strcmp(symbol, "vm_charbat_is_charging") == 0)
    {
        return (VMINT)vm_charbat_is_charging;
    }
    if(strcmp(symbol, "vm_get_chipRID") == 0)
    {
        return (VMINT)vm_get_chipRID;
    }
        if(strcmp(symbol, "vm_sms_is_sms_ready")==0)
        {
             return (VMINT)vm_sms_is_sms_ready;
        }
        
    if(strcmp(symbol, "vm_bt_cm_send_passkey") == 0)
    {
        return (VMINT)vm_bt_cm_send_passkey;
    }

    if(strcmp(symbol, "vm_custom_set_bt_pairing_method") == 0)
    {
        return (VMINT)vm_custom_set_bt_pairing_method;
    }
    if(strcmp(symbol, "vm_pdms_init") == 0)
    {
        return (VMINT)vm_pdms_init;
    }
    if(strcmp(symbol, "vm_pdms_deinit") == 0)
    {
        return (VMINT)vm_pdms_deinit;
    }
    if(strcmp(symbol, "vm_pdms_send_data") == 0)
    {
        return (VMINT)vm_pdms_send_data;
    }
    
    if(strcmp(symbol, "vm_timer_register_block_precise_timer") == 0)
    {
        return (VMINT)vm_timer_register_block_precise_timer;
    }
    if(strcmp(symbol,"vm_bthfp_hf_act_request") == 0)
    {
        return (VMINT)vm_bthfp_hf_act_request;  
    }
    if(strcmp(symbol,"vm_bthfp_hf_act_permit") == 0)
    {
        return (VMINT)vm_bthfp_hf_act_permit;  
    }
    if(strcmp(symbol,"vm_bthfp_hf_query_all_devices_call_count") == 0)
    {
        return (VMINT)vm_bthfp_hf_query_all_devices_call_count;
    }
    if(strcmp(symbol,"vm_bthfp_hf_query_single_device_call_count") == 0)
    {
        return (VMINT)vm_bthfp_hf_query_single_device_call_count;
    }
    if(strcmp(symbol,"vm_bthfp_hf_query_all_devices_group_count") == 0)
    {
        return (VMINT)vm_bthfp_hf_query_all_devices_group_count;
    }
    if(strcmp(symbol,"vm_bthfp_hf_query_single_device_group_count") == 0)
    {
        return (VMINT)vm_bthfp_hf_query_single_device_group_count;
    }
    if(strcmp(symbol,"vm_bthfp_hf_query_call_data") == 0)
    {
        return (VMINT)vm_bthfp_hf_query_call_data;  
    }
    if(strcmp(symbol,"vm_bthfp_hf_query_service_hd") == 0)
    {
        return (VMINT)vm_bthfp_hf_query_service_hd;  
    }
    if(strcmp(symbol,"vm_bthfp_hf_query_connected_dev_info") == 0)
    {
        return (VMINT)vm_bthfp_hf_query_connected_dev_info;  
    }
    if(strcmp(symbol,"vm_bthfp_hf_query_connected_dev_count") == 0)
    {
        return (VMINT)vm_bthfp_hf_query_connected_dev_count;  
    }
    if(strcmp(symbol,"vm_bthfp_hf_is_any_call") == 0)
    {
        return (VMINT)vm_bthfp_hf_is_any_call;  
    }
    if(strcmp(symbol,"vm_bthfp_hf_is_conference_call") == 0)
    {
        return (VMINT)vm_bthfp_hf_is_conference_call;  
    }
    if(strcmp(symbol,"vm_bthfp_hf_is_valid_service_hd") == 0)
    {
        return (VMINT)vm_bthfp_hf_is_valid_service_hd;  
    }
    if(strcmp(symbol,"vm_bthfp_hf_proc_inject_string") == 0)
    {
        return (VMINT)vm_bthfp_hf_proc_inject_string;  
    }
    if(strcmp(symbol,"vm_bthfp_hf_register_notify_callback") == 0)
    {
        return (VMINT)vm_bthfp_hf_register_notify_callback;  
    }
    if(strcmp(symbol,"vm_bthfp_hf_deregister_notify_callback") == 0)
    {
        return (VMINT)vm_bthfp_hf_deregister_notify_callback;  
    }        
    
    if(strcmp(symbol,"vm_create_timer_ts") == 0)
    {
        return (VMINT)vm_create_timer_ts;  
    } 
    if(strcmp(symbol,"vm_create_timer_ex_ts") == 0)
    {
        return (VMINT)vm_create_timer_ex_ts;  
    }     
...
_vm_pcb_t* vm_alloc_pcb(VMWSTR filename, _vm_pcb_t* father, VMINT isFromCard)
{
    ...
    {
    	MMI_TRACE(MMI_MRE_TRC_G10, TRC_MRE_LAUNCH_AUTO_42);
        return NULL;
    }
    ...
    MMI_TRACE(MMI_MRE_TRC_G10, TRC_MRE_LAUNCH_AUTO_41,pcbPtr->pHandle,pcbPtr->isReDriver,pcbPtr->is_support_bg_run);
...
void _vm_pmng_send_sysevt_msg(_vm_pcb_t * pcb, VMINT msg, VMINT param)
{
    ...
    MMI_TRACE(MMI_MRE_TRC_G10, TRC_MRE_LAUNCH_AUTO_49,pcb->pHandle,msg,param);        
    ...
    if (pcb->sysEventHandler)
    {
            ...
            || pcb->event_filter->sys_event_filter(msg, param))
        {
			...
			if(resend)
            {
				...
				MMI_TRACE(MMI_MRE_TRC_G10, TRC_MRE_LAUNCH_AUTO_50,pcb->pHandle,msg,param);        
...
#ifdef __IOT__
VMINT g_pmng_httpd_launch_ret = 0;
static VMINT g_pmng_create_app_para_flag = 0;
#endif
void vm_pmng_set_create_app_param(VMINT param)
{
    g_pmng_create_app_param = param; 
    #ifdef __IOT__   
	  g_pmng_create_app_para_flag = 1;
	  #endif
}
...
#ifdef __IOT__
VMINT vm_pmng_get_create_app_param_flag(void)
{
    VMINT flag = g_pmng_create_app_para_flag;
    g_pmng_create_app_para_flag = 0;
	g_httpd_start_app_flag = 0;
    return flag;
}
#endif
...
static VMINT vm_exec_process(_vm_pcb_t* pcb)
{
	...
	{		
		...
		else
		{
				...
				pcb->pHandle, &status)) != NULL && status != VM_PMNG_UNLOAD)
			{
                MRE_SWLA_START("M06");
			#ifdef __IOT__
				if(g_httpd_start_app_flag == 0)
				{
					_vm_pmng_send_sysevt_msg(pcb, VM_MSG_CREATE, vm_pmng_get_create_app_param());
				}
                else
                {
                	_vm_pmng_send_sysevt_msg(pcb, VM_MSG_CREATE, 0);
                }
			#else
				_vm_pmng_send_sysevt_msg(pcb, VM_MSG_CREATE, vm_pmng_get_create_app_param());
			#endif
			...
			#ifdef __MRE_CORT_TS__
				if(!vm_ts_check_boot_up_source(pcb))
				{
					_vm_pmng_send_sysevt_msg(pcb, VM_MSG_LOAD_TINY, 0);
				}
				else
				{
					_vm_pmng_send_sysevt_msg(pcb, VM_MSG_HIBERNATION_TO_AP_MODE, 0);
				}
			#endif
			...
			else
			{
                ...
                MRE_SWLA_STOP("M04");
				MMI_TRACE(MMI_MRE_TRC_G10, TRC_MRE_LAUNCH_AUTO_51,pcb->pHandle);
				return -1;
			}
            ...
            if (pcb->is_support_bg_run)
            {
                ...
                #ifdef __IOT__
				if(vm_pmng_get_create_app_param_flag())
				{
                    if(parameter_of_start!=NULL)
	                {
	                	#ifdef __INET_HTTPD__
		                inet_httpd_cgi_launch_ret(((cgi_launch_param *)parameter_of_start)->hc_handle,1);
		                g_pmng_httpd_launch_ret = 1;
		                #endif
	                }
				    _vm_pmng_send_sysevt_msg(pcb,VM_MSG_HTTPD,vm_pmng_get_create_app_param());
				}
                #endif
            ...
            else
            {
                ...
                MRE_SWLA_STOP("M04");
				MMI_TRACE(MMI_MRE_TRC_G10, TRC_MRE_LAUNCH_AUTO_52,pcb->pHandle);
                return -1;
            }
                ...
                pcb->pHandle, &status)) != NULL && status == VM_PMNG_UNLOAD)
            {
                ...
                MRE_SWLA_STOP("M04");
				MMI_TRACE(MMI_MRE_TRC_G10, TRC_MRE_LAUNCH_AUTO_53,pcb->pHandle);
                return -1;
        }      
    ...
    MRE_SWLA_STOP("M04");
	MMI_TRACE(MMI_MRE_TRC_G10, TRC_MRE_LAUNCH_AUTO_48);
    return -2;
}
...
void vm_cleanup_unload_status_pcb(void)
{
    ...
    {
        MMI_TRACE(MMI_MRE_TRC_G10,TRC_MRE_PRO_CLEAN_UNLOAD,tmp->pHandle);
...
void vm_pmng_finialize_ex(VMINT param)
{
    ...
    for (i = VM_PMNG_FOREGROUND; i < VM_PMNG_MAX_NUM; i++)
    {
        ...
        while (pcb != NULL)
        {
            if (pcb->isNoScreen == VM_FALSE || param != VM_QUIT_LANGUAGE_CHANGE)
        {
            tmp = pcb->next_pcb;
...
VM_P_HANDLE vm_pmng_create_process(VMWSTR fileName, VMUINT8* mem_base)
{
    ...
    {
    	MMI_TRACE(MMI_MRE_TRC_G10, TRC_MRE_LAUNCH_AUTO_60);
    ...
    if (suffix != VM_EXEC_ROM && suffix != VM_EXEC_VXP && suffix != VM_EXEC_DLL && suffix != VM_EXEC_NFB) {
		MMI_TRACE(MMI_MRE_TRC_G10, TRC_MRE_LAUNCH_AUTO_61,suffix);
    ...
    if (asm_index == MRE_SUPPORT_MAX_APP_NUM) {
		MMI_TRACE(MMI_MRE_TRC_G10, TRC_MRE_LAUNCH_AUTO_62,asm_index);
    ...
    if (suffix == VM_EXEC_VXP || suffix == VM_EXEC_DLL) {
        ...
        if (sysDriver >= L'A' && sysDriver <= L'Z') {
            if (fileDriver == sysDriver) {
				MMI_TRACE(MMI_MRE_TRC_G10, TRC_MRE_LAUNCH_AUTO_63);
                loadFromCard = TRUE;
            }
        }
    }
    ...
    if (pcb->mre_fun_list == NULL) {
		MMI_TRACE(MMI_MRE_TRC_G10, TRC_MRE_LAUNCH_AUTO_66);
    ...
    if (ret != 0) {
		MMI_TRACE(MMI_MRE_TRC_G10, TRC_MRE_LAUNCH_AUTO_67,ret);
    ...
    if (suffix == VM_EXEC_VXP || suffix == VM_EXEC_DLL) {
        ...
        if (pcb->fileHandle < 0) {
			MMI_TRACE(MMI_MRE_TRC_G10, TRC_MRE_LAUNCH_AUTO_68,ret);
    ...
    if (g_app_mem_info[asm_index].adm_id == NULL) {
		MMI_TRACE(MMI_MRE_TRC_G10, TRC_MRE_LAUNCH_AUTO_69);
    ...
    if (ret != 0) {
		MMI_TRACE(MMI_MRE_TRC_G10, TRC_MRE_LAUNCH_AUTO_70,ret);
...
free_pcb:
    vm_free_pcb(pcb);
unknown_type:
    MMI_TRACE(MMI_MRE_TRC_G10, TRC_MRE_PMNG_CREATE_PROCESS, lerr, info, ret);
    {
...
VM_P_HANDLE vm_pmng_create_process_from_preload_info(vm_preload_app_info_t *preload_info, vm_app_mem_struct* mem)
{
    ...
    if (!fileName || fileName[0] == 0)
    {
    	MMI_TRACE(MMI_MRE_TRC_G10, TRC_MRE_LAUNCH_AUTO_39);
        lerr = 1; goto unknown_type;
    }
    ...
    if (preload_info->saved_context.suffix == VM_EXEC_VXP 
        ...
        if (sysDriver >= L'A' && sysDriver <= L'Z') {
            if (fileDriver == sysDriver) {
				MMI_TRACE(MMI_MRE_TRC_G10, TRC_MRE_LAUNCH_AUTO_40);
                loadFromCard = TRUE;
            }
        }
    }
    ...
    pcb = vm_alloc_pcb(fileName, father_pcb, loadFromCard);
    if (pcb == NULL) {
		MMI_TRACE(MMI_MRE_TRC_G10, TRC_MRE_LAUNCH_AUTO_43);
        lerr = 4; goto unknown_type;
    }
    vm_set_pcb_flag(pcb);
    ...
    if (ret != 0) {
		MMI_TRACE(MMI_MRE_TRC_G10, TRC_MRE_LAUNCH_AUTO_44,ret);
        lerr = 7; info = ret; goto free_funlist;
    }
    ...
    if (suffix == VM_EXEC_VXP || suffix == VM_EXEC_DLL) {
        ...
        if (pcb->fileHandle < 0) {
            //MMI_TRACE(MMI_MRE_TRC_G8_PRO, TRC_MRE_PMNG_CREATE_OPEN);
            MMI_TRACE(MMI_MRE_TRC_G10, TRC_MRE_LAUNCH_AUTO_45,pcb->fileHandle);
        }
    }
    ...
    if (mem->adm_id == NULL) {
		MMI_TRACE(MMI_MRE_TRC_G10, TRC_MRE_LAUNCH_AUTO_46);
        lerr = 8; goto free_funlist;
    }
    ...
    ret = vm_exec_process(pcb);
    if (ret != 0) {
		MMI_TRACE(MMI_MRE_TRC_G10, TRC_MRE_LAUNCH_AUTO_47,ret);
        lerr = 9; info = ret; goto reset_asm;
    }
...
/*****************************************************************************
 * FUNCTION
 *  vm_pmng_get_current_authorization
 * DESCRIPTION
 *  
 * PARAMETERS
 *  void
 ...
 *  
 *****************************************************************************/
VMUINT32 vm_pmng_get_current_authorization(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    kal_prompt_trace(MOD_MRE, "[MRE_AUTH]get_current_auth from pcb.");
    if ((pcb = vm_sh_pcb_in_pt(SH_TYPE_R9, currAppR9, NULL)) != NULL)
    {
        kal_prompt_trace(MOD_MRE, "[MRE_AUTH]get_current_auth from pcb, success, auth = %d", pcb->user_auth);
        return pcb->user_auth;
    }
#ifdef __MRE_CORE_SM__
    else        /* Current R9 is SM copy */
    {
        ...
        if (sm_ptr && sm_ptr->pcb)
        {
            kal_prompt_trace(MOD_MRE, "[MRE_AUTH]get_current_auth from pcb, success, sm_auth = %d", sm_ptr->pcb->user_auth);
            return sm_ptr->pcb->user_auth;
        }
    }
#endif /* __MRE_CORE_SM__ */ 
    kal_prompt_trace(MOD_MRE, "[MRE_AUTH]get_current_auth from pcb, fail.");
    return 0xFFFFFFFF;
...
VMINT vm_pmng_deal_key_event(VMINT vmKeyCode, VMINT vmKeyEvent)
{
	...
	_vm_pcb_t* pcb = pcb_table[VM_PMNG_BACKGROUND];
	#else
 ...
 *  
 *****************************************************************************/
void vm_pmng_deal_status_event(VMINT event)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    for (i = VM_PMNG_FOREGROUND; i < VM_PMNG_MAX_NUM; i++)
    {
        for (tmp = pcb_table[i]; tmp != NULL; tmp = pcb)
        {
            ...
            switch(event)
			{
				case 0:
				{
            		_vm_pmng_send_sysevt_msg(tmp, VM_MSG_LOW_BATTERY, 0);
            		break;
				}
				case 1:
				{
            		_vm_pmng_send_sysevt_msg(tmp, VM_MSG_USB_CHARGER_IN, 0);
            		break;
				}
        		case 2:
        		{
        			_vm_pmng_send_sysevt_msg(tmp, VM_MSG_USB_CHARGER_OUT, 0);
        			break;
				}
				default:break;
			}
...
/*****************************************************************************
 * FUNCTION
 *  vm_pmng_deal_nw_info_status_changed_event
 * DESCRIPTION
 *  
 * PARAMETERS
 *  
 ...
 *  
 *****************************************************************************/
mmi_ret vm_pmng_deal_nw_info_status_changed_event(mmi_event_struct *evt)
{
    VM_PROCESS_STATUS i = VM_PMNG_UNLOAD;
    _vm_pcb_t *pcb = NULL, *tmp = NULL;
    ...
    for (i = VM_PMNG_FOREGROUND; i < VM_PMNG_MAX_NUM; i++)
    {
        ...
        {
            pcb = tmp->next_pcb;
            _vm_pmng_send_sysevt_msg(tmp, VM_MSG_NW_INFO_STATUS_CHANGED, 0);
        }
    }	
	return MMI_RET_OK;
}
...
VM_P_HANDLE vm_pmng_set_fg(VMWSTR fileName)
{
	...
	if (NULL == fileName)
	{
		MMI_TRACE(MMI_MRE_TRC_G10, TRC_MRE_LAUNCH_AUTO_15);
		return VM_PMNG_ERROR_PARAM;
	}
    ...
    {
    	MMI_TRACE(MMI_MRE_TRC_G10, TRC_MRE_LAUNCH_AUTO_16);
        return VM_PMNG_NO_SCHEDULE;
    }
...
VMINT vm_pmng_set_inactive(VM_P_HANDLE pHandle)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_MRE_TRC_G10,TRC_MRE_PRO_SET_INACTIVE,pHandle);
...
VMINT vm_pmng_delete_process_int(_vm_pcb_t* pcb, VM_PROCESS_STATUS status, VMINT param)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_MRE_TRC_G10,TRC_MRE_PRO_DELETE,pcb->pHandle,status,param);
...
#ifdef __MMI_WEARABLE_DEVICE__
    if(pen_move_exit.pHandle == pcb->pHandle)
    {
    	pen_move_exit.pHandle =0;
    	pen_move_exit.not_exit = 0;
    }
#endif
    vm_push_pcb_into_status_union(VM_PMNG_CLOSING, 
        vm_pop_pcb_up_status_union(status, pcb->pHandle));
...
VMINT vm_pmng_get_process_list(VM_P_HANDLE *handle_list, VMUINT *num)
{
    ...
    {        
        for (pcb = pcb_table[i]; ((pcb!=NULL)&&(index < *num)); pcb = pcb->next_pcb)
        {
...
VMINT vm_pmng_get_process_property(VM_P_HANDLE handle, vm_process_property *property)
{
    ...
    for (i = VM_PMNG_FOREGROUND; i < VM_PMNG_MAX_NUM; i++)
    {        
        for (pcb = pcb_table[i]; pcb!=NULL; pcb = pcb->next_pcb)
        {
...
static VMINT vm_pmng_get_path(VMINT drv, VMWSTR name, VMWSTR wname)
{
    VMINT find_hdl;
    struct vm_fileinfo_t info;

    kal_wsprintf((WCHAR *)wname, "%c:\\@BTMre\\installed\\bt_notify_mre_%w.vxp", drv, name);
    ...
    kal_wsprintf((WCHAR *)wname, "%c:\\@BTMre\\installed\\%w.vxp", drv, name);
    ...
    kal_wsprintf((WCHAR *)wname, "%c:\\mre\\%w.vxp", drv, name);
...
VMINT vm_pmng_get_driver_rom_path(VMWSTR vxp_name, VMWSTR vxp_wname)
{
    MMI_PRINT(MOD_MRE, MMI_MRE_TRC_G2_SERVICE, "[MRE][launch] get_driver_rom_path <app_path:%s>\n", vxp_name);
    
    if (kal_wstrchr((WCHAR *)vxp_name, '\\') == NULL)
    {
        if (!vm_pmng_get_path(vm_get_removeable_driver(), vxp_name, vxp_wname) )
        {
            MMI_PRINT(MOD_MRE, MMI_MRE_TRC_G2_SERVICE, "[MRE][launch]get_driver_rom_path, in system driver");
            
            if (!vm_pmng_get_path(vm_get_system_driver(), vxp_name, vxp_wname) )
            {
                if (!vm_pmng_get_path(vm_get_nvram_driver(), vxp_name, vxp_wname) )
                {
                    VMUINT i,appcount,smcount,socount,vtpcount, vxpcount;
                    VMWCHAR tempName[64];
                    VMCHAR tempName1[64];
                    
                    MMI_PRINT(MOD_MRE, MMI_MRE_TRC_G2_SERVICE, "[MRE][launch]get_driver_rom_path, in rom driver");
                    vm_init_default_game(&appcount, &smcount, &socount, &vtpcount, &vxpcount);
                    
                    for(i = 0; i < vxpcount; i++)
                    {
                        vm_ascii_to_ucs2((VMWSTR)tempName, 64, (VMSTR)mre_buildin_vxp_array[i].file_name);
                        if(kal_wstrcmp((WCHAR *)tempName, (WCHAR *)vxp_name) == 0)
                        {
#ifndef __MMI_MRE_3RD_ROM_PRELOAD__
                            sprintf(tempName1, "%c:\\%u.rom", 64, mre_buildin_vxp_array[i].address);
#else
                            sprintf(tempName1, "%c:\\%u.nfb", 64, mre_buildin_vxp_array[i].address);
#endif
                            vm_ascii_to_ucs2((VMWSTR)vxp_wname, 64, (VMSTR) tempName1);
                            MMI_PRINT(MOD_MRE, MMI_MRE_TRC_G2_SERVICE, "[MRE][launch]success to get driver_rom_path: %s", tempName1);
                            return VM_SM_SUCCESS;
                        }
                    }
                    MMI_PRINT(MOD_MRE, MMI_MRE_TRC_G2_SERVICE, "[MRE][launch]fail to get driver_rom_path");
    ...
    else
    {
        ...
        vm_wstrcpy(vxp_wname, vxp_name);

        if((find_hdl = vm_find_first(vxp_wname, &info)) >= 0)
        {
            vm_find_close(find_hdl);
        }
        else
