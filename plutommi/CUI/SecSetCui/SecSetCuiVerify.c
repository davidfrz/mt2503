...
typedef struct
{
 ...
 #else
    MMI_BOOL (*verify)(struct cui_verify_cntx_struct_s *cntx, const WCHAR *input);
 #endif
...
static void cui_verify_send_event_for_result_evt(MMI_BOOL success, cui_verify_fail_cause_enum fail_cause ,cui_verify_cntx_struct *cntx)
{
      cui_verify_result_evt_struct result_evt; 
                    
                MMI_FRM_INIT_EVENT(&result_evt, EVT_ID_CUI_VERIFY_RESULT);
                result_evt.success = success;
                result_evt.fail_cause = fail_cause;
                cui_verify_send_to_parent(cntx, &result_evt);
}

static void cui_verify_send_event_for_cancel_evt(cui_verify_cancel_cause_enum cause ,cui_verify_cntx_struct *cntx)
{
      cui_verify_cancel_evt_struct cancel_evt;
                    
            MMI_FRM_INIT_EVENT(&cancel_evt, EVT_ID_CUI_VERIFY_CANCEL);
			cancel_evt.cause = cause;
			cui_verify_post_to_parent(cntx, (void*)&cancel_evt);
}
#endif/*__MMI_SECSET_SLIM_FOR_61__*/
#ifndef __MMI_SECSET_SLIM_FOR_61__
...
static MMI_BOOL cui_verify_is_valid_cntx(cui_verify_cntx_struct *cntx)
{
 ...
 #else
    if(cntx->valid_magic != CUI_VERIFY_VALID_MAGIC || !mmi_frm_group_is_present(cntx->this_grp_id))
		return MMI_FALSE;
 #endif
...
static mmi_ret cui_verify_input_proc(mmi_event_struct *evt)
{
    ...
    switch (evt->evt_id)
    {
        case EVT_ID_CUI_PWD_BASIC_DONE:
        {
          ...
          #else
            if (cntx->type_config->verify(cntx, done_evt->input))
		  #endif/*__MMI_SECSET_SLIM_FOR_61__*/  
            {
            ...
            else
            {
                ...
                cui_verify_send_to_parent(cntx, &result_evt);
             #else
                cui_verify_send_event_for_result_evt(MMI_FALSE, CUI_VERIFY_FAIL_CAUSE_TEMP_FAIL , cntx);
			 #endif/*__MMI_SECSET_SLIM_FOR_61__*/
            }
        ...
        case EVT_ID_CUI_PWD_BASIC_CANCEL:
        {
          ...
          #else
            cui_verify_send_event_for_cancel_evt(CUI_VERIFY_CANCEL_CAUSE_USER_CANCEL ,cntx);
		  #endif
            break;
        }
...
mmi_id cui_verify_create(mmi_id grp_id, cui_verify_type_enum type)
{
  ...
  #ifdef __MMI_SECSET_SLIM_FOR_61__
	memset(cntx ,0x00, sizeof(cui_verify_cntx_struct));//init;
  #endif/*__MMI_SECSET_SLIM_FOR_61__*/
  ...
  #else
    if(type == CUI_VERIFY_TYPE_PHONE_PASSWORD)
	   cntx->type_config = &(cui_verify_type_config[0]);
	else
	   cntx->type_config = &(cui_verify_type_config[1]);
  #endif
  #ifndef __MMI_SECSET_SLIM_FOR_61__
    cntx->input_cui = 0;
...
void cui_verify_set_proc(mmi_id instance_id, mmi_proc_func proc, void *user_data)
{
  ...
  #else
    cntx = (cui_verify_cntx_struct*)mmi_frm_group_get_user_data(instance_id);
    if(cntx == NULL || cntx->valid_magic != CUI_VERIFY_VALID_MAGIC)
	   MMI_ASSERT(0);
  #endif
...
void cui_verify_run(mmi_id instance_id)
{
  ...
  #else
    if(cntx->type_config->type == CUI_VERIFY_TYPE_CHV2 && !cui_verify_sim_pre_check(cntx))//only PIN2 pre check;
  #endif/*__MMI_SECSET_SLIM_FOR_61__*/
    {
...
/* under construction !*/
  ...
  #else    
    memset(extra_msg , 0x00, (CUI_PWD_MAX_EXTRA_MSG_STR_LEN + 30) * sizeof(WCHAR));
	if(cntx->type_config->type == CUI_VERIFY_TYPE_CHV2)//only PIN2 
...
static mmi_ret cui_verify_phone_verify_result_proc(mmi_event_struct *evt)
{
    ...
    if (evt->evt_id == EVT_ID_SRV_SECSET_VERIFY_PHONE_PASSWORD_RESULT)
    {
        ...
        if (verify_result_evt->success)
        {
            ...
            cui_verify_send_to_parent(cntx, &result_evt);
          #else
            cui_verify_send_event_for_result_evt(MMI_TRUE ,CUI_VERIFY_FAIL_CAUSE_NONE, cntx);
		  #endif/*__MMI_SECSET_SLIM_FOR_61__*/
        }
        else
        {
    ...
    for (list_cntx = g_cui_verify_cntx_global.list; list_cntx != NULL; )
    {
            ...
            list_cntx->type_config->type == cntx->type_config->type)
        {
            if (mmi_frm_group_is_present(list_cntx->parent_grp_id))
            {
              ...
              #else
				cui_verify_send_event_for_cancel_evt(CUI_VERIFY_CANCEL_CAUSE_ANOTHER_INST_RUN ,list_cntx);
			  #endif/*__MMI_SECSET_SLIM_FOR_61__*/
            }
        }
...
static MMI_BOOL cui_verify_sim_pre_check(cui_verify_cntx_struct *cntx)
{
    ...
    if (!srv_sim_ctrl_is_available(cntx->sim))
    {
	 ...
	 #else
	    cui_verify_send_event_for_cancel_evt(CUI_VERIFY_CANCEL_CAUSE_UNAVAILABLE ,cntx);
	 #endif
    ...
    else if (srv_sim_ctrl_pwd_is_blocked(cntx->sim, pwd_type))
    {
        ...
        else
        {
		  ...
		  #else
		    cui_verify_send_event_for_result_evt(MMI_FALSE ,CUI_VERIFY_FAIL_CAUSE_OPERATION_BLOCKED ,cntx);
		  #endif
        }

        return MMI_FALSE;
    }
  /*cui verify only for PIN2, but PIN2 not enable/disable OP;*/
  #ifndef __MMI_SECSET_SLIM_FOR_61__
...
static mmi_ret cui_verify_sim_ubchv_proc(mmi_event_struct *evt)
{
    ...
    switch (evt->evt_id)
    {
        ...
        case EVT_ID_SECSET_UBCHV_SIM_ERROR:
        {
            ...
            cui_verify_send_to_parent(cntx, &result_evt);
          #else
            cui_verify_send_event_for_result_evt(MMI_FALSE , CUI_VERIFY_FAIL_CAUSE_SIM_ERROR ,cntx);
		  #endif/*__MMI_SECSET_SLIM_FOR_61__*/
            break;
        }
    }
