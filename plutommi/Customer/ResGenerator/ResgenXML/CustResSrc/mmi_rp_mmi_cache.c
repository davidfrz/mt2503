...
static void mmi_rp_mmi_cache_output_phase2_restore_function(FILE *p_file)
{
    ...
    for (i = 0 ; i < MMI_RP_MMI_CACHE_TYPE_MAX ; i++)
    {
        fprintf(p_file, "extern const kal_uint8 NVRAM_EF_MMI_CACHE_%s_DEFAULT[];\n",
...
#if (defined(__UPDATE_BINARY_FILE__) && !defined(__NVRAM_SUPPORT_CUSTPACK__))
        fprintf(p_file, "    const kal_uint8 *def_buf;\n");
        fprintf(p_file, "    def_buf = NVRAM_EF_MMI_CACHE_%s_DEFAULT;\n",
            mmi_rp_mmi_cache_get_type_str(i));

		fprintf(p_file, "    kal_prompt_trace(MOD_PLUTO_FW, \"[MMI_FRM][NVRAM CACHE]NVRAM_EF_CACHE_%s_DEFAULT factory restore\");\n",
			mmi_rp_mmi_cache_get_type_str(i));
