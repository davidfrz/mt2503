...
static kal_uint16 g_aud_tts_app_id = 0;
static kal_uint32 g_last_seq_no = 0;
...
void aud_tts_set_attr_req_hdlr(ilm_struct *ilm_ptr)
{
    ...
    aud_set_result_and_event(result,TTS_EVT_SET_ATTR);
}
...
void aud_tts_util_close_and_free(void)
{
    ...
    med_free_asm_mem(g_aud_tts_app_id, (void**)&g_aud_tts_buff_p);
...
void aud_tts_play_event_ind(TTS_Handle *hdl, TTS_Event event, void *data)
{
    ...
    ind_p->event = event;
    ind_p->seq_no = g_last_seq_no;

    MED_ASSERT(sizeof(TTS_PROCESS_DATA) <= AUD_TTS_MAX_PROCESS_DATA_SIZE);

    if((event == TTS_WATCH || event == TTS_PROGRESS) && (data != NULL))
    {
...
void aud_tts_process_event_ind_hdlr(ilm_struct *ilm_ptr)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if (g_last_seq_no != ind_p->seq_no ||
        (aud_context_p->state != AUD_MEDIA_PLAY && aud_context_p->state != AUD_MEDIA_PLAY_PAUSED))
    {
        AUD_VALUE_TRACE(ind_p->seq_no, g_last_seq_no, aud_context_p->state);
        return;
    }
    ...
    switch (ind_p->event)
    {
        case TTS_END:
        case TTS_ERROR:
            MED_ASSERT(g_aud_tts_handle != NULL);
            ...
            if(ind_p->event == TTS_END)
            {
                aud_send_audio_play_finish_ind(MED_RES_END_OF_FILE, aud_context_p->identifier);
            }
            else
            {
                aud_send_audio_play_finish_ind(MED_RES_ERROR, aud_context_p->identifier);
            }
            break;

       case TTS_PROCESS:
            g_aud_tts_handle->Process(g_aud_tts_handle, ind_p->event);
            break;
...
void aud_tts_play_req_hdlr(ilm_struct *ilm_ptr)
{
    ...
    kal_uint16 i;
    Media_Status result;
    kal_int32 med_result = MED_RES_BUSY;
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    // TODO: ADD Trace
    ...
    g_aud_tts_app_id = msg_p->app_id;
    buff_size = g_aud_tts_handle->GetBufSize(g_aud_tts_handle);
    MED_ASSERT(buff_size > 0);

    if((g_aud_tts_buff_p = (kal_uint8*)med_alloc_asm_mem(g_aud_tts_app_id, buff_size)) == NULL)
    {
        g_aud_tts_handle->Close(g_aud_tts_handle);
        g_aud_tts_handle = NULL;
    ...
    med_result = (kal_int32) aud_get_res((kal_uint8)result);

play_tts_done:
    aud_set_result_and_event(med_result,TTS_EVT_PLAY);
}
...
void aud_tts_stop(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    AUD_VALUE2_TRACE(aud_context_p->state, __LINE__);
...
void aud_tts_stop_req_hdlr(ilm_struct *ilm_ptr)
{
    ...
    aud_set_result_and_event((kal_int32)MED_RES_OK,TTS_EVT_STOP);
}
...
void aud_tts_pause_req_hdlr(ilm_struct *ilm_ptr)
{
    ...
    aud_set_result_and_event((kal_int32)aud_get_res((kal_uint8)result),TTS_EVT_PAUSE);
}
...
void aud_tts_resume_req_hdlr(ilm_struct *ilm_ptr)
{
    ...
    aud_set_result_and_event((kal_int32) aud_get_res((kal_uint8)result),TTS_EVT_RESUME);
}
