...
DCL_HANDLE DclPWM_Extra_Open(DCL_DEV dev, DCL_FLAGS flags);
DCL_HANDLE DclPWM_Extra_Close(DCL_HANDLE handle);


/*==============add for common dcl layer===============*/
extern DEV_Operations pwm_dev_operations = {DclPWM_Extra_Open,   /*open*/
											NULL,     /*read*/
											NULL,     /*write*/
											DclPWM_Control,    /*control*/ 
											NULL,      /*callback*/
											NULL,     /*query*/
											DclPWM_Extra_Close}; /*close*/
...
DCL_HANDLE DclPWM_Extra_Open(DCL_DEV dev, DCL_FLAGS flags)
{
	switch(dev)
	{
		case VM_DCL_KPLED_PWM:
		{
			return ((DCL_LED_KP - DCL_PWM1) | PWM_HANDLE_MAGIC_NUM);
			break;
		}

		case VM_DCL_PWM1:
		{
			return ((VM_DCL_PWM1 - VM_DCL_HW_PWM_GROUP_START - 1) | PWM_HANDLE_MAGIC_NUM);
			break;
		}
		case VM_DCL_PWM2:
		{
			return ((VM_DCL_PWM2 - VM_DCL_HW_PWM_GROUP_START - 1) | PWM_HANDLE_MAGIC_NUM);
			break;
		}
		case VM_DCL_PWM3:
		{
			return ((VM_DCL_PWM3 - VM_DCL_HW_PWM_GROUP_START - 1) | PWM_HANDLE_MAGIC_NUM);
			break;
		}
		case VM_DCL_PWM4:
		{
			return ((VM_DCL_PWM4 - VM_DCL_HW_PWM_GROUP_START - 1) | PWM_HANDLE_MAGIC_NUM);
			break;
		}
		default:
		{
...
DCL_STATUS DclPWM_Control(DCL_HANDLE handle, DCL_CTRL_CMD cmd, DCL_CTRL_DATA_T *data)
{
   ...
   if(DCL_PWM_IS_HANDLE_MAGIC(handle))
   {
      ...
      iPWMMap = (1<<iPWMNum);
      // if PWM number incorrect or this PWMUsed flag is 0, return invalid.
      #if 0  // for support MRE, mask this check. pwm is simple ,there is no need to do this check
/* under construction !*/
/* under construction !*/
/* under construction !*/
/* under construction !*/
/* under construction !*/
 ...
 //kal_prompt_trace(MOD_DRV_HISR,"[PWM] handle=%x,cmd=%x,pwm=%d",handle,cmd,iPWMNum);
 switch (cmd)
      {
   ...
   /*------------------------------------------------------*/
   // this command is add for mre control PWM
   case PWM_CMD_SET_SOURCE_CLOCK:
         {
            PWM_SET_SOURCE_CLOCK_T	*pdata;
            pdata=&(data->rPWMSetClk);
            comm_pwm_driver->pwmClkInit(iPWMNum,pdata->source_clk,pdata->source_clk_div);
            break;
         }
   case PWM_CMD_SET_COUNTER_AND_THRESHOLD:
   		{
            PWM_SET_COUNTER_THRESHOLD_T *pdata;
            pdata=&(data->rPWMSetCountThreshold);
			
			DCL_PWM_SetCounterAndThreshold(iPWMNum,pdata->counter,pdata->threshold);
			break;
   		}
   /*------------------------------------------------------*/
