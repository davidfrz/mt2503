...
static void mmi_bt_dialor_connect_int(srv_bt_cm_bt_addr* dev_addr, mmi_bt_dialer_conn_callback_item *callback_item, MMI_BOOL is_conn);
...
static void mmi_bt_dialer_conn_confirm_scrn(srv_bt_cm_bt_addr* dev_addr);
static void mmi_bt_dialer_disconn_confirm_scrn(srv_bt_cm_bt_addr* dev_addr);
...
#ifdef __MMI_WEARABLE_DEVICE__
MMI_BOOL g_mmi_bt_dialer_move_rgt = MMI_TRUE;
#endif
/**************************************************************************** 
 * Local Variable
 *****************************************************************************/
...
static MMI_BOOL g_mmi_bt_is_first_connect = MMI_TRUE;
...
#else
	mmi_bt_entry_wristband_setting_menu();
#endif
}
...
void mmi_bt_show_dialor_conn_success(srv_bt_cm_dev_struct* dev_p)
{
	...
	MMI_TRACE(MMI_CONN_TRC_G7_BT, MMI_BT_CONN_SHOW_POP_STRING, string1);
#ifndef __MMI_BT_BTBOX_NOLCD__
    ...
    {
    #ifdef __MMI_BT_BTBOX_NOLCD__
        if(BT_A2DP_SOURCE == srv_a2dp_get_role(&dev_p->bd_addr))
        {
            return;
        }
    #endif /* __MMI_BT_BTBOX_NOLCD__ */
    ...
    if(!srv_bt_cm_get_prevent_connect_flag()&& (srv_bt_cm_get_linked_dev_list_idx() == BT_DEVICE_LINK_NUM))
	{
		srv_bt_cm_set_prevent_connect_flag(MMI_TRUE);  //link count is full, disable visiably.
	}
...
static void mmi_bt_dialer_callback_done_int(srv_bt_cm_bt_addr* dev_addr, MMI_BOOL is_connect)
{
    ...
    if (is_connect)
    {
        ...
        if (mmi_bt_dialer_app_is_connected_ex(&dev_p->bd_addr))
        {
          ...
          #ifdef __MMI_BT_BTBOX_NOLCD__
            idx = mmi_bt_dialer_get_autoreconn_inx_array(&dev_p->bd_addr);
            if(idx >= 0)
                mmi_bt_dialer_clean_autoreconn_dev(&g_mmi_bt_reconn_ctx[idx]);
          #endif /* __MMI_BT_BTBOX_NOLCD__ */
        #endif/*__MMI_BT_DIALER_AUTO_RECONN_SUPPORT__*/
        #ifndef __MMI_BT_BTBOX_NOLCD__
          //  mmi_frm_scrn_close(g_mmi_bt_scr_cntx.main_gid, SCR_BT_CONNECTING);
          mmi_bt_stop_progress_scr(g_mmi_bt_scr_cntx.main_gid, SCR_BT_CONNECTING);
		mmi_frm_scrn_close(g_mmi_bt_scr_cntx.mydev_gid, SCR_BT_DIALER_DISCONNECT_ASK);
        #endif /* __MMI_BT_BTBOX_NOLCD__ */
        }
        else
        {
            MMI_TRACE(MMI_CONN_TRC_G7_BT, MMI_BT_CONN_FAIL_POPUP);
			g_mmi_bt_is_first_connect = MMI_TRUE;  //no profile connected, next connect will be thought as first connect.
        #ifndef __MMI_BT_BTBOX_NOLCD__
            if (show_pop)
            {
    ...
    else
    {
        if (!mmi_bt_dialer_app_is_connected_ex(&dev_p->bd_addr))
        {
          ...
          #else 
            idx = mmi_bt_anti_lost_list_get_idx(&dev_p->bd_addr);
            if (idx != -1)
...
/* under construction !*/
            ...
            #endif /* __MMI_BT_BTBOX_NOLCD__ */
                mmi_wcs_to_asc((CHAR *)string1, (WCHAR *)string);
				MMI_TRACE(MMI_CONN_TRC_G7_BT, MMI_BT_CONN_DIS_STRING_1, string1);
            #ifndef __MMI_BT_BTBOX_NOLCD__
                mmi_wcs_to_asc((CHAR *)string1, (WCHAR *)dev_p->name);
			MMI_TRACE(MMI_CONN_TRC_G7_BT, MMI_BT_CONN_DIS_STRING_NAME, string1);
            #endif /* __MMI_BT_BTBOX_NOLCD__ */
				g_mmi_bt_is_first_connect = MMI_TRUE;
		 ...
		 mmi_bt_stop_progress_scr(g_mmi_bt_scr_cntx.main_gid, SCR_BT_CONNECTING);
	        ...
	        mmi_frm_scrn_close(g_mmi_bt_scr_cntx.mydev_gid, SCR_BT_DIALER_DISCONNECT_ASK); 
                ...
                temp_dev.bt_dialer_reconn_flag = MMI_FALSE;
                ...
                temp_dev.bt_dialer_idx = mmi_bt_dialer_get_index_by_dev_info(dev_addr);
    ...
    if (mmi_bt_dialer_get_time_out_flag())
    {
	 ...
	 if(srv_bt_cm_get_prevent_connect_flag())
	 {
		...
		{
			srv_bt_cm_set_prevent_connect_flag(MMI_FALSE);  //link count is full, disable visiably.
		}
	 }
	 else
	 {
...
void mmi_bt_dialor_callback(srv_bt_cm_bt_addr* dev_addr, srv_bt_cm_connection_type conn_type, MMI_BOOL result,MMI_BOOL is_connect, MMI_BOOL is_indicate)
{
	...
	if(is_indicate && is_connect)  
	{
		if(g_mmi_bt_is_first_connect) //if first connect,  will trigger connect all profile.
		{
#ifdef __MMI_BT_DIALER_AUTO_RECONN_SUPPORT__
      mmi_bt_dialer_auto_reconn_reset();
#endif/*__MMI_BT_DIALER_AUTO_RECONN_SUPPORT__*/
			conn_type = SRV_BT_CM_NO_CONNECTION;  //trigger connect all.			
		}
		else 
		{
			return;
	...
	if(g_mmi_bt_is_first_connect)
	{
		g_mmi_bt_is_first_connect = MMI_FALSE;  //for all connect
	}
	callback_item = mmi_bt_dialer_next_callback_item(&dev_p->bd_addr, conn_type, is_connect, is_indicate);
	if((callback_item->type == SRV_BT_CM_PBAPC_CONNECTION || callback_item->type == SRV_BT_CM_MAPC_CONNECTION) && is_connect)
	{
		mmi_bt_stop_progress_scr(g_mmi_bt_scr_cntx.main_gid, SCR_BT_CONNECTING);
		mmi_frm_scrn_close(g_parent_id, SCR_BT_INQUIRED_POWER_ON);
	}
...
static void mmi_bt_dialor_callback_int(srv_bt_cm_bt_addr* dev_addr,  MMI_BOOL result,MMI_BOOL is_connect, mmi_bt_dialer_conn_callback_item *callback_item)
{
	...
	if(callback_item != NULL)
	{
		if (is_connect)
		{
			...
			{
				mmi_bt_dialer_callback_done(dev_addr, result, is_connect);
			}
		}
		else
		{
...
void mmi_bt_dialor_disconnect(srv_bt_cm_bt_addr* dev_addr)
{
        ...
        (callback_item->indx != 0xFF))
    {
		...
		if(!srv_bt_cm_get_prevent_connect_flag())
		{
			...
			change_vis_dev.is_connect = MMI_FALSE;
			MMI_TRACE(MMI_CONN_TRC_G7_BT,MMI_BT_CONN_CBITEM_NULL);
			if(mmi_bt_dialer_set_conn_vis_enum(change_vis_dev, MMI_TRUE))
			{
			   return;
...
void mmi_bt_dialor_connect_ex(srv_bt_cm_bt_addr* dev_addr)
{	
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
        ...
        (callback_item->indx) != 0xFF)
    {
    ...
    #endif/*__MMI_BT_DIALER_AUTO_RECONN_SUPPORT__*/
        if (show_pop_up && g_mmi_bt_scr_cntx.main_gid)
        {
        ...
        if (callback_item != NULL)
        {
            ...
            memcpy(&change_vis_dev.callback_item,callback_item, sizeof(mmi_bt_dialer_conn_callback_item));
            change_vis_dev.g_vis_type = MMI_BT_CONN_ACTIVE;
				change_vis_dev.is_connect = MMI_TRUE;
            MMI_TRACE(MMI_CONN_TRC_G7_BT,MMI_BT_CONN_CBITEM_NULL);
            if(mmi_bt_dialer_set_conn_vis_enum(change_vis_dev, MMI_TRUE))
            {
                return;
...
void mmi_bt_dialor_connect(srv_bt_cm_bt_addr * dev_addr)
{
 	...
 	mmi_bt_dialer_auto_reconn_reset();
#endif/*__MMI_BT_DIALER_AUTO_RECONN_SUPPORT__*/
	mmi_bt_dialor_connect_ex(dev_addr);
}

void mmi_bt_dialor_auto_reconnect(srv_bt_cm_bt_addr * dev_addr)
{
	mmi_bt_dialor_connect_ex(dev_addr);
}
...
static void mmi_bt_dialor_connect_int(srv_bt_cm_bt_addr* dev_addr, mmi_bt_dialer_conn_callback_item *callback_item, MMI_BOOL is_conn)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_CONN_TRC_G7_BT,MMI_BT_CONN_CONNECT_FROM_DIALER_INT,dev_addr->lap,dev_addr->nap,dev_addr->uap, is_conn);
	if(is_conn)
	{
    callback_item->conn(dev_addr, mmi_bt_dialor_callback);
}
...
mmi_ret mmi_bt_auto_connect_ind_proc(mmi_event_struct* evt)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
void mmi_bt_dialer_auto_connect(mmi_id group_id)
{
    ...
    if ((srv_bt_cm_get_busy_dev() == NULL) && !srv_bt_cm_get_btdialor_app_mode() &&(srv_bt_cm_get_linked_dev_list_idx() <= BT_DEVICE_LINK_NUM) 
		  && !mmi_bt_dialer_app_is_connected_ex(&g_connected_dev_info[0].bd_addr))
    {
        if (srv_bt_cm_get_power_status() == SRV_BT_CM_POWER_OFF)
        {
        ...
        else if(srv_bootup_is_hibernation_mode())
        {
        	MMI_TRACE(MMI_CONN_TRC_G7_BT, MMI_BT_CONN_AUTO_HIBERNATION); 
			return;
		}
#endif		
        else
        {
           ...
           //g_mmi_bt_scr_cntx.main_gid = mmi_frm_group_get_active_id();
           g_mmi_bt_scr_cntx.main_gid = mmi_frm_group_create_ex(GRP_ID_ROOT, GRP_ID_BT_AUTO_CONNECT, mmi_bt_auto_connect_ind_proc, NULL, MMI_FRM_NODE_SMART_CLOSE_FLAG); 
			mmi_bt_dialer_conn_dev_by_addr(g_connected_dev_info[0]);
         //connect device.
        }
    }
...
void mmi_bt_dialer_cent(void)
{
    ...
    if (srv_bt_cm_get_btdialor_app_mode() == MMI_TRUE)
    {
	    ...
	    {    
            ...
            {
#ifdef __MMI_WEARABLE_DEVICE__
				mmi_bt_dialer_disconn_confirm_scrn(&g_mmi_bt_scr_cntx.conn_dev_add);
...
#ifdef __MMI_WEARABLE_DEVICE__
				mmi_bt_dialer_conn_confirm_scrn(&g_mmi_bt_scr_cntx.conn_dev_add);
#else/*__MMI_WEARABLE_DEVICE__*/
				if (mmi_bt_is_dev_cod_match(dev_p->cod, (MMI_BT_AUDIO_MAJOR_SERVICE_MASK|MMI_BT_AV_MAJOR_DEVICE_MASK)) )
				{
					if (srv_bt_cm_is_headset_connected_ex())//headset is connected,
...
void mmi_bt_dialer_link_state_ind(void* para)
{
	...
	if(event->link_type == SRV_BT_CM_LINK_TYPE_LE)
	{
		//mmi_bt_update_status_icon();
		return;
	}
...
//add the addr to the array.
#ifdef __MMI_BT_BTBOX_NOLCD__

    if(BT_A2DP_SOURCE == srv_a2dp_get_role(dev_addr))
        return;
  
    src_idx = mmi_bt_dialer_get_autoreconn_inx_array(dev_addr);
    ASSERT(src_idx>=0);
    g_mmi_bt_reconn_ctx[src_idx].bt_dialer_reconn_flag = MMI_TRUE;
    srv_bt_cm_set_auto_reconn_flag(&g_mmi_bt_reconn_ctx[src_idx].bd_addr, MMI_TRUE);
    notify_id = mmi_bt_anti_lost_get_notify(MMI_BT_ANTI_LOST_BTB_NOTIFY_ABDISC, src_idx);
    mmi_bt_anti_lost_send_notify(notify_id);

    if(!mmi_bt_dialer_app_is_connected_ex(dev_addr) && !srv_bt_cm_is_dev_busy(dev_addr) )
    {
...
U32 mmi_bt_dialer_get_autoreconn_inx_array(srv_bt_cm_bt_addr* dev_addr)
{
    ...
    for (i = 0; i < BT_DEVICE_LINK_NUM; i++)
    {
        ...
        {
        #ifdef __MMI_BT_BTBOX_NOLCD__
            return g_mmi_bt_reconn_ctx[i].bt_dialer_idx;
        #else /* __MMI_BT_BTBOX_NOLCD__ */
            return i;
        #endif /* __MMI_BT_BTBOX_NOLCD__ */
        }
...
void mmi_bt_dialer_auto_reconn_first_start_check(void)
{
    ...
    /*----------------------------------------------------------------*/
#ifdef __MMI_BT_BTBOX_NOLCD__
    i = mmi_bt_dialer_get_reconn_item();
    if (i < BT_DEVICE_LINK_NUM)
    {
        memcpy(&temp_dev, &g_mmi_bt_reconn_ctx[0], sizeof(mmi_bt_reconn_ctx_struct));
        memcpy(&g_mmi_bt_reconn_ctx[0], &g_mmi_bt_reconn_ctx[i], sizeof(mmi_bt_reconn_ctx_struct));
        memcpy(&g_mmi_bt_reconn_ctx[i], &temp_dev, sizeof(mmi_bt_reconn_ctx_struct));
    }
#endif /* __MMI_BT_BTBOX_NOLCD__ */
...
/*****************************************************************************
 * FUNCTION
 *  mmi_bt_dialer_auto_reconn_headset_hdlr
 * DESCRIPTION
 *  N/A
 * PARAMETERS
 * void
 ...
 * void
 *****************************************************************************/
static void mmi_bt_dialer_auto_reconn_headset_hdlr(MMI_BOOL result)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if (result && g_mmi_bt_reconn_ctx[0].bt_dialer_reconn_flag)
    {
        g_mmi_bt_reconn_ctx[0].bt_dialer_reconn_flag = MMI_FALSE;
        mmi_bt_dialer_clean_autoreconn_dev(&g_mmi_bt_reconn_ctx[0]);
        if (!mmi_bt_dialer_is_addr_equal_0(&g_mmi_bt_reconn_ctx[0].bd_addr))
...
void mmi_bt_dialer_auto_reconn_start(void)
{
	...
	if (mmi_bt_is_dev_cod_match(dev_p->cod, (MMI_BT_AUDIO_MAJOR_SERVICE_MASK|MMI_BT_AV_MAJOR_DEVICE_MASK)) )
	{
		MMI_TRACE(MMI_CONN_TRC_G7_BT, MMI_BT_AUTO_RECONN_1);
	    if(!srv_bt_cm_is_headset_connected_ex())//headset is connected, 
		{
			MMI_TRACE(MMI_CONN_TRC_G7_BT, MMI_BT_AUTO_RECONN_2);		
		#ifdef __MMI_HFP_SUPPORT__
			srv_bthf_attach_device(&g_mmi_bt_reconn_ctx[0].bd_addr, mmi_bt_dialer_auto_reconn_headset_hdlr);
		#else
			mmi_bt_entry_connecting(g_mmi_bt_scr_cntx.mydev_gid);
		#endif /* __MMI_HFP_SUPPORT__ */
		}
	...
	{
		mmi_bt_dialor_auto_reconnect(&g_mmi_bt_reconn_ctx[0].bd_addr);
	}
#else /* __MMI_BT_BTBOX_NOLCD__ */
    src_idx = mmi_bt_dialer_get_reconn_item();
    if ( src_idx < BT_DEVICE_LINK_NUM )
    {
        memcpy(&(g_mmi_bt_scr_cntx.conn_dev_add), &g_mmi_bt_reconn_ctx[src_idx].bd_addr, sizeof(srv_bt_cm_bt_addr));
        MMI_TRACE(MMI_CONN_TRC_G7_BT, MMI_BT_CONN_DIALER_AUTO_RECONN,
            g_mmi_bt_reconn_ctx[src_idx].bd_addr.lap,
            g_mmi_bt_reconn_ctx[src_idx].bd_addr.nap,
            g_mmi_bt_reconn_ctx[src_idx].bd_addr.uap, 
            g_mmi_bt_reconn_ctx[src_idx].bt_dialer_reconn_flag, 
            g_mmi_bt_reconn_ctx[src_idx].bt_dialer_reconn_accu_time/1000);


        StartTimer(BT_DIALER_AUTO_RECONN_TIMER, 
            g_mmi_bt_reconn_ctx[src_idx].bt_dialer_reconn_freq, 
            mmi_bt_dialer_auto_reconn_hdlr);
        mmi_bt_dialor_connect(&g_mmi_bt_reconn_ctx[src_idx].bd_addr);
    }

    notify_id = mmi_bt_anti_lost_get_notify(MMI_BT_ANTI_LOST_BTB_NOTIFY_RECONNECT, src_idx);
    mmi_bt_anti_lost_send_notify(notify_id);
#endif /* __MMI_BT_BTBOX_NOLCD__ */
}
...
mmi_ret mmi_bt_dialer_acl_timeout_discon_hdlr(void)
{
    ...
    if (mmi_frm_scrn_is_present(parent_id, SCR_BT_CONNECTING, MMI_FRM_NODE_ALL_FLAG))
    {
       // mmi_frm_scrn_close(parent_id, SCR_BT_CONNECTING);
		 mmi_bt_stop_progress_scr(parent_id, SCR_BT_CONNECTING);
    }
    if (mmi_bt_dialer_app_is_connected())
    {
        mmi_frm_nmgr_popup(
 ...
 * void
 *****************************************************************************/
void mmi_bt_dialer_set_headset_flag(MMI_BOOL flag, srv_bt_cm_bt_addr* dev_addr)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
void mmi_bt_dialer_set_connected_dev_info(srv_bt_cm_dev_struct *dev_info)
{
...
#ifdef __MMI_BT_BTBOX_NOLCD__
  #ifdef __MMI_BT_DIALER_AUTO_RECONN_SUPPORT__
    // Check wether a reconnection items, if yes, save to same place.
    if (!result)
    {
        for(i=0; i<BT_DEVICE_LINK_NUM; i++)
        {
            if( srv_bt_cm_is_dev_addr_equal(&g_mmi_bt_reconn_ctx[i].bd_addr, &dev_info->bd_addr)
                && g_mmi_bt_reconn_ctx[i].bt_dialer_reconn_flag )
            {
                // this bd adress already in g_connected_dev_info. just clean g_connected_dev_info
                result = MMI_TRUE;
    ...
    // check wether it's a new connection. choose an item to place
    if (result)
    {
        
        memcpy(&g_connected_dev_info[g_mmi_bt_reconn_ctx[i].bt_dialer_idx], dev_info, sizeof(srv_bt_cm_dev_struct));
        mmi_bt_dialer_clean_autoreconn_dev(&g_mmi_bt_reconn_ctx[i]);
    }
    else
    {
        for(i=0; i<BT_DEVICE_LINK_NUM; i++)
    ...
    if (result)
    {
        ...
        memset(&g_connected_dev_info[g_mmi_bt_reconn_ctx[i].bt_dialer_idx], 0, sizeof(srv_bt_cm_dev_struct));
        mmi_bt_dialer_clean_autoreconn_dev(&g_mmi_bt_reconn_ctx[i]);
    }
  #endif /* __MMI_BT_DIALER_AUTO_RECONN_SUPPORT__ */

    mmi_bt_dialer_insert_connected_dev_info(dev_info);
...
mmi_id cui_bt_dialer_connecting_create(mmi_id parent_gid)
{
    ...
    else
    {
        ...
        g_mmi_bt_scr_cntx.cnting_cui_gid = GRP_ID_BT_CM_CONNECTING_SCR;
...
mmi_ret cui_bt_dialer_connecting_proc(mmi_event_struct *evt)
{
    ...
    switch (evt->evt_id)
    {
        case EVT_ID_GROUP_DEINIT:
        {
            ...
            g_mmi_bt_scr_cntx.cnting_cui_gid = 0;
...
void cui_bt_dialer_connecting_close(mmi_id cui_gid)
{
	...
	#ifdef __MMI_WEARABLE_DEVICE__	
	mmi_bt_dialer_set_move_right_flag(MMI_TRUE);
    #endif/*__MMI_WEARABLE_DEVICE__*/
}
 ...
 *  void
 *****************************************************************************/
void mmi_bt_dialer_set_vis_change_flag(MMI_BOOL is_change)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
 ...
 *  MMI_BOOL
 *****************************************************************************/
MMI_BOOL mmi_bt_dialer_get_vis_change_flag(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
void mmi_bt_dialer_connect_a2dp_src(srv_bt_cm_bt_addr *dev_addr, mmi_id group_id)
{
    ...
    if(MMI_TRUE != srv_bt_cm_get_auto_reconn_flag(dev_addr))
    {
        ...
        if(GRP_ID_INVALID == g_mmi_bt_scr_cntx.cnting_cui_gid)
		    {
            return;
		    }
...
MMI_BOOL mmi_bt_dialer_disconnect_by_dev_info(srv_bt_cm_dev_struct* dev_p)
{
    ...
    {    
        if (mmi_bt_is_dev_cod_match(dev_p->cod, (MMI_BT_AUDIO_MAJOR_SERVICE_MASK|MMI_BT_AV_MAJOR_DEVICE_MASK)) 
			 || mmi_bt_dialer_get_headset_flag(&(dev_p->bd_addr)))
        {
            mmi_bt_entry_release_device_connections(GRP_ID_BT_CM_SELECT_DEVICE);
        }
        else
        {
            g_mmi_bt_scr_cntx.main_gid = mmi_frm_group_get_active_id();
...
void mmi_bt_dialer_conn_set_vis_state(void)
{
    ...
    vis_dev = mmi_bt_dialer_get_conn_vis_enum();
	  memset(&g_mmi_bt_change_vis_dev, 0, sizeof(mmi_bt_conn_set_vis_struct));  //avoid next set visiablilty use preview parameter
	  MMI_TRACE(MMI_CONN_TRC_G7_BT, MMI_BT_CONN_DIS_SET_VIS_STATE_FUNC, vis_dev.g_vis_type);
        switch(vis_dev.g_vis_type)
        {
            ...
            {
            break;  //normal visiable set no need contine connect
            }
            case MMI_BT_CONN_ACTIVE:
            {
            mmi_bt_dialor_connect_int(&vis_dev.bd_addr, &vis_dev.callback_item, vis_dev.is_connect);
                // call function
                break;
            }
...
#ifdef __MMI_WEARABLE_DEVICE__
void mmi_bt_dialer_conn_confirm_lsk(void)
{
	srv_bt_cm_dev_struct* dev_p = srv_bt_cm_get_paired_dev_info(g_mmi_bt_scr_cntx.hilight_item);

	if (mmi_bt_dialer_app_is_connected_ex(&(dev_p->bd_addr)) || (srv_bt_cm_get_linked_dev_list_idx() == BT_DEVICE_LINK_NUM))
	{
		mmi_bt_popup_display(
					STR_BT_RELEASE_CONN_PROMPT,
					MMI_EVENT_FAILURE,
...
void mmi_bt_dialer_disconn_confirm_lsk(void)
{
	srv_bt_cm_dev_struct* dev_p = srv_bt_cm_get_paired_dev_info(g_mmi_bt_scr_cntx.hilight_item);
...
void mmi_bt_dialer_conn_confirm_scrn(srv_bt_cm_bt_addr* dev_addr)
{
	   ...
	   if (mmi_frm_scrn_enter(
			   ...
			   (FuncPtr) NULL,
			   (FuncPtr) mmi_bt_dialer_conn_confirm_scrn,
			   MMI_FRM_FULL_SCRN) == MMI_FALSE)
	   {
		   return;
	   }
       ...
       SetLeftSoftkeyFunction(mmi_bt_dialer_conn_confirm_lsk, KEY_EVENT_UP);
	   SetRightSoftkeyFunction(mmi_frm_scrn_close_active_id, KEY_EVENT_UP);

}

void mmi_bt_dialer_disconn_confirm_scrn(srv_bt_cm_bt_addr* dev_addr)
{
    if (mmi_frm_scrn_enter(
			   g_mmi_bt_scr_cntx.mydev_gid,
			   SCR_BT_DIALER_DISCONNECT_ASK,
			   (FuncPtr) NULL,
			   (FuncPtr) mmi_bt_dialer_disconn_confirm_scrn,
			   MMI_FRM_FULL_SCRN) == MMI_FALSE)
	   {
		   return;
	   }
		  ...
		  (UI_string_type) GetString(STR_GLOBAL_OK),
			   ...
			   mmi_get_event_based_image(MMI_EVENT_PROGRESS),
			   NULL);
	       SetLeftSoftkeyFunction(mmi_bt_dialer_disconn_confirm_lsk, KEY_EVENT_UP);
		   SetRightSoftkeyFunction(mmi_frm_scrn_close_active_id, KEY_EVENT_UP);

}

mmi_ret mmi_bt_dialer_move_right_hdlr(mmi_event_struct *evt)
{

	
	MMI_TRACE(MMI_CONN_TRC_G7_BT, MMI_BT_CONN_MOVE_RIGHT, evt->evt_id);
    switch (evt->evt_id)
	{
	    case EVT_ID_WEARABLE_DEVICE_MOVE_RIGHT:
		{
			if(!mmi_bt_dailer_get_move_right_flag())
			{
				MMI_TRACE(MMI_CONN_TRC_G7_BT, MMI_BT_CONN_MOVE_RIGHT_DISABLE);
...
void mmi_bt_dialer_set_move_right_flag(MMI_BOOL state)
{
	g_mmi_bt_dialer_move_rgt = state;
	MMI_TRACE(MMI_CONN_TRC_G7_BT, MMI_BT_CONN_MOVE_RIGHT_SET, g_mmi_bt_dialer_move_rgt);
}

MMI_BOOL mmi_bt_dailer_get_move_right_flag(void)
{

	MMI_TRACE(MMI_CONN_TRC_G7_BT, MMI_BT_CONN_MOVE_RIGHT_GET, g_mmi_bt_dialer_move_rgt);
	return g_mmi_bt_dialer_move_rgt;
}
...
U32 mmi_bt_dialer_insert_connected_dev_info(srv_bt_cm_dev_struct *dev_info)
{
	...
	MMI_TRACE(MMI_CONN_TRC_G7_BT, MMI_BT_CONN_INSERT_DEV_INFO, dev_info->bd_addr, dev_info->name);
/*
    kal_prompt_trace(MOD_MMI, "g_connected_dev_info List:");
    for(i=0; i<total_num; i++)
    {
        kal_prompt_trace(MOD_MMI, "%d: %x %s", i, g_connected_dev_info[i].bd_addr, g_connected_dev_info[i].name);
    }
*/
...
U32 mmi_bt_dialer_get_index_by_dev_info(srv_bt_cm_bt_addr *bd_addr)
{
    ...
    /*----------------------------------------------------------------*/
    U32 i;
//    U32 count = mmi_bt_dialer_quick_conn_dev_num();
...
//extern void ShowCommonAppScreen(U8* text, U16	content_icon);

static mmi_ret mmi_bt_entry_wristband_key_proc(mmi_event_struct* evt)
{
	/*----------------------------------------------------------------*/
	/* Local Variables												 */
	/*----------------------------------------------------------------*/
...
static void mmi_bt_entry_wristband_key_longpress()
{
	/*----------------------------------------------------------------*/
	/* Local Variables												 */
	/*----------------------------------------------------------------*/
...
/*****************************************************************************
 * FUNCTION
 *  mmi_bt_entry_wristband_setting_main_proc
 * DESCRIPTION
 *  N/A
 * PARAMETERS
 * mmi_event_struct*
...
static mmi_ret mmi_bt_entry_wristband_setting_main_proc(mmi_event_struct* evt)
{
/*----------------------------------------------------------------*/
/* Local Variables												 */
/*----------------------------------------------------------------*/
U16 str_buff[20];
mmi_frm_scrn_active_evt_struct *scrn_evt;  
U16 img_id;
srv_bt_cm_power_status_enum status;
...
/*----------------------------------------------------------------*/
	switch(evt->evt_id)
	{
		case EVT_ID_SCRN_ACTIVE:
			 status = srv_bt_cm_get_power_status();
			 memset(str_buff, 0, sizeof(20));
			 mmi_wcscpy(str_buff, (WCHAR *) GetString(STR_BT_MENU_TITLE));
			 mmi_wcscat(str_buff, (WCHAR *) L" ");
			 if((status == SRV_BT_CM_POWER_ON) || (status == SRV_BT_CM_POWER_SWITCHING_OFF))
			 {
				 mmi_wcscat(str_buff, (WCHAR *) GetString(STR_GLOBAL_ON));
				 img_id = IMG_WRISTBAND_BT_ON;
			 }
			 else
			 {
				 mmi_wcscat(str_buff, (WCHAR *) GetString(STR_GLOBAL_OFF));
				 img_id = IMG_WRISTBAND_BT_OFF;
			 }
			 ShowCommonAppScreen((U8 *)str_buff, img_id);	
			 if(scrn_evt->is_first_active)
		 	 {
				 mmi_frm_cb_reg_event(EVT_ID_KEY_CLICK_SINGLE, mmi_bt_entry_wristband_key_proc,NULL);
				 mmi_frm_cb_reg_event(EVT_ID_KEY_CLICK_DOUBLE, mmi_bt_entry_wristband_key_proc,NULL);
		 	 }
			 SetKeyHandler(mmi_bt_entry_wristband_key_longpress, KEY_POWER, KEY_LONG_PRESS);					 
			break;

		case EVT_ID_SCRN_DEINIT:
			 mmi_frm_cb_dereg_event(EVT_ID_KEY_CLICK_SINGLE, mmi_bt_entry_wristband_key_proc,NULL);
			 mmi_frm_cb_dereg_event(EVT_ID_KEY_CLICK_DOUBLE, mmi_bt_entry_wristband_key_proc,NULL);			 
			break;
	}
	return MMI_RET_OK;
}
...
void mmi_bt_entry_wristband_setting_menu()
{
/*----------------------------------------------------------------*/
/* Local Variables												 */
/*----------------------------------------------------------------*/
...
/*----------------------------------------------------------------*/
	...
	mmi_frm_scrn_create(g_mmi_bt_scr_cntx.main_gid,
		  			    SRC_ID_BT_WB_SETTING,
		  			    mmi_bt_entry_wristband_setting_main_proc,
		  			    NULL);
