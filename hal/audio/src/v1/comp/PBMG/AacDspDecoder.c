...
{
   int32 I,J,K=0, count, len, tmp;
   uint16 rb_w;
   AAC_DecData *pDecData = (AAC_DecData *)ihdl->pPrivateData; 
   kal_uint16* write_ptr = pDecData->silencepattern;
   ...
   for(J=count; J>=8; J-=8)
   {   
      IDMA_WriteToDSP((kal_uint16*)DSP_DM_ADDR(AAC_PAGE_NUM, rb_w), write_ptr, 8);
      rb_w += 8;
      K++;
   }
   ...
   *AAC_MCU_WRITE_PTR = rb_w;
   tmp = count - K*8;
   IDMA_WriteToDSP((kal_uint16*)DSP_DM_ADDR(AAC_PAGE_NUM, rb_w), write_ptr, tmp);
   ...
   *AAC_MCU_WRITE_PTR = rb_w;
   ...
   count = len - count;
   if ( count > 0 ) {
      IDMA_WriteToDSP((kal_uint16*)DSP_DM_ADDR(AAC_PAGE_NUM, rb_w), write_ptr+tmp, 8-tmp);  	
      rb_w += (kal_uint16)(8-tmp);
      *AAC_MCU_WRITE_PTR = rb_w;
   	  tmp = count-8+tmp;
   	  for(J=tmp; J>=8; J-=8)
   	  {
...
static void AacDecHisrHandler(void *hComponent)
{
   ...
   *DP2_AUDIO_ASP_TASK4_DM_HANDSHAKE = 0; 
   ...
   if (!ihdl->Decode_EOF) {
            ...
            uBytesAvailable = ACU_PeekDataCount((AudComHdl *)ihdl);
   ...
   if( !ihdl->isEOF && !ihdl->Decode_EOF)
   {
      if(((*AAC_ERROR_REPORT == 0x2222)||(*AAC_ERROR_REPORT == 0x1111))){
      	 ...
      	 if (ihdl->mhdl) {
      	    if (ihdl->mhdl->GetDataCount(ihdl->mhdl) == 0)
...
static void AacDec_OpenDevice(AudComHdlInt *ihdl)
{
   ...
   *AAC_DUAL_SCE = 0;
   ...
   Media_GetSilencePattern_AAC(MEDIA_FORMAT_AAC,pDecData->FrameInfo.bIsStereo+1,pDecData->FrameInfo.uSampleRate,0,(kal_uint8 *)pDecData->silencepattern);
   ...
   *AAC_MAIN_CONTROL = DSP_AAC_STATE_START;
...

   {
      ...
      *DSP_AUDIO_FLEXI_CTRL |= (FLEXI_VBI_ENABLE | FLEXI_SD_ENABLE);
      ...
      // move here to reduce memory fragment
      APM_Hook_PcmRut();
