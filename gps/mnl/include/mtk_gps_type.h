...
typedef enum 
{
    MTK_GPS_LLE_HAVE_NVPOS = 0x0,
    MTK_GPS_LLE_NO_SIGNAL,
    MTK_GPS_LLE_NO_EPH,
    MTK_GPS_LLE_INFO_MAX   
} MTK_GPS_LLE_STATE_E;

typedef struct 
{
    MTK_GPS_LLE_STATE_E  currentState;
} MTK_GPS_LLE_INFO_T;
//#endif
/*******************************************************************************
 * Type Definitions
...
typedef enum{
    MTK_GPS_RESTARTTYPE_HOT = 0,
    MTK_GPS_RESTARTTYPE_WARM,
    MTK_GPS_RESTARTTYPE_COLD,
}mtk_gps_restart_condition;
...
typedef struct
{
  ...
  mtk_uint8       u1LocationLowEnergy;          /* enable Location Low Energy */
} mtk_param_al_cfg;
