...
static void cui_menu_tap_hdlr(mmi_tap_type_enum tap_type, S32 index)
{
  ...
  //  if (tap_type == ENUM_TAP_ON_NONHIGHLIGHTED_ITEM) 
   // {
	...
	if (tap_type == ENUM_TAP_ON_NONHIGHLIGHTED_ITEM)
	{
		evt.evt_id = EVT_ID_CUI_MENU_NON_HIGHLIGHTED_ITEM_TAP;
	}
	else
	{
...
#ifdef __MMI_WEARABLE_DEVICE__
void cui_menu_set_radio_list_item_ext(mmi_menu_id menu_id)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/

    S32 menuitem_idx = cui_menu_get_index_from_menu_id_list(menu_id);
    if(menuitem_idx == CUI_MENU_INDEX_NOT_FOUND) 	
    {
        return;    
    }
	
    radio_list_handle_item_select(menuitem_idx);	
}
#endif
...
#ifdef __MMI_WEARABLE_DEVICE__
void cui_menu_set_screen_show_softkey()
{
