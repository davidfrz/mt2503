 ...
 *  
 *****************************************************************************/
srv_mms_uc_error_type_enum srv_mms_hanlde_text_buffer_of_current_slide(S32 buff_len)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
	//Decide encoding type to be used and the actual char_len here

	

        U32 utf8_buffer_size = buff_len;
        U8 *utf8_buffer = (U8*) srv_mms_mem_mgr_viewer_asm_alloc(utf8_buffer_size);
        U32 utf8_len = 0;

        memset(utf8_buffer, 0, utf8_buffer_size);
        utf8_len = mmi_chset_ucs2_to_utf8_string(utf8_buffer, utf8_buffer_size, (kal_uint8*)g_srv_mms_uc_supp.srv_msg.fseditor_buffer);

		if((utf8_len)*ENCODING_LENGTH == buff_len)
		{
			charset = MMA_CHARSET_UTF8;
			char_len = utf8_len;
		}
		else
		{
			charset = MMA_CHARSET_UCS2;
			char_len = buff_len;
		}
    ...
    {   /* check here what will be msg_size after adding text [may exceed the msg_size] */
		...
		//Decide encoding type before this point
		if(charset == MMA_CHARSET_UTF8)
		{
			...
			result = srv_mms_save_buffer_to_file(text_file_path, (U8*) utf8_buffer, char_len);
        ...
        if (result == FS_NO_ERROR)
        {
            ...
            g_mms_message_data.body->curr_slide->txt.object->encoding = charset;
			g_mms_message_data.body->curr_slide->txt.object->charset = charset;
