...
S32 gdi_image_virtualfile_translate_type(S32 image_type)
{
	switch(image_type)
	{
    ...
    case GDI_IMAGE_TYPE_JPG_FILE_OFFSET:
		image_type = GDI_IMAGE_TYPE_JPG_FILE;
		break;
    ...
    case GDI_IMAGE_TYPE_SWFLASH_FILE_OFFSET:
		image_type = GDI_IMAGE_TYPE_SWFLASH_FILE;
		break;
    case GDI_IMAGE_TYPE_ABM_FILE_OFFSET:
        image_type = GDI_IMAGE_TYPE_ABM_FILE;
        break;
    case GDI_IMAGE_TYPE_AB2_FILE_OFFSET:
        image_type = GDI_IMAGE_TYPE_AB2_FILE;
        break;
    case GDI_IMAGE_TYPE_9SLICE_FILE_OFFSET:
        image_type = GDI_IMAGE_TYPE_9SLICE_FILE;
        break;
    default:
...
S32 gdi_image_get_virtual_filename_with_filesize(S8* image_buffer, S8* virtual_fname, U8 file_size)
{
    ...
    mmi_ucs2cat((S8*)virtual_fname,(const S8*)ext_name);
...
GDI_RESULT gdi_image_virtualfile_draw_handler(U32 flag,U32 frame_pos,S32 x,S32 y,S32 w,S32 h,U8* data_ptr,U32 img_size)
{
    ...
    img_type = gdi_image_virtualfile_translate_type(data_ptr[0]);
    if (GDI_IMAGE_TYPE_INVALID == img_type) return GDI_FAILED;
