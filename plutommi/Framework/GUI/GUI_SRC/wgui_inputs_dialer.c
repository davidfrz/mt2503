...
void wgui_inputs_dialer_setup_dialing_keypad(dialing_keypad_struct *dialing_keypad)
{
    ...
    /*----------------------------------------------------------------*/
#ifdef __MMI_BT_BOX__
    U8 result = GUI_DIALER_PAD_NO_ERROR;
    gui_bound bound = {0};
	gui_dialer_pad_style style;
    #ifdef __MMI_BT_BOX_TEST_DIALER_PAD_STYLE__ 
	U8 dialPadType = MMI_PHNSET_BT_BOX_TYPE1;
	S16 error;
	#endif
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    bound.x = MMI_DIALING_KEYPAD_LAYER_X + MMI_DIALING_KEYPAD_X;
    bound.y = MMI_DIALING_KEYPAD_LAYER_Y + MMI_DIALING_KEYPAD_Y;
    bound.width = MMI_DIALING_KEYPAD_WIDTH;
    bound.height = MMI_DIALING_KEYPAD_HEIGHT;
    
	#if  defined(__MMI_UI_DIALER_PAD_ONLY_KEY__)
		style = GUI_DIALER_PAD_STYLE_VK_KEY;
	#else
		style = GUI_DIALER_PAD_STYLE_DIALER_CICUAL_ROLL;	
	#endif
	
    #ifdef __MMI_BT_BOX_TEST_DIALER_PAD_STYLE__ 
	ReadValue(NVRAM_DISPCHAR_BT_BOX_DIALER_TYPE, &dialPadType, DS_BYTE, &error);
	
	if(MMI_PHNSET_BT_BOX_TYPE1 == dialPadType)
	{
		style = GUI_DIALER_PAD_STYLE_DIALER_CICUAL_ROLL;
	}
	else if(MMI_PHNSET_BT_BOX_TYPE2 == dialPadType)
	{
		style = GUI_DIALER_PAD_STYLE_VK_KEY;
	}
	else if(MMI_PHNSET_BT_BOX_TYPE3 == dialPadType)
	{
		style = GUI_DIALER_PAD_STYLE_VK;
	}
	else
	{
		style = GUI_DIALER_PAD_STYLE_DIALER_CICUAL_ROLL;
    ...
    result = gui_dialer_pad_create(style, bound);
    MMI_ASSERT(result == GUI_DIALER_PAD_NO_ERROR);
#else  //__MMI_BT_BOX__
...
void wgui_inputs_dialer_setup_dialing_keypad_with_key(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
