...
extern srv_dtcnt_wlan_scan_setting_struct g_srv_dtcnt_wlan_scan_setting_ctx;
#endif

#if defined(__IOT__) && defined(__MMI_TELEPHONY_SUPPORT__)

#define SRV_DTCNT_READY_CHECK_NUM 5
#define SRV_DTCNT_MAX_ACCOUNT_NUM (SRV_DTCNT_PROF_MAX_ACCOUNT_NUM_PER_SIM * MMI_SIM_TOTAL)
#define SRV_MAX_PLMN_LEN 6
#define SRV_DTCNT_CONF_NAME_LEN (32)

typedef enum
{
    SRV_DTCNT_INIT = 0x01,
    SRV_DTCNT_PLMN = 0x02,
    SRV_DTCNT_INIT_PLMN = 0x04,
    SRV_DTCNT_INIT_PLMN_TOTAL
} srv_dtcnt_init_plmn;
...
}srv_dtcnt_list_disp_struct;

static S32 g_srv_dtcnt_factory = MMI_FALSE;
static CHAR g_srv_dtcnt_sim[MMI_SIM_TOTAL][(SRV_MAX_PLMN_LEN + 1)];
static kal_wchar g_srv_dtcnt_conf[MMI_SIM_TOTAL][SRV_DTCNT_CONF_NAME_LEN];
MMI_BOOL g_srv_sim_changed[MMI_SIM_TOTAL];
static S32 g_srv_dtcnt_init_plmn = 0;
U16 g_srv_dtcnt_plmn_evt_num = 0;
U16 g_srv_dtcnt_plmn_proc_num = 0;
MMI_BOOL g_srv_dtcnt_first_proc_plmn_evt = MMI_TRUE;
MMI_BOOL g_srv_dtcnt_acct_ready = MMI_FALSE;
srv_dtcnt_list_disp_struct g_srv_dtcnt_list_item[SRV_DTCNT_MAX_ACCOUNT_NUM];
U32 g_srv_dtcnt_acct_num;

void (*g_srv_dtcnt_ready_check_cb[SRV_DTCNT_READY_CHECK_NUM])(void);

static void srv_dtcnt_db_init();
mmi_ret srv_dtcnt_on_home_plmn_changed(mmi_event_struct *evt);
static void srv_dtcnt_get_defined_prof(void);
#if defined(__DA_SMART_SELECTION_SUPPORT__)
static void srv_dtcnt_sim_prof_delete(void);
static void srv_dtcnt_delete_any_sim_profile(srv_dtcnt_sim_type_enum sim);
static void srv_dtcnt_add_config_account(void);
static MMI_BOOL srv_dtcnt_is_sim_card_changed(void);
static MMI_BOOL srv_dtcnt_get_sim_plmn(mmi_sim_enum sim, CHAR *out_buffer);
static U8 srv_dtcnt_sim_match(const CHAR *operate, const CHAR *code);
static srv_dtcnt_result_enum srv_dtcnt_add_sim_prof(U8 sim_match, 
...
static void srv_dtcnt_db_init()
{
    ...
    MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, SRV_DAPP_INIT, 1, result);

    if (result != SRV_DTCNT_RESULT_FS_ERROR)
    {
        result = srv_dtcnt_store_open();
        MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, SRV_DAPP_INIT, 3, result);
        ...
        {
            g_srv_dtcnt_factory = MMI_TRUE;
        }
    #endif
    }

#if defined(__DA_SMART_SELECTION_SUPPORT__)
    g_srv_dtcnt_init_plmn |= SRV_DTCNT_INIT;
    for (i = 0; i < MMI_SIM_TOTAL; i++)
    {
        g_srv_sim_changed[i] = MMI_TRUE; // initial value
    }
    MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, SRV_DAPP_INIT, 4, 0);
...
#endif
    {
        MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, SRV_DAPP_INIT, 5, 0);
        
        if (g_srv_dtcnt_init_plmn & SRV_DTCNT_PLMN)
        {
            srv_dtcnt_on_home_plmn_changed(NULL);
        }
        return;
    }
    ...
    {
        MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, SRV_DAPP_INIT, 7, result);
    }
    else
    {
        U8 sim_card_id[4 * (SRV_MAX_PLMN_LEN + 1) + 1];
        S16 error;
        MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, SRV_DAPP_INIT, 9, result);
        memset(sim_card_id, 0, 4 * (SRV_MAX_PLMN_LEN + 1) + 1);
        ReadRecord(NVRAM_EF_DTCNT_SIM_ID_LID, 1, &sim_card_id, NVRAM_MAX_DTCNT_SIM_ID_SIZE, &error);
        if (sim_card_id[0] || sim_card_id[(SRV_MAX_PLMN_LEN + 1)] ||
            sim_card_id[(SRV_MAX_PLMN_LEN + 1) * 2] || sim_card_id[(SRV_MAX_PLMN_LEN + 1) * 3])
        {
            memset(sim_card_id, 0, 4 * (SRV_MAX_PLMN_LEN + 1) + 1);
            WriteRecord(NVRAM_EF_DTCNT_SIM_ID_LID, 1, &sim_card_id, NVRAM_MAX_DTCNT_SIM_ID_SIZE, &error);
            srv_dtcnt_sim_prof_delete();
        }
    }
    
    MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, SRV_DAPP_INIT, 10, 0);
    ...
    {   
        ...
        srv_dtcnt_prof_csd_struct *csd_account;
        #endif

        // just support to read account from custom_config_account.c when turn off smart selection
        MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, SRV_DAPP_INIT, result, 0);
        ...
        while (gprs_account)
        {
            ...
            MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, SRV_DAPP_ADD_SIM_PROF, acct_id, result);
            gprs_account = (srv_dtcnt_prof_gprs_struct *)custom_config_get_account(SRV_DTCNT_BEARER_GPRS);
        }
        #endif
        #ifdef __TCPIP_OVER_CSD__
...
mmi_ret srv_dtcnt_on_home_plmn_changed(mmi_event_struct *evt)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, SRV_DAPP_PROCESS_SIM_CHANGE, 0);
    if(g_srv_dtcnt_first_proc_plmn_evt)
    {
        for (i = 0; i < MMI_SIM_TOTAL; i++)
        {
        ...
        #endif
            {
                g_srv_dtcnt_plmn_evt_num++;
            }
        }
        g_srv_dtcnt_first_proc_plmn_evt = MMI_FALSE;
        MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, SRV_DAPP_ON_HOME_PLMN_CHANGED, 0, 1, g_srv_dtcnt_plmn_evt_num);
    }
    g_srv_dtcnt_plmn_proc_num++;
    MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, SRV_DAPP_ON_HOME_PLMN_CHANGED, 0, 2, g_srv_dtcnt_plmn_proc_num);
    if (g_srv_dtcnt_plmn_proc_num < g_srv_dtcnt_plmn_evt_num)
    {
        MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, SRV_DAPP_ON_HOME_PLMN_CHANGED, 0, 2, g_srv_dtcnt_plmn_proc_num);
    return MMI_RET_OK;
    }

    if (g_srv_dtcnt_init_plmn & SRV_DTCNT_INIT_PLMN)
    {
        MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, SRV_DAPP_PROCESS_SIM_CHANGE, 2);
        return MMI_RET_OK;
    }

    g_srv_dtcnt_init_plmn |= SRV_DTCNT_PLMN;

    if ((g_srv_dtcnt_init_plmn & SRV_DTCNT_INIT) != SRV_DTCNT_INIT)
    {
        MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, SRV_DAPP_PROCESS_SIM_CHANGE, 4);
        return MMI_RET_OK;
    }

    if (!srv_dtcnt_is_sim_card_changed() && !g_srv_dtcnt_factory)
    {
        MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, SRV_DAPP_PROCESS_SIM_CHANGE, 6);
        g_srv_dtcnt_init_plmn |= SRV_DTCNT_INIT_PLMN;
        srv_dtcnt_get_defined_prof();

        return MMI_RET_OK;
    }

    if (g_srv_dtcnt_factory)
    {
        for (i = 0; i < MMI_SIM_TOTAL; i++)
        {
            g_srv_sim_changed[i] = MMI_TRUE;
        }
    }
    ...
    srv_dtcnt_add_config_account();
    srv_dtcnt_get_defined_prof();
    
    g_srv_dtcnt_init_plmn |= SRV_DTCNT_INIT_PLMN;

    MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, SRV_DAPP_PROCESS_SIM_CHANGE, 10);
#endif
    return MMI_RET_OK;
}
...
static void srv_dtcnt_get_defined_prof(void)
{
    ...
    for (i = 0; i < SRV_DTCNT_PROF_TYPE_DM_CONF; i++)
    {
        ...
        if (result == SRV_DTCNT_RESULT_FS_ERROR || acct_id_query.num_ids > SRV_DTCNT_MAX_ACCOUNT_NUM)
        {
            g_srv_dtcnt_acct_ready = MMI_FALSE;
            return;
        }
        
        for (j = 0; j < acct_id_query.num_ids; j++)
        {
            g_srv_dtcnt_list_item[j + g_srv_dtcnt_acct_num].profile_id = acct_id_query.ids[j];
            g_srv_dtcnt_list_item[j + g_srv_dtcnt_acct_num].account_type = (srv_dtcnt_prof_acc_type_enum)(SRV_DTCNT_PROF_TYPE_USER_CONF + i);
        }
        g_srv_dtcnt_acct_num += acct_id_query.num_ids;
    }

    for (i = 0; i < MMI_SIM_TOTAL; i++)
    {
        ret = srv_dtcnt_get_default_account((srv_dtcnt_sim_type_enum)(i + SRV_DTCNT_SIM_TYPE_1), &default_account_id);
        MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, SRV_DAPP_GET_DEFAULT_ACCOUNT, (i + SRV_DTCNT_SIM_TYPE_1), default_account_id, ret);
    }

    /* 5 to get each profile settings */
    for (i = 0; i < g_srv_dtcnt_acct_num; i++)
    {
        //srv_dtcnt_store_update_acc_apinfo(g_srv_dtcnt_list_item[i].profile_id, 0, &(g_srv_dtcnt_list_item[i].app_info));

        srv_dtcnt_get_connection_type(g_srv_dtcnt_list_item[i].profile_id, &conn_type);

        g_srv_dtcnt_list_item[i].connection_type = conn_type;

        srv_dtcnt_get_sim_info(g_srv_dtcnt_list_item[i].profile_id, &sim_info);

        g_srv_dtcnt_list_item[i].sim_info = sim_info;

        if (srv_dtcnt_acct_is_read_only(g_srv_dtcnt_list_item[i].profile_id))
        {
            g_srv_dtcnt_list_item[i].read_only = 1;
        }
        else
        {
            g_srv_dtcnt_list_item[i].read_only = 0;
        }

        if (SRV_DTCNT_BEARER_CSD == 
            srv_dtcnt_get_bearer_type(g_srv_dtcnt_list_item[i].profile_id, SRV_DTCNT_ACCOUNT_PRIMARY))
        {
            g_srv_dtcnt_list_item[i].bearer_type = DATA_ACCOUNT_BEARER_CSD;
        }
        if (SRV_DTCNT_BEARER_GPRS == 
            srv_dtcnt_get_bearer_type(g_srv_dtcnt_list_item[i].profile_id, SRV_DTCNT_ACCOUNT_PRIMARY))
        {
            g_srv_dtcnt_list_item[i].bearer_type = DATA_ACCOUNT_BEARER_GPRS;
        }

    }
    
    ret = srv_dtcnt_get_sim_preference(&sim_info);
    MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, SRV_DAPP_SIM_GET_SIM_PREFERENCE, sim_info, ret);
    ...
    if (!availSimEnum)
    {
        ...
        MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, SRV_DAPP_SIM_SET_SIM_PREFERENCE, SRV_DTCNT_SIM_TYPE_NONE, ret);
    }
    else
    {
        U32 availSimIndex = 0;
        availSimIndex = mmi_frm_sim_to_index(availSimEnum);
        MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, SRV_DAPP_SIM_GET_SIM_PREFERENCE, availSimEnum, availSimIndex);
    #ifdef __MMI_TELEPHONY_SUPPORT__
    ...
    #endif /* __MMI_TELEPHONY_SUPPORT__ */
        {
            ...
            MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, SRV_DAPP_SIM_SET_SIM_PREFERENCE, availSimIndex + 1, ret);
    ...
    g_srv_dtcnt_acct_ready = MMI_TRUE;
    for (i = 0; i < SRV_DTCNT_READY_CHECK_NUM; i++)
    {
        if (g_srv_dtcnt_ready_check_cb[i])
        {
            g_srv_dtcnt_ready_check_cb[i]();
            g_srv_dtcnt_ready_check_cb[i] = NULL;
        }
    }
}
...
static void srv_dtcnt_sim_prof_delete(void)
{
    ...
    for (i = 0; i < MMI_SIM_TOTAL; i++)
    {
        if (g_srv_sim_changed[i])
        {
            srv_dtcnt_delete_any_sim_profile((srv_dtcnt_sim_type_enum)(SRV_DTCNT_SIM_TYPE_1 + i));
        }
    }
}
...
static void srv_dtcnt_delete_any_sim_profile(srv_dtcnt_sim_type_enum sim)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, SRV_DAPP_DELETE_ANY_SIM_PROFILE, sim);
    memset(&delete_query, 0, sizeof(delete_query));
...
static void srv_dtcnt_add_config_account(void)
{
    ...
    while (gprs_account)
    {
        ...
        sim_match = srv_dtcnt_sim_match(NULL, (CHAR *)gprs_account->prof_common_header.user_data);

        MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, SRV_DAPP_ADD_CONFIG_ACCOUNT, 1, sim_match);
        ...
        if (sim_match)
        {
            result = srv_dtcnt_add_sim_prof(sim_match, 
    ...
    while (csd_account)
    {
        ...
        sim_match = srv_dtcnt_sim_match(NULL, (CHAR *)csd_account->prof_common_header.user_data);
        
        MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, SRV_DAPP_ADD_CONFIG_ACCOUNT, 2, sim_match);
...
static MMI_BOOL srv_dtcnt_is_sim_card_changed(void)
{
    ...
    CHAR conf_name[SRV_DTCNT_CONF_NAME_LEN * 2];
    U8 *sim_card_id_temp;
    MMI_BOOL retn;
    U32 i;
    ...
    for (i = 0; i < MMI_SIM_TOTAL; i++)
    {        
        sim_card_id_temp = &sim_card_id[(SRV_MAX_PLMN_LEN + 1) * i];
        memset(g_srv_dtcnt_sim[i], 0, (SRV_MAX_PLMN_LEN + 1));
        retn = srv_dtcnt_get_sim_plmn(mmi_frm_index_to_sim(i), g_srv_dtcnt_sim[i]);    
        MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, SRV_DAPP_SIM_CARD_CHANGED, 10, retn);
        if (!retn)
        {
            memset(g_srv_dtcnt_sim[i], 0, (SRV_MAX_PLMN_LEN + 1));
            if (strcmp((CHAR*)g_srv_dtcnt_sim[i], (CHAR*)sim_card_id_temp))
            {
                memset(sim_card_id_temp, 0, (SRV_MAX_PLMN_LEN + 1));
                sim_change = MMI_TRUE;
            }
            else
            {
                g_srv_sim_changed[i] = MMI_FALSE;
            }
        }
        else
        {
            sprintf(conf_name, "z:\\%s.xml", g_srv_dtcnt_sim[i]);
            mmi_asc_to_ucs2((CHAR*) g_srv_dtcnt_conf[i], (CHAR*) conf_name);
            MMI_PRINT(MOD_MMI_CONN_APP, MMI_CONN_TRC_G6_DTCNT, "[DTCNT APP] sim id = %s-%s : 1", (char *)g_srv_dtcnt_sim[i], (char *)sim_card_id_temp);
        
            if (strcmp((CHAR*)g_srv_dtcnt_sim[i], (CHAR*)sim_card_id_temp)) /* SIM1 changed */
            {
                MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, SRV_DAPP_SIM_CARD_CHANGED, 11, 1); // sim1 changed
            
                strcpy((CHAR*)sim_card_id_temp, (CHAR*)g_srv_dtcnt_sim[i]);
                //WriteRecord(NVRAM_EF_DTCNT_SIM_ID_LID, 1, &sim_card_id, NVRAM_MAX_DTCNT_SIM_ID_SIZE, &error);
                sim_change = MMI_TRUE;
            }
            else
    ...
    MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, SRV_DAPP_SIM_CARD_CHANGED, 50, sim_change);

    if (sim_change)
    {
        WriteRecord(NVRAM_EF_DTCNT_SIM_ID_LID, 1, &sim_card_id, NVRAM_MAX_DTCNT_SIM_ID_SIZE, &error);
        MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, SRV_DAPP_SIM_CARD_CHANGED, 51, sim_change);
    }
    return sim_change;
}
...
static MMI_BOOL srv_dtcnt_get_sim_plmn(mmi_sim_enum sim, CHAR *out_buffer)
{
    ...
    {
        retn = srv_sim_ctrl_get_home_plmn(sim, out_buffer, (SRV_MAX_PLMN_LEN + 1));
        if (retn)
        {
            // get mnc length
        #ifdef __MMI_TELEPHONY_SUPPORT__
        ...
        #endif
            MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, SRV_DAPP_GET_MNC_LEN_IN_EF_AD, digits_of_mnc, sim);
            if ((digits_of_mnc != 0x2) || (digits_of_mnc != 0x3))
            {
                ...
                if (0 == strcmp(temp_mcc, "405"))
                {
                    ...
                    if (MMI_TRUE == srv_sim_ctrl_get_imsi(sim, (CHAR*)imsi, SRV_MAX_IMSI_LEN + 1))
                    {
                        ...
                        MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, SRV_DAPP_GET_IMSI, imsi[4], imsi[5], digits_of_mnc);
                        if (digits_of_mnc == 3)
                        {
...
static U8 srv_dtcnt_sim_match(const CHAR *operate, const CHAR *code)
{
    ...
    if (strcmp(code, sim_id) == 0)
    {
        for (i = 0; i < MMI_SIM_TOTAL; i++)
        {
            if ((g_srv_dtcnt_sim[i][0] != 0) && g_srv_sim_changed[i])
            {
    ...
    {    
        for (i = 0; i < MMI_SIM_TOTAL; i++)
        {
            if ((g_srv_dtcnt_sim[i][0] != 0) && strstr(code, g_srv_dtcnt_sim[i]) && g_srv_sim_changed[i])
            {
                sim_match += (U8)mmi_frm_index_to_sim(i);
            }
        }
...
MMI_BOOL srv_dtcnt_ready_check(FuncPtr ready_callback)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if (g_srv_dtcnt_acct_ready == MMI_TRUE)
    {
        ret = MMI_TRUE;
    }
    else
    {
        for (i = 0; i < SRV_DTCNT_READY_CHECK_NUM; i++)
        {
            if(!g_srv_dtcnt_ready_check_cb[i])
            {
                g_srv_dtcnt_ready_check_cb[i] = ready_callback;
                ret = MMI_FALSE;
                break;
            }
        }
    ...
    if (decoded_acc_id == CBM_WIFI_ACCT_ID)
    {
        ...
        {            
            if (tmp_wlan_p->use_proxy)
            {
                ...
                MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, SRV_DTCNT_API_PROXY_PORT, out_px_info->px_addr, out_px_info->px_port);                
...
MMI_BOOL srv_dtcnt_wlan_is_proxy_exclusive_ip_addr(srv_dtcnt_wlan_proxy_exclusive_ip_addr_struct *exclusive_ip_addr)
{
    ...
    MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, SRV_DTCNT_API_QUERY_URL, exclusive_ip_addr->ip_addr);
...
srv_dtcnt_result_enum srv_dtcnt_wlan_add_proxy_exclusive_ip_addr(srv_dtcnt_wlan_proxy_exclusive_ip_addr_struct *exclusive_ip_addr)
{
	...
	MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, SRV_DTCNT_API_ADD_URL, exclusive_ip_addr->ip_addr);
...
srv_dtcnt_result_enum srv_dtcnt_wlan_delete_proxy_exclusive_ip_addr(srv_dtcnt_wlan_proxy_exclusive_ip_addr_struct *exclusive_ip_addr)
{
	...
	MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, SRV_DTCNT_API_DEL_URL, exclusive_ip_addr->ip_addr);
    for (i = 0; i < g_srv_dtcnt_wlan_proxy_exclusive_ip_addr_list.num_ips; i++)
    {
...
MMI_BOOL srv_dtcnt_wlan_get_connected_ap_prof_id(U32 *prof_id)
{
#if defined (__MMI_WLAN_FEATURES__) && defined(__MTK_TARGET__) && defined(__TCPIP__)
    if (!prof_id)
    {        
    ...
    if (SRV_DTCNT_WLAN_STATUS_CONNECTED != srv_dtcnt_wlan_status() || 
        !g_srv_dtcnt_wlan_ctx.connected_wlan_profile)
    {
        *prof_id = 0;        
        return MMI_FALSE;
    }
    
    *prof_id = g_srv_dtcnt_wlan_ctx.connected_wlan_profile->profile_id;
...
srv_dtcnt_result_enum srv_dtcnt_wlan_set_scan_spec_ssid_list(
    U8 ssid_list_num, 
    const srv_dtcnt_ssid_struct * ssid_list,
    srv_dtcnt_wlan_cb_func_ptr callback,
    void *user_data)
{
#if defined (__MMI_WLAN_FEATURES__) && defined(__MTK_TARGET__) && defined(__TCPIP__)
    //MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, SRV_DTCNT_WLAN_SET_POWER_MODE, srv_dtcnt_wlan_prev_state(), srv_dtcnt_wlan_state(), pwr_mode);
    srv_dtcnt_wlan_desired_ssid_list_job_struct *spec_ssid_list_job = NULL;
    ...
    if(srv_dtcnt_wlan_check_chip_status() == MMI_FALSE)
    {
     ...
     if ((0 == ssid_list_num && ssid_list) || 
         (ssid_list_num && !ssid_list) ||
         DTCNT_WLAN_MAX_SPEC_SSID_LIST_NUM < ssid_list_num)
    {
        if (callback)
        {
            callback(user_data, SRV_DTCNT_WLAN_REQ_RES_FAILED);
    ...
    /* Last set is not completed. */
    if(g_srv_dtcnt_wlan_ctx.set_spec_ssid_list_job != NULL)
    {
        if (callback != NULL)
        {
            callback(user_data, SRV_DTCNT_WLAN_REQ_RES_FAILED);
    ...
    if (DTCNT_WLAN_MAX_SPEC_SSID_LIST_NUM < ssid_list_num)
    {
        ssid_list_num = DTCNT_WLAN_MAX_SPEC_SSID_LIST_NUM;
    }
        
    if(callback != NULL)
    {
        g_srv_dtcnt_wlan_ctx.set_spec_ssid_list_job = OslMalloc(sizeof(srv_dtcnt_wlan_desired_ssid_list_job_struct));
        spec_ssid_list_job = g_srv_dtcnt_wlan_ctx.set_spec_ssid_list_job;
        spec_ssid_list_job->cb_func_ptr = (void *)callback;
        spec_ssid_list_job->user_data = user_data;        
        spec_ssid_list_job->ssid_list_num = ssid_list_num;        
        if (ssid_list)
        {
            memcpy(spec_ssid_list_job->ssid_list, ssid_list, sizeof(srv_dtcnt_ssid_struct) * ssid_list_num);        
        }
        else
        {
            memset(spec_ssid_list_job->ssid_list, 0, sizeof(srv_dtcnt_ssid_struct) * DTCNT_WLAN_MAX_SPEC_SSID_LIST_NUM);        
        }
    }

    /* ssild_list_num = 0 and ssid_list = NULL is allowed. */
...
/* For detail description please refer to DtcntSrvIprot.h */
srv_dtcnt_result_enum srv_dtcnt_wlan_get_scan_spec_ssid_list(
    U8 *ssid_list_num, 
    srv_dtcnt_ssid_struct *ssid_list)
{
    ...
    *ssid_list_num = g_srv_dtcnt_wlan_scan_setting_ctx.ssid_list_num;
    
    memcpy(ssid_list, 
           g_srv_dtcnt_wlan_scan_setting_ctx.ssid_list, 
           sizeof(srv_dtcnt_ssid_struct) * (*ssid_list_num));
...
srv_dtcnt_result_enum srv_dtcnt_wlan_set_roaming_para(
    ...
    void *user_data)
{
    ...
    /* Last set is not completed. */
    if(g_srv_dtcnt_wlan_ctx.set_roaming_para_job != NULL)
    {
        if (callback != NULL)
        {
            callback(user_data, SRV_DTCNT_WLAN_REQ_RES_FAILED);
    ...
    if(callback != NULL)
    {
        g_srv_dtcnt_wlan_ctx.set_roaming_para_job = OslMalloc(sizeof(srv_dtcnt_wlan_enable_roaming_struct));
        g_srv_dtcnt_wlan_ctx.set_roaming_para_job->cb_func_ptr = (void *)callback;
        g_srv_dtcnt_wlan_ctx.set_roaming_para_job->user_data = user_data;
        g_srv_dtcnt_wlan_ctx.set_roaming_para_job->enable_roaming = enable_roaming;     
    }

    /* ssild_list_num = 0 and ssid_list = NULL is allowed. */
    srv_dtcnt_wlan_set_roaming_para_req(enable_roaming);
...
srv_dtcnt_result_enum srv_dtcnt_wlan_get_roaming_para(MMI_BOOL * is_roaming)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    //MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, SRV_DTCNT_WLAN_GET_POWER_MODE, srv_dtcnt_wlan_prev_state(), srv_dtcnt_wlan_state(), out_pwr_mode);
        ...
        *is_roaming = g_srv_dtcnt_wlan_scan_setting_ctx.is_roaming;
...
srv_dtcnt_result_enum srv_dtcnt_wlan_set_scan_dwell_time(
    U16 dwell_time,
    srv_dtcnt_wlan_cb_func_ptr callback, 
    void *user_data)
{
    ...
    /* Last set is not completed. */
    if(g_srv_dtcnt_wlan_ctx.set_dwell_time_job != NULL)
    {
        if (callback != NULL)
        {
            callback(user_data, SRV_DTCNT_WLAN_REQ_RES_FAILED);
    ...
    if(callback != NULL)
    {
        g_srv_dtcnt_wlan_ctx.set_dwell_time_job = OslMalloc(sizeof(srv_dtcnt_wlan_dwell_time_struct));
        g_srv_dtcnt_wlan_ctx.set_dwell_time_job->cb_func_ptr = (void *)callback;
        g_srv_dtcnt_wlan_ctx.set_dwell_time_job->user_data = user_data;
        g_srv_dtcnt_wlan_ctx.set_dwell_time_job->dwell_time = dwell_time;
    }

    /* ssild_list_num = 0 and ssid_list = NULL is allowed. */
    srv_dtcnt_wlan_set_dwell_time_req(dwell_time);
...
srv_dtcnt_result_enum srv_dtcnt_wlan_get_scan_dwell_time(U16 * dwell_time)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    //MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, SRV_DTCNT_WLAN_GET_POWER_MODE, srv_dtcnt_wlan_prev_state(), srv_dtcnt_wlan_state(), out_pwr_mode);
        ...
        *dwell_time = g_srv_dtcnt_wlan_scan_setting_ctx.dwell_time;
