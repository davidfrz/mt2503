...
#define MMI_FRM_WEARABLE_DEVICE_MOVE_THRESOLD      20//LCD_WIDTH/3
#define MMI_FRM_WEARABLE_DEVICE_MOVE_THRESOLD_Y      20
typedef struct
{
	U32 y_thresold;//only move when bigger than this value
	U32 x_thresold;//>, will trigger goback. <, will excute highlight handler
}mmi_wearable_move_thresold;

mmi_wearable_move_thresold g_wearable_move_thresold;
#endif
/***************************************************************************** 
* Typedef
*****************************************************************************/
...
#define TOUCH_PEN_EVENT_TRACK_DISPLAY_AND_PREDICT
#endif

#ifdef TOUCH_PEN_EVENT_TRACK_DISPLAY_AND_PREDICT
gdi_handle touch_track_layer = GDI_NULL_HANDLE;
U8 touch_track_display_switch=0;
U8 touch_track_predict_switch=0;
U8 touch_track_frame_lock=0;
#define MAX_DET_MOVE_PEN_EVENT_NUM  6
static mmi_pen_event_struct estim_buff[MAX_DET_MOVE_PEN_EVENT_NUM];
U16 est_coefficient=16; /**** tune  the sensitive of the predict, more bigger low sensitive,NONE ZERO******/
#endif 
 ...
 *  touch_track_config
 * DESCRIPTION
 *  switch the track display and predict by cache injection cmd
 * PARAMETERS
 *  index [IN] 
 *  string [IN]
 * RETURNS
 *  void
 *****************************************************************************/
void touch_track_config(U8 index, kal_uint8* string)
{
		/*----------------------------------------------------------------*/
		/* Local Variables												  */
		/*----------------------------------------------------------------*/
		...
		/* Code Body													  */
		/*----------------------------------------------------------------*/	
		if (strcmp((char*)string, "TOUCH_TRACK_DISPLAY") == 0)
		{
			if (index == 0)
			{
				touch_track_display_switch=0;	
				
				if(touch_track_layer != GDI_NULL_HANDLE)
				{
				    if(touch_track_frame_lock==0)
				    {	    
						gdi_layer_lock_frame_buffer();
						gdi_layer_push_and_set_active(touch_track_layer);
						gdi_layer_set_source_key(MMI_TRUE,GDI_COLOR_BLACK);
						gdi_layer_clear(GDI_COLOR_BLACK);
						gdi_layer_pop_and_restore_active();
					    gdi_layer_get_blt_layer(&handle0,&handle1,&handle2,&handle3);
					    gdi_layer_set_blt_layer(handle0,handle1,handle2,touch_track_layer);
					    gdi_layer_unlock_frame_buffer();		 
					    gdi_layer_blt(handle0,handle1,handle2,touch_track_layer,0,0,LCD_WIDTH,LCD_HEIGHT);							 
						gdi_image_free_temp_layer(touch_track_layer); 
						touch_track_layer = GDI_NULL_HANDLE;
					}
				}
			}
			else if (index == 1)
			{
				touch_track_display_switch=0xff;
			}
			else
			{
			    error=0xff;
		...
		else if (strcmp((char*)string, "TOUCH_TRACK_PREDICT") == 0)
		{
			if (index == 0)
			{
				touch_track_predict_switch=0;
			}
			else if (index == 1)
			{
				touch_track_predict_switch=0xff;
			}
			else
			{
			   error=0xff;
		...
		else if (strcmp((char*)string, "TOUCH_TRACK_PREDICT_TUNE") == 0)
		{
			if (index == 0)
			{
				est_coefficient+=1;   
				MMI_PRINT(MOD_MMI_FW, MMI_FW_TRC_G1_FRM, "injection TOUCH_TRACK_PREDICT_TUNE cmd,Current Coefficient:%d",est_coefficient);
			}
			else if (index == 1)
			{
			    if(est_coefficient>1) 
			    {
			    	 est_coefficient-=1; // increase the sensitive ,so need to decrease the est_coefficient.minimum is 1.
			    }
				MMI_PRINT(MOD_MMI_FW, MMI_FW_TRC_G1_FRM, "injection TOUCH_TRACK_PREDICT_TUNE cmd,Current Coefficient:%d",est_coefficient);
			}
			else
		...
		if(error==0)
		{
			MMI_PRINT(MOD_MMI_FW, MMI_FW_TRC_G1_FRM, "cacher injection cmd indx:%d,string:%s",index,(char*)string); 
		}
		else
		{	
		   MMI_PRINT(MOD_MMI_FW, MMI_FW_TRC_G1_FRM, "ERROR cacher injection cmd indx:%d,string:%s",index,(char*)string);
		}
}
#endif
...
static void mmi_pen_lookahead_buffer_filter_data(void)
{
	...
	gdi_handle handle2;
	gdi_handle handle3;
	static U8  down_detect=0,down_index=0;
	static U8  up_detect=0,up_index=0; 
	static U8  move_detect=0,move_indx=0;
	static s32 cur_x=0,cur_y=0;
	static s32 last_x=0,last_y=0;
    U32 dist_x=0,dist_y=0,distence=0,move_speed_2=0,dist_estimate=0;
	U16 direct_x=0,direct_y=0,temp=0,i=0,neg_x=0,neg_y=0,need_draw=0;
	S32 sum_x=0,sum_y=0,sum_xy=0;
#endif    
...
#ifdef TOUCH_PEN_EVENT_TRACK_DISPLAY_AND_PREDICT
            if(touch_track_display_switch)
           	{
				if(touch_track_layer == GDI_NULL_HANDLE)
				{
					gdi_image_create_temp_layer(&touch_track_layer, 
						        gdi_act_layer->cf, 
								0, 
								0, 
								LCD_WIDTH, 
				...
				gdi_layer_push_and_set_active(touch_track_layer);
				gdi_layer_set_source_key(MMI_TRUE,GDI_COLOR_BLACK);
				touch_track_frame_lock=0xff;
           	}
		   
			for (rindex = 0; rindex < g_frm_touch_queue.write; rindex++)
			{	
				cur_x=lookahead_buffer[rindex].x_adc;
				cur_y=lookahead_buffer[rindex].y_adc;
				
				switch (lookahead_buffer[rindex].event)
				{
					case DCL_PEN_DOWN:
						 //MMI_PRINT(MOD_MMI_FW, MMI_FW_TRC_G1_FRM, "TOUCH DOWN,X:%d,Y:%d",cur_x,cur_y);
						 if(touch_track_display_switch&&touch_track_layer)
						 {
							 gdi_layer_clear(GDI_COLOR_BLACK);
							 need_draw=0xff;
						 }
						 ...
						 last_x=cur_x;
					     last_y=cur_y;
						 estim_buff[move_detect%MAX_DET_MOVE_PEN_EVENT_NUM].x_adc=cur_x;
						 estim_buff[move_detect%MAX_DET_MOVE_PEN_EVENT_NUM].y_adc=cur_y;
						 move_detect++;						 
						break;
			
					case DCL_PEN_UP:
						 //MMI_PRINT(MOD_MMI_FW, MMI_FW_TRC_G1_FRM, "TOUCH UP,X:%d,Y:%d",cur_x,cur_y);
	                     down_detect=0;
						 up_detect=1;
					     sum_x=0;
				         sum_y=0;
                        ...
                        if(move_detect>1)
                        {
                           /*****************calculate move direction from rencetly two movement point ******************/
						   sum_x=estim_buff[(move_detect-1)%MAX_DET_MOVE_PEN_EVENT_NUM].x_adc-estim_buff[(move_detect-2)%MAX_DET_MOVE_PEN_EVENT_NUM].x_adc;
						   ...
						   sum_y=estim_buff[(move_detect-1)%MAX_DET_MOVE_PEN_EVENT_NUM].y_adc-estim_buff[(move_detect-2)%MAX_DET_MOVE_PEN_EVENT_NUM].y_adc;
						  ...
						  /*****************calculate average movement speed from buffered  points ******************/
                           if(move_detect>MAX_DET_MOVE_PEN_EVENT_NUM)
                           {
                           	  move_detect=MAX_DET_MOVE_PEN_EVENT_NUM;
                           }
						   
                           for(i=0;i<move_detect;i++)
                           {
							    temp=estim_buff[i+1].x_adc>estim_buff[i].x_adc?estim_buff[i+1].x_adc-estim_buff[i].x_adc:estim_buff[i].x_adc-estim_buff[i+1].x_adc;
								dist_x+=temp*temp;		
								temp=estim_buff[i+1].y_adc>estim_buff[i].y_adc?estim_buff[i+1].y_adc-estim_buff[i].y_adc:estim_buff[i].y_adc-estim_buff[i+1].y_adc;	
								dist_y+=temp*temp;
								i++;
                           	}
						   
						   /*****************calculate the finial predicted X,Y via speed and direction******************/
							distence=dist_x+dist_y;
							move_speed_2=distence/(move_detect*move_detect);
							dist_estimate=move_speed_2/est_coefficient;
																
							sum_xy=sum_x+sum_y;
							if(sum_x!=0)
							sum_x=dist_estimate*sum_x/sum_xy;
							if(sum_y!=0)
							sum_y=dist_estimate*sum_y/sum_xy;

							//MMI_PRINT(MOD_MMI_FW, MMI_FW_TRC_G1_FRM, "TOUCH PRIDICT,dist_estimate:%d,Speed:%d,Sum_x:%d,Neg_x:%d,Sum_y:%d,Neg_y:%d",\
					    //                                               dist_estimate,move_speed_2,sum_x,neg_x,sum_y,neg_y);
					...
					case DCL_PEN_MOVE:	
						if(touch_track_predict_switch)
				        {
							estim_buff[move_detect%MAX_DET_MOVE_PEN_EVENT_NUM].x_adc=cur_x;
							estim_buff[move_detect%MAX_DET_MOVE_PEN_EVENT_NUM].y_adc=cur_y;
							move_detect++;	
							//MMI_PRINT(MOD_MMI_FW, MMI_FW_TRC_G1_FRM, "TOUCH MOVE,X:%d,Y:%d,move num:%d",cur_x,cur_y,move_detect);
						}
						break;
				...
				if((sum_x>0||sum_y>0)&&touch_track_predict_switch)
				{
				   //MMI_PRINT(MOD_MMI_FW, MMI_FW_TRC_G1_FRM, "TOUCH preDRAW ,last_x:%d,last_y:%d,sum_x:%d,sum_y:%d",\
				   	                                                       last_x,last_y,sum_x,sum_y);
				   if(touch_track_display_switch&&touch_track_layer)
				   {
					   gdi_draw_line(last_x,last_y,sum_x,sum_y,GDI_COLOR_GREEN);
					   need_draw=0xff;
				   }
				   /*****************add the predicted X,Y  to current UP event******************/
				   lookahead_buffer[rindex].x_adc=sum_x;
				   lookahead_buffer[rindex].y_adc=sum_y;
				   sum_x=0;
				   sum_y=0;
				}
				else if((last_x!=cur_x)||(last_y!=cur_y))
				{
				   if(touch_track_display_switch&touch_track_layer)
				   {
					   gdi_draw_line(last_x,last_y,cur_x,cur_y,GDI_COLOR_RED);
					   need_draw=0xff;
				   }
				   last_x=cur_x;
				   last_y=cur_y;
...
extern g_pen_to_control;
void handle_single_touch_event(void)
{
    mmi_pen_event_struct data;
    U16      interval = 0;
#if defined(__MMI_WEARABLE_DEVICE__)
		mmi_group_event_struct direction_evt;
		mmi_event_struct wearable_device_move_event;
		mmi_ret result_of_dir_evt = MMI_RET_OK;
#endif
    mmi_pen_point_struct pos;
...
#endif /* __MMI_TOUCH_PANEL_SHORTCUT__ */
    {
        ...
        switch (data.event)
        {
            ...
            case DCL_PEN_UP:
                ...
                {
                	S32 delta_x = data.x_adc - g_pen_cntx.pen_down_pos.pos.x;
					S32 delta_y = data.y_adc - g_pen_cntx.pen_down_pos.pos.y;
                    ...
                    g_pen_cntx.is_pen_down = 0;
										...
										if(abs_delta_x > g_wearable_move_thresold.x_thresold)
										{
											if(abs_delta_y < g_wearable_move_thresold.y_thresold ||
												(abs_delta_x > abs_delta_y) )
												{
													if(delta_x > (S32)g_wearable_move_thresold.x_thresold)//if(data.x_adc - g_pen_cntx.pen_down_pos.pos.x > MMI_FRM_WEARABLE_DEVICE_MOVE_THRESOLD)//
													{
															MMI_FRM_INIT_EVENT(&wearable_device_move_event, EVT_ID_WEARABLE_DEVICE_MOVE_RIGHT);
															result_of_dir_evt = MMI_FRM_CB_EMIT_EVENT(&wearable_device_move_event);
															
															if (result_of_dir_evt == MMI_RET_OK || result_of_dir_evt == MMI_RET_ERR_NO_REG_CB)
															{
																	MMI_FRM_INIT_GROUP_EVENT(&direction_evt, EVT_ID_TOUCHSCREEN_MOVE_RIGHT, 0);
																	if(mmi_frm_send_event_to_group(mmi_frm_group_get_active_id(), &direction_evt) == MMI_RET_OK)
																	{
																	        g_pen_to_control = 0;
																			GoBackHistory();
																			break;
													...
													{ 
														if(-delta_x > (S32)g_wearable_move_thresold.x_thresold)//MMI_FRM_WEARABLE_DEVICE_MOVE_THRESOLD
														{
															MMI_FRM_INIT_EVENT(&wearable_device_move_event, EVT_ID_WEARABLE_DEVICE_MOVE_LEFT);
															MMI_FRM_CB_EMIT_EVENT(&wearable_device_move_event);
															
															MMI_FRM_INIT_GROUP_EVENT(&direction_evt, EVT_ID_TOUCHSCREEN_MOVE_LEFT, 0);
															mmi_frm_send_event_to_group(mmi_frm_group_get_active_id(), &direction_evt);
														}
													}
												}
												else if(abs_delta_y> abs_delta_x && abs_delta_y > g_wearable_move_thresold.y_thresold)
												{

												}
												else
												{
													//unhandle in this case
													//g_pen_to_control = 0;
													//break;
												}

											}	
										else if(abs_delta_y > g_wearable_move_thresold.y_thresold)
										{
...
void mmi_pen_init(void)
{
	...
	//init for smart watch
#if defined(__MMI_WEARABLE_DEVICE__)
	g_wearable_move_thresold.x_thresold = MMI_FRM_WEARABLE_DEVICE_MOVE_THRESOLD;
	g_wearable_move_thresold.y_thresold = MMI_FRM_WEARABLE_DEVICE_MOVE_THRESOLD_Y;
 ...
 *  void
 *****************************************************************************/
extern kal_uint8 IRQ_Query_If_TS_Unknown_Resume(void);
void mmi_pen_enable(void)
{
    ...
    if (!g_pen_simulate_driver_cntx.is_AT_command)
    {
        ...
        mmi_pen_drv_enable(MMI_TRUE);
#else
					if(IRQ_Query_If_TS_Unknown_Resume() == 0)
    	{
    		kal_prompt_trace(MOD_MMI, "[ENABLE_TOUCH]power on");
        	mmi_pen_drv_enable(MMI_TRUE);
    	}
		else
		{
			...
			{
				kal_prompt_trace(MOD_MMI, "[ENABLE_TOUCH]power on when bootup completed");
        		mmi_pen_drv_enable(MMI_TRUE);
			}
			else
			{
				kal_prompt_trace(MOD_MMI, "[ENABLE_TOUCH]power off");
				mmi_pen_drv_enable(MMI_FALSE);
				mmi_pen_drv_power_on(MMI_FALSE);
			}
		}
...
#if defined(__TINY_SYS__) 
mmi_ret mmi_tiny_to_ap_power_on_touch(mmi_event_struct *evt)
{
	MMI_ASSERT(evt->evt_id == EVT_ID_GPIO_BACKLIGHT_ON);
	kal_prompt_trace(MOD_MMI, "[POWER_ON_TOUCH]when switch from tiny to ap");
	mmi_pen_drv_power_on(MMI_TRUE);
	mmi_pen_drv_enable(MMI_TRUE);
	return MMI_RET_OK;
}
...
mmi_ret mmi_tiny_to_ap_power_on_touch(mmi_event_struct *evt)
{
	kal_prompt_trace(MOD_MMI, "[POWER_ON_TOUCH]empty callback");
	return MMI_RET_OK;
}
#endif
