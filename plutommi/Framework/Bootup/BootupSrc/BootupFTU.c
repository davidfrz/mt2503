    ...
    const mmi_frm_proc_id_info_struct id_info)
{
    ...
    S16 error;
    S32 first_boot = 0;
    MMI_TRACE(MMI_BOOTUP_TRC_GROUP, TRC_FRM_FTU_FLOW);

    if(srv_bootup_is_auto_power_on_test() == MMI_FALSE)
    {
        ReadValue(NVRAM_BT_DIALER_FIRST_BOOT, &first_boot, DS_SHORT, &error);
    MMI_TRACE(MMI_BOOTUP_TRC_GROUP, TRC_FRM_FTU_FLOW_FIRST_BOOT_FLAG,first_boot);
...
void mmi_bt_dialer_ask_first(void)
{
                ...
                MMI_FRM_FULL_SCRN) == MMI_FALSE)
    {
        ...
        MMI_TRACE(MMI_BOOTUP_TRC_GROUP, TRC_FRM_FTU_FLOW_SCR_FAIL);        
        return;
    }
    ShowCategory74Screen(
                ...
                0,
                0,
                (U8*) GetString(STR_BT_MYDEV_HELP_CONTENT),
                mmi_ucs2strlen(GetString(STR_BT_MYDEV_HELP_CONTENT)),
                NULL);
...
void mmi_bt_dialer_ftu_lsk()
{
    ...
    MMI_TRACE(MMI_BOOTUP_TRC_GROUP, TRC_FRM_FTU_FLOW_OK);    
