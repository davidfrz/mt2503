...
#if defined(__MMI_IME_VKV1_DEAD_KEY__)
/*****************************************************************************
 * FUNCTION
...
#endif /*__MMI_IME_VKV1_DEAD_KEY__*/
...
#endif /*#if defined(__MMI_IME_VKV1_DEAD_KEY__)*/
...
#if defined(__MMI_IME_VKV1_SLIM__)
        MMI_ASSERT((((S32) s->key_width * s->key_height * gdi_layer_get_bit_per_pixel() + 7) >> 3) < sizeof(bitmap_buffer));
#else
...
void gui_virtual_keyboard_draw_key_char_internal(
    virtual_keyboard* v, 
...
void gui_show_virtual_keyboard(virtual_keyboard *v)
{
    ...
    for (k = 0; k < layout->n_matrix_key_layout; k++)
    {
        ...
        for (i = 0; i < n_matrix_row; i++)
        {
            ...
            {
                gui_virtual_keyboard_draw_key_char_internal(
                    v,
                    matrix_string[j],
                    key_x,
                    key_y,
    ...
    for (i = 0; i < layout->n_custom_keys; i++)
    {
        ...
        #endif /*#if defined(__MMI_MAINLCD_320X480__)*/
        else if (!key_char)
        {
    ...
    if (v->key_down_accepted == MMI_TRUE)
    {
        if (gui_virtual_keyboard_is_popup_can_be_shown(v,v->selected_key.index))
        {
			...
			else
			{
				...
				gui_virtual_keyboard_pen_enum temp_event = GUI_VKBD_PEN_NONE;
				gui_virtual_keyboard_get_selected_key(v, v->selected_key.index, MMI_PEN_EVENT_LONG_TAP, &temp_char, &temp_event); 
    ...
    const gui_virtual_keyboard_key_struct** key)
{
    ...
    *key = NULL;
    ...
    for (i = 0; i < v->row_num; i++)
    {
        ...
        {
            key_x = v->x + v->row_list[i].key_list[j].pos_x + v->row_list[i].key_list[j].width;
            if (x > key_x && (j < v->row_list[i].key_count - 1))
            {
                continue;
            }
            if (x < key_x || (j == v->row_list[i].key_count - 1))
    ...
    MMI_BOOL cache)
{
    ...
    if (key_state == GUI_VK_KEY_STATE_LONGTAP_POPUP) //sync with cosmos do not show long tap popup hint
    {
        return;
    }
