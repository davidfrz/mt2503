...
void srv_backlight_init_context_early(void)
{
	...
	{
		  #ifdef __TINY_SYS__ 
  			if( (COLD_BOOT == AP_check_boot_up_source()) ||  (WARM_BOOT == AP_check_boot_up_source()) )
  			{
  				g_gpio_p->lcd_state = 0;// hibernation mode resume
  			}
  			else
  	#endif
  	{
...
static void srv_backlight_lcd_sleep_in(void)
{
    ...
    // tracker + gdi, need capability of sleep in
