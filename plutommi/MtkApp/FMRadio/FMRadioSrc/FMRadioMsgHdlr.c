...
void mmi_fmradio_search_after(void)
{
    ...
    if (mmi_fmradio_is_active())
    {
	    ...
	    else
	    {
			...
			mdi_audio_set_volume(MDI_VOLUME_FMR, MDI_AUD_VOL_MUTE(g_fmradio.volume));
	else
...
MMI_BOOL mmi_fmrdo_continued(S32 result, void *user_data)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
	MMI_TRACE(MMI_MEDIA_TRC_G2_APP,MMI_FMRDO_TRC_CONTINUE,result, g_fmrdo.state);
    ...
    if (g_fmrdo.is_preempted)
    {
        ...
        {
#ifdef __MMI_WEARABLE_DEVICE__
			/* Draw whole screen for wearable device */
...
void mmi_fmrdo_preempted(S32 result, void *user_data)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
	MMI_TRACE(MMI_MEDIA_TRC_G2_APP, MMI_FMRDO_TRC_PREEMPTED, result, g_fmrdo.state);
...
void mmi_fmrdo_goto_channel(U16 channel_index)
{
    ...
    g_fmrdo.channel_index = channel_index;
		...
		{
			wgui_cat1032_refresh_list(mmi_fmrdo_get_channel_count() + 1, channel_index + 1, mmi_fmrdo_channel_list_item_ext, NULL);
		}
#endif
...
void mmi_fmrdo_set_frequency(U16 freq)
{
...
#endif/*__MMI_FM_RADIO_RDS__*/
    g_fmrdo.frequency = freq;
	prev_channel_index = g_fmrdo.channel_index;
    ...
    else
    {
        g_fmrdo.channel_index = -1;
			...
			if(mmi_frm_scrn_get_active_id() == SCR_ID_FMRDO_CHANNEL_LIST)
			{
		if(!(prev_channel_index == -1 && g_fmrdo.channel_index == -1))
		{
				wgui_cat1032_refresh_list(mmi_fmrdo_get_channel_count() + 1, g_fmrdo.channel_index + 1, mmi_fmrdo_channel_list_item_ext, NULL);
			}
	}
#endif
...
#ifdef __MMI_WEARABLE_DEVICE__
	if(!turn_on && (mmi_frm_scrn_get_active_id() == SCR_ID_FMRDO_CHANNEL_LIST))
	{
...
void mmi_fmrdo_search_before(void)
{
	...
	/* Suspend scr locker for searching */
...
void mmi_fmrdo_search_after(void)
{
    ...
    /*----------------------------------------------------------------*/
#ifdef __MMI_WEARABLE_DEVICE__
	/* Resume scr locker after searching is finished/cancelled */
...
#ifdef __MMI_FM_VIA_A2DP__
#ifndef __MMI_WEARABLE_DEVICE__
	/* For wearable, handle this in mmi_fmrdo_preset_list_done to avoid state change to FMRDO_STATE_BT_CONNECTING */
#if defined(__MMI_FM_RADIO_LOUD_SPEAK__)
	if (!g_fmrdo.loud_speaker_on)
	{
#endif
...
#ifdef __MMI_WEARABLE_DEVICE__
	mdi_fmr_freq_seek_start(MIN_FM_FREQUENCY, MMI_TRUE, MMI_FALSE, 100, mmi_fmrdo_hw_search_all_channels);
#else
...
*  void
*****************************************************************************/
void mmi_fmrdo_hw_scan_finish_ext(void)
{
	/*----------------------------------------------------------------*/
	/* Local Variables                                                */
	/*----------------------------------------------------------------*/
	...
	MMI_TRACE(MMI_MEDIA_TRC_G2_APP, MMI_FMRDO_TRC_HW_SCAN_FINISH_EXT);

	if(g_fmrdo.state == FMRDO_STATE_SEARCH_ALL)
	{
		g_fmrdo.state = FMRDO_STATE_SEARCH_ALL_STOPPED;
...
void mmi_fmrdo_hw_search_all_channels(U16 stop_freq, U8 signal_level, MMI_BOOL is_valid)
{
    ...
    /*----------------------------------------------------------------*/
#ifdef __MMI_WEARABLE_DEVICE__
	if (g_fmrdo.state != FMRDO_STATE_SEARCH_ALL || g_fmrdo.state == FMRDO_STATE_SEARCH_ALL_STOPPED)
#else
    if (g_fmrdo.state != FMRDO_STATE_SEARCH_ALL)
#endif
    {
...
#ifdef __MMI_WEARABLE_DEVICE__
		mdi_fmr_freq_seek_start(g_temp_start_freq, MMI_TRUE, MMI_FALSE, 100, mmi_fmrdo_hw_search_all_channels);
#else
		mdi_fmr_freq_seek_start(g_temp_start_freq, MMI_TRUE, MMI_TRUE, 100, mmi_fmrdo_hw_search_all_channels);
#endif		
    }
}
#endif /* __FM_RADIO_HW_SCAN__ */ 
