...
{  
	...
	if(!srv_mre_appmgr_check_current_defined_authorization(SRV_MRE_APPMGR_APP_AUTH_CAMERA))
	{
		vm_log_fatal("%s", "[Security check]camera failed");
		return VM_CAM_ERR_UNSPPUORT_CAMERA_FEATURE;
	}
	vm_log_fatal("%s", "[Security check]camera success");
#endif
