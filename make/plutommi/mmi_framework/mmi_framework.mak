...
SRC_LIST = plutommi\customer\custresource\custnfbprogressimg.c \
           ...
           plutommi\Framework\GUI\gui_src\wgui_slim.c \
...
ifneq  ($(filter __MMI_FRM_INPUT_EVT__, $(strip $(MODULE_DEFS))),)
SRC_LIST += plutommi\Framework\EventHandling\eventssrc\mmi_frm_input_event.c
endif

ifeq  ($(filter __MMI_UI_DISPLAY_LEVEL_NONE__, $(strip $(MODULE_DEFS))),) 
...
ifneq ($(filter __COMMAND_PORT_SUPPORT__, $(strip $(MODULE_DEFS))),)
SRC_LIST += plutommi\Framework\CommandProcessor\CommandProcessorSrc\CommandProcess.c
endif

ifneq ($(filter __IOT__, $(strip $(MODULE_DEFS))),)
SRC_LIST += plutommi\Framework\Bridge\BridgeSrc\CommBridge.c
endif

ifneq ($(filter __IOT__, $(strip $(MODULE_DEFS))),)
SRC_LIST += plutommi\Framework\Bootup\BootupSrc\BootupFlow.c \
           plutommi\Framework\Bootup\BootupSrc\BootupAdp.c \
           plutommi\Framework\Bootup\BootupSrc\BootupMoDIS.c \
           plutommi\Framework\Bootup\BootupSrc\BootupDiskCheck.c \
           plutommi\Framework\Bootup\BootupSrc\BootupInitApps.c \
           plutommi\Framework\Bootup\BootupSrc\BootupOnUsb.c \
           plutommi\Framework\Bootup\BootupSrc\BootupMode.c \
           plutommi\Framework\Shutdown\ShutdownSrc\ShutdownFlow.c
endif
...
SRC_LIST += plutommi\mmi\miscframework\miscframeworksrc\BtSS_register.c
endif

ifneq  ($(filter __MMI_UI_DISPLAY_LEVEL_NONE__, $(strip $(MODULE_DEFS))),)
SRC_LIST += plutommi\Framework\Charset\ascii.c \
...
ifneq  ($(filter __DISPLAY_LEVEL_GDI_ONLY__, $(strip $(MODULE_DEFS))),)            
SRC_LIST += plutommi\Framework\LangModule\LangModuleSrc\CommonLang.c \
...
ifneq ($(filter __MULTI_THREAD_SUPPORT__, $(strip $(MODULE_DEFS))),)
           SRC_LIST += plutommi\Framework\ThreadManager\src\app_task.c \
                       plutommi\Framework\ThreadManager\src\app_thread_mgr.c \
                       plutommi\Framework\CommonFiles\CommonSrc\app_frm_utility.c \
                       plutommi\Framework\MessageMgr\Src\app_frm_message.c \
                       plutommi\Framework\ThreadManager\src\app_thread_mutex.c \
                       plutommi\Framework\ThreadManager\src\app_thread_signal.c \
                       plutommi\Framework\ThreadManager\src\app_frm_relay_signal.c \
                       plutommi\Framework\Launcher\src\app_frm_launcher.c
endif

ifneq ($(filter __IOT__, $(strip $(MODULE_DEFS))),)
           SRC_LIST += plutommi\Framework\RouterCtrl\src\router_ctrl.c
endif
...
INC_DIR = applib\inet\engine\include \
          ...
          plutommi\Framework\Bootup\BootupInc \
          plutommi\Framework\Shutdown\ShutdownInc \
...
ifneq ($(filter __COMMAND_PORT_SUPPORT__, $(strip $(MODULE_DEFS))),)
INC_DIR += plutommi\Framework\CommandProcessor\CommandProcessorInc
endif
ifneq ($(filter __TINY_SYS__, $(strip $(MODULE_DEFS))),)
...
ifneq ($(filter __MULTI_THREAD_SUPPORT__, $(strip $(MODULE_DEFS))),)
INC_DIR += plutommi\Framework\ThreadManager\inc \
           plutommi\Framework\CommonFiles\CommonInc \
...
ifneq ($(filter __IOT__, $(strip $(MODULE_DEFS))),)
INC_DIR += plutommi\Framework\RouterCtrl\inc
endif
ifeq ($(filter __MTK_TARGET__ , $(strip $(MODULE_DEFS))),)
INC_DIR += MoDIS_VC9\plugin\inc \
           MoDIS_VC9\MoDIS
endif
...
ifneq ($(filter  __MMI_WEARABLE_DEVICE_SETTING__, $(strip $(MODULE_DEFS))),)
SRC_LIST += plutommi\Framework\GUI\gui_src\wgui_categories_btbox.c \
			plutommi\Framework\GUI\gui_src\gui_picker.c 
endif

ifneq ($(filter  __MMI_WEARABLE_DEVICE_ALARM__, $(strip $(MODULE_DEFS))),)
SRC_LIST += plutommi\Framework\GUI\gui_src\wgui_categories_btbox.c \
			plutommi\Framework\GUI\gui_src\gui_picker.c 
endif

ifneq ($(filter __MMI_WEARABLE_DEVICE__ , $(strip $(MODULE_DEFS))),)
	SRC_LIST += plutommi\Framework\GUI\GUI_SRC\wgui_swatch_common_pen_handle.c

endif
ifneq ($(filter __HOTKNOT_SUPPORT__ , $(strip $(MODULE_DEFS))),)
	SRC_LIST += plutommi\Framework\HotKnot\src\hotknot.c
	INC_DIR += plutommi\Framework\HotKnot\inc
	INC_DIR += custom\drv\CTP_module\CTP_GOODIX_GT9XX_TRUE_MULTIPLE
endif

# build UBIN symbol array for MoDIS
ifeq ($(filter __MTK_TARGET__,$(strip $(MODULE_DEFS))),)
