...
#endif
#if (MMI_MAX_SIM_NUM >= 2) && !defined(__IOT__)
...
#endif
#ifdef __MOD_RAC__
    { MMI_FRM_PROC_ID_BOOTUP_PROTOCOL, mmi_bootup_boot_protocol, NULL},
#endif
    { MMI_FRM_PROC_ID_BOOTUP_NORMAL_INIT, mmi_bootup_normal_init, NULL},
#if !defined(__IOT__)
	{ MMI_FRM_PROC_ID_BOOTUP_ENABLE_INTERRUPT, mmi_bootup_flow_enable_interrupt, NULL },
#endif
#if defined(__MMI_TOUCH_SCREEN__) && !defined(__IOT__)
...
#endif
#if defined(__MMI_BT_DIALER_SUPPORT__) && !defined(__IOT__)
...
mmi_ret mmi_bootup_flow_start(mmi_event_struct *evt)
{
    ...
    MMI_TRACE(MMI_BOOTUP_TRC_GROUP, TRC_FRM_BOOTUP_FLOW_START);
...
static void mmi_bootup_events_timer_timeout(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_BOOTUP_TRC_GROUP, TRC_FRM_BOOTUP_EVENT_TIMER_TIMEOUT);
...
#ifdef __DISPLAY_LEVEL_GDI_ONLY__
#if defined(__MMI_MAINLCD_240X240__) || defined(__MMI_MAINLCD_128X128__)
extern const U8 power_on_img[];
static GDI_HANDLE poweron_anim_handle = GDI_NULL_HANDLE;
#endif
void ShowPowerOnAnimScr()
{
#if defined(__MMI_MAINLCD_240X240__) || defined(__MMI_MAINLCD_128X128__)
    GDI_HANDLE base_layer;
    S32 w, h;
    S32 img_size;
    img_size = sizeof(power_on_img) - 8;
    gdi_layer_get_base_handle(&base_layer);
    gdi_layer_push_and_set_active(base_layer);
    gdi_layer_clear(GDI_COLOR_BLACK);
    gdi_image_get_dimension_mem(GDI_IMAGE_TYPE_GIF, (U8*)power_on_img + 8, img_size, &w, &h);
    gdi_anim_draw_mem_once(0, 0, (U8*)power_on_img + 8, GDI_IMAGE_TYPE_GIF, img_size, &poweron_anim_handle);
                                    ...
                                    const mmi_frm_proc_id_info_struct id_info)
{
    ...
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
	MMI_BOOL enable_tone = MMI_TRUE;
	U16 power_on_tone = AUD_ID_PROF_TONE1;
	U8 volme = MDI_AUD_VOL_6;
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_BOOTUP_TRC_GROUP, TRC_FRM_BOOTUP_FLOW_EARLY_INIT);
    ...
    srv_prof_set_current_profile_value(SRV_PROF_SETTINGS_POWER_ON_OFF_TONE, &enable_tone);
    srv_prof_set_current_profile_value(SRV_PROF_SETTINGS_POWER_ON_TONE, &power_on_tone);
	srv_prof_set_current_profile_value(SRV_PROF_SETTINGS_RING_VOL_LEVEL, &volme);
...
static void mmi_bootup_disk_check_completed(void *user_data, S32 error_code)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_BOOTUP_TRC_GROUP, TRC_FRM_BOOTUP_DISK_CHECK_COMPLETED, error_code);
...
static void mmi_bootup_emit_interruptible_event(MMI_BOOL post_event)
{
    ...
    MMI_TRACE(MMI_BOOTUP_TRC_GROUP, TRC_FRM_BOOTUP_EMIT_INTERRUPTIBLE_EVENT,
...
#if defined(__IOT__)
#if !defined(__DISPLAY_LEVEL_GDI_ONLY__)
    //tracker no lcd
    srv_gpio_set_device_level(SRV_GPIO_DEV_MAINLCD, SRV_GPIO_DEV_LEVEL0);
#else
    //tracker + gdi
...
#if defined(__IOT__) && defined(__MTK_INTERNAL__)
/* under construction !*/
/* under construction !*/
/* under construction !*/
#endif
