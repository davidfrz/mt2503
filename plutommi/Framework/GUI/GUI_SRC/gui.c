 ...
 *  void
 *****************************************************************************/
void UI_dummy_function_with_2_s32(S32 a, S32 b)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
 ...
 *  void
 *****************************************************************************/
void UI_dummy_function_with_4_s32(S32 a, S32 b, S32 c, S32 d)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
 ...
 *  void
 *****************************************************************************/
void gui_transparent_texture_fill_rectangle(
	S32 x1, 
	S32 y1, 
	S32 x2, 
	S32 y2, 
 ...
 *  void
 *****************************************************************************/
void gui_texture_fill_rectangle(
	S32 x1, 
	S32 y1, 
	S32 x2, 
	S32 y2, 
	PU8 image)
{
    ...
    /*----------------------------------------------------------------*/
    if ((y2 < y1) || (x2 < x1) || 
		(x1 >= GDI_LCD->act_width) || (x2 < 0) 
		|| (y1 >= GDI_LCD->act_height) || (y2 < 0))
    {
        return;
    }
	...
	S32 black_value)
{
    ...
    /*----------------------------------------------------------------*/
    if ((x1 == 0) && (y1 == 0) &&
		(x2 == UI_device_width - 1) && (y2 == UI_device_height - 1))
    {
        /* Full screen case */
    #if (__MMI_UI_FADE_EFFECT_STYLE__ == MMI_UI_FADE_EFFECT_STYLE_DARKEN)
        ...
        #ifdef __MMI_MAINLCD_320X480__
        {
             ...
             */
            if (!gui_check_watermark(0, 0, MMI_STATUS_BAR_HEIGHT))
            {
             ...
             */
            if (!gui_check_watermark(0, MMI_STATUS_BAR_HEIGHT, MMI_TITLE_HEIGHT))
            {
                gdi_draw_darken_rect(
					0, 
					MMI_STATUS_BAR_HEIGHT, 
					UI_device_width - 1, 
					MMI_STATUS_BAR_HEIGHT + MMI_TITLE_HEIGHT - 1, 
					GUI_DARKEN_PERCENT);
             ...
             */
            if (!gui_check_watermark(0, MMI_STATUS_BAR_HEIGHT + MMI_TITLE_HEIGHT, 32))
            {
                gdi_draw_darken_rect(
					0, 
...
/* under construction !*/
             ...
             */
            if (!gui_check_watermark(0, UI_device_height - MMI_BUTTON_BAR_HEIGHT, MMI_BUTTON_BAR_HEIGHT))
            {
                gdi_draw_darken_rect(
					0, 
            ...
            if (!gui_check_watermark(0, UI_device_height - MMI_BUTTON_BAR_HEIGHT - 32, 32))
            {
             #ifdef __MMI_UI_VIRTUAL_KEYBOARD_DEFAULT_VERSION_1__
        ...
        color end_color)//,S32 border_width
{
    ...
    gui_draw_rounded_corner(
    	...
    	end_color, 
    	0, 
    	MMI_GRADIENT_RECT_ROUNDED_CORNER_TOP_LEFT);
...
void gui_one_layer_create_bg_image_cache(void);
void gui_one_layer_free_image_cache(void);

gui_one_layer_draw_bg_stage_enum draw_bg_flag_one_layer = GUI_ONE_LAYER_DRAW_BG_WITH_IMAGE ;

extern MMI_ID_TYPE idle_screen_wallpaper_ID;

gdi_image_cache_bmp_struct gui_one_layer_img_bmp_cache;

void (*gui_one_layer_draw_bg_cb) (S32 x1, S32 y1, S32 x2, S32 y2);


void gui_one_layer_draw_filled_bg_area_with_color(void)
{
	draw_bg_flag_one_layer = GUI_ONE_LAYER_DRAW_BG_WITH_COLOR ; 
}
void gui_one_layer_draw_filled_bg_area_with_image(void)
{
	draw_bg_flag_one_layer = GUI_ONE_LAYER_DRAW_BG_WITH_IMAGE ; 
}
void gui_one_layer_draw_filled_bg_area_with_image_cache(void)
{
	draw_bg_flag_one_layer = GUI_ONE_LAYER_DRAW_BG_WITH_IMAGE_CACHE ; 
}
...
{
	gui_one_layer_draw_filled_bg_area(0,0,UI_device_width-1,UI_device_height-1);

    if(draw_bg_flag_one_layer == GUI_ONE_LAYER_DRAW_BG_WITH_IMAGE_CACHE)
    {   // create BG image cache
        memset(&gui_one_layer_img_bmp_cache,0,sizeof(gdi_image_cache_bmp_struct));
		gui_one_layer_create_bg_image_cache();
	}
	if(draw_bg_flag_one_layer != GUI_ONE_LAYER_DRAW_BG_WITH_CALL_BACK_FUNC)
	{
		gui_one_layer_draw_bg_cb = NULL;
	}
...
void gui_one_layer_draw_filled_bg_area_with_style(
	S32 x1, 
	S32 y1, 
	S32 x2, 
	S32 y2, 
	gui_one_layer_draw_bg_stage_enum  draw_style)
{
	...
	color c;
    gui_one_layer_draw_bg_stage_enum  draw_st = GUI_ONE_LAYER_DRAW_BG_END_OF_ENUM;
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
	gdi_layer_push_and_set_clip(x1, y1, x2, y2);	
	...
	if(draw_style == GUI_ONE_LAYER_DRAW_BG_WITH_COLOR)
	{
		c.r = 0xe4;
		c.g = 0xde;
		c.b = 0xec;
		c.alpha = 100;
		gui_fill_rectangle(rx1, ry1, rx2, ry2, c);
	}
	else if(draw_style == GUI_ONE_LAYER_DRAW_BG_WITH_IMAGE)
	{  
		draw_st = GUI_ONE_LAYER_DRAW_BG_WITH_IMAGE;
	}
	else if(draw_style == GUI_ONE_LAYER_DRAW_BG_WITH_IMAGE_CACHE)
	{  
		if(gui_one_layer_img_bmp_cache.buf_ptr)
		{
			gdi_image_cache_bmp_draw(0, 0, &gui_one_layer_img_bmp_cache);
		}	
		else
		{
			draw_st = GUI_ONE_LAYER_DRAW_BG_WITH_IMAGE;
	...
	else if(draw_style == GUI_ONE_LAYER_DRAW_BG_WITH_CALL_BACK_FUNC)
	{	
		if(gui_one_layer_draw_bg_cb)
		{
			gui_one_layer_draw_bg_cb(x1, y1, x2, y2);
		}
	}

    if(draw_st == GUI_ONE_LAYER_DRAW_BG_WITH_IMAGE)
    {
        gdi_handle source_layer;
        gdi_layer_get_active(&source_layer);
        gdi_push_and_set_alpha_blending_source_layer(source_layer);
        if(mmi_idle_is_active()
			#if defined(__MMI_SEARCH_WEB__)
			||(g_dm_data.s32CatId == MMI_CATEGORY33_ID)
			#endif /*__MMI_SEARCH_WEB__*/
			)
        {
			gui_show_image(0, 0, (PU8) get_image(idle_screen_wallpaper_ID));
        }
		else
		{   
...
void gui_one_layer_draw_filled_bg_area(
	S32 x1, 
	S32 y1, 
	S32 x2, 
	S32 y2)
{
	gui_one_layer_draw_filled_bg_area_with_style(x1,y1,x2, y2, draw_bg_flag_one_layer);
}
void gui_one_layer_regist_draw_filled_bg_area_func(void (*draw_func) (S32 x1, S32 y1, S32 x2, S32 y2))
{
	gui_one_layer_draw_bg_cb = draw_func;
	draw_bg_flag_one_layer = GUI_ONE_LAYER_DRAW_BG_WITH_CALL_BACK_FUNC;
}

/*
to avoid memory leak
*/
static U32 gui_one_layer_image_cache_ref = 0;

void gui_one_layer_create_bg_image_cache()
{
	S32 cache_sz = (UI_device_width) * (UI_device_height) * gdi_layer_get_bit_per_pixel() >> 3;
	S32 x1 = 0;
	S32 y1 = 0;
	S32 x2 = UI_device_width - 1;
	S32 y2 = UI_device_height - 1;

	//to avoid memory leak
	MMI_ASSERT(gui_one_layer_image_cache_ref == 0);
	gui_one_layer_image_cache_ref++;
	
    gui_one_layer_img_bmp_cache.buf_ptr = (U8*)med_alloc_temp_ext_mem(cache_sz, gui_one_layer_free_image_cache);
             
	if(gui_one_layer_img_bmp_cache.buf_ptr)
	{
		gui_one_layer_img_bmp_cache.width = (x2 - x1);
		gui_one_layer_img_bmp_cache.height = (y2 - y1);
...
void gui_one_layer_free_image_cache(void)
{
	if(gui_one_layer_img_bmp_cache.buf_ptr)
	{
		gui_one_layer_img_bmp_cache.buf_ptr = NULL;	
		gui_one_layer_img_bmp_cache.width = 0;
		gui_one_layer_img_bmp_cache.height = 0;
//to avoid memory leak
		gui_one_layer_image_cache_ref--;
	}
}
...
*/
gdi_image_cache_bmp_struct g_img_bmp_cache;
/*
to avoid memory leak
*/
static U32 gui_image_cache_ref = 0;

void gui_create_image_cache(
	S32 x1,// 
	S32 y1,
	S32 x2,
	S32 y2)//
{
	S32 cache_sz = (x2 - x1) * (y2 - y1) * gdi_layer_get_bit_per_pixel() >> 3;

	//to avoid memory leak
	MMI_ASSERT(gui_image_cache_ref == 0);
	gui_image_cache_ref++;
	
    g_img_bmp_cache.buf_ptr = 
		(U8*) mmi_frm_scrmem_alloc_framebuffer(cache_sz);

	if(g_img_bmp_cache.buf_ptr)
	{
		g_img_bmp_cache.width = (x2 - x1);
		g_img_bmp_cache.height = (y2 - y1);
...
//
	if(g_img_bmp_cache.buf_ptr)
    	gdi_image_cache_bmp_draw(x1, y1, &g_img_bmp_cache);
	else
	{
...
void gui_free_image_cache(void)
{
	if(g_img_bmp_cache.buf_ptr)
	{
		mmi_frm_scrmem_free(g_img_bmp_cache.buf_ptr);	
		g_img_bmp_cache.buf_ptr = NULL;
	
		g_img_bmp_cache.width = 0;
		g_img_bmp_cache.height = 0;
...
//        case UI_FILLED_AREA_TYPE_IMAGE_LEFT_ALIGN:
            ...
            break;
			//huking
			///TODO: remove UI_FILLED_AREA_TYPE_IMAGE_RIGHT_ALIGN completely
#if 0			
/* under construction !*/
/* under construction !*/
/* under construction !*/
 ...
 *  void
 *****************************************************************************/
void gui_print_truncated_text_ex(
	S32 x, 
	S32 y, 
	S32 xwidth, 
	UI_string_type st)
 ...
 *****************************************************************************/
U8 gui_print_truncated_bordered_text2(
	S32 x, 
	S32 y, 
	S32 xwidth, 
 ...
 *  void
 *****************************************************************************/
void gui_draw_image_with_alpha_transparent(
	U8* img_ptr, 
	S32 x, 
	S32 y, 
	S32 alpha)  
...
//		 h,4, ((gdi_layer_struct*)temp_layer)->buf_ptr,  (PS8)L"E:\\b.bmp");
   ...
   ((gdi_layer_struct*)temp_layer)->offset_y = ((gdi_layer_struct*)temp_layer)->offset_y + gdi_act_layer->offset_y;
    ...
    gui_set_clip_preset(((gdi_layer_struct*)temp_layer)->offset_x, ((gdi_layer_struct*)temp_layer)->offset_y,
       ((gdi_layer_struct*)temp_layer)->offset_x + w, ((gdi_layer_struct*)temp_layer)->offset_y + h);
 ...
 * DESCRIPTION
 *  gui draw resized image with alpha transparent.
 * PARAMETERS
 *  str       [IN]  Text to display
 * RETURNS
 *  void
 *****************************************************************************/
void gui_draw_resized_image_with_alpha_transparent(U8* img_ptr, S32 x, S32 y, S32 width, S32 height, S32 alpha)  
{
	 gdi_handle temp_layer = NULL;
	 S32 tmp_size;
	 U8 *tmp_buf;
     
	 tmp_size = width * height * GDI_COLOR_FORMAT_32;
	 tmp_buf = (U8*) mmi_frm_scrmem_alloc_framebuffer(tmp_size);	 
...
//		 h,4, ((gdi_layer_struct*)temp_layer)->buf_ptr,  (PS8)L"E:\\b.bmp");
   ...
   ((gdi_layer_struct*)temp_layer)->offset_y = ((gdi_layer_struct*)temp_layer)->offset_y + gdi_act_layer->offset_y;
	...
	// gdi_layer_push_and_set_clip
    gui_set_clip_preset(((gdi_layer_struct*)temp_layer)->offset_x, ((gdi_layer_struct*)temp_layer)->offset_y,
        ((gdi_layer_struct*)temp_layer)->offset_x + width, ((gdi_layer_struct*)temp_layer)->offset_y + height);
    gdi_layer_flatten_with_clipping((gdi_handle)gdi_act_layer,temp_layer, 0 , 0);
    gdi_layer_pop_clip();
...
void gui_draw_resized_image_id_with_alpha_transparent(U16 image_id, S32 x, S32 y, S32 width, S32 height, S32 alpha)
{
	gui_draw_resized_image_with_alpha_transparent((U8*)GetImage(image_id),x, y, width, height, alpha);
}
