...
static MMI_BOOL g_srv_bt_cm_bond_req_flag = MMI_FALSE;
...
void srv_bt_cm_antenna_on_req_hdler(void)
{
    ...
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    bt_poweron_req_struct* msg_p = NULL;

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
#if defined( __GEMINI__ ) || defined(__TINY_SYS__)
...
#endif
#ifdef __TINY_SYS__
    msg_p->mode = g_srv_bt_cm_cntx.tiny_switch_on_mode;
    MMI_TRACE(
        MMI_CONN_TRC_G7_BT,
        SRV_BT_CM_ANTENNA_ON_REQ_HDLER,
        msg_p->mode);
...
/* under construction !*/
    ...
    g_srv_bt_cm_cntx.deactivate_flag = ~0;
    if (SRV_BT_CM_GET_FLAG(SRV_BT_CM_MASK_POWER_ON_TIME_OUT))
    {
		...
		srv_bt_cm_active_complete(MMI_FALSE);		   
		#endif
    }
    else
    {
...
static void srv_bt_cm_serial_addr(srv_bt_cm_bt_addr* addr, U8* name)
{
    if (addr && name && !name[0])
    {
        kal_sprintf(name,"%02x:%02x:%02x:%02x:%02x:%02x",
                    (addr->nap&0xff00)>>8, addr->nap&0x00ff,
                    addr->uap, (addr->lap&0xff0000)>>16,
                    (addr->lap&0x00ff00)>>8, (addr->lap&0x0000ff));
    }
}
/*****************************************************************************
 * FUNCTION
 ...
 *  void
 *****************************************************************************/
U8 srv_bt_cm_discovery_result_ind_hdler(void* msg)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    else
    {
       ...
       srv_bt_cm_serial_addr((srv_bt_cm_bt_addr*)&msg_p->bd_addr, dev_name);
    }

    srv_bt_cm_dev_update_name((srv_bt_cm_bt_addr*)&msg_p->bd_addr, dev_name, name_len);
...
U8 srv_bt_cm_discovery_update_ind_hdler(void* msg)
{
	/*----------------------------------------------------------------*/
	/* Local Variables												  */
	/*----------------------------------------------------------------*/
 ...
 *  void
 *****************************************************************************/
U8 srv_bt_cm_discovery_cnf_hdler(void* msg)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
 ...
 *  void
 *****************************************************************************/
U8 srv_bt_cm_cancel_discovery_cnf_hdler(void* msg)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
void srv_bt_cm_read_local_bd_addr_req_hdler(void)
{
    ...
    /*Gatt is ready, post event notify*/
	  srv_bt_cm_post_notify(SRV_BT_CM_EVENT_BLE_ACTIVATE, para_p);
    #endif
...
/* under construction !*/
    ...
    if (msg_p->result == BTBM_ADP_SUCCESS)
    {
		...
		#ifdef __BT_4_0_BLE__
		srv_bt_cm_copy_addr(&g_srv_bt_cm_cntx.host_dev.le_bd_addr, (srv_bt_cm_bt_addr*) &msg_p->le_bd_addr);
		#endif
        switch (g_srv_bt_cm_cntx.state)
        {
            case SRV_BT_CM_STATE_ACTIVATING:
...
void srv_bt_cm_link_connect_accept_ind_hdler(void* msg)
{
    ...
    if ((cm_state != SRV_BT_CM_STATE_IDLE && 
       ...
       #endif
        #if defined( __BT_4_0_BLE__) && defined(__MMI_BT_DIALER_SUPPORT__)
            || srv_bt_cm_up_to_max_link_num()
        #endif
        )
    {
...
void srv_bt_cm_bond_req_hdler(srv_bt_cm_bt_addr bt_add, U8 sdap_len, U16* sdap_uuid)
{
    ...
    g_srv_bt_cm_bond_req_flag = MMI_TRUE;
...
MMI_BOOL srv_bt_cm_addr_is_nul(srv_bt_cm_bt_addr* bd_addr)
{
...
void srv_bt_cm_bond_ind_hdler(void* msg)
{
	...
	// btcmsrv only care EDR
	if (msg_p->device_type != BTBM_DEVICE_TYPE_BR_EDR)
	{
		return;
	}
	// for just work
	if (msg_p->result == BTBM_ADP_AUTO_ACCEPT)
	{
		srv_bt_cm_add_devname_addr( msg_p->bd_addr, NULL, 0, MMI_FALSE);
		if (srv_bt_cm_get_dev_index((const srv_bt_cm_bt_addr *)&(msg_p->bd_addr), SRV_BT_CM_PAIRED_DEV) 
    ...
    if (msg_p->result == BTBM_ADP_SUCCESS)
    {
    	srv_bt_cm_dev_type_enum dev_type = SRV_BT_CM_PAIRED_DEV;
		
        if (srv_bt_cm_addr_is_nul(&g_srv_bt_cm_cntx.temp_dev.bd_addr))
        {
            srv_bt_cm_copy_addr(&g_srv_bt_cm_cntx.temp_dev.bd_addr, &(msg_p->bd_addr));
            if (g_srv_bt_cm_bond_req_flag == MMI_FALSE)
            {
                dev_type = SRV_BT_CM_RECENT_USED_DEV;
            }
        }
        if (msg_p->cod != 0)
        {
        ...
        srv_bt_cm_dev_add_int((const srv_bt_cm_dev_struct*)&g_srv_bt_cm_cntx.temp_dev, dev_type);

        if (dev_type == SRV_BT_CM_RECENT_USED_DEV)
        {
            return;
        }
    }
...
void srv_bt_cm_bond_cnf_hdler(void* msg)
{
	...
	g_srv_bt_cm_bond_req_flag = MMI_FALSE;
...
void srv_bt_cm_security_level_cnf_hdler(void* msg)
{
    ...
    switch (g_srv_bt_cm_cntx.state)
    {
        ...
        #endif
	#if defined(__MMI_WRISTBAND_SUPPORT__) && !defined(__MMI_SPP_SUPPORT__)
...
void srv_bt_cm_write_scanenable_mode_req_hdler(U8 connected_mode)
{
    ...
    msg_p = (bt_bm_write_scanenable_mode_req_struct*)construct_local_para(
        ...
        TD_CTRL);
    #ifdef __BT_LE_ONLY__
    connected_mode = BTBM_ADP_P_OFF_I_OFF;
    #endif
        ...
        switch (g_srv_bt_cm_cntx.state)
        {
            ...
            case SRV_BT_CM_STATE_VISIBILITY_SETTING:
                ...
                switch (g_srv_bt_cm_cntx.tmp_visibility_type)
                {
                    ...
                    #ifdef __MMI_BT_DIALER_SUPPORT__
                        if(MMI_FALSE == srv_bt_cm_get_prevent_connect_flag())
                    #endif
                        {
                            SRV_BT_CM_RESET_FLAG(SRV_BT_CM_MASK_VIS);
                        }
...
{
	bt_bm_link_state_ind_struct* link_state_msg = (	bt_bm_link_state_ind_struct*)msg;

    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
	#ifdef __BT_4_0_BLE__
	srv_bt_cm_link_type_enum link_type = SRV_BT_CM_LINK_TYPE_EDR;
	#endif
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(
        ...
        g_srv_bt_cm_cntx.ram_flag,
        g_srv_bt_cm_cntx.deactivate_flag);
	MMI_TRACE(MMI_CONN_TRC_G7_BT, SRV_BT_CM_LINK_STATE_IND_HDLER_2,
		link_state_msg->result, 
		((link_state_msg->bd_addr.nap & 0xff00) >> 8),
        (link_state_msg->bd_addr.nap & 0x00ff),
        (link_state_msg->bd_addr.uap),
        ((link_state_msg->bd_addr.lap & 0xff0000) >> 16),
        ((link_state_msg->bd_addr.lap & 0x00ff00) >> 8),
        (link_state_msg->bd_addr.lap & 0x0000ff),
        link_state_msg->device_type);
	#ifdef __BT_4_0_BLE__
		if (link_state_msg->device_type == BTBM_DEVICE_TYPE_LE)
		{
			link_type = SRV_BT_CM_LINK_TYPE_LE;
		}
		//srv_bt_cm_set_link_type((srv_bt_cm_bt_addr*)&link_state_msg->bd_addr, link_type, link_state_msg->result);
	#endif
	#ifdef __MMI_BT_DIALER_SUPPORT__
	if ((link_state_msg->result == BTBM_ADP_FAIL_LINK_PAGE_TIMEOUT || 
		link_state_msg->result == BTBM_ADP_FAIL_LINK_CONNECTION_TIMEOUT)
		#ifdef __BT_4_0_BLE__
        || (link_state_msg->device_type == BTBM_DEVICE_TYPE_LE) 
		#endif
		)
	{
		srv_bt_cm_link_sate_ind_struct link_state_ind;
	#ifdef __BT_4_0_BLE__
		link_state_ind.link_type = link_type;
	#endif
		link_state_ind.reason =link_state_msg->result;
		//fail_conn_to.dev_addr = (srv_bt_cm_bt_addr*)&link_state_msg->bd_addr;
		link_state_ind.dev_addr.lap = link_state_msg->bd_addr.lap;
		
		link_state_ind.dev_addr.uap = link_state_msg->bd_addr.uap;

		link_state_ind.dev_addr.nap = link_state_msg->bd_addr.nap;
		MMI_TRACE(MMI_CONN_TRC_G7_BT,
			SRV_BT_CM_LINK_STATE_IND_HDLER_2,
			link_state_ind.reason, 
		    ((link_state_ind.dev_addr.nap & 0xff00) >> 8),
		    (link_state_ind.dev_addr.nap & 0x00ff),
		    (link_state_ind.dev_addr.uap),
		    ((link_state_ind.dev_addr.lap & 0xff0000) >> 16),
		    ((link_state_ind.dev_addr.lap & 0x00ff00) >> 8),
		    (link_state_ind.dev_addr.lap & 0x0000ff),
		link_state_ind.dev_addr.uap);
		srv_bt_cm_notify(SRV_BT_CM_EVENT_LINK_STATE_IND,(void *)&link_state_ind);
	}
	if(link_state_msg->device_type == BTBM_DEVICE_TYPE_BR_EDR)
	{
		  if(link_state_msg->result == 0)
		  {
			  srv_bt_cm_set_dev_busy_state((srv_bt_cm_bt_addr*)&link_state_msg->bd_addr,SRV_BT_CM_NO_CONNECTION,MMI_TRUE);
		  }
		  else
		  {
			  srv_bt_cm_set_dev_busy_state((srv_bt_cm_bt_addr*)&link_state_msg->bd_addr,SRV_BT_CM_NO_CONNECTION,MMI_FALSE);
		  }
