...
void mmi_callset_call_setting_launch(void)
{
	...
	if (!srv_callset_is_available(MMI_SIM1))
...
#ifdef __MMI_WEARABLE_DEVICE__
	cui_menu_set_item_hidden(menu_gid, MENU_ID_CALLSET_ANS_MODE_OPT, MMI_TRUE);
#endif /*__MMI_WEARABLE_DEVICE__*/
...
void mmi_callset_entry_cb_passwd(mmi_scrn_essential_struct *data)
{
	...
	SetDialingPinOkHandler(mmi_callset_cb_passwd_check);
...
void mmi_callset_cb_passwd_check(U8 *text, U8 *cursor, S32 length)
{
...
#else /*__MMI_WEARABLE_DEVICE__*/
	wBuffer = mmi_callset_cb_get_passwd_buf();
	if (kal_wstrlen(wBuffer) < SRV_SS_PASSWORD_MAX_LENTH)
	{
		mmi_popup_display_simple_ext(
    ...
    else
    {
        ...
        switch (mmi_callset_sim_p->cb_state)
        {
            ...
            case 3:
			#ifdef __MMI_WEARABLE_DEVICE__
				mmi_callset_sel_cb_passwd_done();						
			#else  /*__MMI_WEARABLE_DEVICE__*/
			...
			#ifdef __MMI_WEARABLE_DEVICE__
				mmi_callset_sel_cb_passwd_next();						
			#else  /*__MMI_WEARABLE_DEVICE__*/
