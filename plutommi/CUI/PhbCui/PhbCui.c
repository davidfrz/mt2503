...
extern mmi_phb_entry_type g_phb_entry_type;
/*----------------------------------------------------------------*/
/* Local functions                                                */
/*----------------------------------------------------------------*/
...
#elif defined(__MMI_PHB_PBAPC_SYNC_CONTACT__ ) && defined(__MMI_WEARABLE_DEVICE__)
...
mmi_ret mmi_phb_launch_proc(mmi_event_struct *evt)
{
    ...
    switch (evt->evt_id)
    {
        ...
        case EVT_ID_GROUP_DELETE_REQ_IN_END_KEY:
        {
            ...
            is_present = mmi_frm_scrn_is_present(phb_pbapc_cntx_p->load_gid, SCR_ID_PHB_PROCESSING, MMI_FRM_NODE_ALL_FLAG);
            MMI_TRACE(MMI_PHB_TRACE_GROUP, TRC_MMI_PHB_PBAPC_LAUNCH_PROC, is_present, phb_pbapc_cntx_p->bt_index, phb_pbapc_cntx_p->load_gid);
            if (is_present)
            {
                phb_pbapc_cntx_p->is_inbg[phb_pbapc_cntx_p->bt_index] = MMI_TRUE;
        ...
        case EVT_ID_GROUP_DEINIT:
            ...
            #endif
        #endif /* defined(__MMI_BT_PBAP_CLIENT__) */
			g_phb_entry_type = MMI_PHB_ENTRY_NONE;
		#ifdef __MMI_PHONEBOOK_MEM_SLIM__
        ...
        {			
			if(g_phb_entry_type == MMI_PHB_ENTRY_BT)
            {
	            ...
	            if (SCR_ID_PHB_PROCESSING == scrn_id &&
	                GRP_ID_PHB_MAIN == group_id)
	            {
	                return MMI_RET_DONT_CARE;
	            }

	            if ((SCR_ID_PHB_PBAPC_FAIL == scrn_id && GRP_ID_PHB_MAIN == group_id) ||
	                (SCR_ID_PHB_PBAPC_CONNECT == scrn_id && GRP_ID_PHB_MAIN == group_id))
	            {
	                if (phb_pbapc_cntx_p->flag_ext & 0x08)
	                {
	                    phb_pbapc_cntx_p->flag_ext &= ~0x08;
	                    phb_pbapc_cntx_p->flag_ext |= 0x04;
	                }
        ...
        case EVT_ID_TOUCHSCREEN_MOVE_LEFT:
        {
			...
			if(g_phb_entry_type == MMI_PHB_ENTRY_BT)
			{
	            ...
	            if (SCR_ID_PHB_PBAPC_BT == scrn_id &&
	                GRP_ID_PHB_MAIN == group_id &&
	                phb_pbapc_cntx_p->contact_count > 0)
	            {
	                mmi_phb_pbapc_alpha_search(phb_pbapc_cntx_p->parent_id);
	            }
    ...
    #endif /* defined(__MMI_BT_PBAP_CLIENT__) && defined(__MMI_WEARABLE_DEVICE__) */
...
void mmi_phb_launch_common(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
void mmi_phb_launch(void)
{
	...
	g_phb_entry_type = MMI_PHB_ENTRY_LOCAL;
	
	mmi_phb_launch_common();

}


void mmi_phb_launch_bt(void)
{
	...
	g_phb_entry_type = MMI_PHB_ENTRY_BT;
...
MMI_ID mmi_phb_bt_pre_entry_main_screen(void* param, U32 param_size)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
