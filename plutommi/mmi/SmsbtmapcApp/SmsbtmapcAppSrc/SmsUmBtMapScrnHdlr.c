...
void mmi_sms_bt_get_inbox_list_cb(srv_sms_callback_struct* data)
{
    ...
    if(data->result == MMI_TRUE)
    {
            ...
            g_sms_bt_ui_p[g_sms_active_dev_index].curr_entered == SRV_SMS_BTMAPC_BOX_INBOX)
        {
            ...
            kal_prompt_trace(MOD_MMI, "[SMS]Entry mmi_sms_bt_get_inbox_list_cb  userdata->scrn_id = %d",userdata->scrn_id);
            ...
            {
                kal_prompt_trace(MOD_MMI, "[SMS]Entry mmi_sms_bt_get_inbox_list_cb mmi_sms_um_bt_ui_cc_send_event aprc.st_type = %d is_back_folder = %d", scrn_cntx->aprc.st_type , userdata->is_back_folder);
    ...
    {        
            ...
            g_sms_bt_ui_p[g_sms_active_dev_index].curr_entered == SRV_SMS_BTMAPC_BOX_INBOX)
        {
			...
			kal_prompt_trace(MOD_MMI, "[SMS], Entry mmi_sms_bt_get_inbox_list_cb else flow data->cause = %d",data->cause);
...
MMI_BOOL mmi_sms_um_bt_ui_sh_general_folder_fsm_cb(U16 scrn_id, U16 state, U16 status)
{
    ...
    switch (state)
    {
        ...
        case MMI_UM_UI_FSM_EF_CHECK_DATA:
        {
            ...
            if(scrn_cntx->child_scrn_id ==0)
            {
                for(i = 0; i < g_sms_um_ui_p->cc_cntx.num_of_scrn; i++)
                {
                    if((g_sms_um_ui_p->cc_cntx.scrn_table[i].scrn_id != 0) && (g_sms_um_ui_p->cc_cntx.scrn_table[i].aprc.st_type == MMI_SMS_UM_BT_UI_SCRN_TYPE_LOADING))
                        {
                            tmp_scrn_id=g_sms_um_ui_p->cc_cntx.scrn_table[i].scrn_id;
                            break;
                        }
                }
                scrn_cntx->child_scrn_id = tmp_scrn_id;
                kal_prompt_trace(MOD_MMI, "[SMS],Entry cb %d",tmp_scrn_id);
            }
        #endif
            ...
            {
            	 kal_prompt_trace(MOD_MMI, "[SMS],Entry cb 1111");
        ...
        case MMI_UM_UI_FSM_EF_UPDATE_DATA:
        {
			...
			kal_prompt_trace(MOD_MMI, "[SMS],Entry cb MMI_UM_UI_FSM_EF_UPDATE_DATA");
            if (mmi_sms_um_bt_ui_cc_check_scrn_alive(scrn_cntx->child_scrn_id))
            {
            }
            else
...
void mmi_sms_um_bt_ui_sh_general_folder_tab_entry(mmi_scrn_essential_struct* scr_ess)
{
    ...
    if (scrn_cntx->screen_hdlr.is_scrn_displayed == MMI_FALSE || g_sms_bt_ui_p[g_sms_active_dev_index].mmi_need_update_list)
    {
        ...
        if(g_bt_category_error_flag == MMI_FALSE)
        {
            ...
            kal_prompt_trace(MOD_MMI, "[SMS], Entry mmi_sms_um_bt_ui_sh_general_folder_tab_entry num_item = %d scrn_cntx->screen_hdlr.hilite_info.index = %d",num_item, scrn_cntx->screen_hdlr.hilite_info.index);
        }
        #endif
...
void mmi_sms_um_bt_ui_sh_general_folder_hilite_hdlr(S32 index)
{
    ...
    #ifdef __MMI_WEARABLE_DEVICE__ 
    mmi_sms_bt_um_set_sms_context(scrn_cntx->screen_hdlr.hilite_info.msg_id);
    #endif
...
S32 mmi_sms_um_bt_ui_sh_get_async_item(S32 start_index, gui_iconlist_menu_item *menu_data, S32 num_of_item)
{
    ...
    kal_prompt_trace(MOD_MMI, "[SMS],mmi_sms_um_bt_ui_sh_get_async_item num_of_item = %d  start_index = %d", num_of_item, start_index);
...
#else
	/*Subject*/
        subject_str = srv_sms_btmapc_get_msg_content(msg_detail_info[i]->msg_id);	
		msg_detail_info[i]->subject_length = mmi_ucs2strlen((S8*)subject_str);
        ...
        if (copy_len == 0)
        {
                mmi_ucs2ncpy(
                    (S8*) menu_data[i].item_list[1],
                    (S8*) GetString(STR_ID_SRV_BT_EMPTY_SUBJECT),
