...
if($LEVEL == 1) {
  ...
  system("del /q /f make\\~TS_LinkError.tmp > nul")if(-e "make\\~TS_LinkError.tmp");
  ...
  my ($TARGDIR, $LIS_FILE,$BL_LIS_FILE,$BL_EXT_LIS_FILE,$SYS_PATH,$MAKEFILE,$LogFolder,$BIN_FOLDER,$FLASH_CFG,$PROJECT_NAME,$FSUSAGE_LOG,$NVRAMID_LOG,$FEATURE_OVER,$SYSDRV,$NVRAM_PSEUDO,$INSIDE_MTK,$SCATTER_FILE,$TS_LIS_FILE,$TS_SCATTERFILE) = @ARGV;
  ...
  } elsif (-e "make\\~TS_LinkError.tmp"){
    my $TS_LIS_FILE_PTH = "$TARGDIR\\$TS_LIS_FILE";
    &error_handler("File: $TS_LIS_FILE_PTH did NOT exist!", __FILE__, __LINE__) if(!-e $TS_LIS_FILE_PTH);
    &error_handler("Folder: $SYS_PATH did NOT exist!", __FILE__, __LINE__) if (!-d $SYS_PATH);
    &error_handler("File: $MAKEFILE did NOT exist!", __FILE__, __LINE__) if (!-e $MAKEFILE);
    #re-calculate tiny system size
    print "perl tools\\TSReservedSize_AutoConfig.pl $TS_LIS_FILE_PTH $SYS_PATH $MAKEFILE make\\~TS_LinkError.tmp $PROJECT_NAME $AAPMC_LOG $SCATTER_FILE $TS_SCATTERFILE> $LogFolder\\TSReservedSize_AutoConfig.log 2>&1\n" if($DEBUG);
    $result = system("perl tools\\TSReservedSize_AutoConfig.pl $TS_LIS_FILE_PTH $SYS_PATH $MAKEFILE make\\~TS_LinkError.tmp $PROJECT_NAME $AAPMC_LOG $SCATTER_FILE $TS_SCATTERFILE> $LogFolder\\TSReservedSize_AutoConfig.log 2>&1");
    $remd_str = &getRecomdValue("make\\~TS_LinkError.tmp");
    $check_scrip = "TSReservedSize_AutoConfig";
  } else {
  ...
  if(($result>>8) == ERR::NO_MODIFY) { # NO_MODIFY: recommand a value but not modify custom_featureConfig.h || scatter file is checkin by human
    ...
    if(ERR::AAPMCLOG_SUCCESS == &AAPMCLogParser::Open($AAPMC_LOG)) {
        ...
        foreach my $i (@$IndexArray_ref) {
            ...
            my($filename,$href_option) = ($ChangeRecord_ref->[0],$ChangeRecord_ref->[1]);
            my $debug_str;
            
            foreach my $strName (keys %$href_option)
            {
                $strValue = $href_option->{$strName};
                $debug_str .= "\t$strName=$strValue\n";
            }
            print "Status=$Status:\n\t$filename\n$debug_str\n";
        }
    ...
    if(($check_scrip eq "BLReservedSize_AutoConfig") or ($check_scrip eq "TSReservedSize_AutoConfig")){
  ...
  if((($result>>8) == ERR::AAPMCLOG_SUCCESS && $Status != ERR::ERR_MODIFYFAIL) or
    ...
    if(-e "$LogFolder\\auto_adjust_mem.log" && (($check_scrip eq "BLReservedSize_AutoConfig")or($check_scrip eq "TSReservedSize_AutoConfig")) && ($result>>8) == ERR::NO_NEED_TO_UPDATE){
      my $debug_folder = ($check_scrip eq "BLReservedSize_AutoConfig")? "BOOTLOADER_DEBUG":"TINYSYSTEM_DEBUG";
      system("md $TARGDIR\\$debug_folder") if(!-d "$TARGDIR\\$debug_folder");
      print STDERR "copy $LogFolder\\auto_adjust_mem.log $TARGDIR\\$debug_folder\\auto_adjust_mem.log\n";
      system("copy /y $LogFolder\\auto_adjust_mem.log $TARGDIR\\$debug_folder\\auto_adjust_mem.log > nul");
    }
  } else {
...
sub ckTSLinkError
{
  ...
  my $NEED_CHECK = 0;
  my $link_err_str = "";
  print "enter to check link error for tiny system...\n";
  if(-e $TS_LINKLOG){
    open (FILE_HANDLE, $TS_LINKLOG) or error_handler("cannot open $TS_LINKLOG!",__FILE__,__LINE__);
    while(<FILE_HANDLE>){
        if($_ =~ /Error: (L6220E): /){
            print "[LINK ERROR] $_" if($DEBUG);
            $link_err_str .= $_;
    ...
    if($CK_TS_LK_ERR){
        system("md $TARGDIR\\TINYSYSTEM_DEBUG") if(!-d "$TARGDIR\\TINYSYSTEM_DEBUG");
        system("copy /y $TS_LINKLOG $TARGDIR\\TINYSYSTEM_DEBUG\\$tslogname.error > nul");
    }
  }
  if(($CK_TS_LK_ERR) or ((!$CK_TS_LK_ERR) && (!$TS_LK_ERR))){
    $NEED_CHECK = 1;
    print "No link error for tiny system\n" if(!$CK_TS_LK_ERR && $DEBUG);
    open (TEMPLATE, ">make\\~TS_LinkError.tmp") or error_handler("cannot open make\\~TS_LinkError.tmp!", __FILE__,__LINE__);
    print TEMPLATE "$link_err_str";
    close TEMPLATE;
  }
  return $NEED_CHECK;
}
