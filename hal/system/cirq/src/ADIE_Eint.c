...
kal_uint32 ADIE_EINT_Set_Sensitivity_internal(kal_uint8 eintno, kal_bool sens);
#endif
/*====================================================*/
...
kal_uint32 ADIE_EINT_Set_Sensitivity(kal_uint8 eintno, kal_bool sens)
{
...
#if defined(__TINY_SYS__) /* dongming add for tiny sys */
   {
	   ...
	   if(0 != (unknown_eint_from_tiny & (bitMask << (eintno + ADIE_EINT_START_INDEX))))
	   {
		   return;	/* just return */
	   }
   }
...
kal_uint8 ADIE_EINT_Get_Sensitivity(kal_uint8 eintno)
{
	...
	if(eintno > 16)
	{
		return ((*ADIE_EINT_SENS1) >> (eintno - 16)) & (0x01);
	}
	else
	{
		return ((*ADIE_EINT_SENS0) >> (eintno)) & (0x01);
	}
#else 
	return ((*ADIE_EINT_SENS) >> (eintno)) & (0x01);
#endif    

}

kal_uint8 ADIE_EINT_Get_Polarity(kal_uint8 eintno)
{
	return (ADIE_EINTaddr(eintno) >> 11) & 0x1;
...
/* under construction !*/
				#endif
				if(0 != (unknown_eint_from_tiny & (bitmask << index)))
				{
					ADIE_EINT_Set_Sensitivity_internal(EINT_GLB_TO_ADIE(index),(~(unknown_eint_sens_from_tiny >> index)) & 0x01);
					ADIE_EINT_ResetSWRegister(EINT_GLB_TO_ADIE(index));
					unknown_eint_from_tiny &= (~(bitmask << index));
				}
