...
#if defined(__MMI_SCREEN_LOCK_ANY_TIME__) || defined(__MMI_SCREEN_LOCK_ANY_TIME_WDEVICE__)
mmi_ret mmi_audply_single_play_screen_locker_callback_hdlr(mmi_event_struct *event)
{
    MMI_TRACE(MMI_MEDIA_TRC_G2_APP, MMI_AUDPLY_TRC_SINGLE_PLAY_SCREEN_LOCKER_CALLBACK_HANDLER, event->evt_id, single_play_state, __LINE__);
    ...
    switch (event->evt_id)
    {
        case EVT_ID_SCREEN_LOCK_KEY_PRE_LOCK_EVT_ROUTING:
        {
            ...
            else if (single_play_state == STATE_PLAY)
            {
                return MMI_RET_ERR;
            }
            /* single play in file manager */
            else
            {
                return MMI_RET_OK;
            }
