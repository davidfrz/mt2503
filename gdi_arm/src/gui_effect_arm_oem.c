...
extern  U16 g_gui_sse_no_change_height ;
extern  U16 g_gui_sse_no_change_start_y ;
extern  MMI_BOOL g_gui_some_area_sse_no_change ;
extern  kal_uint8 g_gui_screen_no_change_buffer[NO_CHANGE_AREA_TEMP_DATA_BUFFER];
...
static void gui_screen_switch_effect_draw_world_frame(S32 frame_index)
{
...
#ifdef   GUI_SSP_SUPPORT_FOOTER_AREA_NO_CHANGE
            if(g_gui_some_area_sse_no_change) 
            {                   
    ...
    if (g_gui_sse_world.enable_non_blocking_blt)
    {
        ...
        {   
            gdi_layer_create_cf_using_outside_memory(
                                (gdi_color_format)GUI_SCREEN_SWITCH_EFFECT_PIXEL_BYTE_SIZE,
                                0,
                                0,
                                GDI_LCD_WIDTH,
                                g_gui_sse_no_change_height,
                                &layers[z_order[2]],
                                g_gui_screen_no_change_buffer,
                                (S32)(layer_w * layer_h * pixel_size)); 

            gdi_layer_push_and_set_active(layers[i]);

            /* offset */    
            gdi_layer_set_position(0, g_gui_sse_no_change_start_y);
    ...
    {   
        if (layers[z_order[2]])
        {
            gdi_layer_free(layers[z_order[2]]);
        }
