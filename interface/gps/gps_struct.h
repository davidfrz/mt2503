...
{            
    ...
    gps_nmea_utc_time_struct    utc_time;
    gps_nmea_utc_date_struct    utc_date;
} gps_time_sync_to_local_struct;   // MSG_ID_GPS_TIME_SYNC_TO_LOCAL,  struct name MSG_ID_XXX,  then using XXX_struct
typedef struct 
{
    LOCAL_PARA_HDR
...
typedef enum
{
    ...
    GPS_CMD_SET_LOCATION_REPORT_PREIOD_SLEEP_WORK,  
    GPS_CMD_SET_LOCATION_REPORT_PREIOD_WROK_SLEEP,
    GPS_CMD_SET_ASSIST_DATA_FOR_FAST_FIXED,
    GPS_CMD_SET_EPO_DOWNLOAD,
    GPS_CMD_GET_EPO_DOWNLOAD,
	GPS_CMD_SET_AGPS_ON_OFF,
	GPS_CMD_SET_SUPL_PROFILE,
    GPS_CMD_SET_END
} gps_set_param_enum;
...
typedef struct 
{
  kal_uint16      u2YEAR ;             /* > 2000 */
  kal_uint8       u1MONTH;             /* 1~12 */
  kal_uint8       u1DAY;               /* 1~31*/
  kal_uint8       u1HOUR;              /* 0~23*/
  kal_uint8       u1MIN;               /* 0~59*/ 
  kal_uint8       u1SEC;               /* 0~59*/ 
...
typedef struct 
{
	kal_uint16 addr[AGPS_MAX_PROFILE_ADDR_LEN]; // AGPS server addr
	kal_uint32 port; // AGPS server port
} agps_server_cfg_struct;
...
typedef struct
{
	...
	kal_uint8  GPACC[MAX_BDGSA_SENTENCE_LENGTH+1];
...
typedef struct
{
    LOCAL_PARA_HDR
    kal_uint8 md5[32+1];
}apk_gpsadaptor_download_finish_struct;
typedef struct
{
    LOCAL_PARA_HDR
    kal_uint8 epo_file_name[32];
}epo_download_req_struct;
