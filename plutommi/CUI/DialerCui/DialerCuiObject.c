...
mmi_ret cui_dialer_obj_evt_hdlr(mmi_event_struct *event)
{
    ...
    switch (event->evt_id)
    {
        ...
        {			
			kal_prompt_trace(MOD_MMI, "[DIALER]receive EVT_ID_TOUCHSCREEN_MOVE_RIGHT");
			if(!wgui_cat016_move_to_right_flag())
            {
            	kal_prompt_trace(MOD_MMI, "[DIALER] receive EVT_ID_TOUCHSCREEN_MOVE_RIGHT can not exit");
            	ret = MMI_RET_ERR;
			}
            break;
        }
