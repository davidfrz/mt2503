 ...
 *  void
 *****************************************************************************/
mmi_id mmi_clog_appmgr_launch_bt(void* param, U32 param_size)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
static void mmi_clog_pbapc_bt_mydev_highlight_hdlr(S32 idx)
{
    ...
    MMI_TRACE(MMI_CLOG_TRC_INFO, TRC_MMI_CLOG_BT_BD_DEV_HLT,idx, pbapc_cntx->index);
}
...
void mmi_clog_bt_enter_list(void)
{
    ...
    MMI_TRACE(MMI_CLOG_TRC_INFO, TRC_MMI_CLOG_BT_BD_DEV_ENTER_LIST);

    mmi_clog_launch_ex(GRP_ID_CLOG_BEGIN, MMI_CLOG_MAINMENU_DEF_LT_TYPE, MMI_CLOG_MAINMENU_DEF_LT_ID, 0);
}
...
static S32 mmi_clog_pbapc_bt_get_mydev_item(S32 item_index, gui_iconlist_menu_item *menu_data, S32 data_size)
{
    ...
    for (index = 0; index < data_size; index++)
    {
        ...
        MMI_TRACE(MMI_CLOG_TRC_INFO, TRC_MMI_CLOG_BT_BD_DEV_ITEM, ret, idx);           
        if (ret >= 0)
        {
...
void mmi_clog_bt_launch(void)
{
	...
	mmi_clog_launch_ex(GRP_ID_ROOT, MMI_CLOG_MAINMENU_DEF_LT_TYPE, MMI_CLOG_LT_ID_ALL_CALL, 1);
#else
#if SRV_PBAPC_LINK_NUM >= 2
#if defined (__MMI_BT_PBAP_CLIENT__) && defined (__MMI_PHB_PBAPC_SYNC_CONTACT__)
...
#else
	mmi_clog_launch_ex(GRP_ID_ROOT, MMI_CLOG_SENDKEY_DEF_LT_TYPE, MMI_CLOG_SENDKEY_DEF_LT_ID, 1);
#endif
#else
	mmi_clog_launch_ex(GRP_ID_ROOT, MMI_CLOG_MAINMENU_DEF_LT_TYPE, MMI_CLOG_MAINMENU_DEF_LT_ID, 1);
#endif
#endif
...
void mmi_clog_launch(void)
{
	...
	mmi_clog_launch_ex(GRP_ID_ROOT, MMI_CLOG_MAINMENU_DEF_LT_TYPE, MMI_CLOG_MAINMENU_DEF_LT_ID, 0); 
}
...
void mmi_clog_sendkey_launch(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
 	mmi_clog_launch_ex(GRP_ID_ROOT, MMI_CLOG_SENDKEY_DEF_LT_TYPE, MMI_CLOG_SENDKEY_DEF_LT_ID, 0);
}
...
void mmi_clog_dclt_launch(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    mmi_clog_launch_ex(GRP_ID_ROOT, MMI_CLOG_MAINMENU_DEF_LT_TYPE, MMI_CLOG_LT_ID_DIALED_CALL, 0);
}
...
void mmi_clog_mclt_launch(void)
{
    ...
    /*----------------------------------------------------------------*/
#if defined(__MMI_BT_PBAP_CLIENT__)
	if(g_is_missed_call.is_from_bt == MMI_TRUE)
	{
		mmi_clog_launch_ex(GRP_ID_ROOT, MMI_CLOG_MAINMENU_DEF_LT_TYPE, MMI_CLOG_LT_ID_ALL_CALL, 1);
	}
	else
#endif
	{	
		mmi_clog_launch_ex(GRP_ID_ROOT, MMI_CLOG_MAINMENU_DEF_LT_TYPE, MMI_CLOG_LT_ID_MISSED_CALL, 0);
	}
...
void mmi_clog_rclt_launch(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    mmi_clog_launch_ex(GRP_ID_ROOT, MMI_CLOG_MAINMENU_DEF_LT_TYPE, MMI_CLOG_LT_ID_RECVED_CALL, 0);
}
...
void mmi_clog_aclt_launch(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    mmi_clog_launch_ex(GRP_ID_ROOT, MMI_CLOG_MAINMENU_DEF_LT_TYPE, MMI_CLOG_LT_ID_ALL_CALL, 0);
}
...
void mmi_clog_launch_ex(MMI_ID parent_gid, mmi_clog_lt_type_enum lt_type, mmi_clog_lt_id_enum lt_id,MMI_BOOL is_bt)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
static void mmi_clog_evt_mc_hlt_hdlr(cui_menu_event_struct *evt)
{
#if !defined( __MMI_CLOG_FPBW__) && !defined(__MMI_WEARABLE_DEVICE__)
...
#endif /*!defined( __MMI_CLOG_FPBW__) && !defined(__MMI_WEARABLE_DEVICE__)*/
}
                ...
                MMI_MENU_ID sel_menu_id)
{
 ...
 static void mmi_clog_evt_delete_select_hdlr(
                mmi_clog_inst_struct *inst,
                MMI_MENU_ID sel_menu_id)
{
	/*----------------------------------------------------------------*/
	...
	switch (sel_menu_id)
	{
		...
		{		
			inst->sel_menu_id = sel_menu_id;
			clt_menu_id = mmi_clog_menu_id_del2clt(sel_menu_id);
			ret = mmi_clog_get_num_info_by_menu_id(inst, clt_menu_id, &num_info);
			if (!ret)
			{			  
				mmi_clog_popup(STR_GLOBAL_EMPTY, MMI_EVENT_FAILURE, GRP_ID_CLOG_ADVANCED, inst);
				return ;
			}
			if (num_info.num[SRV_CLOG_LOG_NUM_TYPE_TOTAL_LOGS] == 0)
			{ 
...
static MMI_RET mmi_clog_enter_delete_box_proc(mmi_event_struct *evt)
{
	/*----------------------------------------------------------------*/
	/* Local Variables												  */
	/*----------------------------------------------------------------*/
	...
	switch (evt->evt_id)
	{
		...
		case EVT_ID_CUI_MENU_ITEM_SELECT:
			mmi_clog_evt_delete_select_hdlr(inst, mc_evt->highlighted_menu_id);  
			break;
...
void mmi_clog_enter_delete_box(mmi_clog_inst_struct *inst, MMI_ID parent_gid)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /*----------------------------------------------------------------*/
    /* create group */
    mmi_frm_group_create_ex(parent_gid, GRP_ID_CLOG_ADVANCED, mmi_clog_enter_delete_box_proc, inst, MMI_FRM_NODE_SMART_CLOSE_FLAG);
...
static void mmi_clog_enter_sub_items(mmi_clog_inst_struct *inst, MMI_MENU_ID parent_menu_id)
{
    ...
    switch (inst->sel_menu_id)
    {
		...
		case MENU_ID_CLOG_DELETE_BOX:
			mmi_clog_enter_delete_box(inst, inst->self_gid);
#endif /*!defined(__MMI_WEARABLE_DEVICE__) */
            break;
...
#ifdef __MMI_WEARABLE_DEVICE__
	parent_gid = GRP_ID_CLOG_ADVANCED;
#else
	parent_gid = inst->self_gid;
#endif /*__MMI_WEARABLE_DEVICE__*/

    if (ret != SRV_CLOG_RET_OK)
    {
        MMI_CLOG_IT_TRACE((MMI_CLOG_TRC_INFO, TRC_MMI_CLOG_DELETE_ALL_LOGS_RESULT, handle, pid, ret));
        mmi_clog_popup(STR_GLOBAL_UNFINISHED, MMI_EVENT_FAILURE, parent_gid, inst);
    }
#if defined(__MMI_CLOG_FPBW__)
    else
        mmi_clog_popup(STR_GLOBAL_DONE, MMI_EVENT_SUCCESS, parent_gid, inst);        
#endif /*defined(__MMI_CLOG_FPBW__)*/
#endif /* __MMI_TELEPHONY_SUPPORT__ */
    return SRV_CLOG_RET_OK;
}
#endif /* __MMI_CLOG_TAB_SUPPORT__ */
