...
/*************************************************************************
 * Define CSCI data for cold boot
 *************************************************************************/
...
void ADIE_IRQSensitivity_internal(kal_uint8 code, kal_bool edge);
#endif
/************************************************************************/
...
void ADIE_IRQClearInt(kal_uint8 code)
{
...
#if defined(__TINY_SYS__)
	{
		...
		if(0 != (unknown_irq_from_tiny & (bitmask << IRQ_CODE_ADIE_TO_GLB(code))))
		{
			return; /* just return */
		}
	}
...
#if defined(__TINY_SYS__) /* dongming add for tiny system */
{
	...
	tp_irq_code = IRQ_CODE_ADIE_TO_GLB(tp_irq_code);
	#if 0
/* under construction !*/
/* under construction !*/
/* under construction !*/
...
/* under construction !*/
	...
	if(0 != (unknown_irq_from_tiny & (bitmask << tp_irq_code)))
	{
   		ADIE_IRQSensitivity_internal(IRQ_CODE_GLB_TO_ADIE(tp_irq_code),(~(unknown_irq_sens_from_tiny >> tp_irq_code)) & 0x01); /* use "~" ,because API is  opposite with spec*/
		ADIE_IRQ_ResetSWRegister(IRQ_CODE_GLB_TO_ADIE(tp_irq_code));
		unknown_irq_from_tiny &= (~(bitmask << tp_irq_code));
	}
}
#endif
...
kal_uint8 ADIE_IRQ_Get_Sensitivity(kal_uint8 code)
{
	kal_uint8 line;
	line = ADIE_IRQCode2Line[code];
	return ((*ADIE_IRQ_SENS0) >> (line)) & (0x01);
