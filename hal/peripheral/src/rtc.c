...
#if !defined(__FUE__) && !defined(__UBL__)
#if defined(__TINY_SYS__)
CSCI_DEFINE_VAR(kal_uint8, tiny_al);
CSCI_DEFINE_VAR(kal_uint8, tiny_tc);
static kal_uint32 tiny_resume = 0;
#endif
#endif
...
void rtc_set_1_sec_repeat_cali(kal_int32 offset);
...
void RTC_HISR(void)
{
	...
	status = DRV_RTC_Reg(RTC_IRQ_STATUS);
        ...
        if (1 == tiny_al)
        {
            status |= RTC_IRQ_STATUS_AL_STAT;
            tiny_al = 0;
        }
        
        if (1 == tiny_tc)
        {
            status |= RTC_IRQ_STATUS_TC_STAT;
            tiny_tc = 0;
        }
    #endif //#if defined(__TINY_SYS__)
    ...
    #if defined(__TINY_SYS__)
    //Always send time update message if resume from tiny.
    if (1 == tiny_resume)
    {
...
void RTC_setXOSC_(void)
{
    ...
    F32K_CLK_CTRL_MEASURE_EOSC32_FREQ_T measureEosc32Freq;
    ...
    if (!IsXOSC32K.f32k_is_xosc32 && (key1 == RTC_POWERKEY1_KEY) && (key2 == RTC_POWERKEY2_KEY))
    {
        ...
        if (STATUS_OK == DclF32K_Control(f32k_handler, F32K_CLK_CMD_MEASURE_EOSC32_FREQ, (DCL_CTRL_DATA_T *)&measureEosc32Freq))
        {
            rtc_set_1_sec_repeat_cali(32768-measureEosc32Freq.eosc32Freq);
        }
    }
...
void rtc_set_1_sec_repeat_cali(kal_int32 offset)
{
    ...
    reg_value = RTC_CALI_EOSC32_CALI_WEN_EOSC32|(offset&0x3FFF);

    DRV_RTC_WriteReg(RTC_CALI, reg_value);
    RTC_write_trigger();
#endif
}
...
#if defined(DRV_RTC_INTERNAL_32K_AS_6261)
    DRV_RTC_WriteReg(RTC_DIFF, RTC_DIFF_CALI_RD_SEL_NORMAL);
    RTC_write_trigger_wait();
    RTC_reload();
...
void RTC_TCintr(void)
{
	...
	if (cali_state != (kal_uint8)CAL_STATE_NONE)
	{
		...
		RTC_GetTime_(&current_time);
#if !defined(__TINY_SYS__) || defined(TINY_CALIBRATION)
				...
				if (RTC_CALI_PERIOD_MINS <= tc_min_expire_cnt)
				{
					...
					if (inaccurate_frame_tick == 0)
					{
               		...
               		else
               		{
						...
						no_diff_cnt = 0;
                        #if defined(__TINY_SYS__)
                            AP_BtSS_mode_block(SUSPEND, AP_BTSS_MODE_BLOCKER_RTC);
                            AP_BtSS_mode_block(HIBERNATION, AP_BTSS_MODE_BLOCKER_RTC);
                        #endif //#if defined(__TINY_SYS__)
               		}
				}
				else
		...
		if (one_time_cali == KAL_TRUE)
		{
			...
			need_trigger = KAL_TRUE;
		 	if (ret_value == KAL_FALSE)
		 	{
                ...
                #if defined(__TINY_SYS__)
                    AP_BtSS_mode_unblock(SUSPEND, AP_BTSS_MODE_BLOCKER_RTC);
                    AP_BtSS_mode_unblock(HIBERNATION, AP_BTSS_MODE_BLOCKER_RTC);
                #endif //#if defined(__TINY_SYS__)
		 	}
		}
...
#endif //#if !defined(__TINY_SYS__) || defined(TINY_CALIBRATION)
#elif defined(RTC_SW_CALIBRATION)
	t_rtc current_time;
 	kal_uint32 frame_tick;
#if defined(SLEEP_WORKAROUND_IN_RTC)
...
void RTC_PWIC_First_PowerOn_INIT_RTCTime(t_rtc *rtctime)	
{
    ...
    */        
    ...
    #else //#if defined(DRV_RTC_INTERNAL_32K_SWITCH_FAIL_DETECT)
    ...
    #else //#if defined(DRV_RTC_INTERNAL_32K_AS_6255) || defined(DRV_RTC_INTERNAL_32K_AS_6261)
    ...
    if (!(IsXOSC32K.f32k_is_xosc32))    
    {
        ...
        if (STATUS_OK == DclF32K_Control(f32k_handler, F32K_CLK_CMD_MEASURE_EOSC32_FREQ, (DCL_CTRL_DATA_T *)&measureEosc32Freq))
        {
            rtc_set_1_sec_repeat_cali(32768-EOSC32Cali.eosc32_cali_val);
        }
    }    
    DclF32K_Close(f32k_handler);   
    #endif //#if defined(DRV_RTC_INTERNAL_32K_AS_6255) || defined(DRV_RTC_INTERNAL_32K_AS_6261)
...
#if !defined(__FUE__) && !defined(__UBL__)
#if defined(__TINY_SYS__)
int AP_RTC_suspend(AP_BtSS_mode mode)
{
    return 0;
}

int AP_RTC_resume(AP_BtSS_mode mode)
{
    //if (1 == tiny_tc || 1 == tiny_al)
    {
        tiny_resume = 1;
        drv_active_hisr(DRV_RTC_HISR_ID);
    }
