...
static void srv_pbap_oper_success_ind_hdlr(void *ilm_ptr);
static void srv_pbap_continue_ind_hdlr(void *ilm_ptr);
...
void srv_pbap_init(void)
{
    ...
    Ret = (S32)FS_GetAttributes(PBAP_ADP_WORK_FOLDER);
    if (Ret < FS_NO_ERROR)
    {
        /* Return value should be FS_FILE_NOT_FOUND (-9) only, others treat as fatal error */
        Ret = FS_CreateDir((const kal_wchar*)PBAP_ADP_WORK_FOLDER);
    }

    kal_prompt_trace(MOD_MMI_CONN_APP, "[PBAPSRV] srv_pbap_init() FS_Ret = %d", Ret);
...
static void srv_pbap_set_event_hdlr(void)
{
    ...
    SetProtocolEventHandler((PsFuncPtr) srv_pbap_activate_cnf_hdlr, MSG_ID_BT_PBAP_REGISTER_CNF);

    SetProtocolEventHandler((PsFuncPtr) srv_pbap_deactivate_cnf_hdlr, MSG_ID_BT_PBAP_DEREGISTER_CNF);
    ...
    SetProtocolEventHandler((PsFuncPtr)srv_pbap_continue_ind_hdlr, MSG_ID_BT_PBAP_READ_CONTINUE_IND);

    SetProtocolEventHandler((PsFuncPtr)srv_pbap_oper_success_ind_hdlr, MSG_ID_BT_PBAP_OPER_SUCCESS_IND);

    SetProtocolEventHandler((PsFuncPtr) srv_pbap_disconnect_ind_hdlr, MSG_ID_BT_PBAP_DISCONNECT_IND);

    SetProtocolEventHandler((PsFuncPtr) srv_pbap_abort_ind_hdlr, MSG_ID_BT_PBAP_ABORT_IND);
...
static void srv_pbap_activate_cnf_hdlr(void *ilm_ptr)
{
    ...
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    bt_pbap_register_cnf_struct *cnf = (bt_pbap_register_cnf_struct*) ilm_ptr;
...
static void srv_pbap_readentry_ind_hdlr(void *ilm_ptr)
{
    ...
    if (srv_pbap_p->pbap_cntx.pb_storage == PHB_STORAGE_TOTAL) //< 0)
    {
        ...
        // BlueAngel
        memset(rsp->pbap_data_file, 0, sizeof(rsp->pbap_data_file));
        mmi_ucs2cpy((CHAR *)rsp->pbap_data_file, (const CHAR *)PBAP_ADP_ENTRY_FILE);
        rsp->cont = 0;
...
static void srv_pbap_readfolder_ind_hdlr(void *ilm_ptr)
{
    ...
    if ((srv_pbap_p->to_self & 0x01) && srv_pbap_p->pbap_cntx.state == SRV_PBAP_STATE_BUILDING_FOLDER)
    {
        srv_pbap_p->to_self &= ~0x01;
...
*/
    if ((srv_pbap_p->to_self & 0x02) && srv_pbap_p->pbap_cntx.state == SRV_PBAP_STATE_BUILDING_LIST)
    {
        srv_pbap_p->to_self &= ~0x02;
    ...
    {
        /* myler: clear pre list file */
        FS_Delete(PBAP_ADP_LIST_FILE);
...
static void srv_pbap_connect_ind_hdlr(void *ilm_ptr)
{
    ...
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    bt_pbap_client_connect_ind_struct *ind = (bt_pbap_client_connect_ind_struct*) ilm_ptr;
...
static void srv_pbap_connect_rsp_hdlr(U8 result)
{
    ...
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    bt_pbap_client_connect_rsp_struct *rsp =
        (bt_pbap_client_connect_rsp_struct*) OslConstructDataPtr(sizeof(bt_pbap_client_connect_rsp_struct));
...
static void srv_pbap_auth_ind_hdlr(void *ilm_ptr)
{
    ...
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    bt_pbap_client_auth_challenge_ind_struct *ind = (bt_pbap_client_auth_challenge_ind_struct*) ilm_ptr;
...
/*****************************************************************************
 * FUNCTION
 *  srv_pbap_oper_success_ind_hdlr
 * DESCRIPTION
 *  auth indication hdlr
 * PARAMETERS
 *  
...
static void srv_pbap_authentication_rsp(U8 *password)
{
    ...
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    bt_pbap_client_auth_challenge_rsp_struct * rsp;
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/

    rsp = (bt_pbap_client_auth_challenge_rsp_struct*)OslConstructDataPtr(sizeof(bt_pbap_client_auth_challenge_rsp_struct));
    ...
    srv_pbap_send_msg(MSG_ID_BT_PBAP_CLIENT_AUTH_CHALLENGE_RSP, rsp); 
}
...
/* under construction !*/
        #endif
        {
            ...
            // BlueAngel
            memset(rsp->pbap_data_file, 0, sizeof(rsp->pbap_data_file));
            mmi_ucs2cpy((CHAR *)rsp->pbap_data_file, (const CHAR *)PBAP_ADP_FOLDER_FILE);
            rsp->cont = 0;
            srv_pbap_send_msg(MSG_ID_BT_PBAP_READ_FOLDER_RSP, rsp);    
        }
    }
...
static void srv_pabp_provide_list(void)
{
        ...
        srv_pbap_p->pbap_cntx.vcard_listing.listStartOffset == 0)
    {
        ...
        if (PBAP_SEARCH_ATTRIB_NAME == srv_pbap_p->pbap_cntx.vcard_listing.searchAttribute)
        {
            srv_phb_get_index_array(
                ...
                (U16 *)(srv_pbap_p->pbap_cntx.vcard_listing.searchValue),
                srv_pbap_p->pbap_cntx.vcard_listing.searchValueLength);
        }
        else
        {
...
//in use, not found ,run
    ...
    else
    {
        ...
        else
        {
            ...
            // BlueAngel
            memset(rsp->pbap_data_file, 0, sizeof(rsp->pbap_data_file));
            mmi_ucs2cpy((CHAR *)rsp->pbap_data_file, (const CHAR *)PBAP_ADP_LIST_FILE);
            rsp->cont = 0;
            srv_pbap_send_msg(MSG_ID_BT_PBAP_READ_LIST_RSP, rsp);    
        }
    }
