...
LCD_STATUS_CODE_ENUM config_lcd_layer_window(kal_uint8 lcd_layer, lcd_layer_struct *layer_data)
{
    ...
    g_layer_config[lcd_layer].opacity = layer_data->opacity_value * 255 / LCD_MAX_OPACITY;
#else 
    g_layer_config[lcd_layer].opacity = layer_data->opacity_value << 3;
#endif 
...
LCD_STATUS_CODE_ENUM lcd_fb_update(lcd_frame_update_struct *lcd_para)
{
    ...
    // 	for (i=0; i<GDI_LCD->act_height; i++)
    // 	{
    // 		memcpy (&pTempBuffer[i*GDI_LCD->act_width*4], gdi_w32_lcd_buffer[lcd_para->lcd_id - 1] + (GDI_LCD->act_height - 1 - i) * GDI_LCD->act_width, GDI_LCD->act_width*4);
    // 	}
    lcd_process_roi(g_layer_config, &roi_config);
    // 	for (i=0; i<GDI_LCD->act_height; i++)
    // 	{
    // 		memcpy (gdi_w32_lcd_buffer[lcd_para->lcd_id - 1] + (GDI_LCD->act_height - 1 - i) * GDI_LCD->act_width, &pTempBuffer[i*GDI_LCD->act_width*4], GDI_LCD->act_width*4);
    // 	}
    ...
    if (0)  // For debug
        dump(gdi_w32_lcd_buffer[lcd_para->lcd_id - 1], GDI_LCD->act_width, GDI_LCD->act_height, 4, NULL);
...
LCD_STATUS_CODE_ENUM lcd_fb_update_to_memory(lcd_frame_update_to_mem_struct *para)
{
    ...
    case LCD_WMEM_ARGB8888_WITH_PIXEL_ALPHA:
    ...
    lcd_process_roi(g_layer_config, &roi_config);
    if (0)  // For debug
        dump(roi_config.w2m_add, para->dest_block_width, roi_config.size.y, bpp, NULL);
