...
//    TRC_MSG(TRC_MMI_FRM_HIST_GET_CUR_SEL_PAGE, "[MMI_FRM][Scenario][History] mmi_frm_get_cur_sel_page <cur_sel_index = %d>")
   ...
   /*****************************************************************************************************************
    ...
    /* UPnP */
    TRC_MSG(MMI_UPNP_ERROR_LINE, "[UPnP] ERROR return error %d, line: %d")
    TRC_MSG(MMI_UPNP_GW_INIT, "[UPnP] enter upnp_gw_init()")
    TRC_MSG(MMI_UPNP_GW_BIND_SOC, "[UPnP] udp %d bind 1900 %d")
    TRC_MSG(MMI_UPNP_GW_READ_SEARCH_LEN, "[UPnP] recieve search res len %d")
    TRC_MSG(MMI_UPNP_GW_SEND_SEARCH_LEN, "[UPnP] send search msg len: %d, count: %d")
    TRC_MSG(MMI_UPNP_PARSE_SEARCH, "[UPnP] enter upnp_parse_search_data()")
    TRC_MSG(MMI_UPNP_SEND_DEACRIPTION, "[UPnP] enter upnp_udp_send_description_req()")
    TRC_MSG(MMI_UPNP_PARSE_DEACRIPTION, "[UPnP] enter upnp_parse_description()")
    TRC_MSG(MMI_UPNP_DEACRIPTION_E1, "[UPnP] NO InternetGatewayDevice:1")
    TRC_MSG(MMI_UPNP_DEACRIPTION_E2, "[UPnP] NO WANDevice:1")
    TRC_MSG(MMI_UPNP_DEACRIPTION_E3, "[UPnP] NO WANConnectionDevice:1")
    TRC_MSG(MMI_UPNP_PARSE_CONTROL_URL, "[UPnP] controlURL lenth %d")
    TRC_MSG(MMI_UPNP_IP_ENTER, "[UPnP] enter upnp_tcp_send_IP_req()")
    TRC_MSG(MMI_UPNP_EXCHANGE_ENTER, "[UPnP] enter upnp_tcp_exchange()")
    TRC_MSG(MMI_UPNP_EXCHANGE_WRITE, "[UPnP] upnp_tcp_exchange(SOC_WRITE)")
    TRC_MSG(MMI_UPNP_EXCHANGE_CLOSE, "[UPnP] upnp_tcp_exchange(SOC_CLOSE)")
    TRC_MSG(MMI_UPNP_EXCHANGE_READ, "[UPnP] upnp_tcp_exchange(SOC_READ)total_read = %d")
    TRC_MSG(MMI_UPNP_MMI_MAPPING_ENTER, "[UPnP] enter mmi_frm_mapping_ports()")
    TRC_MSG(MMI_UPNP_MAPPING_ENTER, "[UPnP] enter upnp_map_ports()")
    TRC_MSG(MMI_UPNP_MMI_GET_IP, "[UPnP] enter mmi_frm_get_ext_ip()")
    TRC_MSG(MMI_UPNP_GET_IP_ENTER, "[UPnP] enter upnp_get_ext_ip()")
    TRC_MSG(MMI_UPNP_MMI_DEFAULT_CB, "[UPnP] enter mmi_frm_upnp_default_cb()")
    TRC_MSG(MMI_UPNP_INVOKE_MAPPING_CB, "[UPnP] invoke cb, mapping success")
    TRC_MSG(MMI_UPNP_CREATE_SOC_ERROR, "[UPnP] send map create soc error %d, line:%d")
    TRC_MSG(MMI_UPNP_THREAD_MAPPING_ENTER, "[UPnP] enter sub_thread_map_ports_proc()")
    TRC_MSG(MMI_UPNP_THREAD_IP_ENTER, "[UPnP] enter sub_thread_get_ip_proc()")
    TRC_MSG(MMI_UPNP_GET_IP_INFO, "[UPnP] get IP info result:%d, line:%d")
    TRC_MSG(MMI_UPNP_TRY_MAPPING, "[UPnP] try to mapping %d->%d")
    ...
    */
    ...
    /*Bridge*/
    TRC_MSG(APP_BRIDGE_ALLOCATE_HEADER, "[Bridge Mem]bridgeItemPtr->firstPeerDataPtr: %x")
    TRC_MSG(APP_BRIDGE_ALLOCATE_BUFFER_FULL, "[Bridge Mem]allocate queue full")
    TRC_MSG(APP_BRIDGE_ALLOCATE_BUFFER_NEXT, "[Bridge Mem]data_ptr = bridgeItemPtr->lastPeerDataPtr: %x")
    TRC_MSG(APP_BRIDGE_RELEASE_BUFFER, "[Bridge Mem]after release firstPeerDataPtr: %x")
    TRC_MSG(APP_BRIDGE_RELEASE_EMPTY, "[Bridge Mem]release queue empty")
    TRC_MSG(APP_BRIDGE_OPEN_UART, "[BRIDGE Open]open uart may fail because g_handle[port] is not null  port: %d")
    TRC_MSG(APP_BRIDGE_OPEN_FAIL_UART, "[BRIDGE Open] openPort() fail   status: %d")
    TRC_MSG(APP_BRIDGE_OPEN_SUCCESS_UART, "[BRIDGE Open] openPort() success")
    TRC_MSG(APP_BRIDGE_OPEN_DETAIL_UART, "[BRIDGE Open] openUart() port: %d   band:  %d")
    TRC_MSG(APP_BRIDGE_OPEN_SUCCESS_FIRST_STEP_UART, "[BRIDGE Open] openUartFirstStep() success")
    TRC_MSG(APP_BRIDGE_OPEN_USB_PORT, "[BRIDGE Open] openUsbPort() port: %d ")
    TRC_MSG(APP_BRIDGE_GET_SOCKET_PROPERITIES, "[BRIDGE Open] setConnectSocketFd() soc_getsockopt  option: %d Error: %d")
    TRC_MSG(APP_BRIDGE_SET_SOCKET_PROPERITIES, "[BRIDGE Open] setConnectSocketFd() soc_setsockopt option: %d Error: %d")
    TRC_MSG(APP_BRIDGE_GET_PORT_ID, "[BRIDGE Open] getPipeID() sub_detail_id %d")
    TRC_MSG(APP_BRIDGE_GET_PORT_INVALIDATE, "[BRIDGE Open] getPipeID() invalidate sub_detail_id")
    TRC_MSG(APP_BRIDGE_GET_PORT_DETAIL_ID, "[BRIDGE Open] getPipeID() detail_id %d")
    TRC_MSG(APP_BRIDGE_INIT_SOCK_TO_SOCK,"[BRIDGE Open] SOCKET_PIPE_TYPE => SOCKET_PIPE_TYPE init()")
    TRC_MSG(APP_BRIDGE_INIT_SOCK_TO_UART,"[BRIDGE Open] SOCKET_PIPE_TYPE => UART_PIPE_TYPE init()")
    TRC_MSG(APP_BRIDGE_INIT_SOCK_TO_USB,"[BRIDGE Open] SOCKET_PIPE_TYPE => USB_PIPE_TYPE init()")
    TRC_MSG(APP_BRIDGE_INIT_UART_TO_SOCKET,"[BRIDGE Open] UART_PIPE_TYPE => SOCKET_PIPE_TYPE init()")
    TRC_MSG(APP_BRIDGE_INIT_USB_TO_SOCKET,"[BRIDGE Open] USB_PIPE_TYPE => SOCKET_PIPE_TYPE init()")
    TRC_MSG(APP_BRIDGE_INIT_INSERT_NODE_DIRECT,"[BRIDGE Open] insertPipeTable() insert new node")
    TRC_MSG(APP_BRIDGE_INIT_INSERT_NODE_INDIRECT,"[BRIDGE Open] insertPipeTable() change node  searchIndex   %d   %d   %d") 
    TRC_MSG(APP_BRIDGE_SOCKET_INIT, "[BRIDGE Open] setBridgeConnectFd:  %d")
    TRC_MSG(APP_BRIDGE_CREATE, "[BRIDGE Open] createBridge before init() success")
    TRC_MSG(APP_BRIDGE_CREATE_FAIL, "[BRIDGE Open] initEachPeerInfo fail")
    TRC_MSG(APP_BRIDGE_CREATE_INSERT_ITEM_FAIL, "[BRIDGE Open] insert item fail")
    TRC_MSG(APP_BRIDGE_SEARCH, "[BRIDGE Search] searchSelfInBridge() table bridge index %d")
    TRC_MSG(APP_BRIDGE_SEARCH_FAIL, "[BRIDGE Search] searchSelfInBridge() fail")
    TRC_MSG(APP_BRIDGE_ALLOCATE_DATA, "[BRIDGE Mem] after allocate_bridge_node_data() dataFLCPtr: %x")
    TRC_MSG(APP_BRIDGE_ALLOCATE_CALC, "[BRIDGE Mem] insertBridgeInfo() total_recv_len: %d, already_send: %d, nodeCount: %d, leftDataLen:  %d")
    TRC_MSG(APP_BRIDGE_FROM_UART, "[BRIDGE LOG] recvDataFromPortByBridge() start actSelfType :%d")
    TRC_MSG(APP_BRIDGE_FROM_UART_LOGICAL, "[BRIDGE LOG] sendDataFromSocByBridge() UART_PIPE_TYPE No dataFLCPtr writeUartByByte() sendLen: %d, totalLen: %d, type: %d")
    TRC_MSG(APP_BRIDGE_FROM_UART_LOGICAL_LEFT, "[BRIDGE LOG] sendDataFromSocByBridge() UART_PIPE_TYPE has dataFLCPtr send data")
    TRC_MSG(APP_BRIDGE_CLOSE_TIMER, "[BRIDGE LOG] close timer start")
    TRC_MSG(APP_BRIDGE_START_TIMER, "[BRIDGE LOG] iot_bridge_start_timer()")
    TRC_MSG(APP_BRIDGE_SEND_TO_SOC_FAIL, "[BRIDGE LOG] socket send fail")
    TRC_MSG(APP_BRIDGE_SEND_DATA_BY_TIMER, "[BRIDGE LOG] send data by timer start")
    TRC_MSG(APP_BRIDGE_TIMER_CALLBACK, "[BRIDGE LOG] iot_timer_send_soc_cb() peerIndex: %d")
    TRC_MSG(APP_BRIDGE_CLOSE, "[BRIDGE LOG] iot_timer_send_soc_cb() close bridge")
    TRC_MSG(APP_BRIDGE_INVALIDATE_SOCKET,"[BRIDGE LOG] iot_timer_send_soc_cb() invalidate socket")
    TRC_MSG(APP_BRIDGE_NO_LEFT_DATA_SEND,"[BRIDGE LOG] sendLeftDataByBridgeInternal() has no data to send")
    TRC_MSG(APP_BRIDGE_LEFT_SEND_DATA,"[BRIDGE LOG] peer_node_ptr actPeerVal: %d, dataFLCPtr->totalDataLen: %d,  dataFLCPtr->sendDataLen: %d")
    TRC_MSG(APP_BRIDGE_LEFT_TO_BRIDGE, "[BRIDGE LOG] sendLeftDataByBridgeInternal() type: %d, need_send_data_len : %d, send_data_len: %d,  send_data  %s")
    TRC_MSG(APP_BRIDGE_RELEASE_SOCKET_BUFFER, "[BRIDGE LOG] iot_bridge_soc_free_mem headPtr: %x")
    TRC_MSG(APP_BRIDGE_RELEASE_PEER_NODE, "[BRIDGE LOG] release_bridge_node_data() peer_node_ptr: %x")
    TRC_MSG(APP_BRIDGE_NEED_NEXT_SEND, "[BRIDGE LOG] sendLeftDataByBridgeInternal() can't send all left data already send: %d")
    TRC_MSG(APP_BRIDGE_SEND_LEFT_DATA_START, "[BRIDGE LOG] sendLeftDataByBridge() start")
    TRC_MSG(APP_BRIDGE_RECV_DCL_DATA_INTERNAL, "[BRIDGE LOG] FROM_DCL_TO_Wifi recv total_len:  %d  dcl_real_len: %d  need_cnt_recv_len:  %d")
    TRC_MSG(APP_BRIDGE_DCL_RECV_NODE_BUFFER_FULL, "[BRIDGE LOG] dcl recv node buffer full")
    TRC_MSG(APP_BRIDGE_DCL_READY_TO_WRITE, "[BRIDGE LOG] MSG_ID_UART_READY_TO_WRITE_IND")
    TRC_MSG(APP_BRIDGE_RECV_DCL_DATA, "[BRIDGE LOG] MSG_ID_UART_READY_TO_READ_IND  uart port:  %d")
    TRC_MSG(APP_BRIDGE_SEND_UART_DATA, "[BRIDGE LOG] APP_BRIDGE_FROM_SOC_TO_DCL recv_len:   %d")
    TRC_MSG(APP_BRIDGE_AFTER_SEND_UART_DATA, "[BRIDGE LOG] APP_BRIDGE_FROM_SOC_TO_DCL send_len: %d")
    TRC_MSG(APP_BRIDGE_CLOSE_FLAG_SET, "close flag set, bridge_info_ptr: %d")
    TRC_MSG(APP_BRIDGE_SOC_MSG_TO_SOC, "soc_send_msg_to_soc()")
    TRC_MSG(APP_BRIDGE_START_TIMER_INVOKE, "timer_ptr->timer_id == SOC_BRIDGE_TIMER, invoke_id = %x")
    TRC_MSG(APP_BRIDGE_SEND_LEFT_DATA_END, "[BRIDGE LOG] sendLeftDataByBridge() end")
    TRC_MSG(APP_BRIDGE_SEND_DCL_DATA, "[BRIDGE LOG] send DCL_To_Wifi send len: %d  total_len: %d")
    TRC_MSG(APP_BRIDGE_FREE_NODE_START, "[BRIDGE LOG] freeBridgeItemNode() start  actSelfType:%d, actSelfPort:%d")
    TRC_MSG(APP_BRIDGE_CLOSE_PORT_START,"[BRIDGE Close] close port start closed actSelfPort: %d")
    TRC_MSG(APP_BRIDGE_CLOSE_PORT_END,"[BRIDGE Close] freeBridgeItemNode() end")
    TRC_MSG(APP_BRIDGE_CLOSE_BRIDGE_START,"[BRIDGE Close] closeBridge() start")
    TRC_MSG(APP_BRIDGE_CLOSE_BRIDGE_END,"[BRIDGE Close] closeBridge() end")
    TRC_MSG(APP_BRIDGE_USER_CLOSE_BRIDGE_INVALIDATE, "[BRIDGE Close] user closeBridge() invalidate")
    TRC_MSG(APP_BRIDGE_USER_CLOSE_BRIDGE_SUCCESS, "[BRIDGE Close] user closeBridge() success")
    TRC_MSG(APP_BRIDGE_CLOSE_UNINITIZE, "[BRIDGE Close] uninitiliaze item")
    TRC_MSG(APP_BRIDGE_FREE_SOC_MEM, "[BRIDGE LOG] free_soc_mem() MSG_ID_SOC_TCPIP_SRPDU_RELEASE_RES so->so_type: %d soc_id: %d data_len: %d")
    TRC_MSG(APP_BRIDGE_FREE_SOC_MEM_START, "[BRIDGE LOG] free_soc_mem() MSG_ID_SOC_TCPIP_SRPDU_RELEASE_RES start")
    TRC_MSG(APP_BRIDGE_FREE_SOC_MEM_END, "[BRIDGE LOG] free_soc_mem() MSG_ID_SOC_TCPIP_SRPDU_RELEASE_RES end")
    TRC_MSG(APP_BRIDGE_SOC_SEND_TO_UART, "[BRIDGE LOG] soc_tcpip_receive_ind_hdlr() totalLen: %d")

    /*Command processor*/
    TRC_MSG(APP_FRM_CMD_PROCESSOR_ALLOC_MEM_FAILED, "[CommandProcess.c] app_frm_write_uart() alloc asm memory failed, size = %d")
    TRC_MSG(APP_FRM_CMD_PROCESSOR_REPLY_START, "[CommandProcess.c] app_frm_command_reply() start.")
    TRC_MSG(APP_FRM_CMD_PROCESSOR_REPLY_NO_DATA, "[CommandProcess.c] app_frm_command_reply(): buffer size is 0.")
    TRC_MSG(APP_FRM_CMD_PROCESSOR_WRITE_UART, "[CommandProcess.c] app_frm_write_uart().")
    TRC_MSG(APP_FRM_CMD_PROCESSOR_REPLY_END, "[CommandProcess.c] app_frm_command_reply() end.")
    TRC_MSG(APP_FRM_CMD_PROCESSOR_OPEN_PORT_START, "[CommandProcess.c] app_frm_open_command_port() start.")
    TRC_MSG(APP_FRM_CMD_PROCESSOR_PORT_OPENED_IN_STATIC_TBL, "[CommandProcess.c] Open port failed, port=%d has already been opened in static table.")
    TRC_MSG(APP_FRM_CMD_PROCESSOR_PORT_OPENED_IN_DYNAMIC_TBL, "[CommandProcess.c] Open port failed, port=%d has already been opened in dynamic table.")
    TRC_MSG(APP_FRM_CMD_PROCESSOR_NO_ENOUGH_SPACE_FOR_PORT, "[CommandProcess.c] Open port failed, no enough space to store, port=%d")
    TRC_MSG(APP_FRM_CMD_PROCESSOR_OPEN_PORT_END, "[CommandProcess.c] app_frm_open_command_port() end.")
    TRC_MSG(APP_FRM_CMD_PROCESSOR_CLOSE_PORT_START, "[CommandProcess.c] app_frm_close_command_port() start.")
    TRC_MSG(APP_FRM_CMD_PROCESSOR_CLOSE_PORT_END, "[CommandProcess.c] app_frm_close_command_port() end.")
    TRC_MSG(APP_FRM_CMD_PROCESSOR_CLOSE_WRONG_PORT, "[CommandProcess.c] Warning: port = %d had not been opened.")
    TRC_MSG(APP_FRM_CMD_PROCESSOR_CHECK_FORMAT_START, "[CommandProcess.c] app_frm_check_mmi_cmd_format(): cmd_string: %s.")
    TRC_MSG(APP_FRM_CMD_PROCESSOR_CHECK_NO_PREFIX, "[CommandProcess.c] app_frm_check_mmi_cmd_format(): can not find string AT+[.")
    TRC_MSG(APP_FRM_CMD_PROCESSOR_CHECK_NO_POSTFIX, "[CommandProcess.c] app_frm_check_mmi_cmd_format(): can not find string ].")
    TRC_MSG(APP_FRM_CMD_PROCESSOR_CHECK_PORT_LENGTH, "[CommandProcess.c] app_frm_check_mmi_cmd_format(): port_len = %d.")
    TRC_MSG(APP_FRM_CMD_PROCESSOR_CHECK_PORT_LENGTH_INVALID, "[CommandProcess.c] app_frm_check_mmi_cmd_format(): Port length invalid.")
    TRC_MSG(APP_FRM_CMD_PROCESSOR_CHECK_CR_AND_LF, "[CommandProcess.c] app_frm_check_mmi_cmd_format(): identify sybmol CR or LF = %d.")
    TRC_MSG(APP_FRM_CMD_PROCESSOR_CHECK_NOT_APP_COMMAND, "[CommandProcess.c] app_frm_at_command_process(): not a APP command.")
    TRC_MSG(APP_FRM_CMD_PROCESSOR_GET_PORT_NUMBER, "[CommandProcess.c] app_frm_at_command_process(): port = %d.")
    TRC_MSG(APP_FRM_CMD_PROCESSOR_FIND_IN_STATIC_TBL, "[CommandProcess.c] app_frm_at_command_process() find cmd port in static table.")
    TRC_MSG(APP_FRM_CMD_PROCESSOR_CTRL_BUFFER_NOT_ENOUGH, "[CommandProcess.c] app_frm_at_command_process() control buffer is not enough, size = %d.")
    TRC_MSG(APP_FRM_CMD_PROCESSOR_FIND_IN_DYNAMIC_TBL, "[CommandProcess.c] app_frm_at_command_process() find cmd port in dynamic table.")
    TRC_MSG(APP_FRM_CMD_PROCESSOR_IDENTIFY_WRONG_PORT, "[CommandProcess.c] app_frm_at_command_process() can't identify the command port = %d.")
