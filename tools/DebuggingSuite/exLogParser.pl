...
#*
#* 10 01 2013 ken.lin
#* [MAUI_03449161] [SST][ChangeFeature] Enhance the stack dump display in exLogParser.pl
#* .
#* 
#* 08 31 2012 ken.lin
#* [MAUI_03230501] [SST][NewFeature] exception log parser and add comments as reminder in structure definition
#* .
#* 
#* 08 31 2012 ken.lin
...
$ex_log .= " - boot_mode = $boot_mode_type[$boot_mode]\n";
...
sub parse_CtrlBuffInfo
{
    ...
    if ($code1 == 0x823) {
        ...
        $ex_log .= sprintf("      - ex_his_line = 0x%08X", $ex_his_line);
        if ($ex_his_count != 0 && $is_dbginfo_exist == 1) {
            $ex_log .= " -> " . convert2FuncName($ex_his_line) . "( file: " . convert2FileName($ex_his_line) . ")";
        }
        $ex_log .= "\n";
