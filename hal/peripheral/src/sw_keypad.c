...
kal_uint32 sw_kbd_polling_cnt 		= 0;
kal_uint32 sw_kbd_init_flag			= 0;
...
volatile kal_uint16 sw_kbd_stop_polling_timer_flag = 0;
...
void SW_KBD_EINT_HISR(void)
{
 ...
 /*test for performace */
    ...
    if(polarity) /*key press state*/ 
    {
		...
		sw_kbd_stop_polling_timer_flag = 1;
    }
    else          /*key release state*/
	{
        EINT_Set_Polarity(SW_KBD_EINT_NO, KAL_FALSE);
		sw_kbd_stop_polling_timer_flag = 0;
	}
...
#ifdef SW_KBD_DEBUG	
	drv_trace1(TRACE_GROUP_8, KBD_SW_KBD_polarity,	polarity);
#endif  

    SW_KBD_GET_KEY_STATUS(&sw_kbd_temp_key_status);

#ifdef SW_KBD_DEBUG
		drv_trace4(TRACE_GROUP_8, SW_KBD_KEY_STATUS, sw_kbd_temp_key_status.reg1, sw_kbd_temp_key_status.reg2,sw_kbd_temp_key_status.reg3, 0);
#endif
...
void Sw_kbd_polling_timer_cb(void *parameter)
{
...
#ifdef  SW_KBD_DEBUG
 	sw_kbd_polling_cnt++;
  	drv_trace1(TRACE_GROUP_8, KBD_SW_KBD_POLLING_CNT, sw_kbd_polling_cnt);
	drv_trace4(TRACE_GROUP_8, SW_KBD_POLLING_KEY_STATUS, sw_kbd_temp_key_status.reg1, sw_kbd_temp_key_status.reg2,sw_kbd_temp_key_status.reg3,0);
#endif

	//in release state,if first get status in hisr is  not all oxffffffff, the polling timer will be on,
	//but when in polling timer get the status is all 0xfffff,the polling timer didn't close.
	//so ,if we do not get any key,and in release state,stop polling and debouce timer.
	//if polarity = 0, sw_kbd_stop_polling_timer_flag = 0; else =1
	if( (sw_kbd_temp_key_status.reg1 ==0xffffffff) &&
		(sw_kbd_temp_key_status.reg2 ==0xffffffff) &&
		(sw_kbd_temp_key_status.reg3 ==0xffffffff) &&
		(sw_kbd_stop_polling_timer_flag == 0) )
	{  
		...
		sw_kbd_current_key_status.reg3 = sw_kbd_temp_key_status.reg3;
#ifdef SW_KBD_DEBUG
		drv_trace1(TRACE_GROUP_8, KBD_SW_KBD_STOP_POLLING_IN_POLLING_TIMER,  0);
		sw_kbd_polling_cnt = 0;
#endif
		return;	
	}
...
void sw_kbd_init(void)
{
	...
	//original eint debouce timer is 5ms,but in 2502C project ,
	//there is a risk that cannot get in S mode seldom,so modify to 16ms,
	//16ms is a HW debouce time
	EINT_Set_HW_Debounce(SW_KBD_EINT_NO, 16);/*eint debounce 16ms*/
...
#ifdef SW_KBD_DEBUG
	sw_kbd_init_flag++;
	drv_trace1(TRACE_GROUP_8, KBD_SW_KBD_INIT,  sw_kbd_init_flag);
	drv_trace1(TRACE_GROUP_8, KBD_SW_KBD_INIT,  ust_get_current_time());
#endif
