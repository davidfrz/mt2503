...
#if defined(MT6235) || defined(MT6235B) || defined(MT6268) || defined(MT6253) || defined(MT6236) || defined(MT6236B) || defined(MT6256) || defined(MT6251) || defined(MT6253E) || defined(MT6253L)|| defined(MT6252) || defined(MT6252H)
#define DSP_IDLE                     0
...
static void jamrWriteSpeechMode( Media_Handle_Internal *handle, uint8 bCodecMode, uint8 bTxType )
{
   ...
   if( handle->mh.state == AMR_STATE_WB_PLAY ) {
      *DSP_SD2_AWB_VM_0 = (bCodecMode << 8) | bTxType; /* codec mode | Tx-Type */
   } else
#endif
   {
      *DSP_SD2_VM_17 = bCodecMode;
...
static void jamrWriteSpeechData( Media_Handle_Internal *handle, uint16 *data, uint32 len )
{
   ...
   if( handle->mh.state == AMR_STATE_WB_PLAY ) {
      I = 0;
      dspAddr = DSP_SD2_AWB_VM_1;
      if( len > 17 ) {
         for(; I < 17; I++ )
            *dspAddr++ = *data++;
         dspAddr = DSP_SD2_AWB_VM_18;
      }
      for(; I< len; I++)
         *dspAddr++ = *data++;
   } else
...
#if defined(AMRWB_DECODE) && ( defined(MT6235) || defined(MT6235B) || defined(MT6268) || defined(MT6253) || defined(MT6236) || defined(MT6236B) || defined(MT6256) )
   if( handle->mh.state == AMR_STATE_WB_PLAY ){
      kal_uint32 I;

      *DSP_AWB_SD_CTRL = DSP_INITIALIZED;
      for( I = 0; ; I++ ) {
         if( *DSP_AWB_SD_CTRL == DSP_PLAYING )                /* DSP returns to idle state */
            break;
         ASSERT_REBOOT( I < 40 );
         kal_sleep_task( 1 );
      }
...
static void jamrCloseDevice( Media_Handle_Internal *handle, bool wait )
{
   ...
   if( handle->is_awb && wait ) {
      if(*DSP_AWB_SD_CTRL == DSP_PLAYING)
         *DSP_AWB_SD_CTRL = DSP_STOP;
      for( I = 0; ; I++ ) {
         if( *DSP_AWB_SD_CTRL == DSP_IDLE )                /* DSP returns to idle state */
            break;
         ASSERT_REBOOT( I < 40 );
         kal_sleep_task( 1 );
      }
...
#if defined(AMRWB_DECODE) && ( defined(MT6235) || defined(MT6235B) || defined(MT6268) || defined(MT6253) || defined(MT6236) || defined(MT6236B) || defined(MT6256) )
   if( handle->is_awb ) {
      ASSERT_REBOOT(*DSP_AWB_SD_CTRL == DSP_PLAYING);
      *DSP_AWB_SD_CTRL = DSP_STOP;      /* waiting for DSP to return to DSP_IDLE state */
...
static void jamrHisr( void *data )
{
   ...
   if (handle->mh.state == AMR_STATE_WB_PLAY ) {
      ...
      EXT_ASSERT((((((*DSP_SD2_AWB_VM_0)>>8)&0xFF)>=0x20) &&((((*DSP_SD2_AWB_VM_0)>>8)&0xFF)<=0x28)), (*DSP_SD2_AWB_VM_0), 0, 0);
