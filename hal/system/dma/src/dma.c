...
/*---------------------------------------------*/
/*below is for MMI FOTA feature*/

//#if defined(__UPDATE_BINARY_FILE__)
vff_callback BTIF_DMA_Tx_Callback = NULL;
vff_callback BTIF_DMA_Rx_Callback = NULL;
//#endif
...
//#if defined(__UPDATE_BINARY_FILE__)
void DMA_Register_VFF_Callback(kal_uint16 DMA_Master,vff_callback callback)
{
	
	if(DMA_CON_MASTER_BTIFTX == DMA_Master){
		BTIF_DMA_Tx_Callback = callback;} 
	else if(DMA_CON_MASTER_BTIFRX == DMA_Master){
		BTIF_DMA_Rx_Callback = callback;}
}
//#endif
...
static kal_bool early_dma_init = KAL_FALSE;
#endif
void DMA_Ini(void)
{
   kal_uint8 index;
...
void dma_Vfifo_static_RecHandler_CallBack(kal_uint32 master)
{
    ...
    switch( (master) )
    {
		...
		case DMA_CON_MASTER_BTIFTX:
			ASSERT(BTIF_DMA_Tx_Callback != NULL);
			if(NULL != BTIF_DMA_Tx_Callback){
				BTIF_DMA_Tx_Callback(DMA_CON_MASTER_BTIFTX);}
			break;
		case DMA_CON_MASTER_BTIFRX:
			ASSERT(BTIF_DMA_Rx_Callback != NULL);
			if(NULL != BTIF_DMA_Rx_Callback){
				BTIF_DMA_Rx_Callback(DMA_CON_MASTER_BTIFRX);}
			break;
#endif //__DMA_BT_VIRTUAL_FIFO__					
    }
