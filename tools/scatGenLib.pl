...
sub scatGen_main
{
    ...
    else
    {
        ...
        if ($fota eq 'FOTA_DM')
        {
            ...
            my $is_UBin = &$func('update_binary_file');#IOT_FOTA
            if (defined $is_UBin and $is_UBin eq 'TRUE')
            {
                &sysUtil::sysgen_die("[1.1]Unsupported Feature Combination : FOTA + UPDATE_BINARY_FILE!", __FILE__, __LINE__);
            }
            
            my $diff_fota = &$func('fudiffnet_enable');#REDBEND_FOTA
            if (defined $diff_fota and $diff_fota eq 'DIFF_IMAGE_UPDATE')
            {
                &sysUtil::sysgen_die("[1.1]Unsupported Feature Combination : FOTA + DIFF_IMAGE_UPDATE!", __FILE__, __LINE__);
            }
        }
...
sub scatGen_verno
{
    return "p7.19 -> p7.20 ->p7.21 ->p7.22 ->p7.23 -> p7.24" . " + scatFrame". scatFrame::scatFrame_verno() ;
    # p7.24 by Dongming,  20150515, put l1 obj into cached_extsram when zimage false
    # p7.23 by Dongming, 20150407, mt2502C slim patch back from MT2502C_DEV, tiny system layout reorg.
    # p7.22 by Dongming, 20141216, rock fota under diff fota.
    # p7.21 by Dongming, 20140825, IOT FOTA layout reorg.
    # p7.20 by Dongming, 20140728, New feature for IOT FOTA    
    # p7.19 by Yinli,  20140702, Adjust EXT_BL size to 200kB when differential fota is enabled
    # p7.18 by Yinli,  20140530, Add INTSRAM_CODE size 256byte to reslove memory expired issue
    # p7.17 by Yinli,  20140526, [scatGen.pl]ts_mem_config.h re-org: use global variable to replace macro definition
    # p7.16 by Yinli,  20140519, Support Tiny System AAPMC
    # p7.15 by Yinli,  20140519, Set Tiny-mode logging buffer size depend on AP-mode TST_RING_BUFFER_SIZE
    # p7.14 by Yinli,  20140421, Reserve 12kB ram-space for SWLA when zimage using BEST_PERF policy
    # p7.13 by Yinli,  20140519, Remove DSPRAM bakeup on EMI
...
sub GetBottomRegion
{
    ...
    my $diff_fota = &$feature_query_function_ptr('fudiffnet_enable');
    ...
    my @viva_region = ('VIVA');
    my @diff_fota_region = ('DIFFFOTA_STOR');

    if(defined $diff_fota and $diff_fota eq 'DIFF_IMAGE_UPDATE')
    {
        push @$BottomRegions, @diff_fota_region;
    }
...
sub GetBottomLayout
{
    ...
    my $diff_fota = &$feature_query_function_ptr('fudiffnet_enable');
    # --- --- ---   
    #DIFFERENTIAL FOTA region
    my $diff_fota_layout;
    if(defined $diff_fota and $diff_fota eq 'DIFF_IMAGE_UPDATE')
    {
        
        $diff_fota_layout = <<"__TEMPLATE";
DIFFENFOTA_STOR $RegionInfo_ref->{'DIFFFOTA_STOR'}->[0] $strLVAttribute $RegionInfo_ref->{'DIFFFOTA_STOR'}->[1] 
{
    DIFFENFOTA_STOR_EV +0x0
    {
        ; reserved region for differential fota project
    }
}
__TEMPLATE
       chomp($diff_fota_layout);
    }
# IOT_ROCK_FOTA end
    # --- --- ---   
    # Boot Cert region
...
#High
    ...
    $str_LAYOUT .= $diff_fota_layout if(defined $diff_fota and $diff_fota eq 'DIFF_IMAGE_UPDATE');  
...
sub layout15 #DCMP_ONLY
{
    ...
    my $secure = &$feature_query_function_ptr('secure_support');
    my $diff_fota = &$feature_query_function_ptr('fudiffnet_enable');
    my $rock_fota = &$feature_query_function_ptr('ROCK_FOTA_SUPPORT');
    my $bottom_region_size = 0;
    my $lv_bottom;
    my $useless;
    if(defined $secure and $secure eq 'TRUE' 
    || ($boot_cert ne 'FALSE' and $boot_cert ne 'NONE')
    || (defined $diff_fota and $diff_fota eq 'DIFF_IMAGE_UPDATE'))
    {
        ...
        my $diff_fota = &$feature_query_function_ptr('fudiffnet_enable');
		# IOT_ROCK_FOTA start
		my $DIFFFOTA_STROAGESIZE = 0;
		if(defined $rock_fota and $rock_fota eq 'TRUE'){
            $DIFFFOTA_STROAGESIZE = (defined $diff_fota and $diff_fota eq 'DIFF_IMAGE_UPDATE')? 1 : 0 ;
        }
        else {
            $DIFFFOTA_STROAGESIZE = (defined $diff_fota and $diff_fota eq 'DIFF_IMAGE_UPDATE')? 2 : 0 ;
        }
		# IOT_ROCK_FOTA end
        my %RegionSetting = ('BOOT_CERT'     => [ OCCUPIED_SIZE, 4 * 1024, undef ],
                             ...
                             'DIFFFOTA_STOR' => [ BLOCK_ALIGNED, $DIFFFOTA_STROAGESIZE, undef ]);
    ...
    #----------
    ###UPDATE_ROM layout
    my $isUBin = &$feature_query_function_ptr("update_binary_file");
    my $lv_update_rom = "";
    my $viva_base_str = "+0x400";
    if($isUBin eq 'TRUE')
    {
        ...
        # VIVA size + DCMP + ALICE 
        my $viva_total_size =  $viva_header_size + $nDCMCompressedSize + $comp_alice;
     
        ### VIVA base
        my $viva_base =   $flash_size - $viva_total_size - $bottom_region_size;
        $viva_base_str =  sprintf("0x%08x", &config_query_flash_base_address($bb) + $viva_base);

        ($lv_update_rom,$available_rom) = &layout_updaterom($viva_base_str,$available_rom); 
        $available_rom += 0x400; #in this case, the baseaddress of viva is not +0x400
    ...
    # --- --- ---
    #VIVA Layout
    my $lv_viva = &layoutVIVA($viva_base_str, &CommonUtil::Dec2Hex($viva_header_size));    
...
sub layoutVIVA
{
    ...
    $base_addr =  "+0x0" if ((&FileInfo::is("multiple_binary_files", "TRUE") or &FileInfo::is("UPDATE_BINARY_FILE", "TRUE")) and ($dummy_scatter eq 'TRUE') and &FileInfo::is_NOR());
...
# IOT_ROCK_FOTA start
        my $rock_fota = &$feature_query_function_ptr('ROCK_FOTA_SUPPORT');
        if(defined $rock_fota and $rock_fota eq 'TRUE'){
            $EXT_BL_MIN_SIZE = 120 * 1024; 
        }
        else{
        $EXT_BL_MIN_SIZE = 200 * 1024; 
    }
# IOT_ROCK_FOTA end
    }
    #if FUNET_ENABLE, should reserve 134K size for ext-bootloader
    my $funet_enable = &$feature_query_function_ptr('FUNET_ENABLE');
    if(defined $funet_enable and $funet_enable eq 'FULL_IMAGE_UPDATE')
    {
        $EXT_BL_MIN_SIZE = 134 * 1024; 
    }
...
{
    my $bb = shift;
    my ($TS_ROM1_BASE,$TS_ROM1_SIZE,$TS_ROM2_BASE,$TS_ROM2_SIZE);

    
    #TS_ROM1 LV&EV is on  INTSRAM 
    ($TS_ROM1_BASE,$TS_ROM1_SIZE) = &sysUtil::GetTS_IntsramInfo_under_tiny_mode($bb);
    #TS_ROM2 LV&EV is on  DSPRAM 
    ($TS_ROM2_BASE,$TS_ROM2_SIZE) = &sysUtil::GetTS_DspramInfo($bb);
    #TS_ROM3 LV&EV is on EMI
    #TS_ROM3 is added for tiny system slim. init code, logging code, error handling code will 
    #move to EMI.
    my ($TS_ROM3_BASE,$TS_ROM3_SIZE) = &sysUtil::GetTS_ROM3Info($bb);

    #CSCI include static CSCI and dynamic CSCI
    #static CSCI will be defined as ZI, and dynamic CSCI is located by user on runtime, so CSCI don't occupy flash space, and can be exclude from LV size
    my ($strTSMMDumpSize,$strAPIntsramBakSize,$strTSLogSize,$strTSResEmiSize,$strCSCIInfoSize) = &sysUtil::GetTS_ResMemSize($feature_query_function_ptr,$bb,$feature_config_href);
    $TS_ROM2_SIZE = sprintf("0x%08x",hex($TS_ROM2_SIZE) - hex($strCSCIInfoSize));
    
    $TS_ROM1_SIZE = (exists $feature_config_href->{MAX_TS_ROM1_SIZE})? hex($feature_config_href->{MAX_TS_ROM1_SIZE}) 
                                                                      : hex($TS_ROM1_SIZE) ;
    $TS_ROM2_SIZE = (exists $feature_config_href->{MAX_TS_ROM2_SIZE})? hex($feature_config_href->{MAX_TS_ROM2_SIZE}) 
                                                                     : hex($TS_ROM2_SIZE) ;
    $TS_ROM3_SIZE = (exists $feature_config_href->{MAX_TS_ROM3_SIZE})? hex($feature_config_href->{MAX_TS_ROM3_SIZE}) 
                                                                     : hex($TS_ROM3_SIZE) ;
...
### TINYSYS_ROM3 base and size
    my($ts_rom3_base,$ts_rom3_base_str);
    my($ts_rom3_size,$ts_rom3_size_str);
    $ts_rom3_base = $ts_rom2_base + $ts_rom2_size;
    $ts_rom3_size = $TS_ROM3_SIZE ; 
    $ts_rom3_base_str = sprintf("0x%08x",$ts_rom3_base+&config_query_flash_base_address($bb));
    $ts_rom3_size_str = sprintf("0x%08x",$ts_rom3_size);
### Set MAUI ROM start offset
#print "ts_rom2_base+ts_rom2_size = $ts_rom2_base+$ts_rom2_size\n";
    
    &config_add_rom_offset($ts_rom1_size + $ts_rom2_size + $ts_rom3_size);
...
__LAYOUT
    ...
    my $layout_tinysys_slim = <<"__LAYOUT_SLIM";
TS_ROM3 $ts_rom3_base_str $ts_rom3_size_str
{
}
__LAYOUT_SLIM

    $layout_tinysys .= $layout_tinysys_slim;
    return $layout_tinysys;
}


#****************************************************************************
# subroutine:  layout_updaterom
# return:      update rom layout
#****************************************************************************
sub layout_updaterom
{
    my ($pre_region_addr_str,$available_rom) = @_;
    my ($updaterom_size,$updaterom_base) = (0,0);

    &sysUtil::sysgen_die("[3.1]$bb_folder\\custom_FeatureConfig.h: CONFIG_IOTFOTA_UB_ROM_SIZE need to be defined when UPDATE_BINARY_FILE is TRUE!", __FILE__, __LINE__)
    if (!exists $feature_config_href->{CONFIG_IOTFOTA_UB_ROM_SIZE});
    $updaterom_size = hex($feature_config_href->{CONFIG_IOTFOTA_UB_ROM_SIZE});
    #make the upadterom_size 4k align to match the MPU 4k align request
    my $align_4k = 4096;
    $updaterom_size = ($updaterom_size % $align_4k == 0) ? $updaterom_size : (int($updaterom_size/$align_4k)+1)*$align_4k;

    my $updaterom_size_str = sprintf("0x%08x",$updaterom_size);
    my $updaterom_base_str;
    if($dummy_scatter eq 'TRUE')
    {
        $updaterom_base_str = "+0x0";
        $available_rom -= $updaterom_size;
    }
    else
    {
    	#update_rom base must be block-align to reslove re-pair problem in factory if power loss during update
    	if (&sysUtil::query_cs1_addr($g_bb, hex($pre_region_addr_str), 0) < $updaterom_size)
        {
            print "pre_region_addr_str = $pre_region_addr_str \n";
            print "updaterom_size      = $updaterom_size \n";
            my $cs_pre_region_addr = &sysUtil::query_cs1_addr($g_bb, hex($pre_region_addr_str), 0);
            &sysUtil::sysgen_die("[3.1]$bb_folder\\custom_FeatureConfig.h: NO more space to put CONFIG_IOTFOTA_UB_ROM_SIZE,the update rom is $updaterom_size,but the avaliable rom less than $cs_pre_region_addr", __FILE__, __LINE__);
        }

        $updaterom_base = hex($pre_region_addr_str) - $updaterom_size;
        my $blocksize = &memConfig::Get_flash_block_size(&sysUtil::query_cs1_addr($g_bb, $updaterom_base, 0) - 1);
        my $temp_update_rom_base = &memConfig::Get_block_aligned_addr($updaterom_base);
        if($temp_update_rom_base > $updaterom_base){
            $updaterom_base = $temp_update_rom_base - $blocksize;}
        else{
            $updaterom_base = $temp_update_rom_base;}
        $updaterom_base_str = sprintf("0x%08x",$updaterom_base);
        &WarnMinus($available_rom - (hex($pre_region_addr_str) - $updaterom_base),  __FILE__, __LINE__,"flash can't put in UPDATE_ROM: preRegion_base=$pre_region_addr_str,updaterom_base=$updaterom_base_str,updaterom_size=$updaterom_size_str");
        $available_rom -= (hex($pre_region_addr_str) - $updaterom_base);
    }
    
    # especially reserved region UB_ROM_GFH for IOT_FOTA project,The reason is as below: 
    # 1. to get all symbols which ubin would reference main bin need to full build and link mainbin and ubin together. 
    # 2. Link main bin and ubin with the maui scatterfile 
    # 3. And UBIN GFH would use UB_ROM_GFH link symbol, but maui scatterfile has no UB_ROM_GFH linker symbol
    # 4. to solve the link error , we keep this excution region and name it as UB_ROM_GFH

    #About below scatterAssert function:
    #because RVCT bug, it can not recongnize <=, so, we use ?: exspression

    my $layout_updaterom = <<"__LAYOUT";
UPDATE_ROM $updaterom_base_str $updaterom_size_str  
{ 
    UB_ROM_GFH +0x0
    {
        ; especially reserved region UB_ROM_GFH for IOT_FOTA project 
    }
}
ScatterAssert(LoadLimit(ROM) == LoadBase(UPDATE_ROM) ? 1 : LoadLimit(ROM) < LoadBase(UPDATE_ROM))
__LAYOUT
    return ($layout_updaterom,$available_rom);
}
#****************************************************************************
...
#   High    
    ...
    if(defined $tiny_sys_support and $tiny_sys_support eq 'TRUE')
    {
        ...
        $strTSMMDumpSize = (exists $feature_config_href->{MAX_TS_MM_DUMP_SIZE})? $feature_config_href->{MAX_TS_MM_DUMP_SIZE}
                                                                               : $strTSMMDumpSize;
    	my $ts_res_size = hex($strTSMMDumpSize)+hex($strAPIntsramBakSize)+hex($strTSLogSize)+hex($strTSResEmiSize);
    	$nOffset += $ts_res_size; 
        $RegionInfo{"TS_RESMEM"} = [ $ts_res_size, $nOffset ];

        my ($ts_rom3_ram_base,$ts_rom3_ram_size) = &sysUtil::GetTS_ROM3Info($bb);
        $ts_rom3_ram_size = (exists $feature_config_href->{MAX_TS_ROM3_EMI_SIZE})? $feature_config_href->{MAX_TS_ROM3_EMI_SIZE} 
                                                                 : $ts_rom3_ram_size ;
        $nOffset += hex($ts_rom3_ram_size); 
        $RegionInfo{"TS_EMI_CODE_EV"} = [ hex($ts_rom3_ram_size), $nOffset ];
    ...
    #UB_RAM
    my $updata_bin_support = &$feature_query_function_ptr('UPDATE_BINARY_FILE');
    if(defined $updata_bin_support and $updata_bin_support eq 'TRUE')
    {
        &sysUtil::sysgen_die("[3.1]$bb_folder\\custom_FeatureConfig.h: CONFIG_IOTFOTA_UB_RAM_SIZE need to be defined when UPDATE_BINARY_FILE is turned on!", __FILE__, __LINE__)
        if (!exists $feature_config_href->{CONFIG_IOTFOTA_UB_RAM_SIZE});

        my ($update_bin_ram_size) = hex($feature_config_href->{CONFIG_IOTFOTA_UB_RAM_SIZE});
        #should 32B align for dcm require
    	&sysUtil::sysgen_die("[3.1]$bb_folder\\custom_FeatureConfig.h: CONFIG_IOTFOTA_UB_RAM_SIZE need to be 32Byte align!", __FILE__, __LINE__)
        if (0 != ($update_bin_ram_size % 32));

        $nOffset += $update_bin_ram_size; 
        $RegionInfo{"UB_RAM"} = [ $update_bin_ram_size, $nOffset ];
...
sub layout12
{
    ...
    $viva_base = sprintf("0x%08x", &config_query_flash_base_address($bb) + $fs - $viva_total_size - $bottom_region_size) if((defined $mba and $mba eq 'TRUE') || (defined $isUBin and $isUBin eq 'TRUE'));
    ...
    if($isUBin eq 'TRUE')
    {
        ($lv_update_rom,$available_rom) = &layout_updaterom($viva_base,$available_rom); 
    }
    ...
    if(defined $mba and $mba eq 'TRUE')
    {
        ...
        if($isUBin eq 'TRUE')
        {
            $jbase = ($dummy_scatter eq 'TRUE') ? "+0x0"
                                                 : sprintf("0x%08x", $updaterom_base - $jump_table_size);
        }
...
sub layout5
{
    ...
    if($isUBin eq 'TRUE')
    {
        my $last_region_base_str = sprintf("0x%08x", &config_query_flash_base_address($bb) + $available_rom);
        ($lv_update_rom,$available_rom) = &layout_updaterom($last_region_base_str,$available_rom); 
    }
...
sub layout3
{
    ...
    if($isUBin eq 'TRUE')
    {
        my $last_region_base_str = sprintf("0x%08x", &config_query_flash_base_address($bb) + $query_address);

        ($lv_update_rom,$query_address) = &layout_updaterom($last_region_base_str,$query_address); 
    }
...
# IOT_ROCK_FOTA start
    if(defined $diff_fota and $diff_fota eq 'DIFF_IMAGE_UPDATE')
    {
        ...
        my $nblock_size;
        my $available_size = $norflash_info_href->{'AVAILABLE_SIZE'};
        foreach (1..$diff_fota_blk)
        {
            $nblock_size = &memConfig::Get_flash_block_size($fs,$available_size);
            $diff_fota_size += $nblock_size;
            $fs -= $nblock_size;
        }
        my $diff_fota_base_str = sprintf("0x%08x", &config_query_flash_base_address($bb) + $fs);
        my $diff_fota_Attr = (($alice eq 'TRUE') and $Is_Flashtool_Layout != 1)? "OVERLAY" : "";   
        my $diff_fota_size_str = sprintf("0x%08x", $diff_fota_size);
        
        $diff_fota_layout = <<"__TEMPLATE";
DIFFENFOTA_STOR $diff_fota_base_str $diff_fota_Attr $diff_fota_size_str 
{
    DIFFENFOTA_STOR_EV +0x0
    {
        ; reserved region for differential fota project
...
# IOT_ROCK_FOTA end
    ...
    if($isUBin eq 'TRUE')
    {
        ...
        if(defined $alice and $alice eq 'TRUE')
        {
            ...
            # VIVA size  + ALICE 
            my $viva_total_size =  &sysUtil::query_viva_size($bb) + $comp_alice;
     
            ### VIVA base
            my $viva_base =   $fs - $viva_total_size;
            $viva_base_str =  sprintf("0x%08x", &config_query_flash_base_address($bb) + $viva_base);

            $last_region_base_str = $viva_base_str;
        }
        else
        {
            $last_region_base_str =  sprintf("0x%08x", &config_query_flash_base_address($bb) + $fs); 
        }
        
        ($lv_update_rom,$fs) = &layout_updaterom($last_region_base_str,$fs); 
    ...
    {   
        ...
        $comp_alice = $feature_config_href->{'CONFIG_ALICE_MAX_COMPRESSED_SIZE'};
        my $viva_header_size = &sysUtil::query_viva_size($bb);    
        $VIVA_rom = &layoutVIVA($viva_base_str, &CommonUtil::Dec2Hex($viva_header_size));
    ...
    #----------
    ###UPDATE_ROM layout  ????? do we need fix viva base address. is it enough to only fix updaterom base address
    #my $isUBin = &$feature_query_function_ptr("update_binary_file");
    #my $lv_update_rom = "";
    #if($isUBin eq 'TRUE')
    #{
        ...
        ### last region base
    #    $last_region_base_str = ((&FileInfo::is("alice_support", "TRUE") or &FileInfo::is("dcm_compression_support", "TRUE")) and ($dummy_scatter eq 'TRUE')) 
    #            ? "+0x0"
    #            : sprintf("0x%08x", &config_query_flash_base_address($bb) + $fs);

    #    ($lv_update_rom,$fs) = &layout_updaterom($viva_base_str,$fs); 
...
sub EXTSRAM_type_7
{
    ...
    if(defined $tiny_sys_support and $tiny_sys_support eq 'TRUE')
    {
        $ts_total_size = $RegionInfo_ref->{'TS_CSCIINFO'}[0] + $RegionInfo_ref->{'TS_RESMEM'}[0];
        $ts_total_size += $RegionInfo_ref->{'TS_EMI_CODE_EV'}[0];
    ...
    $ev_string .= &UPDATE_MEM_EV($ram_base+$ram_size-$RegionInfo_ref->{"UB_RAM"}[1],$RegionInfo_ref->{"UB_RAM"}[0]);
    ...
    $ev_string .= &DSPTXRX( $bb, "EXTSRAM", $region_base_tx, $region_base_rx,$region_size_tx, $region_size_rx);
    $ev_string .= &TS_ROM3_EV($ram_base+$ram_size-$RegionInfo_ref->{"TS_EMI_CODE_EV"}[1],$RegionInfo_ref->{"TS_EMI_CODE_EV"}[0]);
...
__TEMPLATE_6260
    ...
    INTSRAM_DATA_PREINIT +0x0 0x00000D00
    {
; data passed from bootloader to maui
         * (PREINIT_INTSRAM_RODATA, PREINIT_INTSRAM_RW, PREINIT_INTSRAM_ZI)
; Please do NOT remove any content below!
...
sub TS_ROM3_EV
{
    my($ts_rom3_ev_base,$ts_rom3_ev_size) = @_;
    my $ts_rom3_ev_base_str = &CommonUtil::Dec2Hex($ts_rom3_ev_base);
    my $ts_rom3_ev_size_str = &CommonUtil::Dec2Hex($ts_rom3_ev_size);
    my $ts_support = &$feature_query_function_ptr('tiny_sys');
    my $ev_string;
    if(defined $ts_support and $ts_support eq 'TRUE')
    {
        #the execution region should not contain "ROM". becuase the sysgen2.pl
        #SCAT_C_Gen_TEMPLATE_1stROM_LENGTH will use this keyword and will cause conflict.
        #so we should not name it as TS_ROM3_EV
        $ev_string = &format_execution_view( "TS_EMI_CODE_EV",
                                          $ts_rom3_ev_base_str,
                                          $ts_rom3_ev_size_str,
                                          [""]
                                        );                                      
...
sub UPDATE_MEM_EV
{
    my($update_bin_mem_base,$update_bin_mem_size) = @_;
    my $update_bin_mem_base_str = &CommonUtil::Dec2Hex($update_bin_mem_base);
    my $update_bin_mem_size_str = &CommonUtil::Dec2Hex($update_bin_mem_size);
    my $update_bin_support = &$feature_query_function_ptr('UPDATE_BINARY_FILE');
    my $ev_string;
    if (defined $update_bin_support and $update_bin_support eq 'TRUE')
    {
        $ev_string = &format_execution_view( "UB_RAM",
                                          $update_bin_mem_base_str,
                                          $update_bin_mem_size_str,
                                          [""]
                                        );                                      
    }
    return $ev_string;  
...
{
    my ($strBaseAddr, $bb) = @_;
    $strBaseAddr = '+0x0' if(!defined $strBaseAddr or ((&FileInfo::is("multiple_binary_files", "TRUE") or &FileInfo::is("UPDATE_BINARY_FILE", "TRUE")) and ($dummy_scatter eq 'TRUE')));
    &sysUtil::sysgen_die("[3.1]$bb_folder\\custom_FeatureConfig.h: CONFIG_DCMP_MAX_COMPRESSED_SIZE need to be defined when DCM_COMPRESION_SUPPORT is turned on!", __FILE__, __LINE__)
       if (!defined($feature_config_href->{'CONFIG_DCMP_MAX_COMPRESSED_SIZE'}));
