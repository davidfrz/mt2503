...
typedef struct
{
    ...
    EMSData *content_ems;
#ifdef __MMI_BT_MAP_CLIENT__
    srv_sms_send_bt_enum send_bt_index;
...
SMS_HANDLE g_cui_sms_bt_send_handle = NULL;
...
#ifdef __MMI_BT_MAP_CLIENT__
static void cui_sms_set_send_bt_index(srv_sms_send_bt_enum send_bt_index);
#endif
...
mmi_pen_hdlr mmi_sms_fte_add_recipient_long_tap_hdlr(mmi_pen_point_struct pos);
#endif
...
/*****************************************************************************
 * FUNCTION
 *  cui_sms_send_get_device_string_item_async
 * DESCRIPTION
 *  
 * PARAMETERS
 *  void
 ...
 *****************************************************************************/
#ifdef __MMI_BT_MAP_CLIENT__
UI_string_type cui_sms_send_get_device_string_item_async(srv_sms_btmapc_bt_enum bt_index, WCHAR* str_ptr)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    ret = srv_bt_mapc_get_bt_addr_by_idx(&dev_addr, bt_index);
    #else
    ret = SRV_BT_MAPC_FAIL_NOT_FOUND;
    #endif		
    if(ret == SRV_BT_MAPC_SUCCESS)	
    {
        dev_info = srv_bt_cm_get_dev_info_by_addr(&dev_addr, SRV_BT_CM_PAIRED_DEV);
        if (dev_info != NULL)
        {            
        ...
        {
            kal_wsprintf(str_ptr, "BT%d", bt_index + 1);
        }
    }
    else
    {
...
static mmi_ret cui_sms_send_proc(mmi_event_struct* evt)
{
    ...
    WCHAR select_send_str_ptr[MAX_SUBMENU_CHARACTERS];
#endif
    /*----------------------------------------------------------------*/
    /* Code Body                                                                                               */
    /*----------------------------------------------------------------*/
        ...
        case EVT_ID_CUI_MENU_LIST_ENTRY:
            {
                ...
                {
                #if defined( __MMI_BT_MAP_CLIENT__) && !defined(__MMI_WEARABLE_DEVICE__)
                    ...
                    if (!srv_bt_mapc_adp_is_connected(SRV_SMS_BTMAPC_BT_INDEX1))
                        cui_menu_set_item_disabled(menu_evt->sender_id, MENU_ID_SMS_SEND_OPT_BY_BT2, MMI_TRUE);
                    #endif /* (BT_MAX_LINK_NUM > 1) */
                ...
                #else
                    #if (BT_MAX_LINK_NUM > 1)
                    cui_sms_send_get_device_string_item_async(SRV_SMS_BTMAPC_BT_INDEX0, select_send_str_ptr);
                    cui_menu_set_item_string(menu_evt->sender_id, MENU_ID_SMS_SEND_OPT_BY_BT, select_send_str_ptr);
                    cui_sms_send_get_device_string_item_async(SRV_SMS_BTMAPC_BT_INDEX1, select_send_str_ptr);
                    cui_menu_set_item_string(menu_evt->sender_id, MENU_ID_SMS_SEND_OPT_BY_BT2, select_send_str_ptr);
        ...
        case EVT_ID_CUI_MENU_ITEM_SELECT:
            {
                ...
                #if (BT_MAX_LINK_NUM > 1)
                    ||(MENU_ID_SMS_SEND_OPT_BY_BT2 == menu_evt->highlighted_menu_id)  //BT2					
                    #endif /* (BT_MAX_LINK_NUM > 1) */	 
                #endif
                    )
                {
...
void cui_sms_send_close(mmi_id sms_send_gid)
{
    ...
    kal_prompt_trace(MOD_MMI, "[SMS], Entry cui_sms_send_close");
...
static MMI_BOOL cui_sms_is_send_enable(void)
{
    ...
    #if (BT_MAX_LINK_NUM > 1)
    if (srv_sms_is_sms_valid() == MMI_FALSE && 
        MMI_FALSE == srv_bt_mapc_adp_is_connected(SRV_SMS_BTMAPC_BT_INDEX0) && 
        MMI_FALSE == srv_bt_mapc_adp_is_connected(SRV_SMS_BTMAPC_BT_INDEX1))
    #else
    if (srv_sms_is_sms_valid() == MMI_FALSE && MMI_FALSE == srv_bt_mapc_adp_is_connected(SRV_SMS_BTMAPC_BT_INDEX0))	
    #endif
        result = MMI_FALSE;
#else
    if (MMI_FALSE == srv_bt_mapc_adp_is_connected(SRV_SMS_BTMAPC_BT_INDEX0) && MMI_FALSE == srv_bt_mapc_adp_is_connected(SRV_SMS_BTMAPC_BT_INDEX1))
        result = MMI_FALSE;
#endif
#else
#ifdef __MMI_TELEPHONY_SUPPORT__
...
#if (defined( __MMI_MAINLCD_96X64___) || defined(__MMI_TELEPHONY_SUPPORT__) && defined(__MMI_WEARABLE_DEVICE__))
  	wgui_override_list_menu_slim_style(WGUI_LIST_MENU_SLIM_STYLE_DRAW_ICON);
#endif /*(defined( __MMI_MAINLCD_96X64___) || defined(__MMI_TELEPHONY_SUPPORT__) && defined(__MMI_WEARABLE_DEVICE__)*/
    wgui_cat1032_show(
...
/* under construction !*/
		...
		wgui_register_pen_long_tap_handler(mmi_sms_fte_add_recipient_long_tap_hdlr);
#endif
...
mmi_pen_hdlr mmi_sms_fte_add_recipient_long_tap_hdlr(mmi_pen_point_struct pos)
{
	...
	if(g_pen_to_control != DM_TITLE1)
	{
		...
		if(hd_data->recipient_info.highlihgt_index != ENTER_NUMBER_INDEX
			 && hd_data->recipient_info.highlihgt_index != ADD_FROM_CONTACTS_INDEX
			 && hd_data->total_num > 0)
		{
			int iwidth = 0;
			int iheight = 0;
...
#endif /*defined(__MMI_BT_MAP_CLIENT__) && defined(__MMI_WEARABLE_DEVICE__) */
#ifdef __MMI_DYNAMIC_SIM_DYNAMIC_UI__
...
{    
    ...
    mmi_id sms_send_gid = cui_sms_get_active_send_gid();
    #if (BT_MAX_LINK_NUM > 1)
    if (!srv_bt_mapc_adp_is_connected(SRV_SMS_BTMAPC_BT_INDEX0) && !srv_bt_mapc_adp_is_connected(SRV_SMS_BTMAPC_BT_INDEX1))
    #else
    if (!srv_bt_mapc_adp_is_connected(SRV_SMS_BTMAPC_BT_INDEX0))	
    #endif
    {
...
static void cui_sms_set_send_bt_index(srv_sms_send_bt_enum send_bt_index)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_COMMON_TRC_G6_MSG, TRC_CUI_SMS_SENDER_SET_SIM_TYPE, send_bt_index);

    sms_send_gid = cui_sms_get_active_send_gid();
    hd_data = cui_sms_get_send_handle_data(sms_send_gid);

    hd_data->para_flag |= SRV_SMS_PARA_BT_INDEX;
    hd_data->send_bt_index = send_bt_index;
}


static void cui_sms_recipient_opt_send_by_bt(srv_sms_send_bt_enum send_bt_index)
{
#if (MMI_MAX_SIM_NUM >= 2)
    cui_sms_set_send_sim_type(SRV_SMS_SIM_1);
#endif
    cui_sms_set_send_bt_index(send_bt_index);
...
static mmi_ret cui_sms_add_recipient_select_hdlr(cui_menu_event_struct *menu_evt)
{
    ...
    switch (cui_menu_evt->highlighted_menu_id)
    {
        case MENU_ID_SMS_RECIPIENT_OPT_SEND:
            ...
            #if (BT_MAX_LINK_NUM > 1)
            cui_sms_pre_device_send_msg();
            #else /*(BT_MAX_LINK_NUM > 1)*/
            cui_sms_recipient_opt_send();
            #endif /*(BT_MAX_LINK_NUM > 1)*/
        #endif /*__MMI_TELEPHONY_SUPPORT__*/
     #else /*defined(__MMI_BT_MAP_CLIENT__) && !defined(__MMI_WEARABLE_DEVICE__)*/
            cui_sms_recipient_opt_send();
     #endif defined(__MMI_BT_MAP_CLIENT__) && !defined(__MMI_WEARABLE_DEVICE__)
...
/* under construction !*/
        ...
        cui_sms_recipient_opt_send_by_bt(SRV_SMS_BT_INDEX0);        
            break;
    #if (BT_MAX_LINK_NUM > 1)
        case MENU_ID_SMS_SEND_OPT_BY_BT2:
        cui_sms_recipient_opt_send_by_bt(SRV_SMS_BT_INDEX1);   
	    break;	        
    #endif /* (BT_MAX_LINK_NUM > 1) */
  #endif /* #if defined(__MMI_BT_MAP_CLIENT__) && !defined(__MMI_WEARABLE_DEVICE__)*/
        ...
        case MENU_ID_SMS_SEND_OPT_BY_SIM1:
		  #if defined(__MMI_BT_MAP_CLIENT__)
            g_sms_send_by_bt = MMI_FALSE;
		  #endif /*defined(__MMI_BT_MAP_CLIENT__)*/
            cui_sms_pre_send_msg();   
            break;
...
{
    cui_sms_send_handle_struct *cui_send_handle;
    kal_prompt_trace(MOD_MMI, "[SMS], Entry cui_sms_send_by_bt_get_list_callback");
	...
	if(callback_data->result == MMI_TRUE)
	{	
        kal_prompt_trace(MOD_MMI, "[SMS], Entry cui_sms_send_by_bt_get_list_callback callback_data->result == MMI_TRUE");
        g_sms_bt_ui_p[g_sms_active_dev_index].last_entered = SRV_SMS_BTMAPC_BOX_UNSENT;
	...
	{
        kal_prompt_trace(MOD_MMI, "[SMS], Entry cui_sms_send_by_bt_get_list_callback callback_data->result == MMI_FALSE");
        StopTimer(MESSAGES_INPROGRESS_TIMER_ID);
        if(g_srv_sms_bt_send_handle != NULL)
        {
	        srv_sms_btmapc_free_send_handle(g_srv_sms_bt_send_handle);
...
static void cui_sms_send_msg_by_bt_req(mmi_id sms_send_gid, SrvSmsCallbackFunc callback_func)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
#endif /* __MMI_MESSAGES_SMS_EMAIL__ */ 
    {
        ...
        kal_prompt_trace(MOD_MMI, "[SMS], Entry cui_sms_send_msg_req");
        g_srv_sms_bt_send_handle = srv_sms_btmapc_get_send_handle();
    ...
    
    {
        ...
        if (para_flag & SRV_SMS_PARA_BT_INDEX)
        {
            srv_sms_btmapc_set_send_bt_index(g_srv_sms_bt_send_handle, hd_data->send_bt_index);
        }
    }
    ...
    if (g_sms_send_by_bt)
    {
        ...
        if(srv_bt_mapc_connect_type(hd_data->send_bt_index) == SRV_BT_MAPC_CONN_SPP) 
        	g_cui_sms_bt_send_handle = srv_sms_btmapc_get_msg_list_by_tab(hd_data->send_bt_index,MMI_FALSE, 
	            SRV_SMS_BTMAPC_BOX_UNSENT,
	            cui_sms_send_by_bt_get_list_callback,
	            (void*)&g_cui_sms_get_list_struct);
        else
        	g_cui_sms_bt_send_handle = srv_sms_btmapc_get_msg_list_by_tab(hd_data->send_bt_index,MMI_TRUE, 
	            SRV_SMS_BTMAPC_BOX_UNSENT,
	            cui_sms_send_by_bt_get_list_callback,
	            (void*)&g_cui_sms_get_list_struct);
    }
...
static void cui_sms_send_msg_req(mmi_id sms_send_gid, SrvSmsCallbackFunc callback_func)
{
	...
	if (g_sms_send_by_bt)
	{
		cui_sms_send_msg_by_bt_req(sms_send_gid, callback_func);
		return;
	}
	else
	{
...
static void cui_sms_save_msg_req(cui_sms_send_handle_struct *cui_send_handle, srv_sms_status_enum set_status, SrvSmsCallbackFunc callback_func)
{
	...
	#endif /*definefd(__MMI_BT_MAP_CLIENT__) && !defined(__MMI_WEARABLE_DEVICE__)*/
	#ifdef __MMI_TELEPHONY_SUPPORT__
...
{    
	...
	if(g_cui_sms_bt_send_handle)
	{
		srv_sms_btmapc_free_cntx(g_cui_sms_bt_send_handle);	
        g_cui_sms_bt_send_handle = NULL;
	}
	g_sms_bt_send_cancel_flag = 1;
