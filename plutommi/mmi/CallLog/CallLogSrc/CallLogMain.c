...
srv_bt_noti_missed_call_evt_struct g_bt_noti;
static mmi_ret mmi_clog_missed_call_pop_scr_proc(mmi_event_struct *evt);
static void mmi_bt_clog_missed_call_popup_view(void);
...
#endif /*__MMI_BT_PBAP_CLIENT__ && __MMI_BT_NOTI_SRV__*/
...
#ifdef __MMI_WEARABLE_DEVICE__
static void mmi_clog_mc_bt_call_launch(srv_bt_noti_missed_call_evt_struct *bt_noti)
{
     S32 ret;
     ...
     call_para.ucs2_num_uri = bt_noti->send_number;
     ...
     call_para.adv_para.phb_data = bt_noti->sender;
...
static mmi_ret mmi_clog_bt_nmgr_func(mmi_event_struct *evt)
{
    ...
    {
    #ifdef __MMI_WEARABLE_DEVICE__
        if (g_bt_noti.missed_call_count == 1)
        {
            mmi_clog_mc_bt_call_launch(&g_bt_noti);
        }
        else
        {     
        ...
        index = data->bt_index;
        MMI_TRACE(MMI_CLOG_TRC_INFO, TRC_MMI_CLOG_BT_NMGR_FUNC, g_is_missed_call.missed_bt_addr[index].lap,g_is_missed_call.missed_bt_addr[index].uap);
...
#if defined (__MMI_BT_PBAP_CLIENT__) && defined (__MMI_BT_NOTI_SRV__) 
#ifdef __MMI_WEARABLE_DEVICE__
mmi_ret mmi_clog_missed_call_pop_entry()
{
    MMI_ID group_id;
    group_id = mmi_frm_group_create_ex(
                    GRP_ID_ROOT, 
                    GRP_ID_BT_CLOG_MISSE_CALL, 
                    mmi_clog_missed_call_pop_scr_proc,
                    NULL, 
    ...
    MMI_TRACE(MMI_CLOG_TRC_INFO, TRC_MMI_CLOG_BT_MC_POP_ENTER, group_id);
    return MMI_TRUE;
}


static void mmi_clog_bt_missed_call_popup_show()
{
    ...
    WCHAR  time[20];
    S8 ts[32];
    MMI_BOOL has_left = MMI_TRUE;
    U8 seperator = 0;
    WCHAR *buf_stamp;
    WCHAR *buf;
    WCHAR* timestamp_buffer;
    total_num = g_bt_noti.missed_call_count;
    bt_tim_stamp = g_bt_noti.timestamp;
    MMI_TRACE(MMI_CLOG_TRC_INFO, TRC_MMI_CLOG_BT_MC_POP_ENTER, total_num, bt_tim_stamp);

#ifdef __BT_4_0_BLE__
    link_type = srv_bt_cm_get_dev_link_type(&g_bt_noti.bt_addr);
    MMI_TRACE(MMI_CLOG_TRC_INFO, TRC_MMI_CLOG_WR_MISS_POP_SHOW,link_type);
...
#endif
    {
        wgui_status_icon_bar_show_icon(STATUS_ICON_BT_MISSED_CALL);
    }
    ...
    {
        str_id = STR_ID_CLOG_MULTI_MISSED_CALLS;
        lsk_str_id = STR_ID_CLOG_MISSED_CHECK;
    }
    else if (total_num == 1)
    {
        str_id = STR_ID_CLOG_SINGLE_MISSED_CALL;
        lsk_str_id = STR_GLOBAL_DIAL;
    } 
    kal_wsprintf(buf, "%d %w", total_num, get_string(str_id));
...
#endif
    {
        ...
        mmi_clog_time_utc2app(bt_tim_stamp, &app_time);
    ...
    #endif
        seperator = '.';
        sprintf((S8*) ts, "%04d%c%02d%c%02d", app_time.nYear, seperator, app_time.nMonth, seperator, app_time.nDay);
        mmi_asc_to_ucs2((S8*) (UI_string_type)time, ts);
        mmi_ucs2cat((PS8) timestamp_buffer, (const PS8) time);
        mmi_ucs2cat((PS8) timestamp_buffer, (const PS8) (L" "));
        time_string((MYTIME*) & app_time, (UI_string_type)time, DT_IDLE_SCREEN);
        mmi_ucs2cat((PS8)timestamp_buffer, (const PS8)time);
    }
  
    wgui_showcategory6333_screen(total_num == 1 ? (UI_string_type)g_bt_noti.sender : NULL, 
        has_left ? (UI_string_type)GetString(lsk_str_id): NULL, 
    ...
    SetLeftSoftkeyFunction(mmi_bt_clog_missed_call_popup_view, KEY_EVENT_UP);
    SetRightSoftkeyFunction(mmi_frm_scrn_close_active_id, KEY_EVENT_UP);
    SetCenterSoftkeyFunction(mmi_bt_clog_missed_call_popup_view, KEY_EVENT_UP);
...
mmi_ret mmi_bt_call_log_event_proc(mmi_event_struct * evt)
{
#ifdef __MMI_HF_SUPPORT__
    MMI_BOOL is_call;
    is_call = srv_ucm_bt_is_any_call();

    MMI_TRACE(MMI_CLOG_TRC_INFO, TRC_MMI_CLOG_BT_BD_LISTERN_CB, is_call, 0);
    if (is_call)
    {
        mmi_frm_group_close(GRP_ID_BT_CLOG_MISSE_CALL);
    }
...
static mmi_ret mmi_clog_missed_call_pop_scr_proc(mmi_event_struct *evt)
{
    switch(evt->evt_id)
    {
        ...
        case EVT_ID_SCRN_ACTIVE:
            mmi_clog_bt_missed_call_popup_show();
            break;
...
mmi_ret mmi_clog_bt_dialer_missed_call_hdlr(mmi_event_struct *evt)
{
    ...
    memset(&g_bt_noti, 0x00, sizeof(srv_bt_noti_missed_call_evt_struct));
    memcpy(&g_bt_noti, bt_noti, sizeof(srv_bt_noti_missed_call_evt_struct));
    MMI_TRACE(MMI_CLOG_TRC_INFO, TRC_MMI_CLOG_PBAPC_BT_MISSED_CALL, bt_noti->missed_call_count,bt_noti->timestamp);
    if(bt_noti->missed_call_count == 0)
    {
        wgui_status_icon_bar_hide_icon(STATUS_ICON_BT_MISSED_CALL);
        mmi_frm_nmgr_cancel_deferred_scenario(MMI_SCENARIO_ID_MISSED_CALL,mmi_clog_missed_call_pop_entry, NULL);

        mmi_frm_group_close(GRP_ID_BT_CLOG_MISSE_CALL);
        return;
    }
    scr_id = mmi_frm_scrn_get_active_id();
    MMI_TRACE(MMI_CLOG_TRC_INFO, TRC_MMI_CLOG_BT_CURR_SCR, scr_id,SCR_ID_CLOG_LAYOUT);
    if (scr_id == SCR_ID_CLOG_LAYOUT)
    {
        return;
    }
...
#ifdef __BT_4_0_BLE__
    link_type = srv_bt_cm_get_dev_link_type(&g_bt_noti.bt_addr);
    MMI_TRACE(MMI_CLOG_TRC_INFO, TRC_MMI_CLOG_BT_WR_MISSED, link_type);
    if (link_type != SRV_BT_CM_LINK_TYPE_LE)
#endif
    {
        wgui_status_icon_bar_show_icon(STATUS_ICON_BT_MISSED_CALL);
    ...
    mmi_frm_nmgr_notify_by_app(
        MMI_SCENARIO_ID_BT_MISSED_CALL,
        MMI_EVENT_BT_MISSED_CALL,
        mmi_clog_missed_call_pop_entry, 
        NULL);
#ifdef	__MMI_HF_SUPPORT__
    is_call = srv_ucm_bt_is_any_call();
    MMI_TRACE(MMI_CLOG_TRC_INFO, TRC_MMI_CLOG_BT_WR_MISSED_I, is_call);
    if (is_call && bt_noti->missed_call_count >= 2)
    {
        mmi_clog_set_bt_status(0);
    }
#endif
}
...
mmi_ret mmi_clog_bt_dialer_missed_call_hdlr(mmi_event_struct *evt)
{
    ...
    MMI_TRACE(MMI_CLOG_TRC_INFO, TRC_MMI_CLOG_BT_MISSED_CALL_HDLR,-1,bt_noti->bt_addr.lap, bt_noti->bt_addr.uap);
    g_is_missed_call.missed_bt_addr[index] = bt_noti->bt_addr;
    MMI_TRACE(MMI_CLOG_TRC_INFO, TRC_MMI_CLOG_BT_MISSED_CALL_HDLR,index, g_is_missed_call.missed_bt_addr[index].lap, g_is_missed_call.missed_bt_addr[index].uap);
    ...
    if(index == 0)
    {
        alert_info.status_bar_icon_para.icon_id = STATUS_ICON_BT1_MISSED_CALL;

    }
    else if (index == 1)
    {
        alert_info.status_bar_icon_para.icon_id = STATUS_ICON_BT2_MISSED_CALL;
    }

#else
    alert_info.status_bar_icon_para.icon_id = STATUS_ICON_BT_MISSED_CALL;
#endif
    if(index == 0)
    {
...
mmi_ret mmi_clog_bt_dialer_bt_disconn_hdlr(mmi_event_struct *evt)
{
    ...
    #else
    #ifdef __MMI_WEARABLE_DEVICE__
        MMI_TRACE(MMI_CLOG_TRC_INFO, TRC_MMI_CLOG_PBAPC_DISCONN, (U32)0,evt->evt_id);
       wgui_status_icon_bar_hide_icon(STATUS_ICON_BT_MISSED_CALL);
       mmi_frm_nmgr_cancel_deferred_scenario(MMI_SCENARIO_ID_MISSED_CALL,mmi_clog_missed_call_pop_entry, NULL);
       mmi_frm_group_close(GRP_ID_BT_CLOG_MISSE_CALL); 
    #else
    ...
    if(notify_evt->evt_id == EVT_ID_PBAP_NOTIFY && (notify_evt->type == EVT_ID_PBAPC_CONNECT_RSP))//EVT_ID_DIALER_DISCONN_EVENT_ID)
    {
        connect_rsp = (srv_pbapc_connect_rsp_struct*)notify_evt->data;
        MMI_TRACE(MMI_CLOG_TRC_INFO, TRC_MMI_CLOG_BT_BW_CONNECT, connect_rsp->new_btd, connect_rsp->ret);
        if (connect_rsp->ret == SRV_PBAPC_RET_OK)
        {
            mmi_clog_pbapc_connect_update_inbg(notify_evt->bt_index);
        }
    }
#endif
    return MMI_RET_OK;
