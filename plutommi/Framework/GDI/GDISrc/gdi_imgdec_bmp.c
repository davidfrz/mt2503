...
#ifdef GDI_IMGDEC_ADJUST_ASPECT_SUPPORT
...
#endif /* GDI_IMGDEC_ADJUST_ASPECT_SUPPORT */
...
GDI_RESULT gdi_imgdec_bmp_draw_handler(gdi_handle out_layer, U32 frame_pos, S32 x, S32 y, S32 w, S32 h, U8 *data_ptr, U32 img_size, BOOL *is_aborted, U32 flag, S32 aspect_flag)
{
    ...
    for (i = 0; i < 3 && (bs_buf == NULL); i++)
    {
        buffer_size = GDI_IMGDEC_BYTESTREAM_BUFFER_SIZE >> i;
    ...
    if (dec.is_from_file)
    {
        ret = gdi_imgdec_bmp_draw_resized_file(&dec, x, y, is_resize, w, h, (S8*)data_ptr, 0);
    }
    else
    {
