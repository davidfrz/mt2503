...
static U8 *wgui_multiline_history_buffer_single = NULL;
static S16 wgui_multiline_history_ID_single = (S16)-1;
#endif
...
//multi_line_input_box MMI_multiline_inputbox_multiline;   //for save multiline data
multi_line_input_box MMI_multiline_inputbox_singleline;  //for save singleline data
...
extern U8 wgui_singleline_inputbox_decimal_flag ;

extern U8 wgui_singleline_inputbox_decimal_input_mode_flag ;

extern MMI_BOOL g_is_disable_up_down_key_register ;
extern MMI_BOOL g_is_disable_left_right_key_register ;

extern void wgui_inputs_sl_handle_left_arrow_repeat(void);
extern void wgui_inputs_sl_handle_right_arrow_repeat(void);
extern void wgui_inputs_sl_cannot_navigate(void);
extern void wgui_inputs_sl_handle_right_softkey_repeat(void);
extern void wgui_inputs_sl_handle_right_softkey_touch_repeat(void);

static void wgui_inputs_ml_handle_input_int_ext(S32 show_counter_flag,multi_line_input_box * editor);
static void wgui_inputs_ml_set_RSK_int_ext(S32 show_counter_flag,multi_line_input_box * editor);
static void wgui_inputs_ml_handle_plus_key_ext(multi_line_input_box * editor);

MMI_BOOL dummy_bool_flag = MMI_FALSE;

PU8 dummy_pu8  = 0;

void (*wgui_inputbox_dummy_function) (void) = NULL;
...
#endif
    extern g_phb_pbapc_show_symbol ;
#endif


editor_multi_and_single_all_data  wgui_inputs_ml_all_data[MULTI_AND_SINGLE_OTHER_DATA_MAX]= 
{   
    {// for multiline
        EDITOR_MULTILINE_TYPE,
        &MMI_multiline_inputbox,   
        &wgui_inputbox_buffer_size, 
        &dummy_pu8,        
        &wgui_inputbox_RSK_label_clear,
        &wgui_inputbox_RSK_label_string,
        &wgui_inputbox_RSK_label_icon, 
        &wgui_multiline_inputbox_decimal_flag,
        &wgui_multiline_inputbox_decimal_input_mode_flag,
        &dummy_bool_flag,
        &dummy_bool_flag,
        &wgui_inputbox_RSK_function,
        &wgui_inputbox_dummy_function,        
        &wgui_multiline_history_buffer ,
        &wgui_multiline_history_ID ,
        &g_wgui_inputs_is_clear_key_delete_all,
    ...
    {// for single line
        EDITOR_SINGLELINE_TYPE,
        &MMI_multiline_inputbox_singleline,
        &wgui_singleline_inputbox_buffer_size,
        &wgui_singleline_inputbox_LSK_label_icon,
        &wgui_singleline_inputbox_RSK_label_clear,
        &wgui_singleline_inputbox_RSK_label_string,
        &wgui_singleline_inputbox_RSK_label_icon,
        &wgui_singleline_inputbox_decimal_flag,
        &wgui_singleline_inputbox_decimal_input_mode_flag,
        &g_is_disable_up_down_key_register,
        &g_is_disable_left_right_key_register,
        &wgui_singleline_inputbox_RSK_function,
        &wgui_singleline_inputbox_LSK_function,
        &wgui_multiline_history_buffer_single,
        &wgui_multiline_history_ID_single,
        &dummy_bool_flag,
        &wgui_inputbox_dummy_function,
...
editor_multi_and_single_all_data *wgui_inputs_ml_get_all_data(multi_line_input_box * editor)
{
    U8 i = 0;

    for(i = 0 ; i < MULTI_AND_SINGLE_OTHER_DATA_MAX ; i++ )
    {
        if(editor == wgui_inputs_ml_all_data[i].editor)
        {
            return &wgui_inputs_ml_all_data[i];
        }
    }
    return NULL;
}
...
#ifdef __MMI_MULTILINE_INPUT_BOX_SINGLINE_STYLE_WITH_FTE__ 
void multiline_inputbox_input_callback_singleline(multi_line_input_box * editor)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/

    editor_multi_and_single_all_data * all_editor_data = NULL;
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    all_editor_data = wgui_inputs_ml_get_all_data(editor);
    if(!all_editor_data)
    {
        return ;
    }
    if(*all_editor_data->wgui_inputbox_input_callback)
    {
        
        #if defined(__MMI_BT_PBAP_CLIENT__)
        #ifndef __MMI_PHB_PBAPC_SYNC_CONTACT__
        if(all_editor_data->editor_type == EDITOR_SINGLELINE_TYPE)
        {
            g_wgui_cat199_refresh_when_input_complete = MMI_TRUE;
        }    
        #endif
        #endif
        (*all_editor_data->wgui_inputbox_input_callback)();
    }    
}
#endif/*__MMI_MULTILINE_INPUT_BOX_SINGLINE_STYLE_WITH_FTE__*/
/*****************************************************************************
...
/* under construction !*/
        ...
        wgui_inputs_ml_create_set_buffer_ext(buffer,buffer_length,text_length,edit_position, &MMI_multiline_inputbox);   
    
#endif/*__MMI_MULTILINE_INPUT_BOX_SINGLINE_STYLE_WITH_FTE__*/    
}

#ifdef __MMI_MULTILINE_INPUT_BOX_SINGLINE_STYLE_WITH_FTE__
void wgui_inputs_ml_create_set_buffer_ext(UI_string_type buffer, S32 buffer_length, S32 text_length, S32 edit_position,multi_line_input_box * editor)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    *all_editor_data->wgui_inputbox_input_callback = UI_dummy_function;
    *all_editor_data->wgui_inputbox_navigate_callback = UI_dummy_function;    
    if(EDITOR_MULTILINE_TYPE == all_editor_data->editor_type)
    {        
        MMI_current_input_ext_type = 0;
        inputbox_x = MMI_multiline_inputbox_x;
        inputbox_y = MMI_multiline_inputbox_y;
        inputbox_width = MMI_multiline_inputbox_width;
        inputbox_height = MMI_multiline_inputbox_height; 
        editor_style = 0;
    }
    else
    {
        inputbox_x = MMI_singleline_inputbox_x;
        inputbox_y = MMI_singleline_inputbox_y;
        inputbox_width = MMI_singleline_inputbox_width;
        inputbox_height = MMI_singleline_inputbox_height;          
        editor_style = 1;
    }
    /* Warning: Temporary change only UCS2 / ASCII. This has to be done in a more formal manner  */
#ifdef __UCS2_ENCODING
    gui_create_multi_line_input_box_set_buffer_2(
        editor,
        inputbox_x,
        inputbox_y,
        inputbox_width,
...
void wgui_inputs_ml_resize(S32 width, S32 height)
{
    ...
    wgui_inputs_ml_resize_ext(width, height, &MMI_multiline_inputbox);
...
void wgui_inputs_ml_resize_ext(S32 width, S32 height,multi_line_input_box * editor)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    gui_resize_multi_line_input_box(editor, width, height);
}
#endif/*__MMI_MULTILINE_INPUT_BOX_SINGLINE_STYLE_WITH_FTE__*/
/*****************************************************************************
 * FUNCTION
...
void wgui_inputs_ml_move(S32 x, S32 y)
{
    ...
    wgui_inputs_ml_move_ext(x, y, &MMI_multiline_inputbox);
...
void wgui_inputs_ml_move_ext(S32 x, S32 y, multi_line_input_box * editor)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    gui_move_multi_line_input_box(editor, x, y);
}
#endif/* __MMI_MULTILINE_INPUT_BOX_SINGLINE_STYLE_WITH_FTE__*/
...
void wgui_inputs_ml_show(void)
{
    ...
    gui_BLT_double_buffer(
        ...
        wgui_inputs_ml_show_ext(&MMI_multiline_inputbox);
        
    #endif/*__MMI_MULTILINE_INPUT_BOX_SINGLINE_STYLE_WITH_FTE__*/   
}

void wgui_inputs_ml_show_ext(multi_line_input_box *editor)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    gui_show_multi_line_input_box(editor);
#if(UI_DOUBLE_BUFFER_SUPPORT)
    gui_BLT_double_buffer(
        editor->x,
        editor->y,
...
/* under construction !*/
    ...
    wgui_inputs_ml_redraw_ext(&MMI_multiline_inputbox);
...
void wgui_inputs_ml_redraw_ext(multi_line_input_box *editor)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
/* under construction !*/
        ...
        if ((wgui_inputs_get_editor_handler()->input_box_sub_type == MMI_EDITOR_INPUT_BOX_EBOOK)
             && mmi_imc_is_clipboard_state() && percentage_callback)
        {
             percentage_callback(editor->visible_line_info->start_line_index);
        }
        #endif
        gui_BLT_double_buffer(
            editor->x,
...
/* under construction !*/
	#endif 
        #endif 
        if(multiline_inputbox_input_callback)
            multiline_inputbox_input_callback();
    }
    ...
    return wgui_inputs_ml_multitap_input_string_ext(str,&MMI_multiline_inputbox);
...
S32 wgui_inputs_ml_multitap_input_string_ext(PU8 str,multi_line_input_box * editor)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /*----------------------------------------------------------------*/
    #ifdef __MMI_EDITOR_MULTILINE_SUPPORT_MULTIGRAPH__
    ret = gui_multi_line_input_box_insert_multitap_string(editor, str);
    
    if (ret)
    {
	    wgui_inputs_ml_redraw_ext(editor);
    #if 0    
	#if((UI_ENABLE_FRAME_SKIPPING) && (ENABLE_MULTILINE_INPUTBOX_FRAME_SKIPPING))
/* under construction !*/
	#endif 
	#endif 
        multiline_inputbox_input_callback_singleline(editor);
	}
...
/* under construction !*/
    ...
    return wgui_inputs_ml_multitap_input_ext(c,&MMI_multiline_inputbox);

#endif/*__MMI_MULTILINE_INPUT_BOX_SINGLINE_STYLE_WITH_FTE__*/
}

#ifdef __MMI_MULTILINE_INPUT_BOX_SINGLINE_STYLE_WITH_FTE__
MMI_BOOL wgui_inputs_ml_multitap_input_ext(UI_character_type c,multi_line_input_box * editor)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
	MMI_BOOL ret = MMI_FALSE;
    S32 wgui_inputbox_buffer_size_ext = 0;
    editor_multi_and_single_all_data * all_editor_data = NULL;
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
...
/* under construction !*/
    #endif
    wgui_inputbox_buffer_size_ext = *all_editor_data->wgui_inputbox_buffer_size;

    
    if (editor->flags & UI_MULTI_LINE_INPUT_BOX_PLUS_CHARACTER_HANDLING)
    {
        if ((c == '+') && (editor->text[0] != '+') && (editor->cursor_p == editor->text) && (editor->text_length >= editor->available_length))
        {
            editor->available_length = (wgui_inputbox_buffer_size_ext + 1) * ENCODING_LENGTH;
            gui_multi_line_input_box_start_input(editor);
        }
        else if (editor->available_length == (wgui_inputbox_buffer_size_ext + 1) * ENCODING_LENGTH)
        {
            if (editor->text_length == editor->available_length)
            {
                if (editor->flags & UI_MULTI_LINE_INPUT_BOX_STATE_MULTITAP)
                {
                    if (editor->cursor_p == editor->text + ENCODING_LENGTH)
                    {
                        if (c != '+')
                        {
                            gui_multi_line_input_box_delete_character_in_pos(editor, 0);
                            gui_multi_line_input_box_confirm_multitap_character(editor);
                            if (editor->text[0] == '+')
                            {
                                editor->available_length = (wgui_inputbox_buffer_size_ext + 1) * ENCODING_LENGTH;
                            }
                            ...
                            {
                                editor->available_length = wgui_inputbox_buffer_size_ext * ENCODING_LENGTH;
                            }
                            gui_multi_line_input_box_stop_input(editor);
                            UI_editor_play_tone_cannot_insert();
                            return MMI_FALSE;
                        }
                    }
            ...
            else if ((editor->text_length == editor->available_length - ENCODING_LENGTH) &&
                    (c != '+') && (editor->cursor_p == editor->text))
            {
                if (!(editor->flags & UI_MULTI_LINE_INPUT_BOX_STATE_MULTITAP))
                {
                    UI_editor_play_tone_cannot_insert();
        ...
        else if ((editor->text_length == editor->available_length) &&
                (editor->cursor_p == editor->text) &&
                (editor->available_length == wgui_inputbox_buffer_size_ext * ENCODING_LENGTH) &&
                (c != '+'))
        {
            UI_editor_play_tone_cannot_insert();
    ...
    ret = gui_multi_line_input_box_insert_multitap_character(editor, c);
    if (ret)
    {
        #ifdef __MMI_EDITOR_MULTILINE_DIALER_STYLE_SUPPORT__
        if (editor->ext_flags & GUI_MULTI_LINE_INPUT_BOX_DIALER_STYLE)
        {
            wgui_inputs_ml_handle_plus_key_ext(editor);
        }
        #endif /* __MMI_EDITOR_MULTILINE_DIALER_STYLE_SUPPORT__ */
...
void wgui_inputs_ml_multitap_input_complete(void)
{
    ...
    wgui_inputs_ml_multitap_input_complete_ext(&MMI_multiline_inputbox);
...
void wgui_inputs_ml_multitap_input_complete_ext(multi_line_input_box * editor)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
/* under construction !*/
    ...
    return wgui_inputs_ml_direct_input_ext( c,&MMI_multiline_inputbox);
...
MMI_BOOL wgui_inputs_ml_direct_input_ext(UI_character_type c,multi_line_input_box * editor)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if (editor->flags & UI_MULTI_LINE_INPUT_BOX_PLUS_CHARACTER_HANDLING)
    {
        if ((c == '+') && (editor->cursor_p == editor->text))
        {
            editor->available_length = (wgui_inputbox_buffer_size_ext + 1) * ENCODING_LENGTH;
        }
        ...
        if ((editor->text[0] == '+') &&
            (((editor->cursor_p == editor->text) &&
              (editor->text_length >= (editor->available_length - ENCODING_LENGTH))) ||
             ((editor->cursor_p == (editor->text + ENCODING_LENGTH)) &&
              (editor->text_length == editor->available_length))))
        {
            UI_editor_play_tone_cannot_insert();
            return MMI_FALSE;
        }
    ...
    if (UI_TEST_LF_CHARACTER(c))
    {
        if (gui_multi_line_input_box_insert_newline(editor))
        {
            ret = MMI_TRUE;
        }
        else
    ...
    {
        ret = gui_multi_line_input_box_insert_character(editor, c);
...
MMI_BOOL wgui_inputs_ml_direct_input_no_draw(UI_character_type c)
{
    ...
    return wgui_inputs_ml_direct_input_no_draw_ext( c,&MMI_multiline_inputbox);
...
MMI_BOOL wgui_inputs_ml_direct_input_no_draw_ext(UI_character_type c,multi_line_input_box * editor)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if (editor->flags & UI_MULTI_LINE_INPUT_BOX_PLUS_CHARACTER_HANDLING)
    {
        ...
        else if ((editor->text[0] == '+') && (editor->cursor_p == editor->text)
            && (editor->text_length >= (editor->available_length - ENCODING_LENGTH)))
        {
            UI_editor_play_tone_cannot_insert();
            return MMI_FALSE;
    ...
    {
        {
            if (*all_editor_data->wgui_inputbox_decimal_input_mode_flag)
            {   
                if (*all_editor_data->wgui_inputbox_decimal_flag)
                {   
                    return MMI_FALSE;
                }
            }
            ...
            if (ret)
            {
                if (*all_editor_data->wgui_inputbox_decimal_input_mode_flag)
                {
                    *all_editor_data->wgui_inputbox_decimal_flag = 1;
                }
            }
        }
    }
...
/* under construction !*/
    ...
    wgui_inputs_ml_delete_character_ext(&MMI_multiline_inputbox);
...
void wgui_inputs_ml_delete_character_ext(multi_line_input_box * editor)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    wgui_inputs_ml_delete_character_no_draw_ext(editor);
    #if 0  /* slim_2_2 */ //
/* under construction !*/
/* under construction !*/
/* under construction !*/
...
void wgui_inputs_ml_delete_character_no_draw(void)
{
    ...
    wgui_inputs_ml_delete_character_no_draw_ext(&MMI_multiline_inputbox);
...
void wgui_inputs_ml_delete_character_no_draw_ext(multi_line_input_box * editor)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    
    {
        if(*all_editor_data->wgui_inputbox_decimal_input_mode_flag)
        {
            if(*all_editor_data->wgui_inputbox_decimal_flag)
            {
                UI_buffer_type p = editor->cursor_p;
                UI_character_type c;
                ...
                if (p != editor->text)
                {
                    ...
                    if (c == (UI_character_type) '.')
                    {
                        *all_editor_data->wgui_inputbox_decimal_flag = 0;
                    }
                }
            }
        }
    }
    gui_multi_line_input_box_delete_character(editor);
}
#endif/*__MMI_MULTILINE_INPUT_BOX_SINGLINE_STYLE_WITH_FTE__*/
...
void wgui_inputs_ml_previous_character(void)
{
    ...
    wgui_inputs_ml_previous_character_ext(&MMI_multiline_inputbox);
...
void wgui_inputs_ml_previous_character_ext(multi_line_input_box * editor)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
#if defined(__MMI_SMART_ALPHABETIC_RESELECTION__)  
    UI_buffer_type words_start_p = editor->cursor_p;
	UI_buffer_type old_cursor_p = editor->cursor_p;
    UI_character_type c;
    MMI_BOOL is_valid_cluster = MMI_FALSE;
    S32 word_length = 0;
#endif
        ...
        && mmi_imc_is_smart_alphabetic_word_reselection_enabled())
    {
        ...
        if (c != 0x20 && c != 0x0D && c != 0x0A && c != 0)
        {
            gui_multi_line_input_box_next(editor);

            is_valid_cluster = mmi_imc_is_valid_alphabetic_word_of_current_input_mode((U8 *)words_start_p, 
                                                                                      (editor->cursor_p - words_start_p)/ENCODING_LENGTH);

            gui_multi_line_input_box_previous(editor);
        }

        if (c == 0x20 || c == 0 || c == 0x0D || c == 0x0A || !is_valid_cluster)
        {
            if (words_start_p > editor->text)
            {
                do
                {
                    words_start_p -= ENCODING_LENGTH;
                ...
                }while (word_length <= mmi_imc_get_max_reselection_word_length() && words_start_p > editor->text);
            }
            
            if (word_length <= mmi_imc_get_max_reselection_word_length() && word_length > 0)
            {
                editor->cursor_p = words_start_p;
                ...
                gui_show_multi_line_input_box_ext(editor, -1, -1);

                if (c != 0 && (c == 0x20 || c == 0x0D || c == 0x0A || !mmi_imc_is_valid_alphabetic_word_of_current_input_mode((U8 *)&c, 1)) 
                    && editor->cursor_p < old_cursor_p
                    && editor->cursor_p == words_start_p)
                {
                    gui_multi_line_input_box_next(editor);
                }    
            
                words_start_p = editor->cursor_p;
                ...
                if ((c != 0x20 && c != 0x0D && c != 0x0A && mmi_imc_is_valid_alphabetic_word_of_current_input_mode((U8 *)&c, 1))&& words_start_p < editor->cursor_p)
                {
                    if (editor->flags & UI_MULTI_LINE_INPUT_BOX_WORD_HIGHLIGHT)
                    {
                        editor->cursor_p = words_start_p;
                        editor->flags &= ~UI_MULTI_LINE_INPUT_BOX_WORD_HIGHLIGHT;
                    ...
                    if(EDITOR_MULTILINE_TYPE == all_editor_data->editor_type)
                        ...
                        if (old_cursor_p != editor->cursor_p)
                        {
                            editor->cursor_fixed_x = -1;
                        }
                    ...
                    else if (mmi_imc_is_valid_alphabetic_word_of_current_input_mode(words_start_p, (editor->cursor_p - words_start_p) / ENCODING_LENGTH))
                    {
                        editor->highlight_start_position = words_start_p;
                        editor->highlight_end_position = editor->cursor_p;
                        editor->flags |= UI_MULTI_LINE_INPUT_BOX_WORD_HIGHLIGHT;
...
#endif /* #if defined(__MMI_SMART_ALPHABETIC_RESELECTION__)   */
    {
        ...
        wgui_inputs_ml_redraw_ext(editor);
        {
            (*all_editor_data->wgui_inputbox_navigate_callback)();
        }
    }
}
#endif/*__MMI_MULTILINE_INPUT_BOX_SINGLINE_STYLE_WITH_FTE__*/
...
void wgui_inputs_ml_next_character(void)
{
    ...
    wgui_inputs_ml_next_character_ext(&MMI_multiline_inputbox);
...
void wgui_inputs_ml_next_character_ext(multi_line_input_box * editor)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
#if defined(__MMI_SMART_ALPHABETIC_RESELECTION__)  
    UI_buffer_type words_end_p = editor->cursor_p;
    UI_buffer_type old_cursor_p = editor->cursor_p;
    MMI_BOOL is_valid_pre_cluster = MMI_FALSE, is_valid_cluster = MMI_FALSE;
    S32 word_length = 0;
#endif
    ...
    if (mmi_imc_is_reselection_input_mode(mmi_imm_get_curr_input_mode()))
    {
        ...
        if (pre_c != 0 && pre_c != 0x20 && pre_c != 0x0D && pre_c != 0x0A)
        {
            ...
            is_valid_pre_cluster = mmi_imc_is_valid_alphabetic_word_of_current_input_mode((U8 *)editor->cursor_p, 
                                       (old_cursor_p - editor->cursor_p)/ENCODING_LENGTH);
        ...
        if (c != 0 && c != 0x20 && c != 0x0D && c != 0x0A)
        {
            ...
            is_valid_cluster = mmi_imc_is_valid_alphabetic_word_of_current_input_mode((U8 *)old_cursor_p, 
                                       (editor->cursor_p - old_cursor_p)/ENCODING_LENGTH);
        ...
        if ((c != 0 && c != 0x20 && c != 0x0D && c != 0x0A && is_valid_cluster) 
            && (pre_c == 0 || pre_c == 0x20 || pre_c == 0x0D || pre_c == 0x0A || !is_valid_pre_cluster))
        {
            if (words_end_p < editor->text + editor->text_length)
            {
                word_length++;
            ...
            if (word_length <= mmi_imc_get_max_reselection_word_length() && word_length > 0)
            {
                ...
                if (editor->cursor_p != words_end_p) 
                {   
                    /* Cursor may be changed by gui_EMS_input_box_locate_cursor() because of cluster */
                    gui_multi_line_input_box_previous(editor);
                }

                words_end_p = editor->cursor_p;
                ...
                if ((c == 0x20 || c == 0 || c == 0x0D || c == 0x0A || !mmi_imc_is_valid_alphabetic_word_of_current_input_mode((U8 *)&c, 1)) && words_end_p > editor->cursor_p)
                {
                    if (editor->flags & UI_MULTI_LINE_INPUT_BOX_WORD_HIGHLIGHT)
                    {
                        editor->flags &= ~UI_MULTI_LINE_INPUT_BOX_WORD_HIGHLIGHT;
                    ...
                    else if (mmi_imc_is_valid_alphabetic_word_of_current_input_mode(editor->cursor_p, (words_end_p - editor->cursor_p) / ENCODING_LENGTH))
                    {
                        editor->highlight_start_position = editor->cursor_p;
                        editor->highlight_end_position = words_end_p;
                        editor->flags |= UI_MULTI_LINE_INPUT_BOX_WORD_HIGHLIGHT;
...
#ifdef __MMI_MULTILINE_INPUT_BOX_SINGLINE_STYLE__
#ifdef __MMI_EDITOR_SL_AUTO_CLEAR_HILITE_MODE__
        !(editor->ext2_flags & UI_MULTI_LINE_INPUT_BOX_AUTO_CLEAR_HIGHLIGHT_MODE) &&            
#endif /*__MMI_EDITOR_SL_AUTO_CLEAR_HILITE_MODE__*/
#endif /*__MMI_MULTILINE_INPUT_BOX_SINGLINE_STYLE__*/
        !mmi_imc_is_ime_state())
    {
        ...
        if (editor->cursor_p > editor->text)
        {
            pre_c = (UI_character_type)((*((editor->cursor_p - ENCODING_LENGTH)+0))|((*((editor->cursor_p - ENCODING_LENGTH)+1))<<8));
        }

        c = (UI_character_type)((*((editor->cursor_p)+0))|((*((editor->cursor_p)+1))<<8));

        if (c == 0 && pre_c != 0 && pre_c != L' ')
        {
            wgui_inputs_ml_direct_input_ext(L' ',editor);
    ...
    wgui_inputs_ml_redraw_ext(editor);
        ...
        #endif /* __MMI_PINYIN_SEARCH__ */ 
        #endif /* __MMI_MULTILINE_INPUT_BOX_SINGLINE_STYLE__ */
        if(*all_editor_data->wgui_inputbox_navigate_callback)
        {
            (*all_editor_data->wgui_inputbox_navigate_callback)();
        }        
}
...
/* under construction !*/
    ...
    wgui_inputs_ml_delete_all_characters_ext(&MMI_multiline_inputbox);
...
void wgui_inputs_ml_delete_all_characters_ext(multi_line_input_box * editor)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    gui_multi_line_input_box_delete_all(editor);
    {    
        if(*all_editor_data->wgui_inputbox_decimal_input_mode_flag)
        {
            *all_editor_data->wgui_inputbox_decimal_flag = 0;
...
/* under construction !*/
    ...
    wgui_inputs_ml_delete_current_character_ext(&MMI_multiline_inputbox);
...
void wgui_inputs_ml_delete_current_character_ext(multi_line_input_box * editor)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    gui_multi_line_input_box_delete_current_character(editor);
    wgui_inputs_ml_redraw_ext(editor);
#if 0    
#if((UI_ENABLE_FRAME_SKIPPING) && (ENABLE_MULTILINE_INPUTBOX_FRAME_SKIPPING))
/* under construction !*/
...
void wgui_inputs_ml_register_keys(void)
{
  ...
  //  SetKeyHandler(wgui_inputs_ml_delete_all_characters, KEY_VOL_DOWN, KEY_LONG_PRESS);
    ...
    wgui_inputs_ml_register_keys_ext(&MMI_multiline_inputbox);
...
void wgui_inputs_ml_register_keys_ext(multi_line_input_box * editor)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
#ifdef __MMI_MULTILINE_INPUT_BOX_SINGLINE_STYLE__
        if(!((editor->ext_flags & GUI_MULTI_LINE_INPUT_BOX_SINGLINE_STYLE)
                 &&(get_wgui_inputs_sl_disable_left_right_key_flag() == MMI_TRUE)))
#endif/*__MMI_MULTILINE_INPUT_BOX_SINGLINE_STYLE__*/
        {
        SetKeyDownHandler(wgui_inputs_ml_previous_character, KEY_LEFT_ARROW);
    ...
    {
            if(*all_editor_data->wgui_inputbox_is_disable_left_right_key_register == MMI_FALSE)
            {
            ...
            if(*all_editor_data->wgui_inputbox_is_disable_up_down_key_register == MMI_FALSE)    
            {
...
void wgui_inputs_ml_set_mask(U8 m)
{
    ...
    wgui_inputs_ml_set_mask_ext(m,&MMI_multiline_inputbox);
...
void wgui_inputs_ml_set_mask_ext(U8 m,multi_line_input_box * editor)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    {
        case 0:
            editor->flags &= ~UI_MULTI_LINE_INPUT_BOX_MASK_CHARACTERS;
            break;
        case 1:
            editor->flags |= UI_MULTI_LINE_INPUT_BOX_MASK_CHARACTERS;
...
/* under construction !*/
    ...
    wgui_inputs_ml_reset_ext(&MMI_multiline_inputbox);
...
#ifdef __MMI_EDITOR_SSP_SUPPORT_NO_DRAW_SCROLLBAR_WHEN_NO_PEN_MOVE__
extern void gui_multiline_ssp_update_scrollbar_cancel_timer(void );
#endif
void wgui_inputs_ml_reset_ext(multi_line_input_box * editor)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
/* under construction !*/
        ...
        gui_cancel_timer(wgui_inputs_ml_vertical_scroll);
#ifdef __MMI_EDITOR_SSP_SUPPORT_NO_DRAW_SCROLLBAR_WHEN_NO_PEN_MOVE__
    gui_multiline_ssp_update_scrollbar_cancel_timer();
#endif
        ...
        *all_editor_data->wgui_inputbox_set_key_callback = NULL;
...
/* under construction !*/
        ...
        *all_editor_data->wgui_inputbox_input_callback = UI_dummy_function;
        *all_editor_data->wgui_inputbox_navigate_callback = UI_dummy_function;
        *all_editor_data->wgui_inputbox_is_disable_up_down_key_register = MMI_TRUE;
        *all_editor_data->wgui_inputbox_is_disable_left_right_key_register = MMI_FALSE;
    }
}
#endif/*__MMI_MULTILINE_INPUT_BOX_SINGLINE_STYLE_WITH_FTE__*/
...
U8 wgui_inputs_ml_is_empty(void)
{
    ...
    return  wgui_inputs_ml_is_empty_ext(&MMI_multiline_inputbox);
...
U8 wgui_inputs_ml_is_empty_ext(multi_line_input_box * editor)
{
#ifdef __ASCII
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
...
MMI_BOOL wgui_inputs_ml_get_display_status(void)
{
    ...
    return wgui_inputs_ml_get_display_status_ext(&MMI_multiline_inputbox); 
...
MMI_BOOL wgui_inputs_ml_get_display_status_ext(multi_line_input_box * editor)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if (editor->flags & UI_MULTI_LINE_INPUT_BOX_DISABLE_DRAW)
    {
        return MMI_FALSE;
    }
    else
...
void wgui_inputs_ml_get_category_history_ext(multi_line_input_box *b, U16 history_ID, U8 *history_buffer)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    all_editor_data = wgui_inputs_ml_get_all_data(b);
    if(!all_editor_data)
    {
        return ;
    }
...
#ifdef __MMI_MULTILINE_INPUT_BOX_SINGLINE_STYLE_WITH_FTE__
        *all_editor_data->wgui_inputbox_history_buffer = history_buffer;
        *all_editor_data->wgui_inputbox_history_ID = (S16)history_ID;
#else
...
//#endif /* __MMI_HIDE_MUL_REMAINING_CHAR__ */ 
    ...
    wgui_inputs_ml_redraw_remaining_characters_display_internal_ext(&MMI_multiline_inputbox);
...
void wgui_inputs_ml_redraw_remaining_characters_display_internal_ext(multi_line_input_box * editor)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
//#ifndef __MMI_HIDE_MUL_REMAINING_CHAR__
...
#endif 
    /* Warning: Temporary change only UCS2 / ASCII. This has to be done in a more formal manner  */
    n = (editor->text_length >> 1) - 1;
    max_n = (editor->available_length >> 1) - 1;
    if ((editor->flags & UI_MULTI_LINE_INPUT_BOX_CHECK_GSM_EXTENDED) &&
        (editor->UCS2_count == 0))
    {
        n += editor->GSM_ext_count;
    }
    if ((editor->flags & UI_MULTI_LINE_INPUT_BOX_PLUS_CHARACTER_HANDLING) &&
        (editor->text[0] == '+'))
    {
    ...
    if (MMI_current_input_ext_type & INPUT_TYPE_EXT_USE_0x81_ENCODING)
    {
        temp_0x81 = srv_phb_convert_to_0x81((U16*) editor->text, MMI_FALSE);
        if (0 != temp_0x81)
        {
            max_n = (editor->allocated_length >> 1) - 4;
            n = temp_0x81 - 3;
        }
    }
    ...
    if (!(editor->flags & UI_MULTI_LINE_INPUT_BOX_INCLUDE_INFO_AREA))
    {
        wgui_inputs_input_information_background(x1, y1, x2, y2);
    }
	gui_push_clip();
...
void wgui_inputs_ml_redraw_remaining_characters_display(void)
{
    ...
    wgui_inputs_ml_redraw_remaining_characters_display_ext(&MMI_multiline_inputbox);
...
void wgui_inputs_ml_redraw_remaining_characters_display_ext(multi_line_input_box * editor)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if(editor->infobar_reminder_char_callback==NULL)
    {
        wgui_inputs_ml_redraw_remaining_characters_display_internal_ext(editor);
    }    
    else
    {
        S32 x1 = wgui_inputbox_information_bar_x;
        ...
        editor->infobar_reminder_char_callback(x1,y1,x2,y2);
    }
}
#endif/*__MMI_MULTILINE_INPUT_BOX_SINGLINE_STYLE_WITH_FTE__*/
...
void wgui_inputs_ml_redraw_input_information_bar(void)
{
    ...
    wgui_inputs_ml_redraw_input_information_bar_ext(&MMI_multiline_inputbox);
...
void wgui_inputs_ml_redraw_input_information_bar_ext(multi_line_input_box * editor)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    gui_lock_double_buffer();
    if(editor->infobar_redraw_callback)
    {
       editor->infobar_redraw_callback(x1,y1,x2,y2); 
    }
    else
    {
       // #if(!((UI_ENABLE_FRAME_SKIPPING) && (ENABLE_MULTILINE_INPUTBOX_FRAME_SKIPPING)))
        if (!(editor->flags & UI_MULTI_LINE_INPUT_BOX_INCLUDE_INFO_AREA) || redraw_on_input_method_change)
       // #endif
            wgui_inputs_input_information_background(x1, y1, x2, y2);
        wgui_inputs_ml_show_current_input_method(x1,y1,x2,y2);
        wgui_inputs_ml_redraw_remaining_characters_display_ext(editor);
    }
    gui_unlock_double_buffer();
    gui_BLT_double_buffer(x1, y1, x2, y2);
}
...
void wgui_inputs_ml_handle_right_softkey_down(void)
{
    ...
    wgui_inputs_ml_handle_right_softkey_down_ext(&MMI_multiline_inputbox);
...
void wgui_inputs_ml_handle_right_softkey_down_ext(multi_line_input_box * editor)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if (gui_multi_line_input_box_get_text_length(editor) > 0)
    {
        if(EDITOR_MULTILINE_TYPE == all_editor_data->editor_type)
        {
            if (g_wgui_inputs_ml_pre_delete_handler)
        ...
        #endif /*__MMI_PHB_QUICK_SEARCH__*/
        #endif /*__MMI_MULTILINE_INPUT_BOX_SINGLINE_STYLE__*/
        wgui_inputs_ml_delete_character_ext(editor);
            ...
            {        
                ...
                refresh_search_list(editor->text);                
            }
        ...
        #endif /*__MMI_MULTILINE_INPUT_BOX_SINGLINE_STYLE__*/
    #if defined(__MMI_WCSS_INPUT_FORMAT_SUPPORT__)
        if ((editor->flags & UI_MULTI_LINE_WCSS_INPUT_BOX_ENABLE) 
            || editor->predefine_char_pos != NULL)
        {
            set_right_softkey_function(wgui_inputs_ml_handle_right_softkey_long_press, KEY_LONG_PRESS);
        }
    #endif
...
void wgui_inputs_ml_handle_right_softkey_up(void)
{
    ...
    wgui_inputs_ml_handle_right_softkey_up_ext(&MMI_multiline_inputbox);
...
void wgui_inputs_ml_handle_right_softkey_up_ext(multi_line_input_box * editor)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if(*all_editor_data->wgui_inputbox_RSK_function)
    {
       (*all_editor_data->wgui_inputbox_RSK_function)(); 
    }
    else
    {
        mmi_event_struct evt;
...
void wgui_inputs_ml_handle_right_softkey_long_press(void)
{
    ...
    if (g_wgui_cat203_search_function)
    {
     ...
     wgui_inputs_ml_handle_right_softkey_long_press_ext(&MMI_multiline_inputbox);
...
void wgui_inputs_ml_handle_right_softkey_long_press_ext(multi_line_input_box * editor)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if(EDITOR_MULTILINE_TYPE == all_editor_data->editor_type)
    {
        if(*all_editor_data->wgui_inputs_is_clear_key_delete_all)
        {
            wgui_inputs_ml_delete_all_characters();
        }
        else
        ...
        if (g_wgui_cat203_search_function)
        {
            wgui_cat203_refresh_search_list(editor->text);
        }
        #endif /* __MMI_CAT203_SUPPORT__ */
    }
    else
    {
        ...
        if (gui_multi_line_input_box_test_first_position(editor))
        {
            return;
        }
        ...
        {        
            wgui_cat103_refresh_search_list(editor->text);        
        }
...
/* under construction !*/
        ...
        wgui_inputs_ml_handle_input_int_ext(show_counter_flag,&MMI_multiline_inputbox);
...
static void wgui_inputs_ml_handle_input_int_ext(S32 show_counter_flag,multi_line_input_box * editor)
{
    ...
    if(all_editor_data->editor_type == EDITOR_MULTILINE_TYPE)
    {
#if defined(__ATV_SMS_SUPPORT__)
#if(UI_DOUBLE_BUFFER_SUPPORT)
        gui_lock_double_buffer();
#endif
#endif
        if (gui_multi_line_input_box_get_text_length(all_editor_data->editor) > 0)
        {
            if (mmi_frm_kbd_is_key_supported(KEY_CLEAR))
            {
                if (*all_editor_data->wgui_inputbox_RSK_label_clear == 0)
                {
                    if (wgui_inputbox_not_empty_callback != NULL && !mmi_imc_is_ime_state())
                    {
                        wgui_inputbox_not_empty_callback();
                    }
                    *all_editor_data->wgui_inputbox_RSK_label_clear = 1;
                    SetKeyDownHandler(wgui_inputs_ml_handle_right_softkey_down, KEY_CLEAR);
                    SetKeyUpHandler(UI_dummy_function, KEY_CLEAR);
                    SetKeyLongpressHandler(wgui_inputs_ml_handle_right_softkey_long_press, KEY_CLEAR);
        ...
        else
        {
            if (mmi_frm_kbd_is_key_supported(KEY_CLEAR))
            {
                if (*all_editor_data->wgui_inputbox_RSK_label_clear == 1)
                {

                    *all_editor_data->wgui_inputbox_RSK_label_clear = 0;
                    if (wgui_inputbox_empty_callback != NULL && !mmi_imc_is_ime_state())
                    {
                        wgui_inputbox_empty_callback();
                    }
            ...
            else
            {
                if (*all_editor_data->wgui_inputbox_RSK_label_clear == 1)
                {
                    set_right_softkey_label(*all_editor_data->wgui_inputbox_RSK_label_string);
                    set_right_softkey_icon(*all_editor_data->wgui_inputbox_RSK_label_icon);
                    if (wgui_inputbox_empty_callback != NULL && !mmi_imc_is_ime_state())
                    {
                        wgui_inputbox_empty_callback();
                    }
        ...
        if (all_editor_data->editor->flags & UI_MULTI_LINE_INPUT_BOX_PLUS_CHARACTER_HANDLING)
        {

#ifdef __MMI_FTE_SUPPORT__
            S32 length = all_editor_data->editor->available_length;
#endif

            if (all_editor_data->editor->text[0] == '+')
            {
                all_editor_data->editor->available_length = (*all_editor_data->wgui_inputbox_buffer_size + 1) * ENCODING_LENGTH;
            }
            else
            {
                all_editor_data->editor->available_length = (*all_editor_data->wgui_inputbox_buffer_size) * ENCODING_LENGTH;
            }

#ifdef __MMI_FTE_SUPPORT__
            if (length != all_editor_data->editor->available_length)
            {
            wgui_inputs_ml_redraw();
        }
#endif
...
void wgui_inputs_ml_handle_input_ext(multi_line_input_box * editor)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    wgui_inputs_ml_handle_input_int_ext(1,editor);
...
void wgui_inputs_ml_handle_navigate(void)
{
    ...
    wgui_inputs_ml_handle_navigate_ext(&MMI_multiline_inputbox);
...
void wgui_inputs_ml_handle_navigate_ext(multi_line_input_box * editor)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if(EDITOR_MULTILINE_TYPE == all_editor_data->editor_type)
    {
        if (gui_multi_line_input_box_test_first_position(all_editor_data->editor))
        {
            if(*all_editor_data->wgui_inputbox_RSK_label_clear)
            {
                set_right_softkey_label(*all_editor_data->wgui_inputbox_RSK_label_string);
                set_right_softkey_icon(*all_editor_data->wgui_inputbox_RSK_label_icon);
    ...
    {
        if (gui_multi_line_input_box_test_first_position(all_editor_data->editor) ||
            (gui_multi_line_input_box_get_text_length(all_editor_data->editor) <= 0))         
        {
            if(*all_editor_data->wgui_inputbox_RSK_label_clear == 1)
            {
                set_right_softkey_label(*all_editor_data->wgui_inputbox_RSK_label_string);
...
void wgui_inputs_ml_setup_ext3(
        S32 x,
        S32 y,
        S32 width,
        S32 height,
        ...
        multi_line_input_box * editor)
{
    ...
    *all_editor_data->wgui_inputbox_buffer_size = buffer_size;
    ...
    wgui_inputs_ml_create_set_buffer_ext((UI_string_type) buffer, buffer_size, l, l,editor);
    if (information_bar_flag)
    {
        wgui_inputbox_information_flag = 1;
    }    
    editor->flags |= additional_flags;
    editor->min_text_height = min_text_height;
    editor->min_line_height = min_line_height;
    editor->header_height = header_height;
    editor->footer_height = footer_height;
    editor->icon_callback = icon_callback;

    if(EDITOR_MULTILINE_TYPE == all_editor_data->editor_type)
    {
        *all_editor_data->wgui_inputbox_RSK_label_icon = RSK_icon;
        *all_editor_data->wgui_inputbox_RSK_label_string = RSK_label;
    ...
    {        
        ...
        *all_editor_data->wgui_inputbox_decimal_input_mode_flag = 0;
        *all_editor_data->wgui_inputbox_decimal_flag = 0;
        *all_editor_data->wgui_inputbox_RSK_label_clear = 0;
        *all_editor_data->wgui_inputbox_RSK_label_icon = RSK_icon;
        *all_editor_data->wgui_inputbox_RSK_label_string = RSK_label;        
...
/* under construction !*/
            ...
            #ifdef __BW_MMI__
                wgui_inputs_ml_move_ext(x, y,editor);
                h = height;
            #else /* __BW_MMI__ */ 
                wgui_inputs_ml_move_ext(x, y + wgui_inputbox_information_bar_height,editor);
                h = height - wgui_inputbox_information_bar_height;
            #endif /* __BW_MMI__ */
        #endif
    }
    ...
    gui_set_font(editor->text_font);
    fh = gui_get_character_height();

    wgui_inputs_ml_resize_ext(width, h,editor); /* it should use user defined width */
...
/* under construction !*/
        ...
        if (wgui_small_editor_information_flag && whether_no_small_screen())
        {
            wgui_inputs_setup_input_information(
                editor->x + 1,
                editor->y - wgui_inputbox_information_bar_height,
                editor->width - 2,
                wgui_inputbox_information_bar_height);
        }
        else
    ...
    is_valid_history = wgui_inputs_ml_set_category_history_ext(editor, category_screen_ID, history_buffer);

    /* app might not close multiline to clear this two variables when exit screen, so reset them when setup*/
    {
        *all_editor_data->wgui_inputbox_history_buffer = NULL;
        *all_editor_data->wgui_inputbox_history_ID =  (S16)-1;
    }
    saved_inputbox_flags = editor->flags;
    ...
    if (input_type & INPUT_TYPE_USE_ENCODING_BASED_LENGTH)
    {
        editor->flags |= UI_MULTI_LINE_INPUT_BOX_USE_ENCODING_BASED_LENGTH;
        if (input_type & INPUT_TYPE_ONE_LESS_CHARACTER)
        {
            editor->flags |= UI_MULTI_LINE_INPUT_BOX_ONE_LESS_CHARACTER;
            UI_TEST_UCS2_COUNT_SET_LENGTH_TYPE2(
                editor->UCS2_count,
                editor->allocated_length,
                editor->available_length);
        }
        else if (input_type & INPUT_TYPE_FORTY_FOUR_LESS_CHARACTER)
        {
            editor->flags |= UI_MULTI_LINE_INPUT_BOX_FORTY_FOUR_LESS_CHARACTER;
            UI_TEST_UCS2_COUNT_SET_LENGTH_TYPE3(
                editor->UCS2_count,
                editor->allocated_length,
                editor->available_length);
...
/* under construction !*/
    ...
    {
#if defined(__MMI_WCSS_INPUT_FORMAT_SUPPORT__)
        editor->change_callback = set_multiline_inputbox_change_callback;    /* Originally for WCSS editor */
#endif 
    }
    if (plus_character_handling)
    {
        editor->flags |= UI_MULTI_LINE_INPUT_BOX_PLUS_CHARACTER_HANDLING;
    }
    if (input_type_flags & INPUT_TYPE_GSM_EXTENDED_HANDLING)
    {
        editor->flags |= UI_MULTI_LINE_INPUT_BOX_CHECK_GSM_EXTENDED;
    }

    if ((saved_inputbox_flags & UI_MULTI_LINE_INPUT_BOX_MASK_CHARACTERS))
    {
        wgui_inputs_ml_set_mask_ext(1,editor);
    }
    ...
    {   
        //editor->text_x =  editor->text_x + 5; 
   
        editor->is_hide_floating_counter = MMI_TRUE;//close floating counter
        *all_editor_data->wgui_inputbox_RSK_label_icon = RSK_icon;
        *all_editor_data->wgui_inputbox_RSK_label_string = RSK_label;
        /* move to the front for wgui_set_default_input_type */
    ...
    if (connect_ime)
    {
        ...
        g_mmi_editor_hanler.input_box_handler = editor;
    ...
    if(EDITOR_MULTILINE_TYPE == all_editor_data->editor_type)
    {
        *all_editor_data->wgui_inputs_is_clear_key_delete_all = MMI_FALSE;
    }     
    wgui_inputs_ml_set_decimal_flag_ext(input_type,editor);  /* slim_2_2 */ //
    #if 0 /* slim_2_2 */ //
/* under construction !*/
/* under construction !*/
/* under construction !*/
...
/* under construction !*/
    ...
    /* for avoid text_offset_y not be calculated. wgui_inputs_ml_show_no_draw() */
    gui_lock_double_buffer();
    gui_show_multi_line_input_box_no_draw(editor);
    gui_unlock_double_buffer();
    UI_UNUSED_PARAMETER(fh);
}
#endif/*__MMI_MULTILINE_INPUT_BOX_SINGLINE_STYLE_WITH_FTE__*/
...
void wgui_inputs_ml_set_RSK_ext(multi_line_input_box * editor)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    wgui_inputs_ml_set_RSK_int_ext(1,editor);
}
...
static void wgui_inputs_ml_set_RSK_int(S32 show_counter_flag)
{
    ...
    wgui_inputs_ml_set_RSK_int_ext(show_counter_flag,&MMI_multiline_inputbox);
...
static void wgui_inputs_ml_set_RSK_int_ext(S32 show_counter_flag,multi_line_input_box * editor)
{
    ...
    if(EDITOR_MULTILINE_TYPE == all_editor_data->editor_type)
    {
        *all_editor_data->wgui_inputbox_RSK_label_clear = 0;
        ...
        all_editor_data->editor->validation_callback = wgui_inputs_ml_validation_callback;
        /* MMI_multiline_inputbox.validation_callback(MMI_multiline_inputbox.text,MMI_multiline_inputbox.cursor_p,MMI_multiline_inputbox.text_length); */
#ifdef __ASCII
        all_editor_data->editor->validation_callback(
                                all_editor_data->editor->text,
                                all_editor_data->editor->cursor_p,
                                all_editor_data->editor->text_length - 1);
#endif /* __ASCII */ 
        all_editor_data->editor->validation_callback(
                                all_editor_data->editor->text,
                                all_editor_data->editor->cursor_p,
                                (all_editor_data->editor->text_length >> 1) - 1);
    }
    else // for sinleline
    {
        ...
        *all_editor_data->wgui_inputbox_RSK_label_clear = 0;
        ...
        all_editor_data->editor->validation_callback = wgui_inputs_sl_validation_callback;
        all_editor_data->editor->validation_callback(
                                all_editor_data->editor->text,
                                all_editor_data->editor->cursor_p,
                                (all_editor_data->editor->text_length >> 1) - 1);
...
/* under construction !*/
    ...
    wgui_inputs_ml_close_ext(&MMI_multiline_inputbox);
...
void wgui_inputs_ml_close_ext(multi_line_input_box * editor)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
/* under construction !*/
    ...
    mmi_input_box_msg_call_back_multiline_ext(input_box_handle, msg_ptr);
...
U32 mmi_input_box_msg_call_back_multiline_ext(void * input_box_handle, mmi_imc_message_struct_p msg_ptr)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    all_editor_data = wgui_inputs_ml_get_all_data(editor_ptr);
    if(!all_editor_data)
    {
        return lresult;
    }
    ...
    case MMI_IMC_MESSAGE_INSERT_CHAR:
...
#endif
        {
            ...
            lresult = wgui_inputs_ml_direct_input_ext((UI_character_type)msg_ptr->param_0,editor_ptr);
            ...
            /* in message app, when input long text to switch sms to mms, popup appears,
               ...
               when inputmode is smart, mmi_imc_disconnet will comfirm condidate and insert a space in the end, so history is changed
               Thus we need get history again to get right value */
            if (lresult && *all_editor_data->wgui_inputbox_history_buffer!= NULL && *all_editor_data->wgui_inputbox_history_ID!= (S16)-1)
            {
                wgui_inputs_ml_get_category_history_ext(editor_ptr, *all_editor_data->wgui_inputbox_history_ID, *all_editor_data->wgui_inputbox_history_buffer);
            }
        }
        break;
    ...
    case MMI_IMC_MESSAGE_INSERT_STRING:
        ...
        for (i = 0; i < str_len; i++)
        {
            ...
            if (temp_str[i] != 0)
            {
                if (MMI_TRUE == wgui_inputs_ml_direct_input_no_draw_ext((UI_character_type)temp_str[i],editor_ptr))
                {
                    total_len ++;
                }
                else
    ...
    #ifdef GUI_INPUT_BOX_CACHE_SUPPORT
            if ((str_len > 10)&&(EDITOR_MULTILINE_TYPE == all_editor_data->editor_type))
            {
                gui_mlc_all_line_update();
            }
    #endif
        ...
        wgui_inputs_ml_redraw_ext(editor_ptr);
#if 0        
#if((UI_ENABLE_FRAME_SKIPPING) && (ENABLE_MULTILINE_INPUTBOX_FRAME_SKIPPING))
/* under construction !*/
#endif 
#endif 
    {
        if (total_len >0)
        {
            multiline_inputbox_input_callback_singleline(editor_ptr);           
        }    
    }
...
/* under construction !*/
    ...
    case MMI_IMC_MESSAGE_REPLACE_HIGHLIGHT_STRING:
        ...
        for (i = 0; i < msg_ptr->param_0; i ++)
        {
            wgui_inputs_ml_delete_character_no_draw_ext(editor_ptr);
        }
...
/* under construction !*/
    ...
    case MMI_IMC_MESSAGE_INSERT_MULTITAP_CHAR:
        ch = (UI_character_type)msg_ptr->param_0;
        lresult = wgui_inputs_ml_multitap_input_ext(ch,editor_ptr);
#ifdef __MMI_MULTILINE_INPUT_BOX_SINGLINE_STYLE__
#ifdef __MMI_PHB_QUICK_SEARCH__
#ifdef __MMI_UI_LIST_HIGHLIGHT_EFFECTS__
        if (((mmi_editor_handler_struct *)input_box_handle)->input_box_sub_type == MMI_EDITOR_QUICK_SEARCH_SINGLE_LINE_INPUT_BOX)
    ...
    case MMI_IMC_MESSAGE_RESET_MULTITAP_STATE:
...
#endif /*__MMI_PHB_QUICK_SEARCH__*/
#endif /*__MMI_MULTILINE_INPUT_BOX_SINGLINE_STYLE__*/
        wgui_inputs_ml_multitap_input_complete_ext(editor_ptr);
        gui_multi_line_input_box_stop_input(editor_ptr);
#ifdef __MMI_MULTILINE_INPUT_BOX_SINGLINE_STYLE__
#ifdef __MMI_PHB_QUICK_SEARCH__
#ifdef __MMI_UI_LIST_HIGHLIGHT_EFFECTS__
        ...
        {
            gui_set_WCSS_Esc_Char(MMI_TRUE);
            gui_change_WCSS_input_method(editor_ptr);
        }
#endif /* defined(__MMI_WCSS_INPUT_FORMAT_SUPPORT__) */ 
        break;
...
/* under construction !*/
    ...
    case MMI_IMC_MESSAGE_CLEAR_ALL:
        ...
        if(*all_editor_data->wgui_inputs_is_clear_key_delete_all)
        {
            wgui_inputs_ml_delete_all_characters_ext(editor_ptr);
        }
        else
        {
    ...
    case MMI_IMC_MESSAGE_CHANGE_TO_INITIAL:
        ...
        /* Because empty callback will not called in ML editor input callback if in IME state, so we need to
           re-call empty callback after change to initial state */
        if (wgui_inputs_ml_is_empty_ext(editor_ptr) && (wgui_inputbox_empty_callback != NULL))
        {
            wgui_inputbox_empty_callback();
        }
        break;
    ...
    case MMI_IMC_MESSAGE_MOVE_CURSOR:
#ifdef __MMI_MULTILINE_INPUT_BOX_SINGLINE_STYLE__
        if((editor_ptr->ext_flags & GUI_MULTI_LINE_INPUT_BOX_SINGLINE_STYLE)
            &&(mmi_frm_is_key_event()))
        {
            U16 keycode = 0, keytype = 0;
            /* get key code and type */
        ...
        switch (msg_ptr->param_0)
        {
            ...
            case MMI_IMC_EDITOR_CURSOR_LEFT:
                for (i = 0; i <(S32)msg_ptr->param_1; i ++)
                {
                    wgui_inputs_ml_previous_character_ext(editor_ptr);
                }
                break;
            case MMI_IMC_EDITOR_CURSOR_RIGHT:
                for (i = 0; i <(S32)msg_ptr->param_1; i ++)
                {
                    wgui_inputs_ml_next_character_ext(editor_ptr);
                }
                break;
        }
    ...
    case MMI_IMC_MESSAGE_RESIZE:
        ...
        case MMI_EDITOR_UCE_MULTLINE_INPUT_BOX:
            if ((S32)msg_ptr->param_1 != editor_ptr->height)
            {   
                MMI_BOOL disable = wgui_inputs_ml_is_disable_ext(editor_ptr);
                gui_resize_multi_line_input_box_no_draw(editor_ptr, (S32)msg_ptr->param_0, (S32)msg_ptr->param_1);

                if (editor_ptr->text_offset_y < (editor_ptr->height - editor_ptr->text_height - 2))
                {
                    editor_ptr->text_offset_y = editor_ptr->height - editor_ptr->text_height - 2;
                }

                if (editor_ptr->text_offset_y > 0)
                {
                    editor_ptr->text_offset_y = 0;
                }
    ...
    case MMI_IMC_MESSAGE_REDRAW:
        ...
        {
            editor_ptr->validation_callback = wgui_inputs_ml_validation_callback;

    #ifdef __ASCII
            editor_ptr->validation_callback(
                editor_ptr->text,
                editor_ptr->cursor_p,
                editor_ptr->text_length - 1);
    #else
            editor_ptr->validation_callback(
                editor_ptr->text,
                editor_ptr->cursor_p,
                (editor_ptr->text_length >> 1) - 1);
    #endif /* __ASCII */ 
...
#endif /*__MMI_MULTILINE_INPUT_BOX_SINGLINE_STYLE__*/
        {
            wgui_inputs_ml_redraw_input_information_bar_ext(editor_ptr);
        }
        break;
    case MMI_IMC_MESSAGE_DRAW_IME_HINT:
#ifdef __MMI_MULTILINE_INPUT_BOX_SINGLINE_STYLE__
    ...
    case MMI_IMC_MESSAGE_PEN_EVENT:
...
#endif
        {
            ...
            isDisableBefore = wgui_inputs_ml_is_disable_ext(editor_ptr);
            ret = (MMI_BOOL)gui_multi_line_input_box_translate_pen_event(
                            editor_ptr,
                            ((mmi_imc_editor_pen_event_struct_p)msg_ptr->param_0)->mmi_pen_event,
                            ((mmi_imc_editor_pen_event_struct_p)msg_ptr->param_0)->pos.x,
                            ...
                            &input_box_event);
            /* translate pen event will change the disable state by category callback */
            isDisableAfter = wgui_inputs_ml_is_disable_ext(editor_ptr);
    ...
    case MMI_IMC_MESSAGE_IS_EDITOR_EMPTY:
        ...
        {   
            lresult = wgui_inputs_ml_is_empty_ext(editor_ptr);
        }
        break;
    ...
    case MMI_IMC_MESSAGE_SET_MASK:
        wgui_inputs_ml_set_mask_ext(1,editor_ptr);
        break;    
    case MMI_IMC_MESSAGE_WCSS_CHECK:
#if defined(__MMI_WCSS_INPUT_FORMAT_SUPPORT__)
        if ((editor_ptr->flags & UI_MULTI_LINE_WCSS_INPUT_BOX_ENABLE) 
...
/* under construction !*/
    ...
    case MMI_IMC_MESSAGE_INSERT_DECIMAL_SYMBOL:    
        ...
        if (gui_multi_line_input_box_test_first_position(editor_ptr)  ||
            (first_char == (UI_character_type)'-' && second_char == 0))
        {
            wgui_inputs_ml_direct_input_ext((UI_character_type) '0',editor_ptr);
        }
        if (wgui_inputs_ml_direct_input_ext((UI_character_type) '.',editor_ptr) == MMI_TRUE)
        {
            {
                *all_editor_data->wgui_inputbox_decimal_flag = 1;
            }
...
/* under construction !*/
    ...
    case MMI_IMC_MESSAGE_INSERT_SPACE:
        wgui_inputs_ml_direct_input_ext((UI_character_type)(0x20),editor_ptr);
        break;
    case MMI_IMC_MESSAGE_CHECK_FLOAT:
            {
                lresult = *all_editor_data->wgui_inputbox_decimal_flag;
            }
        break;
    case MMI_IMC_MESSAGE_SET_MULTITAP_STATE:
        editor_ptr->flags |= UI_MULTI_LINE_INPUT_BOX_STATE_MULTITAP;
    ...
    case MMI_IMC_MESSAGE_GET_EDITOR_SCROLL_BAR_WIDTH:
        lresult = (U32)wgui_inputs_ml_get_scroll_bar_width_ext(editor_ptr);
        break;
    ...
    case MMI_IMC_MESSAGE_CLIPBOARD_INSERT_STRING_WITH_LENGTH:
        ...
        gui_lock_double_buffer();
#endif
        lresult = wgui_inputs_ml_insert_string_ext((UI_string_type)msg_ptr->param_0, (U32)msg_ptr->param_1, MMI_TRUE,editor_ptr);
#if(UI_DOUBLE_BUFFER_SUPPORT)
        gui_unlock_double_buffer();
#endif
        wgui_inputs_ml_redraw_ext(editor_ptr);
...
/* Handle inline multiline */
    ...
    case MMI_IMC_MESSAGE_CLIPBOARD_GET_ALL_TEXT:
        if (((mmi_editor_handler_struct *)input_box_handle)->input_box_sub_type == MMI_EDITOR_INPUT_BOX_MMS_VIEWER)
        {
            ...
            *((U32 *)msg_ptr->param_1) = g_imc_clipboard_data.visible_end_p - g_imc_clipboard_data.visible_start_p;
            ...
            ((mmi_editor_handler_struct *)input_box_handle)->input_box_sub_type == MMI_EDITOR_LINE_BASED_IN_MULTLINE_INPUT_BOX)
        {
            wgui_inputs_ml_jump_to_line(editor_ptr->visible_line_info->start_line_index, MMI_FALSE);
            *((U8 **)(msg_ptr->param_0)) = editor_ptr->text;
            *((U32 *)msg_ptr->param_1) = editor_ptr->visible_line_info->array[editor_ptr->visible_line_info->line_count].offset * ENCODING_LENGTH;
        }
#endif    
    ...
    case MMI_IMC_MESSAGE_CLIPBOARD_CUT_HIGHLIGHT_STRING:
        ...
        {
            /* Call show to recaculate the editor information after change cursor position */
            wgui_inputs_ml_show_ext(editor_ptr);
            editor_ptr->show_action_flag = 1;
        }
        else
#endif
...
/* under construction !*/
    ...
    case MMI_IMC_MESSAGE_CLIPBOARD_CUT_ALL:
        if (msg_ptr->param_0 == 0)
        {
            ...
            {
                /* If cursor is not in last position, it will not clear all the content due to the WCSS cursor move calback */
                editor_ptr->cursor_p = editor_ptr->last_position_p - ENCODING_LENGTH;
                /* Call show to recaculate the editor information after change cursor position */
                wgui_inputs_ml_show_ext(editor_ptr);
            }
    #endif /* defined(__MMI_WCSS_INPUT_FORMAT_SUPPORT__) */ 
     ...
     case MMI_IMC_MESSAGE_CLIPBOARD_IS_DISABLE:
        if (editor_ptr->ext_flags & GUI_MULTI_LINE_INPUT_BOX_DISABLE_CLIPBOARD)
        {
            lresult = MMI_TRUE;
                 ...
                 ((mmi_editor_handler_struct *)input_box_handle)->input_box_sub_type == MMI_EDITOR_LINE_BASED_IN_MULTLINE_INPUT_BOX)
        {
            if (editor_ptr->ext_flags & GUI_MULTI_LINE_INPUT_BOX_AUTO_SCROLL_MODE)
            {
                lresult = MMI_TRUE;
            }
            else
...
/* under construction !*/
    ...
    wgui_inputs_ml_insert_string_ext( str,  len, is_redraw,&MMI_multiline_inputbox);
...
U32 wgui_inputs_ml_insert_string_ext(UI_string_type str, U32 len, MMI_BOOL is_redraw, multi_line_input_box * editor)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    old_show_action_flag = editor->show_action_flag;
    editor->show_action_flag = 0;
    for (i = 0; i < len; i++)
    {
        if (MMI_TRUE == wgui_inputs_is_input_char_valid((UI_character_type)str[i]))
        {
            ...
            else if (MMI_TRUE == wgui_inputs_ml_direct_input_no_draw_ext((UI_character_type)str[i],editor))
            {
                total_len ++;
            }
            else
    ...
    editor->show_action_flag = old_show_action_flag;
...
void wgui_inputs_ml_disable(void)
{
    ...
    wgui_inputs_ml_disable_ext(&MMI_multiline_inputbox);
...
void wgui_inputs_ml_disable_ext(multi_line_input_box * editor)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    editor->flags |= (UI_MULTI_LINE_INPUT_BOX_STATE_DISABLED | UI_MULTI_LINE_INPUT_BOX_DISABLE_CURSOR_DRAW);
    editor->ext_flags |= GUI_MULTI_LINE_INPUT_BOX_DISABLE_FORCE_CURSOR_APPEAR;
    StopTimer(BLINKING_CURSOR);
    //wgui_inputs_ml_redraw();
}
#endif/*__MMI_MULTILINE_INPUT_BOX_SINGLINE_STYLE_WITH_FTE__*/
...
void wgui_inputs_ml_enable(void)
{
    ...
    wgui_inputs_ml_enable_ext(&MMI_multiline_inputbox);
...
void wgui_inputs_ml_enable_ext(multi_line_input_box * editor)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
MMI_BOOL wgui_inputs_ml_is_disable(void)
{
    ...
    return wgui_inputs_ml_is_disable_ext(&MMI_multiline_inputbox);    
...
MMI_BOOL wgui_inputs_ml_is_disable_ext(multi_line_input_box * editor)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if ((editor->flags & UI_MULTI_LINE_INPUT_BOX_STATE_DISABLED) &&
        (editor->flags & UI_MULTI_LINE_INPUT_BOX_DISABLE_CURSOR_DRAW))
    {
        return MMI_TRUE;
    }
    else
...
void wgui_inputs_ml_move_cursor(wgui_inputs_cursor_pos_enum pos)
{
    ...
    wgui_inputs_ml_move_cursor_ext(pos,&MMI_multiline_inputbox);
...
void wgui_inputs_ml_move_cursor_ext(wgui_inputs_cursor_pos_enum pos,multi_line_input_box * editor)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    {
        editor->cursor_p = editor->last_position_p - ENCODING_LENGTH;
    }
    else
    {
        MMI_ASSERT(0);
...
S32 wgui_inputs_ml_get_scroll_bar_width(void)
{
    ...
    return  wgui_inputs_ml_get_scroll_bar_width_ext(&MMI_multiline_inputbox);
...
S32 wgui_inputs_ml_get_scroll_bar_width_ext(multi_line_input_box * editor)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if ((editor->flags & UI_MULTI_LINE_INPUT_BOX_DISABLE_SCROLLBAR) ||
        ((editor->flags & UI_MULTI_LINE_INPUT_BOX_AUTO_DISABLE_SCROLLBAR) && 
         ((editor->vbar.scale != 0) && 
          (editor->vbar.scale >= editor->vbar.range))))
    {
        return 0;
    }
    else
    {
        return editor->vbar.width;
    }
}
#endif/*__MMI_MULTILINE_INPUT_BOX_SINGLINE_STYLE_WITH_FTE__*/
...
void wgui_inputs_ml_set_decimal_flag(U32 input_type)
{
    ...
    wgui_inputs_ml_set_decimal_flag_ext(input_type,&MMI_multiline_inputbox);
...
void wgui_inputs_ml_set_decimal_flag_ext(U32 input_type,multi_line_input_box * editor)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    
    {
        ...
        *all_editor_data->wgui_inputbox_decimal_flag = 0;
        ...
        if (input_type & IMM_INPUT_TYPE_DECIMAL_NUMERIC ||
            input_type & IMM_INPUT_TYPE_SIGNED_DECIMAL_NUMERIC)
        {
            *all_editor_data->wgui_inputbox_decimal_input_mode_flag = 1;
            if (editor->text_length > 2)
            {
                S32 i;
                UI_buffer_type t = editor->text;
                UI_character_type c;
            
                for (i = 0; i < (editor->text_length / ENCODING_LENGTH); i++)
                {
                    UI_STRING_GET_NEXT_CHARACTER(t, c);
                    if (c == (UI_character_type) '.')
                    {
...
static void wgui_inputs_ml_handle_plus_key(void)
{
    ...
    wgui_inputs_ml_handle_plus_key_ext(&MMI_multiline_inputbox);
...
static void wgui_inputs_ml_handle_plus_key_ext(multi_line_input_box * editor)
{
    ...
    if (editor->text[0] == '+')
    {
        if (editor->text != editor->cursor_p)
        {
            editor->available_length = (wgui_inputbox_buffer_size_ext + 1) * ENCODING_LENGTH;
        }
    }
    ...
    {
        U8 done = 0;
        UI_buffer_type current_position = editor->text;
        UI_buffer_type previous_position = editor->text;
        UI_character_type c;

        editor->available_length = wgui_inputbox_buffer_size_ext * ENCODING_LENGTH;
        if (editor->text_length > editor->allocated_length)
        {
            editor->text_length -= ENCODING_LENGTH;
            UI_STRING_GET_NEXT_CHARACTER(current_position, c);
            while (!done)
            {
                UI_STRING_GET_NEXT_CHARACTER(current_position, c);
            ...
            UI_STRING_GET_PREVIOUS_CHARACTER(previous_position, c);
            editor->last_position_p = previous_position;
            if (editor->cursor_p != editor->text)
            {
                UI_STRING_GET_PREVIOUS_CHARACTER(editor->cursor_p, c);
            }
        }
    }
}
