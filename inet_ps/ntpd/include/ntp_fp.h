 ...
 */
#define  HTONS_FP(x) (soc_htonl(x))
#define  HTONL_FP(h, n) do { (n)->l_ui = soc_htonl((h)->l_ui); \
              (n)->l_uf = soc_htonl((h)->l_uf); } while (0)
#define  NTOHS_FP(x) (soc_htonl(x))
#define  NTOHL_FP(n, h) do { (h)->l_ui = soc_htonl((n)->l_ui); \
              (h)->l_uf = soc_htonl((n)->l_uf); } while (0)
#define  NTOHL_MFP(ni, nf, hi, hf) \
   do { (hi) = soc_htonl(ni); (hf) = soc_htonl(nf); } while (0)
#define  HTONL_MFP(hi, hf, ni, nf) \
   do { (ni) = soc_htonl(hi); (nf) = soc_htonl(hf); } while (0)

/* funny ones. Converts ts fractions to net order ts */
#define  HTONL_UF(uf, nts) \
   do { (nts)->l_ui = 0; (nts)->l_uf = soc_htonl(uf); } while (0)
#define  HTONL_F(f, nts) do { (nts)->l_uf = soc_htonl(f); \
            if ((f) & 0x80000000)                         \
               (nts)->l_i = -1;                           \
            else                                          \
               (nts)->l_i = 0;                            \
