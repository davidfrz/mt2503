...
const mmi_bt_hid_key_control_data_struct hid_control_data_desktop[HID_NUM_OF_DESKTOP_CTRL_DATA] =
{
	{ {0x01, 0x00, 0x00, 0x50, 0x00, 0x00, 0x00, 0x00, 0x00} },//left arrow
	{ {0x01, 0x00, 0x00, 0x4F, 0x00, 0x00, 0x00, 0x00, 0x00} },//right arrow
	{ {0x01, 0x00, 0x00, 0x52, 0x00, 0x00, 0x00, 0x00, 0x00} },//up arrow
	{ {0x01, 0x00, 0x00, 0x51, 0x00, 0x00, 0x00, 0x00, 0x00} },//down arrow
	{ {0x01, 0x00, 0x00, 0x28, 0x00, 0x00, 0x00, 0x00, 0x00} },//enter
	{ {0x03, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00} },//vlo.+

};
#ifndef __MMI_BT_APPLICATION_BQB__
const mmi_bt_hid_key_control_data_struct hid_control_data_media_player[HID_NUM_OF_MEDIA_CTRL_DATA] =
{
	{ {0x03, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00} },//paly/pause
	{ {0x03, 0x20, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00} },//stop
	{ {0x03, 0x10, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00} },//previous song
	{ {0x03, 0x08, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00} },//next song
	{ {0x03, 0x04, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00} },//vol.- 
	{ {0x03, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00} },//vlo.+
};
#else /*__MMI_BT_APPLICATION_BQB__*/
const mmi_bt_hid_key_control_data_struct hid_control_data_mouse_pointer[HID_NUM_OF_MEDIA_CTRL_DATA] =
{
	{ {0x02, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00} },//leftbutton
	{ {0x02, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00} },//rightbutton
	{ {0x02, 0x00, 0x00, 0xF0, 0x00, 0x00, 0x00, 0x00, 0x00} },//up
	{ {0x02, 0x00, 0x00, 0x0F, 0x00, 0x00, 0x00, 0x00, 0x00} },//down
	{ {0x02, 0x00, 0xF0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00} },//left
	{ {0x02, 0x00, 0x0F, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00} },//right
};

#endif /*__MMI_BT_APPLICATION_BQB__*/
...
static MMI_RET mmi_hid_group_proc(mmi_event_struct *post_evt)
{
	...
	case EVT_ID_TOUCHSCREEN_MOVE_RIGHT:
	{
		...
		if (SCR_BT_REMOTE_CTRL_PROGRESS == scrn_id &&
			g_hid_scrn_cntx.gid == group_id)
		{
...
/* under construction !*/
	...
	if(j == 0)
	{
		...
		{
		#ifndef __MMI_BT_APPLICATION_BQB__			
			srv_bt_cm_dev_struct* dev_p = (srv_bt_cm_dev_struct *)srv_bt_cm_get_dev_info(addr);
			if(srv_bt_cm_dev_is_support_hid(dev_p))
		#endif /*__MMI_BT_APPLICATION_BQB__*/
			{
				g_hidd_cntx.last_device = *addr;
...
static MMI_RET mmi_hid_entry_app_select(mmi_scrn_essential_struct *info)
{
    ...
    {
        nItems = 1;
        ItemList[nItems - 1] = (U8*)GetString(STR_BT_REMOTE_CTRL_DESKTOP);
	#ifdef __MMI_BT_APPLICATION_BQB__
		nItems++;
        ItemList[nItems - 1] = (U8*)GetString(STR_BT_REMOTE_CTRL_MOUSE_POINTER);
	#endif /*__MMI_BT_APPLICATION_BQB__*/
    }
    else
    {
...
#ifdef __MMI_HID_PRESENTER__
		nItems++;
        ItemList[nItems - 1] = (U8*)GetString(STR_BT_REMOTE_CTRL_PRESENTER);
#endif
#ifdef __MMI_HID_KEYBOARD__
		nItems++;
        ItemList[nItems - 1] = (U8*)GetString(STR_BT_REMOTE_CTRL_KEYBOARD);
#endif
    }
...
#ifdef __MMI_WEARABLE_DEVICE__
#include "Gui.h"
#define MMI_HID_BUTTON_WIDTH       (120)
#define MMI_HID_BUTTON_HEIGHT      (60)
#define MMI_HID_BUTTON_START_X     (0)
#define MMI_HID_BUTTON_START_Y     (MMI_title_height)

#define MMI_HID_BUTTON_MAX_ITEM    (6)
typedef struct
{
    U16 label_str_id;
	S32 x;
    ...
    S32 w;
    S32 h;
}mmi_hid_button_item_sturct;

typedef struct
{
	U8 count;
	S8 touch_idx;
	hid_element_type_enum type;
    mmi_hid_button_item_sturct item[MMI_HID_BUTTON_MAX_ITEM];
}mmi_hid_button_area_sturct;

mmi_hid_button_area_sturct g_mmi_hid_button_area;
...
*  void
*****************************************************************************/
static void mmi_hid_send_data(S32 touch_index, hid_element_type_enum type)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
	if(type == HID_ELEMENT_DESKTOP)
	{
    	mmi_bth_hidd_interrupt_data_req_hdler(9, (U8 *)hid_control_data_desktop[touch_index].ctrl_data);

	}
	else if(type == HID_ELEMENT_MEDIAPLAYER)
	{
#ifdef __MMI_BT_APPLICATION_BQB__
    	mmi_bth_hidd_interrupt_data_req_hdler(4, (U8 *)hid_control_data_mouse_pointer[touch_index].ctrl_data);
#else/*__MMI_BT_APPLICATION_BQB__*/	
		mmi_bth_hidd_interrupt_data_req_hdler(9, (U8 *)hid_control_data_media_player[touch_index].ctrl_data);
#endif /*__MMI_BT_APPLICATION_BQB__*/	
	}
}
/*****************************************************************************
 * FUNCTION
 *  mmi_hid_button_item_redraw
 * DESCRIPTION
 *  redraw button
 * PARAMETERS
 ...
 *  void
 *****************************************************************************/
static void mmi_hid_button_item_redraw(mmi_hid_button_item_sturct *item, MMI_BOOL IsTouch)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
	...
	if (!(current_active_grp_id == g_hid_scrn_cntx.gid 
		&& current_active_scrn_id == SCR_BT_REMOTE_CTRL_APP)) 
		return;

	gdi_layer_get_base_handle(&base_layer);
	gdi_layer_push_and_set_active(base_layer);
	gdi_layer_lock_frame_buffer();	
	gdi_layer_push_and_set_clip(item->x, item->y, item->x + item->w, item->y + item->h);
	if(IsTouch)
		rect_color = gdi_act_color_from_rgb(255, 255, 150, 0);
	else
		rect_color = gdi_act_color_from_rgb(255, 32, 32, 40);
	gdi_draw_solid_rect(item->x, 
			item->y, 
			item->x + item->w, 
			item->y + item->h, 
			rect_color);
	...
	gui_measure_string(get_string(item->label_str_id), &str_width, &str_height);
	x = item->x + (item->w - str_width) / 2;
	y = item->y + (item->h - str_height) / 2;
	gui_print_truncated_text(x, y, MMI_HID_BUTTON_WIDTH, get_string(item->label_str_id));
	gdi_draw_rect(item->x,
			item->y, 
			item->x + item->w, 
			item->y + item->h,
			GDI_COLOR_GRAY);
	gdi_draw_line(UI_device_width - 1,
		MMI_HID_BUTTON_START_Y, 
		UI_device_width - 1, 
		MMI_HID_BUTTON_START_Y + MMI_HID_BUTTON_HEIGHT * (g_mmi_hid_button_area.count / 2) ,
		GDI_COLOR_GRAY);
	gdi_layer_pop_clip();
	gdi_layer_unlock_frame_buffer();
	gdi_layer_pop_and_restore_active();
	
	gdi_layer_blt(base_layer, NULL, NULL, NULL,
 ...
 *  void
 *****************************************************************************/
static void mmi_hid_set_button_item(U16 item_index, U16 str_id, S32 x, S32 y)
{
	...
	/* Code Body													  */
	/*----------------------------------------------------------------*/
	if(item_index >= MMI_HID_BUTTON_MAX_ITEM 
		|| g_mmi_hid_button_area.count >= MMI_HID_BUTTON_MAX_ITEM)
		return;
	g_mmi_hid_button_area.item[item_index].label_str_id = str_id;
	g_mmi_hid_button_area.item[item_index].x = x;
	g_mmi_hid_button_area.item[item_index].y = y;
	g_mmi_hid_button_area.item[item_index].w = MMI_HID_BUTTON_WIDTH;
	g_mmi_hid_button_area.item[item_index].h = MMI_HID_BUTTON_HEIGHT;
	g_mmi_hid_button_area.count++;

}

/*****************************************************************************
 * FUNCTION
 *  mmi_hid_button_pen_up_hdlr
 * DESCRIPTION
 *  This function is to
 * PARAMETERS
 *  point
 ...
 *  void
 *****************************************************************************/
static void mmi_hid_button_pen_up_hdlr(mmi_pen_point_struct point)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if(g_mmi_hid_button_area.touch_idx < 0 ||g_mmi_hid_button_area.touch_idx >= g_mmi_hid_button_area.count)
    {
        return;
    }
    if(g_mmi_hid_button_area.count > 0)
    {
        mmi_hid_button_item_sturct *item = &g_mmi_hid_button_area.item[g_mmi_hid_button_area.touch_idx];
        if (PEN_CHECK_BOUND(point.x, point.y, item->x, item->y, item->w, item->h))
        {
            mmi_hid_send_data(g_mmi_hid_button_area.touch_idx, g_mmi_hid_button_area.type);
        }

        g_mmi_hid_button_area.touch_idx = -1;

        mmi_hid_button_item_redraw(item, MMI_FALSE);
    }
 ...
 *  void
 *****************************************************************************/
static void mmi_hid_button_pen_down_hdlr(mmi_pen_point_struct point)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
	for (i = 0 ; i < g_mmi_hid_button_area.count; i++)
	{
		if (PEN_CHECK_BOUND(point.x, point.y, g_mmi_hid_button_area.item[i].x, g_mmi_hid_button_area.item[i].y, g_mmi_hid_button_area.item[i].w, g_mmi_hid_button_area.item[i].h))
		{
				g_mmi_hid_button_area.touch_idx = i;
				mmi_hid_button_item_redraw(&g_mmi_hid_button_area.item[i], MMI_TRUE);
				return;
		}
	}
 ...
 *  void
 *****************************************************************************/
static void mmi_hid_button_pen_move_hdlr(mmi_pen_point_struct point)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
 ...
 *  void
 *****************************************************************************/
static void mmi_hid_draw_button_area(hid_element_type_enum type)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
	g_mmi_hid_button_area.type = type;
	//base layer
	gdi_layer_get_base_handle(&base_layer);
	gdi_layer_push_and_set_active(base_layer);
	gdi_layer_lock_frame_buffer();	
	...
	{	 	
		...
		gui_measure_string(get_string(g_mmi_hid_button_area.item[i].label_str_id), &str_width, &str_height);
		x = g_mmi_hid_button_area.item[i].x + (g_mmi_hid_button_area.item[i].w - str_width)/2;
		y = g_mmi_hid_button_area.item[i].y + (g_mmi_hid_button_area.item[i].h - str_height)/2;
        gui_print_truncated_text(x, y, MMI_HID_BUTTON_WIDTH, get_string(g_mmi_hid_button_area.item[i].label_str_id));
		gdi_draw_rect(g_mmi_hid_button_area.item[i].x,
			g_mmi_hid_button_area.item[i].y, 
			g_mmi_hid_button_area.item[i].x + g_mmi_hid_button_area.item[i].w, 
			g_mmi_hid_button_area.item[i].y + g_mmi_hid_button_area.item[i].h,
			GDI_COLOR_GRAY);
	}
	gdi_draw_line(UI_device_width - 1,
		MMI_HID_BUTTON_START_Y, 
	...
	if(type == HID_ELEMENT_DESKTOP)
	{
		title_str_id = STR_BT_REMOTE_CTRL_DESKTOP;
	}
	else if(type == HID_ELEMENT_MEDIAPLAYER)
	{
	#ifdef __MMI_BT_APPLICATION_BQB__
		title_str_id = STR_BT_REMOTE_CTRL_MOUSE_POINTER;
	#else  /*__MMI_BT_APPLICATION_BQB__*/
		title_str_id = STR_BT_REMOTE_CTRL_MEDIA_PLAYER;
	#endif /*__MMI_BT_APPLICATION_BQB__*/
	}	
    MMI_title_string = get_string(title_str_id);
    draw_title_left_align();
	
#if defined(__MMI_TOUCH_SCREEN__)	
	wgui_clear_pen_handlers();

	mmi_pen_register_up_handler(mmi_hid_button_pen_up_hdlr);
	mmi_pen_register_down_handler(mmi_hid_button_pen_down_hdlr);
	mmi_pen_register_move_handler(mmi_hid_button_pen_move_hdlr);
#endif

}
U8 g_hid_index = 0;
void mmi_hid_key_down_handler(void)
{
	mmi_pen_point_struct point;
	
	if(g_hid_index >= MMI_HID_BUTTON_MAX_ITEM)
	{
		g_hid_index = 0;
	}
	point.x = g_mmi_hid_button_area.item[g_hid_index].x+1;
	point.y = g_mmi_hid_button_area.item[g_hid_index].y+1;
	mmi_hid_button_pen_down_hdlr(point);
}

void mmi_hid_key_up_handler(void)
{
	mmi_pen_point_struct point;

	point.x = g_mmi_hid_button_area.item[g_hid_index].x+1;
	point.y = g_mmi_hid_button_area.item[g_hid_index].y+1;
	mmi_hid_button_pen_up_hdlr(point);
	g_hid_index++;
...
static MMI_RET mmi_hid_entry_desktop(mmi_scrn_essential_struct *info)
{
	...
	memset(&g_mmi_hid_button_area, 0x0, sizeof(mmi_hid_button_area_sturct));
	g_mmi_hid_button_area.touch_idx = -1;
	mmi_hid_set_button_item(0, STR_BT_REMOTE_CTRL_LEFT_ARROW, MMI_HID_BUTTON_START_X, MMI_HID_BUTTON_START_Y);
	mmi_hid_set_button_item(1, STR_BT_REMOTE_CTRL_RIGHT_ARROW, 
		MMI_HID_BUTTON_START_X + MMI_HID_BUTTON_WIDTH, MMI_HID_BUTTON_START_Y);
	mmi_hid_set_button_item(2, STR_BT_REMOTE_CTRL_UP_ARROW, 
		MMI_HID_BUTTON_START_X, MMI_HID_BUTTON_START_Y + MMI_HID_BUTTON_HEIGHT);
	mmi_hid_set_button_item(3, STR_BT_REMOTE_CTRL_DOWN_ARROW, 
		MMI_HID_BUTTON_START_X + MMI_HID_BUTTON_WIDTH, MMI_HID_BUTTON_START_Y + MMI_HID_BUTTON_HEIGHT);
	mmi_hid_set_button_item(4, STR_BT_REMOTE_CTRL_ENTER, 
		MMI_HID_BUTTON_START_X, MMI_HID_BUTTON_START_Y + MMI_HID_BUTTON_HEIGHT * 2);

	mmi_hid_set_button_item(5, STR_BT_REMOTE_CAPTURE,
		MMI_HID_BUTTON_START_X + MMI_HID_BUTTON_WIDTH, MMI_HID_BUTTON_START_Y + MMI_HID_BUTTON_HEIGHT *2);

	mmi_hid_draw_button_area(HID_ELEMENT_DESKTOP);
    SetRightSoftkeyFunction(mmi_frm_scrn_close_active_id, KEY_EVENT_UP);
	
	g_hid_index = 0;
	SetLeftSoftkeyFunction(mmi_hid_key_down_handler, KEY_EVENT_DOWN);	
	SetLeftSoftkeyFunction(mmi_hid_key_up_handler, KEY_EVENT_UP);	
 ...
 *  void
 *****************************************************************************/
static MMI_RET mmi_hid_entry_mouse_pointer(mmi_scrn_essential_struct *info)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
	...
	memset(&g_mmi_hid_button_area, 0x0, sizeof(mmi_hid_button_area_sturct));
	g_mmi_hid_button_area.touch_idx = -1;
	mmi_hid_set_button_item(0, STR_BT_REMOTE_CTRL_LEFT_BUTTON, 
		MMI_HID_BUTTON_START_X, MMI_HID_BUTTON_START_Y);
	mmi_hid_set_button_item(1, STR_BT_REMOTE_CTRL_RIGHT_BUTTON, 
		MMI_HID_BUTTON_START_X + MMI_HID_BUTTON_WIDTH, MMI_HID_BUTTON_START_Y);
	mmi_hid_set_button_item(2, STR_BT_REMOTE_CTRL_UP,
		MMI_HID_BUTTON_START_X, MMI_HID_BUTTON_START_Y + MMI_HID_BUTTON_HEIGHT);
	mmi_hid_set_button_item(3, STR_BT_REMOTE_CTRL_DOWN,
		MMI_HID_BUTTON_START_X + MMI_HID_BUTTON_WIDTH, MMI_HID_BUTTON_START_Y + MMI_HID_BUTTON_HEIGHT);
	mmi_hid_set_button_item(4, STR_BT_REMOTE_CTRL_LEFT,
		MMI_HID_BUTTON_START_X, MMI_HID_BUTTON_START_Y + MMI_HID_BUTTON_HEIGHT * 2);
	mmi_hid_set_button_item(5, STR_BT_REMOTE_CTRL_RIGHT, 
		MMI_HID_BUTTON_START_X + MMI_HID_BUTTON_WIDTH, MMI_HID_BUTTON_START_Y + MMI_HID_BUTTON_HEIGHT * 2);

	mmi_hid_draw_button_area(HID_ELEMENT_MEDIAPLAYER);
    SetRightSoftkeyFunction(mmi_frm_scrn_close_active_id, KEY_EVENT_UP);
...
static MMI_RET mmi_hid_entry_media_player(mmi_scrn_essential_struct *info)
{
    ...
    /*----------------------------------------------------------------*/
#ifdef __MMI_BT_APPLICATION_BQB__
	return mmi_hid_entry_mouse_pointer(info);
#else /*__MMI_BT_APPLICATION_BQB__*/
	...
	memset(&g_mmi_hid_button_area, 0x0, sizeof(mmi_hid_button_area_sturct));
	g_mmi_hid_button_area.touch_idx = -1;
	mmi_hid_set_button_item(0, STR_BT_REMOTE_CTRL_PLAY_PAUSE, 
		MMI_HID_BUTTON_START_X, MMI_HID_BUTTON_START_Y);
	mmi_hid_set_button_item(1, STR_GLOBAL_STOP, 
		MMI_HID_BUTTON_START_X + MMI_HID_BUTTON_WIDTH, MMI_HID_BUTTON_START_Y);
	mmi_hid_set_button_item(2, STR_BT_REMOTE_CTRL_PREV_SONG,
		MMI_HID_BUTTON_START_X, MMI_HID_BUTTON_START_Y + MMI_HID_BUTTON_HEIGHT);
	mmi_hid_set_button_item(3, STR_BT_REMOTE_CTRL_NEXT_SONG,
		MMI_HID_BUTTON_START_X + MMI_HID_BUTTON_WIDTH, MMI_HID_BUTTON_START_Y + MMI_HID_BUTTON_HEIGHT);
	mmi_hid_set_button_item(4, STR_BT_REMOTE_CTRL_VOL_DOWN,
		MMI_HID_BUTTON_START_X, MMI_HID_BUTTON_START_Y + MMI_HID_BUTTON_HEIGHT * 2);
	mmi_hid_set_button_item(5, STR_BT_REMOTE_CTRL_VOL_UP, 
		MMI_HID_BUTTON_START_X + MMI_HID_BUTTON_WIDTH, MMI_HID_BUTTON_START_Y + MMI_HID_BUTTON_HEIGHT * 2);
