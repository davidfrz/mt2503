 ...
 * Static Declaration
 *****************************************************************************/
static void ntpd_start_timer(void);
...
static void ntpd_network_init(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/    
	MMI_TRACE(MMI_CONN_TRC_G9_NTPD, FUNC_NTPD_NETWORK_INIT);
	
    mmi_frm_set_protocol_event_handler(MSG_ID_APP_SOC_NOTIFY_IND, ntpd_hdlr_soc_notify_ind, MMI_TRUE);
    //mmi_frm_set_protocol_event_handler(MSG_ID_APP_CBM_BEARER_INFO_IND, ntpd_hdlr_cbm_bearer_info_ind, MMI_FALSE);
    mmi_frm_set_protocol_event_handler(MSG_ID_APP_SOC_GET_HOST_BY_NAME_IND, ntpd_hdlr_get_host_by_name_ind, MMI_TRUE);
}
...
static void ntpd_notify_app(kal_int32 error, l_fp *ts)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_CONN_TRC_G9_NTPD, FUNC_NTPD_NOTIFY_APP);
...
static void ntpd_add_server(sockaddr_struct *serv)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_CONN_TRC_G9_NTPD, FUNC_NTPD_ADD_SERVER);
...
static void ntpd_server_send(void)
{
    ...
    for (; server != NULL; server = server->next_server)
    {
        ...
        {
            MMI_TRACE(MMI_CONN_TRC_G9_NTPD, FUNC_NTPD_SERVER_SEND);
...
static struct server *ntpd_find_server(sockaddr_struct *addr)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_CONN_TRC_G9_NTPD, FUNC_NTPD_FIND_SERVER);
...
static void ntpd_recv_proc(struct recvbuf *rbufp)
{
    ...
    MMI_TRACE(MMI_CONN_TRC_G9_NTPD, FUNC_NTPD_RECV_PROC_NO, 1);
    ...
    {
        MMI_TRACE(MMI_CONN_TRC_G9_NTPD, FUNC_NTPD_RECV_PROC_NO, 3);
        return;
    }
    ...
    if (server == NULL)
    {
        MMI_TRACE(MMI_CONN_TRC_G9_NTPD, FUNC_NTPD_RECV_PROC_NO, 5);
        return;
    }
    ...
    {
        MMI_TRACE(MMI_CONN_TRC_G9_NTPD, FUNC_NTPD_RECV_PROC_NO, 7);
        return;
    }
    ...
    {
        MMI_TRACE(MMI_CONN_TRC_G9_NTPD, FUNC_NTPD_RECV_PROC_NO, 9);
    ...
    MMI_TRACE(MMI_CONN_TRC_G9_NTPD, NTPD_RECV_TIMESTAMP_ORG, org.Ul_i.Xl_i,org.Ul_f.Xl_f);
    MMI_TRACE(MMI_CONN_TRC_G9_NTPD, NTPD_RECV_TIMESTAMP_REC, rec.Ul_i.Xl_i,rec.Ul_f.Xl_f);
    MMI_TRACE(MMI_CONN_TRC_G9_NTPD, NTPD_RECV_TIMESTAMP_SORG, server->org.Ul_i.Xl_i,server->org.Ul_f.Xl_f);
    MMI_TRACE(MMI_CONN_TRC_G9_NTPD, NTPD_RECV_TIMESTAMP_DEST, rbufp->recv_time.Ul_i.Xl_i,rbufp->recv_time.Ul_f.Xl_f);
    ...
    {
        MMI_TRACE(MMI_CONN_TRC_G9_NTPD, FUNC_NTPD_RECV_PROC_NO, 11);
    ...
    MMI_TRACE(MMI_CONN_TRC_G9_NTPD, FUNC_NTPD_RECV_PROC_NO, 20);
}
...
void ntpd_input(kal_int8 soc)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_CONN_TRC_G9_NTPD, FUNC_NTPD_INPUT);
...
static U8 ntpd_hdlr_soc_notify_ind(void *msg)
{
    ...
    MMI_TRACE(MMI_CONN_TRC_G9_NTPD, FUNC_NTPD_HDLR_SOC_NOTIFY_IND, event_type);
...
static U8 ntpd_hdlr_cbm_bearer_info_ind(void *msg)
{
    ...
    MMI_TRACE(MMI_CONN_TRC_G9_NTPD, FUNC_NTPD_HDLR_BEARER_INFO_IND, state);
 ...
 *  void
 *****************************************************************************/
static void ntpd_timer_proc(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if (ntpd_cntx_p->state == NTPD_S_SYNC)
    {
        ...
        StartTimer(NTPD_PROTO_TIMER, 1000, ntpd_timer_proc);
    }
}
...
static void ntpd_start_timer(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_CONN_TRC_G9_NTPD, FUNC_NTPD_START_TIMER);

    StartTimer(NTPD_PROTO_TIMER, 2000, ntpd_timer_proc);
}
...
static int ntpd_send_packet(sockaddr_struct *dest, struct pkt *pkt, int len)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_CONN_TRC_G9_NTPD, FUNC_NTPD_SEND_PACKET);
...
static kal_int32 ntpd_server_sync(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_CONN_TRC_G9_NTPD, FUNC_NTPD_SERVER_SYNC);
    ...
    if (ret == SOC_SUCCESS)
    {
        MMI_TRACE(MMI_CONN_TRC_G9_NTPD, FUNC_NTPD_SERVER_SYNC_NO, 5);
    ...
    {
        MMI_TRACE(MMI_CONN_TRC_G9_NTPD, FUNC_NTPD_SERVER_SYNC_NO, 7);
        ntpd_cntx_p->state = NTPD_S_HOST;
    }
    else
    {
        MMI_TRACE(MMI_CONN_TRC_G9_NTPD, FUNC_NTPD_SERVER_SYNC_NO, 9);
        return NTPD_ERROR;
    }

    MMI_TRACE(MMI_CONN_TRC_G9_NTPD, FUNC_NTPD_SERVER_SYNC_NO, 20);
    return NTPD_SUCCESS;
}
...
static void ntp_transmit_packet(struct server *server)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_CONN_TRC_G9_NTPD, FUNC_NTP_TRANSMIT_PACKET);
    ...
    if ((int)server->filter_nextpt >= ntpd_cntx_p->samples)
    {
        ...
        MMI_TRACE(MMI_CONN_TRC_G9_NTPD, FUNC_NTP_TRANSMIT_PACKET_NO, 1);
    ...
    MMI_TRACE(MMI_CONN_TRC_G9_NTPD, FUNC_NTP_TRANSMIT_PACKET_NO, 20);
}
...
static U8 ntpd_hdlr_get_host_by_name_ind(void *msg)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_CONN_TRC_G9_NTPD, FUNC_NTPD_HDLR_GET_HOST_BY_NAME_IND);
    ...
    if ((host_ind == NULL) || (host_ind->result != KAL_TRUE))
    {
        ...
        MMI_TRACE(MMI_CONN_TRC_G9_NTPD, FUNC_NTPD_HDLR_GET_HOST_NO, 1);
        return 0;
    }
    ...
    {
        /* error handling */
        MMI_TRACE(MMI_CONN_TRC_G9_NTPD, FUNC_NTPD_HDLR_GET_HOST_NO, 3);
        return MMI_FALSE;
    }
    ...
    MMI_TRACE(MMI_CONN_TRC_G9_NTPD, FUNC_NTPD_HDLR_GET_HOST_NO, 20);
            ...
            sockaddr_struct *saddr)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_CONN_TRC_G9_NTPD, FUNC_NTPD_GETHOSTBYNAME);
    ...
    if (ret == KAL_FALSE)
    {
        ...
        MMI_TRACE(MMI_CONN_TRC_G9_NTPD, FUNC_NTPD_GETHOSTBYNAME_NO, soc_cause);
    ...
    {
        MMI_TRACE(MMI_CONN_TRC_G9_NTPD, FUNC_NTPD_GETHOSTBYNAME_NO, 9);
        ret = SOC_ERROR;
    }
    else
    {
        MMI_TRACE(MMI_CONN_TRC_G9_NTPD, FUNC_NTPD_GETHOSTBYNAME_NO, 10);
        /* valid IP address */
        saddr->addr_len = 4;
        ret = SOC_SUCCESS;
    }

    MMI_TRACE(MMI_CONN_TRC_G9_NTPD, FUNC_NTPD_GETHOSTBYNAME_NO, 20);
    return ret;
}
...
static int ntpd_create_socket(unsigned int net_id)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_CONN_TRC_G9_NTPD, FUNC_NTPD_CREATE_SOCKET);

    sd = soc_create(SOC_PF_INET, SOC_SOCK_DGRAM, 0, MOD_MMI, net_id);
...
static int ntpd_socket_bind(kal_uint16 port)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_CONN_TRC_G9_NTPD, FUNC_NTPD_SOCKET_BIND);
    ...
    if (soc_id < 0)
    {
        MMI_TRACE(MMI_CONN_TRC_G9_NTPD, FUNC_NTPD_SOCKET_BIND_NO, soc_id);
        return soc_id;
    }
    ...
    if (ret < 0)
    {
        MMI_TRACE(MMI_CONN_TRC_G9_NTPD, FUNC_NTPD_SOCKET_BIND_NO, 31);
        return ret;
    }
...
kal_int32 ntpd_open(struct ntpd_config_struct *config)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_CONN_TRC_G9_NTPD, FUNC_NTPD_OPEN);

    if (config == NULL)
    {
        MMI_TRACE(MMI_CONN_TRC_G9_NTPD, FUNC_NTPD_OPEN_NO, 1);
        return NTPD_ERROR;
    }

    if (ntpd_cntx_p->state != NTPD_S_IDLE)
    {
        MMI_TRACE(MMI_CONN_TRC_G9_NTPD, FUNC_NTPD_OPEN_NO, 3);
        return NTPD_SUCCESS;
    }
    ...
    {
        MMI_TRACE(MMI_CONN_TRC_G9_NTPD, FUNC_NTPD_OPEN_NO, 5);
        return NTPD_ERROR;
    }

    MMI_TRACE(MMI_CONN_TRC_G9_NTPD, FUNC_NTPD_OPEN_NO, 20);
    return NTPD_SUCCESS;
}
...
kal_int32 ntpd_sync(ntpd_sync_cb callback, void *usr_data)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_CONN_TRC_G9_NTPD, FUNC_NTPD_SYNC);
    ...
    {
        MMI_TRACE(MMI_CONN_TRC_G9_NTPD, FUNC_NTPD_SYNC_NO, 3);
        return NTPD_ERROR;
    }
    ...
    if (ret == SOC_SUCCESS)
    {
        MMI_TRACE(MMI_CONN_TRC_G9_NTPD, FUNC_NTPD_SYNC_NO, 5);
        ntpd_add_server(&(ntpd_cntx_p->serv_addr));
        ntpd_cntx_p->state = NTPD_S_SYNC;
        ntpd_server_send();
    }
    ...
    {
        MMI_TRACE(MMI_CONN_TRC_G9_NTPD, FUNC_NTPD_SYNC_NO, 7);
        ntpd_cntx_p->state = NTPD_S_HOST;
    }
    else
    {
        MMI_TRACE(MMI_CONN_TRC_G9_NTPD, FUNC_NTPD_SYNC_NO, 9);
        return NTPD_ERROR;
    }

    MMI_TRACE(MMI_CONN_TRC_G9_NTPD, FUNC_NTPD_SYNC_NO, 20);
    return NTPD_SUCCESS;
}
...
kal_int32 ntpd_sync_abort(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_CONN_TRC_G9_NTPD, FUNC_NTPD_SYNC_ABORT);
...
void ntpd_close(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_CONN_TRC_G9_NTPD, FUNC_NTPD_CLOSE);
...
void ntpd_clock_filter(struct server * peer, s_fp delay, l_fp * offset,  u_fp dispersion)
{
    ...
    MMI_TRACE(MMI_CONN_TRC_G9_NTPD, NTPD_ENTER_CLOCK_FILTER, peer->refid, peer->filter_nextpt);
    MMI_TRACE(MMI_CONN_TRC_G9_NTPD, NTPD_CLOCK_FILTER_OFFSET, offset->Ul_i.Xl_i,offset->Ul_f.Xl_f);
...
/* under construction !*/
    ...
    MMI_TRACE(MMI_CONN_TRC_G9_NTPD,NTPD_PEER_DISPERSION,peer->dispersion);
    if(peer->dispersion > PEER_MAXDISP)
    {
        peer->dispersion = PEER_MAXDISP;
    }
    MMI_TRACE(MMI_CONN_TRC_G9_NTPD, NTPD_CLOCK_FILTER_OFFSET, peer->offset.Ul_i.Xl_i,peer->offset.Ul_f.Xl_f);
...
int ntpd_clock_intersection_selection(int * top, int * bottom)
{
    ...
    MMI_TRACE(MMI_CONN_TRC_G9_NTPD, NTPD_ENTER_CLOCK_INTERSECTION);
    for( ; temp_svr_list != NULL; temp_svr_list = temp_svr_list->next_server)
    {
        ...
        MMI_TRACE(MMI_CONN_TRC_G9_NTPD, NTPD_REACH_NUM,temp_svr_list->reach);
    ...
    if(num_of_survivor <= 0)
    {
        ...
        MMI_TRACE(MMI_CONN_TRC_G9_NTPD, NTPD_CLOCK_INTERSECTION_NO_SERVER);
        return -1;
    }
    ...
    *top = endpoint_vector[endpoint_index-1].value;
    ...
    MMI_TRACE(MMI_CONN_TRC_G9_NTPD,NTPD_CLOCK_INTERSECTION_BOUNDARY,*bottom,*top);
    return 0;
}
...
int ntpd_clock_clustering_selection(int top, int bottom, l_fp * offset)
{
    ...
    MMI_TRACE(MMI_CONN_TRC_G9_NTPD, NTPD_ENTER_CLOCK_CLUSTER_SELECT);
    for( ; temp_svr_list != NULL; temp_svr_list = temp_svr_list->next_server)
    {
    ...
    if(count == 0)
    {
        MMI_TRACE(MMI_CONN_TRC_G9_NTPD, NTPD_CLOCK_CLUSTER_SELECT_NO_SERVER);
    ...
    if(ntpd_cntx_p->ntpd_sys_peer != candidate_vector[0].peer_ptr)
    {
        if(ntpd_cntx_p->ntpd_sys_peer == NULL)
        {
            ...
            //MMI_TRACE(MMI_CONN_TRC_G9_NTPD,NTPD_SELECT_SYNC_SERVER, ntpd_cntx_p->ntpd_sys_peer->refid);
        }
...
void ntpd_clock_select(l_fp * offset)
{
    ...
    MMI_TRACE(MMI_CONN_TRC_G9_NTPD, NTPD_ENTER_CLOCK_SLECT);
    ...
    if(intersection_result == -1)
    {
        ...
        MMI_TRACE(MMI_CONN_TRC_G9_NTPD,NTPD_CLOCK_SELECT_OFFSET,offset->Ul_i.Xl_i,offset->Ul_f.Xl_f);
        return;
    }
