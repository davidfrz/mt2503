...
# To check FLAVOR from project.mak and FLAVOR from command are the same or not.
if ((defined($flavor_from_command)) && ("$flavor_from_command" ne "")) {
  if ("$flavor_from_command" ne "$flavor") {
    print "FLAVOR from project makefile: $flavor\n";
    print "FLAVOR from command line: $flavor_from_command\n";
    die "Error: FLAVOR is not match, please check your command or project makefile.";
  }
}
...
{
  print F "XGC=TRUE \n";
  if ((lc($action) eq "bootloader") || (lc($action) eq "custpack") || (lc($action) eq "tiny_system") || (lc($action) eq "ts_new") || (lc($action) eq "ts_bm_new"))
  {
...
foreach my $action (@theAct) {
  ...
  if ($action eq "clean") {
    if ($#arguments != -1) {
      ...
      if ($build_ubin_only eq "TRUE") {
        system("echo DO_CLEAN_MODULE=TRUE > ${makeFolder}~cleanmod.tmp");
        system("echo CLEAN_MODS=ALL >> ${makeFolder}~cleanmod.tmp");
        system("${makeCmd} -f${makeFolder}${myMF} -r -R CUSTOMER=$custom PROJECT=$project cleanmod");
      } else {
...
sub error_handle
{
  if ($build_ubin_only eq "TRUE") {
    print "$action\n";
    print "build_ubin_only = $build_ubin_only\n";
    ...
    foreach (@files) {
      ...
      if (-d "$bin_file") {
        opendir (DIR, $bin_file) or die "no folder : $bin_file\n";
        my @file = grep {/.+\.bin$/}  readdir DIR;
        close DIR;
        foreach (@file) {
          my $ubin_file = $bin_file."\\$_";
          if (-e "$ubin_file" && $ubin_file =~ /UBIN/i) {
            system ("del /F /Q $ubin_file");
#            print "del $ubin_file\n";
            $flag = 1;
          }
        }
      } elsif(-e "$bin_file" && $bin_file =~ /UBIN/i) {
        system ("del /F /Q $bin_file");
#        print "del $bin_file\n";
        $flag = 1;
      }
    }
    print "Some errors happened during the build process. Delete the update binary file\n" if($flag);
  } 
  elsif(("custpack" ne lc $action) && ("theme_bin" ne lc $action)) {
    print "$action\n";
    my $build_folder = ".\\build\\$custom";
    opendir (DIR, $build_folder) or die "no folder : $build_folder\n";
    my @files = grep {/.+\.bin$/}  readdir DIR;
    ...
    print "skip delete the binarry file while action is custpack or theme_bin\n";
  }
  if ($check_depend)
  {
    my $res = system("perl tools\\ChkDepMod.pl --step 1 $custom $project $platform");
