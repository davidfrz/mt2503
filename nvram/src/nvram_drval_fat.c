...
//    kal_wchar src_path[NVRAM_MAX_PATH_LEN], dest_path[NVRAM_MAX_PATH_LEN];
    kal_wchar a[NVRAM_MAX_PATH_LEN], b[NVRAM_MAX_PATH_LEN];
    kal_wchar *src_path, *dest_path;
    ...
    nvram_query_file_name(nvram_query_folder_index(ldi->category), nvramname, a);

#ifdef __NVRAM_BACKUP_DISK_FAT__
    if (NVRAM_IS_ATTR_BACKUP_FAT(ldi->attr))
    {
        nvram_util_make_lid_filename(ldi, nvramname, KAL_TRUE);
        nvram_query_file_name(NVRAM_NVD_BAK, nvramname, b);
    }
    else
#endif
    {
        nvram_util_make_lid_filename(ldi, nvramname, KAL_FALSE);
        nvram_query_file_name(nvram_query_folder_index(ldi->category), nvramname, b);
    }
    ...
    {
        nvram_util_make_lid_filename(ldi, nvramname, KAL_TRUE);
        nvram_query_file_name(nvram_query_folder_index(ldi->category), nvramname, src_path);
        ...
        {
            nvram_util_make_lid_filename(ldi, nvramname, KAL_FALSE);
            nvram_query_file_name(nvram_query_folder_index(ldi->category), nvramname, dest_path);
        }
    }
    else
    {
            ...
            nvram_attr_enum attr)
{
    ...
    /* read one record at once at seperate into many block */
    if (section_size > working_buffer_size)
    {
        ...
        reserve_buffer_size = NVRAM_MSP_ALIGNMENT;      //16

        for (i = 0 ; i < rec_amount; i++)
        {
            chksum1 = chksum2 = chksum3 = 0;
            ...
            for (j = 0 ; j < rec_size; j += working_buffer_size)
            {
                ...
                if (byte_in_block > working_buffer_size)
                {
                    ...
                    if (FS_NO_ERROR > (result = FS_Read(hFile, working_buffer, byte_in_block, &len)))
                    {
                        nvram_fat_last_line = __LINE__;
                        goto final;
                    }
                ...
                {
                    if (FS_NO_ERROR > (result = FS_Read(hFile, working_buffer, byte_in_block + remainLen + NVRAM_CHKSUM_SIZE, &len)))
                    {
                        nvram_fat_last_line = __LINE__;
                        goto final;
                    }
                ...
                if (attr & NVRAM_ATTR_MSP)
                {
                    ...
                    SST_Secure_Algo(NVRAM_MSP_DECRYPT, (kal_uint32)working_buffer, byte_in_block, nvram_ptr->secret_key, working_buffer);
                }
            #endif
                ...
                chksum1 =  ((chksum1 & 0xFF00) + (tmpchksum & 0xFF00)) | ((chksum1 + tmpchksum) & 0xFF);

                if (last_block)
                {
                    //get checksum from the data
                    chksum2  = *(kal_uint8*) (working_buffer + byte_in_block);
                    chksum2 |= *(kal_uint8*) (working_buffer + byte_in_block + 1) << 8;
                    if ((chksum1 != chksum2))
                    {
                    ...
                    //if  (type == NVRAM_DRV_READ_TYPE_CHKSUM)
                    //{
                    //   kal_mem_cpy(buffer + (NVRAM_CHKSUM_SIZE * i), &chksum2, NVRAM_CHKSUM_SIZE);
                    //}
                }

                if (attr & NVRAM_ATTR_CONFIDENTIAL)
                {
                    custom_nvram_decrypt(nvram_ptr->secret_key, working_buffer, byte_in_block, rec_size);
                }
                /* checksum after decryption*/
                tmpchksum = nvram_util_caculate_checksum(working_buffer, byte_in_block);
                chksum3 =  ((chksum3 & 0xFF00) + (tmpchksum & 0xFF00)) | ((chksum3 + tmpchksum) & 0xFF);

                if  (last_block && type == NVRAM_DRV_READ_TYPE_CHKSUM)
                {
                    kal_mem_cpy(buffer + (NVRAM_CHKSUM_SIZE * i), &chksum3, NVRAM_CHKSUM_SIZE);
                }

                if (type == NVRAM_DRV_READ_TYPE_DATA)
                {
                    kal_mem_cpy(buffer + rec_size * i +  j, working_buffer, byte_in_block);
                }
            }
        }
    }
    else
#endif /* __NVRAM_LARGE_RECORD_SIZE__ */
    {
        ...
        for (i = 0 ; i < rec_amount; )
        {
            ...
            if (FS_NO_ERROR > (result = FS_Read(hFile, working_buffer, (kal_uint32) (section_size * rec_in_block), &len)))
            {
                nvram_fat_last_line = __LINE__;
                goto final;
            }
...
*/
            result = FS_Write(
                        hFile,
                        (void *)(buffer + ((attr & NVRAM_ATTR_MULTI_DEFAULT) ? (i*rec_size) : 0)),
                        rec_size,
                        &len);
        ...
        for (i = 0 ; i < rec_amount; i++)
        {
            for (j = 0; j < rec_size; j += working_buffer_size)
            {
                ...
                nvram_drv_fat_prepare_data(
                    working_buffer,
                    buffer,
                    ((attr & NVRAM_ATTR_MULTI_DEFAULT) ? (i*rec_size) : 0) + j,
                    byte_in_block);
