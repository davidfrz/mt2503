...
#define print_bootup_trace(i) \
        do {  \
            extern void dbg_print_bootup_trace(char *fmt,...); \
            if ((i < SST_INIT_STR_NUM) && (i == system_boot_up_trace[i].trace_id)) {  \
                dbg_print_bootup_trace(system_boot_up_trace[i].p_string); \
            }  \
        } while (0)
...
#define print_bootup_trace_enter(i) \
        do {  \
            ...
            extern void dbg_print_bootup_trace(char *fmt,...); \
            if ((i##_ENTER < SST_INIT_STR_NUM) && (i##_ENTER == system_boot_up_trace[i##_ENTER].trace_id)) {  \
                init_duration = drv_get_current_time(); \
                dbg_print_bootup_trace(system_boot_up_trace[i##_ENTER].p_string); \
            }  \
        } while (0)
...
#define print_bootup_trace_exit(i) \
        do {  \
            extern kal_uint32 init_duration; \
            extern void dbg_print_bootup_trace(char *fmt,...); \
            if ((i##_EXIT < SST_INIT_STR_NUM) && (i##_EXIT == system_boot_up_trace[i##_EXIT].trace_id)) {  \
                init_duration = drv_get_duration_ms(init_duration); \
                dbg_print_bootup_trace(system_boot_up_trace[i##_EXIT].p_string, init_duration); \
            }  \
        } while (0)
...
#define print_bootup_trace_with_info_enter(i, info) \
        do {  \
            ...
            if ((i##_ENTER < SST_INIT_STR_NUM) && (i##_ENTER == system_boot_up_trace[i##_ENTER].trace_id)) {  \
                init_duration = drv_get_current_time(); \
                dbg_print_bootup_trace(system_boot_up_trace[i##_ENTER].p_string, (kal_uint32)(info)); \
            }  \
        } while (0)
...
#define print_bootup_trace_with_info_exit(i, info) \
        do {  \
            ...
            if ((i##_EXIT < SST_INIT_STR_NUM) && (i##_EXIT == system_boot_up_trace[i##_EXIT].trace_id)) {  \
                init_duration = drv_get_duration_ms(init_duration); \
                dbg_print_bootup_trace(system_boot_up_trace[i##_EXIT].p_string, (kal_uint32)(info), init_duration); \
            }  \
        } while (0)

#else //#if defined(__DUAL_TALK_MODEM_SUPPORT__)
#define print_bootup_trace(i) \
        do {  \
...
#endif //#if defined(__DUAL_TALK_MODEM_SUPPORT__)
