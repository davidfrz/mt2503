...
static VMBOOL vm_res_is_need_release(VMWCHAR* FileName)
{
	vm_exec_format_enum format = vm_parse_exec_format(FileName);
...
void vm_res_free_res(_vm_pcb_t* pcb)
{
	...
	if (pcb->resfileHandle > 0) 
	{
		if (pcb->res_data.str_buf != NULL)
		{
			...
			if(vm_res_is_need_release(pcb->fullFileName))
		...
		if(vm_res_is_need_release(pcb->fullFileName) && pcb->fileHandle < 0)
		{
		...
		while ( (sm_ptr = _vm_sm_list_find(SM_FIND_BY_PCB, (VMINT)pcb)) != NULL )
		{
			if (sm_ptr->resfileHandle > 0) 
			{
				if (sm_ptr->res_data.str_buf != NULL)
				{
					...
					if(vm_res_is_need_release(sm_ptr->fullFileName))
						vm_res_id_delete_all(0);
					else
						vm_res_id_delete_all(1);
...
VMINT32 vm_res_delete(VMUINT32 id)
{
    ...
    while (cur_node != NULL)
    {
        if (cur_node->id == id)
        {
			...
			if((pcb != NULL && vm_res_is_need_release(pcb->fullFileName)) || (sm_ptr != NULL && vm_res_is_need_release(sm_ptr->fullFileName)))
            {
