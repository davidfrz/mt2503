...
foreach $line (<INPUT>)
{
	...
	if( !($line =~ m/#/))
	{
		if($line =~ m/PARTIAL_PKG/i && $line =~ m/MBA/i)
		{
			$partial_MBA = true;
		}
		elsif($line =~ m/PARTIAL_PKG/i && $line =~ m/UBIN/i)
		{
			$partial_UBIN = true;
		}
...
foreach $line (<INPUT>)
{
	...
	elsif($line =~ /Serial Flash/i || $line =~ /General Setting/i || $line =~ /NOR Flash/i)
	{
		print OUTPUT $line;
	}
	...
	elsif($line =~ /Bootloader/i || $line =~ /Ext-Bootloader/i || $line =~ /TS_ROM/i )
	{
		$skip_line=2;
	}
	elsif($partial_MBA  &&( $line =~ /\[.*\]/i ) &&( $line !~ /Resource Bin/i ))
	{
		$skip_line=2;
	}
	elsif($partial_UBIN &&( $line =~ /\[.*\]/i ) &&( $line !~ /UBIN/i ))
	{
		$skip_line=2;
	}
