...
void CHRDET_HISR(void)
{
	...
	if (state == DETECTCHRIN)
	{
		...
		#if defined(__TINY_SYS__)
		// When cable in detected, block tiny mode
		// The purpose is to NOT allow system enter Tiny mode
		// Because in Tiny mode, there is NO BMT driver to monitor charging states
		// This may cause VBAT drop to low level in Tiny mode
		AP_BtSS_mode_block(SUSPEND, AP_BTSS_MODE_BLOCKER_BMT);
		#endif // #if defined(__TINY_SYS__)
	...
	else
	{
		...
		// Because in Tiny mode, there is NO BMT driver to monitor charging states
		// This may cause VBAT drop to low level in Tiny mode
		AP_BtSS_mode_unblock(SUSPEND, AP_BTSS_MODE_BLOCKER_BMT);
		#endif // #if defined(__TINY_SYS__)
	}
...
#if !defined(CHR_DET_EINT_PIN)
#define CHR_DET_EINT_PIN PMIC_CHR_EINT_PIN
#endif //#if !defined(CHR_DET_EINT_PIN)

#if defined(__TINY_SYS__)
extern void IRQ_Register_Special_Eint(kal_uint8 eint_no);
#endif
...
#if defined(__TINY_SYS__)
	IRQ_Register_Special_Eint(gCHRDET_EINT_NO);
#endif //#if defined(__TINY_SYS__)
...
#if defined(__TINY_SYS__)
	IRQ_Register_Special_Eint(chr_usb_detect.chr_usb_eint);
#endif //#if defined(__TINY_SYS__)
...
#if defined(__TINY_SYS__)
	IRQ_Register_Special_Eint(gCHRDET_EINT_NO.chr_usb_eint);
#endif //#if defined(__TINY_SYS__)
...
#if defined(__TINY_SYS__)
	IRQ_Register_Special_Eint(g_usb_eint_no);
#endif //#if defined(__TINY_SYS__)
