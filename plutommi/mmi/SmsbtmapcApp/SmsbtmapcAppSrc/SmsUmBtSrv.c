        ...
        S32 user_data)
{
	...
	kal_prompt_trace(MOD_MMI, "[SMS],srv_sms_um_bt_get_msg_info req->msg_number = %d", req->msg_number);
	...
	else
	{
		...
		for (i = 0; i < rsp->msg_number; i++)
		{
			...
			else
			{
                ...
                kal_prompt_trace(MOD_MMI, "[SMS],srv_sms_um_info_add = %d", rsp->msg_number);
            ...
            U16 msg_number)
{
    ...
    S32 i;
	srv_sms_um_box_identity_struct msg_box_ext;

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    msg_box_ext.msg_box_type = msg_box.msg_box_type;
    msg_box_ext.msg_type = msg_box.msg_type;
    msg_box_ext.sim_id = SRV_UM_SIM_ALL;
#if 0
/* under construction !*/
/* under construction !*/
/* under construction !*/
...
/* under construction !*/
    ...
    for (i = 0; i < 3; i++)
    {
        ...
        if (list_type == SRV_UM_LIST_TYPE_MSG_NODE && !srv_sms_um_bt_search(SRV_UM_LIST_TYPE_MSG_NODE, (srv_um_msg_node_struct*)msg_list + peek_start_entry))
        {
            found = MMI_FALSE;
        }
        else if (list_type == SRV_UM_LIST_TYPE_THREAD && !srv_sms_um_bt_search(SRV_UM_LIST_TYPE_THREAD, (srv_um_thread_id_struct*)msg_list + peek_start_entry))
        {
            found = MMI_FALSE;
        }
        else
        ...
        if (!found)
        {
            ...
            if (list_type == SRV_UM_LIST_TYPE_MSG_NODE)
            {
                //peek_msg_pid = srv_sms_um_bt_get_msg_info(msg_box, (srv_um_msg_node_struct*)msg_list + peek_start_entry, peek_msg_number, srv_um_peek_msg_callback, 0);
                srv_sms_um_bt_get_msg_info(msg_box_ext, (srv_sms_um_bt_msg_node_struct*)msg_list + peek_start_entry, peek_msg_number, srv_um_peek_msg_callback, 0);
        ...
        srv_um_bt_msg_info_struct *msg_info[])
{
	...
	for (i = msg_number - 1; i >= 0; i--)
    {
        ...
        {
            #ifdef __MMI_WEARABLE_DEVICE__   
            srv_um_bt_peak_msg(msg_box, SRV_UM_LIST_TYPE_MSG_NODE, msg_list, list_info_size, start_entry, msg_number);
            #endif			
            return SRV_UM_RESULT_DATA_NOT_FOUND;
        }
    }
