...
#if defined(__TINY_SYS__)
kal_uint32  warm_boot_get_seed=0;
#endif
...
CSCI_EXTERN_VAR(kal_uint32,tiny_record_seed);
#endif
...
extern kal_uint32	sys_start_time;
#endif
...
{
#if defined(__TINY_IOT_FAST_BOOT__) && defined(__INIT_LCM_IN_BOOTLOADER__)
	sys_start_time = drv_get_current_time();
#endif
...
static void HWDInitialization(void)
{
    ...
    {
        CSCI_ZI_Region_Init();    //Static CSCI init
        csci_init(1);             //Dynamic CSCI init
    }
    else
        csci_init(0);
#else
...
void systemInitializeResource(void)
{
   ...
   p_mmi_symblo_addr = (unsigned int*)(custom_get_UPDATE_ROM_entry() + UBIN_SYMBOL_INIT_RESOURCE_VARS * sizeof(void *));
   ((kal_uint32 (*)(void))(*p_mmi_symblo_addr))();
#else
...
#if defined(__TINY_SYS__)
   if(KAL_TRUE==Is_warm_boot_before_region_init())
   {
      warm_boot_get_seed=tiny_record_seed;
      return tiny_record_seed;
   }
#endif
...
kal_bool INT_GetRID(kal_char *pRid, kal_int32 buf_len)
{
   //The length of requested ID must be multiple of 8
   //SSS_ASSERT(buf_len>0 && buf_len <= 128 && buf_len%8==0);

    kal_mem_cpy(pRid, g_pass_info.m_bl_maui_share_data.m_me_identity, \
         (buf_len >> 3) < sizeof(g_pass_info.m_bl_maui_share_data.m_me_identity) ? \
         (buf_len >> 3) : sizeof(g_pass_info.m_bl_maui_share_data.m_me_identity) );
