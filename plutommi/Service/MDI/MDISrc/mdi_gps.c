                        ...
                        MMI_BOOL use_tls)
{
    ...
    local_param_ptr->account_id = CBM_DEFAULT_ACCT_ID;
...
void conn_gps_AT_callback(app_command_struct *param, void *user_data)
{
    if (strstr(param->command_buf, "SUPLON"))
  	{
        mdi_gps_set_config(1, "supl.nokia.com", 7275, 1);
  	}
  	else if (strstr(param->command_buf, "SUPLOFF"))
  	{
        mdi_gps_set_config(0, "supl.nokia.com", 7275, 1);
  	}
}

void gps_fota_AT_callback(app_command_struct *param, void *user_data)
{
    if (strstr(param->command_buf, "GNSSUPDATE"))
    {
#if defined(__FUNET_ENABLE__) && !defined(__UBL__)
        GPS_FW_TriggerDL(NULL);
...
mmi_ret mdi_gps_uart_init(void)
{
    ...
    for(i=0 ; i<MDI_GPS_UART_PORT_MAX ; i++)
    {
        ...
        for(j=0 ; j<MDI_GPS_UART_APP_PER_PORT_MAX ; j++)
        {
			...
			g_mdi_gps_cntx.handle[i].gps_uart_debug_raw_cb[j] = NULL;
    ...
    void (*gps_uart_event_callback)(mdi_gps_parser_info_enum type, void *buffer, U32 length))
{
    ...
    else if(mode == MDI_GPS_UART_MODE_DEBUG_RAW_DATA)
    {
        for (j = 0; j < MDI_GPS_UART_APP_PER_PORT_MAX; j++)
        {
            if (g_mdi_gps_cntx.handle[index].gps_uart_debug_raw_cb[j] == gps_uart_event_callback &&
                g_mdi_gps_cntx.handle[index].mod_id[j] == module_id)
            {
                /*this callback function is in the list and module is same too*/
                return MMI_TRUE;
        ...
        for (j = 0; j < MDI_GPS_UART_APP_PER_PORT_MAX; j++)
        {
            if (g_mdi_gps_cntx.handle[index].gps_uart_debug_raw_cb[j] != NULL &&
                g_mdi_gps_cntx.handle[index].mod_id[j] == module_id)
            {
            /*find a mod use uart so no need send request*/
                is_need_send_to_gps = MMI_FALSE;
        ...
        for (j = 0; j < MDI_GPS_UART_APP_PER_PORT_MAX; j++)
        {
            if ((g_mdi_gps_cntx.handle[index].gps_uart_lct_cb[j] != NULL ||
                g_mdi_gps_cntx.handle[index].gps_uart_lct_qop_cb[j] != NULL) &&
                g_mdi_gps_cntx.handle[index].gps_uart_debug_raw_cb[j] == NULL &&
                g_mdi_gps_cntx.handle[index].mod_id[j] == module_id)
            {
                g_mdi_gps_cntx.handle[index].gps_uart_debug_raw_cb[j] = gps_uart_event_callback;
                func_ret = MMI_TRUE;
                break;
            }
        }
    ...
    MDI_RESULT *ret)
{
    ...
    if (*ret == MDI_RES_GPS_UART_SUCCEED)
    {
        ...
        {
            g_mdi_gps_cntx.handle[index].gps_uart_debug_raw_cb[0] = gps_uart_event_callback;
        }
        else if(mode == MDI_GPS_UART_MODE_LOCATION_WITH_QOP)
        {
    ...
    for (index= 0 ; index < MDI_GPS_UART_PORT_MAX; index++)
    {
        ...
        if (g_mdi_gps_cntx.handle[index].port == port)
        {
            for (i=0 ; i< MDI_GPS_UART_APP_PER_PORT_MAX ; i++)
            {
                ...
                else if (g_mdi_gps_cntx.handle[index].gps_uart_debug_raw_cb[i] == gps_uart_event_callback &&
                    g_mdi_gps_cntx.handle[index].mod_id[i] == module_id &&
                    mode == MDI_GPS_UART_MODE_DEBUG_RAW_DATA)
                {
                    g_mdi_gps_cntx.handle[index].gps_uart_debug_raw_cb[i] = NULL;
                    closed_mode = MDI_GPS_UART_MODE_DEBUG_RAW_DATA;
                    MMI_TRACE(MMI_MEDIA_TRC_G2_APP, MDI_GPS_UART_TRC_SET_CLOSE_MODE, closed_mode);
                }
            ...
            for (i=0 ; i< MDI_GPS_UART_APP_PER_PORT_MAX ; i++)
            {
                ...
                else if (g_mdi_gps_cntx.handle[index].gps_uart_debug_raw_cb[i] != NULL &&
                    g_mdi_gps_cntx.handle[index].mod_id[i] == module_id &&
                    closed_mode == MDI_GPS_UART_MODE_DEBUG_RAW_DATA)
                {
                    is_need_close = MMI_FALSE;
       ...
       if (g_mdi_gps_cntx.handle[i].port == port)
        {
            ...
            for(j=0 ; j<MDI_GPS_UART_APP_PER_PORT_MAX ; j++)
            {
                if ((g_mdi_gps_cntx.handle[i].gps_uart_raw_cb[j] != NULL || g_mdi_gps_cntx.handle[i].gps_uart_debug_raw_cb[j] != NULL) &&
                    ...
                    type == MDI_GPS_PARSER_RAW_DATA)
                {
					...
					else if (g_mdi_gps_cntx.handle[i].gps_uart_debug_raw_cb[j] != NULL)
					{
						g_mdi_gps_cntx.handle[i].gps_uart_debug_raw_cb[j](MDI_GPS_PARSER_RAW_DATA,buffer, length);
					}
                }
