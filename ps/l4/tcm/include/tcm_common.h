...
/* Poying: 
 * - In an IPv6 project, numbers of GPRS_MAX_PDP_SUPPORT and MAX_EXT_PDP_CONTEXT 
 *   will be set to 6 to handle the fallback case, so we need to modify them back
 *   to 3 to fit the UT cases.
 * - Note: 
 *   1. The fallback case exists only in the IPv4v6 case.
 *   2. In an project with IPv4v6, currently MAX_EXT_PDP_CONTEXT is defined to 6 in Option.mak.
 */
#if (defined(__TCM_UT__) && defined(__IPV4V6__)) 
    #define TCM_TOT_NSAPI           3 //(GPRS_MAX_PDP_SUPPORT/2)
...
/* Poying: Should be removed if no users. */
#if 1
...
#define TCM_MAX_NSAPI               MAX_VALID_NSAPI    // nsapi_session_enum in "mmi_l3_enums.h"
...
/* Poying: Revise name from mmi to tcm. Reuse l4c's struct. */
...
/* Poying: Revise struct name from mmi to tcm. */
typedef struct
{
...
/* under construction !*/
    ...
    tcm_tft_struct tft_in_use;  /* Traffic flow template currently
                                 * being used by a context.
                                 */
    tcm_tft_struct tft_under_mod;/* Traffic flow template to which the
...
#if ( (defined(__NDIS_SUPPORT__)) || (defined(__TCM_UT__)) ||(defined(__EXT_PDP_CONTEXT_ON__)))
...
extern tcm_packet_filter_struct *tcm_free_list(
                                tcm_packet_filter_struct *l_ptr );

extern tcm_packet_filter_struct *tcm_delete_list(
                                tcm_packet_filter_struct *d_ptr,
                                tcm_packet_filter_struct *s_ptr );

extern void tcm_append_list( tcm_packet_filter_struct **d_ptr,
                             tcm_packet_filter_struct **s_ptr,
			     kal_bool replace );
...
#if ( (defined(__NDIS_SUPPORT__)) || (defined(__TCM_UT__) && defined(__UMTS_RAT__))||(defined(__EXT_PDP_CONTEXT_ON__)) ) // only in 3G load we UT on TCM
