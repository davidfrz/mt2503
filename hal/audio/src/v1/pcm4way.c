...
static void PCMNWAY_Common_Start (void (*pcmnway_dl_hdlr)(void), void (*pcmnway_ul_hdlr)(void), uint32 state, uint32 type, uint32 cfgDL, uint32 cfgUL)
{
    ...
    if(state == PCM2WAY_STATE)
    {
        L1Audio_HookHisrHandler(DP_D2C_PCM2WAY_ENH_DL,(L1Audio_EventHandler)pcmEx_hisrDlHdlr, 0);
        L1Audio_HookHisrHandler(DP_D2C_PCM2WAY_ENH_UL,(L1Audio_EventHandler)pcmEx_hisrUlHdlr, 0);
    }
    else
    {
...
static void Config_and_Check_PCMEx_Start_State (kal_bool is_DACA)
{
    ...
    {
#if defined(MT6261) || defined(MT2501) || defined(MT2502) 
        if( PCM2WAY_STATE == pcmEx.state){     
            SAL_PcmEx_SetStateDL(SAL_PCMEX_TYPE_PNW, SAL_PCMEX_ON, SAL_PCMEX_TYPE_P2W);
            SAL_PcmEx_SetStateUL(SAL_PCMEX_TYPE_PNW, SAL_PCMEX_ON, SAL_PCMEX_TYPE_P2W);
        }else{
            SAL_PcmEx_SetStateDL(SAL_PCMEX_TYPE_PNW, SAL_PCMEX_ON, SAL_PCMEX_TYPE_P4W);
            SAL_PcmEx_SetStateUL(SAL_PCMEX_TYPE_PNW, SAL_PCMEX_ON, SAL_PCMEX_TYPE_P4W);         
        }
#else
    ...
    
    {
        ...
        Sal_PcmEx_Type_t type = is_DACA ? SAL_PCMEX_TYPE_DACA : SAL_PCMEX_TYPE_PNW;
        
        do
        {
            kal_bool is_ready = true;
#if defined(MT6261) || defined(MT2501) || defined(MT2502) 
            if (   DL_on && !SAL_PcmEx_CheckStateDL(type, SAL_PCMEX_RDY, PCM2WAY_STATE == pcmEx.state ? SAL_PCMEX_TYPE_P2W : SAL_PCMEX_TYPE_P4W)
                || UL_on && !SAL_PcmEx_CheckStateUL(type, SAL_PCMEX_RDY, PCM2WAY_STATE == pcmEx.state ? SAL_PCMEX_TYPE_P2W : SAL_PCMEX_TYPE_P4W)
               )
            {
                is_ready = false;
            }               
#else        
            if (   DL_on && !SAL_PcmEx_CheckStateDL(type, SAL_PCMEX_RDY)
                || UL_on && !SAL_PcmEx_CheckStateUL(type, SAL_PCMEX_RDY)
               )
            {
                is_ready = false;
            }
...
static void PCMNWAY_Common_Stop (uint32 type)
{
...
#if defined(MT6261) || defined(MT2501) || defined(MT2502) 
	ASSERT(SAL_PcmEx_CheckStateUL(SAL_PCMEX_TYPE_PNW, SAL_PCMEX_OFF, PCM2WAY_STATE == pcmEx.state ? SAL_PCMEX_TYPE_P2W : SAL_PCMEX_TYPE_P4W) || SAL_PcmEx_CheckStateUL(SAL_PCMEX_TYPE_PNW, SAL_PCMEX_RDY, PCM2WAY_STATE == pcmEx.state ? SAL_PCMEX_TYPE_P2W : SAL_PCMEX_TYPE_P4W));
	ASSERT(SAL_PcmEx_CheckStateDL(SAL_PCMEX_TYPE_PNW, SAL_PCMEX_OFF, PCM2WAY_STATE == pcmEx.state ? SAL_PCMEX_TYPE_P2W : SAL_PCMEX_TYPE_P4W) || SAL_PcmEx_CheckStateDL(SAL_PCMEX_TYPE_PNW, SAL_PCMEX_RDY, PCM2WAY_STATE == pcmEx.state ? SAL_PCMEX_TYPE_P2W : SAL_PCMEX_TYPE_P4W));
    
    SAL_PcmEx_SetStateUL(SAL_PCMEX_TYPE_PNW, SAL_PCMEX_OFF, PCM2WAY_STATE == pcmEx.state ? SAL_PCMEX_TYPE_P2W : SAL_PCMEX_TYPE_P4W);	
    SAL_PcmEx_SetStateDL(SAL_PCMEX_TYPE_PNW, SAL_PCMEX_OFF, PCM2WAY_STATE == pcmEx.state ? SAL_PCMEX_TYPE_P2W : SAL_PCMEX_TYPE_P4W);
    
    AM_PCM_EX_Off((uint32)(&pcmEx));
    if(pcmEx.state == PCM2WAY_STATE)
    {
       L1Audio_UnhookHisrHandler(DP_D2C_PCM2WAY_ENH_DL);
       L1Audio_UnhookHisrHandler(DP_D2C_PCM2WAY_ENH_UL);
    }
    else
    {
    ...
    pcmEx.bandInfo = SAL_PCM_NARROWBAND;
...
void Extended_PCM2WAY_Start(void (*pcm2way_dl_hdlr)(void), void (*pcm2way_ul_hdlr)(void), uint32 type, uint32 cfgDL, uint32 cfgUL)
{
    ...
    ASSERT(pcmEx.bandInfo != SAL_PCM_UNDEFINED);  // Should use PCM2Way_SetFormat() to set format before P2W start
...
void PCM4WAY_GetFromMic(uint16 *ul_buf)
{
    ...
    uint32 I             = SAL_PcmEx_GetBufLen(SAL_PCMEX_PNW_BUF_UL1);
...
void PCM4WAY_PutToSE(const uint16 *ul_data)  
{
    ...
    uint32 I             = SAL_PcmEx_GetBufLen(SAL_PCMEX_PNW_BUF_UL2);
    while(I-- > 0)
    {
        *ptr++ = *ul_data++;
    }
...
void PCM4WAY_GetFromSD(uint16 *dl_buf)
{
   ...
   uint32 I 			= SAL_PcmEx_GetBufLen(SAL_PCMEX_PNW_BUF_DL1);
    while(I-- > 0)
    {
        *dl_buf++ = *ptr++;
    }
...
void PCM4WAY_PutToSpk(const uint16 *dl_data)
{
    ...
    uint32 I             = SAL_PcmEx_GetBufLen(SAL_PCMEX_PNW_BUF_DL2);
    while(I-- > 0)
    {
        *ptr++ = *dl_data++;
    }
...
{
#if defined(MT6261) || defined(MT2501) || defined(MT2502)
    volatile uint16 *ptr = SAL_PcmEx_GetBuf(SAL_PCMEX_AUX_P2W_BUF_UL);
...
{
#if defined(MT6261) || defined(MT2501) || defined(MT2502)
    volatile uint16 *ptr = SAL_PcmEx_GetBuf(SAL_PCMEX_AUX_P2W_BUF_DL);
...
void DACA_Start(void (*daca_dl_hdlr)(void), void (*daca_ul_hdlr)(void), DACA_APP_TYPE app_type)
{
    ...
    pcmEx.bandInfo = app_type & DACA_USE_NB ? SAL_PCM_NARROWBAND : SAL_PCM_WIDEBAND;
    
    if (app_type & DACA_USE_UL)
    {
#if defined(MT6261) || defined(MT2501) || defined(MT2502)
        SAL_PcmEx_SetStateUL(SAL_PCMEX_TYPE_DACA, SAL_PCMEX_ON, 0);
#else
    ...
    {
#if defined(MT6261) || defined(MT2501) || defined(MT2502)
        SAL_PcmEx_SetStateDL(SAL_PCMEX_TYPE_DACA, SAL_PCMEX_ON, 0);
#else
...
void DACA_Stop(DACA_APP_TYPE app_type)
{
...
#if defined(MT6261) || defined(MT2501) || defined(MT2502)
    ASSERT(SAL_PcmEx_CheckStateUL(SAL_PCMEX_TYPE_DACA, pcmEx.app_type & DACA_USE_UL ? SAL_PCMEX_RDY : SAL_PCMEX_OFF, 0));
    ASSERT(SAL_PcmEx_CheckStateDL(SAL_PCMEX_TYPE_DACA, pcmEx.app_type & DACA_USE_DL ? SAL_PCMEX_RDY : SAL_PCMEX_OFF, 0));
    SAL_PcmEx_SetStateUL(SAL_PCMEX_TYPE_DACA, SAL_PCMEX_OFF, 0);
	SAL_PcmEx_SetStateDL(SAL_PCMEX_TYPE_DACA, SAL_PCMEX_OFF, 0);
#else
    ASSERT(SAL_PcmEx_CheckStateUL(SAL_PCMEX_TYPE_DACA, pcmEx.app_type & DACA_USE_UL ? SAL_PCMEX_RDY : SAL_PCMEX_OFF));
    ASSERT(SAL_PcmEx_CheckStateDL(SAL_PCMEX_TYPE_DACA, pcmEx.app_type & DACA_USE_DL ? SAL_PCMEX_RDY : SAL_PCMEX_OFF));
    ...
    pcmEx.am_type  = AM_PCMEX_TYPE_UNDEF;
    pcmEx.bandInfo = SAL_PCM_UNDEFINED;
...
void DACA_GetFromSD(uint16 *dl_buf)
{
    ...
    uint32 I             = SAL_PcmEx_GetBufLen(SAL_PCMEX_DACA_BUF_DL);
    while (I-- > 0)
    {
        *dl_buf++ = *ptr++;
    }
...
{ 
    ...
    uint32 I             = SAL_PcmEx_GetBufLen(SAL_PCMEX_DACA_BUF_UL);
