...
{
    return (power_on_status_enum)uem_get_poweron_mode();
}
...
void uem_set_gpio_status(kal_uint8 gpio_dev_type, kal_uint8 gpio_dev_level)
{
    ...
    if (gpio_dev_type < MAX_GPIO_DEVICE_NUM &&
        ...
        gpio_dev_type != GPIO_DEV_FTE_FEEDBACK_VIB)
    {
        g_uem_cntx_p->rmi_data.gpio_status[gpio_dev_type] = gpio_dev_level;
    }
    else if (gpio_dev_type == GPIO_DEV_FTE_FEEDBACK_VIB)
    {
        if (g_uem_cntx_p->rmi_data.gpio_status[gpio_dev_type] == gpio_dev_level)
            return;
...
kal_uint8 uem2l4_usb_config_action_convert(kal_uint8 action)
{
    ...
    switch (usb_action)
    {
        ...
        case USB_DATA_CONNECTION:
            return DEVUSB_CFG_ACTION_CDROM;
...
kal_uint8 uem2drv_usb_config_action_convert(kal_uint8 action)
{
    ...
    switch (usb_action)
    {
        ...
        case DEVUSB_CFG_ACTION_CDROM:
            return USB_DATA_CONNECTION;
...
void uem_send_msg_to_l1(msg_type msg_id, void *local_data_ptr)
{
    ...
    ilm_ptr->msg_id = msg_id; /* Set the message id */

    ilm_ptr->peer_buff_ptr = NULL;
    
    ilm_ptr->local_para_ptr = (local_para_struct*) local_data_ptr;

    SEND_ILM(MOD_UEM, MOD_L1, EM_L1_SAP, ilm_ptr);
...
void uem_genernal_send_msg(kal_uint16 msg_id, kal_uint32 src_mod, kal_uint16 dest_mod, void *data_ptr)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/    
    ilm_ptr = allocate_ilm((module_type)src_mod);
    ilm_ptr->src_mod_id = (module_type)src_mod;
    ilm_ptr->dest_mod_id = (module_type)dest_mod;
    ilm_ptr->sap_id = INVALID_SAP;
    ilm_ptr->msg_id = (msg_type)msg_id; /* Set the message id */
    
    ilm_ptr->peer_buff_ptr = NULL;
    ilm_ptr->local_para_ptr = (local_para_struct*) data_ptr;

    SEND_ILM((module_type)src_mod, (module_type)dest_mod, INVALID_SAP, ilm_ptr);
...
static void uem_do_set_gain(kal_uint32 type, void* gain)
{
    ...
    {
        case VOL_TYPE_CTN:
            TONE_SetOutputVolume((kal_uint8)(0xFF & (kal_uint32)gain), 0);
            break;
        case VOL_TYPE_KEY:
            KT_SetOutputVolume((kal_uint8)(0xFF & (kal_uint32)gain), 0);
            break;
        case VOL_TYPE_MIC:
            L1SP_SetMicrophoneVolume((kal_uint8)(0xFF & (kal_uint32)gain));
            break;
        case VOL_TYPE_SPH:
            L1SP_SetOutputVolume((kal_uint8)(0xFF & (kal_uint32)gain), 0);
            break;
        case VOL_TYPE_SID:
            L1SP_SetSidetoneVolume((kal_uint8)(0xFF & (kal_uint32)gain));
            break;
        case VOL_TYPE_MEDIA:
            Media_SetOutputVolume((kal_uint8)(0xFF & (kal_uint32)gain), 0);
            break;
	default:
	    break;
    }
