...
#if defined(__TINY_SYS__) && defined(__MMI_BACKLIGHT_SUPPORT__)
extern void pwm_update_saved_backlight_level(kal_uint32 savedLevel);
...
static const U8 *const cache_table[] =
{
...
static kal_uint8* mmi_cache_get_default_value(nvram_lid_enum LID);
#endif
/*****************************************************************************
 * FUNCTION
...
S32 ReadValueSlim(U16 nDataItemId, void *pBuffer, U8 nDataType)
{
    ...
    MMI_TRACE(PLUTO_FW_TRC_NVRAM, TRC_MMI_FRM_NVRAM_READ_VALUE_BEGIN, nDataType, nDataItemId,dataReadFlag[nDataType]);
...
#if defined(__TINY_SYS__) && defined(__MMI_BACKLIGHT_SUPPORT__)
	if(nDataItemId == NVRAM_BYTE_BL_SETTING_LEVEL){
		pwm_update_saved_backlight_level( *((int *)pBuffer));}
#endif
...
S32 WriteValueSlim(U16 nDataItemId, void *pBuffer, U8 nDataType)
{
	...
	MMI_TRACE(PLUTO_FW_TRC_NVRAM, TRC_MMI_FRM_NVRAM_WRITE_VALUE_BEGIN, nDataType, nDataItemId, dataWriteFlashFlag[nDataType]);
...
#if (defined(__UPDATE_BINARY_FILE__) && !defined(__NVRAM_SUPPORT_CUSTPACK__))
	cache_buffer_p = mmi_cache_get_default_value(cache_lid[cache_type]);
	MMI_ASSERT(cache_buffer_p);
...
static void mmi_frm_nvram_cache_restore_factory_mode(void)
{
    ...
    kal_prompt_trace(MOD_PLUTO_FW, "[MMI_FRM][NVRAM CACHE]mmi_frm_nvram_cache_restore_factory_mode(),enter");
    if (!nvram_check_restore_factory_flag())
    {    
		kal_prompt_trace(MOD_PLUTO_FW, "[MMI_FRM][NVRAM CACHE]mmi_frm_nvram_cache_restore_factory_mode(),return");

		return;
	}

    kal_prompt_trace(MOD_PLUTO_FW, "[MMI_FRM][NVRAM CACHE]mmi_frm_nvram_cache_restore_factory_mode(),continue");
 ...
 *  This function is to write record to NVRAM.
 * PARAMETERS
 *  bRstMMICacheDefault     :[IN]    MMI cache default init flag
 * RETURNS
 *  NONE
 *****************************************************************************/
void MMICacheDefaultInit_Int(kal_uint8 *bRstMMICacheDefault)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
	kal_prompt_trace(MOD_PLUTO_FW, "[MMI_FRM][NVRAM CACHE]MMICacheDefaultInit_Int(),start bRstMMICacheDefault = %d", *bRstMMICacheDefault);

	   for (i = 0; i < sizeof(cache_lid) / sizeof(cache_lid[0]); i++)
	   {
		   status = nvram_external_write_data(cache_lid[i], 1, (U8*)cache_table[i], NVRAM_CACHE_SIZE);
		   EXT_ASSERT(status == KAL_TRUE, status, cache_lid[i], __LINE__);		
	   }

	   for (i = 0; i < sizeof(cache_key_lid) / sizeof(cache_key_lid[0]); i++)
	   {
	        /*  cache key value */
            for (j = 0; j < cache_key_record[i]; j++)
            {
			   status = nvram_external_write_data(cache_key_lid[i], j + 1, (U8*)cache_key_table[i] + j * cache_key_size[i],cache_key_size[i]);
			   EXT_ASSERT(status == KAL_TRUE, status, cache_key_lid[i], __LINE__);	
            }
	   }
	   
	   *bRstMMICacheDefault = 0;
	kal_prompt_trace(MOD_PLUTO_FW, "[MMI_FRM][NVRAM CACHE]MMICacheDefaultInit_Int(),done bRstMMICacheDefault = %d", *bRstMMICacheDefault);
}


void MMICacheDefaultInit(kal_uint8 *bRstMMICacheDefault)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    kal_int32 init_flag = 0;
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/

	ReadRecordSlim(NVRAM_EF_CACHE_INIT_LID, NVRAM_MMI_CACHE_INIT_TOTAL, &init_flag, NVRAM_MMI_CACHE_INIT_SIZE);

    kal_prompt_trace(MOD_PLUTO_FW, "[MMI_FRM][NVRAM CACHE]MMICacheDefaultInit(),bRstMMICacheDefault = %d, %x", *bRstMMICacheDefault, init_flag);
    
	if(*bRstMMICacheDefault == 1)
	{
		MMICacheDefaultInit_Int(bRstMMICacheDefault);
		init_flag = 0x89abcdef;
		WriteRecordSlim(NVRAM_EF_CACHE_INIT_LID, NVRAM_MMI_CACHE_INIT_TOTAL, &init_flag, NVRAM_MMI_CACHE_INIT_SIZE);
	}
	else if(init_flag == 0x12345678)
	{
	
        kal_prompt_trace(MOD_PLUTO_FW, "[MMI_FRM][NVRAM CACHE]MMICacheDefaultInit(),init_flag = %x", init_flag);
		if(!nvram_check_restore_factory_flag())
		{
			MMICacheDefaultInit_Int(bRstMMICacheDefault);
		}
...
{
	U8 i = 0;
    for(i = 0; i < MMI_FW_NVRAM_TYPE_TOTAL; i++)
	{
		if(cache_lid[i] == LID)
		{
          return (kal_uint8*)cache_table[i];
		}	
    }
