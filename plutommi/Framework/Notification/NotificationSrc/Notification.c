...
static MMI_BOOL g_is_invoke_timer_start = MMI_FALSE;
...
mmi_ret mmi_frm_nmgr_invoke_defer_event_callback(mmi_event_struct* para)
{
    ...
    if (mmi_frm_nmgr_get_defer_notification_count() > 0)
    {
        ...
        if(g_is_invoke_timer_start == MMI_FALSE)
        {
        	g_is_invoke_timer_start = MMI_TRUE;
			kal_prompt_trace(MOD_MMI_FW,"[NMGR] invoke_defer_event_callback: start invoke timer(%d)", g_is_invoke_timer_start);
...
static void nmgr_alert_pluto_popup_scrn_entry(mmi_scrn_essential_struct *arg)
{
                            ...
                            MMI_FRM_SMALL_SCRN))
    {
        ...
        else
        {
           ...
           rsk_string = (WCHAR *)GetString(STR_ID_NMGR_DISMISS);
#else
		   rsk_string = (WCHAR *)GetString(STR_GLOBAL_CANCEL);
#endif
                                ...
                                mmi_event_notify_enum event_type)
{
    ...
    {/* 
      ...
      */
		...
		kal_prompt_trace(MOD_MMI_FW,"[NMGR] control_app_screen: invoke timer status(%d)", g_is_invoke_timer_start);

        if(g_is_invoke_timer_start == MMI_FALSE)
        {
        	g_is_invoke_timer_start = MMI_TRUE;
			kal_prompt_trace(MOD_MMI_FW,"[NMGR] control_app_screen: start invoke timer(%d)", g_is_invoke_timer_start);
	        StartTimer(MMI_NOTIFICATION_SERVICE_TIMER, 
	            MMI_NOTIFY_INVOKE_TIME, 
	            (FuncPtr)invoke_notify_defer_notification);
		}
        ...
        mmi_frm_nmgr_alert_snd_struct *snd_info)
{
	...
	kal_prompt_trace(MOD_MMI_FW,"[NMGR] control_sound enter: tone_id = %d",tone_id);
    ...
    else
    {
        ...
        if (!srv_prof_if_can_ring())
        {
		    ...
		    kal_prompt_trace(MOD_MMI_FW,"[NMGR] control_sound return:tone_id = %d",tone_id);
            return;
        }
        ...
        {/* If nmgr is playing sound means no sound conflict with MED module currently */
            ...
            if(is_resource_occuiped ||is_video_playing && (display_scrn == MMI_FALSE))    
            {
				...
				kal_prompt_trace(MOD_MMI_FW,"[NMGR] control_sound TONE_GENERAL_BEEP: tone_id = %d",tone_id);
        ...
        if(snd_info)
        {
		    ...
		    kal_prompt_trace(MOD_MMI_FW,"[NMGR] control_sound snd_info: tone_id = %d,sound_type = %d",tone_id,snd_info->sound_type);
        ...
        {
            kal_prompt_trace(MOD_MMI_FW,"[NMGR] control_sound no snd_info: tone_id = %d",tone_id);
            snd_result = srv_prof_play_tone(tone_id, nmgr_sound_callback);
        }
...
static mmi_ret invoke_notify_defer_notification(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
	g_is_invoke_timer_start = MMI_FALSE;
	kal_prompt_trace(MOD_MMI_FW,"[NMGR] invoke_notify_defer_notification: invoke timer stop(%d)", g_is_invoke_timer_start);
    ...
    if (mmi_frm_nmgr_get_defer_notification_count() > 0)
    {
            ...
            (defer_scen_scrn_prio >= active_scen.scrn_priority))
        {
            ...
            if (mmi_frm_nmgr_get_defer_notification_count() > 0)
            {
		        ...
		        {
		        	g_is_invoke_timer_start = MMI_TRUE;
					kal_prompt_trace(MOD_MMI_FW,"[NMGR] invoke_notify_defer_notification: start invoke timer(%d)", g_is_invoke_timer_start);
			        StartTimer(MMI_NOTIFICATION_SERVICE_TIMER, 
			            MMI_NOTIFY_INVOKE_TIME, 
			            (FuncPtr)invoke_notify_defer_notification);
				}
