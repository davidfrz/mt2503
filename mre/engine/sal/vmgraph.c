...
VMINT vm_graphic_is_bmp_file_mtk_format(VMUINT8 *img_data)
{
	if( img_data[0] == GDI_IMAGE_TYPE_BMP && img_data[8] == 'B' && img_data[9] == 'M')
		return TRUE;
	return FALSE;	
}
...
static VMINT vm_graphic_get_img_type(VMUINT8 *img_data, VMUINT8 *img_type)
{
    ...
    else if (vm_graphic_is_bmp_file(img_data) || vm_graphic_is_bmp_file_mtk_format(img_data))
    {
...
static VMINT vm_graphic_get_imgprop(VMUINT8 img_type, VMUINT8* img_data, VMINT img_len,vm_graphic_imgprop *img_prop)
{
	...
	if ( img_type == GDI_IMAGE_TYPE_AB2 || img_type == GDI_IMAGE_TYPE_ABM || vm_graphic_is_jpeg_file_mtk_format(img_data) || vm_graphic_is_bmp_file_mtk_format(img_data))
	{
		img_data += 8;
		img_len -= 8;
	}
...
static VMINT vm_graphic_draw_image_from_memory_internal(VM_GDI_HANDLE dest_layer_handle,VMUINT8* img_data,VMINT img_len,VMINT x,VMINT y,VMINT resized_width,VMINT resized_height,VMBOOL flag, VMINT frame_idx)
{
	...
	if (img_type == GDI_IMAGE_TYPE_AB2  || img_type == GDI_IMAGE_TYPE_ABM || vm_graphic_is_jpeg_file_mtk_format(img_data) || vm_graphic_is_gif_file_mtk_format(img_data) || vm_graphic_is_bmp_file_mtk_format(img_data))
	{
        img_data += 8;
        img_len -= 8;
	}
...
VM_GDI_RESULT vm_graphic_2d_linear_transform_rotate(VMINT src_handle, VMFLOAT theta, VMINT sx,VMINT sy,VMINT sw,VMINT sh,VMINT dx,VMINT dy,VMINT dw,VMINT dh)
{
    ...
    transform.m[0] = (float)cos(theta);
    transform.m[1] = -(float)sin(theta);
    transform.m[2] = -0.5 * sw * (float)cos(theta) + 0.5 * sh * (float)sin(theta) + 0.5 * sw; 
    transform.m[3] = (float)sin(theta);
    transform.m[4] = (float)cos(theta);
    transform.m[5] = -0.5 * sw * (float)sin(theta) - 0.5 * sh * (float)cos(theta) + 0.5 * sh; 
    transform.m[6] = 0.0f;
    transform.m[7] = 0.0f;
    transform.m[8] = 1.0f;

    if (GDI_SUCCEED == gdi_2d_linear_transfrom(
            transform.m,
            layer_trans_tbl[src_handle].handle,
            sx,
            sy,
...
VM_GDI_RESULT vm_graphic_2d_linear_transform_matrix(VMINT src_handle, VMFLOAT *trfmatrix, VMINT sx,VMINT sy,VMINT sw,VMINT sh,VMINT dx,VMINT dy,VMINT dw,VMINT dh)
{
    ...
    transform.m[0] = trfmatrix[0];
    transform.m[1] = trfmatrix[1];
    transform.m[2] = trfmatrix[2];
    transform.m[3] = trfmatrix[3];
    transform.m[4] = trfmatrix[4];
    transform.m[5] = trfmatrix[5];
    transform.m[6] = trfmatrix[6];
    transform.m[7] = trfmatrix[7];
    transform.m[8] = trfmatrix[8];
...
VMINT vm_get_graph_sym(VMCHAR* symbol)
{
    if(strcmp(symbol, "vm_graphic_2d_linear_transform_rotate") == 0)
    {
        return (VMINT)vm_graphic_2d_linear_transform_rotate;
    }
    if(strcmp(symbol, "vm_graphic_2d_linear_transform_matrix") == 0)
    {
        return (VMINT)vm_graphic_2d_linear_transform_matrix;
    }
