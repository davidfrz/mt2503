...
/*****************************************************************************
 * FUNCTION
 *  cui_tone_selector_bt_music_disconnect_callback_ex
 * DESCRIPTION
 ...
 *  void
 *****************************************************************************/
static void cui_tone_selector_bt_music_disconnect_callback_ex(MMI_BOOL result)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
#ifdef __MMI_WEARABLE_DEVICE__
g_play_preview_entry = MMI_FALSE;
#if (defined(__MMI_BT_MUSIC__) || defined(__MMI_AUDIO_PLAYER_BT__) || defined(__MMI_BTD_BOX_MULTIMEDIA_CAT_STYLE__))
    /* Test if BT Music play is connected */
    if (mmi_bt_music_check_connection())
    {
        mmi_bt_music_disconnect(cui_tone_selector_bt_music_disconnect_callback_ex);
        //return;
    }
#endif
#endif
...
static mmi_ret cui_tone_selector_group_proc(mmi_event_struct* evt)
{
    ...
    switch(evt->evt_id)
    {
        ...
        {            
            ...
            {
            #ifdef __MMI_WEARABLE_DEVICE__
                cui_menu_set_screen_show_softkey();
                ...
                {
                    if (g_play_preview_entry == MMI_TRUE)
                    {
                        g_play_preview_entry = MMI_FALSE;
...
#else
        case EVT_ID_CUI_MENU_NON_HIGHLIGHTED_ITEM_TAP:
        ...
        #ifdef __MMI_WEARABLE_DEVICE__
            cui_menu_set_currlist_highlighted_id(menu_evt->sender_id, menu_evt->highlighted_menu_id);
            cui_menu_set_radio_list_item_ext(menu_evt->highlighted_menu_id);
        #endif
...
static void cui_tone_selector_listscr_entry (void)
{
    ...
    cui_menu_id = cui_menu_create(g_tonesel_cntx.cur_gid, 
                                      ...
                                      MMI_FALSE, 
                                      NULL);
    cui_menu_set_default_right_softkey(cui_menu_id, NULL);
