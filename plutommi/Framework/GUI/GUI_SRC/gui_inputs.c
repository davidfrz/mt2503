...
gui_line_struct g_visible_multiline_line_info_single;
gui_line_struct g_multiline_line_info_single;
...
#endif /* defined(__MMI_FTE_SUPPORT__)  || defined( __MMI_WEARABLE_DEVICE__)*/
...
#ifdef __MMI_TOUCH_SCREEN__
MMI_ID_TYPE dialing_key_image_ex[] = 
{
#if defined(__MMI_SLIM_IMG_RES__) && defined(__MMI_MAINLCD_240X320__) && !defined(__MMI_FTE_SUPPORT__)
	0,
#else /* __MMI_SLIM_IMG_RES__ && __MMI_MAINLCD_240X320__ && !__MMI_FTE_SUPPORT__ */
...
#if defined(__MMI_FTE_SUPPORT__) && defined( __MMI_WEARABLE_DEVICE__) && defined(__MMI_WEARABLE_DEVICE_DIALER__) 
    IMG_DIALING_KEY_1,
    IMG_DIALING_KEY_2,
    IMG_DIALING_KEY_3,
    IMG_DIALING_KEY_4,
...
static MMI_ID_TYPE dialing_key_down_image_ex[] = 
{
#if defined(__MMI_FTE_SUPPORT__) && defined( __MMI_WEARABLE_DEVICE__) && defined(__MMI_WEARABLE_DEVICE_DIALER__) 
	IMG_DIALING_KEY_1_DOWN,
	IMG_DIALING_KEY_2_DOWN,
...
#ifdef __MMI_TOUCH_SCREEN__
MMI_ID_TYPE dialing_key_image_pin[] = 
{
#if defined(__MMI_SLIM_IMG_RES__) && defined(__MMI_MAINLCD_240X320__) && !defined(__MMI_FTE_SUPPORT__)
	0,
#else /* __MMI_SLIM_IMG_RES__ && __MMI_MAINLCD_240X320__ && !__MMI_FTE_SUPPORT__ */
...
static MMI_ID_TYPE dialing_key_down_image_pin[] = 
{
#if defined(__MMI_FTE_SUPPORT__) && defined( __MMI_WEARABLE_DEVICE__) && defined(__MMI_WEARABLE_DEVICE_DIALER__)
	IMG_DIALING_KEY_1_DOWN,
	IMG_DIALING_KEY_2_DOWN,
        ...
        S32 clip_y2)
{
	...
	if(b->ext_flags & GUI_MULTI_LINE_INPUT_BOX_SINGLINE_STYLE)
	{
        ...
        y = y +1;
        height = height - 1;
        if((x > (b->x + b->text_x + b->edit_width))||
            (x < (b->x + b->text_x -1)))
        {
            #if(UI_BLINKING_CURSOR_SUPPORT)        
            StopTimer(BLINKING_CURSOR);
...
void gui_create_multi_line_input_box_set_buffer_2(
        multi_line_input_box *b,
        S32 x,
        S32 y,
        S32 width,
        ...
        S32 text_length,
        S32 edit_position,
        U8  editor_style)  // editor_style 0: multiline , 1:singleline
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    {
        memset(&g_visible_multiline_line_info_single, 0, sizeof(g_visible_multiline_line_info_single));
        memset(&g_multiline_line_info_single, 0, sizeof(g_multiline_line_info_single));
        b->visible_line_info = &g_visible_multiline_line_info_single;
        b->line_info = &g_multiline_line_info_single;
    }
...
#endif/*#ifdef __MMI_MULTILINE_INPUT_BOX_SINGLINE_STYLE_WITH_FTE__*/
 ...
 *  void
 *****************************************************************************/
void gui_dialing_key_select_ex(dialing_keypad_struct *dialing_keypad, S32 item_index)
{
	...
	if(g_cat16_style == 1)
	{
		key_down_image = dialing_key_down_image_ex;
	}
	else if(g_cat16_style == 2)
	{
		key_down_image = dialing_key_down_image_pin;

	}
	else
	{
		key_down_image = dialing_key_down_image;
	}
    ...
    {
		//gdi_draw_solid_rect(x1, y1, x2, y2, gdi_act_color_from_rgb(255, 31, 181, 172));
    	if(g_cat16_style == 2 && item_index == 10)
    	{
				
			gui_set_text_color(gui_color(0, 0, 0));
			gui_set_font(&MMI_large_font);
			
			gui_measure_string(temp, &width, &height);
			gui_move_text_cursor(x1 + ((dialing_keypad->key_width - width) >> 1), 
		...
		{
			image_id = *(key_down_image + item_index - 1);
...
void gui_dialing_key_select(dialing_keypad_struct *dialing_keypad, S32 item_index)
{
    ...
    /*----------------------------------------------------------------*/
	 ...
	 gui_dialing_key_select_ex(dialing_keypad, item_index);
...
void gui_show_dialing_keypad(dialing_keypad_struct *dialing_keypad)
{
    ...
    gdi_push_and_set_alpha_blending_source_layer(act_layer);
#ifdef __MMI_WEARABLE_DEVICE__ 
	gdi_layer_push_and_set_clip(MMI_DIALING_KEYPAD_X, MMI_DIALING_KEYPAD_Y, UI_device_width, UI_device_height);	
	gdi_draw_solid_rect(MMI_DIALING_KEYPAD_X, MMI_DIALING_KEYPAD_Y, UI_device_width, UI_device_height, gdi_act_color_from_rgb(255, 0, 180, 255));//32,32,40
	gdi_layer_pop_clip();
	if(g_cat16_style == 1)
	{
		key_image = dialing_key_image_ex;
	}
	else if(g_cat16_style == 2)
	{
		key_image = dialing_key_image_pin;
	}
	else
	{
		key_image = dialing_key_image;
	...
	#else  /*__MMI_WEARABLE_DEVICE__*/
			if(g_cat16_style == 2 && item_index == 9) //"OK"
			{
				WCHAR temp[] = {'O', 'K', 0};
	...
	#endif /*__MMI_WEARABLE_DEVICE__*/
			{
              gui_measure_image(get_image(*(key_image+item_index)), &width, &height);           
              gui_show_image(x1 + ((dialing_keypad->key_width - width) >> 1), 
                y1 + ((dialing_keypad->key_height - height) >> 1), 
                get_image(*(key_image+item_index)));           /* foreground */
			}
#else /* defined(__MMI_FTE_SUPPORT__) */
	#ifndef __MMI_WEARABLE_DEVICE__ 
...
#ifdef __MMI_FSEDITORCUI_REMOVE_SOFTKEY__	
extern void cui_fseditor_lsk_handler_for_editor(void);	
#endif					

#ifdef __MMI_INLINECUI_REMOVE_SOFTKEY__	
extern void cui_inline_editor_lsk_handler_for_editor(void);	
#endif				
        ...
        switch (pen_event)
        {
            ...
            case MMI_PEN_EVENT_ABORT:
                ...
                else if (PEN_CHECK_BOUND(x, y, b->x, b->y, b->width, b->height))
                {
					...
					{																
						cui_fseditor_lsk_handler_for_editor();						
					}		
	                #endif	
					...
					if(pen_event == MMI_PEN_EVENT_REPEAT)
					{																
						cui_inline_editor_lsk_handler_for_editor();						
					}
					#endif				
                }
                else
...
void *user_data_timer = NULL;
U8 ssp_need_update_scrollbar = 0;
U8 ssp_had_cancel_timer = 0;

void gui_multiline_ssp_update_scrollbar(void )
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    multi_line_input_box *b = (multi_line_input_box *)user_data_timer;
    U32 saved_ext_flag = 0;
    
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/  
    kal_prompt_trace(MOD_MMI, "889977    gui_multiline_ssp_update_scrollbar   11111  \n");
    if(ssp_had_cancel_timer)
    {
        ssp_had_cancel_timer = 0;
        return ;
    }
    kal_prompt_trace(MOD_MMI, "889977    gui_multiline_ssp_update_scrollbar   22222  \n");
...
void gui_multiline_ssp_update_scrollbar_start_timer(void )
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/  
    kal_prompt_trace(MOD_MMI, "889977    gui_multiline_ssp_update_scrollbar_start_timer     \n");

    if(ssp_need_update_scrollbar)
    {   
         kal_prompt_trace(MOD_MMI, "889977    gui_multiline_ssp_update_scrollbar_start_timer   222222  \n");
        ssp_need_update_scrollbar = 0;
        ssp_had_cancel_timer = 0;
        user_data_timer = input_box_user_data;
        gui_cancel_timer(gui_multiline_ssp_update_scrollbar);
        gui_start_timer(500, gui_multiline_ssp_update_scrollbar);
    }  
}
void gui_multiline_ssp_update_scrollbar_cancel_timer(void)
{
    kal_prompt_trace(MOD_MMI, "889977    gui_multiline_ssp_update_scrollbar_cancel_timer     \n");
    ssp_had_cancel_timer = 1;
    if(ssp_need_update_scrollbar)
    {
        ssp_need_update_scrollbar = 0;
        kal_prompt_trace(MOD_MMI, "889977    gui_multiline_ssp_update_scrollbar_cancel_timer   22222  \n");
    }
    gui_cancel_timer(gui_multiline_ssp_update_scrollbar);
}
...
/* under construction !*/
    ...
    if (notify_client_status)
        notify_client_status(input_box_user_data, g_ssp_state);
#ifdef __MMI_EDITOR_SSP_SUPPORT_NO_DRAW_SCROLLBAR_WHEN_NO_PEN_MOVE__
    gui_multiline_ssp_update_scrollbar_start_timer();    
#endif
    return MMI_FALSE;
}
...
#ifdef __MMI_MULTILINE_INPUT_BOX_SINGLINE_STYLE_WITH_FTE__
#ifdef  __MMI_MULTILINE_INPUT_BOX_SINGLINE_STYLE__
    extern U32 gui_get_multi_line_edit_width_in_singline(multi_line_input_box *b, multi_line_input_box_show_struct *bs);
#endif /*__MMI_MULTILINE_INPUT_BOX_SINGLINE_STYLE__*/    
#endif /*__MMI_MULTILINE_INPUT_BOX_SINGLINE_STYLE_WITH_FTE__*/ 
void gui_multiline_ssp_get_client_info(void *user_data, S32 *total_offset, S32 *cur_offset, S32 *move_type)
{
    ...
    {//for singleline
        ...
        {  
           ...
           bs.default_direction = BIDI_L; 
           #endif
           // *total_offset = b->text_width- b->edit_width;
           *total_offset =  (b->text_width) - gui_get_multi_line_edit_width_in_singline(b,&bs);
