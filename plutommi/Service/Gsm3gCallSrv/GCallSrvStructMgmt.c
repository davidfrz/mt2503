                ...
                || (!mmi_ucs2strlen((CHAR *)cm_p->phb_data.name))))
        {
            ...
            if (result && mmi_ucs2strlen((CHAR *)mailbox_name))
            {
                mmi_ucs2ncpy((CHAR *)call_info->name,
                    (CHAR *)mailbox_name, (U32)(MMI_PHB_NAME_LENGTH));
                srv_phb_set_invalid_number_id(&(cm_p->phb_data.phb_id));
            }
        }
    #ifdef __MMI_ONE_KEY_SOS_SUPPORT__
        if (cm_p->mo.module_id == SRV_UCM_MODULE_ORIGIN_OKSOS)
        {
            srv_oksos_data_info_struct* oksos_data = OslMalloc(sizeof(srv_oksos_data_info_struct));

            memset(oksos_data, 0, sizeof(srv_oksos_data_info_struct));

            if (SRV_OKSOS_RET_OK == srv_oksos_query_info(cm_p->mo.data_index, oksos_data))
            {
                if (mmi_ucs2strlen((CHAR*)oksos_data->name) > 0)
                {
                    mmi_ucs2ncpy((CHAR*)call_info->name, (CHAR*)oksos_data->name, (U32)(MMI_PHB_NAME_LENGTH));
                }
            }
            OslMfree(oksos_data);
        }
