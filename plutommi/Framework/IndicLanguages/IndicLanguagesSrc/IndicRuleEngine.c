...
#if defined(__MMI_LANG_MYANMAR__) || defined(__MMI_LANG_ZAWGYI_MYANMAR__)
static ire_language_id_enum ire_get_current_myanmar_language_in_phone_setting();
#endif

#if defined(__MMI_LANG_ZAWGYI_MYANMAR__)
static U32 ire_language_zawgyi_mynamar_get_cluster(PU8 start_character, PU8 end_character, PU16 out_cluster, S32* out_len);
static S32 ire_myanmar_zawgyi_language_rules(PU16 glyphoutput, PU16 cluster, S32 nentry);
static S32 ire_myanmar_zawgyi_language_get_text_x(
...
U32 ire_get_character_lang_id(U16 character)
{
	...
	#if defined(__MMI_LANG_MYANMAR__) || defined(__MMI_LANG_ZAWGYI_MYANMAR__)
	if ((character >= 0x1000 && character <= 0x109F) || (character >= 0xE800 && character <= 0xE8C2) || (character >= 0xEF00 && character <= 0xEF9F))
	{
	    if (character >= 0x1000 && character <= 0x109F)
        {
            return ire_get_current_activate_myanmar_language();
        }
        else if (character >= 0xEF00 && character <= 0xEF9F)
        {
            return IRE_LANGUAGE_ID_MYANMAR_ZAWGYI;
        }
        else
        {
...
#if defined(__MMI_LANG_ZAWGYI_MYANMAR__)
    if (langIdIndex == IRE_LANGUAGE_ID_MYANMAR_ZAWGYI)
    {
        ret = ire_language_zawgyi_mynamar_get_cluster(start_char_8,g_indic_rule_engine_cntx.g_cluster_end_p, cluster, &g_indic_rule_engine_cntx.g_display_buffer_len);
        ...
        case HALANT:
            {
                if(prev_char_16 == NULL)
                {
                    ...
                    ire_fill_display_cluster2(curr_cluster ,prev_char_16);
                    #if defined(__MMI_MULTITAP_BANGLA_SPEC__)
                    if(curr_char_16 == 0x09af || curr_char_16 == 0x09b0 )
                    {
                        ret ++;
                        ire_fill_display_cluster2(curr_cluster ,curr_char_16);
                    }
 ...
 *  void
 *****************************************************************************/
S32 ire_get_text_x(S32 *ptext_x, U16 current_character, S32 text_y, U16 d_width, U16 width)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if (!(g_indic_rule_engine_cntx.g_new_line))
    {
        ...
        if (langIdIndex == IRE_LANGUAGE_ID_MYANMAR_ZAWGYI)
        {
            add_width = ire_myanmar_zawgyi_language_get_text_x(
                            current_character,
                            &g_indic_rule_engine_cntx.g_prev_char_text_x,
                            &g_indic_rule_engine_cntx.g_prev_char_dwidth,
                            &g_indic_rule_engine_cntx.g_prev_char_width,
                            ptext_x,
                            d_width,
                            width,
                            MMI_TRUE);
             ...
             (mCharStatusFlagPrev != NORMAL_CON))
        {
                if(g_indic_rule_engine_cntx.error_correction_for_rendering)
            {
                ...
                *ptext_x = g_indic_rule_engine_cntx.g_prev_char_text_x + correction;
                add_width = correction + d_width - g_indic_rule_engine_cntx.g_prev_char_dwidth;
...
#endif /* defined (__MMI_LANG_KHMER__) */
#if defined(__MMI_LANG_ZAWGYI_MYANMAR__)
        case IRE_LANGUAGE_ID_MYANMAR_ZAWGYI:
            return ire_myanmar_zawgyi_language_rules(glyphoutput, cluster, nentry);
#endif /*#if defined(__MMI_LANG_ZAWGYI_MYANMAR__)*/
        default:
            break;
    }
                    ...
                    case KANNADA_RA_CON:
                        {
                            ...
                            else if (!(mCharStatusFlagNext ^ HALANT))
                            {
                                ...
                                #endif /*#if defined(__MMI_MULTITAP_BANGLA_SPEC__)*/
                                {
                                    memcpy(
                                        glyphoutput,
...
S32 ire_get_cluster(PU16 cluster)
{
    ...
    if (langIdIndex == IRE_LANGUAGE_ID_MYANMAR_ZAWGYI)
    {
        nFill = ire_language_zawgyi_mynamar_get_cluster(start_char,g_indic_rule_engine_cntx.g_cluster_end_p, cluster, &g_indic_rule_engine_cntx.g_display_buffer_len);
...
U8 ire_IsValidInput_int(UI_string_type candidate, U8 strlen, UI_string_type previous_string)
{
    ...
    #if defined(__MMI_MULTITAP_BANGLA_SPEC__)
    U16 previous_char3      = previous_string[2];
    #endif
    U16 startRange          = 0;
...
#if defined(__MMI_MULTITAP_BANGLA_SPEC__)
                        if (previous_char == 0x09af && current_char_type == IRE_CHAR_TYPE_HALANT)
                        {
...
/*****************************************************************************
 * FUNCTION
 *  ire_get_current_activate_myanmar_language
 * DESCRIPTION
 *  this api is used for get myanmar language id when __MMI_LANG_MYANMAR__ and __MMI_LANG_ZAWGYI_MYANMAR__ are enable.
 * PARAMETERS
 *	void
 *  S32             [IN]        
 * RETURNS
 * 	language id 
 *****************************************************************************/
ire_language_id_enum ire_get_current_activate_myanmar_language()
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ire_language_id_enum language_id = IRE_LANGUAGE_ID_MYANMAR;
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    language_id = ire_get_current_myanmar_language_in_phone_setting();
    switch (ire_get_activate_myanmar_language())
    {
    case IRE_LANGUAGE_ID_MYANMAR:
        language_id = IRE_LANGUAGE_ID_MYANMAR;
        break;
    case IRE_LANGUAGE_ID_MYANMAR_ZAWGYI:
        language_id = IRE_LANGUAGE_ID_MYANMAR_ZAWGYI;
        break;
    }
    return language_id;
}
...
/*****************************************************************************
 * FUNCTION
 *  ire_get_current_myanmar_language_in_phone_setting
 * DESCRIPTION
 *  this api is used for get myanmar language id in phone setting
 * PARAMETERS
 *	void
 *  S32             [IN]        
 * RETURNS
...
static ire_language_id_enum ire_get_current_myanmar_language_in_phone_setting()
{
    ...
    /*----------------------------------------------------------------*/
    ire_language_id_enum language_id = IRE_LANGUAGE_ID_MYANMAR;
    U8 zawgyi_country_code[] = "my-ZA";
    U8 * current_coutry_code;
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    #if defined(__MMI_LANG_MYANMAR__) && defined(__MMI_LANG_ZAWGYI_MYANMAR__)
    mmi_li_Get_Current_LCC(&current_coutry_code);
    if (strcmp(current_coutry_code, zawgyi_country_code))
    {
        language_id = IRE_LANGUAGE_ID_MYANMAR;
    }
    else
    ...
    #elif defined(__MMI_LANG_MYANMAR__) 
    language_id = IRE_LANGUAGE_ID_MYANMAR;
    #elif defined(__MMI_LANG_ZAWGYI_MYANMAR__)
    language_id = IRE_LANGUAGE_ID_MYANMAR_ZAWGYI;
    #endif
    return language_id;
}
...
MMI_BOOL ire_is_zawgyi_language_activate_in_phone_setting()
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if (ire_get_current_myanmar_language_in_phone_setting() == IRE_LANGUAGE_ID_MYANMAR_ZAWGYI)
    {
        return MMI_TRUE;
    }
    else
...
/*****************************************************************************
 * FUNCTION
 *  ire_set_activate_myanmar_language
 * DESCRIPTION
 *  this api is used for set activate myanmar language id.
 * PARAMETERS
 *  activate_lang             [IN]        
 * RETURNS
 * 	language id 
 *****************************************************************************/
ire_language_id_enum ire_get_activate_myanmar_language()
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ire_language_id_enum lang_bak = ire_set_activate_myanmar_language(IRE_LANGUAGE_ID_INVALID);
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    ire_set_activate_myanmar_language(lang_bak);
    return lang_bak;
}
 ...
 * 	language id 
 *****************************************************************************/
ire_language_id_enum ire_set_activate_myanmar_language(ire_language_id_enum activate_lang)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    static ire_language_id_enum myanmar_activate_language = IRE_LANGUAGE_ID_INVALID;
    ire_language_id_enum old_lang = myanmar_activate_language;
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    myanmar_activate_language = activate_lang;
    return old_lang;
}
 ...
 * DESCRIPTION
 *  this api is used for get key mapping for mynamar & zawgyi.
 * PARAMETERS
 *  index             [IN]        
 * RETURNS
 * key mapping string 
 *****************************************************************************/
UI_string_type get_mynammar_multitap_string_ext(S32 index)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    switch (ire_get_current_myanmar_language_in_phone_setting())
    {
    #if defined(__MMI_LANG_ZAWGYI_MYANMAR__)
    case IRE_LANGUAGE_ID_MYANMAR_ZAWGYI:
        return get_zawgyi_multitap_string(index);
    #endif
    
    #if defined(__MMI_LANG_MYANMAR__)
    case IRE_LANGUAGE_ID_MYANMAR:
    default:
        return get_mynammar_multitap_string(index);
    #elif defined(__MMI_LANG_ZAWGYI_MYANMAR__)
    default:
        return get_zawgyi_multitap_string(index);
    #endif
...
/*****************************************************************************
 * FUNCTION
 *  ire_language_zawgyi_mynamar_get_cluster
 * DESCRIPTION			
 *  This method will identifies the characters which will participate in 
 *  cluster formation 
 * PARAMETERS
...
static U32 ire_language_zawgyi_mynamar_get_cluster(PU8 start_character, PU8 end_character, PU16 out_cluster, S32* out_len)
{
    ...
    U16 current_character = 0;
    PU8 current_character_p = start_character;
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/    
    current_character = IRE_LANG_ID_CONVERT_U8_U16(current_character_p);
    *out_len = 0;
    do
    {
        if(!IS_ZAWGYI_MYANMAR_RANG(current_character) || !zawgyi_check_and_update_current_state(start_character, *out_len, current_character, &current_char_state) && IS_ZAWGYI_MYANMAR_RANG(current_character))
        {
            break;
        }

        current_character_p += 2;
        out_cluster[*out_len] = zawgyi_convert_logic_encoding_to_rendering_encoding(current_character);
        (*out_len)++;
        current_character = IRE_LANG_ID_CONVERT_U8_U16(current_character_p);
    }while (current_character_p != end_character);
    out_cluster[*out_len] = 0;
    return *out_len;
}
...
static S32 ire_myanmar_zawgyi_language_rules(PU16 glyphoutput, PU16 cluster, S32 nentry)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    memcpy(glyphoutput, cluster, nentry*sizeof(U16));
    glyphoutput[nentry] = 0;
    return nentry;
}

/*****************************************************************************
 * FUNCTION
 *  ire_myanmar_zawgyi_language_get_text_x
 * DESCRIPTION			
 *  This method will apply grammer rules on the input cluster buffer
 *  and copy the output buffer i.e glyphoutput
 * PARAMETERS
 *  ptext_x     [OUT]   x position of character character should be.    
 *  prev_text_x         [IN/OUT]        x positon of prev character.
 *  prev_d_width         [IN/OUT]    d_width of prev character.
 *  prev_width             [IN/OUT]    width of prev character
 *  current_character   [IN]            current character
 *  d_width                  [IN]            d_width of current character
 *  width                      [IN]            width of current character
 *  is_new_line            [IN]            flag for check whether this character is first character of a cluster.
 *  RETURNS
 *  void
 *****************************************************************************/
static S32 ire_myanmar_zawgyi_language_get_text_x( 
    ...
    MMI_BOOL is_new_cluster)
{
    ...
    /*----------------------------------------------------------------*/
    S32 move_style = 0;
    static MMI_BOOL right_connect_character_on_first = MMI_FALSE; // this flag is used to check that the first character of a cluster can be overlap with next or not.
    static U16 first_character_d_width = 0;
    S32 add_width = 0;
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/    
    move_style = zawgyi_get_character_shift_direction(current_character);

    if (is_new_cluster)
    {
        if (move_style == 1)
        {
            right_connect_character_on_first = MMI_TRUE;
            first_character_d_width = d_width;
        }
        else
        {
            right_connect_character_on_first = MMI_FALSE;
            first_character_d_width = 0;
        }
        *prev_text_x = *ptext_x;
        *prev_width = width;
        *prev_d_width = width;   //when character is first one of cluster, we use width as dwith to avoid overlap. (because of d_width may be small than width in this language).
        add_width = width;
    }
    else
    {
        if (!right_connect_character_on_first)
        {
            if (move_style == -1) // means move to left
            {
                *ptext_x = *prev_text_x + *prev_width + d_width - width;
                *prev_width += d_width;
                *prev_d_width = *prev_width;
                add_width = d_width;
            }
            else 
            {
                *ptext_x = *prev_text_x + *prev_d_width;
                *prev_text_x = *ptext_x;
                *prev_width = width;
                *prev_d_width = width;
                add_width = width;
        ...
        else // means no movement, draw character as a cluster.
        {
            if (move_style == -1)
            {
                *ptext_x = *prev_text_x + first_character_d_width - (width - d_width);
                *prev_d_width = first_character_d_width + d_width;
            }
            else
            {
                *ptext_x = *prev_text_x + first_character_d_width;
                *prev_d_width = first_character_d_width + width;
            }
            if (*prev_d_width > *prev_width)
            {
                add_width = *prev_d_width - *prev_width;
                *prev_width = *prev_d_width;
            }
            else
            {
                *prev_d_width = *prev_width;
