...
#define GUI_BUTTON_MOVE_THRESHOLD 8
static S32 bt_mv_start_x = 0;
static S32 bt_mv_start_y = 0;
static MMI_BOOL bt_hanlde_valid = MMI_TRUE;

static void gui_button_move_abs_delta(S32 *x, S32 *y)
{
	/*----------------------------------------------------------------*/
	/* Local Variables												  */
	/*----------------------------------------------------------------*/
...
void gui_show_icontext_button(icontext_button *b)
{
    ...
    if (!(b->flags & UI_BUTTON_DISABLE_BACKGND_DISPLAY) && (f != NULL))
    {
        ...
        {
        	gdi_color color_temp;
 	  #ifdef __MMI_WEARABLE_DEVICE__ //
			if (wgui_cat_is_in_call_cateogry(b))
			{
				if (b->flags & UI_BUTTON_STATE_DOWN)
		 			color_temp = gdi_act_color_from_rgb(255, 249, 69, 69);
				else if (b->flags & UI_BUTTON_STATE_DISABLED)
					color_temp = gdi_act_color_from_rgb(255, 96, 97, 98);
		 		else
					color_temp = gdi_act_color_from_rgb(255, 214, 46, 46);
		   	  	gdi_draw_solid_rect(
		                b->x - offset_x,
		                b->y - offset_y,
		                b->x + b->width - 1 - offset_x,
    ...
    else
    {
		...
		if (!(b->flags & UI_BUTTON_DISABLE_BACKGND_DISPLAY)
			&& wgui_cat_is_in_call_cateogry(b))
		{
			gdi_color color_temp;
			gui_push_and_set_clip(
	            b->x - offset_x,
    ...
    if (!(b->flags & UI_BUTTON_DISABLE_TEXT_DISPLAY))
    {
       ...
       // gui_push_text_clip();
        gui_push_and_set_clip(x1, y1, x2, y2);
	   #ifdef __MMI_WEARABLE_DEVICE__
		b->flags &= ~UI_BUTTON_TEXT_WITH_BORDER;
	    if (wgui_cat_is_in_call_cateogry(b))
	   	{
    ...
    /* disable the VFont translucence effect */
        ...
        if (need_truncate == MMI_TRUE)
        {
            if (mmi_fe_get_r2l_state())
            {
            #if defined(__MMI_FTE_SUPPORT__) && defined(__MMI_VECTOR_FONT_SUPPORT__) || defined(__MMI_WEARABLE_DEVICE__)
    ...
    switch (pen_event)
    {
        ...
        case MMI_PEN_EVENT_UP:
            ...
            if (pen_state->pen_inside)
            {
				...
				else if (MMI_FALSE == bt_hanlde_valid)
				{
					bt_hanlde_valid = MMI_TRUE;
					ret = MMI_FALSE;
				}
        ...
        case MMI_PEN_EVENT_MOVE:
            if (PEN_CHECK_BOUND(x, y, button_x, button_y, button_width, button_height))
            {
                ...
                {
                	#ifdef __MMI_WEARABLE_DEVICE_UI_ENHANCE__
					delta_x = x - bt_mv_start_x;
					delta_x = y - bt_mv_start_y;
            		gui_button_move_abs_delta(&delta_x, &delta_y);
					if (x > GUI_BUTTON_MOVE_THRESHOLD || y > GUI_BUTTON_MOVE_THRESHOLD)
					{
						bt_hanlde_valid = MMI_FALSE;
						if (clear_background_fp)
	                    {
	                        clear_background_fp(button_object);
	                    }
