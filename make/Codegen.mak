...
ifeq ($(strip $(BUILD_UBIN_ONLY)),TRUE)
codegen : gencustominfo gencustomlists xml_parser nvram_edit_h_gen nvram_auto_gen genmoduleinfo cgen
else
...
ifeq ($(strip $(AUTO_CHECK_DEPEND)),TRUE)
	...
	@(perl $(FIXPATH)\tools\sys_auto_gen.pl $(SCATTERFILE) $(BL_SCATTERFILE) $(FOTA_SCATTERFILE) $(strip $(THE_MF)) $(FIXPATH)\custom\system\$(strip $(BOARD_VER)) $(strip $(NEED_BUILD_BOOTLOADER)) $(strip $(INSIDE_MTK)) $(strip $(DUMMY_LIS)) $(strip $(FACTORY_BIN_FOLDER))\\FACTORY_ROM $(strip $(OBJ_SYS_AUTO_GEN)) $(strip $(TS_SCATTERFILE)) "$(CC)" "$(VIA)" $(strip $(UBIN_SCATTERFILE))>$(strip $(COMPLOGDIR))\sysgen.log 2>&1) & \
