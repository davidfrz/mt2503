...
{	
    ...
    if(!srv_mre_appmgr_check_current_defined_authorization(SRV_MRE_APPMGR_APP_AUTH_SMS_SEND))
    {
        MMI_TRACE(MMI_MRE_TRC_MOD_VMSMS, TRC_MRE_SMS_SEND_FAIL);
		    return FALSE;
    }
    else
    {
        MMI_TRACE(MMI_MRE_TRC_MOD_VMSMS, TRC_MRE_SMS_SEND_SUCCESS);
    }
#endif
...
#ifdef __MMI_MRE_AUTH_SUPPORT__
    if(!srv_mre_appmgr_check_current_defined_authorization(SRV_MRE_APPMGR_APP_AUTH_SMS_READ))
    {
        MMI_TRACE(MMI_MRE_TRC_MOD_VMSMS, TRC_MRE_SMS_READ_FAIL);
		    return VM_SMS_REQ_ERROR;
    }
    else
    {
        MMI_TRACE(MMI_MRE_TRC_MOD_VMSMS, TRC_MRE_SMS_READ_SUCCESS);
    }
#endif
...
#ifdef __MMI_MRE_AUTH_SUPPORT__
    if(!srv_mre_appmgr_check_current_defined_authorization(SRV_MRE_APPMGR_APP_AUTH_SMS_DELETE))
    {
        MMI_TRACE(MMI_MRE_TRC_MOD_VMSMS, TRC_MRE_SMS_DELETE_FAIL);
        return FALSE;
    }
    else
    {
         MMI_TRACE(MMI_MRE_TRC_MOD_VMSMS, TRC_MRE_SMS_DELETE_SUCCESS);
    }
#endif
...
VMINT vm_sms_delete_msg_list(vm_sms_box_enum box_type,  vm_sms_sim_enum sim_id, VmSmsCallbackFunc callback_func,  void* user_data)
{
    ...
    if(!srv_mre_appmgr_check_current_defined_authorization(SRV_MRE_APPMGR_APP_AUTH_SMS_DELETE))
    {
        MMI_TRACE(MMI_MRE_TRC_MOD_VMSMS, TRC_MRE_SMS_DELETE_LIST_FAIL);
        return FALSE;
    }
    else
    {
        MMI_TRACE(MMI_MRE_TRC_MOD_VMSMS, TRC_MRE_SMS_DELETE_LIST_SUCCESS);
    }
#endif
            ...
            case SRV_SMS_ACTION_QUERY:
                ...
                if(callback_data->action_data != NULL)
                {
                    if(callback_data_ptr.cause == VM_SMS_CAUSE_NO_ERROR)	
                    {
                        ...
                        query_msg_ptr.msg_id_list = NULL;
                        if(query_msg_ptr.msg_num == 0)
                        {
                            vm_log_error("vm_sms_callback:msg_num == 0");
                            break;
                        }
                        else
                        {
                            vm_log_error("vm_sms_callback:msg_num = %d", query_msg_ptr.msg_num);
                        }
...
VMBOOL vm_sms_is_sms_ready(void)
{
    ...
    if(srv_sms_is_sms_ready()==0)  
    {
        vm_log_error("vm_sms_is_sms_ready:srv_sms_is_sms_ready()==0");
        return VM_FALSE;
    }
#endif
    return VM_TRUE;
