...
#ifdef GDI_BW_FLATTEN_TO_BASELAYER
	#if defined(__MTK_TARGET__)
	#pragma arm section zidata = "NONCACHEDZI", rwdata = "NONCACHEDRW"
	#endif
	__align(32) static U8
	lcd_blt_layer_buf[(GDI_LCD_WIDTH*GDI_LCD_HEIGHT*GDI_MAINLCD_BIT_PER_PIXEL)>>3];
	#if defined(__MTK_TARGET__)
	#pragma arm section zidata, rwdata
	#endif
	static gdi_handle lcd_blt_layer_handle = NULL;
#endif
...
extern kal_bool gdi_lcd_is_hw_updating;

#ifdef TOUCH_PEN_EVENT_TRACK_DISPLAY_AND_PREDICT
	extern gdi_handle touch_track_layer;
#endif
...
#endif /* GDI_LCD_SCANLINE_ROTATION */
#ifdef GDI_BW_FLATTEN_TO_BASELAYER
		if ((KAL_FALSE == kal_query_systemInit()) && (KAL_FALSE == gdi_lcd_is_hw_updating))
		{
			gdi_layer_push_and_set_active(lcd_blt_layer_handle);
			ret = gdi_layer_flatten_ext(
								handle0,
								handle1,
								handle2,
								handle3
