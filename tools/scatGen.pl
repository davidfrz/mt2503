...
my $TS_MEM_CONFIG_C       = $ARGV[4] . '\\' . "ts_mem_config.c"; # for tiny_system
...
my $IOTFOTA_SCATTERFILE   = $ARGV[11]; # for IOTFOTA
...
&sysUtil::del_noCheckinHistory_file($TS_MEM_CONFIG_C);
...
{
    &GenTS_MEM_CONFIG_C($TS_MEM_CONFIG_C) if (!-e $TS_MEM_CONFIG_C);
...
#****************************************************************************
# 23 >>> Generate update bin scatter file  when UPDATE_BINARY_FILE is TRUE
#**************************************************************************** 
if(exists $MAKEFILE_OPTIONS{'UPDATE_BINARY_FILE'} and $MAKEFILE_OPTIONS{'UPDATE_BINARY_FILE'} eq 'TRUE' and $g_USE_DUMMY_SCATTER ne 'TRUE')
{
    &sysUtil::del_noCheckinHistory_file($IOTFOTA_SCATTERFILE);
    if(!-e $IOTFOTA_SCATTERFILE)
    {
        &GenIOTFOTAScatter();
    }
}
...
#****************************************************************************
# subroutine:  GenIOTFOTAScatter: Generate IOTFOTA update bin scatter file
# input: 	     $filepath : the file path to save a scatter layout 
# return:      x
# require:    tools/scatGenIOTFOTA.pl -> scatGenIOTFOTA_main()
#****************************************************************************
sub GenIOTFOTAScatter
{
    require "tools/scatGenIOTFOTA.pl";
    require "tools/scatGenLib.pl";
    PrintDependModule();

    my $scat_content = &scatGenIOTFOTA_main(sub { my ($key) = @_;
                                                  if (exists $MAKEFILE_OPTIONS{$key})
                                                  {   return $MAKEFILE_OPTIONS{$key};
                                                  }
                                                  else
    ...
    open (IOTFOTA_SCATTER_TXT, ">$IOTFOTA_SCATTERFILE") or &sysUtil::sysgen_die("[2.0]$IOTFOTA_SCATTERFILE: file error!", __FILE__, __LINE__);    
    print IOTFOTA_SCATTER_TXT &copyright_file_header_for_scatter();
    print IOTFOTA_SCATTER_TXT &description_file_header_for_scatter( $IOTFOTA_SCATTERFILE,
                           "defines the memory map for IOTFOTA\n",
                           "system auto generator ". &scatGen_verno(). " + sysGenUtility" . &sysUtil::sysgenUtility_verno());
    print IOTFOTA_SCATTER_TXT $scat_content;
    close IOTFOTA_SCATTER_TXT;
    
    print "$IOTFOTA_SCATTERFILE is generated\n";
}
#****************************************************************************
...
sub custom_img_config_h_file_body
{
    ...
    my ($ts_rom3_begin_addr_on_flash, $str_ts_rom3_begin_addr_on_flash,$ts_rom3_string) = (0x0,"0x0",""); #for TINY_SYS, feature on MT6261 and after
...
__TEMPLATE
    ...
    else
    {
       	...
       	if (&sysUtil::is_sv5($bb) == 1)
        {
            ...
            my $diff_fota = $MAKEFILE_OPTIONS{'fudiffnet_enable'};
            if(defined $diff_fota and $diff_fota eq 'DIFF_IMAGE_UPDATE')
            {
                my $rock_fota = $MAKEFILE_OPTIONS{'ROCK_FOTA_SUPPORT'};
            ...
            my $funet_enable = $MAKEFILE_OPTIONS{'FUNET_ENABLE'};
            ...
            if(&FileInfo::is("tiny_sys", "TRUE", \%MAKEFILE_OPTIONS))
            {
                ...
                ($TS_ROM1_BASE,$TS_ROM1_SIZE) = &sysUtil::GetTS_IntsramInfo_under_tiny_mode($BOARD_PARAMETERS{'BB'});
                #TS_ROM2 LV&EV is on  DSPRAM 
                ($TS_ROM2_BASE,$TS_ROM2_SIZE) = &sysUtil::GetTS_DspramInfo($BOARD_PARAMETERS{'BB'});
                ...
                $TS_ROM1_SIZE = (exists $FEATURE_CONFIG_Value{MAX_TS_ROM1_SIZE})? hex($FEATURE_CONFIG_Value{MAX_TS_ROM1_SIZE}) 
                                                                                : hex($TS_ROM1_SIZE) ;
                $TS_ROM2_SIZE = (exists $FEATURE_CONFIG_Value{MAX_TS_ROM2_SIZE})? hex($FEATURE_CONFIG_Value{MAX_TS_ROM2_SIZE}) 
                                                                                : hex($TS_ROM2_SIZE) ;

                my ($TS_ROM3_BASE,$TS_ROM3_SIZE) = (0,0);
                ($TS_ROM3_BASE,$TS_ROM3_SIZE) = &sysUtil::GetTS_ROM3Info($BOARD_PARAMETERS{'BB'});
                $TS_ROM3_SIZE = (exists $FEATURE_CONFIG_Value{MAX_TS_ROM3_SIZE})? hex($FEATURE_CONFIG_Value{MAX_TS_ROM3_SIZE}) 
                                                                                : hex($TS_ROM3_SIZE) ;
                ...
                $ts_rom3_begin_addr_on_flash = $nBaseAddress ^ $cs_swap_address ;	
                $str_ts_rom3_begin_addr_on_flash = sprintf("0x%08X", $ts_rom3_begin_addr_on_flash);
                $nBaseAddress += $TS_ROM3_SIZE;
                
                $ts_rom3_string = "#define TINYSYS_ROM3_BEGIN_ADDRESS $str_ts_rom3_begin_addr_on_flash";
...
#****************************************************************************
# subroutine:  ts_mem_config_c_file_body
#****************************************************************************
my @TS_MEM_VARIABLES;
sub ts_mem_config_c_file_body()
{
    ...
    my ($strTSMMDumpSize,$strAPIntsramBakSize,$strTSLogSize,$strTSResEmiSize,$strCSCIInfoSize) = &sysUtil::GetTS_ResMemSize(sub { my ($key) = @_;
                                                                                                                              ...
                                                                                                                              $bb,
                                                                                                                              \%FEATURE_CONFIG_Value);
    $strTSMMDumpSize = (exists $FEATURE_CONFIG_Value{MAX_TS_MM_DUMP_SIZE})? $FEATURE_CONFIG_Value{MAX_TS_MM_DUMP_SIZE}
                                                                          : $strTSMMDumpSize;
    ...
    my ($strTSMMDumpBase,$strAPIntsramBakBase,$strTSLogBase,$strTSResEmiBase,$strCSCIInfoBaseAP,$strCSCIInfoBaseTS);
    ...
    $strAPIntsramBakBase = &CommonUtil::Dec2Hex(hex($strTSMMDumpBase)-hex($strAPIntsramBakSize));
    ...
    my ($strTSEMICodeEVBase,$strTSEMICodeEVSize) = ("0x0","0x0");
    ($strTSEMICodeEVBase,$strTSEMICodeEVSize) = &sysUtil::GetTS_ROM3Info($bb);
    $strTSEMICodeEVSize = (exists $FEATURE_CONFIG_Value{MAX_TS_ROM3_EMI_SIZE})? $FEATURE_CONFIG_Value{MAX_TS_ROM3_EMI_SIZE}
                                                                          : $strTSEMICodeEVSize;

    # always gen :kal_uint32 MAX_TS_EMI_CODE_SIZE = (kal_uint32)$strTSEMICodeEVSize;
    #  kal_uint32 TS_EMI_CODE_BASE = (kal_uint32)$strTSEMICodeEVBase; to avoid build error
    $strTSEMICodeEVBase = &CommonUtil::Dec2Hex(hex($strTSResEmiBase) - hex($strTSEMICodeEVSize));
    $template = <<"__TEMPLATE";
#include "kal_general_types.h"
...
/************************EMI and DSPRAM information ******************************/
kal_uint32 EMI_END_ADDRESS = (kal_uint32)$strExtsramSize;
kal_uint32 TS_DSPRAM_BASE = (kal_uint32)$strDspRamBase;
kal_uint32 MAX_TS_DSPRAM_SIZE = (kal_uint32)$strDspRamSize;

__TEMPLATE
push @TS_MEM_VARIABLES, "EMI_END_ADDRESS";
push @TS_MEM_VARIABLES, "TS_DSPRAM_BASE";
push @TS_MEM_VARIABLES, "MAX_TS_DSPRAM_SIZE";

    $temCSCI = <<"__TEMPLATE";
/************************CSCI INFO SIZE AND BASE DEFINITION **********************/
kal_uint32 MAX_CSCI_INFO_SIZE = (kal_uint32)$strCSCIInfoSize;
kal_uint32 CSCI_INFO_BASE_ON_AP = (kal_uint32)$strCSCIInfoBaseAP;
kal_uint32 CSCI_INFO_BASE_ON_TINY = (kal_uint32)$strCSCIInfoBaseTS;

__TEMPLATE
push @TS_MEM_VARIABLES, "MAX_CSCI_INFO_SIZE";
push @TS_MEM_VARIABLES, "CSCI_INFO_BASE_ON_AP";
push @TS_MEM_VARIABLES, "CSCI_INFO_BASE_ON_TINY";

    $temResMem = << "__TEMPLATE";
/***********************RESERVED MEMORY ON EMI FOR TINY_SYSTEM *******************/
/*user_1: tiny_mem_dump, backup memory dump info of TS mode to EMI before exchange to AP mode*/
kal_uint32 MAX_TS_MEM_DUMP_SIZE = (kal_uint32)$strTSMMDumpSize;
kal_uint32 TS_MM_DUMP_BASE = (kal_uint32)$strTSMMDumpBase;

/*user_2: ap_intsram_bak, backup INTSRAM info on AP to EMI when exchange to tiny system*/
kal_uint32 MAX_AP_INTSRAM_BAK_SIZE = (kal_uint32)$strAPIntsramBakSize;
kal_uint32 AP_INTSRAM_BAK_BASE = (kal_uint32)$strAPIntsramBakBase;

/*user_3: tiny_logging, bakeup log on tiny system to EMI*/
kal_uint32 MAX_TS_LOGGING_SIZE = (kal_uint32)$strTSLogSize;
kal_uint32 TS_LOGGING_BASE = (kal_uint32)$strTSLogBase;

/*user_4: TS_EMI_CODE_EV, reserve extSRAM to support tiny logging ,init code putting on ram */
kal_uint32 MAX_TS_EMI_CODE_SIZE = (kal_uint32)$strTSEMICodeEVSize;
kal_uint32 TS_EMI_CODE_BASE = (kal_uint32)$strTSEMICodeEVBase;

__TEMPLATE
push @TS_MEM_VARIABLES, "MAX_TS_MEM_DUMP_SIZE";
push @TS_MEM_VARIABLES, "TS_MM_DUMP_BASE";
push @TS_MEM_VARIABLES, "MAX_AP_INTSRAM_BAK_SIZE";
push @TS_MEM_VARIABLES, "AP_INTSRAM_BAK_BASE";
push @TS_MEM_VARIABLES, "MAX_TS_LOGGING_SIZE";
push @TS_MEM_VARIABLES, "TS_LOGGING_BASE";
push @TS_MEM_VARIABLES, "MAX_TS_EMI_CODE_SIZE";
push @TS_MEM_VARIABLES, "TS_EMI_CODE_BASE";
...
#****************************************************************************
# subroutine:  GenTS_MEM_CONFIG_C: Generate ts_mem_config.c
# input:      $filepath : ts_mem_config.c path
# return:      x
# require:    tools/scatGenLib.pl -> scatGen_verno();
#****************************************************************************
sub GenTS_MEM_CONFIG_C
{
    my ($strFilePath) = @_;
    open (TS_MEM_CONFIG_C, ">$strFilePath") or &sysUtil::sysgen_die("[2.0]$strFilePath: file error!", __FILE__, __LINE__);
    PrintDependency($strFilePath);
    print TS_MEM_CONFIG_C &sysUtil::copyright_file_header();
    require "tools/scatGenLib.pl";
    PrintDependModule();
    print TS_MEM_CONFIG_C &sysUtil::description_file_header(    "ts_mem_config.c",
    ...
    print TS_MEM_CONFIG_C &ts_mem_config_c_file_body();
    close TS_MEM_CONFIG_C;
