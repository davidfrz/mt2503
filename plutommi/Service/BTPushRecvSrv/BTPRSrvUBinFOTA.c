 ...
 * Description:
 * ------------
 *   This file is for seperate BIN FOTA.
 *
 * Author:
 * -------
 * -------
...
static FS_HANDLE g_ubin_fd = -1;
static WCHAR* g_ubin_path = NULL;
static S32 g_ubin_pack_num = -1; //for one firmware file,  package number splited from SP, watch should receive all  these packages.
static MMI_BOOL is_trigger_update = MMI_FALSE; // normal reboot is async process, in this process, can not receive other ubin again.

static void srv_btpr_ubin_fota_ext_cmd_cb(void *data);
#define FOTA_REBOOT_DELAY_DURATION   5000

void srv_btpr_ubin_fota_init(void)
{
    //register update BIN FOTA command hdlr
    srv_bt_notify_register_result_enum reg_ret = srv_bt_noti_register_callback_ext(
                                                                       BT_PUSH_EXTEND_CMD, 
                                                                       BT_PUSH_UPDATE_BIN_SENDER, 
                                                                       srv_btpr_ubin_fota_ext_cmd_cb);
    if (SRV_BT_NOTIFY_REGISTER_RET_OK == reg_ret)
    {
        MMI_TRACE(MMI_CONN_TRC_G4_FOTA, TRC_SRV_BTPUSHRECV_REGISTER_UBIN_HDLR_SUCCESS);
    }
    else
    {
        MMI_TRACE(MMI_CONN_TRC_G4_FOTA, TRC_SRV_BTPUSHRECV_REGISTER_UBIN_HDLR_FAIL);
    }

    g_ubin_path = srv_funet_get_ubin_path();
    return;
}

static void srv_btpr_ubin_fota_recv_ubin_data_start(srv_bt_noti_callback_struct *p_data)
{
    // if SP transfer data to watch, SP or its BT power off, watch may not receive end package, so data is not invalid
    if (g_ubin_fd >= FS_NO_ERROR)
    {
        MMI_TRACE(MMI_CONN_TRC_G4_FOTA, TRC_SRV_BTPUSHRECV_UBIN_FD_ALREADY_EXIST);
        FS_Close(g_ubin_fd);
        g_ubin_fd = -1;
    }

    g_ubin_fd = FS_Open(g_ubin_path, FS_CREATE_ALWAYS | FS_READ_WRITE);
    if (g_ubin_fd < FS_NO_ERROR)
    {
        MMI_TRACE(MMI_CONN_TRC_G4_FOTA, TRC_SRV_BTPUSHRECV_OPEN_UBIN_FILE_FAIL);
        srv_bt_push_recv_reply_int_value(BT_PUSH_UPDATE_BIN_SENDER, BT_PUSH_UPDATE_BIN_RECEIVER, BT_PUSH_RECV_WRITE_FILE_FAIL);
    }
    else
    {
        g_ubin_pack_num = atoi(p_data->data.buffer.data);
        MMI_TRACE(MMI_CONN_TRC_G4_FOTA, TRC_SRV_BTPUSHRECV_NEED_RECEIVE_PACKAGES, g_ubin_pack_num);
    }
    return;
}

static mmi_ret srv_btpr_ubin_fota_del_ubin_cb(mmi_event_struct *param)
{
    MMI_TRACE(MMI_CONN_TRC_G4_FOTA, TRC_SRV_BTPUSHRECV_DELETE_UBIN_FILE_SUCCESS);
    return 0;
}

static void srv_btpr_ubin_fota_recv_ubin_data_pack(srv_bt_noti_callback_struct *p_data)
{
    U32 nLen;
    S32 ret;
    ...
    if (g_ubin_fd >= FS_NO_ERROR)
    {
        ret = FS_Write(g_ubin_fd, p_data->data.buffer.data, p_data->data.buffer.len, &nLen);
        if (ret >= FS_NO_ERROR && p_data->data.buffer.len == nLen)
        {
            g_ubin_pack_num--; // to ensure data integrity which receiver from remote device
        }
        else if (FS_APP_QUOTA_FULL == ret)
        {
            MMI_TRACE(MMI_CONN_TRC_G4_FOTA, TRC_SRV_BTPUSHRECV_FOTA_UBIN_QUOTA_NOT_ENOUGH);
            FS_Close(g_ubin_fd);
            g_ubin_fd = -1;
            /*if write file fail, delete file, reply fail error to SP*/
            srv_fmgr_async_delete(g_ubin_path, 0, srv_btpr_ubin_fota_del_ubin_cb, NULL);
        ...
        {
            MMI_TRACE(MMI_CONN_TRC_G4_FOTA, TRC_SRV_BTPUSHRECV_WRITE_UBIN_FILE_FAIL);
            FS_Close(g_ubin_fd);
            g_ubin_fd = -1;
            /*if write file fail, delete file, reply fail error to SP*/
            srv_fmgr_async_delete(g_ubin_path, 0, srv_btpr_ubin_fota_del_ubin_cb, NULL);
...
static MMI_BOOL srv_btpr_ubin_fota_recv_ubin_data_end()
{
    if (g_ubin_fd >= FS_NO_ERROR)
    {
        FS_Close(g_ubin_fd);
...
static void srv_btpr_ubin_fota_funet_trigger()
{
    if (SRV_FUNET_FLAG_SUCCESS == srv_funet_trigger(g_ubin_path, SRV_FUNET_UPDATE_FULL))
    {
...
void srv_btpr_ubin_fota_recv_ubin(srv_bt_noti_callback_struct *data)
{
    MMI_PRINT(MOD_MMI_CONN_APP, MMI_CONN_TRC_G4_FOTA, "[BTPushRecvSrv] receive ubin, data len = %d, err_code = %d", 
                                                               data->data.buffer.len, data->err_code);
    switch (data->err_code)
    {
        case BT_PUSH_RECV_FILE_DATA_BEGIN:
            {
                srv_btpr_ubin_fota_recv_ubin_data_start(data);
            }
            break;
        case BT_PUSH_RECV_FILE_DATA_PACK:
            {
                srv_btpr_ubin_fota_recv_ubin_data_pack(data);
            }
            break;
        case BT_PUSH_RECV_FILE_DATA_END:
            if (srv_btpr_ubin_fota_recv_ubin_data_end() == MMI_TRUE)
            {
				...
				*/
				...
				{
					srv_btpr_ubin_fota_funet_trigger();
				}
...
static void srv_btpr_ubin_fota_ext_cmd_cb(void *data)
{
    ...
    {
        if (!strcmp(p_data->sender_id, BT_PUSH_UPDATE_BIN_SENDER) &&
			!strcmp(p_data->receiver_id, BT_PUSH_UPDATE_BIN_RECEIVER) &&
			is_trigger_update != MMI_TRUE)
        {
            srv_btpr_ubin_fota_recv_ubin(p_data);
        }
    }
...
{
    if (srv_fmgr_fs_path_exist(srv_funet_get_ubin_path()) >= 0)
    {
        srv_funet_update_ret_enum fota_ret;

        MMI_TRACE(MMI_CONN_TRC_G4_FOTA, TRC_SRV_BTPUSHRECV_UBIN_IS_EXIST);
