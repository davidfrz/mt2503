...
MMI_BOOL g_mmi_slk_handle_temp_move_rgt = MMI_TRUE;
MMI_BOOL g_mmi_slk_handle_temp_state = MMI_FALSE;
static void mmi_slk_handle_set_move_right_flag(MMI_BOOL state);
static MMI_BOOL mmi_slk_handle_get_move_right_flag(void);
#endif
...
#ifdef __MMI_WEARABLE_DEVICE__
static mmi_ret mmi_slk_handle_move_right_hdlr(mmi_event_struct *evt)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    kal_prompt_trace(MOD_MMI,"[Scrlocker]mmi_slk_handle_move_right_hdlr, %d", evt->evt_id);
    switch (evt->evt_id)
    {
        case EVT_ID_WEARABLE_DEVICE_MOVE_RIGHT:
        {
            if(!mmi_slk_handle_get_move_right_flag())
            {
                kal_prompt_trace(MOD_MMI,"[Scrlocker]MMI_RET_ERR");
                return MMI_RET_ERR;
            }
            break;
        }
...
static void mmi_slk_handle_set_move_right_flag(MMI_BOOL state)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    g_mmi_slk_handle_temp_move_rgt = state;
    kal_prompt_trace(MOD_MMI,"[Scrlocker] move_right_flag = %d",g_mmi_slk_handle_temp_move_rgt);
}
...
static MMI_BOOL mmi_slk_handle_get_move_right_flag(void)
{
    ...
    /*----------------------------------------------------------------*/
    kal_prompt_trace(MOD_MMI,"[Scrlocker] move_right_flag = %d",g_mmi_slk_handle_temp_move_rgt);
    return g_mmi_slk_handle_temp_move_rgt;
}
...
static mmi_ret mmi_slk_handle_pre_touch_routing(mmi_event_struct *event)
{
...
#ifdef __MMI_WEARABLE_DEVICE__
    if((srv_backlight_get_lcd_state(SRV_BACKLIGHT_TYPE_MAINLCD) == 0) || (srv_backlight_get_lcd_state(SRV_BACKLIGHT_TYPE_MAINLCD) == 1))
    {
        if((touch_data->event == MMI_PEN_EVENT_DOWN))
        {
            g_mmi_slk_handle_temp_state = MMI_TRUE;
            mmi_slk_handle_set_move_right_flag(MMI_FALSE);
            mmi_frm_cb_reg_event(EVT_ID_WEARABLE_DEVICE_MOVE_RIGHT, mmi_slk_handle_move_right_hdlr, NULL);
            return MMI_RET_STOP_TOUCH_HANDLE;
        }	
    }
    else if(srv_backlight_get_lcd_state(SRV_BACKLIGHT_TYPE_MAINLCD) == 2)
    {
        if((touch_data->event == MMI_PEN_EVENT_DOWN))
        {
            g_mmi_slk_handle_temp_state = MMI_FALSE;
            mmi_slk_handle_set_move_right_flag(MMI_TRUE);
            mmi_frm_cb_dereg_event(EVT_ID_WEARABLE_DEVICE_MOVE_RIGHT, mmi_slk_handle_move_right_hdlr, NULL);
        }	
    }

    if((touch_data->event == MMI_PEN_EVENT_UP) && g_mmi_slk_handle_temp_state == MMI_TRUE)
    {
        g_mmi_slk_handle_temp_state = MMI_FALSE;
    ...
    if(((srv_backlight_get_lcd_state(SRV_BACKLIGHT_TYPE_MAINLCD) == 0) || (srv_backlight_get_lcd_state(SRV_BACKLIGHT_TYPE_MAINLCD) == 1)) && g_mmi_slk_handle_temp_state == MMI_TRUE) 
    {
        return MMI_RET_STOP_TOUCH_HANDLE;
    }
#endif
...
void mmi_scr_locker_start_timer(void)
{
    ...
    {
        #ifdef __TINY_SYS__
        StartTimer_ts(IDLE_APP_AUTO_LOCK_TIMER_ID, time, mmi_slk_auto_lock_timeout_hdlr,MMI_TRUE);
        #else
