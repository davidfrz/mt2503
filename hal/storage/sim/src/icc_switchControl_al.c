...
usim_status_enum L1sim_Reset_All(sim_power_enum ExpectVolt, sim_power_enum *ResultVolt, kal_bool warm, SIM_ICC_APPLICATION application)
{
	...
	if(KAL_TRUE  == simfactorytest)
	{
		if(application & LOCAL_MODE_MAGIC){ //request from functional test, revise magic for further usage
			...
			kal_print("return sim interface query");
		}
		else{ //request from SIM task, return SIM not found directly

		kal_print("factory mode return SIM_NO_INSERT for sim task");
	...
	
	{
		...
		sprintf(sim_dbg_str,"if:%d default polarity:%d polarity:%d eint:%d ",simHwCb->simInterface,simHwCb->Defaultpolarity
		,simHwCb->polarity,simHwCb->eint);
		kal_print(sim_dbg_str);	

		sprintf(sim_dbg_str,"0xa0060380:0x%x  0xa0060318:0x%x  0xa0060300:0x%x  0xa0060330:0x%x  0xa0060338:0x%x",SIM_Reg(0xa0060380),SIM_Reg(0xa0060318)
			,SIM_Reg(0xa0060300),SIM_Reg(0xa0060330),SIM_Reg(0xa0060338));
		kal_print(sim_dbg_str);	
