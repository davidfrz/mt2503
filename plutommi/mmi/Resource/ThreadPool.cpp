...
ThreadPool::ThreadPool(int poolSize, int waitTimeForThreadsToCompleteMS)
{
    ...
    THREADP_LOG_V("InitializeCriticalSection");
    InitializeCriticalSection(&m_cs);

    THREADP_LOG_V("Create(poolSize) = %d", poolSize);
...
bool ThreadPool::Create(int poolSize)
{
    ...
    if (m_hNotifyShutdown == NULL)
    {
		...
		THREADP_LOG_E("CreateEvent(): threadError = %d", threadError);
    }
    ...
    for (int nIndex = 0; nIndex < poolSize; ++nIndex)
    {
		...
		{
		    DWORD threadError = GetLastError();
			THREADP_LOG_E("_beginthreadex() threadError = %d", threadError);
		}
