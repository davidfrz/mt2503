...
mmi_ret mmi_sms_entry_msg_list_scrn_proc (mmi_event_struct *evt);
void mmi_sms_entry_inbox_with_check(void);
#endif
...
#if defined(__MMI_BT_MAP_CLIENT__) && defined(__MMI_WEARABLE_DEVICE__) && defined(__MMI_TELEPHONY_SUPPORT__)
void mmi_um_entry_main_message_enter_local_msg(void);
void mmi_um_entry_main_message_enter_bt_msg(void);
#endif
mmi_um_entry_type g_um_entry_type = MMI_UM_ENTRY_NONE;
...
/* under construction !*/
	...
	if(g_um_entry_type == MMI_UM_ENTRY_BT)
     {
     	mmi_um_ui_init();
	     srv_um_cache_init();
	     srv_um_listmgr_init();
...
void mmi_um_entry_main_message_enter_local_msg(void)
{
	g_um_entry_type = MMI_UM_ENTRY_LOCAL;
#ifdef __MMI_SUPPORT_ASMV2__
...
void mmi_um_entry_main_message_enter_bt_msg(void)
{
	g_um_entry_type = MMI_UM_ENTRY_BT;
#ifdef __MMI_BT_MAP_CLIENT__
    mmi_sms_bt_app_init();
#endif /* __MMI_BT_MAP_CLIENT__ */
...
MMI_ID g_sms_conn_cui_id;
mmi_sms_bt_scrn_type_enum dis_style = 0;/*1, map disconnect. 2, bt disconnect*/
extern MMI_ID g_sms_cui_id;
extern MMI_BOOL g_um_is_new_group;

void mmi_sms_bt_main_msg_entry_inbox_ext(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    mmi_frm_group_close(GRP_ID_SMS_BT_CONFIM); 
    mmi_sms_um_bt_ui_sh_main_msg_entry_inbox();
}


void mmi_um_entry_main_inbox_sync_done(void)
{
#ifdef __MMI_BT_MAP_CLIENT__
    mmi_sms_bt_app_init();
#endif /* __MMI_BT_MAP_CLIENT__ */
...
void mmi_sms_um_conn_retry_confirm_scrn(srv_bt_cm_bt_addr* dev_addr)
{
    if (mmi_frm_scrn_enter(
            g_sms_bt_gid,
            SCR_ID_SMS_DUMMY,
            (FuncPtr) NULL,
            (FuncPtr) mmi_sms_um_conn_retry_confirm_scrn,
    ...
    ShowCategory163Screen(
            ...
            (UI_string_type) GetString(STR_UM_SYNCING_MSG_LIST_ID),
            mmi_get_event_based_image(MMI_EVENT_PROGRESS),
            NULL);

    SetRightSoftkeyFunction(mmi_um_entry_main_message_enter_bt_msg, KEY_EVENT_UP);
}

void mmi_sms_conn_profile_cb(srv_bt_cm_bt_addr *dev_addr, srv_bt_cm_connection_type conn_type,MMI_BOOL result,MMI_BOOL is_connect, MMI_BOOL is_indicate)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/ 
    ...
    if(g_sms_cui_id != GRP_ID_INVALID)
    {
        cui_bt_dialer_connecting_close(g_sms_cui_id);
        g_sms_cui_id = GRP_ID_INVALID;
    }
#if 0
/* under construction !*/
/* under construction !*/
...
void mmi_sms_conn_profile_lsk_hdlr()
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/ 	
    MMI_ID grp_id = APP_UNIFIEDMESSAGE;//mmi_frm_group_get_active_id();
    ret = srv_bt_mapc_get_bt_addr_by_idx(&addr, g_sms_active_dev_index);
#ifdef __MMI_BLE_DOGP_SUPPORT__
    if (!ret && srv_bt_cm_get_linked_dev_list_idx() > 0)
#else
...
#endif
    {
        ...
        if (cui_id != GRP_ID_INVALID)
        {
            ...
            srv_bt_mapc_connect_req_from_cm(addr, mmi_sms_conn_profile_cb, MMI_TRUE); 
        }
    }
    else
    {
...
static void mmi_sms_entry_connect_lsk()
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    gid = APP_UNIFIEDMESSAGE;
    kal_prompt_trace(MOD_MMI, "[SMS], Entry mmi_sms_entry_connect_lsk");
	
    srv_bt_mapc_set_cur_order_index(g_sms_active_dev_index);
    mmi_bt_dialer_show_popup(APP_UNIFIEDMESSAGE);

    mmi_frm_scrn_close(GRP_ID_SMS_BT_CONFIM, SCR_ID_SMS_BT_CONNECT_CONFIM);
}

void mmi_sms_um_conn_confirm_scrn(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    kal_prompt_trace(MOD_MMI, "[SMS], Entry mmi_sms_um_conn_confirm_scrn");

    if (mmi_frm_scrn_enter(
            GRP_ID_SMS_BT_CONFIM,
            SCR_ID_SMS_BT_CONNECT_CONFIM,
            (FuncPtr) NULL,
            (FuncPtr) mmi_sms_um_conn_confirm_scrn,
...
#endif
        dis_style = MMI_SMS_BT_SCRN_RETRY;
	else
        dis_style = MMI_SMS_BT_SCRN_CONNECT;  

    if (dis_style == MMI_SMS_BT_SCRN_RETRY)
    {
        str_id_softkey = STR_ID_SMS_CONNECT_RETRY;
        str_id_message = STR_UM_SYNC_MSG_LIST_FAIL_ID;
    }
    else if (dis_style == MMI_SMS_BT_SCRN_CONNECT)
    {
        str_id_softkey = STR_GLOBAL_CONNECT;
        str_id_message = STR_ID_SMS_BT_NOT_CONNECTED;
    }

    wgui_showcategory6333_screen(
        NULL,
        (UI_string_type) GetString(str_id_softkey),
        (UI_string_type) GetString(STR_GLOBAL_CANCEL),
        (UI_string_type) GetString(str_id_message),
             NULL);

    //SetLeftSoftkeyFunction(mmi_sms_entry_connect_lsk, KEY_EVENT_UP);
    SetRightSoftkeyFunction(mmi_sms_bt_main_msg_entry_inbox_ext, KEY_EVENT_UP);  

    if (dis_style == MMI_SMS_BT_SCRN_CONNECT)
    {
        SetLeftSoftkeyFunction(mmi_sms_entry_connect_lsk, KEY_EVENT_UP);
    }
    else if (dis_style == MMI_SMS_BT_SCRN_RETRY)
    {
        SetLeftSoftkeyFunction(mmi_sms_conn_profile_lsk_hdlr,KEY_EVENT_UP);
    }
...
void mmi_sms_entry_inbox_with_check(void)
{
	...
	#ifdef __MTK_TARGET__
    if (MMI_FALSE == srv_sms_btmapc_check_mapc_status())//srv_bt_mapc_is_available
    {
        kal_prompt_trace(MOD_MMI, "[SMS], Entry mmi_sms_entry_inbox_with_check---mapc not avaiable");
    ...
    if (mmi_sms_um_bt_ui_cc_get_grp_num() > 0)
    {
        /* destroy may be failed */
        if (MMI_RET_OK != mmi_sms_um_bt_ui_cc_destroy_all_grp())
        {
            kal_prompt_trace(MOD_MMI, "[SMS], Entry mmi_sms_entry_inbox_with_check---instances exist");
            mmi_um_current_not_avaiable_popup();
            mmi_frm_group_close(GRP_ID_SMS_BT_CONFIM);
            mmi_frm_app_close(APP_UNIFIEDMESSAGE);
            return;
    ...
    mmi_sms_bt_main_msg_entry_inbox_ext();
    return;
}

void mmi_sms_entry_main_mesage_connect_check(void)
{	
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if (srv_bt_mapc_adp_is_connected(g_sms_active_dev_index) == MMI_TRUE)
{
        kal_prompt_trace(MOD_MMI, "[SMS], Entry mmi_sms_bt_main_msg_entry_box_ext---inbox");
        mmi_sms_entry_inbox_with_check();
        return;
    }
    ...
    kal_prompt_trace(MOD_MMI, "[SMS], Entry mmi_sms_entry_main_mesage_connect_check dis_style = %d", dis_style);
    mmi_sms_um_conn_confirm_scrn();
}
...
void mmi_sms_bt_main_msg_entry_box_ext(srv_um_msg_btmapc_box_enum msg_box)

{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    mmi_frm_group_create_ex(APP_UNIFIEDMESSAGE,
        GRP_ID_SMS_BT_CONFIM,
        mmi_sms_entry_msg_list_scrn_proc,
        NULL,
...
{
	mmi_um_entry_main_message_enter_local_msg();
	return;
...
void mmi_um_entry_inbox(void)
{
	...
	if(g_um_entry_type == MMI_UM_ENTRY_BT)
    {
    	...
    	mmi_sms_bt_main_msg_entry_box_ext(SRV_UM_MSG_BTMAPC_BOX_INBOX);
	}
	else
#endif
	{
