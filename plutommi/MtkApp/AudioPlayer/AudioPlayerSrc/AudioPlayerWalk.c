...
MMI_BOOL btbox_a2dp_is_output_to_bt(void)
{
    ...
    if(!mmi_a2dp_is_output_to_bt())   //mmi_a2dp_is_output_to_bt  srv_a2dp_is_output_to_bt
    {
        return MMI_FALSE;
    }
#endif
...
void mmi_audply_btbox_play()
{
	...
	if (btbox_a2dp_is_output_to_bt())
	{
	    srv_a2dp_set_app_callback(mmi_audply_btbox_bt_open_callback);
		  if (audply_btbox_data.play_state == AUDPLY_WALK_BTBOX_STATE_PLAY)
		  {
			    mmi_audply_btbox_stop_action();
			    //av_bt_close_codec(); 
		...
		/*av_bt_open(
				...
				KAL_TRUE);*/
	    srv_a2dp_open_source((srv_bt_cm_bt_addr *)mmi_a2dp_get_bt_headset(), MMI_TRUE);
	}
	else
