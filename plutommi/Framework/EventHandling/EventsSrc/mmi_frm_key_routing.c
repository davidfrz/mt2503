...
// tracker + gdi, should turn on backlight while press key or touch.
#if defined(__IOT__) && defined(__DISPLAY_LEVEL_GDI_ONLY__)
mmi_ret mmi_pre_touch_routing_handle(mmi_event_struct *event)
{
    srv_backlight_turn_on(SRV_BACKLIGHT_SHORT_TIME);
    return MMI_RET_OK;
}

mmi_ret mmi_pre_key_routing_handle(mmi_event_struct *event)
{
    srv_backlight_turn_on(SRV_BACKLIGHT_SHORT_TIME);
    return MMI_RET_OK;
}
