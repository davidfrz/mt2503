...
void kbd_get_keypad_status_reg(kbd_map_reg_struct *pkeypad_status)
  {
	...
	EINT_UnMask(KBD_VIA_ADC_EINT_NO);
  #else
...
#ifdef DRV_KBD_ADC_KEYPAD
 	{
		...
		IRQMask(IRQ_GPT_CODE); 
		EINT_Mask(KBD_VIA_ADC_EINT_NO);
		pkeypad_status->kbdmap_reg1= kbd_via_adc_current_key_status;
		EINT_UnMask(KBD_VIA_ADC_EINT_NO);
		IRQMask(IRQ_GPT_CODE); 
 	}
...
void kbd_task_main( task_entry_struct * task_entry_ptr )
{
   ...
   kbd_status_Saved.kbd_map_reg.kbdmap_reg = 0xFFFFFFFF;
   kbd_status_Saved.kbd_map_reg.kbdmap_reg1 = 0xFFFFFFFF;
   kbd_status_Saved.kbd_map_reg.kbdmap_reg2 = 0xFFFFFFFF;
   #else
