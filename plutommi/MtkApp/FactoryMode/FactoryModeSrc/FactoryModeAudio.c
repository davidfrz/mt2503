...
extern MMI_ID g_fm_gourp_id;
MMI_ID g_fm_mic_spk_gourp_id;

/*****************************************************************************
 * FUNCTION
 *  mmi_fm_test_mic_timeout_hdlr
 * DESCRIPTION
 *  
 * PARAMETERS
 *  void
 ...
 *  void
 *****************************************************************************/
static void mmi_fm_test_mic_timeout_hdlr(void)
{
	...
	/* Code Body													  */
	/*----------------------------------------------------------------*/
	kal_prompt_trace(MOD_MMI,"mmi_fm_test_mic_timeout_hdlr");

	StopTimer(FM_GENERAL_TIMER);
 ...
 *  void
 *****************************************************************************/
S32 mmi_fm_start_test_mic(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
	//StartTimer(FM_GENERAL_TIMER, 2000, mmi_fm_test_mic_timeout_hdlr);
	
	kal_sleep_task(kal_milli_secs_to_ticks(200));
	L1SP_SetOutputVolume(196,0);	
	//L1SP_SetOutputDevice(L1SP_LOUD_SPEAKER);
	L1SP_SetMicrophoneVolume(99);
	aud_util_proc_in_med(MOD_MMI,
		mmi_fm_set_loopback,
		MMI_TRUE,
		NULL);
...
/*****************************************************************************
  ...
  *  MMI_RET 
  *****************************************************************************/
static MMI_RET mmi_fm_mic_spk_test_proc(mmi_event_struct *param)
{
	...
	{
		kal_prompt_trace(MOD_MMI, "mmi_fm_mic_spk_test_proc EVT_ID_GROUP_DEINIT");
		
		kal_prompt_trace(MOD_MMI, "mmi_fm_select_mic_spk_timeout_hdlr EchoLoopTestOn = %d",g_fm_contxt->eachloop_contxt.EchoLoopTestOn);
		StopTimer(FM_AUTO_TEST_COMMNON_TIMER);
		//StopTimer(FM_GENERAL_TIMER);
		if(g_fm_contxt->eachloop_contxt.EchoLoopTestOn)
		{
			//FM_SendSetAudioModeReq(AUD_MODE_NORMAL);
		    /* open loopback */
		    kal_sleep_task(kal_milli_secs_to_ticks(200));
...
/*****************************************************************************
 * FUNCTION
 *  mmi_fm_select_mic_spk_timeout_hdlr
 * DESCRIPTION
 *  
 * PARAMETERS
 *  void
 ...
 *  void
 *****************************************************************************/
static void mmi_fm_select_mic_spk_timeout_hdlr(void)
{
...
/*****************************************************************************
  * FUNCTION
  *  mmi_fm_mic_spk_test_enter_screen
  * DESCRIPTION
  *  mmi_fm_mic_spk_test_enter_screen
  * PARAMETERS
  *  ess        [IN]
  ...
  *  void 
  *****************************************************************************/
static void mmi_fm_mic_spk_test_enter_screen(mmi_scrn_essential_struct* ess)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
	kal_prompt_trace(MOD_MMI, "mmi_fm_mic_spk_test_enter_screen");
    if (! mmi_frm_scrn_enter(ess->group_id,
                    ess->scrn_id,
                    NULL, 
                    (FuncPtr)mmi_fm_mic_spk_test_enter_screen,
                    MMI_FRM_FULL_SCRN))
    {
        return;
    }
	kal_prompt_trace(MOD_MMI, "mmi_fm_mic_spk_test_enter_screen EchoLoopTestOn = %d",g_fm_contxt->eachloop_contxt.EchoLoopTestOn);

	ShowCategory7Screen(
			STR_ID_FM_MIC_SPEAKER,
			NULL,//IMG_ID_FM_AUTO_TEST_ROOT,
			0,
			0,
			0,
			0,
			(U8*)GetString(STR_ID_FM_TEST_MIC_SPEAKER),
			NULL);
    StartTimer(FM_AUTO_TEST_COMMNON_TIMER, 500, mmi_fm_select_mic_spk_timeout_hdlr);

}

/*****************************************************************************
 * FUNCTION
 *  mmi_fm_select_mic_spk
 * DESCRIPTION
 *  
 * PARAMETERS
 *  void
 ...
 *  void
 *****************************************************************************/
extern void mmi_fm_select_mic_spk(cui_menu_event_struct *evt)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
	g_fm_mic_spk_gourp_id = mmi_frm_group_create_ex(g_fm_gourp_id,
										   GRP_ID_AUTO_GEN,
										   mmi_fm_mic_spk_test_proc,
										   NULL,
										   MMI_FRM_NODE_SMART_CLOSE_FLAG);

    mmi_frm_scrn_first_enter(g_fm_mic_spk_gourp_id, 
					   SCR_ID_FM_EACHLOOP, 
					   (FuncPtr)mmi_fm_mic_spk_test_enter_screen,
					   NULL); 
