...
#if defined(__TINY_SYS__)
extern kal_bool is_Tiny_MemDump;
#endif
...
//			break;
		...
		case USB_CMD_USB2UART_TX_DMA_CBK:
			...
			break;
	case USB_CMD_DTR_STATUS:
			{
				USB_DRV_CTRL_COM_RESPONSE* presonse;
				presonse =  &(data->rComport_Ctrl_Fun5);
				if((g_UsbACM[USB_PORT[dev]].line_state & CONTROL_LINE_SIGNAL_DTR) == CONTROL_LINE_SIGNAL_DTR)
				{
					presonse->u2Retresult = KAL_TRUE;
				}
				else
				{
					presonse->u2Retresult = KAL_FALSE;
				}
			}
			break;
	case USB_CMD_TX_DATA_IS_EMPTY:
			{
				USB_DRV_CTRL_COM_RESPONSE* presonse;
				presonse =  &(data->rComport_Ctrl_Fun5);
				Buf_IsEmpty(&(USB2UARTPort[USB_PORT[dev]].Tx_Buffer), presonse->u2Retresult);  //presonse->u2Retresult == 1, means Empty
			}
			break;
#ifdef  __USB_HIGH_SPEED_COM_PORT_ENABLE__
...
#if defined(__TINY_SYS__)
	if((ownerid == MOD_TST_READER)&&(INT_BootMode() != MTK_FACTORY_MODE))
	{
		if((USB2UARTPort[usb_port].initialized == KAL_TRUE)&&((g_UsbACM[usb_port].line_state & CONTROL_LINE_SIGNAL_DTR) != CONTROL_LINE_SIGNAL_DTR))
		{
			g_UsbACM[usb_port].config_send_Txilm = KAL_TRUE;  /* for PC set config later then can issue the first message */		
			return 0;
		}
...
#if defined(__TINY_SYS__)
			AP_BtSS_mode_block(SUSPEND, AP_BTSS_MODE_BLOCKER_USB);
#endif
