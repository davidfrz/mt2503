...
static S32 btmtk_map_vcard_create_object_start(S32 handle, const char *object_name)
{
  ...
  //  BTMTK_ASSERT(count <= MAP_ADP_BUFFER_SIZE);
	...
	kal_prompt_trace(MOD_MMI, "[MAPC]btmtk_map_vcard_create_object_start count = %d", count);
    count = FS_Write(handle, buf, count, 0);	
	kal_prompt_trace(MOD_MMI, "[MAPC]btmtk_map_vcard_create_object_start FS_Write:ret = %d", count);
    return count;
}
...
static S32 btmtk_map_vcard_create_object_properties(S32 handle, const char **prop_list)
{
 ...
 //   BTMTK_ASSERT(count <= MAP_ADP_BUFFER_SIZE);
	...
	kal_prompt_trace(MOD_MMI, "[MAPC]btmtk_map_vcard_create_object_properties count = %d", count);
    count = FS_Write(handle, buf, count, 0);
	kal_prompt_trace(MOD_MMI, "[MAPC]btmtk_map_vcard_create_object_properties FS_Write:ret = %d", count);
    return count;
}

static S32 btmtk_map_vcard_create_object_end(S32 handle, const char *object_name)
{
    S32 count = 0;
    char *buf = g_pi_map_cntx.builder_buffer;
  ...
  //  BTMTK_ASSERT(count <= MAP_ADP_BUFFER_SIZE);
	...
	kal_prompt_trace(MOD_MMI, "[MAPC]btmtk_map_vcard_create_object_end count = %d", count);
    count = FS_Write(handle, buf, count,0);
	kal_prompt_trace(MOD_MMI, "[MAPC]btmtk_map_vcard_create_object_end FS_Write:ret = %d", count);
    return count;
}
													  ...
													  )
{
    ...
    if ((fhandle = FS_Open((WCHAR*)filename, FS_READ_WRITE | FS_CREATE)) < 0)
    {
		...
		kal_prompt_trace(MOD_MMI, "[MAPC]btmtk_map_vcard_create_message_by_buffer FS_Open :ret = %d", fhandle);
        return MMI_FALSE;
    }
...
//      prop_list[i++] = msg->FolderPath;
        ...
        {
            ret = FS_Write(fhandle, msg->Originator, msg->OriginatorSize, (U32*)&bytes_written);		
			kal_prompt_trace(MOD_MMI, "[MAPC]create_message_by_buffer FS_Write0:ret=%d,len=%d,written_len=%d", ret, msg->OriginatorSize, bytes_written);
            ret = FS_Write(fhandle, VCARD_LINE_BREAK, 2, (U32*)&bytes_written);
			kal_prompt_trace(MOD_MMI, "[MAPC]create_message_by_buffer FS_Write1:ret=%d,len=2,written_len=%d", ret, bytes_written);
        }
        ...
        for (index = 0; index < MBT_MAP_MAX_MSG_ENVELOPE_NUM;index ++)
        {
            ...
            {
            	ret = FS_Write(fhandle, msg->Envelope[index].Recipient, msg->Envelope[index].RecipientSize, (U32*)&bytes_written);
				kal_prompt_trace(MOD_MMI, "[MAPC]create_message_by_buffer FS_Write2:ret=%d,len=%d,written_len=%d", ret, msg->Envelope[index].RecipientSize, bytes_written);
                ret = FS_Write(fhandle, VCARD_LINE_BREAK, 2, (U32*)&bytes_written);
				kal_prompt_trace(MOD_MMI, "[MAPC]create_message_by_buffer FS_Write3:ret=%d,len=2,written_len=%d", ret, bytes_written);
            }
            else
            {
	...
	{   
	    ...
	    if (rhandle >= 0)
	    {
	        ...
	        while (remain > 0)
	        {
				...
				read = bytes_read;
	            remain -= read;
				ret = FS_Write(fhandle, g_pi_map_cntx.file_buffer, read, (U32*)&bytes_written);
	            if(ret < 0)
	            {	
	            	kal_prompt_trace(MOD_MMI, "[MAPC]create_message_by_buffer FS_Write4:ret=%d,len=%d,written_len=%d", ret, read, bytes_written);
	                FS_Close(fhandle);
	                return MMI_FALSE;
	            }
	    ...
	    {
	    	ret = FS_Write(fhandle, (msg->Content + msglength), msg->FragmentSize[msgcount], (U32*)&bytes_written);
    		if (ret < 0)
    		{
    			kal_prompt_trace(MOD_MMI, "[MAPC]create_message_by_buffer FS_Write5:ret=%d,len=%d,written_len=%d", ret, msg->FragmentSize[msgcount], bytes_written);
				return MMI_FALSE;
		    }
		    else
		    {
	    ...
	    ret = FS_Write(fhandle, VCARD_LINE_BREAK, 2, (U32*)&bytes_written);
		kal_prompt_trace(MOD_MMI, "[MAPC]create_message_by_buffer FS_Write6:ret=%d,len=2,written_len=%d", ret, bytes_written);
                                                    ...
                                                    kal_wchar *file_name)
{
    ...
    if ((fhandle = FS_Open((WCHAR*)file_name, FS_READ_ONLY)) < 0)
    {
		...
		kal_prompt_trace(MOD_MMI, "[MAPC]btmtk_map_vcard_create_message_by_file FS_Open :ret = %d", fhandle);
        return MMI_FALSE;
    }
...
MMI_BOOL btmtk_map_vcard_extract_content_body_by_single_line(S32 fhandle,MBT_MAP_MSG_OBJECT *msg, kal_wchar *to_file)
{
	...
	while ((count = btmtk_map_vcard_get_single_line(g_pi_map_cntx.file_buffer, fhandle)) >= 0 && msg->FragmentNum < MAP_ADP_MAX_FRAGMENT_SIZE_NUM)
    {            
