...
#define INIT_PHASE   0
#define READ_PHASE   1
#define VERIFY_PHASE 2
#define UPDATE_PHASE 3
...
static kal_bool   ubinUpdate = KAL_FALSE;             //IF update bin have UBIN, this flag wil be raised
...
typedef struct{
    ...
    kal_uint32 raw_img_size;
    kal_uint32 split_number;
    kal_uint32 reserved;
    kal_uint32 split_img_size[20];
    
}COMPRESSED_BIN_HEADER;



kal_bool bl_checkIsCompressed(COMPRESSED_BIN_HEADER  compressedHeader)
{
    kal_uint32 magic_num = compressedHeader.magic_number;

    /*Beginning 12 byte of compressed binary can use as magic number*/
    if( magic_num == COMPRESSED_BIN_MAGIC)
        return KAL_TRUE;
    else
        return KAL_FALSE;
}

BL_FUNET_ERROR_CODE bl_decompressed_bin(COMPRESSED_BIN_HEADER compressedHeader)
{
    typedef void (*ZImageNotifyFunction)(kal_uint32, kal_uint32);
    extern kal_bool ZImageDecompressByPartition(kal_uint32 source, kal_uint32 destination,kal_uint32 notifyLength, kal_uint32 notifyTotalLength,kal_uint32 *notifyCurrentLength, ZImageNotifyFunction notifyFunction);
    ...
    int i = 0;
    kal_bool ret = 0;
    kal_uint32 src_bin_size = 0;
    kal_uint32 dest_bin_size = 0;
    BL_FUNET_ERROR_CODE status = BL_FUNET_ERROR_NONE;
    /*Get file size for*/
    SetZImageWorkingMemory(work_buf,DECOMPRESS_WORK_BUF_LEN);
    
    /*bypass header part*/
    bl_fs_Seek(sizeof(COMPRESSED_BIN_HEADER),0);

    BL_PRINT(LOG_INFO, "[Decompression]\n\r");

    for( i = 0; i < compressedHeader.split_number ; i++)
    {
         BL_PRINT(LOG_INFO, "%d..",i);
         src_bin_size = compressedHeader.split_img_size[i];
         if( bl_fs_Read((void *)source, src_bin_size) !=  src_bin_size)
         {
              return BL_FUNET_ERROR_DECOMPRESSED_READ_FAIL;
         }

         dest_bin_size = ((SPLIT_BIN_HEADER *)source)->destinationSize;
         ret = ZImageDecompressByPartition(source, dest,0,0,NULL,NULL);
         /*Shift dest and source pointer*/
         dest+= dest_bin_size;
         bl_UpdateProgress(READ_PHASE, (i+1)*100/compressedHeader.split_number);

         if(ret == KAL_FALSE)
         {
            status = BL_FUNET_ERROR_DECOMPRESSION;
            break;
         }       
    }
...
BL_FUNET_ERROR_CODE bl_readFiletoRam()
{
    ...
    COMPRESSED_BIN_HEADER  compressedHeader ;
    ...
    /*Read From fs to RAM*/      
    if( bl_fs_Read(image_buf, source_length) !=  source_length)
    {
      return BL_FUNET_ERROR_COPY_TO_RAM_FAIL;
    }

    bl_fs_Seek(0,0);
    memcpy(&compressedHeader,image_buf,sizeof(COMPRESSED_BIN_HEADER));
    

   

   /*Decompression*/
    isCompressed = bl_checkIsCompressed(compressedHeader);
    ...
    if(isCompressed)
    {
        ...
        if(compressedHeader.raw_img_size > dest_length)
        {
            return BL_FUNET_ERROR_DECOMPRESSED_BINARY_TOO_LARGE; 
        }
        status =  bl_decompressed_bin(compressedHeader);
...
// Raymond : LCD related code, mark it now and you can remove it if it is really unnecessary
   ...
   const kal_uint32 phasePortion[] = {5, 15 ,30, 40, 10};
...
BL_FUNET_ERROR_CODE bl_DoUpdateMAUI(kal_uint32 addr, kal_uint32 length, IM_OPERATION op, kal_int32 bin_offset, kal_uint32* pInfo)
{
   ...
   for(; flashBlockIdx<=blockEnd; flashBlockIdx++)
   {
      ...
      bl_UpdateProgress(UPDATE_PHASE, bin_offset*100/im_file_size);
 ...
 */
BL_FUNET_ERROR_CODE bl_UnMarkFUNET(BL_FUNET_ERROR_CODE updateStatus)
{
    BL_FUNET_ERROR_CODE status = BL_FUNET_ERROR_NONE;   
    FUNET_Update_Info ui;
    kal_uint32 i;
...
BL_FUNET_ERROR_CODE bl_FinishUpdate(BL_FUNET_ERROR_CODE updateStatus)
{
   BL_FUNET_ERROR_CODE status = BL_FUNET_ERROR_NONE;
   ...
   status = bl_UnMarkFUNET(updateStatus);
...
BL_FUNET_ERROR_CODE bl_ExtraInfoCheck()
{
   ...
   for(i=0; i<sizeof(pPkgInfo->m_extra_info)/sizeof(pPkgInfo->m_extra_info[0]); i++)
   {
      ...
      //check ubin exista and needed update
      if(pPkgInfo->m_extra_info[i].m_bin_type == IOT_UBIN_BIN && pPkgInfo->m_extra_info[i].m_operation == IM_DOWNLOAD)
      {
            ubinUpdate = KAL_TRUE;
      }
      ...
      if((pPkgInfo->m_extra_info[i].m_bin_type >= V_RESOURCE_BINARY && pPkgInfo->m_extra_info[i].m_bin_type < V_RESOURCE_BINARY_END) ||
         ...
         (pPkgInfo->m_extra_info[i].m_bin_type == IOT_UBIN_BIN))
      {
      ...
      //For other images (except filesystem, UBIN), they must be all updated, or all not updated
         ...
         (pPkgInfo->m_extra_info[i].m_bin_type != IOT_UBIN_BIN))
      {
         ...
         //If codePartialUpdate is true, only UBIN, resource and 3rd rom can be marked as IM_DOWNLOAD
      ...
      if(IS_OVERLAP(0, BOOTLOADER_ROM_REGION_LEN, pPkgInfo->m_extra_info[i].m_bin_start_addr, pPkgInfo->m_extra_info[i].m_bin_length) &&
         ...
         && (pPkgInfo->m_extra_info[i].m_bin_type != IOT_UBIN_BIN))
      {
         BL_PRINT(LOG_INFO, "%x %x %x\n\r",BOOTLOADER_ROM_REGION_LEN, custom_ROM_baseaddr(), custom_RAM_baseaddr());
         return BL_FUNET_ERROR_BOOTLOADER_CANNOT_BE_UPDATED;
      }
...
/*This check perform  when only update UBIN*/
BL_FUNET_ERROR_CODE bl_UBINPartialUpdateCheck()
{
    int i = 0;
    kal_uint32 ubinOffset = 0;
    kal_uint32 targetUbinAddr = 0;
    kal_uint32 binUbinSize = 0;

    UBIN_GFH bin_ubin_gfh;
    UBIN_GFH target_ubin_gfh;


    /*Get ubin addr from header */

   if(pDl_Package_GFH->gfh_dl_package_info.m_extra_info[i].m_bin_type == IOT_UBIN_BIN)
   {
      //Bootloader NOR address is start from 0x1xxxxxxx, we need to remap address from header
      targetUbinAddr = pDl_Package_GFH->gfh_dl_package_info.m_extra_info[i].m_bin_start_addr |ROM_ADDR_MASK;
      binUbinSize = pDl_Package_GFH->gfh_dl_package_info.m_extra_info[i].m_bin_length ;
   }

    ubinOffset = pDl_IOT_HEADER->m_extra_info[bl_GetImgIdx(IOT_UBIN_BIN)].m_bin_offset;


    /*Check ubin on target match with header. If match, we can find GFH_FILE_INFO at this address*/
    /*Only check this for 1st time*/
    if(!last_funet_fail_flag)
    {
        if(GFH_FILE_INFO_IsValid((GFH_FILE_INFO_v1*)targetUbinAddr) != 0 || ((GFH_FILE_INFO_v1*)targetUbinAddr)->m_file_type != IOT_UBIN_BIN )
        {
           return BL_FUNET_ERROR_UBIN_ADDR_NOT_MATCH_WITH_TARGET   ;
        }
    }

    
    bl_ReadXIMByte((kal_uint32)&bin_ubin_gfh, ubinOffset, sizeof(UBIN_GFH), KAL_FALSE);
    memcpy((void *)&target_ubin_gfh,(void *)targetUbinAddr,sizeof(UBIN_GFH));

    /*Check binary size smaller than maxSize*/

    if(bin_ubin_gfh.gfh_file_info.m_file_len >=target_ubin_gfh.gfh_ubin_info.m_ubin_max_size )
    {
        return BL_FUNET_ERROR_UBIN_SIZE_OVER_MAXIMUM;
    }


    /*Check binary build time match with target*/
   if(strncmp( (kal_char*)bin_ubin_gfh.gfh_ubin_info.m_mbin_build_time, 
               (kal_char*)target_ubin_gfh.gfh_ubin_info.m_mbin_build_time,
               sizeof(bin_ubin_gfh.gfh_ubin_info.m_mbin_build_time)
     ))
    {
      dbg_print("Mismatched build time\n\r");
      return BL_FUNET_ERROR_UBIN_BUILD_TIME_NOT_MATCH ;
    }

   if(strncmp((kal_char*)bin_ubin_gfh.gfh_ubin_info.m_mbin_project_id, 
              (kal_char*)target_ubin_gfh.gfh_ubin_info.m_mbin_project_id, 
               sizeof(bin_ubin_gfh.gfh_ubin_info.m_mbin_project_id)
     ))
   {
      dbg_print("Mismatched Project ID\n\r");
      return BL_FUNET_ERROR_UBIN_PROJECT_ID_NOT_MATCH ;
   }
...
BL_FUNET_ERROR_CODE bl_VerifyPKGBody()
{
   ...
   if(!last_funet_fail_flag)
   {
       ...
       if( status == BL_FUNET_ERROR_NONE && codePartialUpdate && mbaUpdate)
       {
         status = bl_MBAPartialUpdateCheck();
       }
     
       if( status == BL_FUNET_ERROR_NONE && codePartialUpdate && ubinUpdate)
       {
         status = bl_UBINPartialUpdateCheck();
       }
   }
...
/* under construction !*/
   ...
   if(status == BL_FUNET_ERROR_NONE)
   {
        bl_UpdateProgress(READ_PHASE, 0);
   ...
   */
   ...
   if(status == BL_FUNET_ERROR_NONE)
   {
      ...
      gpt4_busy_wait_us(2000000); // wait 2 s to show progress bar
