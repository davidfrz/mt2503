...
static void btaud_attach_done(MMI_BOOL result)
{
    ...
    if (result)
    {
        ...
        mmi_bt_dialer_set_headset_flag(MMI_TRUE, &info->addr);
        srv_bt_cm_set_dev_link_state(&info->addr,MMI_TRUE);
        if (srv_bt_cm_get_bqb_test_flag())
        {
            srv_bthf_save_config(BTHF_CFG_DEF_DEVICE, &info->addr);
        }
    }
    else
    {
        dev_p = (srv_bt_cm_dev_struct*)srv_bt_cm_get_dev_info(&btaud_cntx->pend_dev);
