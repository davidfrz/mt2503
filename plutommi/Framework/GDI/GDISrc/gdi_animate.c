						...
						U32 frame_pos, S32 anim_count)
{
	...
	GDI_ENTER_CRITICAL_SECTION(gdi_anim_codec_draw_internal)
		...
		ret = gdi_image_parse_info(src_type,imgBuffer,&data_ptr,&img_type,&img_size,&frame_number,&width,&height);
			...
			if(ret==GDI_SUCCEED)
			{
				...
				if(GDI_ANIM.anim_list[i].is_used == GDI_ANIM_FLAG_IS_FREE)
				{
					...
					if(GDI_ANIM.anim_list[i].src_type == GDI_IMAGE_SRC_FROM_FILE)
					{
						...
						mmi_ucs2ncpy((PS8)GDI_ANIM.anim_list[i].image_ptr, (PS8)imgBuffer, SRV_FMGR_PATH_MAX_LEN);
					}
					else
						GDI_ANIM.anim_list[i].image_ptr		= imgBuffer;
