...
#if defined(__UPDATE_BINARY_FILE__) && defined(__MTK_TARGET__)
 ...
 *************************************************************************/
#if defined(__UPDATE_BINARY_FILE__) && defined(__MTK_TARGET__)
typedef struct task_entry_map_t
{
    task_indx_type task_index;
    ubin_symbol_entry_enum entry_id;
}task_entry_map;
...
#undef task_create_function_table
#undef task_entry_map
...
#define task_create_function_table(p1)
...
#define task_entry_map(task, entry) {task, entry},
const task_entry_map task_entry_map_g[]=
{
    {INDX_NIL, 0},
...
#if defined(__UPDATE_BINARY_FILE__) && defined(__MTK_TARGET__)
       else if (sys_comp_config_tbl[task_indx].comp_create_func == (kal_create_func_ptr)TASK_GET_ROM_ENTRY )
       {
            kal_create_func_ptr  comp_create_function = NULL;
            kal_uint32 i = 1;
    
            for(; i < (sizeof(task_entry_map_g)/sizeof(task_entry_map)); i++)
            {
                if(task_indx == task_entry_map_g[i].task_index)
                {
                    comp_create_function = 
                        (kal_create_func_ptr)(*((int*)(custom_get_UPDATE_ROM_entry()) + task_entry_map_g[i].entry_id));
                    break;
                }			
            }

            if ((comp_create_function == NULL) || (comp_create_function(&comp_handler) != KAL_TRUE))
            {
                continue;
            }
       }
