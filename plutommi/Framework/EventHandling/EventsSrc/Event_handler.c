...
#define GetByteIndex(number)	((number)>>3)
#define GetBitIndex(number)		((number)&7)
#define GetFlag(fg, n)			((fg) & (1 << (7-n)))
#define SetFlagSingle(fg, n)	((fg) & (~(1 << (7-n))))
#define SetFlagMulti(fg, n)		((fg) | (1 << (7-n)))
#define FLAG_ARRAY_LENGTH		((MAX_PROTOCOL_EVENT+7)>>3)
...
mmi_frm_event_runtime_info_struct g_event_table_runtime_info;



PsFuncPtr protocol_event_handler_funcptr[MAX_PROTOCOL_EVENT];
U16	protocol_event_handler_event[MAX_PROTOCOL_EVENT];
U8	protocol_event_handler_flagmulti[FLAG_ARRAY_LENGTH];
...
void mmi_frm_execute_current_protocol_handler(U16 eventID, void *MsgStruct, int mod_src, void *Message)
{
    ...
    MMI_TRACE(MMI_FW_TRC_G6_FRM_DETAIL, MMI_EVENT_TRC_EVENT_PROFILING_MAX, g_event_table_runtime_info.max_events);
#endif /* defined (MMI_EVENT_PROFILING) */ 
}
 ...
 * DESCRIPTION
 *  This function is used for register the single protocol event handler
 * PARAMETERS
 *  eventID         [IN] Unique Protocol/System EventID.       
 *  funcPtr         [IN] Function to be executed whenever a event is received from the protocol or system
 * RETURNS
 *  void
 *****************************************************************************/
void mmi_frm_set_single_protocol_event_handler(U16 eventID, PsIntFuncPtr funcPtr)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    mmi_frm_set_protocol_event_handler_int(EVENT_TABLE_MASTER, eventID, (PsFuncPtr)funcPtr, MMI_FALSE);
}
/*****************************************************************************
 * FUNCTION
 *  mmi_frm_set_multi_protocol_event_handler
 * DESCRIPTION
 *  This function is used for register the multi protocol event handler
 * PARAMETERS
 *  eventID         [IN] Unique Protocol/System EventID.       
 *  funcPtr         [IN] Function to be executed whenever a event is received from the protocol or system
 * RETURNS
 *  void
 *****************************************************************************/
void mmi_frm_set_multi_protocol_event_handler(U16 eventID, PsIntFuncPtr funcPtr)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    mmi_frm_set_protocol_event_handler_int(EVENT_TABLE_MASTER, eventID, (PsFuncPtr)funcPtr, MMI_TRUE);
}
/*****************************************************************************
 * FUNCTION
            ...
            MMI_BOOL *pIsMultiInTable)
{
    ...
    /*----------------------------------------------------------------*/
    S16 idx = -1;
    S16 maxEvents = g_event_table_runtime_info.max_events;

	U16 *eventTable = protocol_event_handler_event;
	PsFuncPtr *funcTable = protocol_event_handler_funcptr;
	U8 *flagTable = protocol_event_handler_flagmulti;

	U16 byte_index_current;
	U8 bit_index_current;
	MMI_BOOL flagCurrent;
    ...
    {
        /* eventID match */
        if (eventTable[idx] == eventID)
        {
            /* if handler type is diff., return type in table */
			byte_index_current = GetByteIndex(idx);
			bit_index_current = GetBitIndex(idx);
			flagCurrent = GetFlag(flagTable[byte_index_current], bit_index_current) ? MMI_TRUE : MMI_FALSE;
			//flagCurrent = (MMI_BOOL)GetFlag(flagTable[byte_index_current], bit_index_current);
            if (pIsMultiInTable)
                *pIsMultiInTable = flagCurrent;
            ...
            if (pFuncPtr)
    		{
                ...
                if (*pFuncPtr == NULL)
                {
                    *pFuncPtr = funcTable[idx];
                }
                /* has handler to search */
                else
                {
                    /* handler not found */
                    if (*pFuncPtr != funcTable[idx])
        			{
...
/* under construction !*/
        ...
        if (pFirstEmptyIdx && (*pFirstEmptyIdx == -1) && (eventTable[idx] == MMI_EVENT_INVALID))
        {
            *pFirstEmptyIdx = idx;
        }
...
static void mmi_frm_set_protocol_event_handler_int(mmi_frm_event_table_enum tableType, U16 eventID, PsFuncPtr funcPtr, MMI_BOOL isMultiHandler)
{
    ...
    MMI_TRACE(MMI_FW_TRC_G6_FRM_DETAIL, TRC_MMI_FRM_EVENT_SETPTO_HDLR_DETAIL,
              tableType, g_event_table_runtime_info.used_events, g_event_table_runtime_info.max_events);
    ...
    if (idx >= 0)
    {
        ...
        if (!isMultiHandler)
        {
            ...
            {
                /* just need to update */
                funcTable[idx] = funcPtr;
            }
            else
            {
                /* funcPtr=NULL, means clear => reset it */
				protocol_event_handler_event[idx] = MMI_EVENT_INVALID;
                g_event_table_runtime_info.used_events--;
            }
        }
    ...
    else
    {
        if (funcPtr != NULL)
        {
            ...
            else
            {
                ...
                /* need to enlarge current searching list */
                idx = g_event_table_runtime_info.max_events;
                g_event_table_runtime_info.max_events++;
                MMI_ASSERT(g_event_table_runtime_info.max_events < MAX_PROTOCOL_EVENT);

            }
            g_event_table_runtime_info.used_events++;
            eventTable[idx] = eventID;
            funcTable[idx] = funcPtr;
			if(isMultiHandler)
			{
				flagTable[GetByteIndex(idx)] = SetFlagMulti(flagTable[GetByteIndex(idx)], GetBitIndex(idx));
			}
			else
			{
				flagTable[GetByteIndex(idx)] = SetFlagSingle(flagTable[GetByteIndex(idx)], GetBitIndex(idx));
			}
    ...
    for (idx = (g_event_table_runtime_info.max_events - 1); idx >= 0; idx--)
    {
        if (eventTable[idx] != MMI_EVENT_INVALID)
        {
            break;
        }
    }
    g_event_table_runtime_info.max_events= idx + 1;

    /* record the max value for array size analysis */
    if (maxUsedInHistory[tableType] < g_event_table_runtime_info.max_events)
    {
        MMI_TRACE(MMI_FW_TRC_G6_FRM_DETAIL, MMI_EVENT_TRC_SET_PROTOCOL_EVENT_HDLR_MAX_1, maxUsedInHistory[tableType]);
        maxUsedInHistory[tableType] = g_event_table_runtime_info.max_events;
...
static void mmi_frm_clear_protocol_event_handler_int(mmi_frm_event_table_enum tableType, U16 eventID, PsFuncPtr funcPtr)
{
    ...
    if (idx >= 0)  /* found */
    {
        ...
        eventTable[idx] = MMI_EVENT_INVALID;
...
//#define __MMI_PROTOCOL_EVENT_HANDLER_UT__
#ifdef __MMI_PROTOCOL_EVENT_HANDLER_UT__
...
U8 mmi_frm_protocol_event_handler_ut_func_one(void* para)
{
	kal_prompt_trace(MOD_MMI, "[protocol_event_handler] test func one");
	return MMI_RET_OK;
}

U8 mmi_frm_protocol_event_handler_ut_func_two(void* para)
{
	kal_prompt_trace(MOD_MMI, "[protocol_event_handler] test func two");
	return MMI_RET_STOP_PROTOCOL;
}

U8 mmi_frm_protocol_event_handler_ut_func_three(void* para)
{
	kal_prompt_trace(MOD_MMI, "[protocol_event_handler] test func three");
	return MMI_RET_OK;
}

U8 mmi_frm_protocol_event_handler_ut_func_four(void* para)
{
	kal_prompt_trace(MOD_MMI, "[protocol_event_handler] test func four");
	return MMI_RET_OK;
}

U8 mmi_frm_protocol_event_handler_ut_func_five(void* para)
{
	kal_prompt_trace(MOD_MMI, "[protocol_event_handler] test func five");
	return MMI_RET_OK;
}

mmi_ret ut_mmi_frm_protocol_event_single_handler(void *para)
{
	S16	idx;
	PsFuncPtr tempfunc;
	MMI_BOOL isMultiHandler = MMI_FALSE;
	mmi_frm_set_protocol_event_handler(MSG_ID_MMI_EQ_PROTOCOL_UT_ONE, (PsIntFuncPtr)mmi_frm_protocol_event_handler_ut_func_one, MMI_FALSE);
	tempfunc = NULL;
	idx = mmi_frm_search_event(
			EVENT_TABLE_MASTER,
			MSG_ID_MMI_EQ_PROTOCOL_UT_ONE,
			&tempfunc,
			0,
			MMI_FALSE,
			NULL,
			NULL,
			&isMultiHandler);
	MMI_ASSERT(tempfunc == mmi_frm_protocol_event_handler_ut_func_one);
	MMI_ASSERT(isMultiHandler == MMI_FALSE);
	mmi_frm_set_protocol_event_handler(MSG_ID_MMI_EQ_PROTOCOL_UT_ONE, (PsIntFuncPtr)mmi_frm_protocol_event_handler_ut_func_two, MMI_FALSE);
	tempfunc = NULL;
	idx = mmi_frm_search_event(
			EVENT_TABLE_MASTER,
			MSG_ID_MMI_EQ_PROTOCOL_UT_ONE,
			...
			NULL,
			&isMultiHandler);
	MMI_ASSERT(tempfunc == mmi_frm_protocol_event_handler_ut_func_two);
	MMI_ASSERT(isMultiHandler == MMI_FALSE);
	mmi_frm_set_protocol_event_handler(MSG_ID_MMI_EQ_PROTOCOL_UT_ONE, NULL, MMI_FALSE);
	tempfunc = NULL;
	idx = mmi_frm_search_event(
			EVENT_TABLE_MASTER,
			MSG_ID_MMI_EQ_PROTOCOL_UT_ONE,
			...
			NULL,
			&isMultiHandler);
	MMI_ASSERT(tempfunc == NULL);
	MMI_ASSERT(isMultiHandler == MMI_FALSE);
	MMI_ASSERT(idx == -1);
}

mmi_ret ut_mmi_frm_protocol_event_multi_handler(void *para)
{
	S16 idx;
	PsFuncPtr tempfunc;
	MMI_BOOL isMultiHandler = MMI_FALSE;
	U16 nextSearchIndex = 0;
	mmi_frm_set_protocol_event_handler(MSG_ID_MMI_EQ_PROTOCOL_UT_ONE, (PsIntFuncPtr)mmi_frm_protocol_event_handler_ut_func_one, MMI_TRUE);
	mmi_frm_set_protocol_event_handler(MSG_ID_MMI_EQ_PROTOCOL_UT_ONE, (PsIntFuncPtr)mmi_frm_protocol_event_handler_ut_func_two, MMI_TRUE);
	mmi_frm_set_protocol_event_handler(MSG_ID_MMI_EQ_PROTOCOL_UT_ONE, (PsIntFuncPtr)mmi_frm_protocol_event_handler_ut_func_three, MMI_TRUE);
	tempfunc = NULL;
	idx = mmi_frm_search_event(
			EVENT_TABLE_MASTER,
			MSG_ID_MMI_EQ_PROTOCOL_UT_ONE,
			...
			&isMultiHandler);
	MMI_ASSERT(tempfunc == mmi_frm_protocol_event_handler_ut_func_one);
	MMI_ASSERT(nextSearchIndex - idx == 1);
	MMI_ASSERT(isMultiHandler == MMI_TRUE);
	tempfunc = NULL;
	idx = mmi_frm_search_event(
			EVENT_TABLE_MASTER,
			MSG_ID_MMI_EQ_PROTOCOL_UT_ONE,
	...
	idx = mmi_frm_search_event(
			...
			NULL,
			&isMultiHandler);
	MMI_ASSERT(tempfunc == mmi_frm_protocol_event_handler_ut_func_three);
	MMI_ASSERT(nextSearchIndex - idx == 1);
	MMI_ASSERT(isMultiHandler == MMI_TRUE);
	mmi_frm_clear_protocol_event_handler(MSG_ID_MMI_EQ_PROTOCOL_UT_ONE, mmi_frm_protocol_event_handler_ut_func_one);
	mmi_frm_clear_protocol_event_handler(MSG_ID_MMI_EQ_PROTOCOL_UT_ONE, mmi_frm_protocol_event_handler_ut_func_two);
	mmi_frm_clear_protocol_event_handler(MSG_ID_MMI_EQ_PROTOCOL_UT_ONE, mmi_frm_protocol_event_handler_ut_func_three);
	tempfunc = mmi_frm_protocol_event_handler_ut_func_one;
	idx = mmi_frm_search_event(
			EVENT_TABLE_MASTER,
			MSG_ID_MMI_EQ_PROTOCOL_UT_ONE,
			&tempfunc,
			...
			NULL);
	MMI_ASSERT(idx == -1);
	tempfunc = mmi_frm_protocol_event_handler_ut_func_two;
	idx = mmi_frm_search_event(
			EVENT_TABLE_MASTER,
			MSG_ID_MMI_EQ_PROTOCOL_UT_ONE,
			&tempfunc,
			...
			NULL);
	MMI_ASSERT(idx == -1);
	tempfunc = mmi_frm_protocol_event_handler_ut_func_three;
	idx = mmi_frm_search_event(
			EVENT_TABLE_MASTER,
			MSG_ID_MMI_EQ_PROTOCOL_UT_ONE,
			&tempfunc,
...
mmi_ret ut_mmi_frm_protocol_event_handler_exec(void *para)
{
	int mod_src = MOD_MMI;
	MYQUEUE Message;
	mmi_frm_set_protocol_event_handler(MSG_ID_MMI_EQ_PROTOCOL_UT_ONE, (PsIntFuncPtr)mmi_frm_protocol_event_handler_ut_func_one, MMI_TRUE);
	mmi_frm_set_protocol_event_handler(MSG_ID_MMI_EQ_PROTOCOL_UT_ONE, (PsIntFuncPtr)mmi_frm_protocol_event_handler_ut_func_two, MMI_TRUE);
	mmi_frm_set_protocol_event_handler(MSG_ID_MMI_EQ_PROTOCOL_UT_ONE, (PsIntFuncPtr)mmi_frm_protocol_event_handler_ut_func_three, MMI_TRUE);
	mmi_frm_execute_current_protocol_handler(MSG_ID_MMI_EQ_PROTOCOL_UT_ONE, &Message.oslDataPtr, mod_src, &Message);
...
mmi_ret ut_mmi_frm_protocol_event_handler_statistics(void *para)
{
	S16 idx = -1;
	S16 firstEmpty = 0;
	PsFuncPtr tempfunc;
	mmi_frm_clear_all_protocol_event_handler();
	MMI_ASSERT(g_event_table_runtime_info.max_events == 1);
	MMI_ASSERT(g_event_table_runtime_info.used_events == 1);
	mmi_frm_set_protocol_event_handler(MSG_ID_MMI_EQ_PROTOCOL_UT_ONE, (PsIntFuncPtr)mmi_frm_protocol_event_handler_ut_func_one, MMI_FALSE);
	mmi_frm_set_protocol_event_handler(MSG_ID_MMI_EQ_PROTOCOL_UT_TWO, (PsIntFuncPtr)mmi_frm_protocol_event_handler_ut_func_two, MMI_TRUE);
	mmi_frm_set_protocol_event_handler(MSG_ID_MMI_EQ_PROTOCOL_UT_TWO, (PsIntFuncPtr)mmi_frm_protocol_event_handler_ut_func_three, MMI_TRUE);
	mmi_frm_set_protocol_event_handler(MSG_ID_MMI_EQ_PROTOCOL_UT_THREE, (PsIntFuncPtr)mmi_frm_protocol_event_handler_ut_func_four, MMI_FALSE);
	mmi_frm_set_protocol_event_handler(MSG_ID_MMI_EQ_PROTOCOL_UT_FOUR, (PsIntFuncPtr)mmi_frm_protocol_event_handler_ut_func_five, MMI_TRUE);
	MMI_ASSERT(g_event_table_runtime_info.max_events == 6);
	MMI_ASSERT(g_event_table_runtime_info.used_events == 6);
	mmi_frm_clear_protocol_event_handler(MSG_ID_MMI_EQ_PROTOCOL_UT_TWO, mmi_frm_protocol_event_handler_ut_func_three);
	MMI_ASSERT(g_event_table_runtime_info.max_events == 6);
	MMI_ASSERT(g_event_table_runtime_info.used_events == 5);
	tempfunc = NULL;
	idx = mmi_frm_search_event(
			EVENT_TABLE_MASTER,
			MSG_ID_MMI_EQ_PROTOCOL_UT_TWO,
			&tempfunc,
			0,
			MMI_FALSE,
			NULL,
...
const static mmi_unit_test_struct g_protocol_event_handler_ut_table[] = 
{
	{1, ut_mmi_frm_protocol_event_single_handler},
	{2, ut_mmi_frm_protocol_event_multi_handler},
	{3, ut_mmi_frm_protocol_event_handler_exec},
	{4, ut_mmi_frm_protocol_event_handler_statistics},
};

#define PROTOCOL_EVENT_HANDLER_UT_TABLE_ENTRY sizeof(g_protocol_event_handler_ut_table) / sizeof(g_protocol_event_handler_ut_table[0])

void mmi_frm_protocol_event_handler_ut(U8 index)
{
	/*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if (index == 0)
    {
        for (i = 0; i < PROTOCOL_EVENT_HANDLER_UT_TABLE_ENTRY; i++)
        {
            g_protocol_event_handler_ut_table[i].test();
        }
    }
    else
    {
        for (i = 0; i < PROTOCOL_EVENT_HANDLER_UT_TABLE_ENTRY; i++)
        {
            if (g_protocol_event_handler_ut_table[i].index == index)
            {
                g_protocol_event_handler_ut_table[i].test();
            }
        }
...
#endif /* __MMI_PROTOCOL_EVENT_HANDLER_UT__ */
