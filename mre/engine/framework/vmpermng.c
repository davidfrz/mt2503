...
VMINT vm_per_init( void )
{
	VMINT count = 1;
#ifndef __MRE_CORE_SLIMSANDBOX__
	count = TOTAL_COUNTER - FUNCSRC_COUNTER;
#endif
    mrefun_list = (VMINT *)_vm_kernel_malloc( sizeof(VMINT) * count );

	if( mrefun_list == NULL )
		return PER_OP_FAIL;

    memset( mrefun_list, 0x00, sizeof(VMINT) * count );
...
#else
        resid = MatchInList( hashvalue, HashList, FUNCSRC_COUNTER);
#endif
	}
