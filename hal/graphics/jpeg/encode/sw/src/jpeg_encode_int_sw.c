...
#include "us_timer.h"
#define MAX_ENCODE_CNT  20
kal_uint32 u4EncodeCount = 0;
kal_uint32 u4EncodeStartTime, u4EncodeTime[MAX_ENCODE_CNT];
kal_uint32 u4EncodeSubStartTime;
kal_uint32 u4EncodeFragTime[MAX_ENCODE_CNT];
kal_uint32 u4EncodeQuantFDCTTime[MAX_ENCODE_CNT];
kal_uint32 u4EncodeHuffEncTime[MAX_ENCODE_CNT];
kal_uint32 u4EncodeFragCount = 0, u4EncodeQuantFDCTCount = 0, u4EncodeHuffEncCount = 0;
#endif
...
static JPEG_STATUS_ENUM _jpeg_enc_sw_process_encoder(JPEG_ENCODER_HANDLE handle)
{
   ...
   u4EncodeStartTime = ust_get_current_time();
   u4EncodeFragCount = 0;
   u4EncodeQuantFDCTCount = 0;
   u4EncodeHuffEncCount = 0;
#endif
   ...
   while (--dstCurrentMcuColumn >= 0)
   {
      ...
      while (dstCurrentMcuRow > 0)
      {
         ...
         JPEG_SWLA_LOG("FRG",1);
#if defined(__JPEG_TIMER_DEBUG__)
   		u4EncodeSubStartTime = ust_get_current_time();
#endif
         swjeFragProcProcessFragment(pFragProcessor);
#if defined(__JPEG_TIMER_DEBUG__)
		u4EncodeFragTime[u4EncodeCount] += ust_get_duration(u4EncodeSubStartTime,ust_get_current_time());
		u4EncodeFragCount ++;
#endif
         JPEG_SWLA_LOG("FRG",0);
                  ...
                  while (--duWidth >= 0)
                  {
...
#if defined(__JPEG_TIMER_DEBUG__)
				   		u4EncodeQuantFDCTTime[u4EncodeCount] += ust_get_duration(u4EncodeSubStartTime,ust_get_current_time());
					   	u4EncodeQuantFDCTCount ++;
#endif
                     }
                     ...
                     /// Huffman
                     {
                        ...
                        if (swjeHuffEncProcessDU(pHuffEncoder, compID) < 0)
                        {
                           handle->lastStatus = JPEG_STATUS_ENC_ERROR_INSUFFICIENT_MEMORY;
#if defined(__JPEG_TIMER_DEBUG__)
						   u4EncodeHuffEncTime[u4EncodeCount] += ust_get_duration(u4EncodeSubStartTime,ust_get_current_time());
						   u4EncodeHuffEncCount ++;
#endif
                           JPEG_SWLA_LOG("HUF",0);
                           return handle->lastStatus;
                        }
...
#if defined(__JPEG_TIMER_DEBUG__)
   u4EncodeTime[u4EncodeCount] = ust_get_duration(u4EncodeStartTime,ust_get_current_time());
