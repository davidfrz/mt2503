...
use constant CONFIG_FAIL_FAST_BOOT_ZIMAGE_LIMIT_IS_ZERO => 7;
#****************************************************************************
# Arguments
#****************************************************************************
...
# Check the feature set
{
   ...
   my $fastBootOption = &vivaHelper::GetMakefileOption("__TINY_IOT_FAST_BOOT__");
   ...
   if (defined $fastBootOption)
   {
      $fastbootEnabled = 1;
      debugLog("FAST BOOT is enabled");
   }
...
printLog("Parsing feature configuration file");
{
   ...
   if (defined $featureConfig{FAST_BOOT_LIMIT_ZIMAGE_MAX_DECOMPRESSED_SIZE})
   {
      $fastbootlimitzimagesize = $featureConfig{FAST_BOOT_LIMIT_ZIMAGE_MAX_DECOMPRESSED_SIZE};
   }
   ...
   debugLog("Fast Boot Limit Zimage Size = $fastbootlimitzimagesize");
}
...
sub configBootZImage_ZImage_Alice
{
   ...
   # Configure
   {
      ...
      {
         printLog("fota option:DIFF_IMAGE_UPDATE is opened");
         if ($bootZImageEnabled)
         {
             if ($bootZImageRegionSize > (1048576))
             {
                printLog("warning!!: bootzimage size is bigger than 1MB");
             }
         }
         if ($zImageEnabled)
         {
             if ($zImageRegionSize > (1048576))
             {
                printLog("warning!!: zimage size is bigger than 1MB");
             }
         }
      }
...
sub getFreeRAMSize
{
   ...
   &vivaHelper::GetLISExecutionRegionInfo("TS_EMI_CODE_EV", \%regionInfo);
   $usedRAM += $regionInfo{"max_size"};
   debugLog("TS_EMI_CODE_EV = ".$regionInfo{"max_size"});
   ...
   &vivaHelper::GetLISExecutionRegionInfo("UB_RAM", \%regionInfo);
   $usedRAM += $regionInfo{"max_size"};
   debugLog("UB_RAM = ".$regionInfo{"max_size"});
   ...
   # Check if MBA_COMPRESSION is enabled
   {
      ...
      if ($mbaEnabled)
      {
         ...
         debugLog("Free RAM size = External Total RAM Size - (End of cacheable ZI + TINY RESMEM + TINY CSCIMEM + SWLA RES RAM + DSP TX RAM + DSP RX RAM + UBIN RESMEM) - Cache Dummy End Size - LANG PACK ROM Decompressed Size - CUSTPACK ROM Decompressed Size");
         debugLog("   = $result");
      }
      else
      {
         debugLog("Free RAM size = External Total RAM Size - (End of cacheable ZI + TINY RESMEM + TINY CSCIMEM + SWLA RES RAM + DSP TX RAM + DSP RX RAM + UBIN RESMEM) - Cache Dummy End Size");
         debugLog("   = $result");
      }
   }

   if ($fastbootEnabled == 1)
   {
      if ($result >= $fastbootlimitzimagesize)
      {
         &configFail(CONFIG_FAIL_FAST_BOOT_ZIMAGE_LIMIT_IS_ZERO) if ($fastbootlimitzimagesize == 0);
         $result = $fastbootlimitzimagesize;
         debugLog("Free RAM size are limited by $result");
      }
      else
      {
         debugLog("Free RAM size are small than limited value");
      }
   }
...
sub configFail
{
   ...
   elsif ($failCode == CONFIG_FAIL_FAST_BOOT_ZIMAGE_LIMIT_IS_ZERO)
   {
      $failReason = "the FAST_BOOT_LIMIT_ZIMAGE_MAX_DECOMPRESSED_SIZE can't be zero when the FAST_BOOT is enabled!!";
      $recommend = "Please add the FAST_BOOT_LIMIT_ZIMAGE_MAX_DECOMPRESSED_SIZE to custom_feature_config.h";
   }
   else
   {
      $failReason = "Unknown.";
