...
void srv_ucm_call_scenario_control()
{
    ...
    {
    #ifdef __MMI_HF_SUPPORT__
        if (srv_ucm_bt_query_call_count(SRV_UCM_BT_OUTGOING_STATE, NULL) == 0)
    #endif /* __MMI_HF_SUPPORT__ */
        {
...
void srv_ucm_dial_act_rsp(void *rsp_struct)
{
        ...
        (srv_ucm_query_call_count(SRV_UCM_INCOMING_STATE, SRV_UCM_CALL_TYPE_ALL, NULL) == 0))
    {
        ...
        if (srv_ucm_query_group_count(SRV_UCM_CALL_STATE_ALL, SRV_UCM_CALL_TYPE_ALL, MMI_FALSE, NULL) == 1)
        {
            ...
            #endif /* __IOT_NO_MULTIMEDIA__ */
            #ifdef __MMI_HF_SUPPORT__
                if (srv_bt_cm_is_profile_connected(SRV_BT_CM_HF_CONNECTION) && srv_btsco_is_sco_open())
                {
        ...
        else if ((index_iist.group_index == -1) && (index_iist.call_index == -1)) /* Add to group entry */
        {
            ...
            if (g_ucm_srv_p->call_list.num_group < SRV_UCM_MAX_GROUP)
            {
                ...
                if (g_ucm_srv_p->call_list.group_info[i].call_state == SRV_UCM_OUTGOING_STATE)
                {
                    ...
                    if (SRV_UCM_MODULE_ORIGIN_OKSOS == g_ucm_srv_p->dial_info.module_id)
                    {
                        g_ucm_srv_p->call_list.group_info[i].call_info[0].module_id = SRV_UCM_MODULE_ORIGIN_OKSOS;
                    }
...
void srv_ucm_alert_ind(void *ind_struct)
{
    ...
    /*----------------------------------------------------------------*/
#ifdef __MMI_HFP_SUPPORT__
    if (srv_ucm_query_index_by_id(&int_alert_ind->uid_info, &index))
    {
        g_ucm_srv_p->call_list.group_info[index.group_index].call_info[index.call_index].is_alert = MMI_TRUE;
    }
#endif /* __MMI_HFP_SUPPORT__ */
...
void srv_ucm_ring_ind(void *ind_struct)
{
    ...
    {
    #ifdef __MMI_HF_SUPPORT__
        if (srv_ucm_bt_query_call_count(SRV_UCM_BT_CALL_STATE_MOMT, NULL) > 0)
        {
...
mmi_ret srv_ucm_on_time_change_event(mmi_event_struct *evt)
{
    ...
    if (call_count > 0)
    {
        ...
        MMI_TRACE(MMI_COMMON_TRC_G5_CALL, TRC_SRV_UCM_TIME_CHANGE_HDLR, 'o', 'l', 'd',
        ...
        MMI_TRACE(MMI_COMMON_TRC_G5_CALL, TRC_SRV_UCM_TIME_CHANGE_HDLR, 'n', 'e', 'w',
