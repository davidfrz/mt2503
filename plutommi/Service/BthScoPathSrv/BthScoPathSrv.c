...
static MMI_BOOL suspend_by_hf = MMI_FALSE;
...
MMI_BOOL srv_btsco_connect_sco_req(void (*callback) (void))
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    #if defined(__MMI_A2DP_SUPPORT__) && defined(__BT_SPEAKER_SUPPORT__) && defined(__MMI_HF_SUPPORT__)
...
//    if (mmi_bt_a2dp_query_state() != A2DP_STATE_IDLE)
    if (srv_bt_a2dp_query_state() != A2DP_STATE_IDLE)
    {
 ...
 *  void
 *****************************************************************************/
void srv_btsco_open_stream_callback(U8 profile, mdi_result result, U16 error_cause ,U8 connect_id)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_MEDIA_TRC_G2_APP, SRV_BTSCO_OPEN_STREAM_CALLBACK, profile, connect_id);

#ifdef __MMI_HF_SUPPORT__
    /* Event to notify app SCO open */
    ...
    #ifdef __MMI_HF_SUPPORT__
        if (result == MDI_AUDIO_SUCCESS && srv_bt_cm_is_profile_connected(SRV_BT_CM_HF_CONNECTION))
        {
        #ifdef __MMI_TELEPHONY_SUPPORT__
            if(srv_ucm_is_any_call())
            {
                StartTimer(BT_SCO_CONNECT_TIMER, BT_SCO_REJECT_CONNNECT_DURATION, srv_btsco_dialer_close_stream_req);
                return;
            }
        #endif /* __MMI_TELEPHONY_SUPPORT__ */
            if((srv_bt_dialer_get_incoming_call_ans_status(connect_id) == MMI_FALSE) &&
              (srv_bt_dialer_get_ciev_call(connect_id) == 1) )
            {
                // srv_bt_dialer_set_incoming_call_ans_status(connect_id, MMI_TRUE);
                StartTimer(BT_SCO_CONNECT_TIMER, BT_SCO_REJECT_CONNNECT_DURATION, srv_btsco_dialer_close_stream_req);
                return;
            }
            if (!srv_bt_dialer_is_sco_allow(connect_id))
            {
                StartTimer(BT_SCO_CONNECT_TIMER, BT_SCO_REJECT_CONNNECT_DURATION, srv_btsco_dialer_close_stream_req);
                return;
            }
            ...
            suspend_by_hf = MMI_TRUE;
            mdi_audio_suspend_background_play_by_app(MDI_AUD_SUSPEND_BG_SCO_INQ);

        #ifdef __BT_DIALER_HF_NO_IN_BAND_TONE__
            if (srv_bt_dialer_is_inband_ringtone(connect_id))
            {
                srv_bt_dialer_set_mute_inband_flag(connect_id, MMI_TRUE);
            }
            else
            {
                srv_bt_dialer_set_mute_inband_flag(connect_id, MMI_FALSE);
    ...
    evt.conn_id = connect_id;
    evt.profile = profile;
    evt.is_open = MMI_TRUE;
 ...
 *  void
 *****************************************************************************/
void srv_btsco_close_stream_callback(U8 profile, mdi_result result, U16 error_cause,U8 connect_id)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_MEDIA_TRC_G2_APP, SRV_BTSCO_CLOSE_STREAM_CALLBACK, profile, connect_id);
...
MMI_BOOL srv_btsco_open_stream_req(void)
{
...
#ifdef __MMI_HFP_SUPPORT__
#ifdef __BT_DIALER_SUPPORT__
    if(srv_btsco_is_sco_connect_allowed() && srv_btsco_is_profile_connect_allowed() && !srv_bt_cm_is_profile_connected(SRV_BT_CM_HF_CONNECTION))
#else
...
#endif
    {
        ...
        if (srv_bthf_list_device(&dev_id, 1))
        {
            ...
            is_connect = (MMI_BOOL)mdi_audio_bt_open_stream(BT_HFP, (U16) info.conn_id, info.pContext, NULL);
        }
    }
#endif /* __MMI_HFP_SUPPORT__ */
    return is_connect;
...
void srv_btsco_connect_audio_via_sco(void)
{
    ...
    *   call setup may leave the voice in phone */
#ifdef __BT_DIALER_SUPPORT__
    if(!g_mmi_bt_sco_is_in_call && srv_bt_cm_is_profile_connected(SRV_BT_CM_HFP_CONNECTION) && !srv_bt_cm_is_profile_connected(SRV_BT_CM_HF_CONNECTION) && srv_bt_cm_is_audio_path_to_headset())
#else
    if(!g_mmi_bt_sco_is_in_call && ((srv_bt_cm_is_profile_connected(SRV_BT_CM_HFP_CONNECTION) || srv_bt_cm_is_profile_connected(SRV_BT_CM_HSP_CONNECTION)) && srv_bt_cm_is_audio_path_to_headset()))
#endif
    {
 ...
 *  void
 *****************************************************************************/
MMI_BOOL srv_btsco_dialer_open_stream_req(S32 srv_hd)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    connect_id = srv_bt_dialer_get_connection_id(srv_hd);
    ...
    {
        mdi_audio_set_sco_mode(MDI_AUD_BT_SCO_MODE_BT_DIALER);
        return mdi_audio_bt_open_stream(BT_HF, (U16) connect_id, NULL, NULL);
    }
    return MMI_FALSE;    
}
...
MMI_BOOL srv_btsco_dialer_close_stream_req(void)
{
    ...
    return mdi_audio_bt_close_stream(BT_HF);
}
