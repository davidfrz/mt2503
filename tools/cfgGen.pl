...
# Constants
#****************************************************************************
my $CFGGEN_VERNO     = " V0.38 -> p0.41";
                      #  p0.41 , Support IOT_FOTA
...
print "BUILD_PATH: $BUILD_PATH, MAUI_BIN: $MAUI_BIN, BL_BIN: $BL_BIN, FOTA_BIN: $FOTA_BIN, UBIN_BIN: $UBIN_BIN, MAKEFILE: $MAKEFILE, BB_PATH: $BB_PATH, SCATTER: $SCATTER, CC_CMD=$CC_CMD, VIA_CMD=$VIA_CMD, OPTION_TMP=$OPTION_TMP\n" if ($DebugPrint == 1);
...
foreach (sort {$LoadView{$a}->{'seq'} <=> $LoadView{$b}->{'seq'}} keys %LoadView)
{
    ...
    elsif ($scat_key eq 'UPDATE_ROM')
    {
        $scat_key = $UBIN_BIN;
        if(defined $MAKEFILE_OPTIONS{'update_binary_file'} and $MAKEFILE_OPTIONS{'update_binary_file'} eq 'TRUE')
        {
           &error_handler("$BUILD_PATH\\$UBIN_BIN does not exist! Please check link error for ubin.", __FILE__, __LINE__)if(!-e $BUILD_PATH."\\".$UBIN_BIN);
        }
    }
...
[UBIN]
File=$UBIN_BIN
Load_Address=$LoadView{'UPDATE_ROM'}->{'begin'}
