 ...
 *************************************************************************/
  ...
  kal_uint8* lga_buf_ptr = 0 ;
  kal_uint32 lga_buf_len = 0;
  kal_bool b_lga_log_unsent=KAL_FALSE;
 ...
 *************************************************************************/
extern void tst_indicate_only_write_to_file_status_internal(kal_uint32 cWriteToFileInd);
...
tst_write2_sd_option_t tst_file_query_write2file_option(void)
{
  return (tst_dump_info.tst_write2_sd_option);
}

void tst_file_init_l1_log_buf(void)
{
  tst_dump_info.nL1BufferWritePtr = 0;
  tst_dump_info.nL1BufferVancancy = TST_L1_BUFFER_SIZE_FOR_WRITE_FILE-1;
}

kal_uint32 tst_file_query_l1_log_space(void)
{
  return tst_dump_info.nL1BufferVancancy;  
}

kal_uint32 tst_file_query_ps_log_space(void)
{
  return tst_dump_info.nFileFreeSpace;  
}

kal_uint32 tst_file_reserve_ps_log_space(kal_uint32 length)
{
...
kal_uint32 tst_file_query_disk_space(void)
{
  return tst_dump_info.nDiskFreeSpace;  
}

void tst_file_indicate_write2file_only(kal_uint32 cWriteToFileInd)
{
  tst_indicate_only_write_to_file_status_internal(cWriteToFileInd);
}

void tst_file_set_ps_log_head_packet(kal_bool value)
{
  tst_dump_info.bHeadOfOnePacket = value;
}

kal_bool tst_file_get_ps_log_head_packet(void)
{
  return tst_dump_info.bHeadOfOnePacket;
}

void tst_dump_reset_file_buf_info(void)
{
  ASSERT(tst_err_log_to_file == NO_LOG_ERROR);  //no error  
  tst_dump_info.nBufferPtr = 0;
}

kal_uint32 tst_file_request_file_space_on_disk(kal_uint32 request_space)  //TST_MAX_LOG_SIZE_FOR_SINGLE_FILE
{
...
kal_bool tst_file_reserve_disk_space(kal_uint32 length)
{    
...
void tst_file_dump_log_to_file(kal_uint8 *buffer, kal_uint32 length, kal_uint32 caller_context)
{
  ...
  if (tst_file_ppmode_reader_start_log2ROM == KAL_FALSE) 
  {
    //tst_sys_trace("[PPMode] non-init , reteun");
    return;
  }
...
*
*****************************************************************************/
void tst_file_check_and_dump_isr_log(void)
{
  kal_uint8 *dataPtr;
...
*
*****************************************************************************/
kal_uint32 tst_file_dump_isr_log_to_file(kal_uint8 *buffer, kal_uint32 length)
{
#ifdef __TST_WRITE_TO_FILE_PINGPONG_MODE__
  if (tst_file_ppmode_reader_start_log2ROM == KAL_FALSE) {return 0; }
#endif //__TST_WRITE_TO_FILE_PINGPONG_MODE__
...
void tst_file_set_tst_write2_sd_option(tst_write2_sd_option_t tst_write2_sd_option)
{
  tst_dump_info.tst_write2_sd_option = tst_write2_sd_option;
}

kal_bool tst_file_query_if_write2sd_start(void)
{
...
kal_uint32 tst_file_query_omit_pkt(void)
{
  return tst_dump_info.nOmitPacketCount;  
}

FS_HANDLE tst_file_query_write_handler(void)
{
  return tst_dump_info.fHandle;
}

void tst_file_set_write_handler(FS_HANDLE fHandle)
{
  tst_dump_info.fHandle = fHandle;
}

WCHAR* tst_file_query_assert_folder_name(void)
{
  return tst_dump_info.strAssertFolderName;  
}

kal_int32 tst_file_store_unsaved_log(void)
{
...
kal_bool tst_file_set_already_dump(void)
{
...
void tst_dump_to_file_for_crash(void)
{
  tst_dump_to_file_for_crash_internal();
}

int tst_lga_save_logs(kal_uint8* buf, kal_uint32 len)
{
#if defined(__LOGACC_ENABLE__) && !defined(UNIT_TEST)   
  kal_uint32 _savedMask = 0;  
  #ifdef __TST_WRITE_TO_FILE_PINGPONG_MODE__
  if (tst_file_ppmode_reader_start_log2ROM == KAL_FALSE) {return -1;}
  #endif //__TST_WRITE_TO_FILE_PINGPONG_MODE__
...
kal_uint32 tst_file_notify_sd_ready(void)
{
...
tst_write2_sd_option_t tst_file_query_write2file_option(void) { return TST_Write2File_None;}
kal_uint32 tst_file_query_l1_log_space(void){ return 0xFFFFFFFF; }
kal_uint32 tst_file_query_ps_log_space(void){ return 0xFFFFFFFF; }
void tst_file_init_l1_log_buf(void){}
void tst_file_sp_check_sd_ready(void){}
kal_uint32 tst_file_check_if_output2uart(void){return 0;}
void tst_file_indicate_write2file_only(kal_uint32 cWriteToFileInd){}
kal_uint32 tst_file_dump_isr_log_to_file(kal_uint8 *buffer, kal_uint32 length){return 0;}
void tst_dump_reset_file_buf_info(void){}
kal_bool tst_file_get_ps_log_head_packet(void){ return KAL_FALSE;}
kal_uint32 tst_file_request_file_space_on_disk(kal_uint32 request_space){return 0;}
kal_uint32 tst_file_query_disk_space(void){return 0;}
void tst_file_discard_rest_file_space(void){}
kal_uint32 tst_file_reserve_ps_log_space(kal_uint32 length){return 0;}
void tst_file_set_tst_write2_sd_option(tst_write2_sd_option_t tst_write2_sd_option){}
kal_bool tst_file_query_if_write2sd_start(void){return KAL_FALSE;}
kal_uint32 tst_file_query_omit_pkt(void){return 0;}
FS_HANDLE tst_file_query_write_handler(void){return NULL;}
WCHAR* tst_file_query_assert_folder_name(void){ return NULL;}
kal_uint32 tst_file_query_nBufferPtr(void){return 0;}
kal_bool tst_file_set_already_dump(void){return KAL_FALSE;}
kal_bool tst_file_reserve_disk_space(kal_uint32 length){return KAL_FALSE;}
void tst_file_dump_log_to_file(kal_uint8 *buffer, kal_uint32 length, kal_uint32 caller_context){}
void tst_dump_to_file_for_crash(void){}
void tst_file_set_write_handler(FS_HANDLE fHandle){}
kal_int32 tst_file_store_unsaved_log(void){return 0;}
void tst_file_check_and_dump_isr_log(void){}
void tst_file_set_ps_log_head_packet(kal_bool value){}
int tst_lga_save_logs(kal_uint8* buf, kal_uint32 len){return 0;}
kal_uint32 tst_file_notify_sd_ready(void){return 0;}
