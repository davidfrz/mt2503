...
#ifdef __PHB_USIM_ADDITIONAL_SUPPORT__
    filetype = phb_desc_get_iap_pos((sim_file_index_enum)file_idx, occurrence, pbr_rec, iap_pos);
    if (filetype == 2)
    {
        return KAL_FALSE;
    }
 ...
 *  void
 *****************************************************************************/
kal_bool phb_desc_get_file_path(kal_uint16 idx, kal_uint8 occurrence, kal_uint16 pbr_rec, kal_uint8 *path)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    {
        sim_get_full_path((sim_file_index_enum)idx, path);
    }
    else
    {
...
void phb_data_desc_new()
{
    ...
    {
        phb_ptr->data_desc[i].free_bitmap_bits = (kal_uint16) (phb_get_data_desc_bytes((data_desc_enum)i) << BYTE_SIZE_SHIFT_ARITHMETIC);
...
data_desc_enum phb_data_desc_file_id_to_desc(kal_uint16 file_id, kal_uint8 occurrence)
{
    ...
    switch (file_id)
    {
        ...
        case FILE_G_ANR_IDX:
        case FILE_U_ANR_IDX:
            return (data_desc_enum) (DATA_DESC_ANR + occurrence - 1);
    
        case FILE_G_SNE_IDX:
        case FILE_U_SNE_IDX:
            return (data_desc_enum) (DATA_DESC_SNE + occurrence - 1);
    
        case FILE_G_EMAIL_IDX:
        case FILE_U_EMAIL_IDX:
            return (data_desc_enum) (DATA_DESC_EMAIL + occurrence - 1);
