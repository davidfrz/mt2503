...
extern kal_uint8 EINT_Get_Polarity(kal_uint8 eintno);
...
DCL_STATUS DclEINT_Control(DCL_HANDLE handle,DCL_CTRL_CMD cmd,void * data)
{
	...
	switch(cmd)
	{
		...
		case EINT_CMD_SET_DST_MOD_ID:
		{
			EINT_CTRL_SET_DST_MOD_ID_T *rDstModID;
			rDstModID = (EINT_CTRL_SET_DST_MOD_ID_T *)data;
			if(rDstModID->dst_mod_id > END_OF_MOD_ID)
			{
				ASSERT(0);
				return STATUS_INVALID_ARGUMENT;
			}
			EINT_FUNC[eint_no].is_set_dst_mod_id = KAL_TRUE;
			EINT_FUNC[eint_no].eint_msg_dst_mod = rDstModID->dst_mod_id;
			EINT_FUNC[eint_no].eint_func = eint_mre_cb;
			return STATUS_OK;
			
		}
		
		case EINT_CMD_SET_AUTO_CHANGE_POLARTIY:
		{
			EINT_CTRL_SET_AUTO_CHANGE_POL_T *rAutoChangePolarity;
			rAutoChangePolarity = (EINT_CTRL_SET_AUTO_CHANGE_POL_T *)data;
			if(rAutoChangePolarity->autoChangePol > 1)
			{
				ASSERT(0);
				return STATUS_INVALID_ARGUMENT;
			}
			EINT_FUNC[eint_no].autoChangePol = rAutoChangePolarity->autoChangePol;
			return STATUS_OK;
...
DCL_STATUS DclEINT_Close(DCL_HANDLE handle)
{
	...
	EINT_FUNC[eint_no].is_set_dst_mod_id = KAL_FALSE;
...
/*-----------------------------------*/
	...
	if(EINT_FUNC[eint_no].is_set_dst_mod_id == KAL_FALSE)
	{
		ASSERT(0);
		return;
	}
	...
	DRV_BuildPrimitive(
		       eint_ilm,
			   MOD_DRV_HISR,
			   EINT_FUNC[eint_no].eint_msg_dst_mod,
	...
	/*after send message, change the polarity*/
	if(1 == EINT_FUNC[eint_no].autoChangePol){
		current_polarity = EINT_Get_Polarity(eint_no);
		EINT_Set_Polarity(eint_no,!current_polarity);
	}
