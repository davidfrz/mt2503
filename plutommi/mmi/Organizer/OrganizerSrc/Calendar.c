...
static MMI_BOOL mmi_swatch_calendar_pen_down_hdlr(mmi_pen_point_struct down)
{
  S32 str_w, str_h;
	...
	/**********************************code body***************************************/
		    ...
		    if(96<=down.x && down.x<= 96 + 57 && 
				176 * 4/3 + (57*4/3 - 57)/2 + 10<=down.y && down.y<= 176 * 4/3 + (57*4/3 - 57)/2 + 10 + 57)
			{					
				//monthly view draw hilight;
				gdi_layer_lock_frame_buffer();
			    gdi_layer_push_and_set_clip(96 , 176 * 4/3 + (57*4/3 - 57)/2 + 10, 96 + 57, 176 * 4/3 + (57*4/3 - 57)/2 + 10 + 57);

                gdi_layer_set_source_key(MMI_TRUE,GDI_COLOR_TRANSPARENT);
				gdi_layer_clear(GDI_COLOR_BG);
				
			    gdi_image_draw_id(96, 176 * 4/3 + (57*4/3 - 57)/2 + 10, IMG_CLNDR_LAUNCH_MONTH_VIEW_PRESSED);//(96.176)
			    
			    gdi_layer_pop_clip();

			    gdi_layer_unlock_frame_buffer();
			    gdi_layer_blt_previous(96 , 176 * 4/3 + (57*4/3 - 57)/2 + 10, 96 + 57, 176 * 4/3 + (57*4/3 - 57)/2 + 10 + 57);
...
static MMI_BOOL mmi_swatch_calendar_pen_move_hdlr(mmi_pen_point_struct down, mmi_pen_point_struct move_pos)
{
   S32 str_w, str_h;
	...
	/**********************************************code body***************************************************/
       ...
       #elif defined(__MMI_MAINLCD_240X320__)
	    if(!((0<=down.x && down.x<=240 && 176 * 4/3 + (57*4/3 - 57)/2<=down.y && down.y<=LCD_HEIGHT)))//
	   #elif defined(__MMI_MAINLCD_320X320__)
	   ...
	   #elif defined(__MMI_MAINLCD_240X320__)
	    if(96<=move_pos.x && move_pos.x<=96+57 && 176* 4/3 + (57*4/3 - 57)/2 + 10<=move_pos.y && move_pos.y<= 176* 4/3 + (57*4/3 - 57)/2 + 10 + 57) 
		{
		    //pen move时，移到指定区域，monthly view draw hilight;
								gdi_layer_lock_frame_buffer();
								gdi_layer_push_and_set_clip(96 , 176* 4/3 + (57*4/3 - 57)/2 + 10, 176, 176* 4/3 + (57*4/3 - 57)/2 + 10+57);
				
								gdi_layer_set_source_key(MMI_TRUE,GDI_COLOR_TRANSPARENT);
								gdi_layer_clear(GDI_COLOR_BG);

								gdi_image_draw_id(96, 176* 4/3 + (57*4/3 - 57)/2 + 10, IMG_CLNDR_LAUNCH_MONTH_VIEW_PRESSED);//
								
								gdi_layer_pop_clip();		
								gdi_layer_unlock_frame_buffer();
								gdi_layer_blt_previous(96 , 176* 4/3 + (57*4/3 - 57)/2 + 10, 176, 176* 4/3 + (57*4/3 - 57)/2 + 10+57);
		...
		else
		{
		    //pen move时，移到非指定区域，monthly view draw non_hilight;
								...
								gdi_image_draw_id(96, 176* 4/3 + (57*4/3 - 57)/2 + 10, IMG_CLNDR_LAUNCH_MONTH_VIEW_NORMAL);//
...
static MMI_BOOL mmi_swatch_calendar_click_handle(mmi_pen_point_struct down, mmi_pen_point_struct up)
{
    ...
    if(mmi_swatch_calendar_check_grp_is_calendar() == MMI_FALSE)
     ...
     //0<=pos.x<=240       0<=pos.y<=190 = 240 -50;
       ...
       #elif defined(__MMI_MAINLCD_240X320__)
	    if(96<=up.x && up.x<=96+57 && 176 * 4/3 + (57*4/3 - 57)/2 + 10<=up.y && up.y<= 176 * 4/3 + (57*4/3 - 57)/2 + 10 + 57) 
	   #elif defined(__MMI_MAINLCD_320X320__)
	    //for 320x320;
	   ...
	   #endif
		{
		    ...
		    if(96<=down.x && down.x<=96+57 && 176<=down.y && down.y<= 176+57)
          #elif defined(__MMI_MAINLCD_240X320__)
		    if(96<=down.x && down.x<=96+57 && 176* 4/3 + (57*4/3 - 57)/2 + 10<=down.y && down.y<= 176* 4/3 + (57*4/3 - 57)/2 + 10 + 57)
		  #elif defined(__MMI_MAINLCD_320X320__)
		    //for 320x320;
		    if(96* 4/3 + (57*4/3 - 57)/2 - 5<=down.x && down.x<=96* 4/3 + (57*4/3 - 57)/2 - 5 + 57 && 
			   176* 4/3 + (57*4/3 - 57)/2 + 10<=down.y && down.y<= 176* 4/3 + (57*4/3 - 57)/2 + 10 + 57)
...
static void mmi_show_swatch_calendar_cell_info(swatch_common_pos_struct calendar_layout, U8 alarm_index)
{
   ...
   #elif defined(__MMI_MAINLCD_240X320__)
    gdi_image_draw_id(120-70, ori_calendar_layout_y - TODAY_MARK_OFFSET_Y + 48 + 20, day_image_id[swatch_calendar_show_buffer[alarm_index].nDay / 10]);
	gdi_image_draw_id(120, ori_calendar_layout_y - TODAY_MARK_OFFSET_Y + 48 + 20, day_image_id[swatch_calendar_show_buffer[alarm_index].nDay % 10]);
   #elif defined(__MMI_MAINLCD_320X320__)
    //for 320x320;
