...
#if BT_CVSD_VOC_DEBUG
kal_bool g_isbtvoc = KAL_FALSE;
#endif 
//=============================================================================================
...
#endif   
#if BT_CVSD_VOC_RX_DEBUG

	if(g_isbtvoc)
	{
		BTWriteVocToCatcher(uBlockSize*SCO_RX_PLC_SIZE/2,(uint16*)btsco.pTempPacketBuf);
	}
#endif 
...
#endif
#if BT_CVSD_VOC_TX_DEBUG
	if(g_isbtvoc)
    {
		BTWriteVocToCatcher(uPacketLength*uPacketNumber/2,(uint16*)btsco.pTempPacketBuf);
   }
#endif
}
...
#if BT_CVSD_VOC_DEBUG
void BT_VOC_init(void)
{
    kal_uint32 module_id = MOD_L1AUDIO_SPH_SRV;
    

    if(!tst_trace_check_ps_filter_off(TRACE_GROUP_BT_VOC, &module_id, 0x2))
    {
       g_isbtvoc=KAL_TRUE;
    }     
    else
...
void BTWriteVocToCatcher( uint32 vmLen, uint16 *vmBuf )
{
    uint32 len  = vmLen<<1;
    uint16 *buf = vmBuf;
