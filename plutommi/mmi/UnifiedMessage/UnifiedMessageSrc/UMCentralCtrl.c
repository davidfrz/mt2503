...
extern SMS_HANDLE g_sms_bt_send_template_handle;
extern SMS_HANDLE g_sms_bt_view_handle;
#endif
...
#ifdef __MMI_BT_MAP_CLIENT__
    if ((g_um_entry_type == MMI_UM_ENTRY_BT) && (g_um_is_entry_main_menu || g_um_is_new_group))
    {
...
static mmi_ret mmi_um_ui_scrlock_event_proc(mmi_event_struct * evt)
{
        ...
        case EVT_ID_SCREEN_LOCK_KEY_PRE_LOCK_EVT_ROUTING:
            {
                ...
                {
                    if (SCR_ID_SMS_UM_BT_PROCESSING == curr_scr_id ||
                        SCR_ID_SMS_SENDING == curr_scr_id || 
                        SCR_ID_SMS_BT_PROCESSING == curr_scr_id)
                    {
                        return MMI_RET_ERR;
                    }
                }
...
mmi_ret mmi_um_ui_cc_grp_proc(mmi_event_struct *evt)
{
    ...
    switch (evt->evt_id)
    {
        ...
        case EVT_ID_GROUP_ACTIVE:
        {
            ...
            #ifdef __MMI_SCREEN_LOCK_ANY_TIME_WDEVICE__
            mmi_frm_cb_reg_event(EVT_ID_SCREEN_LOCK_KEY_PRE_LOCK_EVT_ROUTING, mmi_um_ui_scrlock_event_proc, NULL);
            #endif
        ...
        case EVT_ID_GROUP_DEINIT:
        {
            mmi_um_ui_cc_del_grp_cntx(grp_cntx);
 			g_um_entry_type = MMI_UM_ENTRY_NONE;
			
            #if defined(__MMI_WEARABLE_DEVICE__) && defined(__MMI_BT_MAP_CLIENT__)
            mmi_sms_um_bt_ui_cc_del_grp_cntx_ext();
            srv_um_sms_clear_msg_info_cache(SRV_UM_MSG_ALL, SRV_UM_MSG_BOX_ALL);
            mmi_sms_um_bt_ui_cc_delete_all_scrn_ext();
			
            #ifdef __MMI_SCREEN_LOCK_ANY_TIME_WDEVICE__
            mmi_frm_cb_dereg_event(EVT_ID_SCREEN_LOCK_KEY_PRE_LOCK_EVT_ROUTING, mmi_um_ui_scrlock_event_proc, NULL);
            #endif
