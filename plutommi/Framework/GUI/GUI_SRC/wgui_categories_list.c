...
extern void gui_swatch_list_disable_scrollbar(void);
#endif /*__MMI_WEARABLE_DEVICE_UI_CES__*/
...
MMI_BOOL g_cat353_no_title = MMI_FALSE;

void set_cat353_no_title(MMI_BOOL flag)
{
	/*----------------------------------------------------------------*/
   /* Local Variables												 */
   /*----------------------------------------------------------------*/
        ...
        U8 *history_buffer)
{
    ...
    gdi_layer_lock_frame_buffer();
	#ifdef __MMI_WEARABLE_DEVICE_NOTI_UI_ENHANCE__
	if (MMI_FALSE == g_cat353_no_title)
	#endif /*__MMI_WEARABLE_DEVICE_NOTI_UI_ENHANCE__*/
	wgui_cat_add_title_and_softkey(
		title, 
		title_icon, 
...
//add by xl
	wgui_fixed_list_resize_icontext_menu(
			...
			MMI_CONTENT_HEIGHT);
#ifdef __MMI_WEARABLE_DEVICE_NOTI_UI_ENHANCE__
	if (MMI_TRUE == g_cat353_no_title)
	{
		wgui_fixed_list_move_icontext_menu(0,0);
		wgui_fixed_list_resize_icontext_menu(
				MAIN_LCD_DEVICE_WIDTH, 
				MAIN_LCD_DEVICE_HEIGHT);
		resize_fixed_icontext_menuitems(0, 60);
...
{
	gui_swatch_list_disable_scrollbar();
...
#ifdef __MMI_WEARABLE_DEVICE_NOTI_UI_ENHANCE__
WGUI_SWATCH_TWOLINE_LIST_TYPE swath_twoline_list_type = WGUI_SWATCH_TWOLINE_LIST_TYPE1;

void swatch_set_twoline_list_type(WGUI_SWATCH_TWOLINE_LIST_TYPE type)
{	
	/*----------------------------------------------------------------*/
	/* Local Variables												  */
	/*----------------------------------------------------------------*/
	...
	/* Code Body													  */
	/*----------------------------------------------------------------*/
	swath_twoline_list_type = type;
}
#endif /*__MMI_WEARABLE_DEVICE_NOTI_UI_ENHANCE__*/

static void wgui_swatch_two_line_exit(void)
{
	/*----------------------------------------------------------------*/
	/* Local Variables												  */
	/*----------------------------------------------------------------*/
	...
	/*----------------------------------------------------------------*/
	#ifdef __MMI_WEARABLE_DEVICE_UI_ENHANCE__
	swath_twoline_list_type = WGUI_SWATCH_TWOLINE_LIST_TYPE1;
	#endif /*__MMI_WEARABLE_DEVICE_UI_ENHANCE__*/
	wgui_cat1031_tab_exit();
}
/*****************************************************************************
 ...
 *  right_softkey        [IN]        Right softkey label
 *  number_of_items  [IN]        Item count
 *  get_item_func       [IN]        get item funtion
 *  highlighted_item    [IN]        Index of highlighted item
 *  left_icon                [IN]        Left softkey icon
  * right_icon		     [IN]        Right softkey icon
 *  history_buffer        [IN]        History buffer
 * RETURNS
 *  void
 *****************************************************************************/
void wgui_swatch_two_line_menu_list_screen(
				PU8 title,
                PU8 title_icon,
                U16 left_softkey,
                U16 left_icon,
                ...
                U8 *history_buffer)
{
    ...
    S32 left_icon_w = 0, left_icon_h = 0;
    S32 text_w = 0, text_0_h = 0, text_1_h = 0;
    S32 x1 = 0, y1 = 0;
#ifdef __MMI_WEARABLE_DEVICE_NOTI_UI_ENHANCE__
	S32 x2 = 0,y2 = 0;
	...
	S32 text_nums = 2, icon_nums = 1;
	/*----------------------------------------------------------------*/
	/* Code Body													  */
	/*----------------------------------------------------------------*/
#ifdef __MMI_WEARABLE_DEVICE_NOTI_UI_ENHANCE__
	if (WGUI_SWATCH_TWOLINE_LIST_TYPE2 == swath_twoline_list_type) //type1
	{
		text_nums = 3;
		icon_nums = 2;
	}
	...
	x1 = GUI_ICONTEXT_MENUITEM_ICON_X;
	...
	/*if (NULL == left_icon)
	{
		MMI_fixed_icontext_list_menuitem.resize_icon_text_coordinates = swatch_twoline_list_resize_icon_text_coordinates; 
	}*/
    
	if (NULL != left_icon)
	{
	    y1 = (MMI_MENUITEM_MULTIROW_HEIGHT - left_icon_h) / 2; //MMI_TWOLINE_MENUITEM_TOP_GAP;
	}
	//set menuitem first icon position
    set_fixed_icontext_list_icon_coordinates(
		0, 
		x1, 
		y1, 
  ...
  if (WGUI_SWATCH_TWOLINE_LIST_TYPE2 == swath_twoline_list_type) //type1
  {
  	  if (0 == left_icon_w)
	  {
	  	text_w = MMI_CONTENT_WIDTH - MMI_fixed_list_menu.vbar.width - GUI_ICONTEXT_MENUITEM_ICON_X - left_icon_w - MMI_TWOLINE_MENUITEM_RIGHT_TEXT_WIDTH - MMI_ICONTEXT_COLUMN_GAP;
	  }
	  else if (0 != left_icon_w)
	  {
	  	text_w = MMI_CONTENT_WIDTH - MMI_fixed_list_menu.vbar.width - GUI_ICONTEXT_MENUITEM_ICON_X - left_icon_w - MMI_TWOLINE_MENUITEM_RIGHT_TEXT_WIDTH - 2 * MMI_ICONTEXT_COLUMN_GAP;
	  }
  }
  else if (WGUI_SWATCH_TWOLINE_LIST_TYPE1 == swath_twoline_list_type)
#endif /*__MMI_WEARABLE_DEVICE_NOTI_UI_ENHANCE__*/
   text_w = MMI_CONTENT_WIDTH - MMI_fixed_list_menu.vbar.width - GUI_ICONTEXT_MENUITEM_ICON_X - left_icon_w - 2*MMI_ICONTEXT_COLUMN_GAP - 2;
   if (0 != left_icon_w)
   {		
   		x1 = GUI_ICONTEXT_MENUITEM_ICON_X + left_icon_w + MMI_ICONTEXT_COLUMN_GAP;
   }
   else if (0 == left_icon_w)
   {
   		x1 = GUI_ICONTEXT_MENUITEM_ICON_X;
   ...
   gui_set_font(&MMI_medium_font);
   // text_0_h = gui_get_character_height();
   text_0_h = Get_CharHeightOfAllLang(MMI_medium_font.size); //get medium font size
  	
   gui_set_font(&MMI_small_font);
   // text_1_h = gui_get_character_height();
   text_1_h = Get_CharHeightOfAllLang(MMI_small_font.size); //get small font size
   y1 = (MMI_MENUITEM_MULTIROW_HEIGHT - text_0_h - text_1_h - MMI_ICONTEXT_ROW_GAP) / 2;
    
   // y = MMI_TWOLINE_MENUITEM_TOP_GAP;
   //set menuitem first text positon
   set_fixed_icontext_list_text_coordinates(
		0, 
		x1, 
		y1, 
	...
	//y = MMI_MENUITEM_MULTIROW_HEIGHT - MMI_TWOLINE_MENUITEM_BOTTOM_GAP - text_1_h;
	
	//set menuitem second text positon
    set_fixed_icontext_list_text_coordinates(
		1, 
		x1, 
		y1 + text_0_h + MMI_ICONTEXT_ROW_GAP, 
		text_w, 
		text_1_h);

	//compute third text width
#ifdef __MMI_WEARABLE_DEVICE_NOTI_UI_ENHANCE__
	if (WGUI_SWATCH_TWOLINE_LIST_TYPE2 == swath_twoline_list_type) //type1
	{
		y2 = y1;
		x2 = x1 + text_w + MMI_ICONTEXT_COLUMN_GAP;
		text_w = MMI_TWOLINE_MENUITEM_RIGHT_TEXT_WIDTH;
		set_fixed_icontext_list_text_coordinates(
			2, 
			x2, 
			y2, 
		...
		//set menuitem second icon position
	    set_fixed_icontext_list_icon_coordinates(
			1, 
			x2, 
			y2 + text_0_h + MMI_ICONTEXT_ROW_GAP - 3, 
			left_icon_w/2, 
			left_icon_h/2);
	}
#endif /*__MMI_WEARABLE_DEVICE_NOTI_UI_ENHANCE__*/		
