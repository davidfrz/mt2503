...
static S32 mmi_bt_spps_get_handle_by_address(srv_bt_cm_bt_addr* bt_addr);
#endif
...
void mmi_bt_spp_init(void)
{
	srv_spp_init();
	#ifdef __IOT__
	srv_spp_register_profile(SRV_BT_CM_SPP_PROFILE_UUID);
	#endif
...
{	
	...
	#ifdef __CUSTOM_BT_SPP_UUID__
	U8 spp_custom_uuid[16]={
	                     0x97,0xa4,0x2c,0x60,
	                     0xa8,0x26,0x11,0xe4,
	                     0x8b,0x1c,0x00,0x02,
	                     0xa5,0xd5,0xc5,0x1b
...
#ifndef __CUSTOM_BT_SPP_UUID__
	   srv_spp_bind(mmi_spp_context[i].handle, SRV_BT_CM_SPP_PROFILE_UUID);
#else /*__CUSTOM_BT_SPP_UUID__*/
	   srv_spp_bind_128bit_uuid(mmi_spp_context[i].handle, spp_custom_uuid);
#endif /*__CUSTOM_BT_SPP_UUID__*/
...
void mmi_bt_spp_connect_ind(srv_spp_event_cntx* para)
{
  ...
  if(!mmi_bt_spps_is_profie_connected_ex(&dev_addr) && !srv_bt_cm_is_dev_busy(&dev_addr))
	...
	mmi_bt_spp_cntx_temp->bt_addr.lap = dev_addr.lap;
	mmi_bt_spp_cntx_temp->bt_addr.nap = dev_addr.nap;
	mmi_bt_spp_cntx_temp->bt_addr.uap = dev_addr.uap;
...
MMI_TRACE(MMI_BT_SPP_TRACE_GROUP, MMI_BT_SPP_SERVER_DISCONN, disconn_ind.para, disconn_ind.ret);
	...
	if((mmi_bt_spp_cntx_temp->conn_id == disconn_ind.para) && mmi_bt_spp_cntx_temp->is_connected)
	{
	    ...
	    */
	    ...
	    memset(&mmi_bt_spp_cntx_temp->bt_addr,0x0,sizeof(mmi_bt_spp_cntx_temp->bt_addr));
...
static S32 mmi_bt_spps_get_handle_by_address(srv_bt_cm_bt_addr* bt_addr)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_BT_SPP_TRACE_GROUP, MMI_BT_SPP_GET_HANDLE_BY_ADDR, -1);

    if(bt_addr == NULL)
		return SRV_SPP_INVALID_HANDLE;
	
	MMI_TRACE(MMI_BT_SPP_TRACE_GROUP, MMI_BT_SPP_GET_ADDR, bt_addr->lap, bt_addr->uap, bt_addr->nap);
	for (i = 0; i < MMI_ENNABLE_SRV_NUM; i++)
	{
		if(mmi_spp_context[i].bt_addr.lap == bt_addr->lap &&
		   mmi_spp_context[i].bt_addr.uap == bt_addr->uap &&
		   mmi_spp_context[i].bt_addr.nap == bt_addr->nap && (mmi_spp_context[i].handle > 0))
			{
			    MMI_TRACE(MMI_BT_SPP_TRACE_GROUP, MMI_BT_SPP_GET_HANDLE_BY_ADDR, mmi_spp_context[i].handle);
				return mmi_spp_context[i].handle;
			}
	}
	
	if(MMI_ENNABLE_SRV_NUM == i)
	  {
		MMI_TRACE(MMI_BT_SPP_TRACE_GROUP, MMI_BT_SPP_GET_NO_HANDLE);
	    return SRV_SPP_INVALID_HANDLE;
	  }
...
{ 
	...
	handle = mmi_bt_spps_get_handle_by_address(bt_addr);
