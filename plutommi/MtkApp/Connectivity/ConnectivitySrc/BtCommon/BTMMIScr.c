...
U16 ble_scr_buf[MMI_BT_SCR_BUF_SIZE];
	...
	switch (evt->evt_id)
	{
		case EVT_ID_SCREEN_LOCK_KEY_PRE_LOCK_EVT_ROUTING:
			if((g_mmi_bt_scr_cntx.flag & MMI_BT_INQUIRY) || 
				...
				(g_mmi_bt_scr_cntx.flag & MMI_BT_POWER_SWITCH) ||
				(mmi_bt_dailer_get_move_right_flag() == MMI_FALSE))
			{
...
mmi_ret mmi_bt_le_update_status_icon(mmi_event_struct *evt)
{
	switch (evt->evt_id)
	{
		case SRV_LE_CM_EVENT_CONNECT_CNF:
		case SRV_LE_CM_EVENT_DISCONNECT_CNF:
...
#ifdef __MMI_WEARABLE_DEVICE__
static mmi_ret mmi_bt_swatch_setting_group_proc(mmi_event_struct *param)
{
    return MMI_RET_OK;    
}
static void mmi_bt_swatch_hight_hdlr(S32 item_index)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    g_mmi_bt_scr_cntx.hilight_item= item_index;
}

static S32 mmi_bt_swatch_get_ItemData(S32 start_index, gui_iconlist_menu_item *menu_data, S32 num_items)
{
	U8 i,index;
	...
	for (i = 0; i < num_items; i++)
    {
		...
		if(index == 0x00)
		{
			mmi_ucs2cpy((CHAR*)menu_data[i].item_list[0], GetString(STR_BT_POWER));
			if ((status == SRV_BT_CM_POWER_ON) || (status == SRV_BT_CM_POWER_SWITCHING_OFF))
            {
				mmi_ucs2cpy((CHAR*)menu_data[i].item_list[1], GetString(STR_GLOBAL_ON));
            }
            else
            {
               mmi_ucs2cpy((CHAR*)menu_data[i].item_list[1], GetString(STR_GLOBAL_OFF));
            }
		...
		else if(index == 0x01)
		{
			...
			mmi_ucs2cpy((CHAR*)menu_data[i].item_list[0], GetString(STR_BT_DEV_SET_VIS));
...
#else /*!defined(__MMI_BT_TEMP_VISIBILITY__)*/
			{
				...
				switch (srv_bt_cm_get_visibility())
				{
					case SRV_BT_CM_VISIBILITY_ON:
						...
						mmi_ucs2cpy((CHAR*)menu_data[i].item_list[1], GetString(STR_BT_DEV_SET_VIS_PERMANENT_ON));
						break;

					case SRV_BT_CM_VISIBILITY_TEMPORARY_ON:									
						mmi_ucs2cpy((CHAR*)menu_data[i].item_list[1], GetString(STR_BT_DEV_SET_VIS_TEMPORARY_ON));
						break;
		...
		{
		    mmi_ucs2cpy((CHAR*)menu_data[i].item_list[0], GetString(STR_BT_OPPC_CONN_TEST));
		...
		#ifdef __MMI_HIDD_SUPPORT__
		{
		    mmi_ucs2cpy((CHAR*)menu_data[i].item_list[0], GetString(STR_BT_REMOTE_CONTROL));
			mmi_ucs2cpy((CHAR*)menu_data[i].item_list[1], NULL);
		}
		#ifdef __MMI_BT_APPLICATION_BQB__
		else if(index == 0x04)
...
void mmi_bt_swatch_tap_callback(mmi_tap_type_enum tap_type, S32 index)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
#ifdef __MMI_BT_APPLICATION_BQB__
	{
		srv_bt_cm_dev_struct conn_dev = {0};
		ReadRecordSlim(NVRAM_BYTE_BT_DIALER_AUTO_CONN, 1, &conn_dev, sizeof(srv_bt_cm_dev_struct));	
		if (srv_bt_cm_addr_is_nul(&conn_dev.bd_addr))
		{
			mmi_popup_display(L"Failuer, not remote device yet", MMI_EVENT_FAILURE, NULL);
			return;
		}
		#if defined (__MMI_BT_APK_SMART_INSTALL_SUPPORT__)
		 mmi_bt_send_smart_phone_apk(GRP_ID_ROOT, &conn_dev.bd_addr);
		#endif
	}
	else if(index == 0x03)
#endif
...
static void mmi_bt_swatch_delete(void)
{
...
void mmi_bt_entry_main_menu(void)
{
    ...
    g_mmi_bt_scr_cntx.main_gid = mmi_frm_group_create_ex(
		   GRP_ID_ROOT,
		   GRP_ID_BT_CM,
		   mmi_bt_swatch_setting_group_proc,
		   NULL,
		   MMI_FRM_NODE_SMART_CLOSE_FLAG);
	...
	RegisterHighlightHandler(mmi_bt_swatch_hight_hdlr);
	//wgui_fixed_list_register_get_flags_handler(mmi_phone_setup_timesync_flag);
	
	if (gui_buffer != NULL)
	{
		highlighted_item = g_mmi_bt_scr_cntx.hilight_item;
	}
	
	highlighted_item = g_mmi_bt_scr_cntx.hilight_item;
	
	wgui_swatch_two_line_menu_list_screen(
		(PU8)GetString(STR_BT_SWATCH_SETTING),
		NULL,
		NULL,
		NULL,
		NULL,
		item_num,
		mmi_bt_swatch_get_ItemData,
		highlighted_item,
		gui_buffer);
	...
	wgui_register_tap_callback(mmi_bt_swatch_tap_callback);
#endif  
...
mmi_ret mmi_bt_main_group_proc(mmi_event_struct *param)
{
    ...
    switch (param->evt_id)
    {
		...
		case EVT_ID_TOUCHSCREEN_MOVE_RIGHT:
			if(mmi_frm_scrn_get_active_id()==SCR_BT_DIALER_DISCONNECT_ASK)
			{
				return MMI_RET_OK;
			}
			else
...
void mmi_bt_inquiry_dev(void)
{
	...
	#ifdef __MMI_WEARABLE_DEVICE__
//	mmi_bt_dialer_set_move_right_flag(MMI_FALSE);
	#endif
}
...
/*#ifdef __BT_SPEAKER_SUPPORT__
	...
	if ((MMI_TRUE == srv_bt_cm_get_auto_reconn_flag(NULL)) || (srv_bt_cm_get_busy_dev() != NULL))
    {
		mmi_frm_nmgr_popup(MMI_SCENARIO_ID_DEFAULT, 
			MMI_EVENT_FAILURE, 
...
mmi_ret mmi_bt_inquiry_group_proc(mmi_event_struct *param)
{
    ...
    #ifdef __MMI_WEARABLE_DEVICE__	
				case EVT_ID_TOUCHSCREEN_MOVE_RIGHT:
			if(	mmi_frm_scrn_get_active_id() == SCR_BT_SCH_RES &&( !(g_mmi_bt_scr_cntx.flag & MMI_BT_INQUIRY)))
			{
				return MMI_RET_OK;

			}
			if(	mmi_frm_scrn_get_active_id() != SCR_BT_SCH_CANCEL_PRO)
			{
			    mmi_bt_inq_result_cancel();
...
void mmi_bt_entry_inq_result(void)
{
    ...
    wgui_cat1032_show(
        ...
        (WCHAR*)GetString(STR_BT_SCH_RESULT),
#else
		(WCHAR*)GetString(STR_BT_SCH_ING),
#endif/*__MMI_WEARABLE_DEVICE__*/
#ifndef _MMI_BT_ULTRA_SLIM_
        (PU8)GetImage(IMG_BT_SCH_TITLE),
#else
...
SetCenterSoftkeyFunction(mmi_bt_inq_cancel_bonding, KEY_EVENT_UP);
    ...
    if (!(g_mmi_bt_scr_cntx.flag & MMI_BT_INQUIRY))
    {
        ...
        wgui_stop_title_animation();
			...
			ChangeTitleString((U8*) get_string(STR_BT_SCH_RESULT_TILE)); 
			draw_title();
#endif /*__MMI_WEARABLE_DEVICE__*/
    }
}
...
void mmi_bt_entry_bond_result(void *para)
{
    ...
    if (event->result == SRV_BT_CM_RESULT_SUCCESS)
    {
        ...
        {
            MMI_TRACE(MMI_CONN_TRC_G7_BT, MMI_BT_CONN_PAIRED_FLAG, g_bt_paired_from_dialer);
            if ((srv_bt_cm_get_linked_dev_list_idx() < BT_DEVICE_LINK_NUM) && (srv_bt_cm_get_busy_dev() == NULL))
            {
				#ifdef __MMI_BT_DIALER_AUTO_RECONN_SUPPORT__
				mmi_bt_dialer_auto_reconn_reset();
                #endif/*__MMI_BT_DIALER_AUTO_RECONN_SUPPORT__*/
				...
				{
					if(!srv_bt_cm_is_headset_connected_ex() && !mmi_bt_dialer_app_is_connected_ex(&(event->dev_addr)))
					{
            ...
            else
            {
...
#endif  // __MMI_BT_AUTHORIZE__
                {
...
#ifdef __MMI_WEARABLE_DEVICE__
					SetLeftSoftkeyFunction(mmi_bt_swatch_delete, KEY_EVENT_LONG_PRESS);
#endif
#ifndef __MMI_WEARABLE_DEVICE__
...

                    {
						...
						#ifdef __MMI_WEARABLE_DEVICE__
						SetLeftSoftkeyFunction(mmi_bt_dialer_cent, KEY_EVENT_UP);
...
static S32 mmi_bt_get_mydev_item(S32 start_index, gui_iconlist_menu_item* menu_data, S32 num_items)
{
    ...
    for (i = 0; i < num_items; ++i)
    {
        ...
        if (idx == 0)
        {
			...
			#if defined(_MMI_BWUI_BT_SLIM_)|| (defined(_MMI_BT_ULTRA_SLIM_)&&!defined(__MMI_BT_DIALER_SUPPORT__))||(defined(__MMI_WEARABLE_DEVICE__))
...
#else/*__MMI_WEARABLE_DEVICE__*/
			(WCHAR*)GetString(STR_ID_BT_DIALER_TITLE),
#endif/*__MMI_WEARABLE_DEVICE__*/
            get_image(GetRootTitleIcon(MENU_CONN_BT_MAIN)),
            0,
            0,
...
#if defined(__MMI_FTE_SUPPORT__) && defined(__MMI_TOUCH_SCREEN__) && !defined (__MMI_MAINLCD_240X320__)
#ifndef _MMI_BT_ULTRA_SLIM_
//for 2503 AE, the tap and left key callback conflict. So disable when FTE on 240x320 screen.
...
void mmi_bt_entry_set_vis(void)
{
 ...
 return;
	...
	if (srv_bt_cm_get_busy_dev()!= NULL || srv_bt_cm_get_prevent_connect_flag())
    {
		mmi_bt_popup_display(
			   STR_GLOBAL_CURRENTLY_NOT_AVAILABLE,
			   MMI_EVENT_FAILURE,
...
#ifdef __MMI_WEARABLE_DEVICE__
if (mmi_frm_scrn_get_active_id() == SRC_ID_BT_SETTING)
{
...
void mmi_bt_connect_complete(void *para)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
	MMI_TRACE(MMI_CONN_TRC_G7_BT,  MMI_BT_G7_CONNECT_COMPELTED);
 
    if (event_conn->profile_id == SRV_BT_CM_ADVANCED_AUDIO_PROFILE_UUID)
    {
    #ifdef __MMI_A2DP_SUPPORT__
		MMI_TRACE(MMI_CONN_TRC_G7_BT,  MMI_BT_G7_CONNECT_COMPELTED_A2DP, event_conn->profile_id);
        if(srv_a2dp_get_role(event_conn->dev_addr) == BT_A2DP_SOURCE)
        {
            mmi_bt_dialer_set_headset_flag(MMI_TRUE, event_conn->dev_addr);
            #ifdef __MMI_BT_DIALER_AUTO_RECONN_SUPPORT__	
            mmi_bt_dialer_auto_reconn_reset();
            #endif/*__MMI_BT_DIALER_AUTO_RECONN_SUPPORT__*/

            srv_bt_cm_set_dev_link_state(event_conn->dev_addr,MMI_TRUE);
			MMI_TRACE(MMI_CONN_TRC_G7_BT,  MMI_BT_G7_CONNECT_COMPELTED_A2DP_GET_ROLE);     
        }
    #endif
    }
...
void mmi_bt_disconnect_complete(void *para)
{
    ...
    {
        if (mmi_bt_dialer_get_headset_flag(event->dev_addr))
        {
            srv_bt_cm_dev_struct* dev_p ;
            mmi_frm_scrn_close(g_mmi_bt_scr_cntx.mydev_gid, SCR_BT_DIALER_DISCONNECT_ASK);
            ...
            mmi_bt_dialer_set_headset_flag(MMI_FALSE, NULL);
            ...
            if(srv_bt_cm_get_linked_dev_list_idx() < BT_DEVICE_LINK_NUM)
            {
                srv_bt_cm_set_prevent_connect_flag(MMI_FALSE);//recovery visiable
            }
        #else /* __MMI_HFP_SUPPORT__ */
...
void mmi_bt_entry_mydev_del(void)
{
    ...
    if (srv_bt_cm_is_dev_conn_active(&(g_mmi_bt_scr_cntx.conn_dev_add)))
    {
        ...
        mmi_bt_popup_display(STR_BT_DISCONN_DEV_CONN, MMI_EVENT_FAILURE, g_mmi_bt_scr_cntx.mydev_gid);
	#else	
	    mmi_bt_popup_display(STR_BT_RELEASE_CONN_PROMPT, MMI_EVENT_FAILURE, g_mmi_bt_scr_cntx.mydev_gid);	
    #endif
    #endif
        return;
    }
...
typedef struct
{
	...
	U16 profile_id;	
}mmi_bt_entry_about_profile_struct;



const static  mmi_bt_entry_about_profile_struct g_about_table[] = {
													 {SRV_BT_CM_PBAPC_CONNECTION, SRV_BT_CM_OBEX_PBA_PROFILE_CLIENT_UUID},
													 {SRV_BT_CM_MAPC_CONNECTION, SRV_BT_CM_MESSAGE_ACCESS_PROFILE_UUID},
													 {SRV_BT_CM_HF_CONNECTION, SRV_BT_CM_HF_PROFILE_UUID},
													 {SRV_BT_CM_A2DP_CONNECTION,SRV_BT_CM_ADVANCED_AUDIO_PROFILE_UUID},
													 {SRV_BT_CM_HFP_CONNECTION,SRV_BT_CM_HFG_PROFILE_UUID},
													 {SRV_BT_CM_AVRCP_CT_CONNECTION, SRV_BT_CM_AV_REMOTE_CONTROL_TARGET_UUID},
													 {SRV_BT_CM_SPP_CONNECTION,SRV_BT_CM_SPP_PROFILE_UUID}};

													 
void mmi_bt_entry_about_for_setting(U16 *entry_buff)
{
#if defined( __MMI_VERSION_INFO__) || defined(__MMI_WEARABLE_DEVICE__)
		...
		dev_p = (srv_bt_cm_dev_struct *)srv_bt_cm_get_host_dev_info();
		mmi_wcscpy(g_mmi_bt_scr_cntx.ui_buf, (WCHAR *) GetString(STR_BT_BD_NAME));
		mmi_wcscat(g_mmi_bt_scr_cntx.ui_buf, (WCHAR *) L"\n");
		mmi_chset_utf8_to_ucs2_string(
				(U8 *)g_mmi_bt_scr_cntx.scr_buf,
				SRV_BT_CM_BD_NAME_UCS2_LEN,
		...
		mmi_wcscat(g_mmi_bt_scr_cntx.ui_buf, (WCHAR *) GetString(STR_BT_BD_ADDR));
		mmi_wcscat(g_mmi_bt_scr_cntx.ui_buf, (WCHAR *) L"\n");
	
		g_mmi_bt_scr_cntx.scr_buf[0] = 0;
		mmi_bt_get_display_bt_address(g_mmi_bt_scr_cntx.scr_buf, &(dev_p->bd_addr));
	...
	#ifdef __BT_4_0_BLE__
		{
			U8 addr[MMI_BT_SCR_BUF_SIZE];
			addr[0] = 0;
			ble_scr_buf[0] = 0;
			mmi_wcscat(g_mmi_bt_scr_cntx.ui_buf, (WCHAR *) L"LE address:\n");
			mmi_bt_get_display_bt_address(ble_scr_buf, &(dev_p->le_bd_addr));
		#if 0	
/* under construction !*/
/* under construction !*/
/* under construction !*/
...
/* under construction !*/
		#endif	   
			mmi_wcscat(g_mmi_bt_scr_cntx.ui_buf, ble_scr_buf);
			mmi_wcscat(g_mmi_bt_scr_cntx.ui_buf, (WCHAR *) L"\n");
		}
    #endif
...
/* under construction !*/
#else
	mmi_wcscat(g_mmi_bt_scr_cntx.ui_buf, (WCHAR *) L"\n");
	mmi_wcscat(g_mmi_bt_scr_cntx.ui_buf, (WCHAR *) GetString(STR_BT_CONN_SERVICE));
	mmi_wcscat(g_mmi_bt_scr_cntx.ui_buf, (WCHAR *) L"\n");

	for(i = 0;i < 7;i++)
	{
		if(srv_bt_cm_is_profile_connected((srv_bt_cm_connection_type)g_about_table[i].type))
		{
			if (mmi_bt_get_profile_str_id((U16) g_about_table[i].profile_id, &str_p) == MMI_TRUE)
			{
	...
	if(srv_bt_cm_get_existed_conn_num() == 0)
	{
		mmi_wcscat(g_mmi_bt_scr_cntx.ui_buf, (WCHAR *) GetString(STR_BT_NO_SERVICE));
		mmi_wcscat(g_mmi_bt_scr_cntx.ui_buf, (WCHAR *) L"\n");
	}
#ifdef __BT_4_0_BLE__	
	mmi_wcscat(g_mmi_bt_scr_cntx.ui_buf, (WCHAR *) L"LE connected status:\n");
	
	if(SRV_BT_CM_LINK_TYPE_LE & srv_bt_cm_get_link_type())
	{
		mmi_wcscat(g_mmi_bt_scr_cntx.ui_buf, (WCHAR *) L"Connected\n");
	}
	else
	{
		mmi_wcscat(g_mmi_bt_scr_cntx.ui_buf, (WCHAR *) L"Not connected\n");
	}
...
#endif
		mmi_wcscpy(entry_buff, g_mmi_bt_scr_cntx.ui_buf);
...
void mmi_bt_entry_authorize_ind_screen(mmi_scrn_essential_struct *info)
{
    ...
    mmi_wsprintf(
        ...
        str_p,
        g_mmi_bt_scr_cntx.scr_buf);
#if defined(__MMI_WEARABLE_DEVICE__) //__MMI_HIDD_SUPPORT__
	wgui_showcategory6333_screen(
		get_string(STR_GLOBAL_BLUETOOTH),
        get_string(STR_GLOBAL_YES),
        get_string(STR_GLOBAL_NO),
        (UI_string_type) g_mmi_bt_scr_cntx.ui_buf,
        NULL);
#else /*defined(__MMI_WEARABLE_DEVICE__)*/
    ShowCategory7Screen(
        STR_GLOBAL_BLUETOOTH,
        GetRootTitleIcon(MENU_CONN_BT_MAIN),
        STR_GLOBAL_YES,
...
void mmi_bt_show_progress_scr(mmi_id group_id, U16 scrid, U16 title, U16 context_id, FuncPtr fp)
{
    ...
    MMI_TRACE(
        ...
        context_id);
#ifdef __MMI_WEARABLE_DEVICE__	
	if((scrid == SCR_BT_CONNECTING) ||
		(scrid == SCR_BT_POWER_PRO) || 
		(scrid == SCR_BT_SCH_CANCEL_PRO) || 
		(scrid == SCR_BT_PAIR_PRO) || 
		(scrid == SCR_BT_REL_DEV_CONN_PRO))
	{
			mmi_bt_dialer_set_move_right_flag(MMI_FALSE);
	}
#endif/*__MMI_WEARABLE_DEVICE__*/
...
/*****************************************************************************
 * FUNCTION
 *  mmi_bt_entry_progress_scr_exit
 * DESCRIPTION
 *  This function is disable move flag when progressing screen ended
 * PARAMETERS
 *  void
 *
 *
...
void mmi_bt_entry_progress_scr_exit(void)
{

#ifdef __MMI_WEARABLE_DEVICE__
	//if(post_evt->evt_id == EVT_ID_SCRN_DEINIT)
	//{
	mmi_bt_dialer_set_move_right_flag(MMI_TRUE);
	//}	
#endif
...
void mmi_bt_entry_progress_scr(void)
{
    ...
    if (mmi_frm_scrn_enter(
            g_mmi_bt_scr_cntx.prog_scr.gid,
            g_mmi_bt_scr_cntx.prog_scr.scr_id,
            (FuncPtr) mmi_bt_entry_progress_scr_exit,
...
/* under construction !*/
#endif
	scrid = g_mmi_bt_scr_cntx.prog_scr.scr_id;
	if((scrid == SCR_BT_CONNECTING) ||
			(scrid == SCR_BT_POWER_PRO) || 
			(scrid == SCR_BT_SCH_CANCEL_PRO) || 
			(scrid == SCR_BT_PAIR_PRO) || 
    ...
    else
    {
        ...
        ShowCategory163Screen(
            ...
            NULL);
	#else
			ShowCategory165Screen(0,0,0,0,(UI_string_type)(g_mmi_bt_scr_cntx.prog_scr.context_str),0,NULL);
...
void mmi_bt_update_status_icon(void)
{
...
#ifdef __BT_4_0_BLE__	
	conn_type = srv_bt_cm_get_link_type();
#endif
	...
	MMI_TRACE(MMI_CONN_TRC_G7_BT, MMI_BT_G7_GEN_UPDATE_STATUS_CONN_TYPE, conn_type);	
    MMI_TRACE(
        MMI_CONN_TRC_G7_BT,
...
#ifdef __BT_4_0_BLE__	
	wgui_status_icon_bar_reset_icon_display(STATUS_ICON_BT_BLE_CON);
	wgui_status_icon_bar_reset_icon_display(STATUS_ICON_BT_DUAL_CON);
#endif	
		#endif
        ...
        pwrstatus == SRV_BT_CM_POWER_SWITCHING_OFF)
    {
        ...
        if (pwrstatus == SRV_BT_CM_POWER_ON)
        {
		 ...
		 #else
		 	if(conn_type == SRV_BT_CM_LINK_TYPE_LE)
		 	{
		 		MMI_TRACE(MMI_CONN_TRC_G7_BT, MMI_BT_G7_GEN_UPDATE_STATUS_CONN_TYPE_BLE);
				wgui_status_icon_bar_set_icon_display(STATUS_ICON_BT_BLE_CON);
			}
			else if(conn_type == SRV_BT_CM_LINK_TYPE_DUAL)
			{
				MMI_TRACE(MMI_CONN_TRC_G7_BT, MMI_BT_G7_GEN_UPDATE_STATUS_CONN_TYPE_DUAL);
				wgui_status_icon_bar_set_icon_display(STATUS_ICON_BT_DUAL_CON);

			}
			else if(conn_type == SRV_BT_CM_LINK_TYPE_EDR)
			{
	#ifndef _MMI_BWUI_BT_SLIM_
				wgui_status_icon_bar_set_icon_display(STATUS_ICON_BT_CON);
 	#endif
...
mmi_ret mmi_bt_power_off_pre_process(mmi_event_struct *evt)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
	MMI_TRACE(MMI_CONN_TRC_G7_BT, MMI_BT_G7_BT_POWER_OFF_PRE_PROCESS);
	#ifdef __MMI_A2DP_SUPPORT__
	#ifdef __BT_SPEAKER_SUPPORT__
	#ifdef __MMI_BT_SPEAKER_SUPPORT__
