...
void SetSpeechEnhancement( bool ec )
{
	...
	// TODO:  due to strange spe_table setting
	/* 
        if (l1sp.bt_on)
        {   // For BT PLC
...
#if defined(__DUAL_TALK_MODEM_SUPPORT__) && !defined(MT6252)
void l1sp_updateSpeUsrMaskWithWholeValue(uint16 newValue)
{
	...
	kal_trace( TRACE_INFO, L1SP_UPDATE_SPE_MASKS, 3, l1sp.spe_app_mask, l1sp.spe_usr_mask, 0xFFFF, newValue, orgValue);	
...
{
	bool isNeedOffOn =  (AFE61M_getCurrentVoiceSamplingRate()!= AFE61M_getVoiceSamplingRate());
	bool inSpeech = AM_IsAmInSpeechState();
	...
	kal_trace( TRACE_INFO, L1SP_CHANGE_ENH_MODE, prev_sph_mode, mode, l1sp.sph_mode );

	// if( prev_sph_mode != mode ) { // Allow quick adaptation when change mode		
		SetSpeechEnhancement( false );		
	//}
	
	if(isNeedOffOn && inSpeech) {
		kal_prompt_trace(MOD_L1SP, "AM_SpeechOff(1);"); 
		AM_SpeechOff(1);
	}
// -----
...
// -----	
	...
	if (inSpeech ) {
		if(isNeedOffOn) {
			kal_prompt_trace(MOD_L1SP, "AM_SpeechOn(0);"); 
			AM_SpeechOn(0);
		} else {
			AM_SetSRCCtrl();
		}
...
bool sp_getSpeFlag(void)
{
	return l1sp.spe_flag;
}
