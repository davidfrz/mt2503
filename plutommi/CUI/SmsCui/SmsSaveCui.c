...
static void cui_sms_save_msg_callback(srv_sms_callback_struct *callback_data)
{
    ...
    else
    {
        if (callback_data->cause == SRV_SMS_CAUSE_MEM_FULL)
            mmi_display_popup((UI_string_type) GetString(STR_GLOBAL_MEMORY_FULL), MMI_EVENT_FAILURE);
        else
...
static void cui_sms_save_msg(void)
{
    ...
    srv_sms_btmapc_set_save_index(sms_handle, g_sms_active_dev_index);
    if(mmi_sms_get_send_msg_case() == SMS_SEND_CASE_EDIT_DRAFT)
    {	
        srv_sms_btmapc_set_save_msg_id(sms_handle, g_sms_bt_cntx.curr_msg_id);
    }
