...
const static gui_sse_scenario_effect_table_struct g_gui_sse_scenario_effect_tbl [] =
{
    ...
    {GUI_SSE_SCENARIO_WEARABLE_CLOCK_SLIDE_UP, GUI_SCREEN_SWITCH_EFFECT_SLIDE_UP},
    {GUI_SSE_SCENARIO_WEARABLE_CLOCK_SLIDE_DOWN, GUI_SCREEN_SWITCH_EFFECT_SLIDE_DOWN},
#endif
...
U16 g_gui_sse_no_change_height = 0;
U16 g_gui_sse_no_change_start_y = 0;

MMI_BOOL g_gui_some_area_sse_no_change = MMI_FALSE;

 //g_gui_screen_footer_no_change_buffer[FOOTER_TEMP_DATA_BUFFER];

#ifdef __MTK_TARGET__
    #pragma arm section zidata = "DYNAMICCACHEABLEZI_NC"
#endif

  U32 g_gui_screen_no_change_buffer[NO_CHANGE_AREA_TEMP_DATA_BUFFER/4+1];
...
void gui_effect_some_area_slide_no_change_frame(MMI_BOOL no_change_flag,U16 no_change_height,U16 start_y )
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    g_gui_some_area_sse_no_change = no_change_flag;
    
    if(no_change_height > NO_CHANGE_AREA_MAX_HEGITH)
    {
        g_gui_sse_no_change_height =  NO_CHANGE_AREA_MAX_HEGITH;
    }
    else
    {
        g_gui_sse_no_change_height =  no_change_height;
    }
    if((start_y >= 0)&&(start_y < LCD_HEIGHT))
    {
        g_gui_sse_no_change_start_y = start_y;
        if((g_gui_sse_no_change_height + g_gui_sse_no_change_start_y)> LCD_HEIGHT)
        {
           g_gui_sse_no_change_height = g_gui_sse_no_change_height - ((g_gui_sse_no_change_height + g_gui_sse_no_change_start_y)- LCD_HEIGHT);
        }     
        
    }
    else        
    {
        g_gui_some_area_sse_no_change = MMI_FALSE;
    }
}
...
#ifdef   GUI_SSP_SUPPORT_FOOTER_AREA_NO_CHANGE
    if(g_gui_some_area_sse_no_change)//backup footer area 
    {   
        U8 * date_pont = 0;
        date_pont = output_buffer + (g_gui_sse_no_change_start_y*GDI_LCD_WIDTH*GUI_SCREEN_SWITCH_EFFECT_PIXEL_BYTE_SIZE);
        memcpy((U8 *)g_gui_screen_no_change_buffer, date_pont, (g_gui_sse_no_change_height*GDI_LCD_WIDTH*GUI_SCREEN_SWITCH_EFFECT_PIXEL_BYTE_SIZE));
            ...
            else
            {
                if (PhnsetGetScreenSwitchEffectStatus() == MMI_PHNSET_SSE_FULL)
                {
                    ...
                    if (dir > 0)
                    {
                        ...
                        g_gui_screen_switch_effect_type = GUI_SCREEN_SWITCH_EFFECT_SLIDE_LEFT;
						#else
                    ...
                    {
                        #if defined(__MMI_WEARABLE_DEVICE__)
                        g_gui_screen_switch_effect_type = GUI_SCREEN_SWITCH_EFFECT_SLIDE_RIGHT;
						#else
