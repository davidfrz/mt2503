...
#ifdef __MMI_BT_BTBOX_NOLCD__
mmi_bt_anti_lost_state_enum g_mmi_bt_anti_lost_state;
#endif /* __MMI_BT_BTBOX_NOLCD__ */
...
void mmi_bt_anti_lost_init(void)
{
    ...
    MMI_TRACE(MMI_CONN_TRC_G7_BT, 
        ...
        g_mmi_bt_anti_lost_cntx.curr_times[1]);
...
void mmi_bt_cm_anti_lost_hdr(void* msg)
{
    ...
    {
        anti_lost_idx = mmi_bt_anti_lost_list_get_idx(&dev_addr);
	    MMI_TRACE(MMI_CONN_TRC_G7_BT, MMI_BT_ANTILOST_LOS_IDX, anti_lost_idx);
        if (anti_lost_idx != -1)
        {
        #ifndef __MMI_BT_BTBOX_NOLCD__
    ...
    else 
    {
        ...
        if (anti_lost_idx != -1)
        {
            StopTimer(BT_ANTI_LOST_ALARM_DURATION_TIMER+anti_lost_idx);
            g_mmi_bt_anti_lost_cntx.curr_times[anti_lost_idx] = 0;
        }
    }
#endif /* __MMI_BT_BTBOX_NOLCD__ */
}
...
void mmi_bt_anti_lost_entry_indication_screen_ext(mmi_scrn_essential_struct* scr_info)
{
    ...
    MMI_TRACE(MMI_CONN_TRC_G7_BT, MMI_BT_ANTILOST_LOS_STRING, string);
...
void mmi_bt_anti_lost_clean_addr(srv_bt_cm_bt_addr* dev_addr1)
{
    ...
    for (i=0; i<BT_DEVICE_LINK_NUM; i++)
    {
        ...
        #ifdef __MMI_BT_BTBOX_NOLCD__
            g_mmi_bt_anti_lost_cntx.curr_times[i] = 0;
        #endif /* __MMI_BT_BTBOX_NOLCD__ */
...
void mmi_bt_anti_lost_add_addr(srv_bt_cm_bt_addr* mmi_bt_anti_lost_addr)
{
...
#ifdef __MMI_BT_BTBOX_NOLCD__
    if(BT_A2DP_SOURCE == srv_a2dp_get_role(mmi_bt_anti_lost_addr))
    {
    	MMI_TRACE(MMI_CONN_TRC_G7_BT, MMI_BT_ANTILOST_SINK_CONN);       
        return;
    }
#endif /* __MMI_BT_BTBOX_NOLCD__ */
    ...
    for (p = 0; p < BT_DEVICE_LINK_NUM; p++)
    {
        ...
        {
        #ifdef __MMI_BT_BTBOX_NOLCD__
            g_mmi_bt_anti_lost_ind[p].index = mmi_bt_dialer_get_index_by_dev_info(mmi_bt_anti_lost_addr);
        #else /* __MMI_BT_BTBOX_NOLCD__ */
            g_mmi_bt_anti_lost_ind[p].index = p;
        #endif /* __MMI_BT_BTBOX_NOLCD__ */
            memcpy(&g_mmi_bt_anti_lost_ind[p].mmi_bt_anti_lost_addr, mmi_bt_anti_lost_addr, sizeof(srv_bt_cm_bt_addr));
            break;
        }
    }
...
U16 mmi_bt_anti_lost_get_notify(mmi_bt_anti_lost_notify_enum type, U8 idx)
{
    ...
    switch(type)
    {
        case MMI_BT_ANTI_LOST_BTB_NOTIFY_LOST:
          #if (BT_DEVICE_LINK_NUM>1)
            if (idx == 1)
                notify_id = NOTIFY_BTB_SRC2_LOST;
            else
          #endif
        ...
        case MMI_BT_ANTI_LOST_BTB_NOTIFY_ABDISC:
          #if (BT_DEVICE_LINK_NUM>1)
            if (idx == 1)
                notify_id = NOTIFY_BTB_SRC2_ABDISCONNECT;
            else
          #endif
        ...
        case MMI_BT_ANTI_LOST_BTB_NOTIFY_RECONNECT:
          #if (BT_DEVICE_LINK_NUM>1)
            if (idx == 1)
                notify_id = NOTIFY_BTB_SRC2_RECONNECT;
            else
          #endif
        ...
        case MMI_BT_ANTI_LOST_BTB_NOTIFY_RECONNECT_FINISH:
          #if (BT_DEVICE_LINK_NUM>1)
            if (idx == 1)
                notify_id = NOTIFY_BTB_SRC2_RECONN_FINISH;
            else
          #endif
...
void mmi_bt_anti_lost_screen_close(void)
{
    ...
    for (type=MMI_BT_ANTI_LOST_BTB_NOTIFY_LOST; type<MMI_BT_ANTI_LOST_BTB_END; type++)
    {
        ...
        notify_id = mmi_bt_anti_lost_get_notify(type, 1);
        mmi_frm_input_event_notify_remove(notify_id);
    }
 ...
 *  mmi_ret
 *****************************************************************************/
U8 g_bt_anti_lost_double_click = 0;
mmi_ret mmi_bt_anti_lost_input_event_proc(mmi_event_struct *evt)
{
    ...
    switch(evt->evt_id)
    {
        ...
        case EVT_BTB_RECONN_RESTART_D:
            g_bt_anti_lost_double_click++;
            StartTimer(BTB_DOUBLE_CLICK_TIMER, 300, mmi_bt_anti_lost_double_click_hdlr);
            break;
...
static void mmi_bt_anti_lost_reconnect_finish(srv_bt_cm_bt_addr* dev_addr)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    src_idx = mmi_bt_dialer_get_index_by_dev_info(dev_addr);
    notify_id = mmi_bt_anti_lost_get_notify(MMI_BT_ANTI_LOST_BTB_NOTIFY_RECONNECT_FINISH, src_idx);
    mmi_bt_anti_lost_send_notify(notify_id);

    MMI_TRACE(MMI_CONN_TRC_G7_BT, MMI_BT_ANTILOST_RECONN_FINISH, src_idx, notify_id);
}
...
void mmi_bt_anti_lost_notify_hdlr(void* arg)
{
    ...
    /* Code Body                                                        */
    /*----------------------------------------------------------------*/
	MMI_TRACE(MMI_CONN_TRC_G7_BT, MMI_BT_ANTILOST_NOTIFY_HDLR_BY_HUGO, arg, idx);    
    ASSERT(idx<BT_DEVICE_LINK_NUM);

    g_mmi_bt_anti_lost_cntx.idx[idx] = idx;
    g_mmi_bt_anti_lost_cntx.curr_times[idx]++;
    notify_id = mmi_bt_anti_lost_get_notify(MMI_BT_ANTI_LOST_BTB_NOTIFY_LOST, g_mmi_bt_anti_lost_ind[idx].index);

    MMI_TRACE(MMI_CONN_TRC_G7_BT, MMI_BT_ANTILOST_NOTIFY_HDLR, idx, notify_id);

    if(g_mmi_bt_anti_lost_cntx.curr_times[idx] > g_mmi_bt_anti_lost_cntx.times)
    {
        g_mmi_bt_anti_lost_cntx.idx[idx] = -1;
        mmi_frm_input_event_notify_remove(notify_id);
    ...
    {
        mmi_bt_anti_lost_send_notify(notify_id);
        StartTimerEx(BT_ANTI_LOST_ALARM_DURATION_TIMER+idx, 
                MMI_BT_ANTI_LOST_NOTIFY_INTERVAL, 
                mmi_bt_anti_lost_notify_hdlr,
                &g_mmi_bt_anti_lost_cntx.idx[idx]);
    }
}
...
/*****************************************************************************
 * FUNCTION
 *  mmi_bt_anti_lost_double_click_hdlr
 * DESCRIPTION
 *  
 * PARAMETERS
 *  
 ...
 *  
 *****************************************************************************/
void mmi_bt_anti_lost_double_click_hdlr()
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                              */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                        */
    /*----------------------------------------------------------------*/
    if(g_bt_anti_lost_double_click >= 2)
        mmi_bt_dialer_auto_reconn_restart();
}
#endif /* __MMI_BT_BTBOX_NOLCD__ */
