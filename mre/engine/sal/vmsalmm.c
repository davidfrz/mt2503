...
VMINT vm_record_start(VMSTR drv_name, VMSTR dir_name, VMSTR rec_name, VMUINT8 format, VMWSTR full_rec_file_wname, vm_recorder_callback vm_record_cb)
{
	...
	if(!srv_mre_appmgr_check_current_defined_authorization(SRV_MRE_APPMGR_APP_AUTH_RECORDER))
	{
		vm_log_fatal("%s", "[Security check]record failed");
		return VM_RECORD_FAILED;
	}
	vm_log_fatal("%s", "[Security check]record success");
#endif
...
#else
	if (vm_sal_query_calling() == TRUE || vm_audio_preprocess_play(VM_RES_TYPE_AUDIO_HANDLE) == FALSE)
#endif		
		return VM_RECORD_FAILED;
	...
	case VM_FORMAT_WAV:
		...
		vm_wstrcat(full_temp_wname, (VMWSTR)VM_FILE_EXT_WAV);
		break;
	}
	_vm_remap_usr_disk((VMWSTR)full_file_wname, (VMWSTR)full_temp_wname);
