...
#define mmi_frm_malloc(size) (mmi_frm_AFM_is_full((size)) ? mmi_malloc((size)) : mmi_frm_fix_mem_alloc((size)))
 ...
 *  mmi_frm_AFM_is_full
 * DESCRIPTION
 *  This function is used to check the AFM is full
 *  if fail, allocate from control buffer
 * 
 * PARAMETERS
 *  size        : [IN]  allocate size
 * RETURNS
 *  MMI_TRUE if AFM is full, MMI_FALSE if not.
 *****************************************************************************/
extern MMI_BOOL mmi_frm_AFM_is_full(U32 size);
