...
vm_frm_pen_move_exit pen_move_exit = {0,0};
#endif
...
{
	pen_move_exit.pHandle = vm_pmng_get_current_handle();
	pen_move_exit.not_exit = 1;
}
...
static mmi_ret vm_frm_group_proc(mmi_event_struct *evt)
{
    ...
    {        	
		...
		case EVT_ID_TOUCHSCREEN_MOVE_RIGHT:
			if(pen_move_exit.pHandle == pHandle&&pen_move_exit.not_exit==1)
				return MMI_RET_ERR;
			else
			{
				vm_pmng_set_bg_ex(pHandle);
