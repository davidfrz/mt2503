...
#ifndef __MMI_TELEPHONY_SUPPORT_IGNORE_LOCAL_SMS__
#define __MMI_TELEPHONY_SUPPORT_IGNORE_LOCAL_SMS__
#endif
#endif
...
MMI_ID g_sms_cui_id = GRP_ID_INVALID;
...
#ifdef __MMI_WEARABLE_DEVICE__ 
extern void mmi_sms_bt_msg_del_confirm_scrn(void);
#endif
...
#if (defined(__MMI_TELEPHONY_SUPPORT__) && !defined(__MMI_TELEPHONY_SUPPORT_IGNORE_LOCAL_SMS__) && !defined(__MMI_WEARABLE_DEVICE__))
                                 ...
                                 mmi_sms_um_bt_ui_aprc_st_enum aprc_st_type)
{
    ...
    #ifdef __MMI_BLE_DOGP_SUPPORT__ 
    if(srv_bt_mapc_connect_type(g_sms_active_dev_index) == SRV_BT_MAPC_CONN_MAP) 
        StartTimer(MESSAGES_INPROGRESS_TIMER_ID, MMI_SMS_UM_BT_PROGRESS_TIME_OUT, mmi_sms_bt_processing_generic_time_out);
	else
        StartTimer(MESSAGES_INPROGRESS_TIMER_ID, MMI_SMS_UM_BT_PROGRESS_MAX_TIME_OUT, mmi_sms_bt_processing_generic_time_out);		
    #else
    StartTimer(MESSAGES_INPROGRESS_TIMER_ID, MMI_SMS_UM_BT_PROGRESS_TIME_OUT, mmi_sms_bt_processing_generic_time_out);
    #endif
...
void mmi_sms_bt_um_ui_aprc_263_tab_BT(mmi_scrn_essential_struct* et_data)
{
 ...
 if (g_sms_bt_aprc_enter_tab)
    ...
    kal_prompt_trace(MOD_MMI, "[SMS], mmi_sms_bt_um_ui_aprc_263_tab_BT srv_sms_btmapc_drafts_size = %d", srv_sms_btmapc_get_list_size(SRV_UM_MSG_BTMAPC_BOX_DRAFTS));
...
/* under construction !*/
    ...
    wgui_swatch_two_line_menu_list_screen(
        (PU8)get_string(MAIN_MENU_MESSAGES_TEXT),
        (PU8)GetImage(aprc->general_aprc_cntx.title_icon_id),
        (U16)aprc->general_aprc_cntx.lsk_str_id,
        (U16)aprc->general_aprc_cntx.lsk_icon_id,
        (U16)aprc->general_aprc_cntx.rsk_str_id,
        aprc->general_aprc_cntx.num_of_item,
        aprc->get_item_cb,
        aprc->general_aprc_cntx.hilite_idx,
        gui_buffer);
    ...
    RegisterHighlightHandler(mmi_sms_um_bt_ui_sh_general_folder_hilite_hdlr);

    /* register LSK, RSK hanlders */
    SetLeftSoftkeyFunction(aprc->general_aprc_cntx.key_hdlr.lsk_hdlr, KEY_EVENT_UP);
    SetCenterSoftkeyFunction(aprc->general_aprc_cntx.key_hdlr.csk_hdlr, KEY_EVENT_UP);
    SetRightSoftkeyFunction(mmi_frm_scrn_close_active_id, KEY_EVENT_UP);
    #if defined(__MMI_TOUCH_SCREEN__)
    SetLeftSoftkeyFunction(mmi_sms_bt_msg_del_confirm_scrn,KEY_EVENT_LONG_PRESS);
    //wgui_register_tap_callback(aprc->general_aprc_cntx.key_hdlr.lsk_hdlr);
...
//#ifndef __MMI_TELEPHONY_SUPPORT__
    //mmi_sms_um_bt_ui_ut_del_relative_scrn(&(scrn_cntx->child_scrn_id));
       ...
       if (scrn_cntx->screen_hdlr.is_scrn_displayed == MMI_FALSE || g_sms_bt_ui_p[g_sms_active_dev_index].mmi_need_update_list)
       {
           ...
           if(g_bt_category_error_flag == MMI_FALSE)
           {
               MMI_fixed_list_menu.n_items = num_item;
               MMI_fixed_list_menu.highlighted_item = scrn_cntx->screen_hdlr.hilite_info.index;
               kal_prompt_trace(MOD_MMI, "[SMS], Entry mmi_sms_bt_um_ui_aprc_263_tab_BT  num_item = %d scrn_cntx->screen_hdlr.hilite_info.index = %d",num_item, scrn_cntx->screen_hdlr.hilite_info.index);
           }       
       #endif
...
S32 mmi_sms_um_bt_ui_fsm_ef_req_msg_info_cb(S32 pid, srv_um_get_msg_info_result *rsp, S32 para)
{
    ...
    if(para == 0)
    {
        ...
        {
            if((g_sms_um_ui_p->cc_cntx.scrn_table[i].scrn_id != 0) && (g_sms_um_ui_p->cc_cntx.scrn_table[i].aprc.st_type == MMI_SMS_UM_BT_UI_SCRN_TYPE_BT_FOLDER))
            {
                para = g_sms_um_ui_p->cc_cntx.scrn_table[i].scrn_id;
                break;
            }
        }
    }
...
void mmi_sms_um_bt_ui_ut_del_relative_scrn(U16 *scrn_id)
{
    ...
    *scrn_id = 0;
#endif
    kal_prompt_trace(MOD_MMI, "[SMS], Entry mmi_sms_um_bt_ui_ut_del_relative_scrn scrn_id = %d", tmp_scrn_id);
    if (tmp_scrn_id > 0)
    {
