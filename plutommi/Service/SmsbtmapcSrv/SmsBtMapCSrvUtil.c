        ...
        void *user_data)
{
    ...
    kal_prompt_trace(MOD_MMI, "[SMS], Entry srv_sms_btmapc_alloc_cntx");
    ...
    if (cntx_node != NULL)
    {
        ...
        kal_prompt_trace(MOD_MMI, "[SMS], Entry srv_sms_btmapc_alloc_cntx   OslMalloc");
...
void srv_sms_btmapc_free_cntx(void* cntx)
{
    ...
    kal_prompt_trace(MOD_MMI, "[SMS], Entry srv_sms_btmapc_free_cntx");

    if (cntx_node != NULL)
    {
        cntx_node->cntx = NULL;
        kal_prompt_trace(MOD_MMI, "[SMS], Entry srv_sms_btmapc_free_cntx  OslMfree");
        OslMfree(cntx);
    }
}
 ...
 *  
 *****************************************************************************/
void srv_sms_btmapc_free_all_cntx(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    kal_prompt_trace(MOD_MMI, "[SMS], Entry srv_sms_btmapc_free_all_cntx");

    for (i = 0; i < SRV_SMS_MAX_HANDLE; i++)
    {
        if (handle_node_list[i].cntx != NULL)
        {
            cntx_node = &handle_node_list[i];
            kal_prompt_trace(MOD_MMI, "[SMS], Entry srv_sms_btmapc_free_all_cntx  OslMfree cntx_node->cntx = %x", cntx_node->cntx);
            OslMfree(cntx_node->cntx);
            cntx_node->cntx = NULL;
        }
    }	
}
...
void srv_sms_btmapc_set_save_index(SMS_HANDLE sms_handle, srv_sms_btmapc_bt_enum bt_index)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    srv_sms_btmapc_save_cntx_struct *sms_hd = (srv_sms_btmapc_save_cntx_struct*)sms_handle;

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    if (sms_hd)
    {
        sms_hd->bt_index = bt_index;
    }
}


void srv_sms_btmapc_set_save_msg_id(SMS_HANDLE sms_handle, U16 save_msg_id)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if (sms_hd)
    {
        (sms_hd->msg_node).msg_id = save_msg_id;
    }
}
...
/*****************************************************************************
 * FUNCTION
 * srv_sms_btmapc_set_send_bt_index
 * DESCRIPTION
 *  
 * PARAMETERS
 *         
 ...
 *  void
 *****************************************************************************/
void srv_sms_btmapc_set_send_bt_index(SMS_HANDLE sms_handle, srv_sms_send_bt_enum send_bt_index)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    {
        srv_sms_btmapc_send_handle_struct *send_hd = (srv_sms_btmapc_send_handle_struct*)sms_hd->hd_data;
        send_hd->send_bt_index = send_bt_index;
    }
}
