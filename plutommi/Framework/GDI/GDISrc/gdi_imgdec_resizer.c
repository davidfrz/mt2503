...
static void gdi_imgdec_resizer_put_common(gdi_imgdec_resizer_struct *resizer, gdi_color c, BOOL want_draw, U8 type)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
	...
	if(resizer->next_want_sx >= resizer->want_sx_table_end)
	{
		...
		if(resizer->dir_x>0)
			resizer->dest = ((gdi_layer_struct*)(resizer->out_layer))->buf_ptr + (resizer->want_dx1+ resizer->next_want_dy* ((gdi_layer_struct*)(resizer->out_layer))->width )*step;
		else
			resizer->dest = ((gdi_layer_struct*)(resizer->out_layer))->buf_ptr + (resizer->want_dx2+ resizer->next_want_dy* ((gdi_layer_struct*)(resizer->out_layer))->width )*step;
