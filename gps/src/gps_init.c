...
#if defined(MT2502)
#define SIM2_RST_NO_IDEX  (169)
void  mtk_gps_set_sim2_srst_power_domain(void)
{
	DCL_HANDLE handle_clk;
	PMU_CTRL_LDO_BUCK_SET_EN val;
	PMU_CTRL_LDO_BUCK_SET_VOLTAGE val1;

	handle_clk = DclPMU_Open(DCL_PMU, FLAGS_NONE);
	val1.voltage = PMU_VOLT_01_800000_V; //or PMU_VOLT_03_000000_V	 PMU_VOLT_01_800000_V
	val1.mod = VSIM2;
	DclPMU_Control(handle_clk, LDO_BUCK_SET_VOLTAGE, (DCL_CTRL_DATA_T *)&val1);

	val.enable = DCL_TRUE;
	val.mod = VSIM2;
	DclPMU_Control(handle_clk, LDO_BUCK_SET_EN, (DCL_CTRL_DATA_T *)&val);
	DclPMU_Control(handle_clk, LDO_BUCK_SET_VSIM2_GPLDO_EN, (DCL_CTRL_DATA_T *)&val);

	DclPMU_Close(handle_clk);
	kal_print_string_trace(MOD_GPS, TRACE_FUNC, "SET_POWER_DOMAIN_DONE"); 
}
#endif
/*****************************************************************************
 * FUNCTION
...
void GPS_MNL_HW_Check(void)
{
...

    {
       ...
       if (gpio.num_lpoclk != 0xFF)   // set lpo clk pin
        {
#if defined(MT2502)	
           if(gpio.num_lpoclk == SIM2_RST_NO_IDEX)
           {
               mtk_gps_set_sim2_srst_power_domain();
           }
#endif
            GPS_SetupGPIO(gpio.mode_lpoclk, gpio.num_lpoclk);
        }                
