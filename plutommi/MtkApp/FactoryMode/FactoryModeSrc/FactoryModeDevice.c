...
#if (!defined( __MMI_FM_PLUTO_6261_SLIM__) || defined(__MMI_WEARABLE_DEVICE_FACTORY_MODE__))
/*****************************************************************************
 * FUNCTION
 *  mmi_fm_set_vibrator
 * DESCRIPTION
 ...
 *  mmi_ret
 *****************************************************************************/
static mmi_ret mmi_fm_lcd_scrn_leave_proc(mmi_event_struct *evt)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
extern void mmi_fm_handle_color_set_change(void)
{
    ...
    else
    {
           ...
           ( g_fm_gourp_id, 
             ...
             return;
		  #ifdef __MMI_WEARABLE_DEVICE_FACTORY_MODE__
		     mmi_frm_scrn_set_leave_proc(g_fm_gourp_id, SCR_ID_FM_LCD_AUTO, mmi_fm_lcd_scrn_leave_proc);
		  #endif /*__MMI_WEARABLE_DEVICE_FACTORY_MODE__*/
    }
    ...
    switch (common_cntx->sequence_counter)
    {
        ...
        default:
            ...
            if (common_cntx->autoTestMode == 0)  /* Display the screen if not in autotest mode */
            {
				...
				mmi_frm_scrn_close(g_fm_gourp_id,SCR_ID_FM_LCD_AUTO);
 ...
 *  mmi_ret
 *****************************************************************************/
static mmi_ret mmi_fm_adc_scr_leave_proc(mmi_event_struct *evt)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
static void mmi_fm_update_adc_hdlr(void *inMsg)
{
   ...
   kal_prompt_trace(MOD_MMI, "mmi_fm_update_adc_hdlr vbat=%d,bat_temp=%d,vaux=%d,charge_current=%d,vcharge=%d",
   		adc_struct->vbat,
   		adc_struct->bat_temp,
   		adc_struct->vaux,
		adc_struct->charge_current,
   		adc_struct->vcharge);
...
/* under construction !*/
    ...
    kal_prompt_trace(MOD_MMI, "mmi_fm_prepare_adc_list, MenuItemNum = %d",(*MenuItemNum));
...
#define MMI_FM_LED
#if defined(__SUPPORT_LED_FEATURE__)

#if !defined(__MMI_FM_PLUTO_6261_SLIM__)
/*------------------------ LED -------------------------------*/
...
#endif /* !define(__MMI_FM_PLUTO_6261_SLIM__)*/
#endif /*__SUPPORT_LED_FEATURE__ */
...
extern void srv_mre_appmgr_launch_app_iot_cb(U16 *app_path, srv_mre_launch_app_evt_enum evt);
/*****************************************************************************
 * FUNCTION
 *  mmi_fm_handle_backlight_rsp
 * DESCRIPTION
 *  
 * PARAMETERS
 *  void
 ...
 *  void
 *****************************************************************************/
static void mmi_fm_handle_backlight_rsp(void)
{

    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    mmi_fm_backlight_struct *bl_cntx;
    
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    bl_cntx = &(g_fm_contxt->bl_contxt);
    
    bl_cntx->bl_status = KAL_TRUE;
	if(bl_cntx->count >= 3)
	{
		mmi_fm_select_backlight(NULL);
		return;
	}
    if (bl_cntx->bl == 1)
    {
        bl_cntx->bl = 0;
        FM_SetGpioReq(GPIO_DEV_LED_MAINLCD, LED_LIGHT_LEVEL1);
    }
    else
    {
        bl_cntx->bl = 1;
    ...
    StartTimer(FM_VIB_TIMER, 1000, mmi_fm_handle_backlight_rsp);
}
/*****************************************************************************
 * FUNCTION
 *  mmi_fm_select_backlight
 * DESCRIPTION
 *  It is extern entry for backlight select
 * PARAMETERS
 *  evt  [IN]
 * RETURNS
 *  void
 *****************************************************************************/
extern void mmi_fm_select_backlight(cui_menu_event_struct *evt)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if (g_fm_contxt->bl_contxt.bl_status == MMI_FALSE)
    {		
    	FM_SetGpioReq(GPIO_DEV_LED_MAINLCD, LED_LIGHT_LEVEL1);
		mmi_frm_display_dummy_screen();
		mmi_frm_scrn_close_active_id();
        StartTimer(FM_VIB_TIMER, 1000, mmi_fm_handle_backlight_rsp);
		g_fm_contxt->bl_contxt.bl_status = KAL_TRUE;
		g_fm_contxt->bl_contxt.bl = 0;
		g_fm_contxt->bl_contxt.count = 0;
    }
    else
    {
        FM_SetGpioReq(GPIO_DEV_LED_MAINLCD, srv_backlight_get_level(GPIO_DEV_LED_MAINLCD));
		g_fm_contxt->bl_contxt.bl_status = MMI_FALSE;
        StopTimer(FM_VIB_TIMER);
		g_fm_contxt->bl_contxt.count = 0;
		mmi_frm_display_dummy_screen();
		mmi_frm_scrn_close_active_id();
...
/*****************************************************************************
 * FUNCTION
 *  mmi_fm_select_wearable_lcd
 * DESCRIPTION
 *  It is extern entry for lcd select
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
extern void mmi_fm_select_wearable_lcd(cui_menu_event_struct *evt)
{
  	mmi_fm_show_color(0);
}

#ifdef __MMI_WEARABLE_DEVICE_FM_SENSOR_TEST__
/*****************************************************************************
 * FUNCTION
 *  mmi_fm_select_sensor
 * DESCRIPTION
 *  It is extern entry for sensor select
 * PARAMETERS
 *  evt  [IN]
 * RETURNS
 *  void
 *****************************************************************************/
extern void mmi_fm_select_sensor(cui_menu_event_struct *evt)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
	char key_value[] = "C:\\@BTMre\\FM_Sensor_Test.vxp";	
	U16 app_name_w[SRV_MRE_APPMGR_FILEPATH_LEN] = {0};
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
	kal_prompt_trace(MOD_MMI, "[FACTORY_MODE]mmi_fm_select_sensor enter");
	mmi_chset_convert(MMI_CHSET_UTF8, MMI_CHSET_UCS2, (char *)key_value, (char *)app_name_w,  SRV_MRE_APPMGR_FILEPATH_LEN*2);

	if ((fd = FS_Open(app_name_w, FS_OPEN_NO_DIR | FS_READ_ONLY)) < FS_NO_ERROR)
    {
        /* app is not exist */
        mmi_popup_display_simple(
    ...
    FS_Close(fd);
	ret = srv_mre_launch_app_ex(app_name_w, srv_mre_appmgr_launch_app_iot_cb);
	kal_prompt_trace(MOD_MMI, "[FACTORY_MODE]mmi_fm_select_sensor ret = %d", ret);
}

/*****************************************************************************
 * FUNCTION
 *  mmi_fm_select_Gsensor_calibration
 * DESCRIPTION
 *  It is extern entry for sensor select
 * PARAMETERS
 *  evt  [IN]
 ...
 *  void
 *****************************************************************************/
extern void mmi_fm_select_Gsensor_calibration(cui_menu_event_struct *evt)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
	/* make sure your G-sensor calibration vxp path	*/
	char key_value[] = "C:\\@BTMre\\installed\\G_Sensor_Calibration_Test.vxp";
	U16 app_name_w[SRV_MRE_APPMGR_FILEPATH_LEN] = {0};
	srv_mre_launch_app_result_enum ret ;
	FS_HANDLE fd = -1;
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
	kal_prompt_trace(MOD_MMI, "[FACTORY_MODE]mmi_fm_select_Gsensor_calibration enter");
	mmi_chset_convert(MMI_CHSET_UTF8, MMI_CHSET_UCS2, (char *)key_value, (char *)app_name_w,  SRV_MRE_APPMGR_FILEPATH_LEN*2);
    ...
    FS_Close(fd);
	ret = srv_mre_launch_app_ex(app_name_w, srv_mre_appmgr_launch_app_iot_cb);
	kal_prompt_trace(MOD_MMI, "[FACTORY_MODE]mmi_fm_select_Gsensor_calibration ret = %d", ret);
}
#endif /*__MMI_WEARABLE_DEVICE_FM_SENSOR_TEST__*/
/*****************************************************************************
 * FUNCTION
 *  mmi_fm_select_bluetooth
 * DESCRIPTION
 *  It is extern entry for BT select
 * PARAMETERS
 *   evt  [IN]
 * RETURNS
 *  void
 *****************************************************************************/
extern void mmi_fm_select_bluetooth(cui_menu_event_struct *evt)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    mmi_popup_display_simple(
    	(WCHAR*) get_string(STR_GLOBAL_NOT_SUPPORTED),
	    MMI_EVENT_FAILURE,
	    GRP_ID_ROOT,
	    NULL);
#endif /*__MMI_BT_DIALER_SUPPORT__*/
 ...
 *  void
 *****************************************************************************/
extern void mmi_fm_bluetooth_reset(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
 ...
 *  mmi_fm_select_reset
 * DESCRIPTION
 *  It is extern entry for reset select
 * PARAMETERS
 *   evt  [IN]
 * RETURNS
 *  void
 *****************************************************************************/
extern void mmi_fm_select_reset(cui_menu_event_struct *evt)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
