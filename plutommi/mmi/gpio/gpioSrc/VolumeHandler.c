...
void SetDefaultVolumeKeyHandlers(void)
{
    ...
    if (srv_ucm_bt_query_call_count(SRV_UCM_BT_INCOMING_STATE, NULL) > 0)
    {
       ...
       || srv_ucm_bt_query_call_count(SRV_UCM_BT_OUTGOING_STATE, NULL) > 0)
    {
        volume_level = mmi_vol_get_speech_vol();

    #ifdef __MMI_WEARABLE_DEVICE__
        // volume level is from 1 to 7.
        // UI level is from 0 to 6
        if (volume_level - 1 >= 0)
        {
            volume_level -= 1;
        }
...
/*****************************************************************************
 * FUNCTION
 *  EntryScrSetKeyPadVolumeLevelGroupProc
 * DESCRIPTION
 ...
 *  void
 *****************************************************************************/
mmi_ret EntryScrSetKeyPadVolumeLevelGroupProc(mmi_event_struct *evt)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
#ifdef __MMI_WEARABLE_DEVICE__
    /* close the group */
    if (mmi_frm_group_is_present(GRP_ID_VOL_HANDLER))
    {
        mmi_frm_group_close(GRP_ID_VOL_HANDLER);
    }

 //   /* Close the screen if its called again */
  //  if (SCR_KEY_TONE_VOLUME == mmi_frm_scrn_get_active_id())
  //  {
  //      mmi_frm_scrn_close_active_id();
  //  }

    cur_gid = mmi_frm_group_create_ex(GRP_ID_ROOT, GRP_ID_VOL_HANDLER, EntryScrSetKeyPadVolumeLevelGroupProc, NULL,MMI_FRM_NODE_SMART_CLOSE_FLAG);
...
void SpeechVolumeFunction(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
#if defined(__MMI_WEARABLE_DEVICE__) && defined(__MMI_HF_SUPPORT__)
    // speech level is from 1 to 7, 0 means mute, HF don't need
    // UI level is from 0 to 6
    mdi_audio_set_volume(MDI_VOLUME_SPH,(U8)MDI_AUD_VOL_MUTE((U8)(volume_level_UI+1)));
#else
    mdi_audio_set_volume(MDI_VOLUME_SPH,(U8)MDI_AUD_VOL_MUTE((U8)volume_level_UI));
#endif /* __MMI_WEARABLE_DEVICE__ && __MMI_HF_SUPPORT__ */
...
void SpeechVolumeExitFunction(void)
{
    ...
    // speech level is from 1 to 7, 0 means mute, HF don't need
    // UI level is from 0 to 6
    srv_speech_save_mode_volume((U8)MDI_AUD_VOL_MUTE((U8)(volume_level_UI+1)));
#else
