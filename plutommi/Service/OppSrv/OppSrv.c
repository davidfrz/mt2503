...
void srv_opp_event_hdlr(msg_type msg_id, void *msg)
{
    ...
    switch (msg_id)
    {
        ...
        //case MSG_ID_GOEP_DISCONNECT_IND:	//client ok
            //srv_opp_disconnect_ind_handler(msg);// client ok
           // break;								// client ok
...
S32 srv_oppc_send_begin(S32 srv_hd, srv_bt_cm_bt_addr *dst_dev, U8 *buffer, U32 buf_size)
{
	...
	{
		/*Could not send connect request, so, connected fail*/
		/*Only Call back to conn App */
		#ifdef __MMI_BT_APK_SMART_INSTALL_SUPPORT__
			srv_oppc_handle_smart_install(MMI_TRUE, MMI_FALSE);		
		#endif
        return SRV_OPP_RESULT_STATE_BUSY;
	}
...
S32 srv_oppc_send_end(S32 srv_hd) /* here AP need care conn_id or not */
{
    ...
    switch (oppc_cntx->state)
    {
        ...
        case SRV_OPC_SENDING:
			...
			if ((oppc_cntx->sending_len == 0) || (oppc_cntx->remain_put_len == 0))	// disconnect if 1st packed
			{
				SRV_OPC_STATE_TRANS(SRV_OPC_DISCONNECTING);
				srv_oppc_send_disconnect_req(oppc_cntx->goep_conn_id, MMI_FALSE);
			}
...
MMI_BOOL srv_oppc_is_connecting(void)
{
    if (srv_oppc_p->oppc_cntx.state == SRV_OPC_CONNECTING)
    {
        return MMI_TRUE;
...
static void srv_oppc_init_apk_pushed_record(void);
#endif
/*****************************************************************************
 * FUNCTION
 *  srv_oppc_init_cntx
...
static void srv_oppc_init_cntx(void)
{
    ...
    oppc_cntx->goep_conn_id = 0xff;
	#ifdef __MMI_BT_APK_SMART_INSTALL_SUPPORT__
	oppc_cntx->conn_app_cb = NULL;
	srv_oppc_init_apk_pushed_record();
	#endif
	...
	mmi_frm_set_protocol_event_handler(MSG_ID_BT_OPPC_CONNECT_CNF,srv_oppc_connect_cnf_handler,MMI_TRUE);//	
	mmi_frm_set_protocol_event_handler(MSG_ID_BT_OPPC_OPP_SUPPORTED_FORMATS_IND,srv_oppc_supported_formats_ind_handler,MMI_TRUE);//
	mmi_frm_set_protocol_event_handler(MSG_ID_BT_OPPC_PUSH_CNF,srv_oppc_push_cnf_handler,MMI_TRUE);//
	mmi_frm_set_protocol_event_handler(MSG_ID_BT_OPPC_ABORT_CNF,srv_oppc_abort_cnf_handler,MMI_TRUE);//
	mmi_frm_set_protocol_event_handler(MSG_ID_BT_OPPC_DISCONNECT_CNF,srv_oppc_disconnect_ind_handler,MMI_TRUE);
	// TODO:
	// will review later
	mmi_frm_set_protocol_event_handler(MSG_ID_BT_OPPC_TPDISCONNECT_IND,srv_oppc_disconnect_ind_handler,MMI_TRUE);	
}
 ...
 *  void
 *****************************************************************************/
static void srv_oppc_connect_cnf_handler(void *msg)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    goep_connect_cnf_struct *cnf = (goep_connect_cnf_struct*) msg;
    ...
    if (cnf->req_id != oppc_cntx->cm_conn_id)
    {
        return;
    }
    ...
    if (cnf->rsp_code == GOEP_STATUS_SUCCESS)
    {
        ...
        switch (oppc_cntx->state)
        {
            case SRV_OPC_CONNECTING:
                ...
                oppc_cntx->goep_conn_id = cnf->goep_conn_id;
                /* Store the max acceptable obex packet size */
                if (cnf->peer_mru != 0)
                {
                    oppc_cntx->opc_mtu =
                        (cnf->peer_mru < OPPC_MAX_OBEX_PACKET_LENGTH ? cnf->peer_mru : OPPC_MAX_OBEX_PACKET_LENGTH);
                }
                else
                {
            ...
            srv_bt_cm_set_dev_link_state(&oppc_cntx->dev_addr,MMI_TRUE);
#endif
    ...
    else
    {
        ...
        if (cnf->rsp_code == GOEP_STATUS_SCO_REJECT)
        {
 ...
 *  void
 *****************************************************************************/
static void srv_oppc_push_cnf_handler(void *msg)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    goep_push_cnf_struct *rsp = (goep_push_cnf_struct*) msg;
    U32 sent = 0, read = 0;
 ...
 *  void
 *****************************************************************************/
static void srv_oppc_abort_cnf_handler(void *msg)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    goep_abort_cnf_struct *goep_abort_rsp = (goep_abort_cnf_struct*) msg;
    srv_opp_snd_notify_struct snd_notify;
    srv_oppc_cntx_struct* oppc_cntx = &srv_oppc_p->oppc_cntx;
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
...
static void srv_oppc_disconnect_ind_handler(void *msg)
{
    ...
    if(srv_oppc_p->oppc_cntx.state == SRV_OPC_IDLE)
        return ;
...
static void srv_oppc_send_connect_req(U16 opp_client_max_packet_len, srv_bt_cm_bt_addr dest_addr)
{
    ...
    srv_opp_send_ilm(MSG_ID_BT_OPPC_CONNECT_REQ, (local_para_struct*)req);
}
...
static void srv_oppc_send_disconnect_req(U8 goep_conn_id, MMI_BOOL tpdisconn_flag)
{
    ...
    req->uuid_len = 0;
    if (tpdisconn_flag)
    {	// why client cannot call TPDisconnect req.
        msg_id = MSG_ID_BT_OPPC_DISCONNECT_REQ;
    }
    else
    {
        msg_id = MSG_ID_BT_OPPC_DISCONNECT_REQ;
...
static void srv_oppc_send_abort_req(U8 goep_conn_id)
{
    ...
    srv_opp_send_ilm(MSG_ID_BT_OPPC_ABORT_REQ, (local_para_struct*) goep_abort_req);
}
                ...
                U16 frag_len)
{
    ...
    srv_opp_send_ilm(MSG_ID_BT_OPPC_PUSH_REQ, (local_para_struct*) goep_push_req);
}
...
static void srv_oppc_connection_terminated(void)
{
    ...
    srv_bt_cm_stop_conn(oppc_cntx->cm_conn_id);
#ifdef __MMI_BT_DIALER_SUPPORT__
    if (srv_bt_cm_get_existed_conn_num_by_addr(&oppc_cntx->dev_addr) == 0)
    {
    srv_bt_cm_set_dev_link_state(&oppc_cntx->dev_addr,MMI_FALSE);
    }
#endif
...
// initial apk pushed record
static void srv_oppc_init_apk_pushed_record(void )
{
	static MMI_BOOL read_flag = MMI_FALSE;
	srv_oppc_cntx_struct* oppc_cntx = &srv_oppc_p->oppc_cntx;

	if (MMI_TRUE == read_flag)
	{
		return;
	}
	read_flag = MMI_TRUE;
	ReadRecordSlim(NVRAM_EF_BT_APK_SMART_INS_RECORD_LID,1,oppc_cntx->apk_push_record, NVRAM_EF_BT_APK_SMART_INS_RECORD_SIZE);
	ReadValueSlim(NVRAM_BT_OPP_PUSHED_FLAG, &oppc_cntx->apk_pushed_mask, DS_SHORT);
}

// check if pushed successfully previous
MMI_BOOL srv_oppc_apk_pushed_record_check(srv_bt_cm_bt_addr* dest_dev)
{
	srv_oppc_cntx_struct* oppc_cntx = &srv_oppc_p->oppc_cntx;
	MMI_BOOL checked_result = MMI_FALSE;
	U16 next_index = ((oppc_cntx->apk_pushed_mask>>8) & 0x00FF) + SRV_BT_CM_MAX_PAIRED_DEVICE_LIST;
	U32 pushed_number = (oppc_cntx->apk_pushed_mask & 0x00FF);
	U32 count = 0;

	while(count < pushed_number)
	{
		U32 compare_index = (--next_index) % SRV_BT_CM_MAX_PAIRED_DEVICE_LIST;
		if (srv_bt_cm_is_dev_addr_equal(dest_dev, &oppc_cntx->apk_push_record[compare_index]))
		{
			checked_result = MMI_TRUE;
			break;
		}
		++count;
	}
...
/* Record the pushed status*/
static void srv_oppc_update_push_flag_for_pair(srv_bt_cm_bt_addr* dev_addr, MMI_BOOL set_flag)
{
	srv_oppc_cntx_struct* oppc_cntx = &srv_oppc_p->oppc_cntx;
	
	MMI_BOOL pushed_check = srv_oppc_apk_pushed_record_check(dev_addr);
	U8 sp_pushed_number = oppc_cntx->apk_pushed_mask & 0x00FF;
	U16 sp_pushed_index = (oppc_cntx->apk_pushed_mask >> 8) & 0x00FF;
	S32 index = srv_bt_cm_get_dev_index((const srv_bt_cm_bt_addr *)dev_addr,SRV_BT_CM_ALL_DEV);
	MMI_BT_ASSERT(index!=-1);
	
	if (MMI_FALSE == pushed_check && MMI_TRUE == set_flag)
	{
		srv_bt_cm_copy_addr(&oppc_cntx->apk_push_record[sp_pushed_index], dev_addr);
		sp_pushed_index = (++sp_pushed_index)%SRV_BT_CM_MAX_PAIRED_DEVICE_LIST;
		if (sp_pushed_number < SRV_BT_CM_MAX_PAIRED_DEVICE_LIST)
		{
			sp_pushed_number++;
		}
	}
	else if (MMI_TRUE == pushed_check && MMI_FALSE == set_flag && sp_pushed_number > 0)
	{
		sp_pushed_index = (sp_pushed_index + SRV_BT_CM_MAX_PAIRED_DEVICE_LIST -1) % SRV_BT_CM_MAX_PAIRED_DEVICE_LIST;
		sp_pushed_number--;
		if (sp_pushed_number == 0)
		{
			sp_pushed_index = 0;
		}
		memset(&oppc_cntx->apk_push_record[sp_pushed_index], 0, sizeof(srv_bt_cm_bt_addr));
	}

	oppc_cntx->apk_pushed_mask = (sp_pushed_index << 8)|sp_pushed_number;

	WriteRecordSlim(NVRAM_EF_BT_APK_SMART_INS_RECORD_LID, 1, oppc_cntx->apk_push_record, NVRAM_EF_BT_APK_SMART_INS_RECORD_SIZE);
	WriteValueSlim(NVRAM_BT_OPP_PUSHED_FLAG, &oppc_cntx->apk_pushed_mask, DS_SHORT);
}

void srv_oppc_handle_smart_install(MMI_BOOL connect_flag, MMI_BOOL success_flag)
{
	srv_oppc_cntx_struct* oppc_cntx = &srv_oppc_p->oppc_cntx;

	kal_prompt_trace(MOD_MMI_CONN_APP, "OppSrv: srv_oppc_handle_smart_install() connect_flag=%d, success_flag=%d, call_back=0x%08x",
		connect_flag, success_flag, oppc_cntx->conn_app_cb);

	/*Ignore it if cm did not call opp*/
	if(oppc_cntx->conn_app_cb == NULL)
		return ;
	
	/*Write pushed flag for a paired device if push successfully*/
	if (!connect_flag && success_flag)
	{
		srv_oppc_update_push_flag_for_pair(&oppc_cntx->dev_addr, MMI_TRUE);

	}

	/*Call back conn app*/
	if(oppc_cntx->conn_app_cb && connect_flag)
	{
		oppc_cntx->conn_app_cb(&(oppc_cntx->dev_addr), SRV_BT_CM_OPPC_CONNECTION, success_flag, connect_flag, MMI_FALSE);
	}

	/*Clear the call back pointor*/
	if(!connect_flag || !success_flag)
	{
		srv_oppc_clear_conn_app_cb();
	}	
}

MMI_BOOL srv_oppc_set_conn_app_cb(void* conn_cb)
{
	srv_oppc_cntx_struct* oppc_cntx = &srv_oppc_p->oppc_cntx;

	if(oppc_cntx->conn_app_cb)
		return MMI_FALSE;

	oppc_cntx->conn_app_cb = conn_cb;
}

MMI_BOOL srv_oppc_clear_conn_app_cb(void)
{
	srv_oppc_cntx_struct* oppc_cntx = &srv_oppc_p->oppc_cntx;
...
/* under construction !*/
	...
	mmi_frm_set_protocol_event_handler(MSG_ID_BT_OPPS_REGISTER_SERVER_CNF,srv_opps_obex_register_server_rsp_handler,MMI_TRUE);//
	mmi_frm_set_protocol_event_handler(MSG_ID_BT_OPPS_DEREGISTER_SERVER_CNF,srv_opps_obex_deregister_server_rsp_handler,MMI_TRUE);//
	mmi_frm_set_protocol_event_handler(MSG_ID_BT_OPPS_CONNECT_IND,srv_opps_connect_ind_handler,MMI_TRUE);//
	mmi_frm_set_protocol_event_handler(MSG_ID_BT_OPPS_AUTHORIZE_IND,srv_opps_authorize_ind_hdler,MMI_TRUE);//
	mmi_frm_set_protocol_event_handler(MSG_ID_BT_OPPS_PULL_IND,srv_opps_pull_ind_handler,MMI_TRUE);//
	mmi_frm_set_protocol_event_handler(MSG_ID_BT_OPPS_PUSH_IND,srv_opps_push_ind_handler,MMI_TRUE);
	mmi_frm_set_protocol_event_handler(MSG_ID_BT_OPPS_ABORT_IND,srv_opps_abort_ind_handler,MMI_TRUE);//
	mmi_frm_set_protocol_event_handler(MSG_ID_BT_OPPS_DISCONNECT_IND,srv_opps_disconnect_ind_handler,MMI_TRUE);//
	// TODO:
	// will review later
	mmi_frm_set_protocol_event_handler(MSG_ID_BT_OPPS_TPDISCONNECT_IND,srv_opps_disconnect_ind_handler,MMI_TRUE);//	
}
...
static void srv_opps_obex_register_server_req(void)
{
    ...
    srv_opp_send_ilm(MSG_ID_BT_OPPS_REGISTER_SERVER_REQ, (local_para_struct*) req);
}
...
static void srv_opps_obex_deregister_server_req(void)
{
    ...
    srv_opp_send_ilm(MSG_ID_BT_OPPS_DEREGISTER_SERVER_REQ, (local_para_struct*) req);
}
...
static void srv_opps_authorize_ind_hdler(void *msg)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    memset(&auth_notify, 0, sizeof(auth_notify));
    if (opps_cntx->state == SRV_OPS_ACTIVE)
    {
        ...
        strncpy((S8*)auth_notify.dev_name, (S8*)ind->dev_name, MAX_DEV_NAME-1);
...
#ifdef __MMI_BT_DIALER_SUPPORT__
            srv_bt_cm_set_dev_link_state(&opps_cntx->dev_addr,MMI_TRUE);
#endif
...
static void srv_opps_send_disconnect_req(U8 goep_conn_id)
{
    ...
    srv_opp_send_ilm(MSG_ID_BT_OPPS_TPDISCONNECT_REQ, (local_para_struct*)req);
}
...
#ifdef __MMI_BT_DIALER_SUPPORT__
    if (srv_bt_cm_get_existed_conn_num_by_addr(&srv_opps_p->opps_cntx.dev_addr) == 0)
    {
	srv_bt_cm_set_dev_link_state(&srv_opps_p->opps_cntx.dev_addr,MMI_FALSE);
    }
#endif
...
static void srv_opps_authorize_rsp(U8 conn_id, U8 rsp_code)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    res = (goep_authorize_rsp_struct*)construct_local_para(sizeof(*res), TD_CTRL);
    
    res->goep_conn_id = (kal_uint8)conn_id;
    res->rsp_code = (kal_uint8)rsp_code;
    
    srv_opp_send_ilm(MSG_ID_BT_OPPS_AUTHORIZE_RSP, (local_para_struct*)res);
}
...
static void srv_opps_connect_rsp(U8 goep_conn_id, U8 rsp_code)
{
    ...
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    goep_connect_rsp_struct *res;

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    res = (goep_connect_rsp_struct*)construct_local_para(sizeof(*res), TD_CTRL);

    res->goep_conn_id = goep_conn_id;
    res->rsp_code = rsp_code;

    srv_opp_send_ilm(MSG_ID_BT_OPPS_CONNECT_RSP, (local_para_struct*)res);
}    
...
static void srv_opps_abort_rsp(U8 goep_conn_id, U8 rsp_code)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    res = (goep_abort_rsp_struct*)construct_local_para(sizeof(*res), TD_CTRL);
    
    res->goep_conn_id = goep_conn_id;
    res->rsp_code = rsp_code;

    srv_opp_send_ilm(MSG_ID_BT_OPPS_ABORT_RSP, (local_para_struct*)res);
}    
...
static void srv_opps_push_rsp(U8 goep_conn_id, U8 rsp_code)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    res = (goep_push_rsp_struct*)construct_local_para(sizeof(*res), TD_CTRL);
    
    res->goep_conn_id = goep_conn_id;
    res->rsp_code = rsp_code;

    srv_opp_send_ilm(MSG_ID_BT_OPPS_PUSH_RSP, (local_para_struct*)res);
}    
...
static void srv_opps_pull_rsp(U8 conn_id, U8 rsp_code)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    res = (goep_pull_rsp_struct*)construct_local_para(sizeof(*res), TD_CTRL);
    
    res->goep_conn_id = (kal_uint8)conn_id;
    res->rsp_code = (kal_uint8)rsp_code;

    srv_opp_send_ilm(MSG_ID_BT_OPPS_PULL_RSP,(local_para_struct*)res);
}
