...
#include "mmi_rp_app_bluetooth_def.h" // STR_BT_CONN_FAILED
#include "BTConnScr.h" // cui_bt_dialer_connecting_create
 ...
 *  
 *****************************************************************************/
static void mmi_ucm_mute_call_update(U8 call_id, MMI_BOOL mute)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    MMI_TRACE(MMI_COMMON_TRC_G5_CALL, TRC_MMI_UCM_BT_MUTE_UPDATE, call_id, mute, g_ucm_p_bt->muted_call_id);

    // add mute call id (mute)
    if (mute)
    {
        g_ucm_p_bt->muted_call_id |= (1 << (call_id-1));
    }
    // remove mute call id (unmute)
    else
    {
        g_ucm_p_bt->muted_call_id &= (~(1 << (call_id-1)));
    }
 ...
 *  
 *****************************************************************************/
static MMI_BOOL mmi_ucm_is_call_mute_bt(U8 call_id)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if ((1 << (call_id-1) & g_ucm_p_bt->muted_call_id))
    {
        result = MMI_TRUE;
    }
    else
    ...
    MMI_TRACE(MMI_COMMON_TRC_G5_CALL, TRC_MMI_UCM_BT_IS_CALL_MUTE, call_id, result);
    return result;
}
 ...
 *  
 *****************************************************************************/
static void mmi_ucm_incall_mute_action_bt(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    call_info = OslMalloc(sizeof(srv_ucm_bt_call_info_struct));
    ...
    if (srv_speech_is_phone_mute())
    {
        ...
        if (srv_ucm_bt_query_call_data(g_ucm_p_bt->hilite_call_id, call_info))
        {
            ...
            {
                conf_call_count = srv_ucm_bt_query_call_count(call_info->call_state, call_list);

                for (idx = 0; idx < conf_call_count; idx++)
                {
                    mmi_ucm_mute_call_update(call_list[idx], MMI_FALSE);
                }
            }
            else
            {
                mmi_ucm_mute_call_update(g_ucm_p_bt->hilite_call_id, MMI_FALSE);
            }
        }
    #ifdef __MMI_WEARABLE_DEVICE__
    ...
    else
    {
        ...
        if (srv_ucm_bt_query_call_data(g_ucm_p_bt->hilite_call_id, call_info))
        {
            if (call_info->multiParty)
            {
                ...
                for (idx = 0; idx < conf_call_count; idx++)
                {
                    mmi_ucm_mute_call_update(call_list[idx], MMI_TRUE);
                }
            }
            else
            {
                mmi_ucm_mute_call_update(g_ucm_p_bt->hilite_call_id, MMI_TRUE);
            }
        }
    #ifdef __MMI_WEARABLE_DEVICE__
...
void mmi_ucm_call_end_timeout_handler_bt(void)
{
    ...
    MMI_TRACE(MMI_COMMON_TRC_G5_CALL, TRC_MMI_UCM_CALL_END_TIMEOT_HANDLER, call_count, g_ucm_p_bt->is_need_accept);
...
#else
    ShowCategory63Screen((U8*) MessageString, NULL, NULL);
#endif
...
mmi_ret mmi_ucm_call_end_screen_delete_proc_bt(mmi_event_struct *evt)
{
    ...
    switch(evt->evt_id)
    {
        case EVT_ID_SCRN_DEINIT:            
        {
            ...
            if(call_end_p->disp_name != NULL)
            {
                OslMfree(call_end_p->disp_name);
                call_end_p->disp_name = NULL;
            }
            if(call_end_p != NULL)
            {
                OslMfree(call_end_p);
...
void mmi_ucm_refresh_ind_bt(S32 srv_hd)
{
    ...
    else
    {
        ...
        {
            // MMI_ASSERT(0);  remove assert, because of MAUI_03502255.
            // There is a case, 1 call (SP1) + 1 call(SP2_high light)
            // SP2 end -> forece refresh SP1 -> at same time SP1 end before AT+CLCC cnf.
            if (srv_ucm_bt_query_call_count(SRV_UCM_BT_CALL_STATE_ALL, NULL) == 0)
            {
 ...
 *  void
 *****************************************************************************/
void mmi_ucm_update_highlight_bt(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if (srv_ucm_bt_is_valid_service_hd(g_ucm_p_bt->hilite_dev) 
       && srv_ucm_bt_query_group_count_ext(g_ucm_p_bt->hilite_dev, SRV_UCM_BT_CALL_STATE_CONNECTED, NULL) > 0)
    {
        ...
        {
            if (srv_ucm_bt_query_group_count_ext(g_ucm_p_bt->hilite_dev, SRV_UCM_BT_ACTIVE_STATE, group_index) > 0)
            {
                g_ucm_p_bt->hilite_call_id = group_index[0];
            }
            else if (srv_ucm_bt_query_group_count_ext(g_ucm_p_bt->hilite_dev, SRV_UCM_BT_HOLD_STATE, group_index) > 0)
            {
                g_ucm_p_bt->hilite_call_id = group_index[0];
            }
            else
    ...
    {
        if (srv_ucm_bt_query_group_count(SRV_UCM_BT_ACTIVE_STATE, group_index) > 0)
        {
            g_ucm_p_bt->hilite_call_id = group_index[0];
        }
...
void mmi_ucm_release_ind_bt(srv_ucm_bt_release_call_ind_struct* call_info)
{
    ...
    mmi_ucm_mute_call_update(call_info->call_id, MMI_FALSE);
    ...
    memset(call_end_p, 0, sizeof(mmi_ucm_bt_call_end_struct));

    call_end_p->srv_hd = call_info->srv_hd;
    ...
    call_end_p->disp_name = OslMalloc(ENCODING_LENGTH*(SRV_UCM_BT_MAX_NUM_URI_LEN + 1));
    memcpy(call_end_p->disp_name, call_info->dis_name, ENCODING_LENGTH*(SRV_UCM_BT_MAX_NUM_URI_LEN + 1));
...
void mmi_ucm_exit_call_end_bt(mmi_scrn_essential_struct* scr_info)
{
    ...
    StopTimer(UCM_NOTIFY_DURATION_TIMER_BT);
    call_end_p = (mmi_ucm_bt_call_end_struct *)mmi_frm_scrn_get_user_data(scr_info->group_id, scr_info->scrn_id); 

    if (srv_ucm_bt_is_valid_service_hd(call_end_p->srv_hd)
      && srv_ucm_bt_query_call_count_ext(call_end_p->srv_hd, SRV_UCM_BT_CALL_STATE_CONNECTED,NULL) > 0
      && (!mmi_frm_group_is_present(GRP_ID_UCM_BT_INCALL) || !mmi_frm_scrn_is_present(GRP_ID_UCM_BT_INCALL, SCR_ID_UCM_BT_IN_CALL, MMI_FRM_NODE_ALL_FLAG)))
    {
        mmi_ucm_enter_in_call_bt();
    }
}
...
#endif /* __MMI_FTE_SUPPORT__ */
#ifdef __MMI_WEARABLE_DEVICE__
    if (IsMyTimerExist(UCM_VOL_BAR_SHOW_TIMER_BT))
    {
        StopTimer(UCM_VOL_BAR_SHOW_TIMER_BT);
...
void mmi_ucm_go_back_screen_check_bt(void)
{
    ...
    {
        mmi_ucm_bt_close_processing_scrn(GRP_ID_UCM_BT_MT);
    }
    ...
    {
        mmi_ucm_bt_close_processing_scrn(GRP_ID_UCM_BT_INCALL);

        if (mmi_frm_scrn_is_present(GRP_ID_UCM_BT_INCALL, SCR_ID_UCM_BT_DTMF_EDITOR, MMI_FRM_NODE_ALL_FLAG))
        {
...
#endif /* __MMI_UCM_BT_BTBOX_NOLCD__ */
#ifdef __MMI_WEARABLE_DEVICE__
    if (mmi_frm_group_is_present(GRP_ID_UCM_BT_HW_CNF))
    {
        mmi_frm_group_close(GRP_ID_UCM_BT_HW_CNF);
    }
#endif /* __MMI_WEARABLE_DEVICE__ */
}
...
void mmi_ucm_outgoing_call_endkey_bt(void)
{
    ...
    /*----------------------------------------------------------------*/
#ifdef __MMI_WEARABLE_DEVICE__
    if (mmi_frm_scrn_get_active_id() != SCR_ID_UCM_BT_HW_CNF)
    {
        SetOutGoingEndhandler(NULL);
        //SetOutGoingEndButtonState(0);
        // RedrawOutGoingEndButton();
        RedrawCategory17Screen(STR_ID_UCM_BT_ENDING);
    }
    mmi_frm_set_key_handler(UI_dummy_function, KEY_POWER, KEY_EVENT_DOWN);
#else
    if (mmi_frm_group_get_active_id() == GRP_ID_UCM_BT_MO)
    {
...
static void mmi_ucm_incoming_call_endkey_bt(void)
{
    ...
    mmi_ucm_stop_incoming_ringtone_bt();
#ifdef __MMI_WEARABLE_DEVICE__
    RedrawCategory17Screen(STR_ID_UCM_BT_REJECTING);
#endif /* __MMI_WEARABLE_DEVICE__ */
}
...
void mmi_ucm_incoming_call_sendkey_bt(void)
{
    ...
    else
    {
        ...
        DisableLeftSoftkey(STR_ID_UCM_BT_ANSWER, 0);
    #else
        RedrawCategory17Screen(STR_ID_UCM_BT_ACCEPTING);
    #endif /* __MMI_WEARABLE_DEVICE__ */
...
MMI_BOOL mmi_ucm_is_ucm_sg_bt(MMI_ID check_sg_id)
{
    ...
    #ifdef __MMI_WEARABLE_DEVICE__
        (GRP_ID_UCM_BT_HW_CNF == check_sg_id) ||
    #endif
...
void mmi_ucm_vol_up_down_bt(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/ 
    if(!g_ucm_p_bt->vol_ind && srv_ucm_bt_query_call_count(SRV_UCM_BT_CALL_STATE_CONNECTED, NULL) > 0)
    {
        volume_level = mmi_vol_get_speech_vol();

        MMI_TRACE(MMI_COMMON_TRC_G5_CALL, TRC_MMI_UCM_BT_VOL_ADJUST, volume_level);
        ...
        mmi_ucm_set_speaker_vol_action_bt(g_ucm_p_bt->hilite_dev, bt_volume);
    }
...
void mmi_ucm_entry_incoming_call_bt(mmi_scrn_essential_struct* scr_info)
{
    ...
    count = srv_ucm_bt_query_call_count_ext(g_ucm_p_bt->hilite_dev, SRV_UCM_BT_INCOMING_STATE, call_list);

    if (count > 0)
    {
        srv_ucm_bt_query_call_data(call_list[count-1], &call_info);
        mmi_ucm_set_hilite_call_info_bt((U8)call_list[count-1]);
    }
    ...
    ShowCategory17Screen_ext(
            ...
            guiBuffer);
    SetInComingAccepthandler(mmi_ucm_incoming_call_sendkey_bt);
    SetInComingRejecthandler(mmi_ucm_incoming_call_endkey_bt);
    SetKeyHandler(mmi_ucm_enter_hw_key_cnf_bt, KEY_POWER, KEY_EVENT_DOWN);

#ifdef __MMI_WEARABLE_DEVICE__
    if (g_ucm_p_bt->disconnecting == call_info.call_id)
    {
        RedrawCategory17Screen(STR_ID_UCM_BT_REJECTING);
...
#if defined(__MMI_FTE_SUPPORT__) && !defined(__MMI_WEARABLE_DEVICE__)
/*****************************************************************************
 * FUNCTION
...
void mmi_ucm_entry_outgoing_call_bt(mmi_scrn_essential_struct* scr_info)
{
   ...
   mmi_ucm_FTE_scrn_update_hdlr_by_lock_state_bt(mmi_ucm_get_auto_lock_state_bt());
    ...
    ShowCategory17Screen_ext(
            ...
            guiBuffer);
    SetKeyHandler(mmi_ucm_enter_hw_key_cnf_bt, KEY_POWER, KEY_EVENT_DOWN);
    SetOutGoingEndhandler(mmi_ucm_outgoing_call_endkey_bt);
    ...
    if (g_ucm_p_bt->disconnecting == call_info.call_id)
    {
        ...
        SetKeyHandler(UI_dummy_function, KEY_END, KEY_EVENT_DOWN);
        SetKeyHandler(UI_dummy_function, KEY_SEND, KEY_EVENT_DOWN);
        SetKeyHandler(UI_dummy_function, KEY_POWER, KEY_EVENT_DOWN);
    }
#endif /* __MMI_WEARABLE_DEVICE__ */
...
//    mmi_frm_input_event_disable_notify();
    ...
    if (!mmi_frm_group_is_present(GRP_ID_UCM_BT_INCALL))
    {
        ...
        StartTimer(UCM_VOL_STR_SHOW_TIMER_BT, MMI_UCM_BT_VOL_HIDE_STR_TIMER, mmi_ucm_bt_vol_str_timeout_hdlr);
    }
#endif /* __MMI_WEARABLE_DEVICE__ */

    if (curr_act_sg_id != GRP_ID_UCM_BT_INCALL && curr_act_sg_id != GRP_ID_UCM_BT_CALL_END
...
/*****************************************************************************
 * FUNCTION
 *  mmi_ucm_bt_vol_str_timeout_hdlr
 * DESCRIPTION
 *  
 * PARAMETERS
 *  
 ...
 *  MMI_RET_OK
 *****************************************************************************/
void mmi_ucm_bt_vol_str_timeout_hdlr(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
 ...
 * 
 *****************************************************************************/
static void mmi_ucm_hw_end_call_bt(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /*----------------------------------------------------------------*/
    mmi_ucm_end_single_action_bt();
    // mmi_ucm_bt_enter_processing_screen(GRP_ID_UCM_BT_HW_CNF, g_ucm_p_bt->hilite_dev);
    if (mmi_frm_group_is_present(GRP_ID_UCM_BT_HW_CNF))
    {
        mmi_frm_group_close(GRP_ID_UCM_BT_HW_CNF);
    }
 ...
 * 
 *****************************************************************************/
mmi_ret mmi_ucm_move_hd_sg_proc_bt(mmi_event_struct *evt)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
 ...
 * 
 *****************************************************************************/
mmi_ret mmi_ucm_move_close_hd_sg_proc_bt(mmi_event_struct *evt)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
 ...
 *  
 *****************************************************************************/
void mmi_ucm_exit_hw_key_cnf_screen(mmi_scrn_essential_struct* scr_info)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
 ...
 *  
 *****************************************************************************/
void mmi_ucm_entry_hw_key_cnf_screen(mmi_scrn_essential_struct* scr_info)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if (!mmi_frm_scrn_enter(
                    ...
                    (FuncPtr)mmi_ucm_exit_hw_key_cnf_screen,
                    (FuncPtr)mmi_ucm_entry_hw_key_cnf_screen,
                    MMI_FRM_FULL_SCRN))
    {
        return;
    }
    ...
    SetLeftSoftkeyFunction(mmi_ucm_hw_end_call_bt, KEY_EVENT_UP);
    SetCenterSoftkeyFunction(mmi_ucm_hw_end_call_bt, KEY_EVENT_UP);
    SetRightSoftkeyFunction(mmi_frm_scrn_close_active_id, KEY_EVENT_UP);

    SetKeyHandler(mmi_ucm_hw_end_call_bt, KEY_POWER, KEY_EVENT_DOWN);    
 ...
 *  
 *****************************************************************************/
void mmi_ucm_bt_close_hw_key_cnf_scrn(MMI_ID SG_id)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if (mmi_frm_scrn_is_present(SG_id, SCR_ID_UCM_BT_HW_CNF, MMI_FRM_NODE_ALL_FLAG))
    {
        mmi_frm_scrn_close(SG_id, SCR_ID_UCM_BT_HW_CNF);
    }
}

/*****************************************************************************
 * FUNCTION
 *  mmi_ucm_enter_hw_key_cnf_bt
 * DESCRIPTION
 *  
 * PARAMETERS
 *  
 ...
 *  
 *****************************************************************************/
void mmi_ucm_enter_hw_key_cnf_bt(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    mmi_ucm_sg_create_bt(GRP_ID_ROOT, GRP_ID_UCM_BT_HW_CNF);

    if (srv_ucm_bt_query_group_count(SRV_UCM_BT_INCOMING_STATE, NULL) > 0)
    {
        mmi_ucm_stop_incoming_ringtone_bt();

        g_ucm_p_bt->tone_type = MMI_UCM_BT_HW_KEY_BLOCK_TONE;
    }

    mmi_frm_scrn_first_enter(
        GRP_ID_UCM_BT_HW_CNF, SCR_ID_UCM_BT_HW_CNF, 
        (FuncPtr)mmi_ucm_entry_hw_key_cnf_screen, 
        (void*)NULL);
}
 ...
 *  
 *****************************************************************************/
void mmi_ucm_bt_reconnect(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    {
        link_dev_count = srv_bt_cm_get_linked_dev_list_idx();
        if (link_dev_count == 1)
        {
            cui_id = cui_bt_dialer_connecting_create(GRP_ID_UCM_SGL_CONN_CNF);

            if (cui_id != GRP_ID_INVALID)
            {
                cui_bt_dialer_connecting_run(cui_id);
                g_ucm_p_bt->single_conn_cui = cui_id;
                cm_dev_info = srv_bt_cm_get_linked_dev_info(0);
                srv_bt_dialer_single_connect_req((srv_bt_cm_bt_addr*)&cm_dev_info->bd_addr, mmi_ucm_bt_single_connect_cb);
            }
        }
        else if (link_dev_count == 0)
        {
    ...
    {
        mmi_ucm_bt_close_reconnect_cnf_scrn(GRP_ID_UCM_SGL_CONN_CNF);
    }
}

/*****************************************************************************
 * FUNCTION
 *  mmi_ucm_entry_reconnect_cnf_screen
 * DESCRIPTION
 *  
 * PARAMETERS
 *  
 ...
 *  
 *****************************************************************************/
void mmi_ucm_entry_reconnect_cnf_screen(mmi_scrn_essential_struct* scr_info)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if (!mmi_frm_scrn_enter(
                    ...
                    NULL,
                    (FuncPtr)mmi_ucm_entry_reconnect_cnf_screen,
                    MMI_FRM_FULL_SCRN))
    {
        return;
    }
    ...
    wgui_showcategory6333_screen(NULL,
                                ...
                                (UI_string_type)GetString(STR_GLOBAL_CANCEL),
                                (UI_string_type)GetString(STR_ID_UCM_BT_RECONN_CNF),
                                NULL);

    SetLeftSoftkeyFunction(mmi_ucm_bt_reconnect, KEY_EVENT_UP);
    SetCenterSoftkeyFunction(mmi_ucm_bt_reconnect, KEY_EVENT_UP);
    SetRightSoftkeyFunction(mmi_frm_scrn_close_active_id, KEY_EVENT_UP);    

    mmi_frm_scrn_set_leave_proc(scr_info->group_id, SCR_ID_UCM_SGL_CONN_CNF, NULL);
}

/*****************************************************************************
 * FUNCTION
 *  mmi_ucm_bt_close_reconnect_cnf_scrn
 * DESCRIPTION
 *  
 * PARAMETERS
 *  
 ...
 *  
 *****************************************************************************/
void mmi_ucm_bt_close_reconnect_cnf_scrn(MMI_ID SG_id)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if (mmi_frm_scrn_is_present(SG_id, SCR_ID_UCM_SGL_CONN_CNF, MMI_FRM_NODE_ALL_FLAG))
    {
        mmi_frm_scrn_close(SG_id, SCR_ID_UCM_SGL_CONN_CNF);
    }
}
 ...
 *  
 *****************************************************************************/
void mmi_ucm_bt_enter_reconnect_cnf(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    mmi_ucm_sg_create_bt(GRP_ID_ROOT, GRP_ID_UCM_SGL_CONN_CNF);

    if (SCR_ID_UCM_SGL_CONN_CNF == mmi_frm_scrn_get_active_id())
    {
    ...
    mmi_frm_scrn_first_enter(
        GRP_ID_UCM_SGL_CONN_CNF, SCR_ID_UCM_SGL_CONN_CNF, 
        (FuncPtr)mmi_ucm_entry_reconnect_cnf_screen, 
        (void*)NULL);
}
 ...
 *  
 *****************************************************************************/
void mmi_ucm_bt_single_connect_cb(srv_bt_cm_bt_addr *dev_addr, MMI_BOOL result)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_COMMON_TRC_G5_CALL, TRC_MMI_UCM_BT_SGL_CONN_CB, result);

    cui_bt_dialer_connecting_close(g_ucm_p_bt->single_conn_cui);
 ...
 *  
 *****************************************************************************/
void mmi_ucm_bt_reconn_done_bt(U32 event, void *args)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if (SRV_BT_CM_EVENT_CONNECT_RES == event)
    {
        srv_bt_cm_conn_res_struct *ind = (srv_bt_cm_conn_res_struct *)args;
 ...
 *  
 *****************************************************************************/
void mmi_ucm_dtmf_make_call_bt(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if (mmi_ucs2strlen((CHAR*)g_ucm_p_bt->disp_dtmf) > 0)
    {
        mmi_ucm_init_call_para(&make_call_para);
        make_call_para.ucs2_num_uri = (U16*)g_ucm_p_bt->disp_dtmf;
...
void mmi_ucm_entry_in_call_bt(mmi_scrn_essential_struct* scr_info)
{
    ...
    U16 active_name[SRV_UCM_BT_MAX_NUM_URI_LEN + 1];
    U16 held_name[SRV_UCM_BT_MAX_NUM_URI_LEN + 1];
#endif /* __MMI_WEARABLE_DEVICE__ */
    ...
    for (i = 0; i < group_count; i++)
    {
        ...
        if (call_info.call_state == SRV_UCM_BT_ACTIVE_STATE)
        {
            ...
            memcpy(active_name, call_info.dis_name, sizeof(active_name));
            cm_call_info[0].active_caller = (U8*)active_name;
            memcpy(&start_time , &call_info.start_time, sizeof(MYTIME));
            cm_call_info[0].active_call_duration = &start_time;
        ...
        else if (call_info.call_state == SRV_UCM_BT_HOLD_STATE)
        {
            ...
            memcpy(held_name, call_info.dis_name, sizeof(held_name));
            cm_call_info[1].held_caller = (U8*)held_name;
            if (call_info.multiParty)
            {
            ...
            {
                memcpy(&start_time , &call_info.start_time, sizeof(MYTIME));
                cm_call_info[1].active_call_duration = &start_time;
            }
        }
    }
    if (group_count == 1 && call_info.call_state == SRV_UCM_BT_HOLD_STATE)
    {
        SetInCallMuteState(2);
    }
    else
    {
        // lsk_str = (srv_speech_is_phone_mute())?(STR_ID_UCM_BT_UNMUTE):(STR_GLOBAL_MUTE);
        if (mmi_ucm_is_call_mute_bt(g_ucm_p_bt->hilite_call_id))
        {
            if (!srv_speech_is_phone_mute())
            {
                srv_speech_mute_phone();
    ...
    if (group_count > 1)
    {
        SetInCallSwaphandler(mmi_ucm_swap_action_bt);
    }
    SetKeyHandler(mmi_ucm_enter_hw_key_cnf_bt, KEY_POWER, KEY_EVENT_DOWN);
#endif /* __MMI_WEARABLE_DEVICE__ */
    ...
    if (group_count == 1 && call_info.call_state == SRV_UCM_BT_HOLD_STATE)
    {
        SetInCallMutehandler(mmi_ucm_retrieve_action_bt);
    }
    else
    {
        if (srv_bt_cm_get_bqb_test_flag())
        {
            SetInCallMutehandler(mmi_ucm_switch_audio_action_bt);
        }
        else
        {
            SetInCallMutehandler(mmi_ucm_incall_mute_action_bt);
        }
    }
	SetInCallEndhandler(mmi_ucm_in_call_endkey_bt);
#else
    ...
    switch(evt->evt_id)
    {
        ...
        case EVT_ID_TOUCHSCREEN_MOVE_RIGHT:
            if (srv_bt_cm_get_bqb_test_flag() && SCR_KEY_TONE_VOLUME != mmi_frm_scrn_get_active_id()
              && SCR_ID_UCM_BT_DTMF_EDITOR != mmi_frm_scrn_get_active_id())
            {
                SetDefaultVolumeKeyHandlers();
                EntryScrSetKeyPadVolumeLevel();
                result = MMI_RET_ERR;
            }
            else if (SCR_ID_UCM_BT_DTMF_EDITOR != mmi_frm_scrn_get_active_id()
              && !mmi_frm_scrn_is_present(GRP_ID_UCM_BT_INCALL, SCR_ID_UCM_BT_DTMF_EDITOR,MMI_FRM_NODE_ALL_FLAG))
            {
                result = MMI_RET_ERR;
            }
            break;
        ...
        case EVT_ID_TOUCHSCREEN_MOVE_LEFT:
            if (SCR_ID_UCM_BT_DTMF_EDITOR != mmi_frm_scrn_get_active_id()
           && !mmi_frm_scrn_is_present(GRP_ID_UCM_BT_INCALL, SCR_ID_UCM_BT_DTMF_EDITOR,MMI_FRM_NODE_ALL_FLAG))
            {
                mmi_ucm_pre_entry_dtmf_editor_bt();
                result = MMI_RET_ERR;
...
MMI_ID mmi_ucm_sg_create_bt(MMI_ID parent_id, MMI_ID group_id)
{
    ...
    if (MMI_FALSE == mmi_frm_group_is_present(group_id))
    {
        switch (group_id)
        {
            ...
            {
            #ifdef __MMI_WEARABLE_DEVICE__
                proc = mmi_ucm_move_hd_sg_proc_bt;
            #endif
        ...
        #ifdef __MMI_WEARABLE_DEVICE__
            case GRP_ID_UCM_BT_HW_CNF:
            case GRP_ID_UCM_SGL_CONN_CNF:
            {
                flag = MMI_FRM_NODE_SMART_CLOSE_FLAG;
                proc = mmi_ucm_move_close_hd_sg_proc_bt;
            }
            break;
        #endif
...
void mmi_ucm_bt_enter_processing_screen(MMI_ID SG_id, S32 srv_hd)
{
    ...
    mmi_ucm_bt_close_processing_scrn(SG_id); /* delete old processing screen if exists */
 ...
 *  void
 *****************************************************************************/
void mmi_ucm_bt_close_processing_scrn(MMI_ID SG_id)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if (mmi_frm_scrn_is_present(SG_id, SCR_ID_UCM_BT_PROCESSING, MMI_FRM_NODE_ALL_FLAG))
    {
        mmi_frm_scrn_close(SG_id, SCR_ID_UCM_BT_PROCESSING);
    }
}
...
void mmi_ucm_end_and_accpet_entry_confirm_bt(mmi_scrn_essential_struct* scr_info)
{
    ...
    wgui_showcategory6333_screen(NULL,
                                (UI_string_type)GetString(STR_GLOBAL_OK),
                                (UI_string_type)GetString(STR_GLOBAL_CANCEL),
                                (UI_string_type)GetString(STR_ID_UCM_BT_CALL_WILL_BE_ENDED),
                                NULL);
#else /* __MMI_WEARABLE_DEVICE__ */
    ShowCategory165Screen(
        STR_GLOBAL_YES,
 ...
 *  void
 *****************************************************************************/
void mmi_ucm_enter_dialling_bt(srv_ucm_bt_atd_act_req_struct* para)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    srv_ucm_bt_atd_act_req_struct* req;
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if (para == NULL)
    ...
    if (mmi_frm_scrn_is_present(GRP_ID_UCM_BT_MO, SCR_ID_UCM_BT_OUTGOING, MMI_FRM_NODE_ALL_FLAG))
    {    
        mmi_frm_scrn_close(GRP_ID_UCM_BT_MO, SCR_ID_UCM_BT_OUTGOING);
    } 
    ...
    req = OslMalloc(sizeof(srv_ucm_bt_atd_act_req_struct));
    memcpy(req, para, sizeof(srv_ucm_bt_atd_act_req_struct));
...
/*****************************************************************************
 * FUNCTION
 *  mmi_ucm_dialling_leave_proc_bt
 * DESCRIPTION
 *  
 * PARAMETERS
 ...
 *  mmi_ret
 *****************************************************************************/
mmi_ret mmi_ucm_dialling_leave_proc_bt(mmi_event_struct *evt)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    switch(evt->evt_id)
    {
        ...
        {
            mmi_scenario_evt_struct *scenario_evt = (mmi_scenario_evt_struct*)evt;
            srv_ucm_bt_atd_act_req_struct *dial_req = (srv_ucm_bt_atd_act_req_struct *)mmi_frm_scrn_get_user_data(scenario_evt->targ_group, scenario_evt->targ_scrn); 
            /* free memory */
            if(dial_req != NULL)
            {
                OslMfree(dial_req);
...
static void mmi_ucm_mo_processing_timeout_hdlr(void)
{
    ...
    StopTimer(UCM_MO_PROCESSING_TIMER_BT);
    if (srv_ucm_bt_query_call_count(SRV_UCM_BT_CALL_STATE_ALL, NULL) == 0
        &&  mmi_frm_group_is_present(GRP_ID_UCM_BT_MO))
    {
        mmi_popup_display_ext(STR_ID_UCM_BT_CALL_FAILED, MMI_EVENT_FAILURE, NULL);
...
*/
    ...
    dial_req = (srv_ucm_bt_atd_act_req_struct*)mmi_frm_scrn_get_user_data(scr_info->group_id, SCR_ID_UCM_BT_OUTGOING);

    if (mmi_ucs2strlen((CHAR*)dial_req->dis_name) > 0)
    {
        dis_name = (U8*)dial_req->dis_name;
    }
    else if (mmi_ucs2strlen((CHAR*)dial_req->num_uri) > 0)
    {
        dis_name = (U8*)dial_req->num_uri;
    }
    else
    {
        dis_name = NULL;
    ...
    ShowCategory17Screen(
            NULL,   /* title_id */ 
            STR_ID_UCM_BT_END_CALL,        /* left_softkey */
            NULL,                   /* left_softkey_icon */
            NULL,    /* right_softkey */
            NULL,                   /* right_softkey_icon */
            STR_ID_UCM_BT_CALLING,                   /* NotificationStringId */ 
            dis_name, /* NameOrNumber */
            NULL,                /* IP_Number */
            NULL,             /* line icon */
            IMG_ID_UCM_BT_OUTGOING,     /*default_image_id */
            IMG_ID_UCM_BT_OUTGOING,               /* resource_id */
            ...
            WGUI_CATE_MOMT_RES_TYPE_IMAGE_ID,          /* resource_type */
            NULL,                   /* repeat_count, video/swflash only, repeat_count, 0 = infinite */
            NULL,        /* is_visaul_update, video/swflash only, update to LCM or not */
            MMI_FALSE,          /* is_video_aud, video/swflash only, play video's audio */
            MMI_FALSE,         /* is_play_aud_when_start */
            NULL);
#endif
...
#ifdef __MMI_WEARABLE_DEVICE__
    SetDialingKeypadCallHandler_SW(NULL);
    SetKeyHandler(mmi_ucm_enter_hw_key_cnf_bt, KEY_POWER, KEY_EVENT_DOWN);
...
mmi_ret mmi_ucm_bk_call_key_proc_bt(mmi_event_struct *evt)
{
    ...
    if (MMI_FALSE == mmi_ucm_is_ucm_sg_bt(curr_act_sg_id))
    {
        if (srv_ucm_bt_query_call_count(SRV_UCM_BT_CALL_STATE_CONNECTED, NULL) > 0)
        {
            ...
            {
                if (srv_ucm_is_incoming_call() || mmi_ucm_get_hide_state_bt())
                {
                    return MMI_RET_OK;
                }
...
static srv_prof_ret mmi_ucm_btb_notify_callback(srv_prof_ret result)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_CONN_TRC_G7_BT, TRC_MMI_UCM_BTB_NOTIFY_CB, result);
...
void mmi_ucm_btb_send_notify(U16 notify_id)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_CONN_TRC_G7_BT, TRC_MMI_UCM_BTB_SEND_NOTIFY, notify_id);
...
mmi_ret mmi_ucm_btb_input_event_proc(mmi_event_struct *evt)
{
    ...
    switch(evt->evt_id)
    {
        case EVT_UCM_BT_BTBOX_CALL_ANSWER:
            g_ucm_bt_double_click++;
            StartTimer(UCM_DOUBLE_CLICK_TIMER, 300, mmi_ucm_btb_double_click_hdlr);
            break;
        case EVT_UCM_BT_BTBOX_CALL_SWAP:
            g_ucm_bt_double_click = 0;
        ...
        case EVT_UCM_BT_BTBOX_CALL_END:  
            ...
            else if(srv_ucm_bt_query_group_count(SRV_UCM_BT_OUTGOING_STATE, NULL))
            {
                mmi_ucm_end_single_action_bt();
            }
            else if(2 == srv_ucm_bt_query_group_count(SRV_UCM_BT_CALL_STATE_CONNECTED, NULL))
            {
                mmi_ucm_end_all_action_bt();
            }
            else if(1 == srv_ucm_bt_query_group_count(SRV_UCM_BT_CALL_STATE_CONNECTED, NULL))
            {
                mmi_ucm_end_single_action_bt();
            }
            break;
 ...
 *  
 *****************************************************************************/
void mmi_ucm_btb_double_click_hdlr()
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                              */
    /*----------------------------------------------------------------*/
