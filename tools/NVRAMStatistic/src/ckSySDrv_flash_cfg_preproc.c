 ...
 *----------------------------------------------------------*/
    ...
    int fs_emmc_base_address   = EMMC_FS_BASE_ADDRESS / 512;
        ...
        int backup_partition_base = NOR_BOOTING_NOR_DISK0_BASE_ADDRESS;
        int backup_partition_size = NOR_BOOTING_NOR_DISK0_SIZE;
        int bakcup_partition_overlap_size = FS_BKP_OVERLAP_SIZE;
    #else
...
#if defined(__SECURE_DATA_STORAGE__) // __SDS_NVRAM_SUPPORT__
   int sds_total_size = SDS_TOTAL_SIZE;
#else
