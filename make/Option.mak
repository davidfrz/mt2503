...
ifdef FUDIFFNET_ENABLE
  ifneq ($(strip $(FUDIFFNET_ENABLE)),NONE)
    ifneq ($(strip $(CARD_DOWNLOAD)),NONE)
      $(call DEP_ERR_OFFA_OR_OFFB,FUDIFFNET_ENABLE,CARD_DOWNLOAD)
      DEPENDENCY_CONFLICT = TRUE
    endif
# When turn on FUDIFFNET_ENABLE, TSRESERVEDSIZE_AUTOCONFIG & BLRESERVEDSIZE_AUTOCONFIG & ALICE_SUPPORT must be turned off
    ifeq ($(strip $(TSRESERVEDSIZE_AUTOCONFIG)),TRUE)
      $(call DEP_ERR_OFFA_OR_OFFB,FUDIFFNET_ENABLE,TSRESERVEDSIZE_AUTOCONFIG)
      DEPENDENCY_CONFLICT = TRUE
    endif
    ifeq ($(strip $(BLRESERVEDSIZE_AUTOCONFIG)),TRUE)
      $(call DEP_ERR_OFFA_OR_OFFB,FUDIFFNET_ENABLE,BLRESERVEDSIZE_AUTOCONFIG)
      DEPENDENCY_CONFLICT = TRUE
    endif
    ifeq ($(strip $(ALICE_SUPPORT)),TRUE)
      $(call DEP_ERR_OFFA_OR_OFFB,FUDIFFNET_ENABLE,ALICE_SUPPORT)
      DEPENDENCY_CONFLICT = TRUE
    endif
  endif
  ifeq ($(strip $(FUDIFFNET_ENABLE)),DIFF_IMAGE_UPDATE)
    COM_DEFS += __FUDIFFNET_ENABLE__
    COM_DEFS +=  __NOR_FDM_IN_BL__
  endif
  ifeq ($(strip $(PRODUCTION_RELEASE)),FALSE)
    COM_DEFS += __FUDIFFNET_REDBEND_ENABLE__ __FUDIFFNET_ROCK_ENABLE__
  endif
endif
