...
stack_update_sys_statistics(stack_statistics_struct *prev_stats)
{
    ...
    kal_int32 count = 0;
#endif
#if defined(DEBUG_KAL) && defined(__MTK_TARGET__)
    ...
    for (i = 0; i < KALTotalTasks; i++) {
        ...
        /*stack_size = ((TC_TCB *)task_info_g[i].task_id)->tc_stack_size;*/
        ...
        /*stack_size = ((TX_THREAD *)task_info_g[i].task_id)->tx_thread_stack_size;*/
    ...
    for (i = 0; i < KAL_MAX_NUM_HISRS; i++) {
        ...
        /*stack_size = ((TC_HCB *)kal_hisr_ptrs_g[i])->tc_stack_size;*/
        ...
        /*stack_size = ((TX_THREAD *)kal_hisr_ptrs_g[i])->tx_thread_stack_size;*/
    ...
    * to get the biggest number.*/ 
    ...
    {        
        ...
        memcpy(sys_stats.task_runtime_max_amount[i].task.name, \
            task_info_g[i].task_name_ptr, sizeof(task_name_array_s)); 
        memcpy(prev_stats->task_runtime_max_amount[i].task.name, \
            task_info_g[i].task_name_ptr, sizeof(task_name_array_s)); 
    }
