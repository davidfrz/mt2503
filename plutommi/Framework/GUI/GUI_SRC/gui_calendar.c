...
#define GDI_COLOR_MONTH_VIEW_BG       gdi_act_color_from_rgb(255, 255, 0, 126)
#endif/*__ASTER_NEW_STYLE_CHANGE_FOR_CALENDAR__*/
/*****************************************************************************
 * Static Declaration
 ****************************************************************************/
...
#else
    //frame not include title;
    c->x = GUI_CALENDAR_FRAME_START_X;//0;
    c->y = GUI_CALENDAR_FRAME_START_Y - 19;//52->33
    c->width = GUI_CALENDAR_FRAME_WIDTH;//240
	//再适当增加，把整个frame拉伸；
    c->height = GUI_CALENDAR_FRAME_HEIGHT + 55;//152->207
#endif
...
#else
    //not draw vertical_select,so the coordinates is not used for vertical_select;
    c->vertical_select.x = GUI_CALENDAR_FRAME_LINE_PIXEL;
...
#else
    //修改horizontal string layout;
    c->horizontal_string.x = c->vertical_select.x + GUI_CALENDAR_FRAME_LINE_PIXEL;
    //c->horizontal_string.width = c->width - GUI_CALENDAR_FRAME_LINE_PIXEL * 3;
    c->horizontal_string.width = c->width;//240
    if (c->vertical_select.width == 0)
    {
        c->horizontal_string.x = c->horizontal_string.x - GUI_CALENDAR_FRAME_LINE_PIXEL;
        c->horizontal_string.width = c->horizontal_string.width + GUI_CALENDAR_FRAME_LINE_PIXEL;
...
/* under construction !*/
                    ...
                    else
		    {
                        ...
                        return;
						#else
						    /* Out of date boundary, need App to back to end year */
						    result.nYear = 2030;
						    c->highlight_time = result;
							gui_calendar_adjust_date_boundary(c, &c->start_time, &c->end_time, result);
...
/* under construction !*/
                    ...
                    else
                    {
						...
						/* Out of date boundary, need App to back to start year */
                        result.nYear = 1970;
						c->highlight_time = result;
                        gui_calendar_adjust_date_boundary(c, &c->start_time, &c->end_time, result);
...
static void gui_calendar_draw_bg_frame(gui_calendar_struct* c)
{
    ...
    gui_fill_transparent_color( 
		...
		gdi_layer_clear(GDI_COLOR_MONTH_VIEW_BG);
 #endif/*__ASTER_NEW_STYLE_CHANGE_FOR_CALENDAR__*/
...
#ifdef __ASTER_NEW_STYLE_CHANGE_FOR_CALENDAR__
static void gui_calendar_draw_horizontal_bg_frame(gui_calendar_struct* c)
{
	/*----------------------------------------------------------------*/
	/* Local Variables												  */
	/*----------------------------------------------------------------*/
...
/* under construction !*/
		     ...
		     gdi_draw_solid_rect(
				calendar_left_top_x,
				calendar_left_top_y,
				calendar_right_bottom_x + 1,
				calendar_left_top_y + c->horizontal_string.height,
				GDI_COLOR_WHITE);
		  #endif
		}
#endif /* __MMI_FTE_SUPPORT__ */
             ...
             S32 height)
{
 ...
 #ifdef __ASTER_NEW_STYLE_CHANGE_FOR_CALENDAR__
    //new style; (255,0,126) BG
  #if defined(__MMI_MAINLCD_240X240__) 
	gdi_layer_set_source_key(MMI_TRUE,GDI_COLOR_TRANSPARENT);
	//gdi_layer_clear(GDI_COLOR_MONTH_VIEW_BG);
	gdi_draw_solid_rect(0,0,240,240,GDI_COLOR_MONTH_VIEW_BG);
  #elif defined(__MMI_MAINLCD_320X320__) || defined(__MMI_MAINLCD_240X320__) 
    //for 320x320;
    gdi_layer_set_source_key(MMI_TRUE,GDI_COLOR_TRANSPARENT);
	//gdi_layer_clear(GDI_COLOR_MONTH_VIEW_BG);
	gdi_draw_solid_rect(0,0,LCD_WIDTH,LCD_HEIGHT,GDI_COLOR_MONTH_VIEW_BG);
  #endif
  
	//gdi_draw_solid_rect(0,33,240,33 + 26,GDI_COLOR_WHITE);
 #endif/*__ASTER_NEW_STYLE_CHANGE_FOR_CALENDAR__*/
...
/* under construction !*/
    ...
    if ((flags & GUI_CALENDAR_CELL_CURRENT) && !(c->flags & GUI_CALENDAR_HIDE_CURRENT_BACKGROUND))
    {
        if (c->cell_array.current_bg_filler != NULL)
        {
		 ...
		 #else
		   //current day, bg hilight;
		  #ifndef __ASTER_NEW_STYLE_CHANGE_FOR_CALENDAR__
		   gdi_draw_solid_rect(
                cell_x + (cell_width - 26)/2,
                cell_y + (cell_height - 24)/2,
                cell_x + cell_width - 1 - (cell_width - 26)/2,
                cell_y + cell_height - 1 - (cell_height - 24)/2,
            	gdi_act_color_from_rgb(255,56,129,136));
		  #else
		   //new style;
			#if defined (__MMI_MAINLCD_240X240__)  
		   gdi_draw_solid_rect(
			   ...
			   gdi_draw_solid_rect(
	                cell_x + (cell_width - 26)/2,
	                cell_y + (cell_height - 24)/2 + 9,
	                cell_x + cell_width - 1 - (cell_width - 26)/2,
	                cell_y + cell_height - 1 - (cell_height - 24)/2 + 9,
	            	GDI_COLOR_WHITE);
			#elif defined (__MMI_MAINLCD_320X320__) 
			   //for 320x320;
			   gdi_draw_solid_rect(
	                cell_x + (cell_width - 26)/2 - 5,
	                cell_y + (cell_height - 24)/2 + 9,
	                cell_x + cell_width - 1 - (cell_width - 26)/2 - 5,
	                cell_y + cell_height - 1 - (cell_height - 24)/2 + 9,
	            	GDI_COLOR_WHITE);
			#endif
		  #endif/*__ASTER_NEW_STYLE_CHANGE_FOR_CALENDAR__*/
...
/* under construction !*/
        ...
        (cell_date != GUI_CALENDAR_CELL_INVALID_DATE))
    {
        ...
        if ((flags & GUI_CALENDAR_CELL_CURRENT) && !(c->flags & GUI_CALENDAR_HIDE_CURRENT_BACKGROUND))
        {
		 ...
		 #else
		    gui_set_font(&MMI_small_font);//14px;
		 #endif/*__SWATCCH_FOR_CALENDAR_MONTHLY_VIEW_SLIM__*/
        ...
        if (flags & GUI_CALENDAR_CELL_VALID)
        {
            ...
            {    
			   ...
			   #else
			    //new style;(current day text)
				gui_set_text_color(gui_color(255, 0, 126));
			   #endif/*__ASTER_NEW_STYLE_CHANGE_FOR_CALENDAR__*/
			 #endif/*__SWATCCH_FOR_CALENDAR_MONTHLY_VIEW_SLIM__*/
            }
            else
...
/* under construction !*/
        ...
        else
        {
		 ...
		 #else
		   #ifndef __ASTER_NEW_STYLE_CHANGE_FOR_CALENDAR__
		    gui_set_text_color(gui_color(58, 58, 65));
		   #else
		    //new style;(invalid day text)
		    gui_set_text_color(gui_color(255, 124, 189));
		   #endif/*__ASTER_NEW_STYLE_CHANGE_FOR_CALENDAR__*/
		 #endif/*__SWATCCH_FOR_CALENDAR_MONTHLY_VIEW_SLIM__*/
        }
...
#endif //if 0
  #ifdef __SWATCCH_FOR_CALENDAR_MONTHLY_VIEW_SLIM__
       text_x += 8;//draw text offset x + 8;
       text_y -= 8;//draw text offset y - 8;
	   text_x += 1;
	   text_y += 1;
  #endif/*__SWATCCH_FOR_CALENDAR_MONTHLY_VIEW_SLIM__*/
...
/* under construction !*/
                ...
                else
                {
				  ...
				  #ifndef __SWATCCH_FOR_CALENDAR_MONTHLY_VIEW_SLIM__
				    //remove calendar event highlighted icon;
                    if (temp.nDay == c->highlight_time.nDay)
                    {
                        //cell flag which represents this cell date is highlighted date;这天有calendar event;
                    ...
                    {
                        //cell flag which represents this cell date is current day;
...
static void gui_calendar_draw_horizontal_string(gui_calendar_struct* c)
{
    ...
    U8 weekday_str[7][4] = {"Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"};
    ...
    gui_set_font(&MMI_small_font);//14px;
  #ifndef __ASTER_NEW_STYLE_CHANGE_FOR_CALENDAR__
    gui_set_text_color(gui_color(56, 129, 136));
  #else
    //new style;
    gui_set_text_color(gui_color(255, 0, 126));
  #endif/*__ASTER_NEW_STYLE_CHANGE_FOR_CALENDAR__*/
    ...
    for(i = 0; i < c->column; i++)
    {
    ...
    #endif
        {
		 ...
		 #else
		    if(data == 0)
		    sprintf((S8*)temp_text, "%s", weekday_str[temp.DayIndex]);
			else
				wcscpy(str_cur_week, (WCHAR*)GetString(STR_GLOBAL_SUNDAY_SHORT + temp.DayIndex));
            ...
            mmi_asc_to_ucs2((S8*)string_text, temp_text);
			   ...
			   wcscpy(string_text, str_cur_week);
		 #endif	
            weekday_query.String = (U8*)string_text;
            weekday_query.n = mmi_ucs2strlen((const char*)string_text);
            weekday_query.enableTruncated = MMI_FALSE;
...
/* under construction !*/
...
#ifndef __SWATCCH_FOR_CALENDAR_MONTHLY_VIEW_SLIM__
	//remove 整个highlight cell 边框rect的动作;
    ...
    gdi_layer_lock_frame_buffer();
    //该部分操作保留，否则下面Draw title and detail失效；
    if (c->highlight_change_callback != NULL)
    {
        c->highlight_change_callback(c->type, c->highlight_time, c->start_time, c->end_time);
    }
...
void gui_calendar_show(gui_calendar_struct* c)
{
    ...
    if (c->bg_image == NULL)
    {
	    ...
	    gui_calendar_draw_horizontal_bg_frame(c);
	  #endif/*__ASTER_NEW_STYLE_CHANGE_FOR_CALENDAR__*/
#endif
    }
 ...
 #endif
    //包括prev/next icon和date string;  highlight cell rect矩形框；
    gui_calendar_highlight_cell(c);
}
