...
int fota_update_nvram = 0;
extern int read_nvram_update_flag_from_update_info(void);
...
kal_uint8 bRstMMICacheDefault = 0;
#endif
/*******************************************************
 *  Local Function
 *******************************************************/
...
static kal_bool nvram_version_conflict_resolve(kal_uint8 *sysrecord)
{
    ...
    /*****************************************************
        ...
        ******************************************************/
    nvram_util_poweron();
#if !defined(__L1_STANDALONE__) && !defined(EMPTY_MMI) && !defined(EXTERNAL_MMI) && !defined(__UPDATE_BINARY_FILE__)  
    MMICheckDiskDisplay();
#endif
...
#ifdef __FUDIFFNET_ENABLE__
#ifdef __ROCK_FOTA_SUPPORT__
	    fota_update_nvram = read_nvram_update_flag_from_update_info();

		if (fota_update_nvram)
        {
            nvram_trace(TRACE_FUNC, FUNC_NVRAM_INIT, 13, 0);
            nvram_reset_data_items(NVRAM_RESET_FACTORY, NVRAM_APP_RESERVED, 0, 0, 0);
        }
