...
SPI_HANDLE spi_open(kal_uint32 port)
{
    ...
    {  
      ...
      if (kal_query_systemInit()==KAL_FALSE)
      {
       ...
       spi_internal_handle[port].owner = MOD_NIL;   
...
SPI_RESULT spi_ioctl(SPI_HANDLE handle, SPI_IOCTL_CODE code, void* pParam)
{
	...
	case SPI_IOCTL_SET_CONFIG_PARA:
        ...
        break;	
   	case SPI_IOCTL_WRITE_AND_READ:
   		
        {
          SPI_CTRL_WRITE_AND_READE_T* pReadWriteParam = (SPI_CTRL_WRITE_AND_READE_T*) pParam;
           ASSERT(pReadWriteParam);
		    result=spi_readwrite(handle, pReadWriteParam->pu1OutData,pReadWriteParam->pu1InData, pReadWriteParam->u4DataLen,pReadWriteParam->uCount, NULL);	
