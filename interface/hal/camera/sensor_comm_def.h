...
// when we switch between night mode and normal mode.
    ...
    // the feature that comes from MED->CAL->SENSOR
    // the features that sensor may need to support include :
    // CAL_FEATURE_CAMERA_BRIGHTNESS,               // In : Out :
    ...
    // CAL_FEATURE_CAMERA_DSC_MODE,
    // CAL_FEATURE_CAMERA_AE_METERING_MODE
    // CAL_FEATURE_CAMERA_BINNING_INFO, for Raw sensor only
    // for YUV sensor (YUV sensor only)

    // for Web Camera
    // the webcam features that sensor needs to support include:
...
{
    kal_bool IsHisr;
} IMAGE_SENSOR_SET_IN_HISR_STRUCT, *P_IMAGE_SENSOR_SET_IN_HISR_STRUCT;
...
typedef struct
{
    kal_uint16	id;
    SENSOR_TYPE	type;
} SensorInfo, SENSOR_INFO_STRUCT, *P_SENSOR_INFO_STRUCT;
