...
/*------------------------------------------------*/
// below enum is for MRE user, native user should not use it
typedef enum{
	VM_DCL_PWM_START = 0x500, /*PWM device start*/
	VM_DCL_KPLED_PWM,  /*This device is used to control PWM normal function.*/
...
typedef struct
{
	kal_uint8 source_clk; // 0 means 13Mhz, 1means 32K
	kal_uint8 source_clk_div;  // 0 means 0 div, 1 means 2 div, 2 means 4 div, 3 means 8 div.
}PWM_SET_SOURCE_CLOCK_T;

typedef struct
{
	kal_uint16 counter; // 0 means 13Mhz, 1means 32K
	kal_uint16 threshold;  // 0 means 0 div, 1 means 2 div, 2 means 4 div, 3 means 8 div.
...
#define PWM_CMDS    \
   ...
   PWM_CMD_OPEN, \
   PWM_CMD_CLOSE, \
   PWM_CMD_SET_SOURCE_CLOCK, \
   PWM_CMD_SET_COUNTER_AND_THRESHOLD,
...
#define PWM_CTRLS \
   ...
   PWM_SET_SOURCE_CLOCK_T  rPWMSetClk; \
   PWM_SET_COUNTER_THRESHOLD_T rPWMSetCountThreshold;
