...
# $Log:$
#
# 09 09 2014 ray.lin-wang
# [MAUI_03503699] [WEAR][Feature patchback] Separate BIN FOTA patch back to 11CW1418SP4
#
# 09 03 2014 ray.lin-wang
# [MAUI_03503699] [IoT FOTA] Separate bin FOTA check in
# change condition of irc .
#
# 08 26 2014 amber.su
# [MAUI_03515957] [Need Patch] [Sanity Fail] Sanity Test of THOR61A_DEMO_NONE(WEAR_WATCH_240X240) Can't power on is fail
# .
#
# 08 25 2014 ray.lin-wang
# [MAUI_03503699] [IoT FOTA] Separate bin FOTA check in
# .
#
# 08 25 2014 amber.su
...
.PHONY: gendsp gendummylis update_memconfig post_memconfig dcm_debug dcm_done ubin_lib maui_lib irc
...
#with -o obj_list_gen
	...
	@if exist make\~TS_LinkError.tmp \
		(tools\make.exe -fmake\gsm2.mak -r -R -o obj_list_gen ts_config) && \
		(if ERRORLEVEL 1 exit 1)

 # mbis time probe
	@if /I "$(strip $(MBIS_EN))"  EQU "TRUE" (@perl -e "print 'T_E,$@,T,'. time . \"\n\"";>>$(MBIS_BUILD_TIME_TMP))

viva_config: sys_auto_gen maui_lib ubin_lib LINK_BIN_FILE
 # mbis time probe
	@if /I "$(strip $(MBIS_EN))"  EQU "TRUE" (@perl -e "print 'T_S,$@,T,'. time . \"\n\"";>>$(MBIS_BUILD_TIME_TMP))  
...
filesystem_config: sys_auto_gen cksysdrv bootloader cus_ini_mtd_lib ubin_lib cleanbin LINK POSTBUILD ckbinblockusage_check
else
filesystem_config: sys_auto_gen cksysdrv bootloader cus_ini_mtd_lib ubin_lib cleanbin LINK_BIN_FILE LINK_BIN_FILE_CHECK
endif
...
ifeq ($(strip $(NEED_BUILD_UPDATE_BINARY)),TRUE)
	...
	@if exist $(strip $(COMPLIBDIR))\custom.lib (del /q /f $(strip $(COMPLIBDIR))\custom.lib)
	@if exist $(strip $(OBJSDIR))\custom\*.obj (del $(strip $(OBJSDIR))\custom\*.obj)
	@(tools\make.exe -fmake\gsm2.mak -r -R XGC_AND_NOT_BOOTLOADER=FALSE custom.lib) && \
	(if ERRORLEVEL 1 exit 1)
	@if /I "$(strip $(MBIS_EN))"  EQU "TRUE" (@perl -e "print 'T_E,$@,T,'. time . \"\n\"";>>$(MBIS_BUILD_TIME_TMP))
endif
...
ifeq ($(strip $(NEED_BUILD_UPDATE_BINARY)),TRUE)
	@if /I "$(strip $(MBIS_EN))"  EQU "TRUE" (@perl -e "print 'T_S,$@,T,'. time . \"\n\"";>>$(MBIS_BUILD_TIME_TMP))
	@if exist $(strip $(COMPLIBDIR))\system_ubin.lib (del /q /f $(strip $(COMPLIBDIR))\system_ubin.lib)
...
ts_config: sys_auto_gen ts_clean tiny_system_preflow ts_libs LINK_TINY_SYSTEM gen_bl_verno gencompbld bl_preflow LINK_BL BL_POSTBUILD
 # mbis time probe
	@if /I "$(strip $(MBIS_EN))"  EQU "TRUE" (@perl -e "print 'T_S,$@,T,'. time . \"\n\"";>>$(MBIS_BUILD_TIME_TMP))  
	@if not exist $(strip $(TARGDIR))\TINYSYSTEM_DEBUG (md $(strip $(TARGDIR))\TINYSYSTEM_DEBUG)
	@if exist $(strip $(COMPLOGDIR))\auto_adjust_mem.log \
		move /y $(strip $(COMPLOGDIR))\auto_adjust_mem.log $(strip $(TARGDIR))\TINYSYSTEM_DEBUG\auto_adjust_mem.log > nul
	@echo .
	@echo ======== Two-Phase Linking Process Finished ========
	@echo Please refer to $(strip $(TARGDIR))\TINYSYSTEM_DEBUG\auto_adjust_mem.log for more details of two-phase linking
	@echo .
 # mbis time probe
	@if /I "$(strip $(MBIS_EN))"  EQU "TRUE" (@perl -e "print 'T_E,$@,T,'. time . \"\n\"";>>$(MBIS_BUILD_TIME_TMP))
# *************************************************************************
...
irc:
# invalid reference checker
	@if /I "$(strip $(MBIS_EN))"  EQU "TRUE" (@perl -e "print 'T_S,$@,T,'. time . \"\n\"";>>$(MBIS_BUILD_TIME_TMP))

	@echo irc check...
	@if exist $(FIXPATH)\tools\IOTFota\irc.exe \
	($(FIXPATH)\tools\IOTFota\irc.exe $(strip $(TARGDIR))\$(strip $(LIS_FILE)) make\ubin_Option.mak tools\IOTFota\irc_excluded.txt>$(strip $(COMPLOGDIR))\irc_exe.log 2>&1) & \
	(if ERRORLEVEL 1 \
		(echo Error: irc.exe Failed. Please check $(strip $(COMPLOGDIR))\irc_exe.log & exit 1) \
	else \
		(move ".\irc.log" "$(strip $(COMPLOGDIR))\irc.log"))
