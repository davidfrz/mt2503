...
# Constants
#****************************************************************************
my $FILEINFOPARSER_VERNO     = " v0.12 -> p0.13";
                               # p0.13, 2014/05/19, Parse project makefile 'Project specified preprocessor definitions'
...
sub Parse_MAKEFILE
{
    ...
    if(-e $strMAKEFILE_Path)
    {
        ...
        while (<FILE_HANDLE>) 
        {
            ...
            elsif(/^CUSTOM_OPTION\s*\+=\s*(.+)/)
            {
                #print "[Parse_MAKEFILE]$_\n";
                my $strPreDefinition = $1;
                my @preDefinition = split(/\s+/,$strPreDefinition);
                #print "@preDefinition\n";
                foreach my $idx (0..$#preDefinition)
                {
                    my $item = $preDefinition[$idx];
                    if($item =~ /(.+)\s*=\s*(.+)/)
                    {
                        $Info_href->{$1} = $2;
                    }
