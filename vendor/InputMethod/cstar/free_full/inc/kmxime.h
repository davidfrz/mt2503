...
extern const void* kmx_ldb_pinyin_cizu;
extern const void* kmx_ldb_pinyin_danzi;
extern const void* kmx_ldb_pinyin_cizu_big5;

extern const void* kmx_ldb_zhuyin_cizu;
extern const void* kmx_ldb_zhuyin_danzi;
extern const void* kmx_ldb_zhuyin_cizu_gb18030;
...
extern const void* kmx_ldb_icelandic;
...
typedef struct
{
    int                             engine[KMX_IME_ENGINE_SIZE / sizeof(int)];
} KMX_IME_ENGINE;
...
#define KMX_MAX_PINYIN_INPUT        ((KMX_MAX_CHAR_PINYIN + 1) * KMX_MAX_PINYIN_WORD_LENGTH)
...
#define KMX_MAX_STROKE_LENGTH       64
#define KMX_MAX_STROKE_INPUT_LENGTH KMX_MAX_STROKE_LENGTH

typedef struct
{
    unsigned short                  table[KMX_MAX_WORD_MATCH];
    unsigned short                  count;
    unsigned short                  more_available;
...
int kmx_chinese_check_association(
    const KMX_IME_ENGINE_INIT*      init_info,
    unsigned short                  char_code,
    unsigned short*                 buffer,
    int                             number);
...
int kmx_latin_query_max_word_length(
    const void*                     database);

int kmx_latin_set_letter_group(
    KMX_IME_ENGINE*                 engine,
    const unsigned short*           letter_group);

int kmx_latin_is_group_letter(
    KMX_IME_ENGINE*                 engine,
    unsigned short                  chr);
    
int kmx_latin_is_letter_group_enabled(KMX_IME_ENGINE* engine);
...
int kmx_is_valid_bengali_follow_char_ex(
    const unsigned short*       context,
    unsigned short              length,
    unsigned short              c);

int kmx_is_bengali_multigraph_char(
    unsigned short              c);

const unsigned short* kmx_get_bengali_multigraph_candidate(
    unsigned short              c);
...
int kmx_is_valid_myanmar_follow_char_ex(
    const unsigned short*       context,
