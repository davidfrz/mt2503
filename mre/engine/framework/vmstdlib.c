...
{
    if(strcmp(func_name,"vm_log_via_bt") == 0)
        return (VMINT)vm_log_via_bt;    
#ifdef __MRE_CORE_DCL__
    if(strcmp(func_name,"vm_dcl_open") == 0)
        return (VMINT)vm_dcl_open;        
    if(strcmp(func_name,"vm_dcl_control") == 0)
        return (VMINT)vm_dcl_control;        
    if(strcmp(func_name,"vm_dcl_close") == 0)
        return (VMINT)vm_dcl_close;        
    if(strcmp(func_name,"vm_dcl_registercallback") == 0)
        return (VMINT)vm_dcl_registercallback;        
    if(strcmp(func_name,"vm_dcl_read") == 0)
        return (VMINT)vm_dcl_read;        
    if(strcmp(func_name,"vm_dcl_write") == 0)
        return (VMINT)vm_dcl_write; 
    if(strcmp(func_name,"vm_dcl_get_ownerid") == 0)
        return (VMINT)vm_dcl_get_ownerid; 
    if(strcmp(func_name,"vm_irq_mask") == 0)
        return (VMINT)vm_irq_mask; 
    if(strcmp(func_name,"vm_irq_restore") == 0)
        return (VMINT)vm_irq_restore; 
#endif
    if(strcmp(func_name,"vm_mode_block") == 0)
        return (VMINT)vm_mode_block;
    if(strcmp(func_name,"vm_mode_unblock") == 0)
        return (VMINT)vm_mode_unblock;        
    if(strcmp(func_name,"vm_nvram_get_data") == 0)
        return (VMINT)vm_nvram_get_data;
    if(strcmp(func_name,"vm_nvram_set_data") == 0)
        return (VMINT)vm_nvram_set_data;     
	
#ifdef __MRE_CORE_THREAD__
    if(strcmp(func_name,"vm_thread_create") == 0)
        return (VMINT)vm_thread_create;        
    if(strcmp(func_name,"vm_thread_get_current_handle") == 0)
        return (VMINT)vm_thread_get_current_handle;        
    if(strcmp(func_name,"vm_thread_get_main_handle") == 0)
        return (VMINT)vm_thread_get_main_handle;        
    if(strcmp(func_name,"vm_thread_send_msg") == 0)
        return (VMINT)vm_thread_send_msg;        
    if(strcmp(func_name,"vm_thread_get_msg") == 0)
        return (VMINT)vm_thread_get_msg;    
	
    if(strcmp(func_name,"vm_create_thread") == 0)
        return (VMINT)vm_thread_create;        
    if(strcmp(func_name,"vm_get_current_thread_handle") == 0)
        return (VMINT)vm_thread_get_current_handle;        
    if(strcmp(func_name,"vm_get_main_thread_handle") == 0)
        return (VMINT)vm_thread_get_main_handle;        
    if(strcmp(func_name,"vm_send_msg_to_thread") == 0)
        return (VMINT)vm_thread_send_msg;        
    if(strcmp(func_name,"vm_get_thread_message") == 0)
        return (VMINT)vm_thread_get_msg;  
	
    if(strcmp(func_name,"vm_signal_init") == 0)
        return (VMINT)vm_signal_init;
    if(strcmp(func_name,"vm_signal_clean") == 0)
        return (VMINT)vm_signal_clean;
    if(strcmp(func_name,"vm_signal_post") == 0)
        return (VMINT)vm_signal_post;
    if(strcmp(func_name,"vm_signal_wait") == 0)
        return (VMINT)vm_signal_wait;
    if(strcmp(func_name,"vm_signal_timedwait") == 0)
        return (VMINT)vm_signal_timedwait;
    if(strcmp(func_name,"vm_signal_deinit") == 0)
        return (VMINT)vm_signal_deinit;
    if(strcmp(func_name,"vm_mutex_create") == 0)
        return (VMINT)vm_mutex_create;
    if(strcmp(func_name,"vm_mutex_lock") == 0)
        return (VMINT)vm_mutex_lock;
    if(strcmp(func_name,"vm_mutex_unlock") == 0)
        return (VMINT)vm_mutex_unlock;
    if(strcmp(func_name,"vm_thread_sleep") == 0)
        return (VMINT)vm_thread_sleep;
    if(strcmp(func_name,"vm_thread_change_priority") == 0)
        return (VMINT)vm_thread_change_priority;        
#endif

#ifdef __MRE_CORE_ATCMD__
    if(strcmp(func_name,"vm_command_open_port") == 0)
        return (VMINT)vm_command_open_port;
    if(strcmp(func_name,"vm_command_close_port") == 0)
        return (VMINT)vm_command_close_port;
    if(strcmp(func_name,"vm_open_command_port") == 0)
        return (VMINT)vm_command_open_port;
    if(strcmp(func_name,"vm_close_command_port") == 0)
        return (VMINT)vm_command_close_port;
    if(strcmp(func_name,"vm_command_reply") == 0)
        return (VMINT)vm_command_reply;
#endif
	
#ifdef __MRE_SAL_VAD__
	if(strcmp(func_name,"vm_vad_switch_on_off") == 0)
    return (VMINT)vm_vad_switch_on_off;
	if(strcmp(func_name,"vm_vad_set_level") == 0)
    return (VMINT)vm_vad_set_level;
	if(strcmp(func_name,"vm_vad_is_on") == 0)
    return (VMINT)vm_vad_is_on;
	if(strcmp(func_name,"vm_vad_get_level") == 0)
    return (VMINT)vm_vad_get_level;
	if(strcmp(func_name,"vm_vad_register_callback") == 0)
    return (VMINT)vm_vad_register_callback;
#endif

#ifdef __MRE_CORE_SAFETY__
    if(strcmp(func_name,"vm_ce_auth_app_ex") == 0)
        return (VMINT)vm_ce_auth_app_ex;
#endif

#ifdef __MRE_SAL_GPS__
    if(strcmp(func_name,"vm_gps_set_mode") == 0)
        return (VMINT)vm_gps_set_mode;
    if(strcmp(func_name,"vm_gps_close") == 0)
        return (VMINT)vm_gps_close;
    if(strcmp(func_name,"vm_gps_open") == 0)
        return (VMINT)vm_gps_open;
    if(strcmp(func_name,"vm_gps_set_param") == 0)
        return (VMINT)vm_gps_set_param;
#endif
