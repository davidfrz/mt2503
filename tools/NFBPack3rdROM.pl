...
my %EnfbInputFiles = (
"ENFBMMIResB" => "plutommi\\Customer\\CustResource\\CustMMI3rdROM",
"CustENFBThm" => "plutommi\\Customer\\ResGenerator\\ThemeXML\\Temp\\CustEnfbVenusThemeData",
"CustENFBIcn" => "plutommi\\Customer\\ResGenerator\\ThemeXML\\Temp\\CustEnfbVenusAppIconData",
"IME_HW_DATA" => "plutommi\\Customer\\ResGenerator\\hw_data.gbd",
"CustENFBFnt" => "plutommi\\Customer\\CustResource\\CustENFBFontData",
);

my @MREInputFileContentArray = split "\n",`type "plutommi\\Customer\\CustResource\\NFBpack3rdromCust.txt"`;
foreach my $line (@MREInputFileContentArray)
{
	if($line =~ /"(.*)"=>"(.*)"/)
	{
		$EnfbInputFiles{$1} = $2;
...
foreach my $k (keys %EnfbInputFiles){printf "$k=>$EnfbInputFiles{$k}\n";}

my $EnfbReservedHearderSize = 3240;
...
#   my (@ID, $cid, $begin, $length, @Desc) = unpack "C11CVVC12", $buffer;
   my @buf = unpack "C11CVVC11", $buffer;
   my $cid = $buf[11];
   my $begin = $buf[12];
   my $length = $buf[13];
   my $key = pack "C11", splice(@buf, 14, 11);
...
{
	syswrite(HANDLE,"0",1);
	$cur_pos = sysseek(HANDLE, 0, 2);
	print "Tail position after padding : $cur_pos\n";
}
