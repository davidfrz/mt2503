...
/*search filter */
srv_phb_qsearch_filter_struct *qfilter = NULL;
        ...
        mmi_phb_get_index_array_funcptr_type get_index_array_cb)
{
    ...
    switch (vcard_storage)
    {
        case PHB_STORAGE_NVRAM_PB:
            ...
            if (qfilter)
            {
                srv_phb_qsearch_filter_struct qsearch;

                memcpy(&qsearch, qfilter, sizeof(srv_phb_qsearch_filter_struct));
                filter.qsearch_filter = &qsearch;

                srv_phb_mem_free(qfilter);
                qfilter = NULL;
            }
            ...
            cnt = (owner + count) < (index_count) ? (owner + count) : (index_count);
            get_index_array_cb(&error, index_array, cnt);
        ...
        case PHB_STORAGE_SIM1_PB:
            ...
            //get_index_array_cb(&error, index_array, index_count);
            cnt = (count) < (index_count) ? (count) : (index_count);
            get_index_array_cb(&error, index_array, cnt);
        ...
        U16 len)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    qfilter = (srv_phb_qsearch_filter_struct *) srv_phb_mem_malloc(sizeof(srv_phb_qsearch_filter_struct), SRV_PHB_MEMORY_TYPE_CTR);

    qfilter->input_mode = IMM_INPUT_MODE_QUICK_SEARCH_PINYIN;
    qfilter->key_word = key;
    qfilter->key_len = len;

    mmi_phb_get_index_array(vcard_storage,
                            sort_order,
                            index_array,
                            index_array_size,
                            get_index_array_cb);
