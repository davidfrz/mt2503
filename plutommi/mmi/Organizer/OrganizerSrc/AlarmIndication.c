...
#if defined(__MMI_WEARABLE_DEVICE_ALARM__)
...
{	
  ...
  #elif defined(__MMI_MAINLCD_240X320__)
  	gdi_image_draw_id(74, 169 * 4/3, IMG_ALM_ARROW_STOP1+current_stop_frame);
  #elif defined(__MMI_MAINLCD_320X320__)
 ...
 #else
    //new style;
  #if defined(__MMI_MAINLCD_240X240__) //|| defined(__MMI_MAINLCD_240X320__)
    gdi_layer_blt_previous(
      	74, 169,
      	74 + 42,
      	169 + 12);
...
{	
  ...
  #elif defined(__MMI_MAINLCD_240X320__)
    gdi_image_draw_id(124, 187 * 4/3, IMG_ALM_ARROW_SNOOZE1+current_snooze_frame);
  #elif defined(__MMI_MAINLCD_320X320__)
...
static void mmi_alm_indication_down(mmi_pen_point_struct pos)
{
  ...
  #elif defined(__MMI_MAINLCD_240X320__)
    if(!(((down.x < 15 + 57) && (down.x > 15)) && ((down.y < 155 * 4/3 + 57) && (down.y > 155 * 4/3)))
		&& !(((down.x < 168 + 57) && (down.x > 168)) && ((down.y < 155 * 4/3 + 57) && (down.y > 155 * 4/3))))
  #elif defined(__MMI_MAINLCD_320X320__)
    //for 320x320;
	...
	*/
  ...
  #elif defined(__MMI_MAINLCD_240X320__)
    gdi_layer_push_and_set_clip(15, 155 * 4/3, 168 + 57, 155 * 4/3 + 57);
  #elif defined(__MMI_MAINLCD_320X320__)
    ...
    if(((down.x < 15 + 57) && (down.x > 15)) && ((down.y < 155 * 4/3 + 57) && (down.y > 155 * 4/3)))
	{
		gdi_image_draw_id(15, 155 * 4/3, IMG_ALM_STOP_PRESSED);
		gdi_image_draw_id(74, 169 * 4/3, IMG_ALM_ARROW_STOP1);//

		gui_measure_string((UI_string_type)GetString(STR_ID_ALM_STOP_ALARM), &str_w, &str_h); 

        //slide to stop;
		gui_set_font(&MMI_small_font); 
		gui_move_text_cursor(74, 169 * 4/3 + 12 +10);//interval 10;
		gui_set_text_color(gui_color(126, 126, 126));
		gui_print_text((UI_string_type) GetString(STR_ID_ALM_STOP_ALARM));
	}
	if(((down.x < 168 + 57) && (down.x > 168)) && ((down.y < 155 * 4/3 + 57) && (down.y > 155 * 4/3)))
	{
		gdi_image_draw_id(168, 155 * 4/3, IMG_ALM_SNOOZE_PRESSED);
		gdi_image_draw_id(124, 187* 4/3, IMG_ALM_ARROW_SNOOZE1);//(124,187)	

		gui_measure_string((UI_string_type)GetString(STR_ID_ALM_SNOOZE_ALARM), &str_w, &str_h); 
		
		//slide to snooze;
		gui_set_font(&MMI_small_font); 
		gui_move_text_cursor(124+42-str_w, 187 * 4/3 - 10 - str_h);//right align;
		gui_set_text_color(gui_color(126, 126, 126));
		gui_print_text((UI_string_type) GetString(STR_ID_ALM_SNOOZE_ALARM));	
	}
  #elif defined(__MMI_MAINLCD_320X320__)
  ...
  #elif defined(__MMI_MAINLCD_240X320__)
    gdi_layer_blt_previous(15, 155 * 4/3, 168 + 57, 155 * 4/3 + 57);
  #elif defined(__MMI_MAINLCD_320X320__)
...
static void mmi_alm_indication_move(mmi_pen_point_struct pos)
{
    ...
    if(((down.x < 15 + 57) && (down.x > 15)) && ((down.y < 155 * 4/3 + 57) && (down.y > 155 * 4/3)))
	{
		if(((pos.x < 168 + 57) && (pos.x > 15)) && ((pos.y < 155 * 4/3 + 57) && (pos.y > 155 * 4/3)))
		{
			gdi_layer_push_and_set_active(swatch_alarm_indication_cell.cell_layer_handler);
			gdi_layer_lock_frame_buffer();
...
static void mmi_alm_indication_up(mmi_pen_point_struct pos)
{
	...
	if(((down.x < 15 + 57) && (down.x > 15)) && ((down.y < 155 * 4/3 + 57) && (down.y > 155 * 4/3)))
	{
		...
		else
		{
			...
			//stop icon
			gdi_image_draw_id(15, 155 * 4/3, IMG_ALM_STOP_NORMAL);
			gdi_image_draw_id(74, 169 * 4/3, IMG_ALM_ARROW_STOP1);
			if (!IsMyTimerExist(ALARM_TIMER_WAIT_FOR_STOP_TRIANGLE))
			{
				StartTimer(ALARM_TIMER_WAIT_FOR_STOP_TRIANGLE, ALM_TRIANGLE_DURATION, mmi_alm_stop_triangle_icon);
			...
			//snooze icon
			gdi_image_draw_id(168, 155 * 4/3, IMG_ALM_SNOOZE_NORMAL);
			gdi_image_draw_id(124, 187 * 4/3, IMG_ALM_ARROW_SNOOZE1);
			if (!IsMyTimerExist(ALARM_TIMER_WAIT_FOR_SNOOZE_TRIANGLE))
			{
				StartTimer(ALARM_TIMER_WAIT_FOR_SNOOZE_TRIANGLE, ALM_TRIANGLE_DURATION, mmi_alm_snooze_triangle_icon);
			}
	...
	//snooze handle case;
	else if(((down.x < 168 + 57) && (down.x > 168)) && ((down.y < 155 * 4/3 + 57) && (down.y > 155 * 4/3)))
	{
		//if((pos.x < down.x-10) && (pos.y < 224) && (pos.y > 163))
...
static void mmi_alm_entry_watch_indication_screen(void)
{
	...
	*/
      ...
      #elif defined(__MMI_MAINLCD_240X320__)
	    gdi_image_draw_id(29, 63* 4/3 + (55*4/3 -55)/2, IMG_ALM_TIME0+per_time[0]);
		gdi_image_draw_id(67, 63* 4/3 + (55*4/3 -55)/2, IMG_ALM_TIME0+per_time[1]);
		gdi_image_draw_id(105, 63* 4/3 + (55*4/3 -55)/2, IMG_ALM_TIMECOLON);
		gdi_image_draw_id(128, 63* 4/3 + (55*4/3 -55)/2, IMG_ALM_TIME0+per_time[2]);
		gdi_image_draw_id(166, 63* 4/3 + (55*4/3 -55)/2, IMG_ALM_TIME0+per_time[3]);
	  #elif defined(__MMI_MAINLCD_320X320__)
	...
	*/
		...
		gdi_image_draw_id(166, 63, IMG_ALM_TIME0+per_time[3]);
      #elif defined(__MMI_MAINLCD_240X320__)
	    gdi_image_draw_id(29, 63 + (55*4/3 -55)/2, IMG_ALM_TIME0+per_time[0]);
		gdi_image_draw_id(67, 63 + (55*4/3 -55)/2, IMG_ALM_TIME0+per_time[1]);
		gdi_image_draw_id(105, 63 + (55*4/3 -55)/2, IMG_ALM_TIMECOLON);
		gdi_image_draw_id(128, 63 + (55*4/3 -55)/2, IMG_ALM_TIME0+per_time[2]);
		gdi_image_draw_id(166, 63 + (55*4/3 -55)/2, IMG_ALM_TIME0+per_time[3]);
	  #elif defined(__MMI_MAINLCD_320X320__)
      ...
      #elif defined(__MMI_MAINLCD_240X320__)
	    gui_move_text_cursor(204, 63 * 4/3);
	  #elif defined(__MMI_MAINLCD_320X320__)
    ...
    gdi_image_draw_id(124, 187, IMG_ALM_ARROW_SNOOZE1);//(124,187)
  #elif defined(__MMI_MAINLCD_240X320__)
    gdi_image_draw_id(168, 155 * 4/3, IMG_ALM_SNOOZE_NORMAL);//(168,155)
    gdi_image_draw_id(124, 187 * 4/3, IMG_ALM_ARROW_SNOOZE1);//(124,187)
  #elif defined(__MMI_MAINLCD_320X320__)
...
void AlmStopHandler(void)
{
	 ...
	 kal_prompt_trace(MOD_MMI_COMMON_APP, "[alarm]AlmStopHandler().");
}
...
/* under construction !*/
   ...
   #ifdef __MMI_BLE_PXP_SUPPORT__			
			if(MMI_FALSE == s_pxp_is_vibrating)
   #endif
   			{      
                srv_vibrator_off();
				kal_prompt_trace(MOD_MMI_COMMON_APP, "mmi_alm_stop_vibrate(),to srv_vibrator_off().");
		    }
