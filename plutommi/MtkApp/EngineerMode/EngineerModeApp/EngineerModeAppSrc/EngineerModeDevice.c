...
#define SRV_BT_KEYBOARD_MINOR_DEVICE_MASK     (0x00000040) 

#if defined(__BT_OPP_PROFILE__) || defined(__BT_FTS_PROFILE__) || defined(__BT_FTC_PROFILE__)
#ifdef __BT_DIALER_SUPPORT__
#define MMI_EM_BT_HID_TEST_COD  (SRV_BT_NETWORKING_MAJOR_SERVICE_MASK | SRV_BT_CAPTURING_MAJOR_SERVICE_MASK | \
                         		SRV_BT_OBJECT_TRANSFER_MAJOR_SERVICE_MASK |            \
                         		SRV_BT_TELEPHONY_MAJOR_SERVICE_MASK|                  \
                         		SRV_BT_AUDIO_MAJOR_SERVICE_MASK |                      \
                         		SRV_BT_PERIPHERAL_MAJOR_DEVICE_MASK |                \
                         		SRV_BT_KEYBOARD_MINOR_DEVICE_MASK)            
#else  /* __BT_DIALER_SUPPORT__ */   
#define MMI_EM_BT_HID_TEST_COD  (SRV_BT_NETWORKING_MAJOR_SERVICE_MASK | SRV_BT_CAPTURING_MAJOR_SERVICE_MASK| \
                         		SRV_BT_OBJECT_TRANSFER_MAJOR_SERVICE_MASK |            \
                         		SRV_BT_TELEPHONY_MAJOR_SERVICE_MASK |                  \
                         		SRV_BT_PERIPHERAL_MAJOR_DEVICE_MASK |                \
...
static U8 g_em_bt_hid_test_flag = 0;
#endif
...
/*-----------------------------------------------------------------------------*/
/*------------------------------- Device -> HID TEST MODE ---------------------*/
/*-----------------------------------------------------------------------------*/
#ifdef __MMI_HIDD_SUPPORT__

void mmi_em_bt_hidd_init()
{
	S16 error;
	
    ReadValue(BT_EM_HIDD_TEST, &g_em_bt_hid_test_flag, DS_BYTE, &error);
	
	kal_wap_trace(
		MOD_MMI_CONN_APP,
		TRACE_GROUP_7,
		"[EM_HIDD]mmi_em_init g_em_hid_tes_mode = %d",
		g_em_bt_hid_test_flag);
	
	if(g_em_bt_hid_test_flag)
	{
		Custom_Config_Set_BT_COD(MMI_EM_BT_HID_TEST_COD, (kal_bool)g_em_bt_hid_test_flag);
	}
}

mmi_ret mmi_em_bt_hidd_set_done(mmi_event_struct * evt)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    {  
        case EVT_ID_POPUP_QUIT:
            mmi_frm_group_close(GRP_ID_EM_HIDD_TEST_MODE);
            break;
...
static MMI_BOOL mmi_em_bt_hidd_get_test_flag(void)
{
	S16 error;
    ReadValue(BT_EM_HIDD_TEST, &g_em_bt_hid_test_flag, DS_BYTE, &error);
	...
	kal_wap_trace(
		MOD_MMI_CONN_APP,
		TRACE_GROUP_7,
		"[EM_HIDD]mmi_em_get_hid_test_flag g_em_hid_tes_mode = %d",
		g_em_bt_hid_test_flag);
	return (MMI_BOOL)g_em_bt_hid_test_flag;
}

static void mmi_em_bt_hidd_set_test_flag(U8 hid_flag)
{
	S16 error;
	...
	kal_wap_trace(
		MOD_MMI_CONN_APP,
		TRACE_GROUP_7,
		"[EM_HIDD] mmi_em_set_hid_test_flag() hid_flag = %d g_em_hid_tes_mode =%d", 
		hid_flag, 
		g_em_bt_hid_test_flag);

	if(srv_bt_cm_get_power_status() == SRV_BT_CM_POWER_ON
		|| srv_bt_cm_get_power_status() == SRV_BT_CM_POWER_SWITCHING_ON)
	{

		mmi_popup_display_simple_ext(
			STR_ID_EM_HIDD_TEST_MODE_DONE_INFO, 
			MMI_EVENT_SUCCESS, 
			GRP_ID_EM_HIDD_TEST_MODE, 
			NULL);
		return;
	}
	if(g_em_bt_hid_test_flag == hid_flag)
		return;
	else
		g_em_bt_hid_test_flag = hid_flag;

	if(g_em_bt_hid_test_flag)
	{
		cod = MMI_EM_BT_HID_TEST_COD;
	}
	Custom_Config_Set_BT_COD(cod, g_em_bt_hid_test_flag);
	
    WriteValue(BT_EM_HIDD_TEST, &g_em_bt_hid_test_flag, DS_BYTE, &error);
	
	mmi_popup_property_init(&property);
	property.callback = mmi_em_bt_hidd_set_done;
	property.parent_id = GRP_ID_EM_HIDD_TEST_MODE;
	property.user_tag = NULL;
	mmi_popup_display_ext(STR_GLOBAL_DONE, MMI_EVENT_SUCCESS, &property);
}

static mmi_ret mmi_em_bt_hidd_test_mode_proc(mmi_event_struct *evt)
{
	...
	case EVT_ID_CUI_MENU_LIST_ENTRY:
		if (menu_evt->parent_menu_id == MENU_ID_EM_HIDD_TEST_MODE)
		{
		    set = mmi_em_bt_hidd_get_test_flag();
			if(set == 0)
			{
				highlight_menu = MENU_ID_EM_HIDD_TEST_MODE_OFF;
			}
			else
			{
				highlight_menu = MENU_ID_EM_HIDD_TEST_MODE_ON;
			}
			cui_menu_set_default_title_by_id(menu_evt->sender_id, STR_ID_EM_HIDD_TEST_MODE, NULL);
    ...
    case EVT_ID_CUI_MENU_ITEM_TAP:
		if(menu_evt->highlighted_menu_id == MENU_ID_EM_HIDD_TEST_MODE_ON)
		{
			set = 1;			
		}
		else
		...
		mmi_em_bt_hidd_set_test_flag(set);
		break;
...
static void mmi_em_bt_hidd_test_mode_entey()
{
		
		mmi_frm_group_create_ex(GRP_ID_ROOT, 
			GRP_ID_EM_HIDD_TEST_MODE,
			mmi_em_bt_hidd_test_mode_proc, 
			NULL,
			MMI_FRM_NODE_SMART_CLOSE_FLAG);
...
/* under construction !*/
    #endif 
	#ifdef __MMI_HIDD_SUPPORT__
	 	case MENU_ID_EM_HIDD_TEST_MODE:
			mmi_em_bt_hidd_test_mode_entey();
			break;
	#endif /*__MMI_HIDD_SUPPORT__*/
        default:
            break;
