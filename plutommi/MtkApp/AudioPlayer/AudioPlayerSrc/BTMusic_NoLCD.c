...
MMI_BOOL mmi_bt_music_nolcd_is_sink_streaming(void)
{
	...
	MMI_TRACE(MMI_CONN_TRC_G7_BT, MMI_BT_MUSIC_NOLCD_IS_STREAMING, is_streaming);
	if(is_streaming)
		return MMI_TRUE;
	...
	if(is_streaming)
	{
	    MMI_TRACE(MMI_CONN_TRC_G7_BT, MMI_BT_MUSIC_NOLCD_IS_STREAMING, MMI_TRUE);
	    g_btmusic_nolcd_cntx.bt_address = temp_bt_addr;
		return MMI_TRUE;
	}

    MMI_TRACE(MMI_CONN_TRC_G7_BT, MMI_BT_MUSIC_NOLCD_IS_STREAMING, MMI_FALSE);
    return MMI_FALSE;
}
...
void mmi_bt_music_nolcd_A2DP_cb(kal_int32 result)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    switch(event)
    {
        ...
        case SRV_A2DP_CALLBACK_EVENT_STREAM_START_IND:
        {
            ...
            {
                mmi_bt_music_nolcd_is_sink_streaming(); /* This API will update active bt address */
 ...
 *  void
 *****************************************************************************/
void mmi_bt_music_nolcd_launch(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
	MMI_TRACE(MMI_MEDIA_TRC_G2_APP, MMI_BT_MUSIC_NOLCD_LAUNCH, mmi_bt_music_nolcd_is_active(),srv_bt_cm_is_profile_connected(SRV_BT_CM_A2DP_CONNECTION),1);
 ...
 *  void
 *****************************************************************************/
void mmi_bt_music_nolcd_delaunch(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
	MMI_TRACE(MMI_MEDIA_TRC_G2_APP, MMI_BT_MUSIC_NOLCD_DELAUNCH, mmi_bt_music_nolcd_is_active(),srv_bt_cm_is_profile_connected(SRV_BT_CM_A2DP_CONNECTION));
	if(srv_bt_cm_is_profile_connected(SRV_BT_CM_A2DP_CONNECTION))
	{
	    if(mmi_bt_music_nolcd_check_profile(g_btmusic_nolcd_cntx.bt_address))
		    return; //if Active device A2DP connected, keep in BT Music and do nothing
		else
		{  
		    srv_bt_cm_bt_addr *temp_bt_addr;
...
static void mmi_bt_music_nolcd_karaoke()
{
    ...
    if(g_btmusic_nolcd_cntx.is_karaoke_enable == MMI_FALSE)
    {
		...
		if(mmi_bt_music_nolcd_is_sink_streaming() && srv_a2dp_is_codec_open())
        {
...
mmi_ret mmi_bt_music_nolcd_proc(mmi_event_struct *event)
{
    ...
    switch (event->evt_id)
    {
        ...
        case BTBOX_BT_MUSIC_NOLCD_PLAY_PAUSE:
			if(mmi_bt_music_nolcd_is_active())
                mmi_bt_music_nolcd_karaoke_switch();  //mmi_bt_music_nolcd_play_pause();
            break;
